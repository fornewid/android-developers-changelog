---
title: Android 10 features and APIs  |  Android Developers
...
# Android 10 features and APIs Stay organized with collections Save and categorize content based on your preferences.
...
[API diff report](/sdk/api_diff/29/changes) or visit the
[Android API reference](/reference) — look for APIs that were "added in
API level 29". Also be sure to check out Android 10 behavior
changes (for [apps targeting API level 29](/about/versions/10/behavior-changes-10) and for
[all apps](/about/versions/10/behavior-changes-all)), as well as [privacy
changes](/about/versions/10/privacy), to learn about areas where platform changes may
affect your apps.
...
[biometric authentication](/training/sign-in/biometric-auth) support:

* A check for the biometric authentication capability.
* A fallback mechanism that allows a user to authenticate using their device
  PIN, pattern, or password if they cannot authenticate using their biometric
  input.
* A hint that tells the system not to require user confirmation after the user
  has authenticated using an implicit biometric modality. For example, you could
  tell the system that no further confirmation should be required after a user has
  authenticated using face authentication.
...
page](/about/versions/10/behavior-changes-all#tls-1.3).
...
[`android.net.ssl`](/reference/android/net/ssl/package-summary) contain static
...
[`SSLSockets`](/reference/android/net/ssl/SSLSockets).

**Caution:** If you're still accessing TLS functionality using reflection, change
to using the public API. This is because of planned further [restrictions on
non-SDK
...
connectivity](/guide/topics/connectivity/wifi-bootstrap).
...
[Wi-Fi suggest](/guide/topics/connectivity/wifi-suggest).
...
[Wi-Fi Easy Connect](/guide/topics/connectivity/wifi-easy).
...
storage](/training/data-storage/files/external-scoped),
Android 10 adds the following capabilities related to external
storage:

* You can use the [`IS_PENDING`
  flag](/training/data-storage/files/media#pending-media-files) to give your app
  exclusive access to a media file as it's written to disk.
* If you're aware of a specific location where files should be stored, you can
  [provide the system a hint](/training/data-storage/files/media#provide-hint) for
  where to store the newly-written files.
* Each external storage device has a [unique volume
  name](/training/data-storage/files/external#unique-volume-names).
...
For full information, see [Sharing audio input](/guide/topics/media/sharing-audio-input).

### Audio playback capture

Android 10 gives an app the ability to capture audio playback from other apps.
For full information, see [Playback capture](/guide/topics/media/playback-capture).
...
[`PlaybackState.getPosition()`](/reference/android/media/session/PlaybackState#getPosition()),
...
* The seekbar appears if there is an active `MediaSession` and its duration
  (specified by `MediaMetadata.METADATA_KEY_DURATION`) is greater than zero.
  This means that the bar does not appear for indeterminate streams like
  livestreams and radio broadcasts.
* If the session implements `ACTION_SEEK_TO` the user can drag the seekbar to
  control the playback location.
...
For more information, see [Android Native MIDI API](/ndk/guides/audio/midi).

### MediaCodecInfo improvements

Android 10 adds methods to
[`MediaCodecInfo`](/reference/android/media/MediaCodecInfo) that reveal more
information about a codec.

For more information, see [Media codecs](/guide/topics/media/media-codecs).
...
temperature. Apps [register a listener](/reference/android/os/PowerManager#addThermalStatusListener(android.os.PowerManager.OnThermalStatusChangedListener))
in [PowerManager](/reference/android/os/PowerManager), through which the system
...
* Introduction of MONO and NIR CFA enumerations to distinguish between regular
  monochrome camera and near infrared cameras.
...
[Dynamic Depth Format](/static/training/camera2/Dynamic-depth-v1.0.pdf).
...
[HEIC](/reference/android/graphics/ImageFormat#HEIC).
...
* [`isSessionConfigurationSupported(SessionConfiguration sessionConfig)`](/reference/android/hardware/camera2/CameraDevice#isSessionConfigurationSupported(android.hardware.camera2.params.SessionConfiguration))—enables you to query whether or not the passed session
  configuration can be used to create a camera capture session.
* [`LOGICAL_MULTI_CAMERA_ACTIVE_PHYSICAL_ID`](/reference/android/hardware/camera2/CaptureResult#LOGICAL_MULTI_CAMERA_ACTIVE_PHYSICAL_ID)—enables
...
[`isTextEntryKey()`](/reference/android/view/accessibility/AccessibilityNodeInfo#isTextEntryKey())
...
When the [gesture navigation](/guide/navigation/gesturenav) feature is enabled
...
physical keyboard by pressing `Control+Alt+Z`.
...
[`getRecommendedTimeoutMillis()`](/reference/android/view/accessibility/AccessibilityManager#getRecommendedTimeoutMillis(int,%20int))
...
You can use the
[`FillRequest.FLAG_COMPATIBILITY_MODE_REQUEST`](/reference/android/service/autofill/FillRequest#FLAG_COMPATIBILITY_MODE_REQUEST)
...
The
[`UserData.Builder`](/reference/android/service/autofill/UserData.Builder)
...
When using
[`UserData.Builder`](/reference/android/service/autofill/UserData.Builder) in
...
Android 10 adds
[`FillResponse.setUserData()`](/reference/android/service/autofill/FillResponse#setUserData(android.service.autofill.UserData)),
...
[`SurfaceControl`](/reference/android/view/SurfaceControl) API
...
[SDK](/reference/android/view/SurfaceControl) and NDK bindings.
...
Android 10 introduces the
[`WebViewRenderProcessClient`](/reference/android/webkit/WebViewRenderProcessClient)
abstract class, which apps can use to detect if a
[`WebView`](/reference/android/webkit/WebView) has become unresponsive. To
use this class:

1. Define your own subclass and implement its
   [`onRenderProcessResponsive()`](/reference/android/webkit/WebViewRenderProcessClient#onRenderProcessResponsive(android.webkit.WebView,%20android.webkit.WebViewRenderProcess))
   and
   [`onRenderProcessUnresponsive()`](/reference/android/webkit/WebViewRenderProcessClient#onRenderProcessUnresponsive(android.webkit.WebView,%20android.webkit.WebViewRenderProcess))
   methods.
2. Attach an instance of your `WebViewRenderProcessClient` to one or more
   `WebView` objects.
3. If the `WebView` becomes unresponsive, the system calls the
   client's `onRenderProcessUnresponsive()` method, passing the `WebView` and
   `WebViewRenderProcess`. (If the `WebView` is single-process, the `WebViewRenderProcess`
   parameter is null.) Your app can take appropriate action, such as showing a
   dialog box to the user asking if they want to halt the rendering process.
...
![](/static/images/about/versions/10/settings-wifi-part1.png)


**Figure 1.** The user tries to open a web page while the device
is not connected to the network. Chrome pops up the **Internet Connectivity**
settings panel...

![](/static/images/about/versions/10/settings-wifi-part2.png)


**Figure 2.** The user can turn on Wi-Fi and select a network
without leaving the Chrome app.
...
[`Settings.Panel`](/reference/android/provider/Settings.Panel) actions:
...
[`ACTION_INTERNET_CONNECTIVITY`](/reference/android/provider/Settings.Panel#ACTION_INTERNET_CONNECTIVITY)
:   Shows settings related to internet connectivity, such as Airplane mode, Wi-Fi,
    and Mobile Data.

[`ACTION_WIFI`](/reference/android/provider/Settings.Panel#ACTION_WIFI)
...
[`ACTION_NFC`](/reference/android/provider/Settings.Panel#ACTION_NFC)
:   Shows all settings related to near-field communication *(NFC).*

[`ACTION_VOLUME`](/reference/android/provider/Settings.Panel#ACTION_VOLUME)
...
The [Sharing Shortcuts API](/training/sharing/receive#sharing-shortcuts-api)
replaces the [Direct Share
APIs](/about/versions/marshmallow/android-6.0#direct-share).
...
the [`ShortcutManager`](/guide/topics/ui/shortcuts/managing-shortcuts) works.
Since the two APIs are similar, we have expanded the [`ShortcutInfo`
API](/reference/android/content/pm/ShortcutInfo) to make using both
...
[`ShortcutInfo.Builder`](/reference/android/content/pm/ShortcutInfo.Builder)
...
See [Publish direct share targets](/training/sharing/receive#publish-direct-share-targets).
...
See [Adding rich text previews](/training/sharing/send#adding-rich-content-previews)

#### Learn more

For more information on how apps can share data,
see [Sending simple data to other apps](/training/sharing/send) and
...
see [Dark theme](/guide/topics/ui/look-and-feel/darktheme).

## Foreground service types

Android 10 introduces the
[`foregroundServiceType`](/reference/android/R.attr#foregroundServiceType)
...
| `location` [Continue a user-initiated action](/training/location/request-updates#continuation-user-initiated-action) | |
...
[on-device system trace](/topic/performance/tracing/on-device). When you specify
...
[`TextClassifier`](/reference/android/view/textclassifier/TextClassifier)
interface.

### Language detection

The
[`detectLanguage()`](/reference/android/view/textclassifier/TextClassifier#detectLanguage(android.view.textclassifier.TextLanguage.Request))
method works similarly to previously-existing classification methods. It
receives a
[`TextLanguage.Request`](/reference/android/view/textclassifier/TextLanguage.Request)
object and returns a
[`TextLanguage`](/reference/android/view/textclassifier/TextLanguage)
object.
...
The
[`suggestConversationActions()`](/reference/android/view/textclassifier/TextClassifier#suggestConversationActions(android.view.textclassifier.ConversationActions.Request))
method works similarly to existing classification methods. It receives a
[`ConversationActions.Request`](/reference/android/view/textclassifier/ConversationActions.Request)
object and returns a
[`ConversationActions`](/reference/android/view/textclassifier/ConversationActions)
object.

[`ConversationActions`](/reference/android/view/textclassifier/ConversationActions)
objects consist of a list of
[`ConversationAction`](/reference/android/view/textclassifier/ConversationAction)
...
Android 9 introduced the ability to display suggested replies within a
notification. Android 10 expands on this with the ability to
include suggested intent-based actions. Furthermore, the platform is able to
generate these suggestions automatically. Apps can still provide their own
suggestions, or opt out of system-generated suggestions.

The API used to generate these replies is part of
[`TextClassifier`](/reference/android/view/textclassifier/TextClassifier),
and has also been directly exposed to developers in Android 10.
Please read [the section on TextClassifier improvements](#textclass)
...
[`setAllowGeneratedReplies()`](/reference/android/app/Notification.Action.Builder#setAllowGeneratedReplies(boolean))
and
