---
title: Behavior changes: Apps targeting Android 13 or higher  |  Android Developers
...
# Behavior changes: Apps targeting Android 13 or higher Stay organized with collections Save and categorize content based on your preferences.
...
running on Android 13](/about/versions/13/behavior-changes-all).
...
[notification drawer](/guide/topics/ui/notifiers/notifications#appearances).
However, users still see notices related to foreground services in the
[Task Manager](/about/versions/13/changes/fgs-manager),
...
[`NEARBY_DEVICES`](/reference/android/Manifest.permission_group#NEARBY_DEVICES)
...
location](/guide/topics/connectivity/bluetooth/permissions#assert-never-for-location).

**Note:** This change affects your app only if you call a Wi-Fi API. View the
[list of affected APIs](/guide/topics/connectivity/wifi-permissions#check-for-apis-that-require-permission).

Learn more about how to
[request permission to access nearby Wi-Fi devices](/guide/topics/connectivity/wifi-permissions).

### Granular media permissions


![The 2 buttons for the dialog, from top to bottom, are Allow and Don't
  allow](/static/images/about/versions/13/storage-permission-split.svg)


**Figure 1.** System permissions dialog that the user sees when
you request the `READ_MEDIA_AUDIO` permission.

If your app targets Android 13 or higher and needs to
[access media files that other apps have
created](/training/data-storage/shared/media#storage-permission), you must
...
| --- | --- |
| Images and photos | [`READ_MEDIA_IMAGES`](/reference/android/Manifest.permission#READ_MEDIA_IMAGES) |
| Videos | [`READ_MEDIA_VIDEO`](/reference/android/Manifest.permission#READ_MEDIA_VIDEO) |
| Audio files | [`READ_MEDIA_AUDIO`](/reference/android/Manifest.permission#READ_MEDIA_AUDIO) |
...
**Note:** If your app only needs to access images, photos, and videos, consider
using the [photo picker](/about/versions/13/features/photopicker) instead of
declaring the `READ_MEDIA_IMAGES` and `READ_MEDIA_VIDEO` permissions.
...
in Android 10 (API level 29)](/about/versions/10/privacy/changes#app-access-device-location).
...
**Note:** This is a "hard-restricted" permission that cannot be held by an app
until the device's installer allowlists the permission for your app. For more
details, see this guide on
[restricted permissions](https://android.googlesource.com/platform/frameworks/base/+/master/core/java/android/permission/Permissions.md#restricted-permissions).
...
["restricted" state](/topic/performance/background-optimization#bg-restrict) for
...
media controls from
[`PlaybackState`](/reference/android/media/session/PlaybackState) actions. This
...
![Media controls in terms of how they appear on phone and tablets devices,
            using an example of a sample track showing how the buttons may appear](/static/images/about/versions/13/media-controls-phone-tablet.png)
...
notification in the order in which they were [added](/reference/android/app/Notification.Builder#addAction(android.app.Notification.Action)).
In compact mode—for example, in the collapsed quick settings—up to
three actions specified with [`setShowActionsInCompactView()`](/reference/androidx/media/app/NotificationCompat.MediaStyle#setShowActionsInCompactView(int...))
were shown.
...
|  |  |  |
| --- | --- | --- |
| 1 | Play | Current [state](/reference/android/media/session/PlaybackState#getState()) of the `PlaybackState` is one of the following:  * `STATE_NONE` * `STATE_STOPPED` * `STATE_PAUSED` * `STATE_ERROR` |
...
| Custom | `PlaybackState` [actions](/reference/android/media/session/PlaybackState#getActions()) do not include `ACTION_SKIP_TO_PREVIOUS` and `PlaybackState` [custom actions](/reference/android/media/session/PlaybackState#getCustomActions()) include a custom action that hasn't been placed yet. |
...
| Custom | `PlaybackState` [actions](/reference/android/media/session/PlaybackState#getActions()) do not include `ACTION_SKIP_TO_NEXT` and `PlaybackState` [custom actions](/reference/android/media/session/PlaybackState#getCustomActions()) include a custom action that hasn't been placed yet. |
...
[`setForceDark()`](/reference/android/webkit/WebSettings#setForceDark(int))
...
[`isLightTheme`](/reference/android/R.styleable#Theme_isLightTheme). In other
...
[`setAlgorithmicDarkeningAllowed()`](/reference/android/webkit/WebSettings#setAlgorithmicDarkeningAllowed(boolean))
method instead. For backward compatibility with previous Android versions, we
recommend using the equivalent
[`setAlgorithmicDarkeningAllowed()`](/reference/androidx/webkit/WebSettingsCompat#setAlgorithmicDarkeningAllowed(android.webkit.WebSettings,%20boolean))
method in AndroidX.
...
[`targetSdkVersion`](/guide/topics/manifest/uses-sdk-element#target) and theme
settings.
...
[`BluetoothAdapter#disable()`](/reference/android/bluetooth/BluetoothAdapter#disable()) methods are deprecated and always
return `false`.
...
non-SDK interface restrictions in Android 13](/about/versions/13/changes/non-sdk-13).
