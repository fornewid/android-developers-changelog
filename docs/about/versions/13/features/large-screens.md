---
title: Tablet and large screen support  |  Android Developers
...
# Tablet and large screen support Stay organized with collections Save and categorize content based on your preferences.
...
device](/about/versions/13/get#large-screen-avd) for the Android Emulator in
Android Studio or by getting Android 13 on a large screen device from our
[device-maker partners](#large-screen-partners).

If you're just getting started with large screens development, check out [Large
Screens and Foldables](/large-screens) guides to read the latest guidance and
techniques for building great experiences on these devices.

Here's a look at what's new and [some ideas about what to test](#what-to-test)
in your app.
...
![](/static/images/about/versions/13/13-split-screen.png)


Large screen layouts display more
...
[`centerInside`](/reference/android/widget/ImageView#attr_android:scaleType)
option of `ImageView`.
...
screens](/guide/topics/large-screens/media-projection-large-screens).
...
![](/static/images/about/versions/13/large-screen-multitasking.png)
](/images/about/versions/13/large-screen-multitasking.png)](/static/images/about/versions/13/large-screen-multitasking.mp4)


The taskbar lets users launch and switch apps faster, drag and
drop to enter split-screen mode, and swipe up to go back home.
...
mode](/about/versions/12/behavior-changes-all#multi-window-mode), regardless of
...
you [test your apps in split-screen mode](/develop/ui/compose/layouts/adaptive/support-multi-window-mode#api_level_31_or_higher).
...
to call [`setOnApplyWindowInsetsListener`](/reference/androidx/core/view/ViewCompat#setOnApplyWindowInsetsListener(android.view.View,androidx.core.view.OnApplyWindowInsetsListener))
and handle changes in the inset's dimensions as described in
[System bars insets](/training/gestures/edge-to-edge#system-bars-insets).

**Note:** Although the Taskbar doesn't have its own inset type, its dimension can be
retrieved using [`WindowInsetsCompat.Type.navigationBars()`](https://developer.android.com/reference/kotlin/androidx/core/view/WindowInsetsCompat.Type#navigationBars())
or [`WindowInsetsCompat.Type.systemBars()`](https://developer.android.com/reference/kotlin/androidx/core/view/WindowInsetsCompat.Type#systemBars()).

## Improved compatibility experience

[Many apps are already optimized for large screen devices](/large-screens/stories)
...
![](/static/images/about/versions/13/13-compatibility-mode.png)


Compatibility mode improvements provide a better user experience
for apps that are not optimized for large screens.
...
letterboxing](/guide/topics/large-screens/enhanced-letterboxing).
...
dispatching a [`MotionEvent`](/reference/android/view/MotionEvent) with
[`ACTION_CANCEL`](/reference/android/view/MotionEvent#ACTION_CANCEL). Android 13
enhances palm rejection identification by adding
[`FLAG_CANCELED`](/reference/android/view/MotionEvent#FLAG_CANCELED) to the
motion event object.
...
[`ACTION_POINTER_UP`](/reference/android/view/MotionEvent#ACTION_POINTER_UP) and
`FLAG_CANCELED`.
...
quality guidelines](/docs/quality-guidelines/large-screens-app-quality) to
...
embedding](/develop/ui/views/layout/activity-embedding).
...
device (see the
[`screenOrientation`](/reference/android/R.attr#screenOrientation) manifest
...
[device-maker partners](#large-screen-partners). After you finish
...
| --- | --- |
| **Screen sizes, device posture, and rotation**  Check how your app responds when the size of the screen changes due to device posture changes (such as unfolding a foldable device or putting a foldable device in tabletop mode). Also try rotating the device in each of these states to ensure that your app responds optimally. | **Documentation**  * [Support different display sizes](/develop/ui/compose/layouts/adaptive/support-different-display-sizes) * [Make your app fold aware](/develop/ui/compose/layouts/adaptive/foldables/make-your-app-fold-aware)  **Samples**  * [Google I/O sample app](https://github.com/google/iosched) * [Jetnews sample app](https://github.com/android/compose-samples/tree/main/JetNews) * [Jetcaster sample app (tabletop support)](https://github.com/android/compose-samples/tree/main/Jetcaster) * [Trackr sample app](https://github.com/android/trackr)  **Codelabs**  * [Support foldable and dual-screen devices with Jetpack WindowManager](/codelabs/android-window-manager-dual-screen-foldables#0)  **Technical talks**  * [Build Android UIs for any screen size](https://www.youtube.com/watch?v=ir3LztqbeRI) * [Design beautiful apps on foldables and large screens](https://www.youtube.com/watch?v=DJeJIJKOUbI) |
| **Taskbar interaction and split-screen mode**  Check how your app responds when viewed with the taskbar on large screens. Check that your app's UI isn't cut off or blocked by the taskbar, try entering and exiting split-screen and multi-window modes using the taskbar, and test quickly switching between your app and other apps using the taskbar.  If your app has a fixed orientation and is not resizable, check how your app responds to compatibility mode adjustments such as letterboxing. | **Documentation**  * [Multitasking and productivity changes](#multitasking) * [Enhanced letterboxing](/guide/topics/large-screens/enhanced-letterboxing) * [Multi-window support](/guide/topics/ui/multi-window)  **Samples**  * [Google I/O sample app](https://github.com/google/iosched) * [Jetnews sample app](https://github.com/android/compose-samples/tree/main/JetNews) * [Trackr sample app](https://github.com/android/trackr)  **Technical talks**  * [What's new for large screens and foldables in Android and ChromeOS](https://www.youtube.com/watch?v=6-925K3hMHU) * [Build Android UIs for any screen size](https://www.youtube.com/watch?v=ir3LztqbeRI) * [Design beautiful apps on foldables and large screens](https://www.youtube.com/watch?v=DJeJIJKOUbI) |
| **Multi-window mode**  Check how your app responds when running in multi-window mode on large screens (sw >= 600dp), especially if your app sets [`resizeableActivity="false"`](/guide/topics/manifest/application-element#resizeableActivity).  If your app sets [`resizeableActivity="true"`](/guide/topics/manifest/application-element#resizeableActivity), also be sure to check how your app responds when running in multi-window mode on small screens (sw < 600dp).  If your app uses Display APIs such as [`Display.getRealSize()`](/reference/android/view/Display#getRealSize(android.graphics.Point)) and [`Display.getRealMetrics()`](/reference/android/view/Display#getRealMetrics(android.util.DisplayMetrics)), use the `WindowMetrics` APIs in the Jetpack [`WindowManager`](/jetpack/androidx/releases/window) library instead. | **Documentation**  * [Changes to multi-window mode in Android 12](/about/versions/12/behavior-changes-all#multi-window-mode) * [Display API deprecations and WindowMetrics updates](/about/versions/12/behavior-changes-all#displaymetrics) * [Multi-window support](/guide/topics/ui/multi-window)  **Samples**  * [Google I/O sample app](https://github.com/google/iosched) * [Jetnews sample app](https://github.com/android/compose-samples/tree/main/JetNews) * [Trackr sample app](https://github.com/android/trackr)  **Technical talks**  * [What's new for large screens and foldables in Android and ChromeOS](https://www.youtube.com/watch?v=6-925K3hMHU) * [Build Android UIs for any screen size](https://www.youtube.com/watch?v=ir3LztqbeRI) * [Design beautiful apps on foldables and large screens](https://www.youtube.com/watch?v=DJeJIJKOUbI) |
| **Media projection**  If your app uses media projection, check how your app responds while playing back, streaming, or casting media on large screen devices. Be sure to account for device posture changes on foldable devices as well. | **Documentation**  * [Media projection on large screens](/guide/topics/large-screens/media-projection-large-screens) * [Capture video and audio playback](/guide/topics/media/av-capture)  **Samples**  * [Jetcaster sample app (tabletop support)](https://github.com/android/compose-samples/tree/main/Jetcaster)  **Technical talks**  * [Best practices for video apps on foldable devices](https://www.youtube.com/watch?v=DBAek_P0nEw) |
| **Camera preview**  For camera apps, check how your camera preview UI responds on large screens when your app is constrained to a portion of the screen in multi-window or split-screen mode. Also check how your app responds when a foldable device's posture changes. | **Documentation**  * [Changes to camera preview in Android 12](/about/versions/12/behavior-changes-all#camera-preview) * [Camera preview on large screens](/training/camera2/camera-preview-large-screens) |