---
title: Storage updates in Android 11  |  Android Developers
...
# Storage updates in Android 11 Stay organized with collections Save and categorize content based on your preferences.
...
storage](/training/data-storage#scoped-storage), which makes it easier for
developers to fulfill their [storage use
cases](/training/data-storage/use-cases) after they migrate to using this
storage model.
...
changes](/training/data-storage/use-cases#opt-out-scoped-storage) associated
...
the [scoped storage](/training/data-storage#scoped-storage) model is enabled.
Before you target Android 11, [migrate
data](/training/data-storage/use-cases#migrate-legacy-storage) to a directory
...
* [`DEFAULT_SCOPED_STORAGE`](/about/versions/11/reference/compat-framework-changes#default_scoped_storage)
  (enabled for all apps by default)
* [`FORCE_ENABLE_SCOPED_STORAGE`](/about/versions/11/reference/compat-framework-changes#force_enable_scoped_storage)
...
space](/training/data-storage/app-specific#query-free-space).

1. Check for free space by invoking the
   [`ACTION_MANAGE_STORAGE`](/reference/kotlin/android/os/storage/StorageManager#action_manage_storage) intent action.
2. If there isn't enough free space on the device, prompt the user to give your
   ...
   [`ACTION_CLEAR_APP_CACHE`](/reference/kotlin/android/os/storage/StorageManager#action_clear_app_cache) intent action.
...
[app-specific directory on external storage](/training/data-storage/app-specific#external). To
access the directory that the system provides for your app, call
[`getExternalFilesDirs()`](/reference/android/content/Context#getExternalFilesDirs(java.lang.String)).
...
groups of media files](/training/data-storage/shared/media#manage-groups-files).
...
paths](/training/data-storage/shared/media#direct-file-paths). These APIs
...
storage](/training/data-storage/app-specific#internal) world-accessible to other
apps. Apps that target Android 9 or higher [cannot make the files in their data
directories
world-accessible](/about/versions/pie/android-9.0-changes-28#per-app-selinux).
...
directory](/training/data-storage/app-specific#external) within external
storage.
...
[`ACTION_OPEN_DOCUMENT_TREE`](/training/data-storage/shared/documents-files#grant-access-directory)
...
[`ACTION_OPEN_DOCUMENT_TREE`](/training/data-storage/shared/documents-files#grant-access-directory)
or the
[`ACTION_OPEN_DOCUMENT`](/training/data-storage/shared/documents-files#open-file)
...
2. Do one of the following:
   * Enable the
     [`RESTRICT_STORAGE_ACCESS_FRAMEWORK`](/about/versions/11/reference/compat-framework-changes#restrict_storage_access_framework)
...
* If your app hasn't opted out of
  [scoped storage](/training/data-storage#scoped-storage) and
  requests the
  ...
  [shared storage](/training/data-storage/shared) using this app, you
  need to have the [all files
  access](/about/versions/11/privacy/storage#all-files-access) permission.
...
permissions](/training/data-storage/shared/media#request-permissions) when
...
[notice](/training/data-storage/manage-all-files#all-files-access-google-play).
