---
title: Permissions updates in Android 11  |  Android Developers
...
# Permissions updates in Android 11 Stay organized with collections Save and categorize content based on your preferences.
...
permissions](/training/permissions/requesting#one-time).

**Note:** If your app already follows best practices when it [requests runtime
permissions](/training/permissions/requesting), you don't need to change your
app to support one-time permissions.
...
[requesting permissions at runtime](/training/permissions/requesting), you
...
permissions](/guide/topics/permissions/overview#runtime_requests_android_60_and_higher),
which are the permissions that display a runtime prompt to the user when
requested.

Learn more about how the system [auto-resets permissions of unused
apps](/training/permissions/requesting#auto-reset-permissions-unused-apps).
...
permissions](/privacy/best-practices#permissions), you don't need to change your
app to support this behavior change.

Learn more about how to [handle permission
denial](/training/permissions/requesting#handle-denial) in your app.
...
#### Certain apps are automatically granted SYSTEM\_ALERT\_WINDOW permission upon request

Certain classes of apps are automatically granted the `SYSTEM_ALERT_WINDOW`
permission upon request:

* Any app that has
  [`ROLE_CALL_SCREENING`](/reference/android/app/role/RoleManager#ROLE_CALL_SCREENING)
...
#### MANAGE\_OVERLAY\_PERMISSION intents always bring user to system permissions screen
...
[`READ_PHONE_NUMBERS`](/reference/kotlin/android/Manifest.permission#read_phone_numbers)
permission, instead of the `READ_PHONE_STATE` permission.

* The `getLine1Number()` method in both the
  [`TelephonyManager`](/reference/kotlin/android/telephony/TelephonyManager#getline1number)
  class and the
  [`TelecomManager`](/reference/kotlin/android/telecom/TelecomManager#getline1number)
  class.
* The unsupported `getMsisdn()` method in the
  [`TelephonyManager`](/reference/kotlin/android/telephony/TelephonyManager)
  class.
...
1. Change your declaration of `READ_PHONE_STATE` so that your app uses the
   permission only on Android 10 (API level 29) and lower.
