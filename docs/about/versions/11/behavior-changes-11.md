---
title: Behavior changes: Apps targeting Android 11  |  Android Developers
...
# Behavior changes: Apps targeting Android 11 Stay organized with collections Save and categorize content based on your preferences.
...
running on Android 11](/about/versions/11/behavior-changes-all).
...
* **[Scoped storage enforcement](/about/versions/11/privacy/storage#scoped-storage):**
...
* **[Permissions auto-reset](/about/versions/11/privacy/permissions#auto-reset):** If
  users haven't interacted with an app for a few months, the system auto-resets
  the app's sensitive permissions.
* **[Background location
  access](/about/versions/11/privacy/location#background-location):** Users must be directed
  to system settings in order to grant the background location permission to apps.
* **[Package visibility](/about/versions/11/privacy/package-visibility):** When an app
  queries for the list of installed apps on the device, the returned list is
  filtered.

To learn more, see the [Privacy](/about/versions/11/privacy) page.
...
To disable this feature, see the [`allowNativeHeapPointerTagging`](/guide/topics/manifest/application-element#allowNativeHeapPointerTagging)
...
[`Toast.makeText()`](/reference/kotlin/android/widget/Toast#maketext) that
don't call [`setView()`](/reference/kotlin/android/widget/Toast#setview).
...
use the
[`addCallback()`](/reference/android/widget/Toast#addCallback(android.widget.Toast.Callback))
...
* The [`getView()`](/reference/kotlin/android/widget/Toast#getview) method
  returns `null`.
* The return values of the following methods don't reflect the actual values, so
  you shouldn't rely on them in your app:
  + [`getHorizontalMargin()`](/reference/android/widget/Toast#getHorizontalMargin())
  + [`getVerticalMargin()`](/reference/android/widget/Toast#getVerticalMargin())
  + [`getGravity()`](/reference/android/widget/Toast#getGravity())
  + [`getXOffset()`](/reference/android/widget/Toast#getXOffset())
  + [`getYOffset()`](/reference/android/widget/Toast#getYOffset())
* The following methods are no-ops, so your app shouldn't use them:
  + [`setMargin()`](/reference/android/widget/Toast#setMargin(float,%20float))
  + [`setGravity()`](/reference/android/widget/Toast#setGravity(int,%20int,%20int))
...
[`Manifest.permission.WRITE_APN_SETTINGS`](/reference/android/Manifest.permission#WRITE_APN_SETTINGS)
privileged permission to read or access the [Telephony](/reference/android/provider/Telephony)
...
visibility](/about/versions/11/privacy/package-visibility), apps that target
...
accessibility button](/guide/topics/ui/accessibility/service#button). If you
...
[`accessibilityFlags`](/reference/kotlin/android/accessibilityservice/AccessibilityServiceInfo#android:accessibilityflags)
...
* [`android.media.action.VIDEO_CAPTURE`](/reference/android/provider/MediaStore#ACTION_VIDEO_CAPTURE)
* [`android.media.action.IMAGE_CAPTURE`](/reference/android/provider/MediaStore#ACTION_IMAGE_CAPTURE)
* [`android.media.action.IMAGE_CAPTURE_SECURE`](/reference/android/provider/MediaStore#ACTION_IMAGE_CAPTURE_SECURE)
...
resources](/studio/build/shrink-code#shrink-resources) or other methods to
...
you can use either [Android Studio](/studio/publish/app-signing#sign_release),
or the [`apksigner`](/studio/command-line/apksigner) tool on the command line.
...
WorkManager](/topic/libraries/architecture/workmanager/migrating-fb) and
[Migrating from GCMNetworkManager to
WorkManager](/topic/libraries/architecture/workmanager/migrating-gcm).
...
[`Editor.clear`](/reference/android/content/SharedPreferences.Editor#clear())
is called, a callback is now made to
[`OnSharedPreferenceChangeListener.onSharedPreferenceChanged`](/reference/android/content/SharedPreferences.OnSharedPreferenceChangeListener#onSharedPreferenceChanged(android.content.SharedPreferences,%20java.lang.String))
with a `null` key.
...
non-SDK interface restrictions in Android 11](/about/versions/11/non-sdk-11). To learn more
