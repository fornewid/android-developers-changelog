...
[API diff report](/sdk/api_diff/30/changes). For details on new APIs
...
changes [for apps that target Android R](/about/versions/11/behavior-changes-11)
and [for all apps](/about/versions/11/behavior-changes-all), as well as
[privacy changes](/about/versions/11/privacy).

## New experiences

### Device controls

Android 11 includes a new [`ControlsProviderService`](/reference/android/service/controls/ControlsProviderService) API that
...
information, see [Control external devices](/guide/topics/ui/device-control).
...
For more information, see [media controls](/guide/topics/media/media-controls).
...
[`DisplayCutout`](/reference/android/view/DisplayCutout)`.getSafeInset…()`
...
* Call
  [`DisplayCutout.getWaterfallInsets()`](/reference/android/view/DisplayCutout#getWaterfallInsets())
...
**Note:** If you do not set the window to `LAYOUT_IN_DISPLAY_CUTOUT_MODE_ALWAYS`,
Android displays the window in letterboxed mode, avoiding the notch and
waterfall areas.
...
with [`TYPE_HINGE_ANGLE`](/reference/android/hardware/Sensor#TYPE_HINGE_ANGLE),
and a new
[`SensorEvent`](/reference/android/hardware/SensorEvent) that can monitor
...
See [Foldables](/guide/topics/ui/foldables#hinge_angle).
...
[People and conversations](/guide/topics/ui/conversations).

#### Chat Bubbles

[Bubbles](/guide/topics/ui/bubbles) are now available to developers to
...
[conversation requirements](/guide/topics/ui/conversations). Specifically,
...
* The [`BubbleMetadata.Builder()`](/reference/android/app/Notification.BubbleMetadata.Builder#Notification.BubbleMetadata.Builder())
  constructor with no parameters is deprecated. Instead, use either of the two new
  constructors [`BubbleMetadata.Builder(PendingIntent, Icon)`](/reference/android/app/Notification.BubbleMetadata.Builder#Notification.BubbleMetadata.Builder(android.app.PendingIntent,%20android.graphics.drawable.Icon))
  or [`BubbleMetadata.Builder(String)`](/reference/android/app/Notification.BubbleMetadata.Builder#Notification.BubbleMetadata.Builder(java.lang.String)).
* Create [`BubbleMetadata`](/reference/android/app/Notification.BubbleMetadata)
  from a shortcut ID by calling `BubbleMetadata.Builder(String)`.
  The string passed should match the shortcut ID provided to
  [`Notification.Builder`](/reference/android/app/Notification.Builder).
* Create bubble icons with
  [`Icon.createWithContentUri()`](/reference/android/graphics/drawable/Icon#createWithContentUri(java.lang.String)),
  or with the new method
  [`createWithAdaptiveBitmapContentUri()`](/reference/android/graphics/drawable/Icon#createWithAdaptiveBitmapContentUri(java.lang.String)).

### 5G visual indicators

For information on displaying 5G indicators on users' devices, see
[Tell your users when they're on
5G](/training/connectivity/5g/enhance-with-5g#indicator).

## Privacy

Android 11 introduces a large number of changes and
restrictions to enhance user privacy. To learn more, see the
[Privacy](/about/versions/11/privacy) page.
...
library](/jetpack/androidx/releases/biometric).

#### Authentication types

Android 11 introduces the
[`BiometricManager.Authenticators`](/reference/kotlin/android/hardware/biometrics/BiometricManager.Authenticators)
interface, which you can use to [declare the types of authentication that your
app supports](/training/sign-in/biometric-auth#declare-supported-authentication-types).
...
[`getAuthenticationType()`](/reference/kotlin/android/hardware/biometrics/BiometricPrompt.AuthenticationResult#getauthenticationtype).

#### Additional support for auth-per-use keys

Android 11 provides more support for [authentication using
auth-per-use keys](/training/sign-in/biometric-auth#auth-per-use-keys).
...
[in-depth guide on sharing large datasets](/about/versions/11/features/shared-datasets).
...
**Note:** This change only affects device restarts that occur because of an OTA
update. If your app always needs access to CE keys before the user enters their
PIN, pattern, or password following a device restart, continue to [support
Direct Boot](/training/articles/direct-boot).
...
[Run apps on a hardware device](/studio/run/device#wireless).
...
latest [SDK Platform-Tools](/studio/releases/platform-tools) installed. If it is
not supported, the default installation method is silently used.

Use the following [adb command](/studio/command-line/adb#incremental) to use the
...
[GWP-Asan guide](/ndk/guides/gwp-asan).

### Neural Networks API 1.3

Android 11 expands and improves the [Neural Networks
API (NNAPI)](/ndk/guides/neuralnetworks).
...
* **QoS API:** The new Quality of Service API includes support for
  ...
  + [`ANeuralNetworksDevice_wait()`](/ndk/reference/group/neural-networks#aneuralnetworksdevice_wait)
...
* **Memory domain input/output:** NNAPI 1.3 includes support for memory domains
  ...
  [`ANeuralNetworksMemoryDesc`](/ndk/reference/group/neural-networks#group___neural_networks_1ga92661bd0904009e8d0c9e8e1bf40bd84)
  and
  [`ANeuralNetworkMemory`](/ndk/reference/group/neural-networks#group___neural_networks_1ga9a6b7719f0613ba9e2c93cffd97ebfc0)
  ...
  + [`ANeuralNetworksMemoryDesc_free()`](/ndk/reference/group/neural-networks#aneuralnetworksmemorydesc_free)
...
* **Control flow:** NNAPI 1.3 includes support for general control flow with the
  ...
  + [`ANeuralNetworksModel_setOperandValueFromModel()`](/ndk/reference/group/neural-networks#aneuralnetworksmodel_setoperandvaluefrommodel)
  + [`ANeuralNetworks_getDefaultLoopTimeout()`](/ndk/reference/group/neural-networks#aneuralnetworks_getdefaultlooptimeout)
  + [`ANeuralNetworks_getMaximumLoopTimeout()`](/ndk/reference/group/neural-networks#aneuralnetworks_getmaximumlooptimeout)
  + [`ANeuralNetworksExecution_setLoopTimeout()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_setlooptimeout)
...
Use the [NDK Thermal API](/ndk/reference/group/thermal) in
...
API is similar to the [Java Thermal API](/about/versions/10/features#thermal); you can
...
[`WindowInsetsAnimation.Callback`](/reference/android/view/View#setWindowInsetsAnimationCallback(android.view.WindowInsetsAnimation.Callback))
to [`View.setWindowInsetsAnimationCallback()`](/reference/android/view/View#setWindowInsetsAnimationCallback(android.view.WindowInsetsAnimation.Callback)).
(You can set this listener on *any* view, it doesn't have to be an `EditText`.)
The IME calls your listener's
[`onPrepare()`](/reference/android/view/WindowInsetsAnimation.Callback#onPrepare(android.view.WindowInsetsAnimation))
method, then it calls
[`onStart()`](/reference/android/view/WindowInsetsAnimation.Callback#onStart(android.view.WindowInsetsAnimation,%20android.view.WindowInsetsAnimation.Bounds))
at the beginning of the transition. It then calls
[`onProgress()`](/reference/android/view/WindowInsetsAnimation.Callback#onProgress(android.view.WindowInsets,%20java.util.List%3Candroid.view.WindowInsetsAnimation%3E))
at each progression in the transition. When the transition has finished, the IME calls
[`onEnd()`](/reference/android/view/WindowInsetsAnimation.Callback#onEnd(android.view.WindowInsetsAnimation)).
At any point in the transition, you can find out how much progress the transition has made by calling
[`WindowInsetsAnimation.getFraction()`](/reference/android/view/WindowInsetsAnimation#getFraction()).
...
[`setOnApplyWindowInsetsListener()`](/reference/android/view/View#setOnApplyWindowInsetsListener(android.view.View.OnApplyWindowInsetsListener))
...
[`controlWindowInsetsAnimation()`](/reference/android/view/WindowInsetsController#controlWindowInsetsAnimation(int,%20long,%20android.view.animation.Interpolator,%20android.view.WindowInsetsAnimationControlListener))
method:
...
* Many formatting APIs now support a new return object type that extends
  `FormattedValue`.
* The `LocaleMatcher` API is enhanced with a builder class, support for the
  [`java.util.Locale`](/reference/kotlin/java/util/Locale) type, and a result
  class featuring additional data about a match.
...
* [`MediaCodec.LinearBlock`](/reference/kotlin/android/media/MediaCodec.LinearBlock)
* [`MediaCodec.OutputFrame`](/reference/kotlin/android/media/MediaCodec.OutputFrame)
* [`MediaCodec.QueueRequest`](/reference/kotlin/android/media/MediaCodec.QueueRequest)

##### New methods:

* [`MediaCodec.getQueueRequest()`](/reference/kotlin/android/media/MediaCodec#getQueueRequest(kotlin.Int))
* [`MediaCodec.getOutputFrame()`](/reference/kotlin/android/media/MediaCodec#getOutputFrame(kotlin.Int))
* [`MediaCodec.LinearBlock.isCodecCopyFreeCompatible()`](/reference/kotlin/android/media/MediaCodec.LinearBlock#isCodecCopyFreeCompatible(kotlin.Array))

In addition, the behavior of two methods in [`MediaCodec.Callback()`](/reference/kotlin/android/media/MediaCodec.Callback) have changed:

[`onInputBufferAvailable()`](/reference/kotlin/android/media/MediaCodec.Callback#onInputBufferAvailable(android.media.MediaCodec,%20kotlin.Int))
...
[`onOutputBufferAvailable()`](/reference/kotlin/android/media/MediaCodec.Callback#onOutputBufferAvailable(android.media.MediaCodec,%20kotlin.Int,%20android.media.MediaCodec.BufferInfo))
...
Android 11 enhances
[`MediaCodec`](/reference/kotlin/android/media/MediaCodec)
...
[`FEATURE_LowLatency`](/reference/kotlin/android/media/MediaCodecInfo.CodecCapabilities#feature_lowlatency)
to
[`MediaCodecInfo.CodecCapabilities.isFeatureSupported()`](/reference/kotlin/android/media/MediaCodecInfo.CodecCapabilities#isfeaturesupported).

To turn low-latency decoding on or off, do either of the following:

* Set the new key
  [`KEY_LOW_LATENCY`](/reference/kotlin/android/media/MediaFormat#key_low_latency)
  to 0 or 1 using
  [`MediaCodec.configure()`](/reference/kotlin/android/media/MediaCodec#configure).
* Set the new parameter key
  [`PARAMETER_KEY_LOW_LATENCY`](/reference/kotlin/android/media/MediaCodec#parameter_key_low_latency)
  to 0 or 1 using
  [`MediaCodec.setParameters()`](/reference/kotlin/android/media/MediaCodec#setparameters).

**Note:** Supporting low-latency decoding can require additional resources, such as
higher power consumption. Use low-latency decoding only when necessary.

#### New AAudio function AAudioStream\_release()

The function
[`AAudioStream_close()`](/ndk/reference/group/audio#group___audio_1gad5f5e59999bb4f4b5ec6f5efcd09520b)
...
The new function
[`AAudioStream_release()`](/ndk/reference/group/audio#group___audio_1ga3a3f524d1a36022ff0300699bb0c8f7b)
...
[MediaParser](/reference/android/media/MediaParser) is a new low level API for
media extraction. It is more flexible than
[MediaExtractor](/reference/android/media/MediaExtractor) and provides additional
control over media extraction functionality.

#### Audio capture from a USB device

When an application without [`RECORD_AUDIO`](/reference/android/Manifest.permission#RECORD_AUDIO) permission uses [`UsbManager`](/reference/android/hardware/usb/UsbManager#openDevice(android.hardware.usb.UsbDevice)) to request direct access to a USB audio device with audio capture
...
**Note:** This behavior only applies to apps that connect directly to USB
peripherals using the [`UsbManager` API](/reference/android/hardware/usb/UsbManager#openDevice(android.hardware.usb.UsbDevice)). The vast majority of media players, games, and
communication apps use the Audio APIs and aren't affected by this
change.
...
regardless of the selected use case. See [Sharing Audio Input](/guide/topics/media/sharing-audio-input#concurrent-capture-r).
...
or listening to audio in the home or car. See [the output switcher](/guide/topics/media/media-routing#output-switcher).
...
[Passpoint](/guide/topics/connectivity/passpoint).

#### Wi-Fi Suggestion API is expanded

Android 11 expands the
[Wi-Fi Suggestion API](/guide/topics/connectivity/wifi-suggest)
to increase your app's network management capabilities, including the following:

* Connectivity management apps can manage their own networks by allowing
  disconnection requests.
* Passpoint networks are integrated into the Suggestion API and can be
  suggested to the user.
* Analytics APIs enable you to get information about the quality of your
  networks.

#### CallScreeningService updates

Starting in Android 11, a
[CallScreeningService](/reference/android/telecom/CallScreeningService) can
request
...
[call details](/reference/android/telecom/Call.Details)
for incoming calls.

If a `CallScreeningService` holds the
[`READ_CONTACTS`](/reference/android/Manifest.permission#READ_CONTACTS)
...
spoofing](/guide/topics/connectivity/telecom/prevent-spoofing).

#### Open Mobile API updates

For information on OMAPI support on Android 11 and higher, see [Open Mobile API
reader support](/guide/topics/connectivity/omapi).
...
network usage](/training/basics/network-ops/managing#manage-usage).

#### Allow multiple installed Passpoint configurations with the same FQDN

Starting in Android 11, you can use
[`PasspointConfiguration.getUniqueId()`](/reference/android/net/wifi/hotspot2/PasspointConfiguration#getUniqueId())
...
**Note:** Each configuration is identified by a unique key that depends on the
content of the configuration. In order to update an existing profile, you must
remove it using
[`WifiManager.removePasspointConfiguration()`](/reference/android/net/wifi/WifiManager#removePasspointConfiguration(java.lang.String)).
Not removing the existing configuration will cause a new profile to be added
with both configurations.

#### GNSS antenna support

Android 11 introduces the
[`GnssAntennaInfo`](/reference/kotlin/android/location/GnssAntennaInfo) class, which makes it
...
information](/guide/topics/sensors/gnss#antenna-calibration-information).
...
[Image decoder developer guide](/ndk/guides/image-decoder).
...
[Frame rate guide](/guide/topics/media/frame-rate).
...
[`Window.setPreferMinimalPostProcessing()`](/reference/android/view/Window#setPreferMinimalPostProcessing(boolean)),
or set the window's
[`preferMinimalPostProcessing`](/reference/android/R.attr#preferMinimalPostProcessing)
...
**Note:** If the user disables minimal post-processing, or if the display does not
support low latency mode, calling `Window.setPreferMinimalPostProcessing()`
has no effect.

#### Performant graphics debug layer injection

Applications can now load external graphics layers
([GLES](/ndk/guides/rootless-debug-gles),
[Vulkan](/ndk/guides/graphics/validation-layer)) into native application code to
...
[meta-data element](/guide/topics/manifest/meta-data-element) in your app
...
[`setCameraAudioRestriction()`](/reference/android/hardware/camera2/CameraDevice#setCameraAudioRestriction(int)).

#### Expanded camera support in Android emulator

For information on the expanded support for cameras in the emulator starting
with Android 11, see [Camera support](/studio/run/advanced-emulator-usage#camera).
...
* [`getConcurrentCameraIds()`](/reference/android/hardware/camera2/CameraManager#getConcurrentCameraIds())
  returns a `Set` of combinations of camera IDs that can stream concurrently
  with guaranteed stream combinations when configured by the same application
  process.
* [`isConcurrentSessionConfigurationSupported()`](/reference/android/hardware/camera2/CameraManager#isConcurrentSessionConfigurationSupported(java.util.Map%3Cjava.lang.String,%20android.hardware.camera2.params.SessionConfiguration%3E))
  queries whether camera devices can concurrently support the corresponding
  session configurations.

#### Better support for HEIF images with multiple frames

Beginning with Android 11, if you call [`ImageDecoder.decodeDrawable()`](/reference/android/graphics/ImageDecoder#decodeDrawable(android.graphics.ImageDecoder.Source)) and pass an HEIF image containing a sequence of frames (such as an animation or a burst photo), the method returns an [`AnimatedImageDrawable`](/reference/android/graphics/drawable/AnimatedImageDrawable) containing the entire image sequence. On earlier versions of Android, the method returned a `BitmapDrawable` of just a single frame.

If the HEIF graphic contains multiple frames that are not in a sequence, you can retrieve an individual frame by calling [`MediaMetadataRetriever.getImageAtIndex()`](/reference/android/media/MediaMetadataRetriever#getImageAtIndex(int)).
...
* The user-facing explanation of an accessibility service now allows for HTML
  and images in addition to plain text. This flexibility makes it easier to
  explain to end-users what your service does and how it can help them.
* To work with a description of a UI element's state that's more semantically
  meaningful than `contentDescription`, call the `getStateDescription()` method.
* To request that touch events bypass the system's touch explorer, call
  [`setTouchExplorationPassthroughRegion()`](/reference/kotlin/android/accessibilityservice/AccessibilityService#setTouchExplorationPassthroughRegion).
  Similarly, to request that gestures bypass the system's gesture detector, call
  [`setGestureDetectionPassthroughRegion()`](/reference/kotlin/android/accessibilityservice/AccessibilityService#setGestureDetectionPassthroughRegion(kotlin.Int,%20android.graphics.Region)).
* You can request IME actions, such as "enter" and "next", as well as
  screenshots of windows that don't enable the `FLAG_SECURE` flag.
...
[`setProcessStateSummary()`](/reference/android/app/ActivityManager#setProcessStateSummary(byte%5B%5D))
...
[`ApplicationExitInfo`](/reference/kotlin/android/app/ApplicationExitInfo)
...
[`getTimestamp()`](/reference/kotlin/android/app/ApplicationExitInfo#gettimestamp)
method.

**Note:** Some devices cannot report low-memory kills. On these devices, the
`getHistoricalProcessExitReasons()` method returns `REASON_SIGNALED` instead of
`REASON_LOW_MEMORY`, and the return value of
[`getStatus()`](/reference/kotlin/android/app/ApplicationExitInfo#getstatus) is
`SIGKILL`.  
  
To check whether a device can report low-memory kills, call
[`ActivityManager.isLowMemoryKillReportSupported()`](/reference/kotlin/android/app/ActivityManager#islowmemorykillreportsupported).
...
[`ResourcesLoader`](/reference/kotlin/android/content/res/loader/ResourcesLoader)
and
[`ResourcesProvider`](/reference/kotlin/android/content/res/loader/ResourcesProvider)
...
to an app's
[`Resources`](/reference/kotlin/android/content/res/Resources) instance. In
...
[`loadFromDirectory()`](/reference/kotlin/android/content/res/loader/ResourcesProvider#loadfromdirectory)
...
[`AssetManager`](/reference/kotlin/android/content/res/AssetManager)
API class, just like with assets bundled in the APK.

### APK signature scheme v4

Android 11 adds support for [APK Signature Scheme v4](/studio/command-line/apksigner#v4-signing-enabled). This scheme
...
[ADB incremental APK installation](/about/versions/11/features#incremental), which speeds up APK install.
...
[intent filter](/guide/components/intents-filters#Receiving) in the app
...
[`PackageManager`](/reference/kotlin/android/content/pm/PackageManager) API
class:

* [`getMimeGroup()`](/reference/kotlin/android/content/pm/PackageManager#getmimegroup)
* [`setMimeGroup()`](/reference/kotlin/android/content/pm/PackageManager#setmimegroup)
...
**Note:** `mimeGroup` strings are defined on a per-package basis. Within the same
package, you can use the same `mimeGroup` string in multiple intent filters or
components to declare a MIME group that is shared between them. Different
packages cannot share a MIME group, but they can use the same `mimeGroup` string
...
[view hint](/guide/topics/text/autofill-optimize#hints) is a particularly good
...
expands
[`AssistStructure.ViewNode`](/reference/android/app/assist/AssistStructure.ViewNode)
with a new
[`getHintIdEntry()`](/reference/android/app/assist/AssistStructure.ViewNode#getHintIdEntry())
...
[`FillEventHistory`](/reference/kotlin/android/service/autofill/FillEventHistory)
reports a new
[`TYPE_DATASETS_SHOWN`](/reference/android/service/autofill/FillEventHistory.Event#TYPE_DATASETS_SHOWN)
...
[`TYPE_DATASET_SELECTED`](/reference/android/service/autofill/FillEventHistory.Event#TYPE_DATASET_SELECTED)
...
[Integrating autofill with keyboards](/guide/topics/text/ime-autofill).
...
[`shareData()`](/reference/kotlin/android/view/contentcapture/ContentCaptureManager#sharedata)
