---
title: Grant partial access to photos and videos  |  Android Developers
...
# Grant partial access to photos and videos Stay organized with collections Save and categorize content based on your preferences.
...
maintain full control over your implementation, [adapt your implementation](#app-gallery-picker)
...
| Target SDK | [`READ_MEDIA_VISUAL_USER_SELECTED`](#permissions) declared | Selected Photos Access enabled | UX Behavior |
| --- | --- | --- | --- |
| SDK 33 | No | No | N/A |
| Yes | Yes | [Controlled by the app](#media-reselection) |
| SDK 34 | No | Yes | Controlled by the system [(compat behaviour)](#compatibility-mode) |
| Yes | Yes | [Controlled by the app](#media-reselection) |
...
![The .](/static/about/versions/14/images/partial-photo-video-access.png)


**Figure 1.** The new dialog lets a user select specific
...
Photos Access, the system runs your app in a [compatibility mode](#compatibility-mode).
...
new [`READ_MEDIA_VISUAL_USER_SELECTED`](/reference/android/Manifest.permission#READ_MEDIA_VISUAL_USER_SELECTED) permission to control media
...
![The .](/static/about/versions/14/images/partial-photo-video-access-reselect.png)


**Figure 2.** The new dialog also lets a user reselect which
...
**Note:** Create a UI element in your app that the user must press before you
re-request the `READ_MEDIA_IMAGES` or `READ_MEDIA_VIDEO` permission. The user
shouldn't be surprised to see the system dialog again.
...
library](/topic/libraries/architecture/paging/v3-overview) to help ensure good performance.
...
by enabling the [`QUERY_ARG_LATEST_SELECTION_ONLY`](/reference/android/provider/MediaStore#QUERY_ARG_LATEST_SELECTION_ONLY):
...
**Note:** The permission grant may still be denied manually by the user, the device
corporate policy or by a permission auto-reset. Always check for the permission
instead of assuming a previously granted state.
...
our [permission best practices](/training/permissions/requesting#principles).
...
the actual state. The permission state can change after [permission reset](/about/versions/11/privacy/permissions#auto-reset),
[app hibernation](/about/versions/12/behavior-changes-12#app-hibernation), a user-initiated change in your app's settings, or when
your app goes into the background. Instead, check for storage permissions using
[`ContextCompat.checkSelfPermission()`](/reference/androidx/core/content/ContextCompat#checkSelfPermission(android.content.Context,java.lang.String)).
...
[adapted your app](#app-gallery-picker) to use the new [`READ_MEDIA_VISUAL_USER_SELECTED`](/reference/android/Manifest.permission#READ_MEDIA_VISUAL_USER_SELECTED)
...
[figure 1](#figure-1)), the `READ_MEDIA_IMAGES` and `READ_MEDIA_VIDEO` permissions are
...
If your app is following [permissions best practices](#best-practices), this change shouldn't
...
for your users, we recommend [implementing photo picker](/training/data-storage/shared/photopicker) or [adapting your
app's gallery picker](#app-gallery-picker) to handle this behavior directly using the
