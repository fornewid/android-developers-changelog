---
title: Add support for built-in and custom predictive back animations  |  Android Developers
...
# Add support for built-in and custom predictive back animations Stay organized with collections Save and categorize content based on your preferences.



If you've already [migrated your app to the new system back APIs](/guide/navigation/custom-back/predictive-back-gesture), you can
opt in to predictive back to automatically receive in-app
animations and also support custom transitions.
...
See the [material component developer guidance on GitHub](https://github.com/material-components/material-components-android/blob/master/docs/foundations/PredictiveBack.md#predictive-back-material-components) for more
information.
...
Read more about [supporting predictive back](/guide/navigation/predictive-back-gesture#update-default).
...
[`OnBackPressedCallback`](/reference/kotlin/androidx/activity/OnBackPressedCallback)
we've introduced the `handleOnBackProgressed`, `handleOnBackCancelled` and
`handleOnBackStarted` methods to animate objects while the user swipes back. Use
...
[`OnBackAnimationCallback`](/reference/kotlin/android/window/OnBackAnimationCallback)
...
on Android 14 and above to create Predictive Back transitions.
...
* Predictive Back transitions are supported with AndroidX transitions,
  but not with framework transitions. We recommend migrating away from framework
  transitions.
* Predictive Back transitions are supported on Android 14+ devices and are
...
and higher, you can use [`overrideActivityTransition`](/reference/android/app/Activity#overrideActivityTransition(int,%20int,%20int)) instead of
...
* Call either opening or closing transitions within Activity B's `onCreate`
  method.
* When the user navigates to Activity B, use `OVERRIDE_TRANSITION_OPEN`. When
  the user swipes to navigate back to Activity A, use
...
* When specifying `OVERRIDE_TRANSITION_CLOSE`, the `enterAnim` is Activity A's
  ...
  **Note:** If `exitAnim` isn't set or is set to `0`, the default cross-activity
  predictive animation (shown in the preceding video clip) plays instead.