...
regardless of
[`targetSdkVersion`](/guide/topics/manifest/uses-sdk-element#target). You should
...
targeting Android 14](/about/versions/14/behavior-changes-14).
...
alarms](/about/versions/14/changes/schedule-exact-alarms).

### Context-registered broadcasts are queued while apps are cached

On Android 14, the system can
[place context-registered broadcasts in a queue](/develop/background-work/background-tasks/broadcasts#android-14) while the app
is in the [cached state](/guide/components/activities/process-lifecycle). This is similar to the queuing
...
Starting in Android 14, when your app calls [`killBackgroundProcesses()`](/reference/android/app/ActivityManager#killBackgroundProcesses(java.lang.String)),
...
**Note:** It isn't possible for a 3rd-party application to improve the memory,
power, or thermal behavior of an Android device. You should ensure that your app
is compliant with [Google Play's policy regarding misleading claims](https://support.google.com/googleplay/android-developer/answer/9888077#zippy=%2Cexamples-of-common-violations).
...
the [`BluetoothGatt#requestMtu(int)`](/reference/android/bluetooth/BluetoothGatt#requestMtu(int)) API, and disregards all subsequent MTU
requests on that ACL connection.

**Note:** This change doesn't have an impact unless the peripheral device isn't
handling the MTU negotiation properly and accepting any MTU size even if it
doesn't support it. In such cases, it could cause issues when your app sends
large amounts of data from Android 14 devices.
...
* Your peripheral device should respond to the Android device's MTU request
  ...
  the remote provided value (for example, `min(517, remoteMtu)`)
...
Android 14 introduces a new reason an app can be placed into the [restricted standby bucket](/topic/performance/appstandby#restricted-bucket).
The app's jobs trigger ANR errors multiple times due to [`onStartJob`](/reference/android/app/job/JobService#onStartJob(android.app.job.JobParameters)),
[`onStopJob`](/reference/android/app/job/JobService#onStopJob(android.app.job.JobParameters)), or [`onBind`](/reference/android/app/Service#onBind(android.content.Intent)) method timeouts.
(See [JobScheduler reinforces callback and network behavior](/about/versions/14/behavior-changes-14#jobscheduler-reinforces-behavior) for changes
to `onStartJob` and `onStopJob`.)

**Note:** See [power management](/topic/performance/power/power-details) to see how the app is impacted if it is placed
into the restricted bucket. The app is moved back to the [active](/topic/performance/appstandby#active-bucket) bucket
when the user launches the app to the foreground, just as in previous Android versions.

To track whether or not the app has entered the restricted standby bucket,
we recommend logging with the API [`UsageStatsManager.getAppStandbyBucket()`](/reference/android/app/usage/UsageStatsManager#getAppStandbyBucket())
on job execution or [`UsageStatsManager.queryEventsForSelf()`](/reference/android/app/usage/UsageStatsManager#queryEventsForSelf()) on app startup.

### mlock limited to 64 KB

In Android 14 (API level 34) and higher, the platform reduces the maximum memory
that can be locked using [`mlock()`](/reference/android/system/Os#mlock(long,%20long)) to 64 KB per process. In
...
[By design](/guide/components/activities/process-lifecycle), an app's process is in a cached state when it's moved to the
...
[services](/guide/components/services), [`JobScheduler`](/reference/android/app/job/JobScheduler), and [Jetpack WorkManager](/topic/libraries/architecture/workmanager) – shouldn't be
...
notifications by setting [`Notification.FLAG_ONGOING_EVENT`](/reference/android/app/Notification#FLAG_ONGOING_EVENT) through
[`Notification.Builder#setOngoing(true)`](/reference/android/app/Notification.Builder#setOngoing(boolean)) or
[`NotificationCompat.Builder#setOngoing(true)`](/reference/android/app/Notification.Builder#setOngoing(boolean)). The behavior of
...
* If the user selects a **Clear all** notification action (which helps with
  accidental dismissals)

Also, this new behavior doesn't apply to notifications in the
following use cases:

* [`CallStyle`](/reference/androidx/core/app/NotificationCompat.CallStyle) notifications
...
Learn more in the guide about how [data safety information is more visible](/about/versions/14/changes/data-safety)
on Android 14.
...
(200%)](/about/versions/14/features#test-scaling) to
...
[`targetSdkVersion`](/guide/topics/manifest/uses-sdk-element) lower than 23
...
The media store supports queries for the [`OWNER_PACKAGE_NAME`](/reference/android/provider/MediaStore.MediaColumns#OWNER_PACKAGE_NAME) column, which
indicates the [app that stored a particular media file](/training/data-storage/shared/media#app-attribution). Starting in Android
14, this value is redacted unless at least one of the following conditions is
true:

* The app that stored the media file has a package name that is always visible
  to other apps.
* The app that queries the media store requests the [`QUERY_ALL_PACKAGES`](/reference/android/Manifest.permission#QUERY_ALL_PACKAGES)
  permission.
...
Learn more about how [Android filters package visibility](/training/package-visibility) for privacy
purposes.