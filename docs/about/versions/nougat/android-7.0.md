---
title: Android 7.0 for Developers  |  Android Developers
...
# Android 7.0 for Developers Stay organized with collections Save and categorize content based on your preferences.
...
[Android 7.0 behavior changes](/about/versions/nougat/android-7.0-changes) to learn about areas where platform changes
...
* On Android TV devices, apps can put themselves in [picture-in-picture
  mode](/guide/topics/ui/picture-in-picture), allowing them to continue showing content while the user browses or
  interacts with other apps.

![Mobile running apps in split screen mode](/static/images/android-7.0/mw-portrait.png)
...
For more information, see the [Multi-Window Support](/guide/topics/ui/multi-window)
...
* **Template updates**: We're updating notification templates to
...
* **Bundled notifications**: The system can group messages
  ...
  this model.
* **Direct reply**: For real-time communication apps, the
...
* **Custom views**: Two new APIs enable you to leverage system
  decorations, such as notification headers and actions, when using custom
  views in notifications.

![Mobile displaying bundled message notifications](/static/images/android-7.0/notifications-1.png)

![Mobile displaying single message notification](/static/images/android-7.0/notifications-3.png)

![Mobile displaying inline message reply within notification interface](/static/images/android-7.0/notifications-2.png)

**Figure 2.** Bundled notifications and direct reply.

To learn how to implement the new features, see the
[Notifications](/guide/topics/ui/notifiers/notifications)
guide.
...
![Illustration of how Doze applies a first level of system activity restrictions to improve battery life](/static/images/android-7.0/doze-diagram-1.png)
...
period of time, Doze applies the full CPU and network restrictions on `PowerManager.WakeLock`, `AlarmManager` alarms, and
GPS/Wi-Fi scans.
...
your app to Doze](/training/monitoring-device-state/doze-standby#assessing_your_app) now.
...
Since Android 5.0, `JobScheduler` has been the
...
`CONNECTIVITY_ACTION`, `ACTION_NEW_PICTURE`, and `ACTION_NEW_VIDEO` — since those can wake the
...
Optimizations](/topic/performance/background-optimization) documentation for details.

## SurfaceView

Android 7.0 brings synchronous movement to the `SurfaceView`
class, which provides better battery performance
than `TextureView` in certain cases: When rendering video or
3D content, apps with scrolling and animated video position use less power with
`SurfaceView` than with `TextureView`.

The `SurfaceView` class enables more battery-efficient compositing on
...
copies than `TextureView`.

A `SurfaceView` object's content position is now updated synchronously
with the containing app content. One result of this change is that simple
translations or scales of a video playing in a `SurfaceView`
...
`SurfaceView` instead of `TextureView`.

## Data Saver

![Data Saver in Settings](/static/images/android-7.0/datasaver.png)
...
Android 7.0 extends the `ConnectivityManager` to provide apps a
way to [retrieve the
user's Data Saver preferences](/training/basics/network-ops/data-saver#status) and [monitor
preference changes](/training/basics/network-ops/data-saver#monitor-changes). All apps should check whether the user has enabled Data
...
For more information, see the [API documentation](/ndk/guides/graphics).

## Quick Settings Tile API

![Quick Settings tiles in the notification shade](/static/images/android-7.0/quicksettings.png)
...
* Numbers blocked on calls are also blocked on texts* Blocked numbers can persist across resets and devices through the Backup &
    Restore feature* Multiple apps can use the same blocked numbers list
...
incoming call's `Call.Details`, such as:

* Reject the incoming call* Do not allow the call to the call log* Do not show the user a notification for the call
...
should follow, see [Multilingual Support](/guide/topics/resources/multilingual-support).
...
* **Check that a device contains an emoji before inserting it.**
  ...
  system font, use the `hasGlyph(String)` method.
* **Check that an emoji supports variation selectors.**
  ...
  *emoji variation sequences* section of the
  [Unicode documentation on variations](https://www.unicode.org/Public/UNIDATA/StandardizedVariants.txt).
* **Check that an emoji supports skin tone.** Android 7.0 allows users to modify the
  ...
  skin tone modifiers, use the `hasGlyph(String)`
  method. You can determine which emojis use skin tones by reading the
  [Unicode documentation](http://unicode.org/emoji/charts/full-emoji-list.html).
...
To learn more about the Android ICU4J APIs, see [ICU4J Support](/guide/topics/resources/internationalization#nougat).
...
them. Please get in touch with the WebView team on the [Chromium bug tracker](https://bugs.chromium.org/p/chromium/issues/entry?template=Webview%20Bugs).
...
bug tracker](https://bugs.chromium.org/p/chromium/issues/entry?template=Webview%20Bugs) so that we can fix them before a new version of WebView is
released.
...
supported OpenGL ES version at runtime, see the [OpenGL ES API guide](/develop/ui/views/graphics/opengl/about-opengl).
...
For more information, see [Android TV Recording APIs](/training/tv/tif/content-recording).
...
For more information, see [Direct Boot](/training/articles/direct-boot).
...
description](/training/articles/security-key-attestation#certificate_schema_keydescription) data structure is set to the TrustedEnvironment security
level.

For more information, see the
[Key Attestation](/training/articles/security-key-attestation)
developer documentation.
...
* **Custom trust anchors.** Lets an application customize which
...
* **Debug-only overrides.** Lets an application developer safely debug
  secure connections of their application without added risk to the installed
  base.
* **Cleartext traffic opt-out.** Lets an application protect itself from
  accidental usage of cleartext traffic.
* **Certificate pinning.** An advanced feature that lets an application
  limit which server keys are trusted for secure connections.

For more information, see [Network security configuration](/training/articles/security-config).
...
[Network security configuration](/training/articles/security-config) to
...
[sign an app](/studio/publish/app-signing#release-mode) in Android Studio and how to  [configure
the build file for signing apps](/studio/build/build-variants#signing) using the Android Plugin for Gradle.

## Scoped Directory Access

In Android 7.0, apps can use new APIs to request access to specific [external
storage](/guide/topics/data/data-storage#filesExternal) directories, including directories on removable media such as SD
...
For more information, see the
[Scoped
Directory Access](/training/data-storage) developer documentation.
...
this by overriding the `onProvideKeyboardShortcuts()` method.
...
* You can set an icon from a resource ID by calling
  `setIconResourceId()`.
* You can show an icon from the network by calling
...
information by calling `setInfoIntent()`.
...
The API provides a `FrameMetrics` object, which
...
`DocumentsProvider` to return document URIs that can be
used with an `ACTION_VIEW` intent even if they
...
Storage Access Frameworks guide](/guide/topics/providers/document-provider#virtual).