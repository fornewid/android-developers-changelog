...
report](/sdk/api_diff/s-dp1/changes). For details on new APIs visit the [Android
API reference](/reference) — new APIs are highlighted for visibility. Also, to
...
Android 12](/about/versions/12/behavior-changes-12) and [for all
apps](/about/versions/12/behavior-changes-all).
...
![Material You](/static/images/about/versions/12/material-you.jpeg)
...
See [Android 12 widgets improvements](/about/versions/12/features/widgets) for
more information.
...
content](/guide/topics/input/receive-rich-content).
...
guide](/guide/topics/ui/splash-screen) for more details.

### Rounded corner APIs

Android 12 introduces [`RoundedCorner`](/reference/android/view/RoundedCorner)
and [`WindowInsets.getRoundedCorner(int
position)`](/reference/android/view/WindowInsets#getRoundedCorner(int)),
which provide the radius and center point for rounded corners.

For more information, see [Rounded corners](/guide/topics/ui/look-and-feel/rounded-corners).
...
tick](/reference/android/os/VibrationEffect.Composition#PRIMITIVE_LOW_TICK) that
...
actuators](/reference/android/os/VibratorManager) independently in game
...
[constants](/reference/android/view/HapticFeedbackConstants) and
[primitives](/reference/android/os/VibrationEffect.Composition#summary) as
building blocks for rich haptic effects - constants to enhance UI events and
[haptic composer](/reference/android/os/VibrationEffect.Composition) to sequence
...
See the [`HapticGenerator`](/reference/android/media/audiofx/HapticGenerator)
...
![Diagram illustrating indexing and searching within AppSearch](/static/images/about/versions/12/appsearch.png)
...
[developer guide](/guide/topics/search/appsearch), and begin using it with
the [AppSearch Jetpack library](/jetpack/androidx/releases/appsearch), which
provides a developer-friendly API surface as well as annotation processor
support.

### Game Mode

The [Game Mode API](/games/gamemode/gamemode-api) and [Game Mode
interventions](/games/gamemode/gamemode-interventions) allow you to optimize
...
For more information, see [Game Mode](/games/gamemode).
...
gestures](/develop/ui/views/picture-in-picture#gestures) for the PiP
window:
...
Android 12 added [significant cosmetic improvements](/develop/ui/views/picture-in-picture#smoother-transition)
to the animated
...
* [A new API flag for smoother transition to PiP mode with gesture navigation](/develop/ui/views/picture-in-picture#setautoenterenabled)

  Use the [`setAutoEnterEnabled`](/reference/android/app/PictureInPictureParamsBuilder#setAutoEnterEnabled(boolean))
...
* [Smoother animations when entering and exiting out of PiP mode](/develop/ui/views/picture-in-picture#set-sourcerecthint)

  The [`SourceRectHint`](/reference/android/app/PictureInPictureParams.Builder#setSourceRectHint(android.graphics.Rect))
  flag is now reused to implement smoother animation when entering and exiting
  PiP mode.
* [A new API flag to disable seamless resizing for non-video content](/develop/ui/views/picture-in-picture#seamless-resizing)

  The [`SeamlessResizeEnabled`](/reference/android/app/PictureInPictureParams.Builder#setSeamlessResizeEnabled(boolean))
...
[`Notification.CallStyle`](/reference/android/app/Notification.CallStyle)
...
Use [`forIncomingCall()`](/reference/android/app/Notification.CallStyle#forIncomingCall(android.app.Person,%20android.app.PendingIntent,%20android.app.PendingIntent))
...
Use [`forOngoingCall()`](/reference/android/app/Notification.CallStyle#forOngoingCall(android.app.Person,%20android.app.PendingIntent))
...
Use [`forScreeningCall()`](/reference/android/app/Notification.CallStyle#forScreeningCall(android.app.Person,%20android.app.PendingIntent,%20android.app.PendingIntent))
...
providing animated images in [`MessagingStyle()`](/reference/android/app/Notification.MessagingStyle#MessagingStyle(android.app.Person))
and [`BigPictureStyle()`](/reference/android/app/Notification.BigPictureStyle#BigPictureStyle())
...
mode](/training/system-ui/immersive#immersive-consolidated-behavior). In
addition, Android 12 provides [backward compatibility behavior for sticky
immersive
mode](/training/system-ui/immersive#sticky-immersive-compat-android11-lower).
...
sharing](/guide/components/activities/recents#url-sharing).
...
![A vertical timeline shows the different apps that have
         accessed location information, and at what time the accesses occurred](/static/images/about/versions/12/privacy-dashboard.svg)
...
users](/training/permissions/explaining-access#privacy-dashboard-show-rationale)
...
devices](/guide/topics/connectivity/bluetooth), especially for apps that don't
require access to device location.

**Note:** The [Companion Device
Manager](/guide/topics/connectivity/companion-device-pairing) provides a more
streamlined method of connecting to companion devices. The system provides the
pairing UI on behalf of your app. If you want more control over the pairing and
connecting experience, use the Bluetooth permissions introduced in
Android 12.
...
permissions](/guide/topics/connectivity/bluetooth/permissions#declare-android11-or-lower),
declare a [more modern set of Bluetooth
permissions](/guide/topics/connectivity/bluetooth/permissions#declare-android12-or-higher).

### Permission group lookup

On Android 12 or higher, you can query how the system organizes
platform-provided [permissions](/guide/topics/permissions/overview) into
permission groups:

* To determine the permission group into which the system has placed a
  platform-defined permission, call
  [`getGroupOfPlatformPermission()`](/reference/android/content/pm/PackageManager#getGroupOfPlatformPermission(java.lang.String,%20java.util.concurrent.Executor,%20java.util.function.Consumer%3Cjava.lang.String%3E)).
* To determine the platform-defined permissions that the system has placed into
  a particular permission group, call
  [`getPlatformPermissionsForGroup()`](/reference/android/content/pm/PackageManager#getPlatformPermissionsForGroup(java.lang.String,%20java.util.concurrent.Executor,%20java.util.function.Consumer%3Cjava.util.List%3Cjava.lang.String%3E%3E)).

**Note:** One of the [basic principles](/training/permissions/requesting#principles)
of using Android permissions is to not assume system behavior. Don't assume that
a particular permission is in a particular group. Instead, use the APIs
...
[`SYSTEM_ALERT_WINDOW`](/reference/android/Manifest.permission#SYSTEM_ALERT_WINDOW)
permission.

After declaring the
[`HIDE_OVERLAY_WINDOWS`](/reference/android/Manifest.permission#HIDE_OVERLAY_WINDOWS)
permission, an app can call
[`setHideOverlayWindows()`](/reference/android/view/Window#setHideOverlayWindows(boolean))
...
[picture-in-picture](/guide/topics/ui/picture-in-picture) or
[bubbles](/guide/topics/ui/bubbles).

### Known signers permission protection flag

Starting in Android 12, the
[`knownCerts`](/reference/android/R.attr#knownCerts) attribute for
[signature-level permissions](/guide/topics/permissions/overview#signature)
allows you to refer to the digests of known [signing
certificates](/studio/publish/app-signing#certificates-keystores) at declaration
time.
...
apps](/guide/topics/permissions/defining#grant-signature-permissions), without
...
owners (DPOs)](/work/device-admin) that use
...
[`setDevicePropertiesAttestationIncluded()`](/reference/android/security/keystore/KeyGenParameterSpec.Builder#setDevicePropertiesAttestationIncluded(boolean))
method.

The generated device properties include the following
[`Build`](/reference/android/os/Build) fields:
...
supports the
[`setAuthenticationRequired()`](/reference/android/app/Notification.Action.Builder#setAuthenticationRequired(boolean))
method, which allows your app to [require that a device is
unlocked](/guide/topics/ui/notifiers/notifications#ActionsRequireUnlockedDevice)
...
authentication user experience. The new [`BiometricManager.Strings`](/reference/android/hardware/biometrics/BiometricManager.Strings)
nested class includes the [`getButtonLabel()`](/reference/android/hardware/biometrics/BiometricManager.Strings#getButtonLabel()),
[`getPromptMessage()`](/reference/android/hardware/biometrics/BiometricManager.Strings#getPromptMessage()),
and [`getSettingName()`](/reference/android/hardware/biometrics/BiometricManager.Strings#getSettingName())
...
See [compatible media transcoding](/guide/topics/media-apps/video-app/compatible-media-transcoding) for more details.
...
See [Performance class](/topic/performance/performance-class)
for more details.
...
[`MediaFormat`](/reference/android/media/MediaFormat#KEY_VIDEO_QP_B_MAX) API
and also in the
...
See [Audio focus in Android 12 and higher](/media/optimize/audio-focus#audio-focus-12)
for more details.
...
library](/training/camerax/extensions-api) already supports
...
[`Camera2`](/reference/android/hardware/camera2/package-summary)
...
the [same set of
extensions](/reference/android/hardware/camera2/CameraExtensionCharacteristics) as
in CameraX, and those are already supported on [many different
devices](/training/camerax/devices), so you can use them without any
...
sensors. The [new APIs](/reference/android/hardware/camera2/CameraMetadata#REQUEST_AVAILABLE_CAPABILITIES_ULTRA_HIGH_RESOLUTION_SENSOR)
...
For more information, see [Provide apps direct access to tombstone traces](/ndk/guides/debug#tombstone-traces)
...
Android 12 adds the new [`RenderEffect`](/reference/android/graphics/RenderEffect)
...
Effects can also be applied to the underlying [`RenderNode`](/reference/android/graphics/RenderNode)
for `View`s by calling [`View.setRenderEffect(RenderEffect)`](/reference/android/view/View#setRenderEffect(android.graphics.RenderEffect)).
...
[`ImageDecoder`](/ndk/reference/group/image-decoder) API has been expanded
...
size](/topic/performance/reduce-apk-size#minimize)
and benefit from future updates related to security and performance.

For more details on the API, refer to the [API reference](/ndk/reference/group/image-decoder) and the [sample on GitHub](https://github.com/android/ndk-samples/tree/develop/webp/image-decoder).
...
* Guarantee that the process will continue running while the device stays within
  range.

To use the APIs, your devices must be connected using [Companion Device
Manager](/reference/android/companion/CompanionDeviceManager). For more
information, see
[`CompanionDeviceManager.startObservingDevicePresence()`](/reference/android/companion/CompanionDeviceManager#startObservingDevicePresence(java.lang.String))
and
...
![](/static/images/about/versions/12/companion-device-profile.png)


A permissions dialog that uses a companion device profile to
...
* On devices running Android 12 (API level 31) and higher, you can use the
  [`onServiceLost()`](/reference/android/net/wifi/aware/DiscoverySessionCallback#onServiceLost(android.net.wifi.aware.PeerHandle,%20int)) callback to be alerted when your app has lost a discovered service due to the
  service stopping or moving out of range.
* The way that multiple data-paths (NAN Data Paths) are set up is changing to be
  more efficient. Earlier versions used L2 messaging to exchange peer
  information of the initiators, which introduced latency. On devices running
  Android 12 and higher, the responder (server) can be configured
  to accept any peer—that is, it doesn’t need to know the initiator
  information upfront. This speeds up datapath bringup and enables multiple
  point-to-point links with only one network request.
* To prevent the framework from rejecting discovery or connection requests due
  ...
  higher, you can call
  [`WifiAwareManager.getAvailableAwareResources()`](/reference/android/net/wifi/aware/WifiAwareManager#getAvailableAwareResources()).
  This method's return value lets you get the number of available data
  paths, the number of available publish sessions, and the number of available
  subscribe sessions.
...
connections](/guide/topics/connectivity/wifi-bootstrap) will not disconnect your
existing Wi-Fi connection when creating the connection to the peer device. To
check for support for this feature, use
[`WifiManager.isMultiStaConcurrencySupported()`](/reference/android/net/wifi/WifiManager#isMultiStaConcurrencySupported()).
...
behavior](/guide/topics/connectivity/nfc/hce#screen-off-12-higher).
...
* Media store support for `MediaDocumentsProvider` when your app [retrieves a
  ...
  URI](/training/data-storage/shared/documents-files#retrieve-equivalent-media-uri).
* A directory for [voice
  recordings](/training/data-storage/shared/media#well-defined-collections).
* The [`MANAGE_MEDIA`](/reference/android/Manifest.permission#MANAGE_MEDIA)
  permission, which allows an app to [perform media management
  operations](/training/data-storage/shared/media#management-permission)
  without showing a confirmation dialog to the user for each operation.
* Apps that have both the
  ...
  for managing another app's storage space, provided that the other app [creates
  the custom activity](/training/data-storage/app-specific#create-storage-management-activity).
...
Android 12 introduces the
[`setRequireUserAction()`](/reference/android/content/pm/PackageInstaller.SessionParams#setRequireUserAction(int))
method for apps that use the
[`PackageInstaller`](/reference/android/content/pm/PackageInstaller) API.
...
| **Class** | **APIs** |
| --- | --- |
| `java.lang.Deprecated` | * [`forRemoval()`](/reference/java/lang/Deprecated#forRemoval())* [`since()`](/reference/java/lang/Deprecated#since()) |
| `java.lang.Byte` | * [Constructors deprecated](/reference/java/lang/Byte#public-constructors_1)* [`compareUnsigned()`](/reference/java/lang/Byte#compareUnsigned(byte,%20byte)) |
| `java.lang.Short` | * [Constructors deprecated](/reference/java/lang/Short#public-constructors_1)* [`compareUnsigned()`](/reference/java/lang/Short#compareUnsigned(short,%20short)) |
| `java.lang.Math` | * [`floorDiv()`](/reference/java/lang/Math#floorDiv(long,%20int))* [`floorMod()`](/reference/java/lang/Math#floorMod(long,%20int))* [`multiplyExact()`](/reference/java/lang/Math#multiplyExact(long,%20int))* [`multiplyFull()`](/reference/java/lang/Math#multiplyFull(int,%20int))* [`multiplyHigh()`](/reference/java/lang/Math#multiplyHigh(long,%20long)) |
| `java.lang.StrictMath` | * [`floorDiv()`](/reference/java/lang/StrictMath#floorDiv(long,%20int))* [`floorMod()`](/reference/java/lang/StrictMath#floorMod(long,%20int))* [`multiplyExact()`](/reference/java/lang/StrictMath#multiplyExact(long,%20int))* [`multiplyFull()`](/reference/java/lang/StrictMath#multiplyFull(int,%20int))* [`multiplyHigh()`](/reference/java/lang/StrictMath#multiplyHigh(long,%20long)) |
| `java.util.Set` | [`copyOf()`](/reference/java/util/Set#copyOf(java.util.Collection%3C?%20extends%20E%3E)) |
| `java.util.Map` | [`copyOf()`](/reference/java/util/Map#copyOf(java.util.Map%3C?%20extends%20K,%20?%20extends%20V%3E)) |
| `java.util.List` | [`copyOf()`](/reference/java/util/List#copyOf(java.util.Collection%3C?%20extends%20E%3E)) |
| `java.time.Duration` | * [`dividedBy()`](/reference/java/time/Duration#dividedBy(java.time.Duration))* [`toDaysPart()`](/reference/java/time/Duration#toDaysPart())* [`toHoursPart()`](/reference/java/time/Duration#toHoursPart())* [`toMillisPart()`](/reference/java/time/Duration#toMillisPart())* [`toMinutesPart()`](/reference/java/time/Duration#toMinutesPart())* [`toNanosPart()`](/reference/java/time/Duration#toNanosPart())* [`toSecondsPart()`](/reference/java/time/Duration#toSecondsPart())* [`truncatedTo()`](/reference/java/time/Duration#truncatedTo(java.time.temporal.TemporalUnit)) |
| `java.time.LocalTime` | * [`ofInstant()`](/reference/java/time/LocalTime#ofInstant(java.time.Instant,%20java.time.ZoneId))* [`toEpochSecond()`](/reference/java/time/LocalTime#toEpochSecond(java.time.LocalDate,%20java.time.ZoneOffset)) |