...
targeting Android 12](/about/versions/12/behavior-changes-12).
...
events](/training/gestures/scroll#term) changes.
...
gestures](/training/gestures/scroll).
...
implementation to Android 12](/guide/topics/ui/splash-screen/migrate).

Additionally, starting in Android 12, the system always applies the new [Android
system default splash screen](/about/versions/12/features/splash-screen) on
[cold](/topic/performance/vitals/launch-time#cold) and
[warm starts](/topic/performance/vitals/launch-time#warm) for all apps.
By default, this system default splash screen is constructed using your app’s
launcher icon element and the
[`windowBackground`](/topic/performance/vitals/launch-time#solutions-3) of your
theme (if it's a single color).

For more details, see the [splash screens developer guide](/guide/topics/ui/splash-screen).
...
* Verify the domain using [Android App
  Links](/training/app-links/verify-android-applinks).

  On apps that target Android 12 or higher, the system
  changes how it [automatically
  verifies](/training/app-links/verify-android-applinks#auto-verification)
  your app's Android App Links. In your app's [intent
  filters](/training/app-links/verify-android-applinks#add-intent-filters),
  check that you include the `BROWSABLE` category and support the `https`
  scheme.

  On Android 12 or higher, you
  can [manually
  verify](/training/app-links/verify-android-applinks#manual-verification)
  your app's Android App Links, to test how this updated logic affects your
  app.
* [Request the user to associate your app with the
  domain](/training/app-links/verify-android-applinks#request-user-associate-app-with-domain)
  in system settings.
...
11, we introduced the [`WindowMetrics`](/reference/android/view/WindowMetrics)
...
their window, and [`Configuration.densityDpi`](/reference/android/content/res/Configuration#densityDpi)
to query the current density.

For broader compatibility with older versions of Android, you can use the
Jetpack [`WindowManager`](/jetpack/androidx/releases/window) library, which
includes a [`WindowMetrics`](/reference/androidx/window/layout/WindowMetrics) class
that supports Android 4.0 (API level 14) and higher.

#### Examples of how to use WindowMetrics

First, be sure your app's activities are [fully resizable](/guide/topics/ui/multi-window).
...
[`WindowMetricsCalculator.computeMaximumWindowMetrics()`](/reference/androidx/window/layout/WindowMetricsCalculator#computeMaximumWindowMetrics(android.app.Activity)).
...
**Note:** Any library that uses `MediaProjection` should also follow this advice and
query the appropriate `WindowMetrics`.
...
[`minWidth`](/reference/android/content/pm/ActivityInfo.WindowLayout#attr_android:minWidth)
and
[`minHeight`](/reference/android/content/pm/ActivityInfo.WindowLayout#attr_android:minHeight)
...
services](/guide/components/foreground-services), devices that run
Android 12 or higher can delay the display of foreground service
notifications by 10 seconds, with [a few
exceptions](/guide/components/foreground-services#notification-immediate). This
...
bucket](/topic/performance/appstandby#restricted-bucket) as an App Standby
...
5. Restricted: App consumes a great deal of system resources, or may exhibit
...
[`getAppStandbyBucket()`](/reference/android/app/usage/UsageStatsManager#getAppStandbyBucket()).
...
1. Call
   [`getLocationPowerSaverMode()`](/reference/android/os/PowerManager#getLocationPowerSaveMode())
   to check how the device's location features behave when Battery Saver is active.
2. If this returns [`LOCATION_MODE_FOREGROUND_ONLY`](/reference/android/os/PowerManager#LOCATION_MODE_FOREGROUND_ONLY), your app
   will continue to receive location updates while in the foreground or running a
   foreground service while Battery Saver is on and the screen is off.
...
![The dialog has two sets of options, one above the
         other](/static/images/about/versions/12/approximate-location-full-prompt.svg)


**Figure 1.** System permissions dialog that allows the user
to grant approximate location information.

On devices that run Android 12 or higher, [users can
request](/training/location/permissions#approximate-request) that your app have
access to only [approximate location](/training/location/permissions#accuracy)
information.

**Note:** If your app requests `ACCESS_COARSE_LOCATION` but not
`ACCESS_FINE_LOCATION`, then this change doesn't affect your app.
...
request](/training/permissions/requesting#allow-system-manage-request-code).

**Caution:** On some releases of Android 12, this change only affects
apps that target Android 12 or higher. In those releases, if your
app requests `ACCESS_FINE_LOCATION`, it must also request
`ACCESS_COARSE_LOCATION` to show the system permissions dialog.
...
[toggles](/training/permissions/explaining-access#toggles), and how to check
that your app follows best practices regarding the
[`CAMERA`](/reference/android/Manifest.permission#CAMERA) and
...
Learn more about these
[indicators](/training/permissions/explaining-access#indicators) and how to
...
![Quick settings tiles are labeled 'Camera access' and
         'Mic access'](/static/images/about/versions/12/mic-camera-toggles.svg)


**Figure 2.** Microphone and camera toggles in
Quick Settings.

![A rounded rectangle in the upper-right corner, which
         includes a camera icon and a microphone icon](/static/images/about/versions/12/mic-camera-indicators.svg)


**Figure 3.** Microphone and camera indicators, which show
recent data access.
...
visibility](/training/package-visibility) into other apps:

* [`getAllPermissionGroups()`](/reference/android/content/pm/PackageManager#getAllPermissionGroups(int))
* [`getPermissionGroupInfo()`](/reference/android/content/pm/PackageManager#getPermissionGroupInfo(java.lang.String,%20int))
* [`getPermissionInfo()`](/reference/android/content/pm/PackageManager#getPermissionInfo(java.lang.String,%20int))
* [`queryPermissionsByGroup()`](/reference/android/content/pm/PackageManager#queryPermissionsByGroup(java.lang.String,%20int))
...
* **Your app uses 512-bit key sizes.** Conscrypt doesn't support this key size.
  If necessary, update your app's cryptography logic to use different key sizes.
* **Your app uses invalid key sizes with `KeyGenerator`.**
  Conscrypt's implementation of
  [`KeyGenerator`](/reference/javax/crypto/KeyGenerator) performs additional
  ...
  if these keys are used with a [`Cipher`](/reference/javax/crypto/Cipher).
...
* **You initialize your Galois/Counter Mode (GCM) ciphers using a size other
  ...
  [`GcmParameterSpec`](/reference/javax/crypto/spec/GCMParameterSpec) requires an
...
app](/guide/topics/text/copy-paste#Pasting) for the first time, a toast message
notifies the user of this clipboard access.

The text inside the toast message contains the following format:
`APP pasted from your clipboard.`

**Note:** Your app might call
[`getPrimaryClipDescription()`](/reference/android/content/ClipboardManager#getPrimaryClipDescription())
to receive information about the [current data that's on the
clipboard](/guide/topics/text/copy-paste#ClipClasses). When your app calls this
method, the system doesn't show a toast message.
...
* Stylized text, using
  [`isStyledText()`](/reference/android/content/ClipDescription#isStyledText()).
* Different classifications of text, such as URLs, using
  [`getConfidenceScore()`](/reference/android/content/ClipDescription#getConfidenceScore(java.lang.String)).

### Apps can't close system dialogs

To improve user control when interacting with apps and the system, the
[`ACTION_CLOSE_SYSTEM_DIALOGS`](/reference/android/content/Intent#ACTION_CLOSE_SYSTEM_DIALOGS)
intent action is deprecated as of Android 12. Except for [a few
special cases](#close-system-dialogs-exceptions), when your app tries to [invoke
an intent](/guide/components/intents-filters) that contains this action, the
...
* Your app is running an [instrumentation
  test](/training/testing/unit-testing/instrumented-unit-tests).
* Your app targets Android 11 or lower and is showing a window
  ...
  **Note:** If your app targets Android 12, you don't need to use
  `ACTION_CLOSE_SYSTEM_DIALOGS` in this situation. That's because, if your app
  calls
  ...
  notification drawer automatically.
* Your app targets Android 11 or lower. In addition, the user has
  ...
  buttons](/training/notify-user/build-notification#Actions), and your app is
  processing a [service](/guide/components/services) or [broadcast
  receiver](/guide/components/broadcasts) in response to that user action.
* Your app targets Android 11 or lower and has an active
  [accessibility service](/guide/topics/ui/accessibility/service). If your app
  targets Android 12 and wants to close the notification bar, use
  the
  [`GLOBAL_ACTION_DISMISS_NOTIFICATION_SHADE`](/reference/android/accessibilityservice/AccessibilityService#GLOBAL_ACTION_DISMISS_NOTIFICATION_SHADE)
...
events](/training/gestures) where an overlay obscures the app in an unsafe way.
In other words, the system blocks touches that pass through certain windows,
with [a few exceptions](#untrusted-touch-events-exceptions).
...
* Overlays that require the
  ...
  and use the `FLAG_NOT_TOUCHABLE` flag.
...
* **Interactions within your app.** Your app shows the overlay, and the overlay
  appears only when the user is interacting with your app.
* **Trusted windows.** These windows include (but aren't limited to) the
  following:

  + [Accessibility windows](/reference/android/view/WindowManager.LayoutParams#TYPE_ACCESSIBILITY_OVERLAY)
  + [Input method editor (IME) windows](/reference/android/view/WindowManager.LayoutParams#TYPE_INPUT_METHOD)
  + [Assistant windows](/reference/android/view/WindowManager.LayoutParams#FLAG_NOT_TOUCHABLE)**Note:** Windows of type
  [`TYPE_APPLICATION_OVERLAY`](/reference/android/view/WindowManager.LayoutParams#TYPE_APPLICATION_OVERLAY)
  **aren't** trusted.
* **Invisible windows.** The window's root view is
  [`GONE`](/reference/android/view/View#GONE) or
  [`INVISIBLE`](/reference/android/view/View#INVISIBLE).
* **Completely transparent windows.** The
  [`alpha`](/reference/android/view/WindowManager.LayoutParams#alpha) property
...
[Logcat](/studio/command-line/logcat) logs the following message:
...
the following [ADB command](/studio/command-line/adb) in a terminal window:
...
**Note:** The system applies the new behavior only to launcher activities that are
the root of their tasks—that is, to activities that declare an [Intent
filter](/reference/android/content/IntentFilter) with both
[`ACTION_MAIN`](/reference/android/content/Intent#ACTION_MAIN) and
[`CATEGORY_LAUNCHER`](/reference/android/content/Intent#CATEGORY_LAUNCHER). For
...
app are able to more quickly resume your app from a [warm state](/topic/performance/vitals/launch-time#warm),
instead of having to completely restart the app from a
[cold state](/topic/performance/vitals/launch-time#cold).
...
[providing custom back navigation](/guide/navigation/navigation-custom-back),
...
[`setFrameRate()`](/reference/android/view/Surface#setFrameRate(float,%20int,%20boolean))
...
calling [`getAlternativeRefreshRates()`](/reference/android/view/Display.Mode#getAlternativeRefreshRates()).
Generally, the callback [`onDisplayChanged()`](/reference/android/hardware/display/DisplayManager.DisplayListener#onDisplayChanged(int))
...
* [`isPasspointTermsAndConditionsSupported()`](/reference/android/net/wifi/WifiManager#isPasspointTermsAndConditionsSupported()):
  *Terms and conditions* is a [Passpoint](/guide/topics/connectivity/passpoint)
...
For more information, see [Wi-Fi suggestion API for internet connectivity](/guide/topics/connectivity/wifi-suggest).
...
non-SDK interface restrictions in Android 12](/about/versions/12/non-sdk-12). To learn more
about non-SDK interfaces generally, see [Restrictions on non-SDK
interfaces](/guide/app-compatibility/restrictions-non-sdk-interfaces).