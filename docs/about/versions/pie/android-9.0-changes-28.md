---
title: Behavior changes: apps targeting API level 28+  |  Android Developers
...
# Behavior changes: apps targeting API level 28+ Stay organized with collections Save and categorize content based on your preferences.
...
[Behavior changes: all apps](/about/versions/pie/android-9.0-changes-all).

## Foreground services

Apps that target Android 9 or higher and use foreground services must request
the [`FOREGROUND_SERVICE`](/reference/android/Manifest.permission#FOREGROUND_SERVICE)
permission. This is a [normal permission](/guide/topics/permissions/normal-permissions),
...
requesting [`FOREGROUND_SERVICE`](/reference/android/Manifest.permission#FOREGROUND_SERVICE),
the system throws a [`SecurityException`](/reference/java/lang/SecurityException).
...
In Android 9, [`Build.SERIAL`](/reference/android/os/Build#SERIAL) is
...
[`isCleartextTrafficPermitted()`](/reference/android/security/NetworkSecurityPolicy#isCleartextTrafficPermitted())
...
share a single [`WebView`](/reference/android/webkit/WebView) data
directory among
[multiple processes](/guide/components/processes-and-threads). Typically,
...
[`android.webkit`](/reference/android/webkit/package-summary) package, such
as [`WebView`](/reference/android/webkit/WebView) and
[`CookieManager`](/reference/android/webkit/CookieManager), in only one
process. For example, you should move all
[`Activity`](/reference/android/app/Activity) objects that use a `WebView`
into the same process. You can more strictly enforce the "one process only" rule
by calling
[`disableWebView()`](/reference/android/webkit/WebView#disableWebView()) in
...
[`WebView`](/reference/android/webkit/WebView) in more than one process,
you must assign a unique data directory suffix for each process, using the
[`WebView.setDataDirectorySuffix()`](/reference/android/webkit/WebView#setDataDirectorySuffix(java.lang.String))
...
**Note:** Even if you use `setDataDirectorySuffix()`, the system doesn't share
cookies and other web data across your app's process boundaries. If multiple
processes in your app need access to the same web data, you need to copy it
between those processes yourself. For example, you can call
[`getCookie()`](/reference/android/webkit/CookieManager#getCookie(java.lang.String))
and
[`setCookie()`](/reference/android/webkit/CookieManager#setCookie(java.lang.String,%20java.lang.String,%20android.webkit.ValueCallback%3Cjava.lang.Boolean%3E))
to manually transfer cookie data from one process to another.
...
[private data](/guide/topics/data/data-storage#filesInternal) is accessible
only by that app.

To share files with other apps, use a [content
provider](/guide/topics/providers/content-provider-basics).
...
[`NetworkStatsManager`](/reference/android/app/usage/NetworkStatsManager)
class to query for that traffic.

In particular,
[`getMultipathPreference()`](/reference/android/net/ConnectivityManager#getMultipathPreference(android.net.Network))
...
The [`ConnectivityManager.NetworkCallback`](/reference/android/net/ConnectivityManager.NetworkCallback)
...
**Note:** The `android:required="false"` attribute is required for apps that have a
minimum SDK of 23 or lower, because on devices with API levels lower than 24,
the `org.apache.http.legacy` library is not available. (On those devices, the
Apache HTTP classes are available on the bootclasspath.)
...
[WebView](/reference/android/webkit/WebView) currently disables the feature
...
[WebView](/reference/android/webkit/WebView) as opaque
