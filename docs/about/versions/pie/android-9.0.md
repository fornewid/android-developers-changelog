---
title: Android 9 features and APIs  |  Android Developers
...
# Android 9 features and APIs Stay organized with collections Save and categorize content based on your preferences.
...
[API diff report](/sdk/api_diff/28/changes) or visit the
[Android API reference](/reference/packages). Also be sure to check out
[Android 9 Behavior Changes](/about/versions/pie/android-9.0-changes-all) to learn
about areas where platform changes may affect your apps.

## Indoor positioning with Wi-Fi RTT

![New RTT APIs support indoor positioning in your apps.](/static/images/about/versions/pie/rtt-nav.png)
...
[RTT APIs](/reference/android/net/wifi/rtt/package-summary) to measure the
...
[Wi-Fi location: ranging with RTT](/guide/topics/connectivity/wifi-rtt).

## Display cutout support

![Developer options screen showing different cutout sizes](/static/images/about/versions/pie/emulator-devoptions-cutout_2x.png)
...
[`getDisplayCutout()`](/reference/android/view/WindowInsets#getDisplayCutout())
method.
...
**Note:** We recommend that you test the content display around the cutout area by
using a device or emulator running Android 9.
...
![Messaging notifications](/static/images/about/versions/pie/p-messaging.jpg)

MessagingStyle with photo attached.

![Messaging notification](/static/images/about/versions/pie/p-replies.jpg)
...
* Simplified support for conversation participants: The
  [`Person`](/reference/android/app/Person)
  class is used to identify people involved in a conversation, including their
  avatars and URIs. Many other APIs, such as
  [`addMessage()`](/reference/android/app/Notification.MessagingStyle#addMessage(java.lang.CharSequence,%20long,%20android.app.Person)),
...
* Support for images: Android 9 now displays images in Messaging Notifications
  ...
  [`setData()`](/reference/android/app/Notification.MessagingStyle.Message#setData(java.lang.String,%20android.net.Uri))
...
* Save replies as drafts: Your app can retrieve the
  [`EXTRA_REMOTE_INPUT_DRAFT`](/reference/android/app/Notification#EXTRA_REMOTE_INPUT_DRAFT)
...
* Identify if a conversation is a group conversation: You can use
  [`setGroupConversation()`](/reference/android/app/Notification.MessagingStyle#setGroupConversation(boolean))
  to purposefully identify a conversation as a group or non-group conversation.
* Set the semantic action for an intent: The
  [`setSemanticAction()`](/reference/android/app/Notification.Action.Builder#setSemanticAction(int))
...
* SmartReply: Android 9 supports the same suggested replies available in your
  messaging app. Use
  [`RemoteInput.setChoices()`](/reference/android/app/RemoteInput.Builder#setChoices(java.lang.CharSequence%5B%5D))
  to provide an array of standard responses to the user.

### Channel settings, broadcasts, and Do Not Disturb

Android 8.0 introduced [Notification Channels](/guide/topics/ui/notifiers/notifications#ManageChannels),
...
* Blocking channel groups: Users can now block entire groups of channels
  ...
  [`isBlocked()`](/reference/android/app/NotificationChannelGroup#isBlocked())
  ...
  the new
  [`getNotificationChannelGroup()`](/reference/android/app/NotificationManager#getNotificationChannelGroup(java.lang.String))
  method.
* New broadcast intent types: The Android system now sends broadcast intents
  ...
  [`NotificationManager`](/reference/android/app/NotificationManager#constants)
...
* [`NotificationManager.Policy`](/reference/android/app/NotificationManager.Policy#constants)
  has three new Do-Not-Disturb priority categories:

  + [`PRIORITY_CATEGORY_ALARMS`](/reference/android/app/NotificationManager.Policy#PRIORITY_CATEGORY_ALARMS)
    prioritizes alarms.
  + [`PRIORITY_CATEGORY_MEDIA`](/reference/android/app/NotificationManager.Policy#PRIORITY_CATEGORY_MEDIA)
    prioritizes sounds from media sources, such as media and voice navigation.
  + [`PRIORITY_CATEGORY_SYSTEM`](/reference/android/app/NotificationManager.Policy#PRIORITY_CATEGORY_SYSTEM)
    prioritizes system sounds.
* `NotificationManager.Policy` also has seven new Do-Not-Disturb constants you
  can use to suppress visual interruption:

  + [`SUPPRESSED_EFFECT_FULL_SCREEN_INTENT`](/reference/android/app/NotificationManager.Policy#SUPRESSED_EFFECT_FULL_SCREEN_INTENT)
    prevents the notification from launching full-screen activity.
  + [`SUPPRESSED_EFFECT_LIGHTS`](/reference/android/app/NotificationManager.Policy#SUPRESSED_EFFECT_LIGHTS)
    blocks notification lights.
  + [`SUPPRESSED_EFFECT_PEEK`](/reference/android/app/NotificationManager.Policy#SUPRESSED_EFFECT_PEEK)
    prevents notifications from sliding briefly into view ("peeking").
  + [`SUPPRESSED_EFFECT_STATUS_BAR`](/reference/android/app/NotificationManager.Policy#SUPRESSED_EFFECT_STATUS_BAR)
    prevents notifications from appearing in the status bar on devices that support status bars.
  + [`SUPPRESSED_EFFECT_BADGE`](/reference/android/app/NotificationManager.Policy#SUPRESSED_EFFECT_BADGE)
    blocks badges on on devices that support badging. For more information, refer to
    [Modify a notification badge](https://developer.android.com/training/notify-user/badges.html).
  + [`SUPPRESSED_EFFECT_AMBIENT`](/reference/android/app/NotificationManager.Policy#SUPRESSED_EFFECT_AMBIENT)
    blocks notifications on devices that support ambient displays.
  + [`SUPPRESSED_EFFECT_NOTIFICATION_LIST`](/reference/android/app/NotificationManager.Policy#SUPRESSED_EFFECT_NOTIFICATION_LIST)
    prevents notifications from appearing in the list view on devices that support a list view, such as notification shade or lockscreen.
...
cameras](/reference/android/hardware/camera2/CameraMetadata#REQUEST_AVAILABLE_CAPABILITIES_LOGICAL_MULTI_CAMERA).
...
parameters](/reference/android/hardware/camera2/params/SessionConfiguration)
...
support](/reference/android/hardware/camera2/CameraMetadata#CONTROL_AE_MODE_ON_EXTERNAL_FLASH)
and access to [OIS
timestamps](/reference/android/hardware/camera2/CaptureResult#STATISTICS_OIS_TIMESTAMPS)
for app-level image stabilization and special effects.

In Android 9 the [multi-camera
API](/reference/android/hardware/camera2/CameraMetadata#REQUEST_AVAILABLE_CAPABILITIES_LOGICAL_MULTI_CAMERA)
...
cameras](/reference/android/hardware/camera2/CameraCharacteristics) on
supported devices.

## ImageDecoder for drawables and bitmaps

Android 9 introduces the
[`ImageDecoder`](/reference/android/graphics/ImageDecoder)
class, which provides a modernized approach for decoding images. Use this class
instead of the [`BitmapFactory`](/reference/android/graphics/BitmapFactory)
and [`BitmapFactory.Options`](/reference/android/graphics/BitmapFactory.Options)
APIs.

`ImageDecoder` lets you create a
[`Drawable`](/reference/android/graphics/drawable/Drawable) or a
[`Bitmap`](/reference/android/graphics/Bitmap) from a byte buffer, a file,
or a URI. To decode an image, first call
[`createSource()`](/reference/android/graphics/ImageDecoder#createSource(java.nio.ByteBuffer))
with the source of the encoded image. Then, call
[`decodeDrawable()`](/reference/android/graphics/ImageDecoder#decodeDrawable(android.graphics.ImageDecoder.Source))
or [`decodeBitmap()`](/reference/android/graphics/ImageDecoder#decodeBitmap(android.graphics.ImageDecoder.Source))
by passing the [`ImageDecoder.Source`](/reference/android/graphics/ImageDecoder.Source)
object to create a [`Drawable`](/reference/android/graphics/drawable/Drawable)
or a [`Bitmap`](/reference/android/graphics/Bitmap). To change the
default settings, pass [`OnHeaderDecodedListener`](/reference/android/graphics/ImageDecoder.OnHeaderDecodedListener) to
`decodeDrawable()` or `decodeBitmap()`. `ImageDecoder` calls
[`onHeaderDecoded()`](/reference/android/graphics/ImageDecoder.OnHeaderDecodedListener#onHeaderDecoded(android.graphics.ImageDecoder,%20android.graphics.ImageDecoder.ImageInfo,%20android.graphics.ImageDecoder.Source))
...
[`AnimatedImageDrawable`](/reference/android/graphics/drawable/AnimatedImageDrawable)
class.
...
* To scale the decoded image to an exact size, pass the target dimensions into
  [`setTargetSize()`](/reference/android/graphics/ImageDecoder#setTargetSize(int,%20int)).
  You can also scale images using a sample size. Pass the sample size directly to
  [`setTargetSampleSize()`](/reference/android/graphics/ImageDecoder#setTargetSampleSize(int)).
* To crop an image within the range of the scaled image, call
  [`setCrop()`](/reference/android/graphics/ImageDecoder#setCrop(android.graphics.Rect)).
* To create a mutable bitmap, pass `true` into
  [`setMutableRequired()`](/reference/android/graphics/ImageDecoder#setMutableRequired(boolean)).

`ImageDecoder` also lets you add customized and complicated effects to an image
such as rounded corners or
circle masks. Use
[`setPostProcessor()`](/reference/android/graphics/ImageDecoder#setPostProcessor(android.graphics.PostProcessor))
with an instance of the
[`PostProcessor`](/reference/android/graphics/PostProcessor)
class to execute whatever drawing commands you want.

**Note:** When you post-process an
[`AnimatedImageDrawable`](/reference/android/graphics/drawable/AnimatedImageDrawable),
the effects appear in all frames of the animation.
...
[`ImageDecoder`](/reference/android/graphics/ImageDecoder). The following
...
File format ([HEIF](/reference/android/media/MediaFormat#MIMETYPE_IMAGE_ANDROID_HEIC)
...
[`MediaMuxer`](/reference/android/media/MediaMuxer#addTrack(android.media.MediaFormat))
and [`MediaExtractor`](/reference/android/media/MediaExtractor#getTrackFormat(int))
...
using [`ImageDecoder`](/reference/android/graphics/ImageDecoder) or
[`BitmapFactory`](/reference/android/graphics/BitmapFactory) (which obtains
a bitmap from a JPEG file). You can then use
[`HeifWriter`](/reference/androidx/heifwriter/HeifWriter) to write HEIF
still images from YUV byte buffers, or instances of
[`Surface`](/reference/android/view/Surface) or
[`Bitmap`](/reference/android/graphics/Bitmap).

Media metrics are also available from the
[`AudioTrack`](/reference/android/media/AudioTrack#getMetrics()),
[`AudioRecord`](/reference/android/media/AudioRecord#getMetrics()),
and [`MediaDrm`](/reference/android/media/MediaDrm#getMetrics()) classes.

Android 9 introduces methods to the
[`MediaDRM`](/reference/android/media/MediaDrm) class to get metrics, HDCP
...
report](/sdk/api_diff/28/changes) for details.

In Android 9, the [AAudio](/ndk/guides/audio/aaudio/aaudio) API adds
...
[`AudioEffect`](/reference/android/media/audiofx/AudioEffect) API to control the
effects.

Android 9 introduces the
[`AudioEffect`](/reference/android/media/audiofx/AudioEffect) API for
[dynamics processing](/reference/android/media/audiofx/DynamicsProcessing).
...
Beginning in Android 9, [`JobScheduler`](/reference/android/app/job/JobScheduler)
...
When adding jobs, make sure to use [`setEstimatedNetworkBytes()`](/reference/android/app/job/JobInfo.Builder#setEstimatedNetworkBytes(long,%20long)),
[`setPrefetch()`](/reference/android/app/job/JobInfo.Builder#setPrefetch(boolean)),
and [`setRequiredNetwork()`](/reference/android/app/job/JobInfo.Builder#setRequiredNetwork(android.net.NetworkRequest))
when appropriate to help
`JobScheduler` handle the work properly. When your job executes,
be sure to use the [`Network`](/reference/android/net/Network) object
returned by
[`JobParameters.getNetwork()`](/reference/android/app/job/JobParameters#getNetwork()).
...
The [Neural Networks API](/ndk/guides/neuralnetworks) was introduced
...
* Element-wise mathematical operations:
  + [`ANEURALNETWORKS_DIV`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0a139794099b4137599bbc73af18b0d42a)
  + [`ANEURALNETWORKS_SUB`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0a06a4248fe5ec71820ab95b87613780be)
* Array operations:
  + [`ANEURALNETWORKS_BATCH_TO_SPACE_ND`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0a2bdfefbdc6409b4bbcacc16c72002703)
  + [`ANEURALNETWORKS_SPACE_TO_BATCH_ND`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0a91f17c92abe95e211de39c3715acd535)
  + [`ANEURALNETWORKS_SQUEEZE`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0a1207019989837ee9d10c5b6663504933)
  + [`ANEURALNETWORKS_STRIDED_SLICE`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0a89695302f8b1e7ae7ce8f4d8c0b8a752)
  + [`ANEURALNETWORKS_TRANSPOSE`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0a92d7bc95eb68525334b6cfe80cd271ee)
  + [`ANEURALNETWORKS_PAD`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0aaced01fc41e401b81cefcf53780558d1)
  + [`ANEURALNETWORKS_MEAN`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0a047fe95a35b27f45c05432b6ca18eb6c)
...
[`ANeuralNetworksModel_relaxComputationFloat32toFloat16()`](/ndk/reference/group/neural-networks#group___neural_networks_1gab822719f98f0c92e5da3684cdaca6ba0),
...
[Autofill Framework](/guide/topics/text/autofill) guide.
...
[Android Protected
Confirmation](/training/articles/security-android-protected-confirmation)
guide.
...
[`BiometricPrompt`](/reference/android/hardware/biometrics/BiometricPrompt)
...
**Note:** Before using `BiometricPrompt` in your app, you should first use the
[`hasSystemFeature()`](/reference/android/content/pm/PackageManager#hasSystemFeature(java.lang.String))
method to ensure that the device supports `FEATURE_FINGERPRINT`, `FEATURE_IRIS`,
or `FEATURE_FACE`. If a device doesn't support biometric authentication, you can
fall back to verifying the user's PIN, pattern, or password using the
[`createConfirmDeviceCredentialIntent()`](/reference/android/app/KeyguardManager#createConfirmDeviceCredentialIntent(java.lang.CharSequence,%20java.lang.CharSequence))
method.
...
Module](/training/articles/keystore#HardwareSecurityModule).
...
securely](/training/articles/keystore#ImportingEncryptedKeys).
...
**Note:** Devices running Android 8.1 (API level 27) or lower don't support
changing the signing certificate. If your app's `minSdkVersion` is `27` or
lower, use an old signing certificate to sign your app in addition to the new
signature.

Learn more on how to rotate keys using
[`apksigner`](/studio/command-line/apksigner#usage-rotate).
...
**Note:** When the `unlockedDeviceRequired` flag is enabled, encryption and
signature verification can still happen at any time. The flag prevents **only
...
by passing `true` to the [`setUnlockedDeviceRequired()`](/reference/android/security/keystore/KeyGenParameterSpec.Builder#setUnlockedDeviceRequired(boolean))
...
* The user has [enabled
  backup](https://support.google.com/pixelphone/answer/7179901) using Android 9
  or higher.
* The user has [set a screen
  lock](https://support.google.com/android/answer/2819522) for their device
...
Service](/about/versions/pie/security/ckv-whitepaper) whitepaper.
...
conditions](/guide/topics/data/autobackup#define-device-conditions) under which
...
Backup Overview](/guide/topics/data/backup).
...
runtime using [`setAccessibilityPaneTitle()`](/reference/android/view/View#setAccessibilityPaneTitle(java.lang.CharSequence)).
For example, you could provide a title for the content area of a
[`Fragment`](/reference/android/support/v4/app/Fragment) object.

#### Heading-based navigation

If your app displays textual content that includes logical headings, set the
[`android:accessibilityHeading`](/reference/android/R.attr#accessibilityHeading)
attribute to `true` for the instances of
[`View`](/reference/android/view/View) that represent those headings. By
...
[`android:focusable`](/reference/android/R.attr#focusable) attribute to
determine when they should read a
[`ViewGroup`](/reference/android/view/ViewGroup), or a collection of
[`View`](/reference/android/view/View) objects, as a single unit. That way,
...
[`android:screenReaderFocusable`](/reference/android/R.attr#screenReaderFocusable)
...
:   Added features in the accessibility framework give you access to
    ...
    and [`ACTION_HIDE_TOOLTIP`](/reference/android/view/accessibility/AccessibilityNodeInfo.AccessibilityAction#ACTION_HIDE_TOOLTIP)
    to instruct instances of [`View`](/reference/android/view/View) to show or
    hide their tooltips.
...
:   Android 9 introduces support for two additional device actions in the
    ...
    using the
    [`GLOBAL_ACTION_LOCK_SCREEN`](/reference/android/accessibilityservice/AccessibilityService#GLOBAL_ACTION_LOCK_SCREEN)
    and [`GLOBAL_ACTION_TAKE_SCREENSHOT`](/reference/android/accessibilityservice/AccessibilityService#GLOBAL_ACTION_TAKE_SCREENSHOT)
...
The [`getSource()`](/reference/android/view/accessibility/AccessibilityRecord#getSource())
method returns the root view of the window associated with each event.

If an app has defined [accessibility pane titles](#a11y-pane-titles) for its
[`View`](/reference/android/view/View) objects, your service can recognize
...
make your app more accessible and to build accessibility services, see [Accessibility](/guide/topics/ui/accessibility).
...
changes](/about/versions/pie/android-9.0-changes-all#screen-rotation-changes).

![Rotating mobile showing new rotation mode letting users manually trigger rotation](/static/images/about/versions/pie/rotate-changes.gif)
...
* Precomputed Text: The
  [`PrecomputedText`](/reference/android/text/PrecomputedText) class improves
  ...
  the main thread.
* Magnifier: The [`Magnifier`](/reference/android/widget/Magnifier) class is a
...
* Smart Linkify: Android 9 enhances the
  [`TextClassifier`](/reference/android/view/textclassifier/TextClassifier) class,
...
**Note:** You don't need to [instrument your code](/reference/android/os/Trace) to
record traces, but doing so can help you see what parts of your app's code may
be contributing to hanging threads or UI jank.

To learn more about this tool, see [Perform on-device system
tracing](/studio/profile/systrace-on-device).