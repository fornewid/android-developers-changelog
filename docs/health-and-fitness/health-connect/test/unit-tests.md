---
title: Create unit tests using the Health Connect Testing library  |  Android health & fitness  |  Android Developers
...
# Create unit tests using the Health Connect Testing library Stay organized with collections Save and categorize content based on your preferences.
...
You can use the library to create [local unit tests](/training/testing/local-tests), which typically verify
the behavior of the classes in your app that interact with the [Health Connect
client](/reference/kotlin/androidx/health/connect/client/HealthConnectClient).
...
* An in-memory representation of records, so you can insert, remove, delete and
  read them
...
[Dependency Injection in Android](/training/dependency-injection). To know more about fakes, read
[Using test doubles in Android](/training/testing/fundamentals/test-doubles).
...
**Caution:** You should also have tests to verify correct behavior when the client
throws a SecurityException. Users can revoke permissions at any time.
...
multiple responses. See [`Stub`](/reference/kotlin/androidx/health/connect/client/testing/stubs/Stub) and [`MutableStub`](/reference/kotlin/androidx/health/connect/client/testing/stubs/MutableStub) for more information.

## Summary of edge cases

* Verify that your app behaves as expected when the client throws exceptions.
  Check the documentation of each function to figure out what exceptions you
  should check.
* Verify that every call you make to the client is preceded by the proper
  permissions check.
* Verify your pagination implementation.
* Verify what happens when you fetch multiple pages but one has an expired
  token.