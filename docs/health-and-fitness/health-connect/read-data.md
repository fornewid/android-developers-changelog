---
title: Read raw data  |  Android health & fitness  |  Android Developers
...
# Read raw data Stay organized with collections Save and categorize content based on your preferences.
...
Developer video for reading and writing data](https://www.youtube.com/watch?v=NAx7Gv_Hk7E&t=122) in Health
Connect.
...
* **Background reads**: By requesting an extra permission from the user, you
  ...
  See the complete [background read example](/health-and-fitness/guides/health-connect/develop/read-data#background-read-example).
...
To read records, create a [`ReadRecordsRequest`](/reference/kotlin/androidx/health/connect/client/request/ReadRecordsRequest) and supply
it when you call [`readRecords`](/reference/kotlin/androidx/health/connect/client/HealthConnectClient#readRecords(androidx.health.connect.client.request.ReadRecordsRequest)).
...
certain time. For an extended example with [`SensorManager`](/reference/android/hardware/SensorManager),
see the [step count](/health-and-fitness/guides/basic-fitness-app/read-step-count-data) data guide.
...
**Note:** If you're interested in obtaining calculated data such as averages and
totals, it is recommended to use [aggregation](/health-and-fitness/guides/health-connect/develop/aggregate-data).
You can set [smaller time slices](/health-and-fitness/guides/health-connect/develop/aggregate-data#buckets) whenever you aggregate.
The aggregation API also contains logic to handle duplicate records, and lessens
the chances of rate limiting.
...
user within a certain time by using [`WorkManager`](/reference/kotlin/androidx/work/WorkManager):
...
**Note:** If the user doesn't grant all of the permissions that are required for
background reads, your app should still run, and it should perform as many tasks
as it can with the permissions that the user granted.

The [`ReadRecordsRequest`](/reference/kotlin/androidx/health/connect/client/request/package-summary#ReadRecordsRequest(androidx.health.connect.client.time.TimeRangeFilter,kotlin.collections.Set,kotlin.Boolean,kotlin.Int,kotlin.String)) parameter has a default `pageSize` value of 1000.
...
[Plan to avoid rate limiting](/health-and-fitness/guides/health-connect/plan/rate-limiting).
...
The restrictions can be removed by requesting a [Read permission](/health-and-fitness/guides/health-connect/develop/read-data#read-older-data).

To read historical data, you need to indicate the package name as a
[`DataOrigin`](/reference/kotlin/androidx/health/connect/client/records/metadata/DataOrigin) object in the `dataOriginFilter` parameter of your
[`ReadRecordsRequest`](/reference/kotlin/androidx/health/connect/client/request/ReadRecordsRequest).
...
Write data](/health-and-fitness/health-connect/write-data)

[Next

Read aggregated data

arrow\_forward](/health-and-fitness/health-connect/aggregate-data)