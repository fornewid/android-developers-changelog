---
title: Write medical data  |  Android health & fitness  |  Android Developers
...
# Write medical data Stay organized with collections Save and categorize content based on your preferences.
...
3. Create a data source ([`MedicalDataSource`](/reference/kotlin/androidx/health/connect/client/records/MedicalDataSource))
4. Write a medical resource ([`MedicalResource`](/reference/kotlin/androidx/health/connect/client/records/MedicalResource))
...
A [`MedicalDataSource`](/reference/kotlin/androidx/health/connect/client/records/MedicalDataSource) in Health Connect represents a
...
* If all records originate from the same source, create one
  `MedicalDataSource`.
* If records originate from multiple sources, you can create a single
  `MedicalDataSource` if:
  + The data is reconciled.
  + All records have a unique combination of FHIR resource type and FHIR
    resource ID. Within a `MedicalDataSource`, there can only be one record
    of a FHIR resource type with any FHIR resource ID.
* Otherwise, to prevent duplicate IDs, create a `MedicalDataSource` for each
  data source.
...
* Display Name (required) - User facing display name for the data source,
  uniquely identified per writing app.
* FHIR Base URI (required) - For data coming from a FHIR server this should be
  ...
  As an example, if a client app supports [deep linking](/training/app-links/deep-linking),
...
* FHIR Version (required) - The FHIR version. Must be a
  [supported version](/health-and-fitness/guides/medical-records/data-format).
...
**Note:** Your app can only delete records that it has written. An app cannot delete
records written by another app.**Caution:** Be careful when using this API, as deleting a `MedicalDataSource` also
deletes **all** medical resource records associated with that data source.
...
A [`MedicalResource`](/reference/kotlin/androidx/health/connect/client/records/MedicalResource) in Health Connect represents an FHIR
...
* FHIR Version (required) - The FHIR version. Must be a [supported
  version](/health-and-fitness/guides/medical-records/data-format).
...
* Medical Resource type (populated automatically) - The [user-facing
  category](/health-and-fitness/guides/medical-records/data-format#medical-resource-types) of the resource, mapping to user-facing
  permissions.
...
**Note:** Your app can only delete records that it has written. An app cannot delete
records written by another app.
...
Data format](/health-and-fitness/health-connect/medical-records/data-format)

[Next

Read medical data

arrow\_forward](/health-and-fitness/health-connect/medical-records/read-data)