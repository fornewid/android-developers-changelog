...
[User Guide](https://developer.android.com/training/sign-in/biometric-auth) [Code Sample](https://github.com/android/security-samples) Authenticate with biometrics or device credentials, and perform cryptographic operations.

| Latest Update | Stable Release | Release Candidate | Beta Release | Alpha Release |
|---|---|---|---|---|
| April 22, 2026 | [1.1.0](https://developer.android.com/jetpack/androidx/releases/biometric#1.1.0) | - | - | [1.4.0-alpha07](https://developer.android.com/jetpack/androidx/releases/biometric#1.4.0-alpha07) |

## Declaring dependencies

To add a dependency on Biometric, you must add the Google Maven repository to your
...
[existing issues](https://issuetracker.google.com/issues?q=componentid:559537+status:open)
...
`androidx.biometric:biometric:1.4.0-alpha07` and `androidx.biometric:biometric-compose:1.4.0-alpha07` are released. Version 1.4.0-alpha07 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/4f1927c2c3b66d0c3a6b9118974d818d2dc5a06a..df4b49eda6f6834b6bc4c8aa30a581fa577a511e/biometric).

### Version 1.4.0-alpha06

March 25, 2026

`androidx.biometric:biometric:1.4.0-alpha06` and `androidx.biometric:biometric-compose:1.4.0-alpha06` are released. Version 1.4.0-alpha06 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/ec985eed3cba8444e5aaa52a748333397a1298f3..8fc7d4fb5903eb70538401e5e147bd05b4a2d345/biometric).

**New Features**

- Multiple Fallback Options Support ([Ib078e](https://android-review.googlesource.com/#/q/Ib078e8ba84214aeb580972001f0a8a06961c9c52), [b/349218025](https://issuetracker.google.com/issues/349218025))
- Starting with Android 36.1 (Baklava), biometric prompt now supports up to four custom fallback options instead of a single negative button. Developers can provide multiple custom fallback options with associated icons (on supported Android versions) via `AuthenticationRequest.Biometric.Fallback`. Clicks on these options are now delivered via `AuthenticationResult.CustomFallbackSelected` instead of an error code.

**API Changes**

- Remove `JvmSynthetic` from `AuthenticationRequest`. ([Ia2dcb](https://android-review.googlesource.com/#/q/Ia2dcb84cbc315264889929498ba5e747468b40df), [b/434257610](https://issuetracker.google.com/issues/434257610))
- Add `KeyAgreement` support to `BiometricPrompt.CryptoObject`. ([I73029](https://android-review.googlesource.com/#/q/I7302920f722bba7a360efb93fcdae74274d710ef), [b/400115331](https://issuetracker.google.com/issues/400115331))
- Update `registerForAuthenticationResult()` to use `ComponentActivity` instead `FragmentActivity` for broader compatibility. ([Id5db6](https://android-review.googlesource.com/#/q/Id5db612b62fe6758ea095d7e816f0afb6de26c9d), [b/349213716](https://issuetracker.google.com/issues/349213716))

**Bug Fixes**

- Fixed an issue where the fingerprint icon would disappear during device rotation. ([I366f2](https://android-review.googlesource.com/#/q/I366f22df766ea29e8fad74e7370e692e37971382))
- Moved fingerprint dialog destruction to `onDestroy()`. ([I619b0](https://android-review.googlesource.com/#/q/I619b06b436a165bd3722968bc65dbd383af25898))
- Updated the `compileSdk` to version 36.1 to support new Baklava platform features. ([I9da03](https://android-review.googlesource.com/#/q/I9da037a654393c4c7e9eb3cdbc808a4d51bd69d5))

### Version 1.4.0-alpha05

December 17, 2025

`androidx.biometric:biometric:1.4.0-alpha05` and `androidx.biometric:biometric-compose:1.4.0-alpha05` are released. Version 1.4.0-alpha05 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/cd41781824511ce4d5c4a05d2df4aaaee669f0bc..ec985eed3cba8444e5aaa52a748333397a1298f3/biometric).

**New Features**

- A new `biometric-compose` library
  - The new `biometric-compose` library simplifies biometric integration in Compose-based UIs. Developers can now use the `rememberAuthenticationLauncher` function to handle authentication requests and callbacks within their composables.([I4a88d](https://android-review.googlesource.com/q/I4a88dcfb2422ade2a7efd1370198acfaa17193c5))

**API Changes**

- The [activity-result-pattern Biometric library](https://developer.android.com/reference/androidx/biometric/AuthenticationUtils) introduces new API changes:
  - The `onAuthFailure` callback has been moved to `AuthenticationResultCallback` and renamed to `onAuthAttemptFailed`. ([Ied96b](https://android-review.googlesource.com/#/q/Ied96b1a29c9e82347f08e947fb56da81356d5c5d), [/Id6a73](https://android-review.googlesource.com/#/q/Id6a7379e2ca6a2ad1a135ca751214957a2be8df7))
  - Allow apps to provide an executor for their callbacks. ([Id6a73](https://android-review.googlesource.com/#/q/Id6a7379e2ca6a2ad1a135ca751214957a2be8df7))
  - Support multiple authentication launchers in a single Activity/Fragment: This is enabled by a new internal tracking mechanism (`currentAuthenticationKey`) that correctly routes results to the specific launcher instance that requested them. ([I2848c](https://android-review.googlesource.com/q/I2848c6591f953b853f68efa152823ad959f1daee))
- Deprecate `androidx.core.hardware.fingerprint.FingerprintManagerCompat`: `androidx.core.hardware.fingerprint.FingerprintManagerCompat` now is a no-op, doing nothing or returning false. A new internal `androidx.biometric.FingerprintManagerCompat` is used within `androidx.biometric` library for backward compatibility. ([I0360b](https://android-review.googlesource.com/q/I0360b3ef856865097df80d3d98953978d1e86de7), [If5687](https://android-review.googlesource.com/q/If56875d905a17971b48d30aa07e25c5ee468792e))

**Bug Fixes**

- Fix an issue of returning lock out error in `canAuthenticate()` unintentionally ([I33ae1](https://android-review.googlesource.com/q/I33ae17cb46aec1e6aa0e9b5013cf5e4a61f4ca42), [b/431824303](https://issuetracker.google.com/issues/431824303))
