...
[User Guide](https://developer.android.com/jetpack/compose/tutorial) [Code Sample](https://github.com/android/compose-samples) Build animations in their Jetpack Compose applications to enrich the user experience.
...
`androidx.compose.animation:animation-*:1.13.0-alpha03` is released. Version 1.13.0-alpha03 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/f3ed195f0c9ef9eafe437351d974c9ec49ecc2ea..f78fd2b24de25ea980397a47192d2e5539a34229/compose/animation).

**API Changes**

- Removed `ExperimentalDeferredTransitionApi`. Deferred transition APIs no longer require experimental opt-in. ([Ib6430](https://android-review.googlesource.com/#/q/Ib64300bf3bce149fe8119343be539859c4ae084b))
...
- Introduced `CapturedAnimatedVisibility`, an alternative to `AnimatedVisibility` that immediately disposes child composition on exit and animates the last rendered frame via a `GraphicsLayer`. ([I6c5b3](https://android-review.googlesource.com/#/q/I6c5b336a2dd8303e5e4fa78b927ce4491ec13410))
- Renamed the `rememberTransition` overload that accepts a `DeferredTransitionState` to `rememberDeferredTransition` to disambiguate it from the standard `TransitionState` variants. ([I7bb7e](https://android-review.googlesource.com/#/q/I7bb7e398e092853a33ce0fc5bc79529b70e3d1d4))
...
- `Transition.Crossfade` and `createChildTransition` are now stable. `EnterTransition.with` has been removed (use `togetherWith`). ([I38f66](https://android-review.googlesource.com/#/q/I38f6697b35453a94229dca76718c25bfc1792638))
...
- Optimized file-level initialization of `animation` top-level value class properties by using dynamic getters. ([I88b86](https://android-review.googlesource.com/#/q/I88b861656fcd8436370a287b85eaaf6e8ceec41f))
- Optimized companion object initialization of `compose:animation` value classes (`StartOffsetType`, `ArcMode`, `SlideDirection`) by using dynamic getters. ([I4162e](https://android-review.googlesource.com/#/q/I4162e1b6751e1ebd387207c77e86622e3078c60c))
...
`androidx.compose.animation:animation-*:1.12.1` is released. Version 1.12.1 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/963bf914f78b389bdddef0da7f36bee19d897274..8ac973819d33d32e53e43f8e544388fb9de83700/compose/animation).
...
- `SeekableTransitionState` now properly handles off-thread state changes. Previously, if the state was modified off the UI thread using `Snapshot.withMutableSnapshot()`, the transition incorrectly tried to handle the update off of the UI thread.([0aba38](https://android-review.googlesource.com/#/q/0aba38e))
- Validate duration in `InfiniteRepeatableSpec` to prevent 0-duration cycles ([151b6d](https://android-review.googlesource.com/#/q/151b6d9))
- Respect `visibilityThreshold` in custom `AnimationSpec` for `animateFloatAsState` ([3a1cdc](https://android-review.googlesource.com/#/q/3a1cdc0))
