...
[existing issues](https://issuetracker.google.com/issues?q=componentid:1676744+status:open)
...
`androidx.health.connect:connect-testing:1.0.0-alpha04` is released. Version 1.0.0-alpha04 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/4c37298a97c16270c139eb812ddadaba03e23a52..a370f37882b112a391a758d12b19da606aa65a8e/health/connect/connect-testing).

**API Changes**

- Added `FakePermissionControllerOverrides` to allow tests to stub and intercept permission checks and revocations, and updated `FakePermissionController` for thread-safe in-memory storage. ([2af6e8b](https://android.googlesource.com/platform/frameworks/support/+/2af6e8bbbb6d30378bf0afd2a76cc52f63984fe1))
- Added `HealthConnectClient#getChanges(changeLogsToken, pageSize)` that allows setting the soft limit for the number of change logs returned. ([I6837d](https://android-review.googlesource.com/#/q/I6837d64fa9549566225a16e8baa25cbee2662b32))
- Updated facade class names in testing stubs (`Stub` and `MutableStub`). ([c7657ba](https://android.googlesource.com/platform/frameworks/support/+/c7657ba76490a1e3570f2580bfc8b017c7b1be99))

**Bug Fixes**

- Fixed a bug in `FakeHealthConnectClient` where open-ended `TimeRangeFilter` evaluation in `readRecords` would fail when running under non-UTC system time zones. ([Ifbc32](https://android-review.googlesource.com/#/q/Ifbc323e6d4a32e1bf667725aab4ccf402c1278a7))
...
`androidx.health.connect:connect-client:1.2.0-alpha05`, `androidx.health.connect:connect-client-external-protobuf:1.2.0-alpha05`, and `androidx.health.connect:connect-client-proto:1.2.0-alpha05` are released. Version 1.2.0-alpha05 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/df4b49eda6f6834b6bc4c8aa30a581fa577a511e..5fdfb3300e5dbcb4dfefe48c38e193a5cf1c6eac/health/connect).

**New Features**

- The minimum SDK requirement for this library is now API 24 (`minSdk 24`). ([07cc6ea](https://android.googlesource.com/platform/frameworks/support/+/07cc6ea9413a439370e4baccae250d842acff222))

**API Changes**

- Added `@IntRange` and `@FloatRange` annotations to `ExerciseSegment` repetitions, `setIndex`, and `rateOfPerceivedExertion` properties. ([If7a47](https://android-review.googlesource.com/#/q/If7a47f7d23f5179b0b77eb8023234c7d72423bd6))

**Bug Fixes**

- Secured Health Connect intents on Android versions prior to 14. ([03720de](https://android.googlesource.com/platform/frameworks/support/+/03720de80979937b0462682bd61bf8dbfbcbe215))
- Fixed a signature validation issue in Health Connect. ([17957fc](https://android.googlesource.com/platform/frameworks/support/+/17957fcb324a42599abc868ffe8204a0384b2aee))

### Version 1.2.0-alpha04

April 22, 2026

`androidx.health.connect:connect-client:1.2.0-alpha04`, `androidx.health.connect:connect-client-external-protobuf:1.2.0-alpha04`, and `androidx.health.connect:connect-client-proto:1.2.0-alpha04` are released. Version 1.2.0-alpha04 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/4f1927c2c3b66d0c3a6b9118974d818d2dc5a06a..df4b49eda6f6834b6bc4c8aa30a581fa577a511e/health/connect).

**API Changes**

- Matchmaking APIs: Introduced new methods to help users discover and connect compatible data sources that can provide the health data your app needs.
  - `checkIfMatchmakingIsPossible`: A suspend function to verify if there are any matching applications or devices available to show before launching the flow.
  - `createMatchmakingIntent`: Generates an Intent to launch a flow where users can discover and grant write permissions to apps and devices capable of providing the health record types your app reads.

### Version 1.2.0-alpha03

March 25, 2026

`androidx.health.connect:connect-client:1.2.0-alpha03`, `androidx.health.connect:connect-client-external-protobuf:1.2.0-alpha03`, and `androidx.health.connect:connect-client-proto:1.2.0-alpha03` are released. Version 1.2.0-alpha03 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/4350deab5806bf95370a4d012d7eeaa70a10be44..5c4bebf7730d4fc1064d9dfeb0f24c5833fc9649/health/connect).

**New Features**

- Introduce new fields to `ExerciseSessionRecord` and `ExerciseSegment` to support richer exercise tracking ([I3c176](https://android-review.googlesource.com/#/q/I3c1765ce940b72d3d2d339ff3773b9702eb8e441))

**API Changes**

- Add `HealthConnectClient#getChanges(changeLogsToken, pageSize)` to set a soft limit for the number of change logs returned ([I6837d](https://android-review.googlesource.com/#/q/I6837d64fa9549566225a16e8baa25cbee2662b32))
- Enable support for activity intensity for Health Connect APK ([8c7923c](https://android-review.googlesource.com/#/q/I312499639f50859fffbea14586020fbf90f3363d))
- Defer record validation checks to platform on Android U+ ([7682d18](https://android-review.googlesource.com/#/q/I81cc097511864059f4cbc0ae5dd34fde38a86a10))

**Bug Fixes**

- Fix aggregation for activity intensity and mindfulness ([0b312833](https://android-review.googlesource.com/#/q/I0a2c8b1dbeaead0823640bda8f8adfe6e06df779))
...
August 24, 2022
`androidx.health.connect:connect-client:1.0.0-alpha04` is released. [Version 1.0.0-alpha04 contains these commits.](https://android.googlesource.com/platform/frameworks/support/+log/dd1e45e8550560087f6447a34a9145048b5766f4/health/connect/connect-client)
...
[androidx.health](https://developer.android.com/jetpack/androidx/releases/health#health-connect-client-1.0.0-alpha04)
page.
