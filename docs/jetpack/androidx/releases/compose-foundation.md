...
- Adds support to specify the style of individual parts of a compound composable (such as `TextField` or `DatePicker`) directly in the style parameter of the composable. This enables, for example, creating a `TextField`-like composable that allows customizing each of the decorator boxes using a style, enabling them to be overridden at a theme level or at the call site. ([I43c0f](https://android-review.googlesource.com/#/q/I43c0f1456cae4dd8ca8ca0212b5fe35e7813e617))
...
- This change refactors the Styles API into two independent pieces: the Styles DSL and the modifiers that are configured by the DSL. The previous `Style`-based implementation is still present but will be deprecated and removed. Any uses of it should be replaced with the new `CommonStyle` based API.
- Key components introduced:
  - `CustomStyle` \& `CommonStyle`: Base interfaces and merging helpers for declaring composable styles.
  - `StyleResolver`: Resolves style properties in composition and binds to layout nodes via `Modifier.styleResolver`.
  - `Modifier scopes`: `Modifier.appearance`, `Modifier.placement`, and dynamic callback-based `Modifier.padding` and `Modifier.graphicsLayer`.
  - Property primitives: `stylePropertyOf`, `styleLocalOf`.
  - Types useful for style properties: `Breadth`, `Fill`, and `Shadows`.
- The new modifiers are useful for implementing a design system using the Styles API but are not required. A design system may use these modifiers or provide its own. ([Ieb8cc](https://android-review.googlesource.com/#/q/Ieb8ccd0560bb32a179d64a53e028a0117d903db2), [b/549921960](https://issuetracker.google.com/issues/549921960))
