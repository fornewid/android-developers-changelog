---
title: Savedstate  |  Jetpack  |  Android Developers
...
API Reference  
[androidx.savedstate](/reference/kotlin/androidx/savedstate/package-summary)
...
| --- | --- | --- | --- | --- |
| March 25, 2026 | [1.4.0](/jetpack/androidx/releases/savedstate#1.4.0) | - | - | [1.5.0-alpha02](/jetpack/androidx/releases/savedstate#1.5.0-alpha02) |

## Declaring dependencies

To add a dependency on SavedState, you must add the Google Maven repository to your
...
[existing issues](https://issuetracker.google.com/issues?q=componentid:878252%20status:open)
...
`androidx.savedstate:savedstate-*:1.5.0-alpha01` is released. Version 1.5.0-alpha01 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/56090e32a2b7028588a6dd0c73d706c6dd8dd1ee..1a508f033de883ba2853b9f9ae1853eec7010638/savedstate).

**New Features**

* Enable generic serialization for `SavedState` objects, allowing them to be encoded and decoded using standard formats like CBOR or JSON across all platforms. ([Iafda4](https://android-review.googlesource.com/#/q/Iafda41cf33e694b72f38acf2dfaf34d156b73709), [b/425919375](https://issuetracker.google.com/issues/425919375))

**API Changes**

* Add `putKotlinSerializable` and `getKotlinSerializable` extension functions to `SavedStateWriter` and `SavedStateReader` to allow directly storing and retrieving Kotlin Serializable objects using keys, streamlining the integration with `SavedState` APIs. ([Iba02e](https://android-review.googlesource.com/#/q/Iba02ed1a48285a26dd5910b98849175bbf11e8cf))
...
* Add new Kotlin Multiplatform (KMP) targets to SavedState `*-compose` artifact. Lifecycle now supports the following platforms in total: JVM (Android and Desktop), Native (Linux, iOS, watchOS, macOS, MinGW), and Web (JavaScript, WasmJS).
  ([/Idcf26](https://android-review.googlesource.com/#/q/Idcf2618b6b648bed4c29a04bc8e83589bd1bdd65))
...
* We also have included `saved`, a lazy property delegate, to make it easy to store `@Serializable` classes in a `SavedStateRegistryOwner` (e.g., `ComponentActivity`, `Fragment`, etc.) and have those classes automatically be restored across process death and recreation. Please note the `saved` delegate is lazy and will not call the `init` lambda or save anything into the `SavedStateRegistry` until it is accessed.
  ...
  ```
* There is a similar `saved` property delegate for `SavedStateHandle` added in [Lifecycle `2.9.0`](/jetpack/androidx/releases/lifecycle#2.9.0).
...
* We also have included `saved`, a lazy property delegate, to make it easy to store `@Serializable` classes in a `SavedStateRegistryOwner` (e.g., `ComponentActivity`, `Fragment`, etc.) and have those classes automatically be restored across process death and recreation. Please note the `saved` delegate is lazy and will not call the `init` lambda or save anything into the `SavedStateRegistry` until it is accessed. ([I66739](https://android-review.googlesource.com/#/q/I667391f94de531e5535435b3b05bdef8bde5f61f), [b/376027806](https://issuetracker.google.com/issues/376027806))
  ...
  ```
* There is a similar `saved` property delegate for `SavedStateHandle` added in [Lifecycle `2.9.0-alpha07`](/jetpack/androidx/releases/lifecycle#2.9.0-alpha07).
...
* `SavedState` now depends on [Lifecycle `2.6.1`](/jetpack/androidx/releases/lifecycle#2.6.1). ([c1f621](https://android.googlesource.com/platform/frameworks/support/+/c1f62187db7f83e7db1bc1c1fb0b122d7ed34ee8))
...
* `SavedStateRegistryController` now allows
  early attachment of the `SavedStateRegistry` via
  `performAttach()`. ([Ice4bf](https://android-review.googlesource.com/#/q/Ice4bf56901f54a425c61e7a224df7507ec776645))
...
* **`ViewTreeSavedStateRegistryOwner` API**: A new `ViewTreeSavedStateRegistryOwner.get(View)` API allows you to retrieve the containing `SavedStateRegistry` given a `View` instance. You must upgrade to [Activity `1.2.0`](/jetpack/androidx/releases/jetpack/androidx/releases/activity#1.2.0), [Fragment `1.3.0`](/jetpack/androidx/releases/jetpack/androidx/releases/fragment#1.3.0), and [AppCompat `1.3.0-alpha01`](/jetpack/androidx/releases/jetpack/androidx/releases/appcompat#1.3.0-alpha01) or higher to populate this correctly.
...
* A new `ViewTreeSavedStateRegistryOwner.get(View)` API allows you to retrieve the containing `SavedStateRegistry` given a `View` instance. You must upgrade to [Activity `1.2.0-alpha05`](/jetpack/androidx/releases/activity#1.2.0-alpha05), [Fragment `1.3.0-alpha05`](/jetpack/androidx/releases/fragment#1.3.0-alpha05), and [AppCompat `1.3.0-alpha01`](/jetpack/androidx/releases/appcompat#1.3.0-alpha01) to populate this correctly. ([aosp/1298679](https://android-review.googlesource.com/1298679))
