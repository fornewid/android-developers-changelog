---
title: Google Assistant plugin for Android Studio  |  Android Developers
...
# Google Assistant plugin for Android Studio Stay organized with collections Save and categorize content based on your preferences.
...
[`actions.intent.START_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/start-exercise) BII.

## Locale support

Creating previews for specific locales in the test tool varies by BII.
The page for each BII in the [built-in intent reference](/reference/app-actions/built-in-intents) provides
...
2. In the **Plugins** section, go to **Marketplace** and search for
   "Google Assistant plugin."
3. Install the tool and restart Android Studio.

You can also download the plugin directly from the
[Jetbrains public repository](//plugins.jetbrains.com/plugin/16739-google-assistant).
...
* [Sign in](/studio/intro#sign-in) to Android Studio (version 4.0 or later).
* With the same account, sign in to the Google app on your Android test device.
* With the same account, get [Play Console access](//support.google.com/googleplay/android-developer/answer/2528691)
  to the uploaded app package to be tested.
* Open the Google app on your Android test device and finish the initial
  Assistant setup process.
* Enable [device data syncing](//myaccount.google.com/deviceapps) on your test device.

## Limitations

The Google Assistant plugin has the following limitations:

* App Actions that incorporate [web inventory](/guide/app-actions/web-inventory)
  and [foreground app invocation](/guide/app-actions/foreground-app)
  can't be tested directly in the test tool or by Android debug bridge (`adb`)
  commands. To test those App Actions, first create a preview using the test
  tool. Then, trigger those App Actions by interacting with Google Assistant
  on your physical device.
* Inline inventory for the [`actions.intent.OPEN_APP_FEATURE`](/reference/app-actions/built-in-intents/common/open-app-feature)
  BII can only be tested for a period of six hours after a preview is
  created or updated. Update the test tool preview or create a new preview to
  reset the six-hour time period.
...
1. Sign in to the [Play Console](//play.google.com/apps/publish).
2. Follow the instructions under **Set up application licensing** in this
   [Play Console Help topic](//support.google.com/googleplay/android-developer/answer/6062777).

   ![Add a license tester via the Google Play Console.](/static/guide/app-actions/images/license-tester-1.png)


   **Figure 1.** Adding a license tester.

   **Note:** This step grants read and write preview access to the
   Google Account.
3. Invite the license tester’s Google Account as a Google Play Console read-only
   ...
   2. In **App Access**, ensure the **Admin (all permissions)** checkbox is
      cleared.
...
use the Google Assistant plugin to [create a preview](#preview) for the test user.
...
**Note:** Use the App Actions test tool with app builds installed to your
device from Android Studio.

The test tool creates previews based on your Google Account and app package name
([application ID](/studio/build/application-id)), so you can test multiple apps with the same
...
**Note:** If you don't have an active preview, then you must create one before you
can use the plugin for testing your App Action.

To create a preview, do the following in Android Studio:

1. Open the App Actions test tool.
2. Optionally enter an app name and locale for testing. The default
   app name and locale are `test app action` and `en`.
3. Click **Create Preview**. If prompted, review and accept the App Actions
   policies and terms of service. Once your preview is created, the test tool
   window updates to display information about BIIs found in your
    [shortcuts.xml](https://developer.android.com/guide/app-actions/action-schema) file.
...
**Caution:** Changes you make to your  [shortcuts.xml](https://developer.android.com/guide/app-actions/action-schema) file don't automatically
propagate to your existing preview. You must update your preview in the test
tool before you can test new changes.
...
For example, the [`actions.intent.GET_FOOD_OBSERVATION`](/reference/app-actions/built-in-intents/health-and-fitness/get-food-observation) BII
...
2. In the **Select Target Device** section, choose the device where you want to
   trigger your App Action.
3. In the **Test App Action** section, click the **Run App Action** button.

App Actions triggered in the test tool use the displayed
[Android Debug Bridge](/studio/command-line/adb) (`adb`) command. The generated `adb` shell command
...
[Android Studio log files](//intellij-support.jetbrains.com/hc/en-us/articles/207241085-Locating-IDE-log-files), not as Logcat
...
Logs related to App Actions for your app are available in [Logcat](/studio/debug/am-logcat).
...
1. Access your Logcat log messages by clicking **Logcat** in the Android Studio
   tool window bar.
...
![App Actions test tool assistant](/static/guide/app-actions/images/test-tool-helper.png)
