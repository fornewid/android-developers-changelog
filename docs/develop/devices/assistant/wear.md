---
title: Google Assistant for Wear OS  |  Android Developers
...
# Google Assistant for Wear OS Stay organized with collections Save and categorize content based on your preferences.
...
With Assistant App Actions, Android developers can extend [Wear OS apps](/wear) to
...
and media apps](/media/implement/assistant). The following Health and Fitness BIIs are supported for
Wear OS apps:

* [`START_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/start-exercise)
* [`STOP_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/stop-exercise)
* [`PAUSE_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/pause-exercise)
* [`RESUME_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/resume-exercise)
...
App Actions are described in `shortcuts.xml` with Android [capability](/develop/ui/views/launch/shortcuts/adding-capabilities#define_capabilities_in_shortcutsxml) elements.
Capability elements pair [built-in intents](/guide/app-actions/intents) (BII), which are semantic
...
![App Actions flow](/static/develop/devices/assistant/images/app-actions-wear-flow.png)
...
2. Assistant matches the request to a pre-trained model (BII), and extracts any
   parameters found in the query that are supported by the BII.
3. In the example, Assistant matches the query to the [`PAUSE_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/pause-exercise) BII,
   and extracts the exercise name parameter, “hike.”
4. The app is triggered via its `shortcuts.xml` capability fulfillment
...
**Note:** This only applies to App Actions functionality on wearables.
[General queries to the Assistant](//assistant.google.com/platforms/wearables/) such as getting things done, home control,
and helpful info for the day are handled automatically through the Google
Assistant app.
...
1. Match the in-app functionality you want to voice enable to
   a [corresponding BII](/guide/app-actions/get-started#identify_app_functionality).
2. Declare support for Android shortcuts in your main activity
   ...
   ```
3. Add an [`<intent-filter>`](/guide/topics/manifest/intent-filter-element) element to AndroidManifest.xml. This enables
   Assistant to use deep links to connect to your app's content.
4. [Create shortcuts.xml](/guide/app-actions/action-schema#overview) to provide fulfillment details for your BIIs. You use
   `capability` shortcut elements to declare to Assistant the BIIs your app
   supports. For more information, see [Add capabilities](/develop/ui/views/launch/shortcuts/adding-capabilities).
5. In `shortcuts.xml`, implement a [capability](/develop/ui/views/launch/shortcuts/adding-capabilities#define_capabilities_in_shortcutsxml) for your chosen BII. The
   following sample demonstrates a capability for the [`START_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/start-exercise) BII:
...
6. If applicable, expand support for user speech variations using an
   [inline inventory](/guide/app-actions/inline-inventory), which represents features and content in your app.
...
information, see [Google Assistant plugin for Android Studio](/guide/app-actions/test-tool). Once you have
tested your app and created a test release, you can request an
[App Actions review](/guide/app-actions/get-started#request-review) and deploy. Review the following best practices for
guidance on handling common errors.

**Note:** The App Actions review is separate from the [Google Play review](//play.google.com/console/about/guides/releasewithconfidence/#review-guidance-for-your-first-app-launch) and does
not affect your status in Google Play. While your app may pass the Google Play
review and be published, App Actions will not function for your app's users
until the App Actions review is completed.
...
[`ConfirmationActivity`](/reference/androidx/wear/activity/ConfirmationActivity) messaging.

Error case | Example user interaction | App response || Activity already ongoing | "Start my *ExerciseName*"   "Resume my *ExerciseName*" | Display error: Already ongoing activity." |
