---
title: Suggest shortcuts using the In-App Promo SDK  |  Assistant  |  Android Developers
...
# Suggest shortcuts using the In-App Promo SDK Stay organized with collections Save and categorize content based on your preferences.
...
[Grow your app with app actions](/develop/devices/assistant/grow-overview).
...
**Note:** The In-App Promo SDK doesn't support shortcuts bound to
[custom intents](/guide/app-actions/custom-intents).
...
* Extend your Android app to
  [use App Actions](/guide/app-actions/get-started).
* Include `com.google.android.googlequicksearchbox` within the `<queries>`
  ...
  ```
* Use [Android App Bundles](/guide/app-bundle) to publish your apps.
...
4. Create the suggestion using the shortcut intent. There are two methods you
   ...
   * **`createShortcutSettingsIntent`:** returns an Android intent that moves
     ...
     ```
5. Call [`startActivity`](/reference/android/app/Activity#startActivity(android.content.Intent))
   using the Android intent returned during the previous step.

   **Note:** If the shortcut you suggest conflicts with an existing shortcut, the
   user remains in the context of your app and is prompted to set a new
   utterance for the shortcut.
...
Due to
[package visibility filtering](/training/package-visibility#declare-other-apps),
...
Ensure that you
[submit your app as an Android App Bundle](/guide/app-bundle).
