---
title: Create shortcuts.xml  |  Assistant  |  Android Developers
...
# Create shortcuts.xml Stay organized with collections Save and categorize content based on your preferences.
...
query. The [built-in intents reference](/reference/app-actions/built-in-intents) lists the fields that each BII is
...
**Note:** A *fallback* `intent` that does not require a parameter to be present to
fulfill a user request is required for each `capability`. These fallback
intents improve the user experience by allowing actions to succeed even
for vague user queries like *"Hey Google, Ask ExampleApp what did I eat."*
...
2. Add a reference to `shortcuts.xml` in `AndroidManifest.xml` using a
   [`<meta-data>`](/guide/topics/manifest/meta-data-element) tag in the `Activity` that has intent
...
[Create static shortcuts](/guide/topics/ui/shortcuts/creating-shortcuts#static) in the Android developer documentation.

**Note:** If you're using an obfuscation tool for your release APK, disable
obfuscation for your app's `shortcuts.xml` file by using an allowlist. If you
don't, you're likely to encounter an error when you upload your APK to the
Google Play Console.

The Jetpack library [`androidx.core:core:1.6.0`](/jetpack/androidx/releases/core)
...
[Getting started with Android Jetpack](/jetpack/getting-started).
...
**Note:** Assistant can also display dynamic shortcuts to your users. Dynamic
shortcuts are customized for your users, and may be created and updated at
runtime. For details, see [Push dynamic shortcuts to Assistant](/guide/app-actions/dynamic-shortcuts).
...
* **Capability shortcuts**. Create shortcuts that launch an instance of your
  ...
  [`START_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/start-exercise) BII capability in your fitness app.

  These shortcuts contain `intent`, `shortLabel`, and `longLabel` attributes,
  making them eligible to be suggested and fulfilled as [chips](//material.io/components/chips/android#using-chips) in proactive
  ...
  [`<capability-binding>`](#capability-binding) tag.
* **Entity shortcuts**. Entity shortcuts provide a list of supported parameter
  ...
  [`START_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/start-exercise)
  ...
  details, see [Inline inventory for App Actions](#inline-inventory).

  **Note:** Static shortcuts declared as entity shortcuts are dropped by the
  [`LauncherApps` API](/reference/android/content/pm/LauncherApps), meaning they are available to Android launchers.
...
| --- | --- | --- |
| [`<capability>`](#capability) | `<shortcuts>` | `android:name`  `app:queryPatterns` (only applicable for [custom intents](/guide/app-actions/custom-intents)) |
| [`<intent>`](#intent) | [`<capability>`](#capability) | `android:action` (optional)  `android:targetClass` (optional)  `android:targetPackage` (optional)  `android:data` (optional) |
| [`<url-template>`](#url-template) | [`<intent>`](#intent) | `android:value` |
| [`<extra>`](#extra) | [`<intent>`](#intent) | `android:key`  `android:value`  Only applicable for [foreground app invocation](/guide/app-actions/foreground-app) |
| [`<parameter>`](#parameter) | [`<intent>`](#intent) | `android:name`  `android:key`  `android:mimeType` (only applicable for [custom intents](#custom-intents))  `android:required` (optional)  `app:shortcutMatchRequired` (optional) |
| [`<shortcut-fulfillment>`](#shortcut-fulfillment) | [`<capability>`](#capability) | Only applicable for [inline inventory](/guide/app-actions/inline-inventory) |
| [`<parameter>`](#parameter_for_shortcut-fulfillment) | [`<shortcut-fulfillment>`](#shortcut-fulfillment) | `android:name` |
| [`<slice>`](#slice) | [`<capability>`](#capability) | Only applicable for [Android Slices](#slice) |
...
[`<intent>`](#intent) to handle fulfillment of the action.

Attributes:

* `android:name`: Built-in intent Action ID (for example,
  [`actions.intent.GET_FOOD_OBSERVATION`][]). For a list of supported
  built-in intents, see the [built-in intent reference](/reference/app-actions/built-in-intents).
* `app:queryPatterns`: A string array resource of queries expected from the
  user for this intent. This attribute is only applicable to
  [custom intents](#custom-intents), as BIIs already include models of the
  common ways users express the tasks they're trying to do, or the information
  they seek.

**Note:**
App Actions built-in *intents* are distinct from Android *intents*. BIIs model
a user query for a task the user wishes to perform in your app. Android
intents, by contrast, launch destinations in your app with included
context. You declare `capability` tags to map a BII to a corresponding Android
intent, enabling users to launch in-app functionality by invoking the BII
with Assistant.

#### <intent>

Android [`intent`](#intent) element defining how a user query should be
...
* `android:action`: the intent `Action` type. Defaults to [`ACTION_VIEW`](/reference/android/content/Intent#ACTION_VIEW).
* `android:targetClass`: Target Activity class, for example:
  `"com.example.exercise.ExerciseActivity"`
* `android:targetPackage`: Package containing the target Activity class, for
  example: `"com.example.exercise`
* `android:data`: This field is overwritten by [`<url-template>`](#url-template)
  if that tag is declared in the `intent`.

**Note:** `android:targetClass` and `android:targetPackage` are required for intents
that explicitly target an Activity. The `android:data` attribute may also be
used to define a target Activity. For details, see
[Intent fulfillment options](#fulfillment-intents).

#### <url-template>

Template for constructing a [deep link](#deep-link-fulfillment) URI to
...
[Wikipedia article on URL templates](//en.wikipedia.org/wiki/URL_Template). The
template format follows the [RFC6570 URI template specification](//tools.ietf.org/html/rfc6570).
...
[URL templates in fulfillment](#url-templates-in-fulfillment).

**Note:** `<url-template>` is an optional method for targeting an Activity in an
`intent`. For details, see [Intent fulfillment options](#fulfillment-intents).
...
[Parameter data and matching](#url-templates-in-fulfillment).

Attributes:

* `android:name`: Name of the BII parameter to associate with this `intent`
  parameter. The name should be a leaf-level field of the BII parameter (for
  example, `foodObservation.aboutFood.name`).
* `android:key`: Developer-defined key of a BII parameter value. For example,
  you might define `contact_name` for the `message.recipient.name` BII
  parameter.
* `android:mimeType`: The mimeType of the parameter, such as `text/*`. This
  field is only required for parameters of [custom intents](#custom-intents).
* `android:required`: Declares whether the user query needs to include this
  parameter for this intent to be used for fulfillment. If the parameter is not
  available, Assistant attempts to fulfill the user query using the next
  `intent` defined for the `capability`.

#### <shortcut-fulfillment>

Specifies that an `intent` defined in an [inline inventory](/guide/app-actions/inline-inventory) shortcut for a
specified [parameter](#parameter_for_shortcut-fulfillment) be used for fulfillment.
For details, see [Fulfillment using shortcut intents](/guide/app-actions/inline-inventory#shortcut-fulfillment).

#### <parameter> (for `<shortcut-fulfillment>`)

Optional attribute that maps a single BII parameter to inline inventory
shortcut fulfillment. For details, see [Fulfillment using shortcut intents](/guide/app-actions/inline-inventory#shortcut-fulfillment).

Attribute:

* `android:name`: Name of the BII parameter to associate to inline inventory
  shortcut fulfillment. The name should be a leaf-level field of the BII
  parameter (for example, `menuItem.name`).

#### <slice>

Enables Assistant to embed the result of a query matching this `capability` as
an Android Slice. For details, see
[Integrate App Actions with Android Slices](/guide/app-actions/legacy/slices).
...
| Shortcuts.xml tag | Contained in | Attributes |
| --- | --- | --- |
| [`<shortcut>`](#shortcut) | `<shortcuts>` | `android:shortcutId`  `android:shortcutShortLabel`  `android:shortcutLongLabel` (optional)  `android:icon` (optional) |
| [`<intent>`](#shortcut-intent) | [`<shortcut>`](#shortcut) | `android:action`  `android:targetClass` (optional)  `android:targetPackage` (optional)  `android:data` (optional) |
| [`<capability-binding>`](#capability-binding) | [`<shortcut>`](#shortcut) | `android:key` |
| [`<parameter-binding>`](#parameter-binding) | [`<capability-binding>`](#capability-binding) | `android:key` (optional)  `android:value` |
| [`<extra>`](#shortcut-extra) | [`<shortcut>`](#shortcut) | `android:name` (optional)  `android:value`  Only applicable for [Enum parameter matching](#enum-matching). |
...
An Android [`<shortcut>`](/guide/topics/ui/shortcuts) defined in `shortcuts.xml` with certain attributes
...
[string resources](/guide/topics/resources/string-resource#String).

Attributes:

* `android:shortcutId`: Identifier for this shortcut.
* `android:shortcutShortLabel`: String resource representing a brief shortcut
  phrase. For example, `"@string/callDavidShort"` representing the value "Call
  David."
* `android:shortcutLongLabel`: String resource representing a long shortcut
  phrase. For example, `"@string/callDavidLong"` representing the value "Make an
  audio call to David."
...
`shortcut` intent attributes are identical to `capability` [`intent`](#intent)
attributes.
...
* `android:key`: The `android:name` attribute of the `capability` this
  `shortcut` is bound to. For example,
...
For more details, see [Inline inventory for App Actions](#inline-inventory).

Attributes:

* `android:key`: The name of the `capability` BII parameter to associate
  this shortcut to. For example, `exercise.name`.
* `android:value`: the `entity` value. This can be a single `entity` or a
  resource list.
...
[schema.org/Enumeration](//schema.org/Enumeration). It is required for parameter fields
whose types are subtypes of `schema.org/Enumeration` (for example:
[`MealTypeBreakfast`](//schema.googleapis.com/MealTypeBreakfast)).
...
For more details, see [Matching enumerated parameter values](#enum-matching).
...
* **Explicit intents**: Launch a specific app component by defining the
  `targetClass` and `targetPackage` attributes for the [`intent`](#intent).
...
* **Deep links**: Launch app destinations using Android deep links by defining
  a [`<url-template>`](#url-template) tag within the `intent` element. This
  method is useful if your app navigation already relies on deep links.
* **Intent data**: You can provide a fulfillment URI in the [`intent`](#intent)
  `android:data` attribute. This field is overwritten by `<url-template>` data
  if that tag is also defined within the `intent`.

**Note:** These fulfillment options are mutually exclusive. For example, if you
define an explicit [`intent`](#intent) by using the `targetClass` and
`targetPackage` attributes, you don't need to also define a
...
Alternately, you can declare a [`<url-template>`](#url-template) tag in the
`capability` that contains placeholders for dynamic parameters. This template
maps to one of your Android activities, using an [App Links URL](/training/app-links),
a custom scheme, or an [Intent-based URL](//developer.chrome.com/multidevice/android/intents).
...
[deep links](/training/app-links/deep-linking) for fulfillment. The following sample defines a `<url-template>`:
...
**Note:** If the generated URL for an `intent` using a `<url-template>` for
fulfillment is app-linked, and there are relevant intent-filters added in the
`AndroidManifest.xml` resource file, the `targetClass` and `targetActivity`
attributes don't need to be defined in the `intent`.
...
example, the [supported text values](/reference/app-actions/built-in-intents/health-and-fitness/record-food-observation#supported-text-id-values) of the [`RECORD_FOOD_OBSERVATION`](/reference/app-actions/built-in-intents/health-and-fitness/record-food-observation) BII. For
these parameters, Assistant matches the user's query ("Breakfast") to an
entity whose `sameAs` value matches the enum schema URL
([`https://schema.googleapis.com/MealTypeBreakfast`](//schema.googleapis.com/MealTypeBreakfast)). To associate enum
...
inline inventory. For details, see [Inline inventory](/guide/app-actions/inline-inventory).
...
* [`app:queryPatterns`](/guide/app-actions/custom-intents#query-patterns): Array resource that declares the
...
* `android:mimeType`: Parameter type of a custom intent. This field is
  ...
  parameters, a [supported semantic type](/guide/app-actions/custom-intents#supported-types) must be declared.

For more details, see [Custom intents](/guide/app-actions/custom-intents).