---
title: Google Assistant for Android  |  Android Developers
...
# Google Assistant for Android Stay organized with collections Save and categorize content based on your preferences.
...
**Try it out:** Have an app published to the Play Store on your device?
Launch it by telling Assistant, *"Hey Google, open AppName."* Assistant
can open your app with no integration work required from you.

App Actions enable deeper voice control, enabling users to launch your apps and
perform tasks like:

* **Launching features from Assistant**: Connect your app's capabilities to
  user queries that match predefined semantic patterns or built-in intents.
* **Displaying app information on Google surfaces**: Provide
  [Android widgets](/guide/app-actions/widgets) for Assistant to display, offering inline answers, simple
  confirmations, and brief interactions to users without changing context.
* **Suggesting voice shortcuts from Assistant**: Use Assistant to proactively
  suggest tasks in the right context for users to discover or replay.

App Actions use [built-in intents](/reference/app-actions/built-in-intents) (BIIs) to enable these and dozens more use
cases across popular task categories. See the [App Actions](#app_actions)
...
* [Navigate to the nearest restaurant on their driving route](/reference/app-actions/built-in-intents/travel/get-local-business)
* [Find the closest parking garage](/reference/app-actions/built-in-intents/travel/get-parking-facility)
* [Locate nearby EV charging stations](/reference/app-actions/built-in-intents/travel/get-charging-station)
...
shortcuts in your app with the App Actions [in-app promo SDK](/guide/app-actions/in-app-promo-sdk).
...
**Note:** App Actions, like all Actions that support built-in intents,
trigger on a number of factors, including quality and relevancy to the user's request.
Based on those factors, Google may exercise discretion in surfacing your Action in response to
user requests.
...
You declare BIIs in your app using Android [capability elements](/guide/topics/ui/shortcuts/adding-capabilities). When you
upload your app using the [Google Play console](//play.google.com/console), Google registers the
capabilities declared in your app and makes them available for users to access
from Assistant.

**Note:** The [Built-in intents reference](/reference/app-actions/built-in-intents) offers BIIs across several functional
categories, or "verticals," making it simple for developers to enable voice
control of their app by matching their app shortcuts to supported voice
intents.
...
* Assistant performs natural language analysis on the query, matching the
  ...
  the [`actions.intent.START_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/start-exercise) BII matches the query.
...
* Assistant generates an Android intent to launch the in-app destination of the
  request, using information you provide in the `<capability>`. Assistant
...
* Assistant fulfills the user request by launching the generated Android
  intent. You configure the `intent` to launch a screen in your app or to
  display a widget within Assistant.

![When a user provides a query to Google Assistant, Assistant responds
            by launching an app destination for the user.](/static/guide/app-actions/images/assistant-flow.png)


**Figure 1.** Example App Actions user query flow.

**Note:** App Actions generate intents that are used to launch in-app destinations
to fulfill user requests. An intent can specify either an explicit Android
`Activity` or an Android [deep link](/training/app-links/deep-linking) that launches an `Activity`. Apps with
navigation built around Android deep links may prefer to use these links to
fulfill an action. For more information, see [Create Shortcuts.xml](/guide/app-actions/action-schema).

After a user completes a task, you use the
[Google Shortcuts Integration Library](/guide/app-actions/dynamic-shortcuts) to push a dynamic shortcut of the
...
[App Actions BII reference](/reference/app-actions/built-in-intents).
...
**Note:** If your app [exports its activities](/guide/topics/manifest/activity-element#exported) by allowing other
on-device apps to invoke the exported activity, Assistant may automatically use
these intents to bootstrap your app's support for Assistant queries. You can
override this behavior by implementing your own App Actions as covered in this
section or opt out using the [App Actions support form](/guide/app-actions/app-actions-support).

## Test App Actions

During development and testing, you use the [Google Assistant plugin](/guide/app-actions/test-tool) for
...
Follow the [App Actions pathway](//developers.google.com/learn/pathways/app-actions) to build an App Action using our sample
Android app. Then, continue on to our guide to
[build App Actions for your own app](/guide/app-actions/get-started). You can also explore
these additional resources for building App Actions:

* Download and explore our [sample fitness Android app on GitHub](//github.com/actions-on-google/appactions-fitness-kotlin).
* [r/GoogleAssistantDev](//www.reddit.com/r/GoogleAssistantDev): The official Reddit community for
  developers working with Google Assistant.
* If you have a programming question about App Actions, submit a post to
  [Stack Overflow](//stackoverflow.com/questions/tagged/app-actions) using the "android"
  and "app-actions" tags. Before posting, ensure your question is
  [on topic](//stackoverflow.com/help/on-topic) and that you've read the guidance for
  [how to ask a good question](//stackoverflow.com/help/how-to-ask).
* Report bugs and general issues with App Actions features in our
  [public issue tracker](//issuetracker.google.com/issues/new?component=617864&template=1257475).
