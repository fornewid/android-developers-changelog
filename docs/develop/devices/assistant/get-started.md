---
title: Build App Actions  |  Assistant  |  Android Developers
...
# Build App Actions Stay organized with collections Save and categorize content based on your preferences.
...
1. Identify the in-app functionality to trigger and its matching
   [built-in intent (BII)](/reference/app-actions/built-in-intents).
...
following the [App Actions codelab](//codelabs.developers.google.com/codelabs/appactions/).

**Caution:** Due to regulatory restrictions, apps participating in the [Designed
for Families](https://support.google.com/googleplay/android-developer/answer/9893335) (DFF) program are not allowed to have App Actions. If you submit
an App Action for review for an app that is included in the DFF program, it
cannot be approved.
...
* You must have a Google Account with access to the
  [Google Play Console](//play.google.com/apps/publish/).
* Your app must be published to the Google Play Store, because App Actions are
  only available for apps published there. Also, make sure that your app is
  not intended to be used in a work profile, as App Actions are not supported
  by Managed Google Play.
* You need a physical or virtual device to test your App Actions on.
* Install the latest compatible version of Android Studio supported by the
  [Google Assistant Plugin](https://plugins.jetbrains.com/plugin/16739-google-assistant/versions).
* You must use the same Google Account to [sign in](/studio/intro#sign-in) to Android
  Studio, the Google app on your test device, and the Google Play Console.
* You must set up Assistant on your test device and test it by performing a
  touch & hold on the **Home** button.
...
[built-in intent reference](/reference/app-actions/built-in-intents) to find appropriate BIIs
...
There are [common BIIs](/reference/app-actions/built-in-intents/common) that almost any Android app can use, such as extending
your in-app search to Assistant with the [`actions.intent.GET_THING`](/reference/app-actions/built-in-intents/common/get-thing)
BII or letting users launch specific app features with their voice by
implementing the [`actions.intent.OPEN_APP_FEATURE`](/reference/app-actions/built-in-intents/common/open-app-feature) BII.

**Important:** As part of the [App Actions deployment requirements](/guide/app-actions/get-started#request-review), apps that
have a search function must implement the `actions.intent.GET_THING`
BII so that users can search for content in your app with
commonly used voice commands like *"Hey Google, search for running shoes on
Example App."*

There are also BIIs that enable *vertical*, or category-specific, use cases. For
example, an exercise app might use the [`actions.intent.START_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/start-exercise)
BII.

**Note:** For cases where there isn't a BII for your app functionality, you can use
a [custom intent](/guide/app-actions/custom-intents) to extend your app with App Actions.
...
launch an activity [explicitly](/guide/components/intents-filters#ExampleExplicit) by specifying the `targetClass` and
`targetPackage` fields. If your app already implements [Android deep link](/training/app-links/deep-linking)
...
links](#deep-links) section.

## Provide fulfillment details for built-in intents

Most of building an App Action is declaring a [capability](/guide/topics/ui/shortcuts/adding-capabilities) in the
[`shortcuts.xml`](/guide/app-actions/action-schema) resource file of your Android app where you
...
**Note:** The Jetpack library [`androidx.core:core:1.6.0`](/jetpack/androidx/releases/core) (or later) is required in your Android
project to avoid compilation errors when defining App Actions capabilities in
`shortcuts.xml`. For details, see [Getting started with Android Jetpack](/jetpack/getting-started).

For most BIIs, you extract intent parameters from the user query
based on [schema.org](//schema.org) entities. Your app then uses those BII
...
**Tip:** When your app provides multiple intents to fulfill for a single
BII, Assistant attempts to resolve them in the order they appear in within the
`capability` element.

For important details about adding App Actions to the `shortcuts.xml` file,
refer to [Create `shortcuts.xml`](/guide/app-actions/action-schema). That page also
describes how to specify the parameter values your app expects.

## Implement the GET\_THING built-in intent
...
[implement any other BII](/guide/app-actions/intents). You can use multiple fulfillments for
...
For more details, see [Push dynamic shortcuts to Assistant](/guide/app-actions/dynamic-shortcuts).

## Preview your App Actions

During development and testing, use the
[Google Assistant plugin](/guide/app-actions/test-tool) for Android Studio to test that
...
**Note:** You can also add additional testers for your App Actions
integration, even if they don't share the same Play console account. To
learn more, see
[Add additional testers](/guide/app-actions/test-tool#additional-testers).
...
**Note:** The App Actions test tool only supports testing BIIs in the
en-US locale unless example queries for other locales are provided in the
corresponding [built-in intent reference](/reference/app-actions/built-in-intents).
...
release](//support.google.com/googleplay/android-developer/answer/9859348).

## Create a test release

When you're ready to test your App Actions with additional testers,
create an internal or closed [test release](//support.google.com/googleplay/android-developer/answer/9845334) of your app.
By default, your internal and
closed release testers can access App Actions that have already been
[reviewed and approved](#request-review).

To grant testing access to all App Actions, including unapproved actions,
instruct your testers to join the
[App Actions Development Program](//groups.google.com/g/app-actions-development-program) Google Group. Members of this
group have access to all App Actions in closed and
internal test releases without having to create previews using the
[app actions test tool](/guide/app-actions/test-tool). It can take up to three hours after joining the group
for access to become available.

**Note:** App Actions in open testing track releases are not available to users
until the actions have been reviewed and approved. For more information, see the
next section.
...
1. Accept the App Actions terms of service in the Google Play Console
   ...
   ![App Actions Terms of Service in the Google Play console.](/static/guide/app-actions/images/play_app_actions_tos.png)
2. [Upload your app](/studio/publish/upload-bundle), containing `shortcuts.xml`, to the
...
3. After you upload your app to the Play Console, Google contacts you at the
   ...
   of your App
   Actions review. You can also [contact](//support.google.com/actions-console/contact/support) Assistant Developer Support
...
**Note:** If your app is restricted based on login credentials, provide a
test login with app submission. To provide test credentials, select your app in
the Play Console, browse to **Policy > App Content**, and follow the
instructions listed for **App access**.
...
**Note:** The deep link can be based on [App Links URLs](/studio/write/app-link-indexing),
[intent-based URLs](//developer.chrome.com/multidevice/android/intents), or custom schemes, like `myapp://do.thing/param=value`.
...
* In your app manifest file, the activity has `android:exported=true`, so it can
  be launched using intents from Google Assistant.
* If using App Links URLs, follow all the steps in
...
* **Direct users to user-intended content**
  ...
  For example, implementations of the [`START_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/start-exercise) BII help users
  initiate an exercise of a certain type, for example, running or swimming. The only
  exception to this policy is when your [`OPEN_APP_FEATURE`](/reference/app-actions/built-in-intents/common/open-app-feature) BII directs users
...
* **Implement relevant App Actions BIIs**
  ...
  don't implement the [`START_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/start-exercise) BII, which is
  recommended for apps in the Heath and Fitness category.
* **Implement relevant custom intents**

  Defined queries for [custom intents](/guide/app-actions/custom-intents) relate to the app's content and
