---
title: Camera orientations  |  ChromeOS  |  Android Developers
...
![Natural orientation illustration with a phone, a laptop and an object from the observer side](/images/develop/chromeos/camera-1.avif)
...
![Natural orientation illustration with a phone, a laptop and an object from the observer side](/images/develop/chromeos/camera-2.avif)

The following illustrations show how things look from the point of view of an observer, looking at the device screen:

![Sensor orientation illustration with a phone, a laptop and an object from the observer side](/images/develop/chromeos/camera-3.avif)

Consider the following scene:

![A scene with a cute Android figurine (bugdroid)](/images/develop/chromeos/camera-4.avif)

| Phone | Laptop |
| --- | --- |
| Image illustration from looking through the back camera sensor on a phone | Image illustration from looking through the back camera sensor on a laptop |
...
![90-deg display rotation illustration with a phone, a laptop and an object from the observer side](/images/develop/chromeos/camera-9.avif)
...
* If you use [`Display#getRotation()`](/reference/android/view/Display#getRotation()), you'd be getting the **counterclockwise** rotation as mentioned in this document.
* If you use [OrientationEventListener#onOrientationChanged(int)](/reference/android/view/OrientationEventListener#onOrientationChanged(int)), you'd be getting the **clockwise** rotation instead.
...
![Combined orientation illustration with a phone and a laptop unrotated, and an object](/images/develop/chromeos/camera-12.avif)
...
![Combined orientation illustration with a phone and a laptop unrotated, and an object](/images/develop/chromeos/camera-13.avif)
...
Let's start off with a common situation - capturing a JPEG photo. In the camera2 API, you can pass [`JPEG_ORIENTATION`](/reference/android/hardware/camera2/CaptureRequest#JPEG_ORIENTATION) in the capture request to specify how much you want your output JPEGs to be rotated clockwise.
...
You can see the sample code for this logic in the [JPEG\_ORIENTATION documentation](/reference/android/hardware/camera2/CaptureRequest#JPEG_ORIENTATION). Note that `deviceOrientation` in the documentation's sample code is using the clockwise rotation of the device. Hence the signs for display rotation are reversed.
...
| --- | --- | --- |
| 0 | A portrait-shaped image with the bugdroid's head pointed upwards | A landscape-shaped image with the bugdroid's head pointed upwards |
| 90 | A landscape-shaped image with the bugdroid's head pointed upwards | A portrait-shaped image with the bugdroid's head pointed upwards |
| 180 | A portrait-shaped image with the bugdroid's head pointed upwards | A landscape-shaped image with the bugdroid's head pointed upwards |
| 270 | A landscape-shaped image with the bugdroid's head pointed upwards | A portrait-shaped image with the bugdroid's head pointed upwards |
...
![Image illustration showing a stretched bugdroid as a result of fitting a portrait-shaped preview into a landscape-shaped viewfinder](/images/develop/chromeos/camera-22.avif)
...
![Illustration of an activity whose aspect ratio is greater than the aspect ratio of the viewfinder inside](/images/develop/chromeos/camera-23.avif)
...
![Illustration of an activity whose aspect ratio is less than the aspect ratio of the viewfinder inside](/images/develop/chromeos/camera-24.avif)
...
| --- | --- | --- |
| 0 | A portrait-shaped image with the bugdroid's head pointed upwards | A landscape-shaped image with the bugdroid's head pointed upwards |
| 90 | A portrait-shaped image with the bugdroid's head pointed to the right | A landscape-shaped image with the bugdroid's head pointed to the right |
| 180 | A landscape-shaped image with the bugdroid's head pointed upwards | A portrait-shaped image with the bugdroid's head pointed upwards |
| 270 | A landscape-shaped image with the bugdroid's head pointed upwards | A portrait-shaped image with the bugdroid's head pointed upwards |
...
![Illustration of a phone with a display rotation of 90 degrees, and an object](/images/develop/chromeos/camera-33.avif)

**1. Set the size of the TextureView to be identical to the preview size chosen**

Suppose the preview size you chose is `previewWidth × previewHeight` where `previewWidth > previewHeight` (sensor output is landscape-shaped by nature). When configuring a capture session, one should call [`SurfaceTexture#setDefaultBufferSize(int width, height)`](/reference/android/graphics/SurfaceTexture#setDefaultBufferSize(int,%20int)) to specify the preview size (`previewWidth × previewHeight`).
...
![Image illustration of a portrait-shaped preview stretched to fit inside a TextureView of the same size of the preview size chosen](/images/develop/chromeos/camera-34.avif)
...
Fix stretching by utilizing [`View#setScaleX(float)`](/reference/android/view/View#setScaleX(float)) and [`View#setScaleY(float)`](/reference/android/view/View#setScaleY(float))

* setScaleX(`sourceWidth / previewWidth`)
* setScaleY(`sourceHeight / previewHeight`)

![Image illustration showing the procedure of the stretched preview being scaled back to its original dimensions](/images/develop/chromeos/camera-35.avif)
...
You can do this by [`View#setRotation(float)`](/reference/android/view/View#setRotation(float))

* setRotation(`-displayRotation`), since it does a clockwise rotation.

![Image illustration showing the procedure of the preview being rotated to match to the display orientation of the device](/images/develop/chromeos/camera-36.avif)

##### Sample

* [`PreviewView`](/reference/androidx/camera/view/PreviewView) from [camerax](/training/camerax) in Jetpack handles TextureView layout as previously described. It configures the transformation with [PreviewCorrector](https://cs.android.com/androidx/platform/frameworks/support/+/androidx-master-dev:camera/camera-view/src/main/java/androidx/camera/view/preview/transform/PreviewCorrector.java).
