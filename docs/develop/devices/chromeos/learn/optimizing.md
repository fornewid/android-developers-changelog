---
title: Optimize for ChromeOS  |  Android Developers
...
# Optimize for ChromeOS Stay organized with collections Save and categorize content based on your preferences.
...
[comprehensive list of tests](/develop/devices/chromeos/learn/tests) to learn more about testing your app for
...
their entire contents to the user, read the guidelines in [Window management](/develop/devices/chromeos/learn/window-management).

![](/static/images/develop/chromeos/resizable.png)
...
* Handle the [activity lifecycle](/develop/ui/compose/layouts/adaptive/support-multi-window-mode#lifecycle) correctly in
  multi-window mode and make sure you're continuing to update the UI, even when your app is not the
  topmost focused window.
* Make sure that your app adjusts its layout appropriately whenever the user resizes its window.
* Customize the initial dimensions of the app's window by specifying its
  [launch size](/develop/devices/chromeos/learn/window-management#initial_launch_size).
* Be aware that the
  [orientation
  of the app's root activity](/develop/devices/chromeos/learn/window-management#the_root_activity_and_orientation) affects all of its windows.
...
the top bar. For more information, see [Styles and themes](/training/material/theme).
...
* All touch scrollable surfaces scroll on mouse wheel events, as shown in
  figure 2.
* Hover states are implemented with careful judgment to improve
  UI discovery without overwhelming the user, as shown in figure 3.

![](/static/images/develop/chromeos/scroll_on_mousewheel.png)

**Figure 2.** Scrolling with mouse wheel.

![](/static/images/develop/chromeos/hover_states.png)
...
You can set the appropriate `PointerIcon` to use
when users interact with an element by applying the `pointerHoverIcon` modifier.

In your apps, show all of the following:

* I-beam pointers for text
* Resize handles at resizable layer edges
* Open- and closed-hand pointers for content that can be panned or dragged
  using a click-and-drag gesture
* Processing spinners

The `PointerIcon` class provides [constants](/reference/kotlin/androidx/compose/ui/input/pointer/PointerIcon#summary) that
...
Similarly, handle all important UI elements using [tab navigation](/develop/ui/compose/touch-input/focus).
...
![](/static/images/develop/chromeos/transversal_tabbing.png)

**Figure 4.** Transversal tabbing.

![](/static/images/develop/chromeos/hover_controls.png)
...
[Input compatibility on large screens](/guide/topics/large-screens/input-compatibility-large-screens).
...
Android [context menus](/develop/ui/compose/touch-input/input-compatibility-on-large-screens#right-click), which are another accelerator for taking users to
...
![](/static/images/develop/chromeos/contextual_menu.png)
...
functionality to your app. For more information, see [Drag](/develop/ui/compose/touch-input/input-compatibility-on-large-screens#drag_and_drop).

![](/static/images/develop/chromeos/drag_and_drop.png)
...
stylus APIs, see [Input compatibility on large screens](/guide/topics/large-screens/input-compatibility-large-screens).
...
* Improve placement of UI affordances by adapting the app to mouse usage instead
  of thumb usage.
* Optimize the size of videos and images, establish a set of maximum width
  and height for all media, and maximize readability and scannability.
...
* Avoid fullscreen modal UI. Use inline UI, such as progress indicators and
  alerts, for all non-critical actions.
* Use improved UI components, such as time and date pickers, text fields, and
  menus that are designed for mouse, keyboard, and larger screens.
* Use inline edits, additional columns, or modal UI instead of a new activity
  for a small to medium edit feature.
* Remove or modify floating action buttons (FABs) for better keyboard
  navigation. By default, a FAB positions last in transversal tabbing
  order. Instead, make it first, because it is the primary action, or
  replace it with another, higher-level affordance.

![](/static/images/develop/chromeos/responsive_layout.png)
...
[CameraX](/media/camera/camerax) library.

For more details, see the [camera preview](/media/camera/camera2/camera-preview)
issues explanation.
...
[Working
with fixed-volume devices](/guide/topics/media-apps/volume-and-earphones#fixed-volume).

### Change screen brightness

You can't adjust device brightness on ChromeOS. Calls to the
`system settings` and
`WindowManager.LayoutParams` are ignored.