---
title: Window management  |  ChromeOS  |  Android Developers
...
![An app window on different devices.](/static/images/develop/chromeos/fullscreen_and_windows.png)
...
sizes](/training/multiscreen/screensizes), then your app also works well
...
as multiple apps competing for exclusive resources like the camera or microphone
and the possibility that a visible app is not necessarily the active app. Read
[Multi-window support](/guide/topics/ui/multi-window) for information on how to
handle these issues.
...
* Use a launch size only in desktop environments.
  ...
  [`<activity>`](/guide/topics/manifest/activity-elemen):
...
**Note:** These options work only if the activity started is a root
activity. You can also do this using a springboard activity to clear
...
* Respond to configuration changes dynamically by calling
  ...
  to the activity's manifest. For more information about handling
  configuration changes, read
  [Handling configuration changes](/guide/topics/resources/runtime-changes).
* Let the system restart the activity. In this case, implement
  `onSaveInstanceState` and use the [ViewModel architecture
  component](/topic/libraries/architecture/viewmodel) to restore the previous
  saved state.

When using [Jetpack Compose](/jetpack/compose), the resizing behavior depends on
...
* The screen position might change. Always use the
  system to perform window-to-screen and screen-to-window coordinate
  transformations.
* If you are using Android's view system, your window layout
  automatically changes when its size changes.
* If you don't use the view system and take over the surface, your app must
  handle size changes on its own.
* For native apps, use the `mLastContent` members or use the content view to
  determine the initial size.
* When the app is running, listen to `onContentRectChangedNative` or
  `onGlobalLayout` events to react to size changes.
* When the app's size changes, rescale or reload layouts and artwork
...
rotation value is
[`Surface.ROTATION_0`](/reference/android/view/Surface#ROTATION_0).
...
sensors, use the [`Display.getRotation()`](/reference/android/view/Display#getRotation())
...
* If you support only one orientation, add the information to the manifest so
  ...
  Chromebooks are often convertibles, and an upside-down app is a bad user
  experience.
* Try to stay with a single selected orientation. Avoid requesting one
  orientation in the manifest and setting another programmatically later.
...
* There are window controls in Chrome to toggle between all available
  layouts. By choosing the correct orientation option, you can ensure that the
  user has the correct layout after launching the app. If an app is available
  in portrait and landscape, make it default to landscape, if possible. After
  this option is set, it is remembered on a per-app basis.
* Try to avoid unnecessary orientation changes. For example, if the activity
  ...
  user and might restart the app the app can't handle it. It's better to set
...
* Don'''t call `finish()` in your activity's `onDestroy` method. This causes the
  app to close upon resize and not restart.
* Don't use window types that aren't compatible, such as `TYPE_KEYGUARD` and
  `TYPE_APPLICATION_MEDIA`.
* Make activity restarts fast by caching objects that have been
  previously allocated.
* If you don't want the user to resize your app, specify
  `android:resizeableActivity=false` in your manifest file.
