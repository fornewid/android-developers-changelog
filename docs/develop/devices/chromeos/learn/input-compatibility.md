---
title: https://developer.android.com/develop/devices/chromeos/learn/input-compatibility
...
[Video](https://www.youtube.com/watch?v=FPuaaYpUd5s)
...
Most apps override the [`onKeyUp`](https://developer.android.com/reference/android/view/KeyEvent.Callback#onKeyUp(int,%20android.view.KeyEvent)) event and add the expected behavior for each received keycode, as follows.
...
Using `onKeyUp` prevents apps from receiving multiple events if a key is held down or released slowly. Games and apps that expect users to hold down keyboard keys can look for the [`onKeyDown`](https://developer.android.com/reference/android/view/KeyEvent.Callback#onKeyDown(int,%20android.view.KeyEvent)) event.


Depending on an app's needs, overriding `onKeyUp` for the entire Activity usually provides the needed behavior. If needed, an [`onKeyListener`](https://developer.android.com/reference/android/view/View.OnKeyListener) can be added to a specific view instead. For example an app may only listen for the Enter key in specific EditText, and not the Activity, in order to implement send functionality only when the user is typing in a chat box.


When you add keyboard support, follow the Android [Keyboard handling documentation](https://developer.android.com/training/keyboard-input/commands).
...
ChromeOS automatically handles most mouse and trackpad events so that they act like touch events on an Android phone. This includes two-finger touchpad/mouse wheel scrolling. Most apps generally only need to handle three desktop-centric events: [Right-click](https://developer.android.com/develop/devices/chromeos/learn/input-compatibility#right-click), [Hover](https://developer.android.com/develop/devices/chromeos/learn/input-compatibility#hover), and [Drag-and-Drop](https://developer.android.com/develop/devices/chromeos/learn/input-compatibility#drag-and-drop).
...
To add drag-and-drop support, follow the Android [Drag-and-Drop documentation](https://developer.android.com/guide/topics/ui/drag-drop) and take a look at this [ChromeOS blog post](https://medium.com/androiddevelopers/android-on-chrome-os-implementing-drag-drop-2cc2bdcdc621).
...
![An example of band multi-select with a mouse and pointer.](https://developer.android.com/static/images/develop/chromeos/RecyclerView-Selection-Demo.gif)
...
Chromebooks support up to four game controllers. Developers should use the standard Android [Game Controller APIs](https://developer.android.com/training/game-controllers) to handle them.


Buttons are mapped to common values following a [common mapping](https://developer.android.com/training/game-controllers/controller-input#button). Unfortunately, not all game controller manufacturers follow the same mapping conventions. You can provide a much better experience if you allow users to select different popular controller mappings.
