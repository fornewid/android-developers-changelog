---
title: Prepare the development environment  |  ChromeOS  |  Android Developers
...
# Prepare the development environment Stay organized with collections Save and categorize content based on your preferences.
...
Bridge](/studio/command-line/adb) to deploy your app and debug different interactions with
Chromebooks.
...
1. Go to **Settings** and [turn on Linux](https://developers.google.com/chromeos/app-development/develop), if you haven't
   ...
   ![Turning on Linux in ChromeOS settings.](/static/images/topic/arc/turnon_linux.gif)


   **Figure 1.** Turning on Linux in ChromeOS settings.
...
4. Toggle **Enable ADB debugging**. The Chromebook restarts.
   ...
   ![Enabling ADB debugging in Linux settings.](/static/images/topic/arc/debug_settings.gif)


   **Figure 2.** Enabling ADB debugging in Linux settings.

   **Note:** If the ADB toggle is not available after you enable Linux, or if it
   can't be toggled, you might have to factory-reset your device.
5. After the Chromebook restarts, a message lets you know that there might be
   ...
   ![Notice about non-Play Store apps after enabling ADB.](/static/images/develop/chromeos/login_notice.jpg)


   **Figure 3.** Notice about non-Play Store apps after enabling ADB.
...
After you have set up [Android Studio](/studio/install#chrome-os) and ADB, you can push your
...
![Chromebook listed in Android Studio's device menu.](/static/images/develop/chromeos/as_devices.png)


**Figure 4.** Chromebook listed in Android Studio's device menu.
...
![Authorizing ADB and running an app from Android Studio.](/static/images/develop/chromeos/run_app.gif)


**Figure 5.** Authorizing ADB and running an app from Android Studio.
...
3. An authorization dialog for USB debugging appears. Grant the authorization:
   ...
   ![USB debugging authorization dialog.](/static/images/develop/chromeos/usb_dialog.png)


   **Figure 6.** USB debugging authorization dialog.
...
![Connecting to a Chromebook and installing an APK via the terminal.](/static/images/develop/chromeos/adb_connect.gif)


**Figure 7.** Connecting to a Chromebook and installing an APK via the terminal.
...
1. Make sure you have [enabled ADB debugging](#enable-adb-debugging).
2. Get the IP address of your Chromebook using the following steps:

   * Click the clock in the bottom-right area of the screen.
   * Click the gear icon.
   * Click the network type you are connected to, such as Wi-Fi or mobile
     data, then the name of the network.
   * Take note of the IP address.

**Tip:** Another quick way to find your Chromebook's IP address is to click the
clock in the bottom right, then click the Wi-Fi icon, and finally click the Network info
button.
...
**Note:** If your network prohibits these kinds of connections, you can set up your
own router or hotspot or you can try Ethernet using a port or dongle.
...
5. Click **Allow** when prompted to allow debugging. Your ADB
   session establishes.