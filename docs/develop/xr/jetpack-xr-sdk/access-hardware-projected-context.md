---
title: Use a projected context to access AI glasses hardware  |  Android XR for Jetpack XR SDK  |  Android Developers
...
# Use a projected context to access AI glasses hardware Stay organized with collections Save and categorize content based on your preferences.
...
(instead of the phone's hardware), is to use a [projected context](/develop/xr/jetpack-xr-sdk/ai-glasses/support-different-types#projected-context).
...
[profiles](/develop/connectivity/bluetooth/profiles) are supported. Using this approach lets any Android app that
...
[`RECORD_AUDIO`](/reference/kotlin/android/Manifest.permission#record_audio) permission is controlled by the phone and not the glasses.
...
* Within the projected context's scope, the AI glasses' primary,
...
**Important:** Battery power and thermal dissipation on AI glasses devices is often
...
or microphone) by using [`createHostDeviceContext(context)`](/reference/kotlin/androidx/xr/projected/ProjectedContext#createHostDeviceContext(android.content.Context)) to get the host
...
* Don't use [`getApplicationContext()`](/reference/kotlin/android/content/Context#getapplicationcontext) because the application context
  can incorrectly return the AI glasses' context if a glasses activity was the
