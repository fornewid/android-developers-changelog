---
title: https://developer.android.com/develop/xr/jetpack-xr-sdk/run/emulator/glasses-troubleshoot
url: https://developer.android.com/develop/xr/jetpack-xr-sdk/run/emulator/glasses-troubleshoot
source: md.txt
---
...
You might encounter issues while [creating virtual devices](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/create-avds/glasses) or
[running your app's augmented experiences on the emulator](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/emulator/glasses). If you do
...
While [pairing the emulators using the Pairing Assistant](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/create-avds/glasses#pair-avds), the process fails.
...
1. In the Android Studio **Device Manager** , find the [glasses AVD
...
2. Check that the glasses AVD has fully launched. You'll see a "Pair phone
   to ..." message.

   ![The glasses AVD has fully
...
3. In the Android Studio **Device Manager** , find the [phone AVD
   that you created](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/emulator/glasses) to act as a host device for the glasses emulator,
   and click **Start**.
...
9. Select the glasses AVD from the list of devices.

   ![Select the glasses AVD from the list that
...
## Emulator for glasses not in pairing mode

While [pairing the emulators](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/create-avds/glasses#pair-avds) using the Pairing Assistant or manually by
...
1. Stop the glasses emulator.
2. In the Android Studio **Device Manager** , find the glasses AVD
   and click **Wipe Data** in the overflow menu.

   ![Click "Wipe Data" to start the glasses emulator while wiping user
   data.](https://developer.android.com/static/images/develop/xr/jetpack-xr-sdk/run/ai-glasses-avd-wipe-data.webp)
3. Attempt to pair again [using the Pairing Assistant](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/create-avds/glasses#pair-avds), and then try
   [manually pairing](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/emulator/glasses-troubleshoot#pairing-assistant-fix) if that doesn't succeed.

## Paired emulators don't reconnect devices on relaunch

After [initial pairing](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/create-avds/glasses#pair-avds), the emulators don't reconnect the devices to each
other when launched.
...
1. Stop the phone emulator and the glasses emulator.
...
3. Wait for the phone emulator to fully boot, and then relaunch the glasses
   emulator.
...
5. Find the glasses device and select
   **Device details**.

6. Select **Forget \> Forget device**.

7. [Pair the devices](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/create-avds/glasses#pair-avds) again.

## Glasses won't associate with phone

During [pairing](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/create-avds/glasses#pair-avds), the glasses won't associate with the phone.

### Cause

A connectivity issue is stopping the Glasses Core app from
detecting the glasses AVD.
...
3. Find the glasses device and select **Device details**.
4. Select **Forget \> Forget device**.
5. [Pair the devices](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/create-avds/glasses#pair-avds) again.
...
Your [projected activity](https://developer.android.com/develop/xr/jetpack-xr-sdk/glasses/first-activity) won't launch on the glasses emulator.
...
devices on relaunch](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/emulator/glasses-troubleshoot#emulators-dont-reconnect).

## Can't use Gemini Live on the glasses emulator

While trying to use Gemini Live on the glasses emulator, you see the
following messages:
...
## Can't install the public glasses emulators

You can't install the glasses emulators on the latest Android Studio Canary.

### Cause

You installed the glasses emulators before it was available as part of Android Studio Canary in the [Developer Preview 3](https://android-developers.googleblog.com/2025/12/build-for-ai-glasses-with-android-xr.html).

### Fix

Uninstall your previous glasses emulators and install the public version.
...
After uninstalling the previous EAP glasses emulator, see our [documentation](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/create-avds/glasses)
to install the public glasses emulators in Android Studio Canary.

## See also

- [Create virtual devices for audio glasses and display glasses](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/create-avds/glasses)
- [Run your app's augmented experiences on the emulator](https://developer.android.com/develop/xr/jetpack-xr-sdk/run/emulator/glasses)