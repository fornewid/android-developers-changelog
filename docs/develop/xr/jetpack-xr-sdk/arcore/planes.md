...
ARCore for Jetpack XR provides the state of planes through a [`StateFlow`](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines.flow/-state-flow/) that emits the state of planes. Subscribing to planes in a session notifies your app when planes are added, updated, or removed.
