---
title: Create, control, and manage entities  |  Android XR for Jetpack XR SDK  |  Android Developers
...
# Create, control, and manage entities Stay organized with collections Save and categorize content based on your preferences.
...
[`Entity`](/reference/kotlin/androidx/xr/scenecore/Entity) instances such as [3D models](/develop/xr/jetpack-xr-sdk/add-3d-models), [stereoscopic video](/develop/xr/jetpack-xr-sdk/add-spatial-video), and
[`PanelEntity`](/reference/kotlin/androidx/xr/scenecore/PanelEntity) using Jetpack SceneCore.
...
[Session](/develop/xr/jetpack-xr-sdk/add-session) API to gain access to the scene graph. The scene graph aligns with
...
behavior to entities](#add-common) on this page.

## About the ActivitySpace

Each [`Session`](/reference/kotlin/androidx/xr/runtime/Session) has an [`ActivitySpace`](/reference/kotlin/androidx/xr/scenecore/ActivitySpace) that is automatically created
...
entities along the scene graph by setting its [`parent`](/reference/kotlin/androidx/xr/scenecore/Entity#parent()) or by using
[`addChild()`](/reference/kotlin/androidx/xr/scenecore/Entity#addChild(androidx.xr.scenecore.Entity)).
...
subclasses, like the [`GltfModelEntity`](/reference/kotlin/androidx/xr/scenecore/GltfModelEntity), have additional behaviors that
...
adjusting the [`Pose`](/reference/kotlin/androidx/xr/runtime/math/Pose) of a parent `Entity` results in all of its children
...
location is a [`Vector3`](/reference/kotlin/androidx/xr/runtime/math/Vector3) consisting of x, y, z numerical positions. The
rotation is represented by a [`Quaternion`](/reference/kotlin/androidx/xr/runtime/math/Quaternion). The position of an `Entity` is
...
**Important:** When discussing the location of an `Entity`, remember the `Entity`
can be located at a specific point in the Scene Graph and it also has a position
in 3D space.

To disable an `Entity`, use [`setEnabled()`](/reference/kotlin/androidx/xr/scenecore/Entity#setEnabled(kotlin.Boolean)). This makes it invisible and
...
To resize an `Entity` while keeping its overall shape, use [`setScale()`](/reference/kotlin/androidx/xr/scenecore/Entity#setScale(kotlin.Float)).
...
**Caution:** The scale of an `Entity` impacts the units of its coordinate system.
For example, changing the scale to 2, makes it so every unit in this entity no
longer represents 1 meter in the real world, it represents 2 meters in the real
world. This also cascades down to any children of the entity.

### Add common behavior to entities

You can use the following components to add common behavior to entities:

* [`MovableComponent`](/reference/kotlin/androidx/xr/scenecore/MovableComponent): Allows the user to move entities
* [`ResizableComponent`](/reference/kotlin/androidx/xr/scenecore/ResizableComponent): Allows the user to resize entities with
  consistent UI patterns
* [`InteractableComponent`](/reference/kotlin/androidx/xr/scenecore/InteractableComponent): Lets you capture input events for custom
  interactions
...
[`ResizableComponent.create()`](/reference/kotlin/androidx/xr/scenecore/ResizableComponent#create).

**Preview:** Currently, not all components can be used in all types of entities.
Refer to the documentation of the specific component to see which entities are
supported.

To add the specific component behavior to an `Entity` use the
[`addComponent()`](/reference/kotlin/androidx/xr/scenecore/Entity#addComponent(androidx.xr.scenecore.Component)) method.

#### Use `MovableComponent` to make an Entity user-movable

The [`MovableComponent`](/reference/kotlin/androidx/xr/scenecore/MovableComponent) allows an `Entity` to be movable by the user.
...
](/static/develop/xr/jetpack-xr-sdk/videos/movablecomponent.mp4)
...
[`MovableComponent.createSystemMovable()`](/reference/kotlin/androidx/xr/scenecore/MovableComponent#createSystemMovable(androidx.xr.runtime.Session,kotlin.Boolean)), moves your `Entity` when the
...
in a similar way to how panels are scaled by the system in [home space](/develop/xr/jetpack-xr-sdk/transition-home-space-to-full-space).
...
or ceiling. To specify anchor options, specify a set of [`AnchorPlacement`](/reference/kotlin/androidx/xr/scenecore/AnchorPlacement)
...
**Note:** If you are creating your spatialized UI with [Jetpack Compose for XR](/develop/xr/jetpack-xr-sdk/ui-compose),
you may choose to use the [`movable`](/develop/xr/jetpack-xr-sdk/subspacemodifiers#movable) subspace modifier to enable movement
of your panels.

#### Use `ResizableComponent` to make an Entity user-resizable

The [`ResizableComponent`](/reference/kotlin/androidx/xr/scenecore/ResizableComponent) allows users to resize an `Entity`. The
...
handles the update events. You can respond to different [`ResizeState`](/reference/kotlin/androidx/xr/scenecore/ResizeEvent.ResizeState)
...
ResizableComponent.kt
```

**Note:** If you are creating your spatialized UI with [Jetpack Compose for XR](/develop/xr/jetpack-xr-sdk/ui-compose),
you may choose to use the [`resizable`](/develop/xr/jetpack-xr-sdk/subspacemodifiers#resizable) subspace modifier to enable resizing
of your panels.

#### Use `InteractableComponent` to capture user input events

The [`InteractableComponent`](/reference/kotlin/androidx/xr/scenecore/InteractableComponent) lets you capture input events from the user,
...
input information provided in the [`InputEvent`](/reference/kotlin/androidx/xr/scenecore/InputEvent) parameter.

* [`InputEvent.action`](/reference/kotlin/androidx/xr/scenecore/InputEvent#action()) specifies the type of input such as [hovering](/reference/kotlin/androidx/xr/scenecore/InputEvent.Action#ACTION_HOVER_ENTER())
  or [tapping](/reference/kotlin/androidx/xr/scenecore/InputEvent.Action#ACTION_DOWN()) on an entity
* [`InputEvent.source`](/reference/kotlin/androidx/xr/scenecore/InputEvent#source()) specifies where the input came from such as
  [hand](/reference/kotlin/androidx/xr/scenecore/InputEvent.Source#SOURCE_HANDS()) or [controller](/reference/kotlin/androidx/xr/scenecore/InputEvent.Source#SOURCE_CONTROLLER()) input
* [`InputEvent.pointerType`](/reference/kotlin/androidx/xr/scenecore/InputEvent#pointerType()) specifies whether the input came from the
  right hand or left hand

For a full list of all the `InputEvent` constants, see the [reference
documentation](/reference/kotlin/androidx/xr/scenecore/InputEvent).
