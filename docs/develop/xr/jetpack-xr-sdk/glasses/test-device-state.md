---
title: Automate virtual device management with projected test rules  |  Android XR for Jetpack XR SDK  |  Android Developers
...
# Automate virtual device management with projected test rules Stay organized with collections Save and categorize content based on your preferences.
...
to verify connection states. With all the [different types](/develop/xr/jetpack-xr-sdk/ai-glasses/support-different-types),
[capabilities](/develop/xr/jetpack-xr-sdk/ai-glasses/check-capabilities), and [device states](/develop/xr/jetpack-xr-sdk/ai-glasses/check-availability) of audio glasses and display glasses,
this challenge can become even more complex. To test for this diverse set of
cases, use the [Projected Test Rule](/reference/kotlin/androidx/xr/projected/testing/ProjectedTestRule) APIs to automate the setup and
...
* The rule resets properties like `isDeviceConnected` to their defaults before
  every test. You don't need to manually reset these properties in `@After`
  methods.

### Test different device capabilities

Display glasses can show UIs built with Jetpack Compose Glimmer. By default, the
[`ProjectedTestRule`](/reference/kotlin/androidx/xr/projected/testing/ProjectedTestRule) enables `CAPABILITY_VISUAL_UI`. To define exactly what
...
* The rule resets properties like `capabilities` to their defaults before
  every test. You don't need to manually reset these properties in `@After`
  methods.
* When you need to verify behavior specifically for an activity that is
  projected to display glasses, use `launchTestProjectedDeviceActivity`.