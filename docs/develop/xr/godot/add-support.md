---
title: Add support for Android XR to an existing Godot XR project  |  Android XR for Godot  |  Android Developers
...
# Add support for Android XR to an existing Godot XR project Stay organized with collections Save and categorize content based on your preferences.
...
See a pull request to learn with a real-world example.

arrow\_forward](https://github.com/m4ym4y/museum-of-all-things/pull/154)
...
1. Update your Godot version to 4.6.2 or higher. See the documentation on
   [migrating to a new version](https://docs.godotengine.org/en/stable/tutorials/migrating/index.html) if you need additional help for
   your project.
...
3. Configure your project's settings for Android XR:
   ...
   ![Enable the ](/static/images/develop/xr/godot/use-gradle-build.png)

   1. In the **XR Features** section, select **OpenXR** for the **XR Mode**,
      and select **Enable AndroidXR Plugin**.

   ![Configure the options in the ](/static/images/develop/xr/godot/xr-features.png)
...
arrow\_forward](https://github.com/m4ym4y/museum-of-all-things/pull/153)
...
2. In the **Extensions** section, select **Hand Tracking** and **Hand
   ...
   ![Configure the options in the ](/static/images/develop/xr/godot/project-settings-extensions.png)
3. Find the **Meta** subsection in the **Extensions** section, and select
   **Hand Tracking Mesh** and **Hand Tracking Aim**.

   ![Configure the options in the ](/static/images/develop/xr/godot/project-settings-extensions-meta.png)

   **Note:** These two extensions are listed in a **Meta** section because they
   were contributed to OpenXR by Meta, but they are also supported on Android
   XR and are required to implement hand tracking.
...
1. Add three additional `XRController3D` nodes to your `XROrigin3D` node.
   ...
   1. Name one "HandTrackingLeft", and set the tracker property to
      `/user/hand_tracker/left`.
   2. Name another "HandTrackingRight", and set the tracker property to
      `/user/hand_tracker/right`.
   3. Name the last "HandTrackingAimLeft", and set the tracker property to
      `/user/fbhandaim/left`.

   If your project's original `XRController3D` nodes were named
   "XRController3D\_left" and "XRController3D\_right", your scene would look
   something like this:

   ![What your scene might look like after you've added the controller nodes.](/static/images/develop/xr/godot/controller-nodes.png)
2. [Connect the signal](https://docs.godotengine.org/en/stable/getting_started/step_by_step/signals.html#connecting-a-signal-in-the-editor) `tracking_changed` on
   **HandTrackingLeft** and **HandTrackingRight** to individual functions that
   ...
   (**XRController3D\_left** and **XRController3D\_right** in the preceding
   example).
...
5. Add `XRHandModifier3D` nodes as children to these `OpenXRFbHandTrackingMesh`
   ...
   controller nodes.](/static/images/develop/xr/godot/controller-child-nodes.png)
...
2. Delete the **Simple Controller** profile to avoid compatibility issues with
...
4. Map this profile to one or more action sets however you like.
   ...
   **Tip:** All of the open-source projects that are linked from this guide and use
   use Hand Interaction mapped Hand Interaction's **Pinch** path to a
   newly-created "pinch" action in the "godot" action set. For more
   information, see the action maps of those projects. Starting in Godot 4.7,
   the Hand Interaction profile will have [more reasonable action map
...
app handles user input with hand tracking](#optional-steps).
...
use the **HandTrackingAimLeft** node that [you added earlier](#hand-tracking-add-configure-nodes) to handle this.
...
In Android XR, [pinching is used for many foundational system actions](/design/ui/xr/guides/foundations#understanding-system), such
...
[Hand Interaction profile that you created](#hand-tracking-hand-interaction-profile) to create a virtual action:
...
* Checks if the `float` value is greater than or less than specific thresholds
  in the `XRController3D` `input_float_changed` signals.
...
tracking setup steps](#hand-tracking-add-configure-nodes)).
...
* This code relies on a `pinch_pressed` virtual action that [you can create to
  register pinches as button presses](#pinches-button-presses).
...
* While the pinch is held, the relative distance and direction from the
  original pinch location is turned to a `Vector2` and mapped virtually to the
  normal thumbstick input.
* Visual feedback of this input is also given to the player in the form of two
  billboarded quad meshes, illustrating the thumbstick position.
...
* Set the `environment_blend_mode` of the OpenXR `XRInterface` to
  `XR_ENV_BLEND_MODE_ALPHA_BLEND`.
* Set the `background_mode` of the `WorldEnvironment` node to `BG_COLOR`.
* Set the `background_color` of the `WorldEnvironment` node to any color that
...
2. In the **Androidxr** section, select **Light Estimation**.
   ...
   ![Configure the options in the ](/static/images/develop/xr/godot/project-settings-androidxr.png)
3. Add an `OpenXRAndroidLightEstimation` node to your scene tree and connect it
   ...
   ![Options for the ](/static/images/develop/xr/godot/openxr-android-light-estimation.png)
...
* When disabling light estimation, the original direction of the
  `DirectionalLight3D` must be restored manually.
* For a full example of a project using passthrough and Light Estimation,
  check out [Expedition to Blobotopia](https://gitlab.com/snopek-games/expedition-to-blobotopia) on GitLab.