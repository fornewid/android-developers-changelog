...
In Android 11 and higher, the Quick Access Device Controls feature
lets users quickly view and control external devices such as lights,
thermostats, and cameras from a user affordance within three interactions from a
default launcher. The device OEM chooses which launcher to use. Device
aggregators---for example, Google Home---and third-party vendor apps can
provide devices for display in this space. This page shows you how to display
device controls in this space and link them to your control app.

> [!NOTE]
> **Note:** To use these features, your project's `minSdk` must be `30` or higher.
...
|---|---|---|
| ![Toggle widget for device controls](https://developer.android.com/static/images/device-control/device-control-toggle.png) Toggle | ![Toggle with slider widget](https://developer.android.com/static/images/device-control/device-control-toggle-slider.png) Toggle with slider | ![Range slider widget for device controls](https://developer.android.com/static/images/device-control/device-control-range.png) Range (can't be toggled on or off) |
| ![Stateless toggle control widget](https://developer.android.com/static/images/device-control/device-control-toggle-stateless.png) Stateless toggle | ![Temperature panel widget in closed state](https://developer.android.com/static/images/device-control/device-control-panel1.png) Temperature panel (closed) |

**Figure 2.** Collection of templated widgets.

Touching and holding a widget takes you to the app for deeper control. You can
...
![Temperature panel widget in open state](https://developer.android.com/static/images/device-control/device-control-panel2.png) **Figure 3.** Open temperature panel widget.

## Create the service

This section shows how to create the
[`ControlsProviderService`](https://developer.android.com/reference/android/service/controls/ControlsProviderService). This service tells the
Android system UI that your app contains device controls that must be displayed
in the **Device controls** area of the Android UI.

The `ControlsProviderService` API assumes familiarity with reactive streams, as
defined in the [Reactive Streams GitHub project](https://github.com/reactive-streams/reactive-streams-jvm/blob/v1.0.3/README.md)
and implemented in the [Java 9 Flow interfaces](https://docs.oracle.com/javase/9/docs/api/java/util/concurrent/Flow.html).
The API builds on the following concepts:
...
- `createPublisherFor`: creates a `Publisher` for a list of given controls, as identified by their string identifiers. Use `Control.StatefulBuilder` to build these `Control` objects, because the publisher must assign a state to each control.
...
[`createPublisherForAllAvailable`](https://developer.android.com/reference/android/service/controls/ControlsProviderService#createPublisherForAllAvailable()) method to
advertise the available controls to the system. This method uses the
`Control.StatelessBuilder` builder class, because the state of each control is
unknown.

Once the controls appear in the Android UI, users can select favorite
controls.
...
After you sync your Gradle files, add the following snippet to your `Service` to
...
figure 4:
![System user interface for device controls](https://developer.android.com/static/images/ui/device_controls_ui.png) **Figure 4.** Device controls in the system menu.
...
![System menu displaying a light and thermostat control](https://developer.android.com/static/images/ui/device_controls_example_1.png) **Figure 5.** Light and thermostat controls to add.
...
thermostat controls.
![Controls displaying a light and a thermostat](https://developer.android.com/static/images/ui/device_controls_example_2.png) **Figure 6.** Light and thermostat controls.