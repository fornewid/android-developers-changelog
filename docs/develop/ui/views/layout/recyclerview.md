...
**Note:** RecyclerView is the name of both the class and the library that contains
it. On this page, `RecyclerView` in `code font` always means the class
in the RecyclerView library.
...
* **[`RecyclerView`](/reference/androidx/recyclerview/widget/RecyclerView)**
  is the [`ViewGroup`](/reference/android/view/ViewGroup) that contains the views
...
* The *layout manager* arranges the individual elements in your list. You can
  ...
  [`LayoutManager`](/reference/androidx/recyclerview/widget/RecyclerView.LayoutManager)
  abstract class.
...
options](/guide/topics/ui/layout/recyclerview-custom) that let you tailor your
...
* [`GridLayoutManager`](/reference/androidx/recyclerview/widget/GridLayoutManager)
  arranges the items in a two-dimensional grid:
  + If the grid is arranged vertically, `GridLayoutManager` tries to make all
    the elements in each row have the same width and height, but different rows
    can have different heights.
  + If the grid is arranged horizontally, `GridLayoutManager` tries to make all
    the elements in each column have the same width and height, but different
    columns can have different widths.
* [`StaggeredGridLayoutManager`](/reference/androidx/recyclerview/widget/StaggeredGridLayoutManager)
  is similar to `GridLayoutManager`, but it does not require that items in a
  row have the same height (for vertical grids) or items in the same column
  have the same width (for horizontal grids). The result is that the items
...
* [`onCreateViewHolder()`](/reference/androidx/recyclerview/widget/RecyclerView.Adapter#onCreateViewHolder(android.view.ViewGroup,%20int)):
...
* [`onBindViewHolder()`](/reference/androidx/recyclerview/widget/RecyclerView.Adapter#onBindViewHolder(VH,%20int)):
  ...
  holder's [`TextView`](/reference/android/widget/TextView) widget.
* [`getItemCount()`](/reference/androidx/recyclerview/widget/RecyclerView.Adapter#getItemCount()):
...
customization](/guide/topics/ui/layout/recyclerview-custom).

## Enable edge-to-edge display

Follow these steps to enable an edge-to-edge display for a `RecyclerView`:

* Set up a backward compatible edge-to-edge display by calling
  [`enableEdgeToEdge()`](/develop/ui/views/layout/edge-to-edge#enable-edge-to-edge-display).
* If the list items initially overlap the system bars, apply insets on the
  `RecyclerView`. You can do this by setting
  [`android:fitsSystemWindows`](/reference/android/view/View#attr_android:fitsSystemWindows)
  to `true` or by using
  [`ViewCompat.setOnApplyWindowInsetsListener`](/reference/androidx/core/view/ViewCompat#setOnApplyWindowInsetsListener(android.view.View,androidx.core.view.OnApplyWindowInsetsListener)).
* Allow the list items to draw under the system bars while scrolling by setting
  [`android:clipToPadding`](/reference/android/view/ViewGroup#attr_android:clipToPadding)
