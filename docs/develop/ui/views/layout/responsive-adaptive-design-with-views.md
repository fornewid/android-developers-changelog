---
title: Responsive/adaptive design with views  |  Views  |  Android Developers
...
# Responsive/adaptive design with views Stay organized with collections Save and categorize content based on your preferences.
...
mode](/guide/topics/ui/multi-window).
...
![](/static/images/screens_support/layout-editor_2x.png)
...
ConstraintLayout](/training/constraint-layout).
...
![](/static/images/screens_support/layout-match-parent_2x.png)
...
If you're using a [`LinearLayout`](/reference/android/widget/LinearLayout), you
can also expand the child views based on [layout
weight](/guide/topics/ui/layout/linear#Weight) so the views proportionally fill
...
`LinearLayout` to
`ConstraintLayout`](/studio/write/layout-editor#convert-constraint). Then you
can [define weighted layouts with constraint
chains](/training/constraint-layout#constrain-chain).

**Note:** When using `ConstraintLayout`, *don't use `match_parent`*. Instead, set
the dimension to `0dp` to enable a special behavior called "match constraints,"
which is generally the same as the behavior of `match_parent`. For more
information, see [Adjust the view
size](/training/constraint-layout#adjust-the-view-size) in [Build a
Responsive UI With ConstraintLayout](/training/constraint-layout).
...
[`SlidingPaneLayout`](/reference/androidx/slidingpanelayout/widget/SlidingPaneLayout)
...
**Note:** The one exception to standard `layout_weight` behavior is when using
`SlidingPaneLayout`
[v1.2.0](/jetpack/androidx/releases/slidingpanelayout#version_120_3) and later
on foldable devices, in which case `SlidingPaneLayout` automatically adjusts the
size of the panes so they are on either side of any fold or hinge.
...
qualifiers](/guide/topics/resources/providing-resources).

![](/static/images/screens_support/sizes-phone-tablet_2x.png)


**Figure 5.** The same app using different layouts for different display sizes.
...
mode](/guide/topics/ui/multi-window)) when selecting the layout for your app.

To create alternative layouts in Android Studio, see [Use layout variants to
optimize for different screens](/studio/write/layout-editor#create-variant) in
[Develop a UI with Views](/studio/write/layout-editor).
...
pixels](/training/multiscreen/screendensities#TaskUseDP) (dp).
...
![](/static/images/screens_support/layout-adaptive-breakpoints_2x.png)


**Figure 6.** Recommended width breakpoints to support different
screen sizes.
...
change](/guide/topics/resources/runtime-changes) with the new window size, which
...
resources overview](/guide/topics/resources/providing-resources).
...
* Compute your app's width and height window size classes using the
  ...
  function from the [Jetpack WindowManager](/jetpack/androidx/releases/window)
  library
* Inflate the layout resource for the current window size classes

**Note:** Most apps can implement adaptive layouts by considering only the width
window size class.

For more information, see [Window size
classes](/develop/ui/views/layouts/window-size-classes).
...
[SlidingPaneLayout](#slidingpanelayout) above) could be implemented with one
...
To learn more, see the [Fragments](/training/basics/fragments) overview.
...
WindowManager API](/reference/kotlin/androidx/window/embedding/package-summary)
calls.
...
![](/static/images/guide/topics/ui/foldables/fold-screen-ratios.png)


**Figure 7.** Various screen aspect ratios.

If you don't have access to devices for all the different screen sizes you want
to test, you can use the [Android Emulator](/studio/run/emulator) to emulate
