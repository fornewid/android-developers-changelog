---
title: Support multi-window mode  |  Views  |  Android Developers
...
* **Android 7.0** (API level 24) introduced split-screen mode on small screen
  ...
  + **Picture-in-picture mode** enables users to continue video playback
    while interacting with another app (see [Picture-in-picture support](/guide/topics/ui/picture-in-picture)).
  + **Desktop windowing mode**, in which users can freely resize each
    ...
    [`resizeableActivity="false"`](/guide/topics/manifest/application-element#resizeableActivity) to ensure the system always shows
...
* **Android 12** (API level 31) makes multi-window mode standard behavior.
  ...
  + **On large screens** ([medium](/develop/ui/views/layout/window-size-classes) or [expanded](/develop/ui/views/layout/window-size-classes) window size class), the
  ...
  + **On small screens** ([compact](/develop/ui/views/layout/window-size-classes) window size class), the system checks
...
* **Android 16** (API level 36) overrides screen orientation, aspect ratio,
  ...
  + **On large screens** (smallest width >= 600dp) the system ignores
    ...
    [App orientation, aspect ratio, and resizability](/develop/ui/compose/layouts/adaptive/app-orientation-aspect-ratio-resizability#exceptions) exceptions.
...
1. Open the [Recents screen](/guide/components/recents)
...
conjunction with [`FLAG_ACTIVITY_NEW_TASK`](/reference/kotlin/android/content/Intent#flag_activity_new_task), for example:
...
Multi-window mode does not change the [activity lifecycle](/training/basics/activity-lifecycle). However, the
...
activities remain in the [`RESUMED`](/reference/kotlin/androidx/lifecycle/Lifecycle.State#RESUMED) state when the device is in multi-window
...
[picture-in-picture mode](/guide/topics/ui/picture-in-picture). It's also possible that no activity has focus at a
given time, for example, if the notification drawer is open. The [`onStop()`](/reference/kotlin/android/app/Activity#onstop)
...
state. All other visible activities are [`STARTED`](/reference/kotlin/androidx/lifecycle/Lifecycle.State#STARTED) but are not `RESUMED`.
...
[`ON_PAUSE`](/reference/kotlin/androidx/lifecycle/Lifecycle.Event#ON_PAUSE) lifecycle event. Instead, the activity should begin playback in
response to [`ON_START`](/reference/kotlin/androidx/lifecycle/Lifecycle.Event#ON_START), and pause playback in response to [`ON_STOP`](/reference/kotlin/androidx/lifecycle/Lifecycle.Event#ON_STOP). If
you handle the lifecycle events directly instead of using the [Lifecycle](/topic/libraries/architecture/lifecycle)
package, pause video playback in your [`onStop()`](/reference/kotlin/android/app/Activity#onstop) handler, and resume
playback in [`onStart()`](/reference/kotlin/android/app/Activity#onStart()).
...
changes](/guide/topics/resources/runtime-changes). This also happens when the user resizes the app or puts the app back
...
[`windowBackground`](/reference/kotlin/android/R.attr#windowbackground) attribute or by the default
[`windowBackgroundFallback`](/reference/kotlin/android/R.attr#windowbackgroundfallback) style attribute.

## Exclusive resource access

To help support the multi-resume feature, use the
[`onTopResumedActivityChanged()`](/reference/kotlin/android/app/Activity#ontopresumedactivitychanged) lifecycle callback.
...
[`CameraManager.AvailabilityCallback#onCameraAccessPrioritiesChanged()`](/reference/kotlin/android/hardware/camera2/CameraManager.AvailabilityCallback#oncameraaccessprioritieschanged)
...
After an app receives a [`CameraDevice.StateCallback#onDisconnected()`](/reference/kotlin/android/hardware/camera2/CameraDevice.StateCallback#ondisconnected)
callback, subsequent calls on the camera device will throw a
[`CameraAccessException`](/reference/kotlin/android/hardware/camera2/CameraAccessException).

## Window metrics

Android 11 (API level 30) introduced the following [`WindowManager`](/reference/kotlin/android/view/WindowManager) methods
to provide the bounds of apps running in multi-window mode:

* [`getCurrentWindowMetrics()`](/reference/kotlin/android/view/WindowManager#getcurrentwindowmetrics): Returns a [`WindowMetrics`](/reference/kotlin/android/view/WindowMetrics) object for
  the current windowing state of the system.
* [`getMaximumWindowMetrics()`](/reference/kotlin/android/view/WindowManager#getmaximumwindowmetrics): returns `WindowMetrics` for the largest
  potential windowing state of the system.

The Jetpack WindowManager library methods [`computeCurrentWindowMetrics()`](/reference/kotlin/androidx/window/layout/WindowMetricsCalculator#computeCurrentWindowMetrics(android.app.Activity))
and [`computeMaximumWindowMetrics()`](/reference/kotlin/androidx/window/layout/WindowMetricsCalculator#computeMaximumWindowMetrics(android.app.Activity)) offer similar functionality
...
[`Display`](/reference/kotlin/android/view/Display) methods [`getSize()`](/reference/kotlin/android/view/Display#getsize) and [`getMetrics()`](/reference/kotlin/android/view/Display#getmetrics) were deprecated in
API level 30 in favor of the new `WindowManager` methods.

Android 12 (API level 31) deprecates `Display` methods [`getRealSize()`](/reference/kotlin/android/view/Display#getrealsize) and
[`getRealMetrics()`](/reference/kotlin/android/view/Display#getrealmetrics) and updates their behavior to more closely match the
behavior of `getMaximumWindowMetrics()`.

**Note:** Use [`Configuration#densityDpi`](/reference/kotlin/android/content/res/Configuration#densitydpi) instead of `getMetrics()` or
...
[filters on Google Play](/google/play/filters) to restrict your app's availability on such devices.
...
([medium](/develop/ui/views/layout/window-size-classes) or [expanded](/develop/ui/views/layout/window-size-classes) window size class), all apps run in multi-window
...
Set this attribute in your manifest's [`<activity>`](/guide/topics/manifest/activity-element) or [`<application>`](/guide/topics/manifest/application-element)
...
* **Large screens** ([medium](/develop/ui/views/layout/window-size-classes) or [expanded](/develop/ui/views/layout/window-size-classes) window size class): All apps
...
* **Small screens** ([compact](/develop/ui/views/layout/window-size-classes) window size class): If
...
user's choice of aspect ratio (see [User per-app overrides](/guide/practices/device-compatibility-mode#user_per-app_overrides)).

If you're building a game, see [App orientation, aspect ratio, and
resizability](/develop/ui/compose/layouts/adaptive/app-orientation-aspect-ratio-resizability#exceptions) to learn how you can exclude your game from the Android 16 (API
level 36) changes.

### `supportsPictureInPicture`

Set this attribute in your manifest's [`<activity>`](/guide/topics/manifest/activity-element) node to indicate whether
...
configuration changes. See [Add videos using picture-in-picture (PiP)](/training/tv/playback/picture-in-picture).

### `configChanges`

To handle multi-window configuration changes yourself, such as when a user
resizes a window, add the [`android:configChanges`](/reference/kotlin/android/R.attr#configchanges) attribute to your app
manifest [`<activity>`](/guide/topics/manifest/activity-element) node with at least the following values:
...
callback to [`onConfigurationChanged()`](/reference/kotlin/android/app/Activity#onconfigurationchanged) instead of being destroyed and
recreated. You can then manually update your views, reload resources, and
perform other operations as needed.

### `<layout>`

On Android 7.0 (API level 24) and higher, the [`<layout>`](/guide/topics/manifest/layout-element) manifest element
...
* `android:gravity`: Initial placement of the activity when launched in
  desktop windowing mode. See the [`Gravity`](/reference/kotlin/android/view/Gravity) class for suitable values.
...
[Control the system UI visibility](/training/system-ui)).

The system ignores changes to the [`android:screenOrientation`](/reference/kotlin/android/R.attr#screenorientation) attribute.

### Multi-window mode queries and callbacks

The [`Activity`](/reference/kotlin/android/app/Activity) class offers the following methods to support multi-window
mode:

* [`isInMultiWindowMode()`](/reference/kotlin/android/app/Activity#isinmultiwindowmode): Indicates whether the activity is in
  multi-window mode.
* [`isInPictureInPictureMode()`](/reference/kotlin/android/app/Activity#isinpictureinpicturemode): Indicates whether the activity is in
...
* [`onMultiWindowModeChanged()`](/reference/kotlin/android/app/Activity#onmultiwindowmodechanged): The system calls this method whenever the
...
* [`onPictureInPictureModeChanged()`](/reference/kotlin/android/app/Activity#onpictureinpicturemodechanged): The system calls this method whenever
...
The [`Fragment`](/reference/kotlin/androidx/fragment/app/Fragment) class exposes versions of many of these methods; for example,
[`Fragment.onMultiWindowModeChanged()`](/reference/kotlin/androidx/fragment/app/Fragment#onMultiWindowModeChanged(boolean)).

### Picture-in-picture mode

To put an activity in picture-in-picture mode, call
[`enterPictureInPictureMode()`](/reference/kotlin/android/app/Activity#enterpictureinpicturemode_1) This method has no effect if the device does
not support picture-in-picture mode. For more information, see [Add videos using
picture-in-picture (PiP)](/training/tv/playback/picture-in-picture).
...
[`FLAG_ACTIVITY_LAUNCH_ADJACENT`](/reference/kotlin/android/content/Intent#flag_activity_launch_adjacent), which tells the system to try to create the
...
[`ActivityOptions.setLaunchBounds()`](/reference/kotlin/android/app/ActivityOptions#setlaunchbounds). The method has no effect if the device
...
dropped content see the [`DropHelper`](/reference/kotlin/androidx/draganddrop/DropHelper) API. For comprehensive drag-and-drop
guidance, see [Enable drag and drop](/guide/topics/ui/drag-drop).
...
[multi-window attributes](#launch) to request a specific location for the new
...
activity side by side in the *same* task window in [activity embedding](/develop/ui/views/layout/activity-embedding).
...
in your launcher activity's manifest and don't use a [launch mode](/guide/topics/manifest/activity-element#lmode) that
...
[`FLAG_ACTIVITY_MULTIPLE_TASK`](/reference/kotlin/android/content/Intent#flag_activity_multiple_task) or [`FLAG_ACTIVITY_NEW_DOCUMENT`](/reference/kotlin/android/content/Intent#flag_activity_new_document) is set.
...
the list. See [Intent types](/guide/components/intents-filters#Types).

On Android 15 (API level 35) and higher,
[`PROPERTY_SUPPORTS_MULTI_INSTANCE_SYSTEM_UI`](/reference/kotlin/android/view/WindowManager#property_supports_multi_instance_system_ui) enables you to declare support
...
[list‑detail](/develop/ui/compose/layouts/adaptive/canonical-layouts#list-detail) presentation, which runs in a single window.
...
mode by checking the return value of [`Activity#isInMultiWindowMode()`](/reference/kotlin/android/app/Activity#isinmultiwindowmode).
...
* Perform several resize operations in rapid succession. Verify that your app
  ...
  usage with Memory Profiler](/studio/profile/memory-profiler)).
