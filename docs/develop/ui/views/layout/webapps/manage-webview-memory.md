...
- **Native memory footprint:** Most `WebView` memory---including rendered
  graphics, the DOM tree, and JavaScript runtime memory---is allocated in native
  memory, not on the Java heap. A Java heap dump (`.hprof`) only shows a
  lightweight Java wrapper object and does not capture the true memory used by
  web content.
...
To determine whether memory growth is caused by lingering Java wrappers or
native allocations, inspect the `Objects` section of [`dumpsys meminfo`](https://developer.android.com/tools/dumpsys):
...
- **Instance leak:** If `WebViews` or `Activities` increments on each
  ...
  wrapper or host `Activity` (for example, missing `ViewGroup.removeView()` or
  retained listener references). Because a leaked `Activity` pins its entire
  view tree and decoded image resources in memory, repeated visits will
  rapidly exhaust the Java heap and cause `OutOfMemoryError` crashes.
...
1. Find the process ID (PID) of the isolated renderer service:
   ...
   The output displays the isolated process record and its PID (for example, `22155`):
...
1. **Isolate the leak type (Java versus native):**
   ...
   - *Observation:* If `Activities` and `WebViews` counts remain steady (for example, 1--2 active instances), the app isn't leaking `Activity` contexts or Java `WebView` wrappers.
...
- [Trace WebView performance](https://developer.android.com/develop/ui/views/layout/webapps/trace-webview)