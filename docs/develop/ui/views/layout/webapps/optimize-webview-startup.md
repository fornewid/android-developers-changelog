---
title: Optimize WebView startup  |  Views  |  Android Developers
...
# Optimize WebView startup Stay organized with collections Save and categorize content based on your preferences.
...
* **Using layouts**: Calling `setContentView()` or `layoutInflater.inflate()`
  on an XML resource that includes a `<WebView>`.

Implicit startup can also negatively affect your business metrics, such as [app
startup time](/topic/performance/vitals/launch-time) and time-to-first-display. If implicit initialization isn't
optimal for your app, use [`startUpWebView`](/reference/androidx/webkit/WebViewCompat#startUpWebView(android.content.Context,androidx.webkit.WebViewStartUpConfig,androidx.webkit.WebViewCompat.WebViewStartUpCallback)) instead.
...
(such as in [`Application.onCreate`](/reference/android/app/Application#onCreate())), but don't wait for the callback to
...
bug on the [public issue tracker](//issuetracker.google.com/issues/new?component=460423).