...
| **Native \<-\> Web bridge** | **Yes.** You can use a JavaScript bridge for deep, bidirectional communication between web content and native app code. | **Limited.** You can use the [window.postMessage()](https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage) method for basic string passing. |
