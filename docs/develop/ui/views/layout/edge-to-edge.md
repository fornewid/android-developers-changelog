---
title: Display content edge-to-edge in views  |  Views  |  Android Developers
...
# Display content edge-to-edge in views Stay organized with collections Save and categorize content based on your preferences.
...
[
](/static/images/guide/navigation/e2e-intro.mp4)
...
2. Implement [adaptive layouts](/develop/ui/views/layout/responsive-adaptive-design-with-views) to optimize the user experience
   on different form factors
3. Handle any visual overlaps
4. Consider showing scrims behind the system bars

![an example of imagery behind the status bar](/static/images/guide/navigation/e2e-imagery-behind-status-bar.png)
...
**Important:**
[Edge-to-edge is enforced](/about/versions/15/behavior-changes-15#edge-to-edge)
on Android 15 (API level 35) and higher once your app targets SDK 35. If your
app is not already edge-to-edge, portions of your app may be obscured and you
must handle insets. Depending on the app, this work may or may not be
significant.
...
[`enableEdgeToEdge`](/reference/androidx/core/view/WindowCompat#enableEdgeToEdge(android.view.Window)) in `onCreate` of your `Activity`.
...
[Manually set up the edge-to-edge display](/develop/ui/views/layout/edge-to-edge-manually).
...
button](/guide/topics/ui/floating-action-button) (FAB) in figure 3 is partially
obscured by the navigation bar:

![an example of edge-to-edge implemented, but the nav bar is covering the FAB](/static/images/guide/navigation/e2e-after-implementation.png)


**Figure 3.** Navigation bar overlapping an FAB in an
edge-to-edge layout.

To avoid this kind of visual overlap in either gesture mode or button mode, you
can increase the view's margins using
[`getInsets(int)`](/reference/androidx/core/view/WindowInsetsCompat#getInsets(int))
with
[`WindowInsetsCompat.Type.systemBars()`](/reference/androidx/core/view/WindowInsetsCompat.Type#systemBars()).
...
![a translucent nav bar not covering the FAB](/static/images/guide/navigation/e2e-system-bars-top.png)


**Figure 4.** Resolving visual overlap in button
mode.

The same applies to gesture navigation mode, as shown in figure 5:

![edge-to-edge with gesture navigation](/static/images/guide/navigation/e2e-system-bars-bottom.png)


**Figure 5.** Resolving visual overlap in gesture
navigation mode.
...
![An example of system gesture insets](/static/images/guide/navigation/e2e-system-gesture-insets.png)
...
[`getInsets(int)`](/reference/androidx/core/view/WindowInsetsCompat#getInsets(int))
with
[`WindowInsetsCompat.Type.systemGestures()`](/reference/androidx/core/view/WindowInsetsCompat.Type#systemGestures()).
...
[`ViewPager2`](/reference/androidx/viewpager2/widget/ViewPager2).
...
![an example of system gesture inset measurements](/static/images/guide/navigation/e2e-system-gesture-insets-measurements.png)
...
[`BottomAppBar`](/reference/com/google/android/material/bottomappbar/BottomAppBar),
[`BottomNavigationView`](/reference/com/google/android/material/bottomnavigation/BottomNavigationView),
[`NavigationRailView`](/reference/com/google/android/material/navigationrail/NavigationRailView)
and [`NavigationView`](/reference/com/google/android/material/navigation/NavigationView)

However, [`AppBarLayout`](/reference/com/google/android/material/appbar/AppBarLayout)
doesn't automatically handle insets. Add
[`android:fitsSystemWindows="true"`](/reference/android/view/View#attr_android:fitsSystemWindows)
to handle top insets.

Read how to handle insets with
[Material Components in Compose](/develop/ui/compose/layouts/insets#material3-components).
...
[`WindowInsetsCompat.CONSUMED`](/reference/androidx/core/view/WindowInsetsCompat#CONSUMED())
...
![Broken inset dispatching example](/static/images/guide/navigation/broken_inset_dispatching.png)


**Figure 8.** Broken inset dispatching example.
Insets don't dispatch to sibling views after ViewGroup 1 consumes insets on
Android 10 (API level 29) and earlier, causing TextView 2 to overlap with
the system navigation bar. However, insets are dispatched to sibling views
on Android 11 (API level 30) and higher, as expected.
...
insets, available on
[AndroidX Core and Core-ktx 1.16.0-alpha01](/jetpack/androidx/releases/core#1.16.0-alpha01)
and higher.
...
![Fixed inset dispatching example](/static/images/guide/navigation/fixed_inset_dispatching.png)
...
Once your app targets SDK 35 or higher, [edge-to-edge is enforced](/about/versions/15/behavior-changes-15#edge-to-edge).
...
[Android system bars design guidance](/develop/ui/views/layout/responsive-adaptive-design-with-views) and [Edge-to-edge design](/design/ui/mobile/guides/layout-and-content/edge-to-edge) to
...
1. Update your `androidx-core` dependency to [1.16.0-beta01](/jetpack/androidx/releases/core#1.16.0-beta01) or higher
2. Wrap your XML layout in `androidx.core.view.insets.ProtectionLayout` and
   assign an ID.
3. Programmatically access the `ProtectionLayout` to set protections, specifying
   the side and a [`GradientProtection`](/reference/kotlin/androidx/core/view/insets/GradientProtection) for the status bar.
...
](/static/develop/ui/views/images/StatusBarProtection.mp4)
...
* If you already have your layout wrapped in a `ProtectionView`, you can pass an
  additional `ColorProtection` or `GradientProtection` to the `setProtections`
  method. Before doing so, ensure `window.isNavigationBarContrastEnforced =
  false`.
...
[download the video](/static/develop/ui/views/images/edge-to-edge/recycler-view-e2e.mp4) and watch it with a video player.
](/static/develop/ui/views/images/edge-to-edge/recycler-view-e2e.mp4)

See the code snippets in the
[Create dynamic lists with RecyclerView](/develop/ui/views/layout/recyclerview#enable-edge-to-edge-display)
section for a code sample.

### Make full-screen dialogs edge-to-edge

To make full-screen dialog's edge-to-edge, call [`enableEdgeToEdge`](/reference/androidx/core/view/WindowCompat#enableEdgeToEdge(android.view.Window)) on the Dialog.
...
* [Android system bars](/design/ui/mobile/guides/foundations/system-bars)
* [Edge-to-edge design](/design/ui/mobile/guides/layout-and-content/edge-to-edge)
