---
title: Build a responsive UI with ConstraintLayout  |  Views  |  Android Developers
...
see [Build a UI with Layout Editor](/studio/write/layout-editor).
...
![](/static/training/constraint-layout/images/constraint-fail_2x.png)

**Figure 1.** The editor shows view C below A, but it
has no vertical constraint.

![](/static/training/constraint-layout/images/constraint-fail-fixed_2x.png)
...
![](/static/studio/images/buttons/layout-editor-errors.png).
To help you avoid missing constraints, the Layout Editor automatically adds
constraints for you with the
[Autoconnect and infer constraints](#use-autoconnect-and-infer-constraints)
features.
...
1. Ensure you have the `maven.google.com` repository declared
   ...
   ```
2. Add the library as a dependency in the module-level
   `build.gradle` file, as shown in the following example. The latest
   version might be different than what is shown in the example.
...
![](/static/training/constraint-layout/images/layout-editor-convert-to-constraint_2x.png)
...
1. Open your layout in Android Studio and click the **Design** tab at the
   bottom of the editor window.
2. In the **Component Tree** window, right-click the layout and click
...
1. In the **Project** window, click the module folder and select
   **File > New > XML > Layout XML**.
2. Enter a name for the layout file and enter
...
](/static/images/training/constraint-layout/constraint-layout-constrain-left.mov)
...
3. Do one of the following:
   * Click a constraint handle and drag it to an available anchor point.
     This point can be the edge of another view, the edge of the layout, or a
     guideline. Notice that as you drag the constraint handle, the Layout
     Editor shows potential connection anchors and blue overlays.
   * Click one of the **Create a connection**
     ![](/static/studio/images/buttons/attributes-plus-icon_2x.png)
     buttons in the **Layout** section of the **Attributes** window, as shown
     in figure 4.

     ![](/static/images/training/constraint-layout/constraint-layout-create-constraint-2x.png)
...
[default margin](#adjust-the-view-margins) to separate the two views.
...
* In the **Layout** section of the **Attributes** window, click
  ...
  ![](/static/images/training/constraint-layout/constraint-layout-delete-2-2x.png)
...
](/static/images/training/constraint-layout/constraint-layout-opposing.mov)
...
[switch the size](#adjust-the-view-size) to "match constraints." If
you want to keep the current size but move the view so that it's not centered,
[adjust the constraint bias](#adjust-the-constraint-bias).
...
![](/static/training/constraint-layout/images/parent-constraint_2x.png)
...
![](/static/training/constraint-layout/images/position-constraint_2x.png)
...
**Align**
![](/static/studio/images/buttons/layout-editor-align.png)
in the toolbar to select the alignment type.

![](/static/training/constraint-layout/images/alignment-constraint_2x.png)

**Figure 9.** A horizontal alignment
constraint.

![](/static/training/constraint-layout/images/alignment-constraint-offset_2x.png)
...
![](/static/training/constraint-layout/images/baseline-constraint_2x.png)
...
![](/static/studio/images/buttons/layout-editor-guidelines.png)
...
![](/static/training/constraint-layout/images/guideline-constraint_2x.png)
...
1. Click **Guidelines**
   ![](/static/studio/images/buttons/layout-editor-guidelines.png)
   in the toolbar, and then click **Add Vertical Barrier** or
   **Add Horizontal Barrier**.
2. In the **Component Tree** window, select the views you want inside the
   barrier and drag them into the barrier component.
3. Select the barrier from the **Component Tree**, open the
   **Attributes**
   ![](/static/studio/images/buttons/window-properties.png)
   window, and then set **barrierDirection**.
...
![](/static/training/constraint-layout/images/barrier-constraint_2x.png)
...
[switch the size](#adjust-the-view-size) to "match constraints."

[

](/static/images/training/constraint-layout/constraint-layout-adjust-bias.mov)

**Video 3.** Adjusting the constraint bias.

## Adjust the view size

![](/static/images/training/constraint-layout/constraint-layout-editor-attributes-2x.png)
...
* ![](/static/studio/images/buttons/layout-width-fixed.png)
  **Fixed**: specify a specific dimension in the following text box or by
  resizing the view in the editor.
* ![](/static/studio/images/buttons/layout-width-wrap.png)
  **Wrap Content**: the view expands only as much as needed to fit its
  contents.

+ **layout\_constrainedWidth**

Set this to `true` to let the horizontal dimension change to
respect constraints. By default, a widget set to `WRAP_CONTENT`
isn't limited by constraints.

* ![](/static/studio/images/buttons/layout-width-match.png)
  **Match Constraints**: the view expands as much as possible to meet the
  constraints on each side, after accounting for the view's margins. However, you
  can modify that behavior with the following attributes and values. These
  attributes take effect only when you set the view width to "match constraints":
...
lets you [set a size ratio](#set-size-as-a-ratio).
...
![](/static/images/training/constraint-layout/constraint-layout-ratio-2x.png)
...
![](/static/images/training/constraint-layout/constraint-layout-margin-icon-2x.png)
...
![](/static/images/training/constraint-layout/constraint-layout-margin-2x.png)
...
![](/static/training/constraint-layout/images/constraint-chain_2x.png)
...
![](/static/training/constraint-layout/images/constraint-chain-styles_2x.png)
...
2. **Spread inside:** the first and last views are affixed to the
   constraints on each end of the chain, and the rest are evenly
   distributed.
3. **Weighted:** when the chain is set to **spread** or
   **spread inside**, you can fill the remaining space by setting one or more
   views to "match constraints" (`0dp`). By default, the space is
   evenly distributed between each view that's set to "match constraints," but
   you can assign a weight of importance to each view using the
   `layout_constraintHorizontal_weight` and
   `layout_constraintVertical_weight` attributes.
   This works the same way as `layout_weight` in a
   [linear layout](/guide/topics/ui/layout/linear):
   the view with the highest weight value gets the most space, and
   views that have the same weight get the same amount of space.
4. **Packed:** the views are packed together after margins are accounted
...
![](/static/studio/images/buttons/layout-editor-action-chain.png)
...
](/static/images/training/constraint-layout/constraint-layout-chain.mov)

**Video 4.** Creating a horizontal chain.

Here are a few things to consider when using chains:

* A view can be a part of both a horizontal and a vertical chain, so you can
...
* Although the orientation of a chain is vertical or horizontal, using one
  doesn't align the views in that direction. To achieve the proper position for
  each view in the chain, include other constraints, such as
...
![](/static/studio/images/buttons/layout-editor-infer.png)
...
![](/static/studio/images/buttons/layout-editor-autoconnect-on.png)
...
**Note:** `ConstraintSet` animations animate only the size and
position of child elements. They don't animate other attributes, such as color.
...
`findViewById()` calls. It also provides compile-time type safety
for code that interacts with views. Consider using view binding instead of
`findViewById()`.
