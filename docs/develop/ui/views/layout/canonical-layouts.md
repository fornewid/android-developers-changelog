---
title: Canonical layouts  |  Views  |  Android Developers
...
sure which Android APIs to use, jump to the [Applicability](#applicability)
...
[Applicability](#applicability) section for help deciding which technology is
most suitable for your app.

The [`SlidingPaneLayout`](/reference/androidx/slidingpanelayout/widget/SlidingPaneLayout) library is designed for implementation of
...
or fragments. [`ViewModel`](/reference/androidx/lifecycle/ViewModel) is recommended because of its ability to store
...
For more information, see [Activity embedding](/develop/ui/views/layout/activity-embedding).
...
A [`RecyclerView`](/reference/androidx/recyclerview/widget/RecyclerView) efficiently renders a large number of items in a single
column. A [`GridLayoutManager`](/reference/androidx/recyclerview/widget/GridLayoutManager) lays out items in a grid, allowing
...
can be overridden by creating a custom [`SpanSizeLookup`](/reference/androidx/recyclerview/widget/GridLayoutManager.SpanSizeLookup). Adjust the span to
emphasize some items over others.

On compact-width displays that have enough space for only one column, use
[`LinearLayoutManager`](/reference/androidx/recyclerview/widget/LinearLayoutManager) instead of `GridLayoutManager`.
...
[`LinearLayout`](/reference/android/widget/LinearLayout) or [`ConstraintLayout`](/reference/androidx/constraintlayout/widget/ConstraintLayout). Establish the window size classes
...
* **Compact:** In the app resources `layout` folder, place content that
  renders the supporting pane below the main content or inside a bottom sheet
* **Medium:** In the `layout-w600dp` folder, provide supporting pane content
  that results in the main content and supporting pane rendering side by side,
  splitting the horizontal display space equally
* **Expanded:** In the `layout-w840dp` folder, include supporting pane content
  that results in the main content and supporting pane rendering side by side;
  however, the supporting pane takes up only 30% of the horizontal space,
  leaving the remaining 70% for the main content

Use a [`ViewModel`](/reference/androidx/lifecycle/ViewModel) for communication between the main content and the
...
For examples of the canonical layouts implemented in different types of apps, see the [large screen gallery](/large-screens/gallery).

![](/static/images/develop/ui/views/canonical_layouts_flowchart.png)
