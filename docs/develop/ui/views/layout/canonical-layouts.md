...
If you're already familiar with the adaptive app canonical layouts but aren't sure which Android APIs to use, jump to the [Applicability](https://developer.android.com/develop/ui/views/layout/canonical-layouts#applicability) section for help determining which layout is right for your app's use cases.
...
A list-detail layout can be created with a variety of technologies, including Compose, views, and activity embedding (for legacy apps). See the [Applicability](https://developer.android.com/develop/ui/views/layout/canonical-layouts#applicability) section for help deciding which technology is most suitable for your app.

The [`SlidingPaneLayout`](https://developer.android.com/reference/androidx/slidingpanelayout/widget/SlidingPaneLayout) library is designed for implementation of list‑detail layouts based on views or fragments.
...
Implement a communication methodology to pass data between the list-detail views or fragments. [`ViewModel`](https://developer.android.com/reference/androidx/lifecycle/ViewModel) is recommended because of its ability to store business logic and survive configuration changes.
...
For an example implementation, see the [List-detail with sliding pane](https://github.com/android/user-interface-samples/tree/main/CanonicalLayouts/list-detail-sliding-pane) sample.
...
For an example implementation, see the [List-detail with activity embedding](https://github.com/android/user-interface-samples/tree/main/CanonicalLayouts/list-detail-activity-embedding) sample.
...
A [`RecyclerView`](https://developer.android.com/reference/androidx/recyclerview/widget/RecyclerView) efficiently renders a large number of items in a single column. A [`GridLayoutManager`](https://developer.android.com/reference/androidx/recyclerview/widget/GridLayoutManager) lays out items in a grid, allowing configuration of the item sizes and spans.

Configure the grid columns based on the size of the available display area to set the minimum allowable width for items.

The `GridLayoutManager` default spanning strategy, which is one span per item, can be overridden by creating a custom [`SpanSizeLookup`](https://developer.android.com/reference/androidx/recyclerview/widget/GridLayoutManager.SpanSizeLookup). Adjust the span to emphasize some items over others.

On compact-width displays that have enough space for only one column, use [`LinearLayoutManager`](https://developer.android.com/reference/androidx/recyclerview/widget/LinearLayoutManager) instead of `GridLayoutManager`.

For an example implementation, see the [Feed with views](https://github.com/android/user-interface-samples/tree/main/CanonicalLayouts/feed-view) sample.
...
A supporting pane layout is implemented using a helper layout such as [`LinearLayout`](https://developer.android.com/reference/android/widget/LinearLayout) or [`ConstraintLayout`](https://developer.android.com/reference/androidx/constraintlayout/widget/ConstraintLayout). Establish the window size classes that divide the amount of horizontal display space available for your app into three categories: compact (\< 600dp), medium (\>= 600dp), and expanded (\>= 840dp).
...
Use a [`ViewModel`](https://developer.android.com/reference/androidx/lifecycle/ViewModel) for communication between the main content and the supporting pane whether using views, fragments, or a combination.
