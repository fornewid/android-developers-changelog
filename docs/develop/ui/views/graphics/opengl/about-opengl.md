---
title: OpenGL ES  |  Views  |  Android Developers
...
# OpenGL ES Stay organized with collections Save and categorize content based on your preferences.
...
do this, see [Checking OpenGL ES version](#version-check).
...
* [Displaying graphics with OpenGL ES](/training/graphics/opengl)
...
NDK, see the [Android NDK](/tools/sdk/ndk).
...
:   This class is a `View` where you can draw and manipulate objects using
    ...
    [Responding to touch events](/training/graphics/opengl/touch).
...
* OpenGL ES 3.0/3.1/3.2 API Packages
  + `android.opengl` - This package provides the interface to the OpenGL ES 3.0/3.1
    ...
    starting with Android 5.0 (API level 21). Version 3.2 is available starting with Android 7.0 (API
    level 24).
    - `GLES30`
    - `GLES31`
    - `GLES31Ext` ([Android Extension Pack](#aep))
    - `GLES32`
...
these requirements in your [AndroidManifest.xml](/guide/topics/manifest/manifest-intro)
...
* **OpenGL ES version requirements** - If your application requires a specific
  ...
  **Note:**
  ...
  device, see [Checking OpenGL ES version](#version-check).
* **Texture compression requirements** - If your application uses texture
  ...
  using [`<supports-gl-texture>`](/guide/topics/manifest/supports-gl-texture-element).
  For more information about available texture compression
  formats, see [Texture compression support](#textures).
  ...
  [Google Play and texture compression filtering](/guide/topics/manifest/supports-gl-texture-element#market-texture-filtering) section of the `<supports-gl-texture>` documentation.
...
![](/static/images/opengl/coordinates.png)
...
1. **Add matrix to vertex shaders** - Create a variable for the view projection matrix
   ...
   **Note:** The example above defines a single transformation matrix
   ...
   independently.
2. **Access the shader matrix** - After creating a hook in your vertex shaders to
   ...
   ```
3. **Create projection and camera viewing matrices** - Generate the projection and
   ...
   ```
4. **Apply projection and camera viewing matrices** - To apply the projection and
...
For a complete example of how to apply projection and camera view with OpenGL ES 2.0, see the [Displaying graphics with OpenGL ES](/training/graphics/opengl)
class.
...
![Coordinates at
vertices of a triangle](/static/images/opengl/ccw-winding.png)
...
[OpenGL ES version dashboard](/about/dashboards#OpenGL).
...
[Choosing an OpenGL API version](#choosing-version).
...
[Checking OpenGL ES version](#version-check)
...
[query the device](#gl-extension-query) and review the *OpenGL extension names*,
...
* **Adaptable Scalable Texture Compression (ASTC)** - A texture compression format
  designed to supersede prior formats. More flexible than previous formats due to support for various
  block sizes.
  + `GL_KHR_texture_compression_astc_ldr`
  + `GL_KHR_texture_compression_astc_hdr`(high dynamic range)
* **S3TC (DXT*n*/DXTC)** - S3 texture compression (S3TC) has several
  ...
  name:
  + `GL_EXT_texture_compression_s3tc`Some devices only support the DXT1 format variation; this limited support is represented by the
...
[<supports-gl-texture>](/guide/topics/manifest/supports-gl-texture-element). Using this declaration enables filtering by external services such as Google Play, so that
your app is installed only on devices that support the formats your app requires. For details, see
[OpenGL manifest declarations](/develop/ui/views/graphics/opengl/about-opengl#manifest).
...
1. Run the following code on your target devices to determine what texture compression
   ...
   **Warning:** The results of this call *vary by device model!* You
   ...
   supported.
2. Review the output of this method to determine what OpenGL extensions are supported on the
   device.
...
minimum version of the API your application requires in your [manifest](#manifest), but
...
1. Attempt to create the higher-level OpenGL ES context (`EGLContext`) and
   check the result.
...
* **Device Compatibility** - Developers should consider the types of devices,
  ...
  device compatibility](#compatibility) section.
* **Texture Support** - The OpenGL ES 3.0 API has the best support for texture
  ...
  transparency. The 2.0 API implementations include support for ETC1, however
  this texture format doesn't support transparency. To implement transparency with compressed
  textures you must either use two ETC1 textures (split between color and alpha) or provide resources
  in other compression formats supported by the devices you are targeting. For more information,
  see [Texture compression support](#textures).
