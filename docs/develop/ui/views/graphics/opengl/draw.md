---
title: Draw shapes  |  Views  |  Android Developers
...
# Draw shapes Stay organized with collections Save and categorize content based on your preferences.
...
`onSurfaceCreated()` method of your
...
* *Fragment Shader* - OpenGL ES code for rendering the face of a shape with colors or
  textures.
...
`draw()` method from within your renderer’s `onDrawFrame()` method:
...
![](/static/images/opengl/ogl-triangle.png)
...
`GLSurfaceView` is displayed. You can fix that problem using a projection and camera
view in the next lesson.

Lastly, the triangle is stationary, which is a bit boring. In the
[Add motion](/training/graphics/opengl/motion) lesson, you make this shape
