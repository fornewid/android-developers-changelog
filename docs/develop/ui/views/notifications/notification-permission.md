---
title: Notification runtime permission  |  Views  |  Android Developers
...
functionality](#request-in-context).
...
[requesting other runtime permissions](/training/permissions/requesting), as
shown in the following sections.

The permission that you need to
[declare in your app's manifest file](/training/permissions/declaring)
...
* Post [notifications related to foreground
  services](/develop/ui/views/notifications#foreground-service). These
...
notifications unless it qualifies for an [exemption](#exemptions). All
...
* If your app targets Android 13 or higher, your app has complete
...
* If your app targets 12L (API level 32) or lower, the system shows
  ...
  create a [notification channel](/training/notify-user/channels), or when your
  app starts an activity and then creates its first notification channel. This
  is usually on app startup.
...
[eligible apps](#eligibility) when the user upgrades their device to
...
[backup and restore](https://support.google.com/android/answer/2819582)
feature.  
...
[Task Manager](/develop/background-work/services/fgs/handle-user-stopping)
...
[media sessions](/media/media3/session/control-playback) are
...
1. Declares the
   ...
   permission.
2. Implements the
   [`ConnectionService`](/reference/android/telecom/ConnectionService)
   interface.
...
following
sets of [Android Debug Bridge (ADB)](/studio/command-line/adb) commands allow
...
[trigger a permissions prompt](/training/permissions/requesting#request-permission)
...
[`areNotificationsEnabled()`](/reference/android/app/NotificationManager#areNotificationsEnabled()).
...
sends, and
[they can revoke the permission](/training/permissions/requesting#handle-denial)
at any time.