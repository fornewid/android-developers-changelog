...
The magnifier is already integrated with platform widgets such as`TextView`,`EditText`, and`WebView`. It provides consistent text manipulation across apps. The widget comes with a simple API and can be used to magnify any`View`depending on your app's context.
...
Assuming the background color is visible within the magnifier, the magnifier's content is stale, as a region of the view with the old background still displays. To refresh the content, use the[`update()`](https://developer.android.com/reference/android/widget/Magnifier#update())method, as follows:  
...
When finished, close the magnifier by calling the[`dismiss()`](https://developer.android.com/reference/android/widget/Magnifier#dismiss())method:  
