---
title: Spans  |  Views  |  Android Developers
...
# Spans Stay organized with collections Save and categorize content based on your preferences.
...
change [`TextPaint`](/reference/android/text/TextPaint) properties, draw on a
[`Canvas`](/reference/android/graphics/Canvas), and change text layout.
...
**Note:** To help you create a modern and maintainable codebase, Android KTX
contains extension functions that make working with spans easier. To learn more,
see the
[androidx.core.text](/reference/kotlin/androidx/core/text/package-summary)
package documentation and the [Spantastic text styling with
Spans](https://medium.com/androiddevelopers/spantastic-text-styling-with-spans-17b0c16b4568)
blog post.
...
All three classes extend the [`Spanned`](/reference/android/text/Spanned)
interface. `SpannableString` and `SpannableStringBuilder` also extend the
[`Spannable`](/reference/android/text/Spannable) interface.
...
_flags_)`](/reference/android/text/Spannable#setSpan(java.lang.Object,%20int,%20int,%20int))
...
the
[`Spannable.SPAN_EXCLUSIVE_INCLUSIVE`](/reference/android/text/Spanned#SPAN_EXCLUSIVE_INCLUSIVE)
flag to include inserted text, and use
[`Spannable.SPAN_EXCLUSIVE_EXCLUSIVE`](/reference/android/text/Spanned#SPAN_EXCLUSIVE_EXCLUSIVE)
to exclude the inserted text.

The following example shows how to attach a
[`ForegroundColorSpan`](/reference/android/text/style/ForegroundColorSpan) to a
string:
...
![An image showing a grey text, partially red.](/static/develop/ui/views/text-and-emoji/images/spans-fg-color.png)


**Figure 1.** Text styled with a
`ForegroundColorSpan`.
...
![An image showing how the span include more text when SPAN_EXCLUSIVE_INCLUSIVE is used.](/static/develop/ui/views/text-and-emoji/images/spans-fg-color-2.png)


**Figure 2.** The span expands to include
additional text when using
`Spannable.SPAN_EXCLUSIVE_INCLUSIVE`.
...
![An image showing a text with multiple spans: `ForegroundColorSpan(Color.RED)` and `StyleSpan(BOLD)`](/static/develop/ui/views/text-and-emoji/images/spans-bold-red.png)


**Figure 3.** Text with multiple spans:
`ForegroundColorSpan(Color.RED)` and
`StyleSpan(BOLD)`.

## Android span types

Android provides over 20 span types in the
[android.text.style](/reference/android/text/style/package-summary) package.
Android categorizes spans in two primary ways:

* How the span affects text: a span can affect text appearance or text
  metrics.
* Span scope: some spans can be applied to individual characters, while others
  must be applied to an entire paragraph.

![An image showing different span categories](/static/develop/ui/views/text-and-emoji/images/spans-span-categories.png)
...
spans extend the
[`CharacterStyle`](/reference/android/text/style/CharacterStyle) class.
...
![An image showing how to underline text using an `UnderlineSpan`](/static/develop/ui/views/text-and-emoji/images/spans-underlinespan.png)


**Figure 5.** Text underlined using an
`UnderlineSpan`.
...
[`UpdateAppearance`](/reference/android/text/style/UpdateAppearance) and extend
[`CharacterStyle`](/reference/android/text/style/CharacterStyle).
...
[`MetricAffectingSpan`](/reference/android/text/style/MetricAffectingSpan)
class.

The following code example creates a
[`RelativeSizeSpan`](/reference/android/text/style/RelativeSizeSpan) that
...
![An image showing the usage of RelativeSizeSpan](/static/develop/ui/views/text-and-emoji/images/spans-relativesizespan.png)


**Figure 6.** Text made larger using a
`RelativeSizeSpan`.
...
implement [`ParagraphStyle`](/reference/android/text/style/ParagraphStyle). To
...
![](/static/develop/ui/views/text-and-emoji/images/spans-paragraphs.png)


**Figure 7.** In Android, paragraphs end with a
new line (`\n`) character.

The following code example applies a
[`QuoteSpan`](/reference/android/text/style/QuoteSpan) to a paragraph. Note that
...
![An image showing an example of QuoteSpan](/static/develop/ui/views/text-and-emoji/images/spans-quotespan.png)


**Figure 8.** A `QuoteSpan`
...
test](/training/testing/fundamentals) to verify that the correct spans are added
...
[nextSpanTransition()](/reference/android/text/Spanned#nextSpanTransition(int,%20int,%20java.lang.Class))
instead of
[getSpans()](/reference/android/text/Spanned#getSpans(int,%20int,%20java.lang.Class%3CT%3E)),
...
[`TextView.setText()`](/reference/android/widget/TextView#setText(int))
...
type)`](/reference/android/widget/TextView#setText(java.lang.CharSequence,%20android.widget.TextView.BufferType)),
...
In this example, the
[`BufferType.SPANNABLE`](/reference/android/widget/TextView.BufferType#SPANNABLE)
...
[`RecyclerView.ViewHolder`](/reference/androidx/recyclerview/widget/RecyclerView.ViewHolder),
...
[`Spannable.Factory`](/reference/android/text/Spannable.Factory) and override
[`newSpannable()`](/reference/android/text/Spannable.Factory#newSpannable(java.lang.CharSequence)).
