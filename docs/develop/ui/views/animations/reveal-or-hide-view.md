---
title: Reveal or hide a view using animation  |  Views  |  Android Developers
...
# Reveal or hide a view using animation Stay organized with collections Save and categorize content based on your preferences.
...
**Note:** [Jetpack Compose](/jetpack/compose) is the preferred way to build Android
apps. See the Compose [Animations](/jetpack/compose/animation) documentation.
...
one [`View`](/reference/android/view/View) or
[`ViewGroup`](/reference/android/view/ViewGroup) while simultaneously
...
](/static/training/animation/anim_crossfade.mp4)
...
1. Create member variables for the views that you want to crossfade. You need
   these references later when modifying the views during the animation.
2. Set the visibility of the view that is being faded in to
   [`GONE`](/reference/android/view/View#GONE). This prevents the view
   from using layout space and omits it from layout calculations, which speeds
   up processing
3. Cache the [`config_shortAnimTime`](/reference/android/R.integer#config_shortAnimTime)
   ...
   [`config_longAnimTime`](/reference/android/R.integer#config_longAnimTime)
   and
   [`config_mediumAnimTime`](/reference/android/R.integer#config_mediumAnimTime)
   are also available.
...
1. For the view that is fading in, set the alpha value to 0 and the visibility
   to [`VISIBLE`](/reference/android/view/View#VISIBLE) from its initial
   setting of `GONE`. This makes the view visible but transparent.
2. For the view that is fading in, animate its alpha value from 0 to 1. For the
   view that is fading out, animate the alpha value from 1 to 0.
3. Using
   [`onAnimationEnd()`](/reference/android/animation/Animator.AnimatorListener#onAnimationEnd(android.animation.Animator))
   in an
   [`Animator.AnimatorListener`](/reference/android/animation/Animator.AnimatorListener),
   set the visibility of the view that is fading out to `GONE`. Even though the
   alpha value is 0, setting the view's visibility to `GONE` prevents the view
   from using layout space and omits it from layout calculations, which speeds
   up processing.
...
](/static/develop/ui/views/animations/anim_card_flip.mp4)
...
[`onCreateView()`](/reference/android/app/Fragment#onCreateView(android.view.LayoutInflater,%20android.view.ViewGroup,%20android.os.Bundle))
...
[`FrameLayout`](/reference/android/widget/FrameLayout) that you can add
...
* Replaces the displayed fragment with a new fragment and animates this event
  with the custom animations you created.
...
of UI elements. The
[`ViewAnimationUtils.createCircularReveal()`](/reference/android/view/ViewAnimationUtils#createCircularReveal(android.view.View,%20int,%20int,%20float,%20float))
method lets you animate a clipping circle to reveal or hide a view. This
animation is provided in the
[`ViewAnimationUtils`](/reference/android/view/ViewAnimationUtils) class,
...
[`INVISIBLE`](/reference/android/view/View#INVISIBLE) when the animation
completes.

## Additional resources

* [Animation](/jetpack/compose/animation) with Jetpack Compose.
* [Gestures](/jetpack/compose/touch-input/gestures) with Jetpack Compose.