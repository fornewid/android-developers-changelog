---
title: Create custom haptic effects  |  Views  |  Android Developers
...
# Create custom haptic effects Stay organized with collections Save and categorize content based on your preferences.




This page covers the examples of how to use different [haptics APIs](/develop/ui/views/haptics/haptics-apis) to
create custom effects beyond the standard [vibration waveforms](/develop/ui/views/haptics/actuators) in an Android
app.

This page includes the following examples:

* [Custom vibration patterns](#custom_vibration_patterns)
  + [Ramp up pattern](#ramp_up_pattern): A pattern that begins smoothly.
  + [Repeating pattern](#repeating_pattern): A pattern with no end.
  + [Pattern with fallback](#pattern_with_fallback): A fallback demonstration.
* [Vibration compositions](#vibration_compositions)
  + [Resist](#resist): A drag effect with dynamic intensity.
  + [Expand](#expand): A rise then fall effect.
  + [Wobble](#wobble): A wobbly effect using the `SPIN` primitive.
  + [Bounce](#bounce): A bouncing effect using the `THUD` primitive.
* [Vibration waveform with envelopes](#vibration-waveform-with-envelopes)
  + [Bouncing spring](#bouncing-spring): A springy bouncing effect using basic envelope
    effects.
  + [Rocket launch](#rocket-launch): A rocket launch effect using waveform envelope
    effects.

For additional examples, see [Add haptic feedback to events](/develop/ui/views/haptics/haptic-feedback), and always
follow [haptics design principles](/develop/ui/views/haptics/haptics-principles).
...
The [Android haptics API reference](/develop/ui/views/haptics/haptics-apis) provides details on how to check for
...
* Use delays of 50 ms or longer for discernible gaps between two primitives,
  also taking into account the [primitive duration](/reference/android/os/Vibrator#getPrimitiveDurations(int...)) if possible.
...
* Use scales of 0.5, 0.7, and 1.0 to create a low, medium, and high intensity
  ...
  **Note:** A 0.0 `scale` parameter indicates minimum perceivable vibration, not
  off.

## Create custom vibration patterns

Vibration patterns are often used in attentional haptics, such as notifications
and ringtones. The [`Vibrator`](/reference/android/os/Vibrator) service can play long vibration patterns
...
**Important:** Start and end a waveform at zero amplitude whenever possible. This
avoids any sudden changes in the input voltage amplitude to the actuator which
may cause the actuator to resonate; in other words, vibrate at its resonance
frequency. Also, some drivers apply *active braking* if the waveform ends at 0,
so the vibration stops more quickly. See [Vibration actuators primer](/develop/ui/views/haptics/actuators) for
details.
...
Waveforms are represented as [`VibrationEffect`](/reference/android/os/VibrationEffect) with three parameters:

1. **Timings:** an array of durations, in milliseconds, for each waveform
   segment.
2. **Amplitudes:** the wanted vibration amplitude for each duration specified
   in the first argument, represented by an integer value from 0 to 255, with 0
   representing the vibrator "off state" and 255 being the device's maximum
   amplitude.
3. **Repeat index:** the index in the array specified in the first argument to
   start repeating the waveform, or -1 if it should play the pattern only once.
...
[hardware-dependent capability](/develop/ui/views/haptics/haptics-apis#amplitude_control). Playing a waveform on a low-end device
...
**Note:** The ON/OFF pattern is actually specified in the API as a OFF/ON sequence
of durations. See more details in the [API reference documentation](/reference/android/os/VibrationEffect#createWaveform(long%5B%5D,%20int)).
...
**Note:** See [Add haptics feedback to events](/develop/ui/views/haptics/haptic-feedback) for predefined haptic effects and
how to use them.

The process for [creating custom vibration patterns](#custom_vibration_patterns), described previously on
...
The composition [primitives](/develop/ui/views/haptics/custom-haptic-effects#primitives), described earlier on this page, are
implemented by the device manufacturer. They provide a crisp, short, and
pleasant vibration that aligns with [haptics principles](/develop/ui/views/haptics/haptics-principles) for clear haptics.
For more details about these capabilities and how they work, see [Vibration
actuators primer](/develop/ui/views/haptics/actuators).
...
[`VibrationEffect.Composition`](/reference/android/os/VibrationEffect.Composition). Here is an example of a slowly rising
...
[primitives](/develop/ui/views/haptics/custom-haptic-effects#primitives) can also be dynamically set based on the user interaction. This
is illustrated in the following example of a view animation controlled by a drag
gesture and augmented with haptics.

![](/static/develop/ui/views/haptics/images/demo-portrait.svg)
![Animation of a circle being dragged down.](/static/develop/ui/views/haptics/images/resist-demo.gif)

![Plot of input vibration waveform.](/static/develop/ui/views/haptics/images/resist-waveform.svg)
...
There are two [primitives](/develop/ui/views/haptics/custom-haptic-effects#primitives) for ramping up the perceived vibration intensity:
[`PRIMITIVE_QUICK_RISE`](/reference/android/os/VibrationEffect.Composition#PRIMITIVE_QUICK_RISE) and [`PRIMITIVE_SLOW_RISE`](/reference/android/os/VibrationEffect.Composition#PRIMITIVE_SLOW_RISE). They both reach
the same target, but with different durations. There is only one primitive for
ramping down, [`PRIMITIVE_QUICK_FALL`](/reference/android/os/VibrationEffect.Composition#PRIMITIVE_QUICK_FALL). These primitives work better
...
![](/static/develop/ui/views/haptics/images/demo-portrait.svg)
![Animation of an expanding circle.](/static/develop/ui/views/haptics/images/expand-demo.gif)

![Plot of input vibration waveform.](/static/develop/ui/views/haptics/images/expand-waveform.svg)
...
One of the key [haptics principles](/develop/ui/views/haptics/haptics-principles) is to delight users. A fun way to
introduce a pleasant unexpected vibration effect is to use
[`PRIMITIVE_SPIN`](/reference/android/os/VibrationEffect.Composition#PRIMITIVE_SPIN). This primitive is most effective when it is called more
...
[primitives](/develop/ui/views/haptics/custom-haptic-effects#primitives). Two spins without any gap (0 ms in between) creates a tight
...
![](/static/develop/ui/views/haptics/images/demo-portrait.svg)
![Animation of an elastic shape bouncing](/static/develop/ui/views/haptics/images/wobble-demo.gif)

![Plot of input vibration waveform](/static/develop/ui/views/haptics/images/wobble-waveform.svg)
...
interactions. [`PRIMITIVE_THUD`](/reference/android/os/VibrationEffect.Composition#PRIMITIVE_THUD) can create a strong and reverberating
...
![](/static/develop/ui/views/haptics/images/demo-portrait.svg)
![Animation of a dropped ball bouncing off the bottom of the screen.](/static/develop/ui/views/haptics/images/bounce-demo.gif)

![Plot of input vibration waveform.](/static/develop/ui/views/haptics/images/bounce-waveform.svg)
...
The process for [creating custom vibration patterns](#custom_vibration_patterns) lets you control the
...
* **[`BasicEnvelopeBuilder`](/reference/android/os/VibrationEffect.BasicEnvelopeBuilder):** An accessible approach to creating
  hardware-agnostic haptic effects.
* **[`WaveformEnvelopeBuilder`](/reference/android/os/VibrationEffect.WaveformEnvelopeBuilder):** A more advanced approach to creating
  haptic effects; requires familiarity with haptics hardware.

Android doesn't provide fallbacks for envelope effects. If you require this
support, complete the following steps:

1. Check a given device supports envelope effects using
   [`Vibrator.areEnvelopeEffectsSupported()`](/reference/android/os/Vibrator#areEnvelopeEffectsSupported()).
2. Disable the consistent set of experiences that are unsupported, or use
   [custom vibration patterns](#custom_vibration_patterns) or [compositions](/develop/ui/views/haptics/actuators#vibrator-output-acceleratio) as fallback
   alternatives.

To create more basic envelope effects, use the `BasicEnvelopeBuilder` with these
parameters:

* An **[intensity](/develop/ui/views/haptics/actuators#vibration-acceleration-levels)** value in the range \( [0, 1] \), which represents
  the perceived strength of the vibration. For example, a value of \( 0.5 \)
  is perceived as half the global maximum intensity that can be achieved by
  the device.
* A **sharpness** value in the range \( [0, 1] \), which represents the
  ...
  **Note:** Setting an initial sharpness is optional. If omitted, the framework
  defaults the initial sharpness to the value of the first control point.
...
**Caution:** When using the basic envelope API for envelope based vibrations, check
that your effect concludes with a control point of zero intensity. Otherwise, an
[`IllegalStateException`](/reference/java/lang/IllegalStateException) occurs.
...
[frequency-to-output-acceleration mapping (FOAM)](/develop/ui/views/haptics/actuators#vibrator-output-acceleratio) through the
[`VibratorFrequencyProfile`](/reference/android/os/vibrator/VibratorFrequencyProfile).

* An amplitude value in the range \( [0, 1] \), which represents the
  achievable vibration strength at given frequency, as determined by the
  device FOAM. For example, a value of \( 0.5 \) generates half the maximum
  output acceleration that can be achieved at the given frequency.
* A **frequency** value, specified in Hertz.

  **Note:** Setting an initial frequency for `WaveformEnvelopeBuilder` is
  optional. If omitted, the framework defaults the initial frequency to the
  value of the first control point.
* A **duration** value, which represents the time, in milliseconds, taken to
  transition from the last control point to the new one.

**Note:** To help your vibration patterns play correctly, use frequencies within the
device's capabilities. The system completely ignores vibration effects that
contain unsupported frequencies. Refer to the [`VibratorFrequencyProfile` API
reference](/reference/android/os/vibrator/VibratorFrequencyProfile) to determine the supported frequency range. In addition, check
device support for envelope effects using [`VibratorEnvelopeEffectInfo`](/reference/android/os/vibrator/VibratorEnvelopeEffectInfo).
...
A previous sample uses [`PRIMITIVE_THUD`](/reference/android/os/VibrationEffect.Composition#PRIMITIVE_THUD) to simulate [physical bounce
interactions](#bounce). The [basic envelope API](/reference/android/os/VibrationEffect.BasicEnvelopeBuilder) offers significantly finer
...
![](/static/develop/ui/views/haptics/images/demo-portrait.svg)
![Animation of a dropped spring bouncing off the bottom of the screen.](/static/develop/ui/views/haptics/images/bouncing-spring.gif)

![Plot of input vibration waveform.](/static/develop/ui/views/haptics/images/spring-waveform-graph.svg)
...
simulate a bouncy spring reaction. The [`WaveformEnvelopeBuilder`](/reference/android/os/VibrationEffect.WaveformEnvelopeBuilder) unlocks
...
![](/static/develop/ui/views/haptics/images/demo-portrait.svg)
![Animation of a rocket ship taking off from the bottom of the screen.](/static/develop/ui/views/haptics/images/rocket-launch.gif)

![Plot of input vibration waveform.](/static/develop/ui/views/haptics/images/rocket-waveform-graph.svg)
