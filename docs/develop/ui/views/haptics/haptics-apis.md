---
title: Android haptics API reference  |  Views  |  Android Developers
...
consider Android haptics [design principles](/develop/ui/views/haptics/haptics-principles) when choosing among
...
* [Clear haptics](/develop/ui/views/haptics/haptics-principles#clear_haptics) are crisp and clean sensations
  that are less jarring for users.
* [Rich haptics](/develop/ui/views/haptics/haptics-principles#rich_haptics) have greater expressiveness and
  often require more feature-rich hardware.

| API surface | Availability | Clear haptics | Rich haptics |
| --- | --- | --- | --- |
| [HapticFeedbackConstants](#haptic_feedback_constants) | Android 1.5+ (per constant) |  |  |
| [Predefined VibrationEffect](#predefined_vibration_effect) | Android 10+ |  |  |
| [VibrationEffect Composition](#vibration_effect_composition) | Android 11+ (per constant) |  |  |
| [On/off, one-shot and waveform vibrations](#on_off_vibrations) | Android 1 |  |  |

Additionally, [notification APIs](#notification_apis), described on this page,
...
* [Does the device have a vibrator?](#has_vibrator)
* [Amplitude control](#amplitude_control) allows smoother, richer haptic
  effects, but is not supported by all devices.
* [`VibrationAttributes()`](#vibration_attributes) helps
...
The [`HapticFeedbackConstants`](/reference/android/view/HapticFeedbackConstants) class provides action-based constants to allow
...
Using the [`View.performHapticFeedback`](/reference/android/view/View#performHapticFeedback(int)) method with these constants does not
require any special permissions for the app. It is subject to the
[`View.hapticFeedbackEnabled`](/reference/android/view/View#attr_android:hapticFeedbackEnabled) property, which if set to `false` will disable
all haptic feedback calls on the view, including default ones.The primary
related setting the [`View.hapticFeedbackEnabled`](/reference/android/view/View#attr_android:hapticFeedbackEnabled) property, which if set to
...
[`HapticFeedbackConstants`](/reference/android/view/HapticFeedbackConstants).

### Usage of `HapticsFeedbackConstants`

For details on using `HapticFeedbackConstants`, see [Add haptic feedback to
events](/develop/ui/views/haptics/haptic-feedback).

## Predefined `VibrationEffect`

The [`VibrationEffect`](/reference/android/os/VibrationEffect#constants) class
...
The [`Vibrator.areEffectsSupported`](/reference/android/os/Vibrator#areEffectsSupported(int...)) and [`Vibrator.areAllEffectsSupported`](/reference/android/os/Vibrator#areAllEffectsSupported(int...))
...
* [`VIBRATION_EFFECT_SUPPORT_YES`](/reference/android/os/Vibrator#VIBRATION_EFFECT_SUPPORT_YES)
  indicates that the device has optimized support for this effect.
* [`VIBRATION_EFFECT_SUPPORT_NO`](/reference/android/os/Vibrator#VIBRATION_EFFECT_SUPPORT_NO)
  indicates that the device does not have optimized support, but still uses
  the platform fallback.
* [`VIBRATION_EFFECT_SUPPORT_UNKNOWN`](/reference/android/os/Vibrator#VIBRATION_EFFECT_SUPPORT_UNKNOWN)
...
For details on using a predefined `VibrationEffect`, see [Use a predefined `VibrationEffect` to generate haptic feedback](/develop/ui/views/haptics/haptic-feedback#using_a_predefined_vibrationeffect).
...
vibrations can be created using the [`BasicEnvelopeBuilder`](/reference/android/os/VibrationEffect.BasicEnvelopeBuilder) and
[`WaveformEnvelopeBuilder`](/reference/android/os/VibrationEffect.WaveformEnvelopeBuilder) classes.
...
**Caution:** Check that the frequency specified in their envelope effects fall
within the device's supported range, as defined by the device's
[frequency-to-output-acceleration mapping (FOAM)](/develop/ui/views/haptics/actuators#vibrator-output-acceleration). If values exceed these
limits, the device doesn't play any vibration.
...
[`getMaxSize()`](/reference/android/os/vibrator/VibratorEnvelopeEffectInfo#getMaxSize())
:   Retrieves the maximum number of control points supported for an envelope
    effect.

[`getMinControlPointDurationMillis()`](/reference/android/os/vibrator/VibratorEnvelopeEffectInfo#getMinControlPointDurationMillis())
:   Retrieves the minimum duration supported, in milliseconds, between two
    control points within an envelope effect.

[`getMaxControlPointDurationMillis()`](/reference/android/os/vibrator/VibratorEnvelopeEffectInfo#getMaxControlPointDurationMillis())
:   Retrieves the maximum duration supported, in milliseconds, between two
    control points within an envelope effect.

[`getMaxDurationMillis()`](/reference/android/os/vibrator/VibratorEnvelopeEffectInfo#getMaxDurationMillis())
...
[create vibration waveform with envelopes](/develop/ui/views/haptics/custom-haptic-effects#vibration-waveform-with-envelopes).

## `VibrationEffect` composition

A [`VibrationEffect`](/reference/android/os/VibrationEffect) composition is a vibration effect created using the
[`VibrationEffect.startComposition`](/reference/android/os/VibrationEffect#startComposition()) API. This API allows expressive
[rich haptics](/develop/ui/views/haptics/haptics-principles#rich_haptics) by creating a sequence of primitives with
...
**Caution:** If a composition contains primitives that aren't supported by the device, the
entire `VibrationEffect.Composition` won't play any vibration. Therefore,
to ensure a predictable experience, the set of primitives used in your app
should be checked for support when considering which haptic experiences to
enable.

Per-primitive support can be retrieved using the
[`Vibrator.arePrimitivesSupported`](/reference/android/os/Vibrator#arePrimitivesSupported(int...)) method. Alternatively, a set of primitives
may be checked together by using the [`Vibrator.areAllPrimitivesSupported`](/reference/android/os/Vibrator#areAllPrimitivesSupported(int...))
method - this is equivalent to `AND`-ing the per-primitive support.

**Note:** The support for checking individual primitives is fully available starting
in Android 12 (SDK 31). In Android 11, the check relies only in the ability to
compose primitives, returning `TRUE` for all requested primitive IDs.

### Usage of `VibrationEffect` Compositions

For details on using `VibrationEffect` compositions, see [Create vibration compositions](/develop/ui/views/haptics/custom-haptic-effects#vibration_compositions).
...
with [Haptics design principles](/develop/ui/views/haptics/haptics-principles) because they can generate
[buzzy haptics](/develop/ui/views/haptics/haptics-principles#buzzy_haptics); avoid them except as a last resort.
...
virtually all devices [with a vibrator](#has_vibrator), as described later on
this page.
...
[`Vibrator.hasAmplitudeControl`](/reference/android/os/Vibrator#hasAmplitudeControl()). See the [amplitude control](/develop/ui/views/haptics/haptics-apis#amplitude_control)
for more details.
...
expressive [`VibrationEffect`](/reference/android/os/VibrationEffect) class, where these simple vibrations are created
using [`VibrationEffect.createOneshot`](/reference/android/os/VibrationEffect#createOneShot(long,%20int)) or [`VibrationEffect.createWaveform`](/reference/android/os/VibrationEffect#createWaveform(long%5B%5D,%20int)).
...
* AndroidX
  + [`NotificationChannelCompat.Builder.setVibrationPattern`](/reference/androidx/core/app/NotificationChannelCompat.Builder#setVibrationPattern(long%5B%5D))
  + [`NotificationCompat.setVibrate`](/reference/androidx/core/app/NotificationCompat.Builder#setVibrate(long%5B%5D))
* Android
  + [`NotificationChannel.setVibrationPattern`](/reference/android/app/NotificationChannel#setVibrationPattern(long%5B%5D))
  + (deprecated)
    [`NotificationBuilder.setVibrate`](/reference/android/app/Notification.Builder#setVibrate(long%5B%5D))

All of these forms take a basic [on-off waveform pattern](#on_off_vibrations),
...
You can obtain a non-null [`Vibrator`](/reference/android/os/Vibrator) class from
...
can call [`hasVibrator()`](/reference/android/os/Vibrator#hasVibrator()) to
...
can be queried using the [`HAPTIC_FEEDBACK_ENABLED`](/reference/android/provider/Settings.System#HAPTIC_FEEDBACK_ENABLED) key, where a value of zero
means disabled.

### Vibration attributes

Vibration attributes (currently in the form of [`AudioAttributes`](/reference/android/media/AudioAttributes)) can be
...
[rich haptics](/develop/ui/views/haptics/haptics-principles#rich_haptics), as well as potentially allowing user control of
default haptic intensities.

Amplitude control support can be checked by calling
[`Vibrator.hasAmplitudeControl`](/reference/android/os/Vibrator#hasAmplitudeControl()). If a vibrator does not have amplitude support,
...
[`Vibration.areEnvelopeEffectsSupported`](/reference/android/os/Vibrator#areEnvelopeEffectsSupported()) to determine if your device
