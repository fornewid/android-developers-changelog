---
title: Input compatibility on large screens  |  Views  |  Android Developers
...
Android framework. For example, a [`Button`](/reference/kotlin/android/widget/Button) is focusable by
...
To learn more, see [Focus Handling](/reference/android/view/View#FocusHandling).
...
such as for
an [`EditText`](/reference/android/widget/EditText)
...
Most apps override the [`onKeyUp()`](/reference/kotlin/android/view/KeyEvent.Callback#onKeyUp(int,%20android.view.KeyEvent)) callback and add the expected behavior
...
activity, or you can add an [`OnKeyListener`](/reference/kotlin/android/view/View.OnKeyListener) to a specific view. With an
`OnKeyListener`, you app can, for example, listen for the `Enter` key
in a specific `EditText` to implement *send* functionality only when the user is
typing in a chat box.

For more information, see [Handle keyboard actions](/training/keyboard-input/commands).
...
Shortcuts can be enabled by implementing [`dispatchKeyShortcutEvent()`](/reference/kotlin/android/view/Window.Callback#dispatchkeyshortcutevent) to
intercept all key combinations (`Alt`,
`Ctrl`, `Shift`, and `Meta`) for a given keycode.
To check for a specific modifier key, use:

* [`KeyEvent.isCtrlPressed()`](/reference/kotlin/android/view/KeyEvent#isCtrlPressed()),
* [`KeyEvent.isShiftPressed()`](/reference/kotlin/android/view/KeyEvent#isShiftPressed()),
* [`KeyEvent.isAltPressed()`](/reference/kotlin/android/view/KeyEvent#isAltPressed()),
* [`KeyEvent.isMetaPressed()`](/reference/kotlin/android/view/KeyEvent#ismetapressed), or
* [`KeyEvent.hasModifiers()`](/reference/kotlin/android/view/KeyEvent#hasmodifiers).
...
games when `W` means "walk forward" and `Shift+W` means "run
forward".
...
See also [Keyboard Shortcuts Helper](/develop/ui/compose/touch-input/keyboard-input/keyboard-shortcuts-helper).
...
A stylus event is reported as a touchscreen event by [`View#onTouchEvent()`](/reference/kotlin/android/view/View#ontouchevent)
or [`View#onGenericMotionEvent()`](/reference/kotlin/android/view/View#ongenericmotionevent) and contains a
[`MotionEvent#getSource()`](/reference/kotlin/android/view/MotionEvent#getsource) of type [`SOURCE_STYLUS`](/reference/kotlin/android/view/InputDevice#SOURCE_STYLUS).
...
**Note:** any views that have been registered for a context menu using
[`Activity.registerForContextMenu()`](/reference/android/app/Activity#registerForContextMenu(android.view.View)) should automatically work with both
touch & hold and right‑click without the need to register a context click
listener.
...
[Enable drag and drop](/guide/topics/ui/drag-drop)
...
[`View#onGenericMotionEvent()`](/reference/kotlin/android/view/View#ongenericmotionevent) and use [`MotionEvent.getSource()`][] to
distinguish between [`SOURCE_MOUSE`](/reference/kotlin/android/view/InputDevice#source_mouse) and [`SOURCE_TOUCHSCREEN`](/reference/kotlin/android/view/InputDevice#source_touchscreen).

Examine the `MotionEvent` object to implement the required behavior:

* Movement generates `ACTION_HOVER_MOVE` events.
* Buttons generate `ACTION_BUTTON_PRESS` and `ACTION_BUTTON_RELEASE` events.
  You can also check the current state of all mouse and trackpad buttons using
  [`getButtonState()`](/reference/kotlin/android/view/MotionEvent#getbuttonstate).
