---
title: Track touch and pointer movements  |  Views  |  Android Developers
...
# Track touch and pointer movements Stay organized with collections Save and categorize content based on your preferences.
...
[`ACTION_MOVE`](/reference/android/view/MotionEvent#ACTION_MOVE) event
whenever the current touch contact position, pressure, or size changes. As
described in [Detect common gestures](/training/gestures/detector), all
these events are recorded in the
[`MotionEvent`](/reference/android/view/MotionEvent) parameter of
`onTouchEvent()`.
...
ViewGroup](/training/gestures/viewgroup#vc).
...
* History. You can find the size of a gesture's history by calling the
  ...
  You can then obtain the positions, sizes, time, and pressures of each of the
  historical events by using the motion event's
  `getHistorical<Value>`
  methods. History is useful when rendering a trail of the user's finger, such
  as for touch drawing. See the `MotionEvent` reference for details.
...
[`VelocityTracker`](/reference/android/view/VelocityTracker) class.
...
[`ACTION_UP`](/reference/android/view/MotionEvent#ACTION_UP). After an
...
[`requestPointerCapture()`](/reference/android/view/View#requestPointerCapture())
...
[`onPointerCaptureChange(true)`](/reference/android/view/View#onPointerCaptureChange(boolean)).
...
[`ACTION_OUTSIDE`](/reference/android/view/MotionEvent#ACTION_OUTSIDE)
...
* If you're using a custom view, override
  [`onCapturedPointerEvent(MotionEvent)`](/reference/android/view/View#onCapturedPointerEvent(android.view.MotionEvent)).
* Otherwise, register an
  [`OnCapturedPointerListener`](/reference/android/view/View.OnCapturedPointerListener).
...
[`getX()`](/reference/android/view/MotionEvent#getX()) and
