---
title: Handle multi-touch gestures  |  Views  |  Android Developers
...
# Handle multi-touch gestures Stay organized with collections Save and categorize content based on your preferences.
...
* `ACTION_POINTER_DOWN`:
  ...
  `getActionIndex()`.
* `ACTION_MOVE`:
  sent when a change occurs in a gesture, involving any number of
  pointers.
* `ACTION_POINTER_UP`:
...
* **Index**: a `MotionEvent` stores pointer
  information in an array. The index of a pointer is its position within this
  array. Most of the `MotionEvent` methods take the pointer index as
  a parameter, rather than the pointer ID.
* **ID**: each pointer also has an ID mapping that stays
  persistent across touch events to allow for tracking of an individual pointer
  across the entire gesture.
...
`MotionEventCompat`
class, a class in the [Support
Library](/tools/support-library). Use `MotionEventCompat` to provide the best support
for a wide range of platforms. `MotionEventCompat` is
*not* a replacement for the `MotionEvent` class. Rather,
it provides static utility methods to which you pass your
`MotionEvent` object to receive the desired action associated
with that event.
...
](/static/images/ui/multi_touch.mp4)

**Figure 1.** Multi-touch drawing
patterns.
