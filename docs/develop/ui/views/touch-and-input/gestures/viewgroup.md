---
title: Manage touch events in a ViewGroup  |  Views  |  Android Developers
...
# Manage touch events in a ViewGroup Stay organized with collections Save and categorize content based on your preferences.
...
### Process ACTION\_OUTSIDE events
...
to dispatch to the appropriate `View` or
...
`TouchDelegate` class to make it
...
`ImageButton` is the \_delegate
view\_—that is, the child whose touch area the parent extends. Here is the layout file:
...
* Gets the parent view and posts a `Runnable`
  on the UI thread. This makes sure that the parent lays out its children before calling the
  `getHitRect()`
...
* Instantiates a `TouchDelegate`, passing in the expanded hit rectangle and the
  `ImageButton` child view as parameters.
* Sets the `TouchDelegate` on the parent view so that touches within the touch delegate
