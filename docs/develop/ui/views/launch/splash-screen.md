---
title: Splash screens  |  Views  |  Android Developers
...
# Splash screens Stay organized with collections Save and categorize content based on your preferences.
...
see [Migrate your existing splash screen implementation to Android
12](/guide/topics/ui/splash-screen/migrate).

Starting in Android 12, the
[`SplashScreen`](/reference/android/window/SplashScreen) API lets apps launch
...
[`Window`](/reference/android/view/Window) and
therefore covers an
[`Activity`](/reference/android/app/Activity).

[
](/static/images/guide/topics/ui/splash-screen/splash-screen-gmail-example.mp4)
...
[`SplashScreen`](/reference/kotlin/androidx/core/splashscreen/SplashScreen)
...
start](/topic/performance/vitals/launch-time#cold)) or the `Activity` isn't
created (a [warm start](/topic/performance/vitals/launch-time#warm)), the
...
![An image showing the elements contained in a splash screen](/static/images/guide/topics/ui/splash-screen/splash-screen-composition.png)


**Figure 2.** Customizable elements of a splash
screen.
...
[adaptive icon](/guide/practices/ui_guidelines/icon_design_adaptive), its
...
* App icon without an icon background: this must be 288×288 dp and fit within
  a circle 192 dp in diameter.
...
![An image showing different icon dimensions for solid and transparent background](/static/images/guide/topics/ui/splash-screen/splash-screen-icon-dimensions.png)


**Figure 3.** Splash screen icon dimensions for
solid and transparent backgrounds, respectively.
...
![An image showing the launch sequence in twelve consecutive frames, beginning with the launcher icon being tapped and filling the screen as it enlarges](/static/images/ui/splash-sequence.png)
...
2. Splash screen (shown during the "wait" portion of the sequence): the splash
   ...
   branding. It must meet the [requirements](#splash-screen-animate-reqs)
   described in this page to work properly.
3. Exit animation: this consists of the animation that hides the splash screen.
   If you want to [customize it](#customize-animation), use the
   [`SplashScreenView`](/reference/android/window/SplashScreenView) and its
...
* Set a single window background color with no transparency. Day and Night
  ...
  [`SplashScreen` compat library](/reference/kotlin/androidx/core/splashscreen/SplashScreen).
* Make sure the animated icon meets the following specifications:

  + **Format:** the icon must be an
    [AnimatedVectorDrawable (AVD)](/reference/android/graphics/drawable/AnimatedVectorDrawable)
    XML.
  + **Dimensions:** an AVD icon must be four times the size of an adaptive
    icon, as follows:
    - The icon area must be 432 dp—in other words, four times the
      108 dp area of an unmasked adaptive icon.
    - The inner two-thirds of the image is visible on the launcher icon,
      and must be 288 dp—in other words, four times the 72 dp that
      makes up the inner masked area of an adaptive icon.
  + **Duration:** we recommend not exceeding 1,000 ms on phones. You can use
    a delayed start, but this can't be longer than 166 ms. If the app
...
* Establish an appropriate time to dismiss the splash screen, which happens as
  ...
  [keeping the splash screen on-screen for longer periods](#suspend-drawing).

### Splash screen resources

[
](/static/images/guide/topics/ui/splash-screen/splash-screen-news-avd-preview.mp4)
...
1. Use
   [`windowSplashScreenBackground`](/reference/android/R.attr#windowSplashScreenBackground)
   ...
   ```
2. Use
   [`windowSplashScreenAnimatedIcon`](/reference/android/R.attr#windowSplashScreenAnimatedIcon)
   ...
   [`AnimationDrawable`](/reference/android/graphics/drawable/AnimationDrawable)
   ...
   ```
3. Use
   [`windowSplashScreenAnimationDuration`](/reference/android/R.attr#windowSplashScreenAnimationDuration)
   ...
   animation using
   [`SplashScreenView.getIconAnimationDuration`](/reference/android/window/SplashScreenView#getIconAnimationDuration()).
   ...
   ```
4. Use [`windowSplashScreenIconBackgroundColor`](/reference/android/R.attr#windowSplashScreenIconBackgroundColor)
   ...
   ```
5. You can use
   [`windowSplashScreenBrandingImage`](/reference/android/R.attr#windowSplashScreenBrandingImage)
   ...
   ```
6. You can use
   [`windowSplashScreenBehavior`](/reference/android/R.attr#windowSplashScreenBehavior)
   ...
   [`SPLASH_SCREEN_STYLE_ICON`](/reference/android/window/SplashScreen#SPLASH_SCREEN_STYLE_ICON),
...
[`Activity.getSplashScreen()`](/reference/android/app/Activity#getSplashScreen()).
...
[animated vector drawable](/reference/android/graphics/drawable/AnimatedVectorDrawable)
...
[`SplashScreenView.getIconAnimationStart`](/reference/android/window/SplashScreenView#getIconAnimationStart())
...
* [Migrate your existing splash screen implementation to Android 12 and
  higher](/develop/ui/views/launch/splash-screen/migrate)
* [Now in Android](https://github.com/android/nowinandroid) app,
  which shows a real-world implementation of a splash screen