---
title: Create shortcuts  |  Views  |  Android Developers
...
# Create shortcuts Stay organized with collections Save and categorize content based on your preferences.
...
![An image showing the contrast between app shortcuts and pinned shortcuts](/static/images/guide/topics/ui/shortcuts/pinned-shortcuts.png)
...
* **[Static
  shortcuts](/guide/topics/ui/shortcuts/creating-shortcuts#static)  are best for apps that link to content using a consistent
  structure throughout the lifetime of a user's interaction with the
  app.** Because most launchers
  [only display four
  shortcuts](/guide/topics/ui/shortcuts/best-practices) at once, static shortcuts are useful for performing a routine
  task in a consistent way, such as if the user wants to view their calendar or
  email in a specific way .
* **[Dynamic
  shortcuts](/guide/topics/ui/shortcuts/creating-shortcuts#dynamic) are used for actions in apps that are
  context-sensitive.** Context-sensitive shortcuts are tailored to the
...
* **[Pinned
  shortcuts](/guide/topics/ui/shortcuts/creating-shortcuts#pinned) are used for specific user-driven actions.** For example,
  a user might want to pin a specific website to the launcher. This is
  beneficial because it lets the user perform a custom action—like
...
1. In your app's `AndroidManifest.xml` file, find the activity whose
...
`android:icon`
:   The [bitmap](/topic/performance/graphics) or
    [adaptive
    icon](/guide/practices/ui_guidelines/icon_design_adaptive) that the launcher uses when displaying the shortcut to the user. This
    ...
    **Note:** Shortcut icons can't include
    [tints](/guide/topics/graphics/drawables#DrawableTint).
...
:   The action that the system launches when the user selects the shortcut.
    ...
    **Note:** This `intent` element can't include string
    resources.

    You can provide multiple intents for a single shortcut. See
    [Manage
    multiple intents and activities](/guide/topics/ui/shortcuts/managing-shortcuts#multiple-intents-activities),
    [Set
    an intent](/develop/ui/views/components/settings/customize-your-settings#set-an-intent), and the
    `TaskStackBuilder`
...
`capability-binding`
:   Declares the [capability](/guide/topics/ui/shortcuts/adding-capabilities)
    linked with the shortcut.

    In the previous example, the shortcut is linked to a capability declared
    for
    [`CREATE_MESSAGE`](//developers.google.com/assistant/app/reference/built-in-intents/communications/create-message),
    which is an [App Actions](//developers.google.com/assistant/app)
...
to appear on Google surfaces—like Assistant—with the
 [Google Shortcuts Integration Library](#gsi-library).
...
[shortcut limits](/guide/topics/ui/shortcuts#shortcut-limitations)
...
**Note:** We recommend using `pushDynamicShortcut` to push
...
2. Create a `ShortcutInfo` object in one of two ways, depending
   ...
   1. If the shortcut exists, create a `ShortcutInfo` object that
   ...
   2. If you're pinning a new shortcut, create a `ShortcutInfo`
      object that contains an ID, an intent, and a short label for the new
      shortcut.
   **Note:** Because the system performs
   [backup
   and restore](/guide/topics/ui/shortcuts/managing-shortcuts#backup-restore) on pinned shortcuts automatically, these shortcuts' IDs
...
3. Pin the shortcut to the device's launcher by calling
   ...
   **Note:** If the user doesn't let the shortcut be pinned to the
   ...
   [Update
   shortcuts](/guide/topics/ui/shortcuts/managing-shortcuts#update-shortcuts).

The following code snippet demonstrates how to create a pinned shortcut.

**Note:** Instances of the `ShortcutManager` class must be
obtained using
...
![An image showing the custom dialog activity that shows the prompt 'Do
  ...
  options are 'No thanks' and 'Add icon'.](/static/images/guide/topics/ui/shortcuts/pinned-shortcuts-dialog.png)
...
4. Your app creates the shortcut using the
