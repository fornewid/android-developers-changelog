---
title: Use collection widgets  |  Views  |  Android Developers
...
# Use collection widgets Stay organized with collections Save and categorize content based on your preferences.
...
[`RemoteViewsService`](/reference/android/widget/RemoteViewsService) to display
collections that are backed by remote data, such as from a [content
provider](/guide/topics/providers/content-providers). The widget presents the
...
:   An
    ...
    [`ViewAnimator`](/reference/android/widget/ViewAnimator) that animates
...
use an [`Adapter`](/reference/android/widget/Adapter) to bind their user
interface to their data. An `Adapter` binds individual items from a set of data
to individual [`View`](/reference/android/view/View) objects.
...
[`RemoteViews`](/reference/android/widget/RemoteViews) object. To include a
...
![](/static/images/appwidgets/StackWidget.png)
...
* If the user touches the top view, the widget displays the
  [`Toast`](/reference/android/widget/Toast) message "Touched view *n*," where
...
manifest](/guide/topics/appwidgets#Manifest), you need to make it possible for widgets with
collections to bind to your `RemoteViewsService`. Do this by declaring the
service in your manifest file with the permission
[`BIND_REMOTEVIEWS`](/reference/android/Manifest.permission#BIND_REMOTEVIEWS).
...
[`AppWidgetProvider`](/reference/android/appwidget/AppWidgetProvider) subclass
typically goes in
...
[`setRemoteAdapter()`](/reference/android/widget/RemoteViews#setRemoteAdapter(int,%0Aandroid.content.Intent)). This tells the collection view where to get its data.
...
[`ContentProvider`](/reference/android/content/ContentProvider) whose data
...
**Note:** See
[`WeatherDataProvider.java`](https://android.googlesource.com/platform/development/+/master/samples/WeatherListWidget/src/com/example/android/weatherlistwidget/WeatherDataProvider.java)
for a code sample demonstrating how to persist a widget’s underlying data using
`ContentProvider`.
...
[`onCreate()`](/reference/android/widget/RemoteViewsService.RemoteViewsFactory#o%0AnCreate()) and
[`getViewAt()`](/reference/android/widget/RemoteViewsService.RemoteViewsFactory#%0AgetViewAt(int)).
...
class](/guide/topics/appwidgets#handle-events), you normally use
[`setOnClickPendingIntent()`](/reference/android/widget/RemoteViews#setOnClickPendingIntent(int,%0Aandroid.app.PendingIntent)) to set an object's click behavior—such as to
cause a button to launch an [`Activity`](/reference/android/app/Activity). But
...
[`setOnClickFillInIntent()`](/reference/android/widget/RemoteViews#setOnClickFillInIntent(int,%0Aandroid.content.Intent)). This entails setting up a pending intent template for
...
* The `StackWidgetProvider`—an [`AppWidgetProvider`](/reference/android/appwidget/AppWidgetProvider)
...
* This broadcast is intercepted by the `StackWidgetProvider` class's
  [`onReceive()`](/reference/android/appwidget/AppWidgetProvider#onReceive(android.content.Context,%0Aandroid.content.Intent)) method, and the widget displays the `Toast` message
...
**Note:** The `StackWidget` sample uses a broadcast, but typically a widget launches
an activity in a scenario like this.
...
[`PendingIntent`](/reference/android/app/PendingIntent) template to determine
...
![](/static/images/appwidgets/appwidget_collections.png)
...
To do this, use the
[`AppWidgetManager`](/reference/android/appwidget/AppWidgetManager) to call
[`notifyAppWidgetViewDataChanged()`](/reference/android/appwidget/AppWidgetManager#notifyAppWidgetViewDataChanged(int,%0Aint)). This call results in a callback to your `RemoteViewsFactory` object’s
[`onDataSetChanged()`](/reference/android/widget/RemoteViewsService.RemoteViewsFactory#onDataSetChanged())
...
[`getLoadingView()`](/reference/android/widget/RemoteViewsService.RemoteViewsFactory#getLoadingView())
...
items)`](/reference/android/widget/RemoteViews#setRemoteAdapter(int,%20android.widget.RemoteViews.RemoteCollectionItems))
...
**Note:** When calling
[`setViewTypeCount()`](/reference/android/widget/RemoteViews.RemoteCollectionItems.Builder#setViewTypeCount(int)),
pass in the maximum number of different layout IDs that are used by
`RemoteViews` in this collection. Otherwise, the view type count is inferred
from the provided items, and the adapter is recreated when sending an update
that uses additional layouts.