---
title: Add previews to your widget picker  |  Views  |  Android Developers
...
# Add previews to your widget picker Stay organized with collections Save and categorize content based on your preferences.
...
ability to provide [`RemoteViews`](/reference/android/widget/RemoteViews?) to the widget picker

Apps can use `setWidgetPreview` in [`AppWidgetManager`](/reference/kotlin/android/appwidget/AppWidgetManager?). To prevent abuse
...
[`previewLayout`](/reference/android/appwidget/AppWidgetProviderInfo#previewLayout)
...
[Build accurate previews that include dynamic items](#build-accurate-previews).
...
widget, or as a fallback for scalable previews, specify the [`previewImage`](/reference/android/appwidget/AppWidgetProviderInfo#previewImage)
attribute.
...
![](/static/images/appwidgets/missing-list.png)
...
a widget preview for a widget with a [collection view](/guide/topics/appwidgets/collections)—that is, a
...
* A placeholder collection view with fake items. For example, you can mimic a
  `ListView` by providing a placeholder `LinearLayout` with several fake list
  items.
...
items are [`TextView`](/reference/android/widget/TextView) objects. It can be
