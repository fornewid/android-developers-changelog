---
title: Create a search interface  |  Views  |  Android Developers
...
# Create a search interface Stay organized with collections Save and categorize content based on your preferences.
...
* The **search widget** is an instance of
  `SearchView` that you can
  place anywhere in your layout. By default, the search widget behaves like a
  standard
  `EditText`
  widget and doesn't do anything, but you can configure it so that the Android
  system handles all input events, delivers queries to the appropriate
  activity, and provides search suggestions—just like the search
  dialog.
...
user input into the search widget yourself, using various callback methods and
listeners. If you want to handle user input yourself, read the reference
...
* A searchable activity
  :   The `Activity`
      that receives the search query, searches your data, and displays the search
      results.
...
[search configuration](/develop/ui/views/search/searchable-config).
...
`SearchableInfo`
object. You can't create this object yourself at runtime. You must declare the
search configuration in XML.

The search configuration file must include the
[`<searchable>`](/guide/topics/search/searchable-config#searchable-element)
...
`android:hint` as "Search <content-or-product>". For example,
...
[search suggestions](#SearchSuggestions) and
[voice search](#VoiceSearch). For detailed information about the
search configuration file, see the
[Search configuration](/guide/topics/search/searchable-config)
reference document.
...
1. Declare the activity to accept the `ACTION_SEARCH` intent in an
   [`<intent-filter>`](/guide/topics/manifest/intent-filter-element)
   element.
2. Specify the search configuration to use in a
   `<meta-data>`
   element.
...
with the
`DEFAULT` value—which you usually see in `<activity>`
elements—because the system delivers the `ACTION_SEARCH`
intent explicitly to your searchable activity using its component name.
...
1. [Receive the query](#ReceivingTheQuery).
2. [Search your data](#SearchingYourData).
3. [Present the results](#PresentingTheResults).
...
* If your data is stored in a SQLite database on the device, performing a
  full-text search—using FTS3, rather than a `LIKE`
  query—can provide a more robust search across text data and can
  produce results significantly faster. See
  [sqlite.org](http://sqlite.org/fts3.html) for
  information about FTS3 and the
  `SQLiteDatabase`
...
* If your data is stored online, then the perceived search performance might
  ...
  `android.net`
  for a reference of network APIs and
  [`ProgressBar`](/reference/android/widget/ProgressBar)
  for information about how to display a progress indicator.

**Note:** If you plan to use a local database, see
[Save data in a local database using
...
[creating a searchable activity](#SearchableActivity), a searchable
...
default, so you don't need to add this declaration to
`SearchableActivity`.
...
[options menu](/guide/topics/ui/menus#options-menu) or UI layout
that calls `onSearchRequested()`.

**Note:** If your app uses an [app
bar](/training/appbar), then don't use the search dialog for your search interface. Instead,
use the [search widget](#UsingSearchWidget) as a collapsible view
in the app bar.
...
by calling
`setDefaultKeyMode`—or
`DEFAULT_KEYS_SEARCH_LOCAL`—during
your activity's
`onCreate()`
method.
...
data](#SearchContextData)—discussed in another section of this document—end the
...
in [Introduction to
activities](/guide/components/activities#Lifecycle). However, if the current activity is the searchable activity,
then one of two things happens:

* By default, the searchable activity receives the
  `ACTION_SEARCH` intent with a call to `onCreate()`,
  and a new instance of the activity is brought to the top of the activity
  stack. There are now two instances of your searchable activity in the
  activity stack, so tapping the Back button takes you back to the previous
  instance of the searchable activity, rather than exiting the searchable
  activity.
* If you set `android:launchMode` to `"singleTop"`,
  then the searchable activity receives the `ACTION_SEARCH` intent
  with a call to
  `onNewIntent(Intent)`,
  passing the new `ACTION_SEARCH` intent. For example, here's how
  you might handle this case, in which the searchable activity's launch mode
  is `"singleTop"`:
  ...
  [performing a search](#PerformingSearch), all the code to handle the
...
the `onSearchRequested()` callback method. To activate the search
dialog in your activity, always call `onSearchRequested()`.
Otherwise, `onSearchRequested()` isn't called and
customizations—such as the addition of `appData` in the
preceding example—are missed.

## Use the search widget

![An image showing a search view in the app top bar](/static/images/ui/search-ui-topbar-33.png)
...
doesn't fit in the app bar. See the section about [using
both the widget and the dialog](#UsingBoth).

### Configure the search widget

After you create a
[search configuration](#SearchableConfiguration) and a
[searchable activity](#SearchableActivity), enable assisted search
...
[search suggestions](#SearchSuggestions) for the search widget.
...
providers](/training/appbar/action-views).
...
[action view](/guide/topics/ui/actionbar#ActionView) and enable
...
this situation, see
[Add the app bar](/guide/topics/ui/actionbar).
...
the [Search configuration](/develop/ui/views/search/searchable-config) reference for more
...
:   These suggestions are words that the user previously used as search
    ...
    custom search suggestions](/develop/ui/views/search/adding-recent-query-suggestions) for more information.
...
:   These are search suggestions that you provide from your own data source to
    ...
    suggestions](/develop/ui/views/search/adding-custom-suggestions) for more information.