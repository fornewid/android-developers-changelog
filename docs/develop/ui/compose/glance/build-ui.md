---
title: Build UI with Glance  |  Jetpack Compose  |  Android Developers
...
Glance supports [`Scaffold`](/reference/kotlin/androidx/glance/appwidget/components/Scaffold.composable#Scaffold(androidx.glance.GlanceModifier,kotlin.Function0,androidx.glance.unit.ColorProvider,androidx.compose.ui.unit.Dp,kotlin.Function0)) objects. Place your `Column`, `Row`, and
`Box` composables within a given `Scaffold` object.

![Image of a column, row, and box layout.](/static/develop/ui/compose/images/column_row_box.png)
...
**Note:** Glance translates its composables into actual `Views`, using the
defined modifiers and parameters to create the underlying view. Thus, the same
Android View concepts (such as weights) apply when building certain composables
with Glance.
...
**Note:** Glance translates a `LazyColumn` into an actual `ListView` with the
translation of the provided items. Thus, the same limitations and restrictions
of `RemoteViews` collections apply.
...
**Note:** The best fitting size is one of the provided `DpSize` that fits within
the available bounding box with minimal distortion. If none fits, the smallest
one is used.
...
**Note:** Responsive layouts were introduced in Android 12. Before Android 12, the
composable is called for each size at which the app widget may be displayed
(similar to `SizeMode.Exact`). For each size, the best view is chosen, which is
the largest one that fits in the available space, or the smallest one if none
fits.
...
**Note:** See the [CompoundButtonAppWidget.kt](https://cs.android.com/androidx/platform/frameworks/support/+/androidx-main:glance/glance-appwidget/integration-tests/demos/src/main/java/androidx/glance/appwidget/demos/CompoundButtonAppWidget.kt) sample for a more detailed
example.
...
| --- | --- | --- | --- |
| Filled Button | alt_text | [Component](https://cs.android.com/androidx/platform/frameworks/support/+/androidx-main:glance/glance-appwidget/src/main/java/androidx/glance/appwidget/components/Buttons.kt) |  |
| Outline Buttons | alt_text | [Component](https://cs.android.com/androidx/platform/frameworks/support/+/androidx-main:glance/glance-appwidget/src/main/java/androidx/glance/appwidget/components/Buttons.kt) |  |
| Icon Buttons | alt_text | [Component](https://cs.android.com/androidx/platform/frameworks/support/+/androidx-main:glance/glance-appwidget/src/main/java/androidx/glance/appwidget/components/Buttons.kt) | Primary / Secondary / Icon-only |
| Title Bar | alt_text | [Component](https://cs.android.com/androidx/platform/frameworks/support/+/androidx-main:glance/glance-appwidget/src/main/java/androidx/glance/appwidget/components/TitleBar.kt) |  |
...
Manage and update GlanceAppWidget](/develop/ui/compose/glance/glance-app-widget)

[Next

Implement a Glance theme

arrow\_forward](/develop/ui/compose/glance/theme)