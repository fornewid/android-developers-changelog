---
title: Set item behavior  |  Jetpack Compose  |  Android Developers
...
# Set item behavior Stay organized with collections Save and categorize content based on your preferences.
...
| `Auto`  (default) | Use the item's maximum intrinsic size.  For example, a `Text` composable's maximum intrinsic width is the width of all its text on a single line - no wrapping. | ``` FlexBox {     RedRoundedBox(         Modifier.flex { basis(FlexBasis.Auto) }     )     BlueRoundedBox(         Modifier.flex { basis(FlexBasis.Auto) }     ) } FlexBoxSnippets.kt ``` | Items sized based on their intrinsic size using basis Auto. |
| Fixed `dp` | A fixed size in Dp. | ``` FlexBox {     RedRoundedBox(         Modifier.flex { basis(200.dp) }     )     BlueRoundedBox(         Modifier.flex { basis(100.dp) }     ) } FlexBoxSnippets.kt ``` | Items sized to a fixed dp value using basis. |
| Percentage | A percentage of the container size. | ``` FlexBox {     RedRoundedBox(         Modifier.flex { basis(0.7f) }     )     BlueRoundedBox(         Modifier.flex { basis(0.3f) }     ) } FlexBoxSnippets.kt ``` | Items sized as a percentage of container size using basis. |
...
|  |  |
| --- | --- |
| ``` FlexBox {     RedRoundedBox(         title = "400dp",         modifier = Modifier.flex { grow(1f) }     )     BlueRoundedBox(title = "100dp")     GreenRoundedBox(title = "100dp") } FlexBoxSnippets.kt ``` | Each child has a basis value of `100dp`. There is `300dp` of extra space. Three items with 100dp basis each, in a 600dp container, before growth. Child 1 grows by `300dp` to fill the extra space. First item grows to fill 300dp of extra space. |
...
|  |  |
| --- | --- |
| ``` FlexBox {     RedRoundedBox(         title = "150dp",         modifier = Modifier.flex { grow(1f) }     )     BlueRoundedBox(         title = "200dp",         modifier = Modifier.flex { grow(2f) }     )     GreenRoundedBox(         title = "250dp",         modifier = Modifier.flex { grow(3f) }     ) } FlexBoxSnippets.kt ``` | Each child has a basis value of `100dp`. There is `300dp` of extra space. Three items with 100dp basis each, in a 600dp container, before growth, with different grow values. The total grow value is 6.  Child 1 grows by (1 / 6) \* 300 = `50dp`  Child 2 grows by (2 / 6) \* 300 = `100dp`  Child 3 grows by (3 / 6) \* 300 = `150dp` Items grow to fill 300dp of extra space based on relative grow values. |
...
| `700dp` | Two items in a 700dp container. |
| `500dp` | First item shrinks as container size reduces to 500dp. |
| `450dp` | First item shrinks further as container size reduces to 450dp. |

## Item alignment

Use `alignSelf` to control how an item is aligned to the cross axis. This
overrides the [`alignItems` property](/develop/ui/compose/layouts/adaptive/flexbox/container-behavior#align-distribute) of the container for this item. It
...
![Five children of varying sizes overriding the alignItems property.](/static/develop/ui/compose/images/layouts/adaptive/flexbox/item-alignment.png)
...
![Two rounded boxes, with the first containing the text Hello and the second containing the text World.](/static/develop/ui/compose/images/layouts/adaptive/flexbox/itemorder.png)
...
arrow\_forward](/develop/ui/compose/layouts/adaptive/grid)