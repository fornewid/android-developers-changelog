---
title: Add shadows in Compose  |  Jetpack Compose  |  Android Developers
...
* [`Modifier.shadow()`](/reference/kotlin/androidx/compose/ui/Modifier#(androidx.compose.ui.Modifier).shadow(androidx.compose.ui.unit.Dp,androidx.compose.ui.graphics.Shape,kotlin.Boolean,androidx.compose.ui.graphics.Color,androidx.compose.ui.graphics.Color)): Creates an elevation-based shadow behind a composable
  that conforms to Material Design guidelines.
* [`Modifier.dropShadow()`](/reference/kotlin/androidx/compose/ui/Modifier#%28androidx.compose.ui.Modifier%29.dropShadow%28androidx.compose.ui.graphics.Shape,kotlin.Function1%29): Creates a customizable shadow that appears behind a
  composable, making it appear elevated.
* [`Modifier.innerShadow()`](/reference/kotlin/androidx/compose/ui/Modifier#%28androidx.compose.ui.Modifier%29.innerShadow%28androidx.compose.ui.graphics.Shape,kotlin.Function1%29): Creates a shadow inside the borders of a
  composable, making it appear pressed into the surface behind it.
...
[animate shadows](#animate-shadows) upon user interaction and how to chain the
`innerShadow()` and `dropShadow()` modifiers to
create [gradient shadows](#create-gradient),
[neumorphic shadows](#create-neumorphic), and more.

## Create basic shadows

[`Modifier.shadow()`](/reference/kotlin/androidx/compose/ui/Modifier#(androidx.compose.ui.Modifier).shadow(androidx.compose.ui.unit.Dp,androidx.compose.ui.graphics.Shape,kotlin.Boolean,androidx.compose.ui.graphics.Color,androidx.compose.ui.graphics.Color)) creates a basic shadow following [Material Design
...
![A gray shadow cast around a white rectangular shape.](/static/develop/ui/compose/images/graphics/shadows/simple-shadow.png)


**Figure 1.** An elevation-based shadow created with `Modifier.shadow()`.

**Note:** You can only use elevation with `Modifier.shadow()`, and cannot customize
properties like shadow spread or radius. For more granular control of the shadow
rendering, use [drop shadows](#implement-drop) or
[inner shadows](#implement-inner) instead.

## Implement drop shadows

Use the [`dropShadow()`](/reference/kotlin/androidx/compose/ui/Modifier#%28androidx.compose.ui.Modifier%29.dropShadow%28androidx.compose.ui.graphics.Shape,kotlin.Function1%29) modifier to draw an accurate shadow behind your
...
* The `dropShadow()` modifier is applied to the inner `Box`. The shadow has the
  following characteristics:
  + A rounded rectangle shape (`RoundedCornerShape(20.dp)`)
  + A blur radius of `10.dp`, making the edges soft and diffused
  + A spread of `6.dp`, which expands the shadow's size and makes it larger than
    the box casting it
...
![A gray drop shadow cast around a white rectangular shape.](/static/develop/ui/compose/images/graphics/shadows/drop-shadow.png)


**Figure 2.** A drop shadow drawn around the shape.

## Implement inner shadows

To create an inverse effect to `dropShadow()`, use
[`Modifier.innerShadow()`](/reference/kotlin/androidx/compose/ui/Modifier#%28androidx.compose.ui.Modifier%29.innerShadow%28androidx.compose.ui.graphics.Shape,kotlin.Function1%29), which creates the illusion that an element is
...
![A gray inner shadow inside a white rectangular shape.](/static/develop/ui/compose/images/graphics/shadows/inner-shadow.png)


**Figure 3.** An application of `Modifier.innerShadow()` on a rounded corner
rectangle.

## Animate shadows on user interaction

To make your shadows respond to user interactions, you can integrate shadow
properties with [Compose's animation APIs](/develop/ui/compose/animation/introduction). When a user presses a button, for
...
* Declares the start and end states for the parameters to animate upon press
  with `transition.animateColor` and `transition.animateFloat`.
* Uses [`updateTransition`](/reference/kotlin/androidx/compose/animation/core/updateTransition.composable#updateTransition(kotlin.Any,kotlin.String)) and provides it with the chosen
  `targetState (targetState = isPressed)` to verify all animations are
  synchronized. Whenever `isPressed` changes, the transition object automatically
  manages the animation of all child properties from their current values to the
  new target values.
* Defines the `buttonPressAnimation` specification, which controls the timing
  and easing of the transition. It specifies a [`tween`](/reference/kotlin/androidx/compose/animation/core/package-summary#tween(kotlin.Int,kotlin.Int,androidx.compose.animation.core.Easing)) (short for
  in-between) with a duration of 400 milliseconds and an `EaseInOut` curve, which
  means the animation starts slow, speeds up in the middle, and slows down at the
  end.
* Defines a `Box` with a chain of modifier functions that apply all the
  animated properties to create the visual element, including the following:
  + [.`clickable()`](/reference/kotlin/androidx/compose/ui/Modifier#(androidx.compose.ui.Modifier).clickable(kotlin.Boolean,kotlin.String,androidx.compose.ui.semantics.Role,androidx.compose.foundation.interaction.MutableInteractionSource,kotlin.Function0)): A modifier that makes the `Box` interactive.
  + `.dropShadow()`: Two outer drop shadows are applied first. Their color and
    alpha properties are linked to the animated values (`blueDropShadow`, etc)
    and create the initial raised appearance.
  + `.innerShadow()`: Two inner shadows are drawn on top of the background.
    Their properties are linked to the other set of animated values
    (`innerShadowColor1`, etc) and create the indented appearance.

### Result

[

](/static/develop/ui/compose/images/graphics/shadows/AnimatedShadows3.mp4)
...
* `brush = Brush.sweepGradient(colors)` colors the shadow with a gradient that
  rotates through a list of predefined `colors`, creating a rainbow-like effect.
...
](/static/develop/ui/compose/images/graphics/shadows/inhaleExhale2.mp4)
...
2. Apply two faint, opposing drop shadows: a light shadow to one corner, and a
   dark shadow to the opposite corner.
...
![A white rectangular shape with a neumorphic effect against a white background.](/static/develop/ui/compose/images/graphics/shadows/neumorphic-shadow.png)
...
![A red border around a white rectangle with a blue shadow against a yellow background.](/static/develop/ui/compose/images/graphics/shadows/neobrutal-shadow.png)
...
* Two chained `dropShadow()` modifiers with distinct properties are applied,
  followed by a `background()` modifier.
* Chained `innerShadow()` modifiers are applied to forge the metallic rim effect
  around the component's edge.

#### Result

The previous code snippet produces the following:

![A white realistic shadow around a black rounded shape.](/static/develop/ui/compose/images/graphics/shadows/realistic-shadow.png)
...
Mesh gradients](/develop/ui/compose/graphics/draw/mesh-gradient)