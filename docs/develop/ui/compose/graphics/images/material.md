...
The `Icon` composable is a convenient way to draw a single color icon on screen that follows [Material Design guidelines](https://material.io/design/iconography/system-icons.html#grid-and-keyline-shapes). To use `Icon`, include the [Compose Material](https://developer.android.com/jetpack/androidx/releases/compose-material) library (or the [Compose Material 3](https://developer.android.com/jetpack/androidx/releases/compose-material3) library).
...
By default, the `Icon` composable is tinted with `LocalContentColor.current` and is `24.dp` in size. It also exposes a `tint` color parameter (which leverages the same mechanism for tinting as described in the [Image tint](https://developer.android.com/develop/ui/compose/graphics/images/customize#tint-image) section). The `Icon` composable is intended for use for small icon elements. You should use the `Image` composable for more customization options.
