...
way to publish both Sharing Shortcuts and `ChooserTargets`. For
instructions, see [Use AndroidX to provide both Sharing Shortcuts and
ChooserTargets](https://developer.android.com/develop/ui/compose/sharing/direct-share-targets#androidx-compat-library) on this page.
...
3. If you're developing a communication app, report shortcut usage through
   ...
   communications apps](https://developer.android.com/develop/ui/compose/sharing/direct-share-targets#track-shortcut-usage-comms-apps) on this page for more information. For example, report usage for messages sent by the user by
...
- Call [`setCategories()`](https://developer.android.com/reference/androidx/core/content/pm/ShortcutInfoCompat.Builder#setCategories(java.util.Set%3Cjava.lang.String%3E)) to associate the shortcut with the appropriate [`mimeType`
  attributes](https://developer.android.com/develop/ui/compose/sharing/receive#supporting-mime-types). For example, for an SMS app, if the contact is not RCS- or MMS-enabled, you wouldn't associate the corresponding shortcut with non-text MIME types such as `image/*` and `video/*`.
...
:   Starting with Android 10, categories are also used to filter shortcuts that
    ...
    target](https://developer.android.com/develop/ui/compose/sharing/direct-share-targets#declare-share-target) for details. This field is required for shortcuts
...
:   Specifies whether or not a shortcut is valid when it has been unpublished or
    ...
    is long lived, it can be cached by various system services even after it has
    been unpublished as a dynamic shortcut.

    Making a shortcut long lived can improve its ranking. See [Get the best
    ranking](https://developer.android.com/develop/ui/compose/sharing/direct-share-targets#get-best-ranking) for details.
