---
title: About notifications  |  Jetpack Compose  |  Android Developers
...
# About notifications Stay organized with collections Save and categorize content based on your preferences.
...
* [Create a notification](/develop/ui/compose/notifications/create-notification):
  learn how to build and issue a basic notification.
* [Notification permission](/develop/ui/compose/notifications/notification-permission):
  learn how to handle the `POST_NOTIFICATIONS` runtime permission.
* [Notification channels](/develop/ui/compose/notifications/channels):
  let users control notification behaviors.
* [Expandable notifications](/develop/ui/compose/notifications/expanded):
  show more content using styles like `BigTextStyle` or `MessagingStyle`.
* [Bubbles](/develop/ui/compose/notifications/bubbles):
  use floating, expandable overlays for conversations.
* [Progress-centric notifications](/develop/ui/compose/notifications/progress-centric):
  track user-initiated journeys like rideshare and deliveries.
* [Call-style notifications](/develop/ui/compose/notifications/call-style):
  distinguish incoming and ongoing calls from other notification types.
* [Live update notifications](/develop/ui/compose/notifications/live-update):
  track progress for ongoing activities.
* [View-specific notification guides](/develop/ui/views/notifications):
  guides for implementing notifications in View-based apps.

For more information about notification design and interaction patterns, see the
[Notifications design
guide](/design/ui/mobile/guides/home-screen/notifications).
...
[`setLocalOnly()`](/reference/android/app/Notification.Builder#setLocalOnly(boolean)). An example is a notification confirming that a media
download has completed. For more guidance specific to Wear OS notifications, see
[Wear OS notifications](/training/wearables/notifications) guidance.

### Status bar and notification drawer

When you issue a notification, it first appears as an icon in the status bar.

![](/static/images/ui/notifications/notification-area_2x.png)
...
![](/static/images/ui/notifications/notification-drawer_2x.png)
...
services](/develop/background-work/services/fgs/handle-user-stopping).
...
![](/static/images/ui/notifications/heads-up_2x.png)
...
* The user's activity is in fullscreen mode, such as when the app uses
  [`fullScreenIntent`](/reference/android/app/Notification#fullScreenIntent).
* The notification has high priority and uses ringtones or vibrations on
  devices running Android 7.1 (API level 25) and lower.
* The [notification channel](#ManageChannels) has high importance on devices
...
![](/static/images/ui/notifications/lock-screen_2x.png)

**Figure 4.** Notifications on the lock screen with sensitive content
hidden.

To learn more, see [Set lock screen
visibility](/develop/ui/compose/notifications/create-notification#lockscreenNotification).
...
badge](/develop/ui/views/notifications/badges).
...
wearable-specific features to your notification](/training/wearables/notifications#add-wearable-features).

![](/static/images/ui/notifications/wear_2x.png)
...
3. Time stamp: provided by the system, but you can override it using
   ...
   or hide it using
   `setShowWhen(false)`.
4. Large icon: optional; usually used only for contact photos.
   Don't use it for your app icon. Set using
   `setLargeIcon()`.
5. Title: optional; set using
   `setContentTitle()`.
6. Text: optional; set using
   `setContentText()`.
...
layout](/develop/ui/views/notifications/custom-notification).

For more information about how to create a notification with these features and
more, read [Create a notification](/develop/ui/compose/notifications/create-notification).
...
![](/static/images/ui/notifications/notification-actions_2x.png)
...
notification](/develop/ui/compose/notifications/create-notification).
...
action, pass `true` into [`setAuthenticationRequired()`](/reference/android/app/Notification.Action.Builder#setAuthenticationRequired(boolean)) when creating your
...
read [Create an expandable notification](/develop/ui/compose/notifications/expanded).

## Notification updates and groups

To avoid flooding your users with multiple or redundant notifications when you
have additional updates, [update an existing notification](/develop/ui/compose/notifications/create-notification#Updating) rather than
issuing a new one or use the [inbox-style notification](/develop/ui/compose/notifications/expanded#inbox-style) to show
...
![](/static/images/ui/notifications/notification-group_2x.png)

**Figure 10.** A collapsed and expanded notification group.

To learn how to add notifications to a group, see [Create a group of
notifications](/develop/ui/views/notifications/group).

**Note:** If the same app sends four or more notifications and doesn't specify a
grouping, the system automatically groups them together.
...
channels](/develop/ui/compose/notifications/channels).
...
the notification's [`priority`](/reference/androidx/core/app/NotificationCompat.Builder#setPriority(int)).

On Android 8.0 (API level 26) and higher, the importance of a notification is
determined by the [`importance`](/reference/android/app/NotificationChannel#setImportance(int)) of the channel the notification is posted
to. Users can change the importance of a notification channel in the system
settings, as shown in figure 12.

![](/static/images/ui/notifications/channel-alert-levels_2x.png)
...
importance](/develop/ui/compose/notifications/create-notification#Priority).
...
* Priority only: users can configure which system-wide categories can
  interrupt them, such as only alarms, reminders, events, calls, or messages.
  For messages and calls, users can filter based on sender or caller, as shown
  in figure 13.

![](/static/images/ui/notifications/do-not-disturb-filter-settings_2x.png)
...
[set a system-wide category](/develop/ui/compose/notifications/create-notification#metadata).

## Notifications for foreground services

A notification is required when your app is running a *foreground
service*—a [`Service`](/reference/android/app/Service) running in the
...
services](/guide/components/foreground-services). If you are building a media
player, also read [Playing media in the background](/media/implement/playback-app#playing_media_in_the_background).
...
![](/static/images/ui/notifications/notification-cooldown-settings.png)
