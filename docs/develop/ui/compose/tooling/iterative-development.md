---
title: Iterative code development  |  Jetpack Compose  |  Android Developers
...
* **Match UI to target image**: Select to upload a reference design mock.
  The agent then suggests code changes to make your implementation match the
  design as closely as possible.

![](/static/studio/preview/features/images/align-ui-agent.gif)


**Figure 1.** Update the UI to match a target image.

* **Transform UI**: Use natural language prompts (for example, "change the
  button color to blue") to describe specific modifications you want to make
  to your UI.

![](/static/studio/preview/features/images/transfrom-ui-agent-both.gif)


**Figure 2.** Ask the agent to transform the UI.
...
requires Jetpack Compose Runtime 1.3.0 or
higher and AGP 8.1.0-alpha05 or higher. Google is continuously working to improve this
feature and welcomes your feedback.
If you find an issue, [please report it](https://issuetracker.google.com/issues/new?component=192708&template=840533).
Include information from Logcat and a description of the code change you were
making. You can also check the [list of open issues](https://issuetracker.google.com/issues?q=status:open%20componentid:1189787&s=created_time:desc).
...
**Important:** The manual mode shortcut was updated to
`Control+'` (`Command+'` on macOS) in Koala Feature Drop.
Previous versions of Android Studio
used `Control+\` (`Command+\` on macOS) as the manual mode
shortcut.
...
* Automatic: Changes are applied in your device or emulator when you update a
  composable function.
...
[Apply Changes](#apply-changes). Instead, it's designed to optimize your
...
2. Live Edit as much as possible until you need to make a change that Live Edit
   doesn't support, such as adding new methods while the app is running.
3. After you make an unsupported change, click **Run** ![Run
   icon](/static/studio/images/buttons/toolbar-run.png) to restart
...
3. Choose the **Empty Compose Activity** template for **Phone and Tablet**, and
   ...
   ![Template selection in Android Studio](/static/studio/images/run/empty-compose-activity.png)


   **Figure 3.** Templates you can choose from. For Live Edit,
...
4. Complete the **New Project** dialog with the required information: name,
   ...
   ![Example project settings from Step 4 entered in Android Studio](/static/studio/images/run/example-project-settings.png)


   **Figure 4.** Example project settings.
5. Click **Finish**.
...
![Changes to Greeting method applied on a device](/static/studio/images/run/device-le-example.png)


**Figure 6.** Test device displaying Live Edit changes to the
`Greeting` method.
...
![Live Edit out of date icon](/static/studio/images/run/live-edit-out-of-date.png)
...
* [Only applies to Android Studio Giraffe and higher] Live Edit requires [Compose Runtime
  1.3.0 or higher](/develop/ui/compose/bom/bom-mapping).
...
* A running app might crash when you edit it with Live Edit. If this happens,
  ...
  button](/static/studio/images/buttons/toolbar-run-darkmode.png) button.
...
* [Live Edit](#live-edit) supports iterative development with Jetpack Compose
...
**Deprecated:** Live Edit of literals, an earlier version of Live Edit that is
limited to just a few supported types, is deprecated. We recommend using
[Live Edit](#live-edit) instead.
...
updating
dynamically](/static/develop/ui/compose/images/tooling-live-literals-demo.gif)
...
Literals](/static/develop/ui/compose/images/live-editing-of-literals.gif)
...
[Apply Changes](/studio/run#use-apply-changes) lets you update code and resources
without having to redeploy your app to an emulator or physical device (with some
[limitations](/studio/run#apply-changes-limitations)).
...
button](/static/develop/ui/compose/images/tooling-apply-changes.png)
...
Preview and debug animations](/develop/ui/compose/tooling/animation-preview)

[Next

Editor actions

arrow\_forward](/develop/ui/compose/tooling/editor-actions)