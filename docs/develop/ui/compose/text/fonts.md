---
title: Work with fonts  |  Jetpack Compose  |  Android Developers
...
are [included](/reference/kotlin/androidx/compose/ui/text/font/FontFamily#summary):
...
![The words ](/static/develop/ui/compose/images/text-different-fonts.png)

You can use the `fontFamily` attribute to work with custom fonts and typefaces
defined in the `res/font` folder:

![Graphical depiction of the res > font folder in the development environment ](/static/develop/ui/compose/images/text-font-folder.png)

This example shows how you would define a `fontFamily` based on those font
files and using the [`Font` function](/reference/kotlin/androidx/compose/ui/text/font/package-summary#Font(kotlin.Int,androidx.compose.ui.text.font.FontWeight,androidx.compose.ui.text.font.FontStyle)):
...
![The words ](/static/develop/ui/compose/images/text-font-family.png)

To learn how to set the typography in your entire app, see [Custom design systems in Compose](/develop/ui/compose/designsystems/custom).

## Downloadable fonts

Starting in [Compose
1.2.0](/jetpack/androidx/releases/compose-ui#1.2.0),
...
1. Add the dependency:
   ...
   ```
2. Initialize the
   [`GoogleFont.Provider`](/reference/kotlin/androidx/compose/ui/text/googlefonts/GoogleFont.Provider)
   ...
   * A list of sets of hashes for the certificates to verify the identity of the
     ...
     [in the `font_certs.xml` file](https://github.com/android/compose-samples/blob/main/Jetchat/app/src/main/res/values-v23/font_certs.xml) in the
     Jetchat sample app.- Define a
     [`FontFamily`](/reference/kotlin/androidx/compose/ui/text/font/FontFamily):
     ...
     You can query for other parameters for your font like weight and style with
     [`FontWeight`](/reference/kotlin/androidx/compose/ui/text/font/FontWeight) and
     ...
     - Configure the `FontFamily` to be
       used in your Text composable function:
...
![](/static/develop/ui/compose/images/text-dl-fontfamily.png)

You can also define
[Typography](/reference/kotlin/androidx/compose/material3/Typography) to use
your `FontFamily`:
...
Pass it to the
[`createFontFamilyResolver`](/reference/kotlin/androidx/compose/ui/text/font/package-summary#createFontFamilyResolver(android.content.Context))
...
[`isAvailableOnDevice`](/reference/kotlin/androidx/compose/ui/text/googlefonts/package-summary#(androidx.compose.ui.text.googlefonts.GoogleFont.Provider).isAvailableOnDevice(android.content.Context))
...
![Five configurations of the same variable font with different axis values.](/static/develop/ui/compose/images/text/vf-configs.png)


**Figure 1.** Text using the same variable font customized with different axis
values.
...
1. Download the variable font you want to use (for example [Roboto Flex](https://fonts.google.com/specimen/Roboto+Flex)%7B:.external%7D)) and
   ...
   **Note:** Variable fonts are not currently supported via downloadable fonts. See the
   latest updates in [this bug](https://issuetracker.google.com/issues/223262013).
2. To load a variable font, define a `FontFamily` using the font placed in the
   ...
   [weight](/reference/kotlin/androidx/compose/ui/text/font/FontVariation#weight(kotlin.Int)), [width](/reference/kotlin/androidx/compose/ui/text/font/FontVariation#width(kotlin.Float)), and [slant](/reference/kotlin/androidx/compose/ui/text/font/FontVariation#slant(kotlin.Float)). These are standard axes that are
...
4. Extract the settings into a set of constants for easier reuse and replace the
   ...
   VariableFontsSnippets.kt
   ```
5. Configure the Material Design 3 [typography](/develop/ui/compose/designsystems/material3#typography) to use the `FontFamily`:
   ...
   This sample uses `displayLarge` [Material 3 typography](/develop/ui/compose/designsystems/material3#typography), which has different
...
![Three different texts, all showcasing different font configurations.](/static/develop/ui/compose/images/text/vf-standard-axes.png)
...
1. To use custom axes, define functions for the custom `ascenderHeight` and
   ...
   * Define guardrails for the values they can accept. As you can see in the
     [Variable fonts catalog](https://fonts.google.com/variablefonts?vfquery=roboto+flex), `ascenderHeight (YTAS)` has a
     minimum value of `649f` and a maximum of `854f`.
   * Return the font setting, so the configuration is ready to add to the font. In
     the `FontVariation.Setting()` method, the axis name (`YTAS, XTRA`) is
     hardcoded, and it takes the value as a parameter.
...
![Three different texts showing different configurations for variable fonts, with custom axes set— some have higher lowercase ascenders and are wider than before. ](/static/develop/ui/compose/images/text/vf-custom-axes.png)
