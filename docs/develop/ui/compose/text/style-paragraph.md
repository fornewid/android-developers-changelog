...
The `textAlign` parameter lets you set the horizontal [alignment](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/TextAlign) of the text within a `Text` composable surface area.
...
To add multiple styles in a paragraph, you can use [`ParagraphStyle`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/ParagraphStyle) in an [`AnnotatedString`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/AnnotatedString), which can be annotated with styles of arbitrary annotations. Once a portion of your text is marked with a `ParagraphStyle`, that portion is separated from the remaining text as if it had line feeds at the beginning and end.

For more information about adding multiple styles in a text, see [Add multiple styles in text](https://developer.android.com/develop/ui/compose/text/style-text#multiple-styles).

`AnnotatedString` has a [type-safe builder](https://kotlinlang.org/docs/reference/type-safe-builders.html) to make it easier to create: [`buildAnnotatedString`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/package-summary#buildAnnotatedString(kotlin.Function1)). The following snippet uses `buildAnnotatedString` to set `ParagraphStyle`:
...
[`includeFontPadding`](https://developer.android.com/reference/android/widget/TextView#attr_android:includeFontPadding) is a legacy property that adds extra padding based on font metrics at the top of the first line and bottom of the last line of a text. Starting from Compose BOM version `2024.01.01`, `includeFontPadding` is set to `false` by default, which brings the default text layout more in line with common design tools.

The ability to configure [`lineHeight`](https://developer.android.com/reference/android/widget/TextView#attr_android:lineHeight) is not new-- it has been available since Android Q. You can configure `lineHeight` for `Text` using the `lineHeight` parameter, which distributes the line height in each line of text. You can then use the new [`LineHeightStyle API`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle) to further configure how this text is aligned within the space, and remove whitespace.

You may want to adjust [`lineHeight`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/TextStyle#lineHeight()) using the text unit "em" (relative font size) instead of "sp" (scaled pixels) for improved precision. For more information about selecting an appropriate text unit, see [`TextUnit`](https://developer.android.com/reference/kotlin/androidx/compose/ui/unit/TextUnit).
...
In addition to adjusting `lineHeight`, you can now further center and style text using configurations with the [`LineHeightStyle` API](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle): [`LineHeightStyle.Alignment`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle.Alignment) and [`LineHeightStyle.Trim`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle.Trim) (`includeFontPadding` must be set to `false` for Trim to work). Alignment and Trim use the measured space in between lines of text to more appropriately distribute it to all lines-- including a single line of text and the top line of a block of text.

[`LineHeightStyle.Alignment`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle.Alignment) defines how to align the line in the space provided by the line height. Within each line, you can align the text to the top, bottom, center, or proportionally. `LineHeightStyle.Trim` then allows you to leave or remove the extra space to the top of the first line and bottom of the last line of your text, generated from any `lineHeight` and Alignment adjustments. The following samples show how multi-line text looks with various [`LineHeightStyle.Trim`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle.Trim) configurations when alignment is centered (`LineHeightStyle.Alignment.Center`).
...
See the [Fixing Font Padding in Compose Text](http://goo.gle/font-padding-compose) blog post to learn more about the context of this change, how `includeFontPadding` worked in the View system, and the changes made for Compose and the new [`LineHeightStyle`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle) APIs.

## Insert line breaks

The [`LineBreak`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/LineBreak) API defines the criteria by which text is split across multiple lines. You can specify the type of line breaking you want in the `TextStyle` block of your `Text` composable. The preset line breaking types include the following:
...
You can also customize `LineBreak` with the [`Strictness`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/LineBreak.Strictness) and [`WordBreak`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/LineBreak.WordBreak) APIs, which were **designed specifically for CJK languages**. You may not always see the effects of these APIs in non-CJK languages. Overall, line-breaking rules are defined based on locale.
...
The [`Hyphens`](https://developer.android.com/reference/kotlin/androidx/compose/ui/text/style/Hyphens) API lets you add hyphenation support to your app. *Hyphenation* refers to inserting a dash-like punctuation mark to indicate that a word is divided across lines of text. When enabled, hyphenation is added between the syllables of a word at appropriate hyphenation points.
