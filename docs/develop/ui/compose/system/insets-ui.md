...
When the IME is closed, `imePadding()` applies no padding. Consequently,
no insets are consumed, and the `Spacer` height matches the bottom system bars.
...
When the IME closes, the changes happen in reverse: the `Spacer` expands
from zero height as `imePadding()` applies less than the bottom system bars.
Finally, the `Spacer` height matches the system bars once the IME fully
animates out.
...
The value of insets is updated *after* the composition phase, but *before* the
