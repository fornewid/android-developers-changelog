...
take to launch your app and directly perform a specific task. A capability
consists of the name of the action and an `intent` targeting the destination in
your app that resolves the user intent.
...
1. Create a `shortcuts.xml` resource by following the instructions in [Create
   static shortcuts](https://developer.android.com/develop/ui/compose/system/shortcuts/creating-shortcuts#static).
2. Include the following required information in your capability:
   ...
   - **Parameter mappings:** each `intent` might contain parameters to be
     passed as `extra` data of the intent.
