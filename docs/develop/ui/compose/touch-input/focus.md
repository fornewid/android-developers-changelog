---
title: Focus in Compose  |  Jetpack Compose  |  Android Developers
...
* A ChromeOS user might use the *arrow keys* on their physical keyboard to
  navigate the screen.
* Someone playing a game could use their attached *game controller* to navigate
...
**Note:** For information about manually controlling focus order, see
[Control traversal order](/develop/ui/compose/accessibility/traversal).
...
* TABs: one-dimensional navigation, going *forward* or *backward*. TAB
  navigation advances focus to the next or previous element in the hierarchy. By
  default, Compose follows the declaration of the `Composables`. One-directional
  navigation can be achieved through the `tab` key on a keyboard, or the Rotary
  Bezel on a watch, and this kind of focus search will visit each element on the
  screen.
* Arrow keys: two-dimensional navigation, going *left, right, up*, or *down*.
  Two-dimensional navigation can be achieved through a D-Pad on a TV or arrow
  keys on a keyboard, and its traversal order only visits elements at a given
  level. You can use the D-Pad center and Back button to go down and back up to a
  different level.
...
![Screenshot of a list of buttons placed vertically one below the other in a small form factor.](/static/develop/ui/compose/images/touchinput/focus_compact_screen.png)
...
![Screenshot of a list of buttons placed in two columns side by side in a bigger form factor.](/static/develop/ui/compose/images/touchinput/focus_horizontal_navigation.gif)
...
![Screenshot of a list of buttons placed in two columns side by side in a bigger form factor.](/static/develop/ui/compose/images/touchinput/focus_vertical_navigation.gif)
...
**Note:** Focus properties are a special case, in which the parents always win in
case of collisions or duplicates.This is something to keep in mind when working
with focus.
...
arrow\_forward](/develop/ui/compose/touch-input/focus/change-focus-traversal-order)