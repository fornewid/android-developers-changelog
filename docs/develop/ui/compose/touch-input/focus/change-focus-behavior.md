---
title: Change focus behavior  |  Jetpack Compose  |  Android Developers
...
on your screen. For example, you might want to [group composables](#provide-coherent), [prevent
focus](#making-composable-unfocusable) on a certain composable, explicitly [request focus](#request-keyboard) on one,
[capture or release focus](#capture-release-focus), or [redirect focus](#redirect-focus) on entry or exit. This
...
![Animation of an app showing a top horizontal navigation and a list of items below.](/static/develop/ui/compose/images/touchinput/one_directional_navigation_1.gif)


**Figure 1**. Animation of an app showing a top horizontal navigation and a list of items below
...
![Animation of an app showing a top horizontal navigation and a list of items below.](/static/develop/ui/compose/images/touchinput/one_directional_navigation_2.gif)


**Figure 2**. Animation of an app showing a top horizontal navigation and a list of items below
...
**Note:** While the following code makes the composable unfocusable, it does not
prevent the composable from responding to touch or click events.
...
The first thing to do is associate a [`FocusRequester`](/reference/kotlin/androidx/compose/ui/focus/FocusRequester) object with the
...
modifier called [`Modifier.focusRequester`](/reference/kotlin/androidx/compose/ui/Modifier#(androidx.compose.ui.Modifier).focusRequester(androidx.compose.ui.focus.FocusRequester)):
...
You can call [FocusRequester's `requestFocus` method](/reference/kotlin/androidx/compose/ui/focus/FocusRequester#requestFocus()) to send actual focus requests. You should invoke this method outside of a `Composable` context
...
**Note:** A composable can only capture focus if it is already focused; for example,
by using `requestFocus` or tapping the element.
...
**Note:** You can specify that a specific `Composable` should receive focus with the
low-level `focusTarget()` Modifier but this is not recommended. Instead, use the
higher level `focusable()` Modifier.
...
|  |  |
| --- | --- |
| ``` Box(     Modifier         .onFocusChanged {}         .focusRequester(Default)         .focusable() ) FocusSnippets.kt ``` | ``` Box(     Modifier         .focusRequester(Default)         .onFocusChanged {}         .focusable() ) FocusSnippets.kt ``` |
...
![Animation of a screen showing two columns of buttons placed side by side and animating focus from one column to the other.](/static/develop/ui/compose/images/touchinput/two_dimensional_navigation_1.gif)


**Figure 3**. Animation of a screen showing two columns of buttons placed side by side and animating focus from one column to the other
...
**Note:** We refer to *exit* when the focus search leaves the level in which it's
exploring, and *enter* otherwise.
...
![Animation of a screen showing two columns of buttons placed side by side and animating focus from one column to the other.](/static/develop/ui/compose/images/touchinput/two_dimensional_navigation_2.gif)


**Figure 4**. Animation of a screen showing two columns of buttons placed side by side and animating focus from one column to the other
...
* [Change focus traversal order](/develop/ui/compose/touch-input/focus/change-focus-traversal-order)
...
arrow\_forward](/develop/ui/compose/touch-input/focus/react-to-focus)