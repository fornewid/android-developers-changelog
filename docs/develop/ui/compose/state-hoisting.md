...
The [`LazyColumn`](https://developer.android.com/reference/kotlin/androidx/compose/foundation/lazy/LazyColumn.composable) state is hoisted to the conversation screen so the app can perform UI logic and read the state from all composables that require it:
...
These plain classes are created and remembered in the Composition. Because they follow the [composable's lifecycle](https://developer.android.com/develop/ui/compose/lifecycle), they can take types provided by the Compose library such as [`rememberNavController()`](https://developer.android.com/reference/kotlin/androidx/navigation/compose/rememberNavController.composable#rememberNavController(kotlin.Array)) or [`rememberLazyListState()`](https://developer.android.com/reference/kotlin/androidx/compose/foundation/lazy/rememberLazyListState.composable#rememberLazyListState(kotlin.Int,kotlin.Int)).
...
`LazyListState` encapsulates the state of the [`LazyColumn`](https://developer.android.com/reference/kotlin/androidx/compose/foundation/lazy/LazyColumn.composable) storing the `scrollPosition` for this UI element. It also exposes methods to modify the scroll position by for instance scrolling to a given item.
...
The same best practice applies if these events are navigation events, you can learn more about that in the [navigation docs](https://developer.android.com/guide/navigation/use-graph/navigate#events).
...
`inputMessage` is a variable storing the [`TextField`](https://developer.android.com/reference/kotlin/androidx/compose/material/TextField.composable) state. Every time the user types in new input, the app calls business logic to produce `suggestions`.
