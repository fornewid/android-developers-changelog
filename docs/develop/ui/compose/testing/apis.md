---
title: Testing APIs  |  Jetpack Compose  |  Android Developers
...
* **Assertions** are used to verify that the elements exist or have certain
  attributes.
* **Actions** inject simulated user events on the elements, such as clicks or
  other gestures.

Some of these APIs accept a [`SemanticsMatcher`](/reference/kotlin/androidx/compose/ui/test/SemanticsMatcher) to refer to one or more
*nodes* in the semantics tree.

**Note:** Testing a Compose UI is different from testing a view-based UI. The
view-based UI toolkit clearly defines what a view is. A view occupies a
