...
- `mutableStateOf(value)` creates a `MutableState`, which is an observable type in Compose. Any changes to its value schedule recomposition of any composable functions that read that value.
