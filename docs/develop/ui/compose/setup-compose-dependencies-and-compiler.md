...
**Note:** The Compose Compiler Gradle Plugin is only available from Kotlin 2.0+. For
migration instructions, see ["Jetpack Compose compiler moving to the Kotlin
...
1. In your `libs.versions.toml` file, remove any reference to the Compose
   Compiler.
...
1. In your projects root `build.gradle.kts` file, add the following snippet to the
   plugins section.
...
**Note:** Jetpack Compose is shipped using a Bill of Materials (BOM), to keep the
versions of all library groups in sync. Read more about it in the [Bill of
