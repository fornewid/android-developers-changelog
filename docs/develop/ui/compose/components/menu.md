---
title: Menus  |  Jetpack Compose  |  Android Developers
...
![A dropdown menu with two options displayed. An icon with three vertical dots indicates that clicking it opens the menu.](/static/develop/ui/compose/images/components/basicmenu1.png)
...
![A dropdown menu triggered by an icon with three vertical dots. The menu displays two selectable options, Option 1 and Option 2.](/static/develop/ui/compose/images/components/MinimalDropdownMenu.png)
...
![A dropdown menu with many options, requiring scrolling to view all
  items.](/static/develop/ui/compose/images/components/LongBasicDropdownMenu.png)
...
* The `leadingIcon` and `trailingIcon` parameters add icons to the start and
  end of a `DropdownMenuItem`.
* An `IconButton` triggers the menu's expansion.
* The `DropdownMenu` contains several `DropdownMenuItem` composables, each
  representing a selectable action.
* [`HorizontalDivider`](/reference/kotlin/androidx/compose/material3/package-summary#HorizontalDivider(androidx.compose.ui.Modifier,androidx.compose.ui.unit.Dp,androidx.compose.ui.graphics.Color)) composables insert a horizontal line to separate
...
![A dropdown menu with sections for Profile, Settings, Send Feedback, About, and Help. Each option has an icon, such as a person icon for Profile.](/static/develop/ui/compose/images/components/DropdownMenuWithDetails.png)
