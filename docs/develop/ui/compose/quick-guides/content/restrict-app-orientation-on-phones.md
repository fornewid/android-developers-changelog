---
title: Restrict app orientation on phones and foldable outer screens but not on larger displays  |  Jetpack Compose  |  Android Developers
...
**Note:** For apps targeting Android 16 (API level 36) and higher, orientation,
resizability, and aspect ratio restrictions no longer apply on displays with
smallest width >= 600dp. You can learn more at [Behavior changes: Apps targeting
Android 16 or higher](/about/versions/16/behavior-changes-16#ignore-orientation).
...
You can either avoid declaring the [`screenOrientation`](/guide/topics/manifest/activity-element#screen) element of the app
...
**Note:** `screenOrientation` is ignored (or is equivalent to `unspecified`) when
apps run in multi‑window mode. So, if you restrict your app to one
orientation, the app might not work properly in multi‑window mode.
...
See [`screenOrientation`](/guide/topics/manifest/activity-element#screen).
...
Add the [Jetpack WindowManager](/jetpack/androidx/releases/window) libraries to the module's `build.gradle` or
...
[`WindowMetricsCalculator#computeMaximumWindowMetrics()`](/reference/kotlin/androidx/window/layout/WindowMetricsCalculator#computeMaximumWindowMetrics(android.app.Activity)) method to obtain the
device screen size as a [`WindowMetrics`](/reference/kotlin/androidx/window/layout/WindowMetrics) object. The window metrics can be
compared to window size classes to decide when to restrict orientation.

[Windows size classes](/develop/ui/compose/layouts/adaptive/use-window-size-classes) provide the breakpoints between small and large
screens.

Use the [`WindowSizeClass#minWidthDp`](/reference/kotlin/androidx/window/core/layout/WindowSizeClass#minWidthDp()) and
[`WindowSizeClass#minHeightDp`](/reference/kotlin/androidx/window/core/layout/WindowSizeClass#minHeightDp()) breakpoints to determine the screen size:
...
**Note:*** The examples are implemented as methods of an activity; and so, the activity is dereferenced as `this` in the argument of `computeMaximumWindowMetrics()`.
* The `computeMaximumWindowMetrics()` method is used instead of [`computeCurrentWindowMetrics()`](/reference/kotlin/androidx/window/layout/WindowMetricsCalculator) because the app can be launched in multi‑window mode, which ignores the screen orientation setting. There's no point in determining the app window size and overriding the orientation setting unless the app window is the entire device screen.

See [WindowManager](/jetpack/androidx/releases/window#declaring_dependencies) for instructions about declaring dependencies to make the
...
[`Activity#setRequestedOrientation()`](/reference/kotlin/android/app/Activity#setrequestedorientation) to override the manifest's
...
activity recreation, refer to [Handle configuration changes](/guide/topics/resources/runtime-changes).
...
**Note:** If you did not specify `android:screenOrientation` in the manifest,
replace the occurrences of `ActivityInfo.SCREEN_ORIENTATION_FULL_USER` with
`ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED` to match the defaults.

## Key points

* [`screenOrientation`](/guide/topics/manifest/activity-element#screen): App manifest setting that enables you to specify
  how your app responds to device orientation changes
* [Jetpack WindowManager](/jetpack/androidx/releases/window): Set of libraries that enable you to determine the
  size and aspect ratio of the app window; backward compatible to API level 14
* [`Activity#setRequestedOrientation()`](/reference/kotlin/android/app/Activity#setrequestedorientation): Method with which you can change
  the app orientation at runtime
