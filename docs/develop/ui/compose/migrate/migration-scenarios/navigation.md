...
keep Fragments for [hosting your composable content](/develop/ui/compose/migrate/interoperability-apis/compose-in-views). The recommended end
goal for a Compose-first architecture is to replace Fragments entirely with
screen-level composables managed by Navigation Compose.**Note:** Currently, it's not possible to use any single form of the Navigation
