---
title: Migrate RecyclerView to Lazy list  |  Jetpack Compose  |  Android Developers
...
**Tip:** Provide additional parameters to `items()` to optimize your list: use
the `key` parameter to provide a unique key for the underlying data so that
...
concept to `RecyclerView`'s view types) so you can reuse item compositions more
efficiently.
...
* [Lists and Grids](/develop/ui/compose/lists#item-animations): Documentation for how to implement lists and grids in
  Compose.
