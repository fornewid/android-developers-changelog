---
title: Compare Compose and View metrics  |  Jetpack Compose  |  Android Developers
...
development](/develop/ui/compose/why-adopt). However, take into consideration how
...
**Note:** The results here are specific to the Sunflower sample app. Your metrics
may vary as APK size and build time depend on several factors, such as number of
modules, dependencies and versions you use, etc.
...
shrinking enabled](/studio/build/shrink-code#shrink-resources), using R8 full mode, and measured using [APK Analyzer](/studio/debug/apk-analyzer).
...
standalone [`gradle-profiler`](/build/profile-your-build#gradle-profiler) tool, which executes a build several times so
...
binding](/topic/libraries/data-binding), migrating dependencies that use [kapt to KSP](/build/migrate-to-ksp), and updating
...
[benefits of Compose](/develop/ui/compose/why-adopt), especially around developer productivity increases
...
You can read more case studies in [Adopt Compose for Teams](/develop/ui/compose/adopt/for-large-teams).
...
composables](/develop/ui/compose/lifecycle) and [Jetpack Compose
phases](/develop/ui/compose/phases) documentation.

### Baseline Profiles

[Baseline Profiles](/topic/performance/baselineprofiles/overview) are an
...
recommend that you [add a Baseline Profile](/topic/performance/baselineprofiles/create-baselineprofile) to your
...
Furthermore, Compose provides [smart recompositions](#smart-recomposition),
...
Jetpack Compose enforces a [single layout pass](/develop/ui/compose/layouts/custom)
...
[Jetpack Macrobenchmark](/studio/profile/macrobenchmark) library. To learn how
...
Since Jetpack Compose is an unbundled library, it doesn't benefit from [Zygote](/topic/performance/memory-overview#SharingRAM) that preloads the View system's
UI Toolkit classes and drawables. Jetpack Compose 1.0 utilizes profile
installation for *release* builds. [Profile installers](/jetpack/androidx/releases/profileinstaller) let apps specify critical code to
...
**Note:** Compose profiles aren't available on Android L or M, since the app is
compiled AOT by Android Framework on devices running these versions. The
