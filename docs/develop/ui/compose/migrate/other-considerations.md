---
title: Other considerations  |  Jetpack Compose  |  Android Developers
...
* Material Design 1 using the
  [AppCompat](/jetpack/androidx/releases/appcompat) library (i.e.
  `Theme.AppCompat.*`)
...
* Material Design 2 using the
  [Compose Material](/jetpack/androidx/releases/compose-material) library
  (i.e. `androidx.compose.material.MaterialTheme`)
* Material Design 3 using the
  [Compose Material 3](/jetpack/androidx/releases/compose-material3) library
  (i.e. `androidx.compose.material3.MaterialTheme`)
...
* [Material 2 to Material 3 in Compose](/develop/ui/compose/designsystems/material2-material3)
...
See [Design systems in Compose](/develop/ui/compose/designsystems) and [Migrating XML themes to Compose](/develop/ui/compose/designsystems/views-to-compose) to learn more.

## Navigation

If you use the [Navigation component](/guide/navigation/get-started) in your app, see
the [Navigate from Compose in a Fragment-based app](/guide/navigation/use-graph/navigate#compose-interop) and
...
[`createAndroidComposeRule`](/reference/kotlin/androidx/compose/ui/test/junit4/package-summary#createAndroidComposeRule())
...
See [Testing your Compose layout](/develop/ui/compose/testing) to learn more about testing. For
interoperability with UI testing frameworks, see
[interoperability with Espresso](/develop/ui/compose/testing#espresso-interop) and
[interoperability with UiAutomator](/develop/ui/compose/testing#uiautomator-interop).

## Integrating Compose with your existing app architecture

[Unidirectional Data Flow](/develop/ui/compose/architecture#udf) (UDF) architecture
...
function, as explained in [Compose and other libraries](/develop/ui/compose/libraries).

**Note:** Due to their [lifecycle and scoping](/topic/libraries/architecture/viewmodel#lifecycle),
*you should access and call `ViewModel` instances at screen-level
composables*—that is, close to a root composable called from an activity,
fragment, or destination of a Navigation graph. *You should never pass down
`ViewModel` instances to other composables*. Instead, pass only the data
...
**Note:** The same instance of a `ViewModel` type is used in all composables
unless the composable is a destination of the navigation graph or different
...
For more information, see [Side-effects in Compose](/develop/ui/compose/side-effects).
...
like a traditional view. See an example of this with [View Binding](/topic/libraries/view-binding)
below:
...
truth](/develop/ui/compose/migrate/other-considerations#state-source-of-truth).
...
achieve a unidirectional data flow, as explained further in [state hoisting](/develop/ui/compose/state#state-hoisting).
...
about in the [`CompositionLocal` documentation](/develop/ui/compose/compositionlocal#inversion-of-control).
...
classes](/develop/ui/compose/layouts/adaptive/use-window-size-classes) to learn more.

Additionally, refer to [Support different display sizes](/develop/ui/compose/layouts/adaptive/support-different-display-sizes)
...
read through
[Nested scrolling interop](/develop/ui/compose/touch-input/pointer-input/scroll#nested-scrolling-interop).
...
[`ComposeView.consumeWindowInsets`](/reference/kotlin/androidx/compose/ui/platform/ComposeView#(androidx.compose.ui.platform.ComposeView).consumeWindowInsets())
to `false`.

For more information, read the [`WindowInsets` in Compose](/develop/ui/compose/system/insets) documentation.
...
* [Window insets in Compose](/develop/ui/compose/system/insets)