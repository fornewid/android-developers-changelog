---
title: Broadcasts overview  |  Background work  |  Android Developers
...
**Note:** The system optimizes the delivery of broadcasts in order to maintain
optimal system health. Therefore, delivery times of broadcasts are not
guaranteed. Apps that need low-latency interprocess communication should
consider [bound services](/guide/components/bound-services).
...
intent, see [Intents and Intent Filters](/guide/components/intents-filters).
...
[`ACTION_AIRPLANE_MODE_CHANGED`](/reference/android/content/Intent#ACTION_AIRPLANE_MODE_CHANGED) is `android.intent.action.AIRPLANE_MODE`.
...
In [Android 16](/about/versions/16/behavior-changes-all#ordered-broadcast-priority), broadcast delivery order using the [`android:priority`](/guide/topics/manifest/intent-filter-element#priority)
attribute or [`IntentFilter.setPriority()`](/reference/android/content/IntentFilter#setPriority(int)) across different processes
...
([`SYSTEM_LOW_PRIORITY`](/reference/android/content/IntentFilter#SYSTEM_LOW_PRIORITY) + 1, [`SYSTEM_HIGH_PRIORITY`](/reference/android/content/IntentFilter#SYSTEM_HIGH_PRIORITY) - 1).
Only system components are allowed to set `SYSTEM_LOW_PRIORITY`,
`SYSTEM_HIGH_PRIORITY` as broadcast priority.

#### Android 14

While apps are in a [cached state](/guide/components/activities/process-lifecycle), the system optimizes broadcast delivery
for system health. For example, the system defers less important system
broadcasts such as [`ACTION_SCREEN_ON`](/reference/android/content/Intent#ACTION_SCREEN_ON) while the app is in a cached state.
Once the app goes from the cached state into an [active process lifecycle](/guide/components/activities/process-lifecycle),
the system delivers any deferred broadcasts.

Important broadcasts that are [declared in the manifest](#manifest-declared-receivers) temporarily remove
apps from the cached state for delivery.

#### Android 9

Beginning with Android 9 (API level 28), The [`NETWORK_STATE_CHANGED_ACTION`](/reference/android/net/wifi/WifiManager#NETWORK_STATE_CHANGED_ACTION)
...
[`getConnectionInfo()`](/reference/android/net/wifi/WifiManager#getConnectionInfo()) instead.
...
your app specifically). You can still use a [context-registered receiver](#context-registered-receivers)
...
[`CONNECTIVITY_ACTION`](/reference/android/net/ConnectivityManager#CONNECTIVITY_ACTION) broadcast using
[`registerReceiver(BroadcastReceiver, IntentFilter)`](/reference/androidx/core/content/ContextCompat#registerReceiver(android.content.Context,android.content.BroadcastReceiver,android.content.IntentFilter,int)). Declaring a receiver
...
an [`Activity`](/reference/android/app/Activity) context, you receive broadcasts as long as the activity
...
1. In your app's module-level build file, include version 1.9.0 or higher of
   the [AndroidX Core library](/jetpack/androidx/releases/core):
...
6. To stop receiving broadcasts, call
   [`unregisterReceiver(android.content.BroadcastReceiver)`](/reference/android/content/Context#unregisterReceiver(android.content.BroadcastReceiver)). Be sure to
...
* [`LifecycleResumeEffect`](/reference/kotlin/androidx/lifecycle/compose/LifecycleResumeEffect.composable#LifecycleResumeEffect(kotlin.Any,androidx.lifecycle.LifecycleOwner,kotlin.Function1)) or activity `onResume`/`onPause` lifecycle
  methods: The broadcast receiver only receives updates while the app is
  in its resumed state.
* [`LifecycleStartEffect`](/reference/kotlin/androidx/lifecycle/compose/LifecycleStartEffect.composable#LifecycleStartEffect(kotlin.Any,androidx.lifecycle.LifecycleOwner,kotlin.Function1)) or activity `onStart`/`onStop` lifecycle
  methods: The broadcast receiver only receives updates while the app is
  in its resumed state.
* [`DisposableEffect`](/reference/kotlin/androidx/compose/runtime/DisposableEffect.composable#DisposableEffect(kotlin.Any,kotlin.Function1)): The broadcast receiver only receives updates
  while the composable is in the composition tree. This scope is not attached
  to the activity lifecycle scope. Consider registering the receiver on the
  application context. This is because the composable could theoretically
  outlive the activity lifecycle scope and leak the activity.
* Activity `onCreate`/`onDestroy`: The broadcast receiver receives updates
  while the activity is in its created state. Make sure to unregister in
  `onDestroy()` and not `onSaveInstanceState(Bundle)` because this might not
  be called.
* A custom scope: For example, you can register a receiver in your `ViewModel`
  scope, so it survives activity recreation. Make sure to use the application
  context to register the receiver on, as the receiver can outlive the activity
  lifecycle scope and leak the activity.
...
broadcasts that are [exempted from that restriction](/guide/components/broadcast-exceptions). Implicit broadcasts
are broadcasts that don't target your app specifically. In most cases, you can
use [scheduled jobs](/topic/performance/scheduling) instead.
...
1. Specify the [`<receiver>`](/guide/topics/manifest/receiver-element) element in your app's manifest.
   ...
   to.
2. Subclass [`BroadcastReceiver`](/reference/android/content/BroadcastReceiver) and implement
   [`onReceive(Context, Intent)`](/reference/android/content/BroadcastReceiver#onReceive(android.content.Context,%20android.content.Intent)). The broadcast receiver in the following
...
The system creates a new [`BroadcastReceiver`](/reference/android/content/BroadcastReceiver) component object to handle
each broadcast that it receives. This object is valid only for the duration of
the call to [`onReceive(Context, Intent)`](/reference/android/content/BroadcastReceiver#onReceive(android.content.Context,%20android.content.Intent)). Once your code returns from this
method, the system considers the component no longer active.

### Effects on process state

Whether your [`BroadcastReceiver`](/reference/android/content/BroadcastReceiver) is operating or not affects its contained
process, which can alter its system-killing likelihood. A foreground process
executes a receiver's [`onReceive()`](/reference/android/content/BroadcastReceiver#onReceive(android.content.Context,%20android.content.Intent)) method. The system runs the process
...
[`JobService`](/reference/android/app/job/JobService) from the receiver using the [`JobScheduler`](/reference/android/app/job/JobScheduler) so the
system knows the process is still working. [Background Work Overview](/guide/background)
...
* The [`sendOrderedBroadcast(Intent, String)`](/reference/android/content/Context#sendOrderedBroadcast(android.content.Intent,%20java.lang.String)) method sends broadcasts to
  one receiver at a time. As
  each receiver executes in turn, it can propagate a result to the next
  receiver. It can also completely abort the broadcast so that it doesn't
  reach other receivers.
  You can control the order in which receivers run within the
  same app process. To do so, use the `android:priority` attribute of
  the matching intent-filter.
  Receivers with the same priority are run in an arbitrary order.
* The [`sendBroadcast(Intent)`](/reference/android/content/Context#sendBroadcast(android.content.Intent,%20java.lang.String)) method sends broadcasts to all receivers
  in an
  undefined order. This is called a Normal Broadcast. This is more efficient,
  but means that receivers cannot read results from other receivers, propagate
  data received from the broadcast, or abort the broadcast.

The following code snippet demonstrates how to send a broadcast by creating an
Intent and calling [`sendBroadcast(Intent)`](/reference/android/content/Context#sendBroadcast(android.content.Intent)).
...
The broadcast message is wrapped in an [`Intent`](/reference/android/content/Intent) object. The intent's
...
intent with [`putExtra(String, Bundle)`](/reference/androidx/core/content/ContextCompat#registerReceiver(android.content.Context,android.content.BroadcastReceiver,android.content.IntentFilter,int)). You can also limit a broadcast to
a set of apps in the same organization by calling [`setPackage(String)`](/reference/android/content/Intent#setPackage(java.lang.String)) on
the intent.

**Note:** Although you can use intents for both sending broadcasts and starting
activities with [`startActivity(Intent)`](/reference/android/content/Context#startActivity(android.content.Intent)), these actions are completely
unrelated. Broadcast receivers can't see or capture intents used to start an
activity; likewise, when you broadcast an intent, you can't find or start an
activity.
...
When you call [`sendBroadcast(Intent, String)`](/reference/android/content/Context#sendOrderedBroadcast(android.content.Intent,%20java.lang.String)) or
[`sendOrderedBroadcast(Intent, String, BroadcastReceiver, Handler, int, String,
Bundle)`](/reference/android/content/Context#sendOrderedBroadcast(android.content.Intent,%20java.lang.String,%20java.lang.String,%20android.content.BroadcastReceiver,%20android.os.Handler,%20int,%20java.lang.String,%20android.os.Bundle))
...
[`BLUETOOTH_CONNECT`](/reference/android/Manifest.permission#BLUETOOTH_CONNECT) or define a custom permission with the
[`<permission>`](/guide/topics/manifest/permission-element) element. For information on permissions and security in
general, see the [System Permissions](/guide/topics/security/permissions).

**Note:** Custom permissions are registered when the app is installed. The app that
defines the custom permission must be installed before the app that uses it.

### Receive broadcasts with permissions

If you specify a permission parameter when registering a broadcast receiver
(either with
[`registerReceiver(BroadcastReceiver, IntentFilter, String, Handler)`](/reference/android/content/Context#registerReceiver(android.content.BroadcastReceiver,%20android.content.IntentFilter,%20java.lang.String,%20android.os.Handler)) or in
[`<receiver>`](/guide/topics/manifest/receiver-element) tag in your manifest), then only broadcasters who have
requested the permission with the [`<uses-permission>`](/guide/topics/manifest/uses-permission-element) tag in their
...
* If many apps have registered to receive the same broadcast in their
  ...
  For example, the [`CONNECTIVITY_ACTION`](/reference/android/net/ConnectivityManager#CONNECTIVITY_ACTION) broadcast is delivered only to
  context-registered receivers.
* Don't broadcast sensitive information using an implicit intent. Any app can
  ...
  + In Android 4.0 (API level 14) and higher, you can specify a
    [package](/guide/topics/manifest/manifest-element#package) with [`setPackage(String)`](/reference/android/content/Intent#setPackage(java.lang.String)) when sending a broadcast.
    The system restricts the broadcast to the set of apps that match the
    package.
* When you register a receiver, any app can send potentially malicious
  ...
  + For manifest-declared receivers, you can set the [android:exported](/guide/topics/manifest/receiver-element#exported)
...
* Because a receiver's [`onReceive(Context, Intent)`](/reference/android/content/BroadcastReceiver#onReceive(android.content.Context,%20android.content.Intent)) method runs on the
  ...
  information, see [Effect on process state](#effects-process-state) To perform long running work,
  we recommend:

  + Calling [`goAsync()`](/reference/android/content/BroadcastReceiver#goAsync()) in your receiver's `onReceive()` method and
    passing the [`BroadcastReceiver.PendingResult`](/reference/android/content/BroadcastReceiver.PendingResult) to a background
    thread. This keeps the broadcast active after returning from
    `onReceive()`. However, even with this approach the system expects you
    to finish with the broadcast very quickly (under 10 seconds). It does
    allow you to move work to another thread to avoid glitching the main
    thread.
  + Scheduling a job with the [`JobScheduler`](/reference/android/app/job/JobScheduler). For more information,
    see [Intelligent Job Scheduling](/topic/performance/scheduling).
* Don't start activities from broadcast receivers because the user experience
  ...
  displaying a [notification](/develop/ui/views/notifications).