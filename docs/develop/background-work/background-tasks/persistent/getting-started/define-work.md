---
title: Define work requests  |  Background work  |  Android Developers
...
# Define work requests Stay organized with collections Save and categorize content based on your preferences.
...
Work is defined in WorkManager using a [`WorkRequest`](/reference/androidx/work/WorkRequest). In order to
...
[`WorkRequest`](/reference/androidx/work/WorkRequest) itself is an abstract base class. There are two
derived implementations of this class that you can use to create the request,
[`OneTimeWorkRequest`](/reference/androidx/work/OneTimeWorkRequest) and [`PeriodicWorkRequest`](/reference/androidx/work/PeriodicWorkRequest).
...
* **Power Management**: [Power management restrictions](/topic/performance/power/power-details), such as Battery
...
[standby bucket](/topic/performance/appstandby) and process importance.
...
[Power management resource limits](/topic/performance/power/power-details) for more details.
...
Here is how you use the [`PeriodicWorkRequest`](/reference/androidx/work/PeriodicWorkRequest) to create a
...
[JobScheduler API](/reference/android/app/job/JobScheduler)).

### Flexible run intervals

If the nature of your work makes it sensitive to run timing, you can configure
your [`PeriodicWorkRequest`](/reference/androidx/work/PeriodicWorkRequest) to run within a **flex
...
[`PeriodicWorkRequest.MIN_PERIODIC_INTERVAL_MILLIS`](/reference/androidx/work/PeriodicWorkRequest#MIN_PERIODIC_INTERVAL_MILLIS) and the flex
interval must be greater than or equal to
[PeriodicWorkRequest.MIN\_PERIODIC\_FLEX\_MILLIS`](/reference/androidx/work/PeriodicWorkRequest#MIN_PERIODIC_FLEX_MILLIS).

### Effect of Constraints on Periodic Work

You can apply [constraints](#work-constraints) to periodic work. For example, you could add a
...
[`Constraints`](/reference/androidx/work/Constraints) ensure that work is deferred until optimal conditions are
met. The following constraints are available to WorkManager:

|  |  |
| --- | --- |
| **NetworkType** | Constrains the [type of network](/reference/androidx/work/NetworkType) required for your work to run. For example, Wi-Fi (`UNMETERED`). |
...
[`Result.retry()`](/reference/androidx/work/ListenableWorker.Result#retry()) from your worker. Your work is then
rescheduled according to a [backoff delay](/reference/androidx/work/WorkRequest#DEFAULT_BACKOFF_DELAY_MILLIS) and [backoff policy](/reference/androidx/work/BackoffPolicy).
...
* *Backoff delay* specifies the minimum amount of time to wait before retrying
  ...
  (or [MIN\_BACKOFF\_MILLIS](/reference/androidx/work/WorkRequest#MIN_BACKOFF_MILLIS)).
* *Backoff policy* defines how the backoff delay should increase over time for
  ...
  [`LINEAR`](/reference/androidx/work/BackoffPolicy#LINEAR) and
  [`EXPONENTIAL`](/reference/androidx/work/BackoffPolicy#EXPONENTIAL).
...
Every work request has a [unique identifier](/reference/androidx/work/WorkRequest#getId()), which can be used to identify
that work later in order to [cancel](/topic/libraries/architecture/workmanager/how-to/managing-work#cancelling) the work or [observe its progress](/topic/libraries/architecture/workmanager/how-to/states-and-observation#observing).
...
For example, [`WorkManager.cancelAllWorkByTag(String)`](/reference/androidx/work/WorkManager#cancelAllWorkByTag(java.lang.String)) cancels
all work requests with a particular tag, and
[`WorkManager.getWorkInfosByTag(String)`](/reference/androidx/work/WorkManager#getWorkInfosByTagLiveData(java.lang.String)) returns a list of the
...
the `WorkRequest` you can use [`WorkInfo.getTags()`](/reference/androidx/work/WorkInfo#getTags()).

From your `Worker` class, you can retrieve its set of tags using
[ListenableWorker.getTags()](/reference/androidx/work/ListenableWorker#getTags()).
...
Input values are stored as key-value pairs in a [`Data`](/reference/androidx/work/Data) object
...
the input arguments by calling [`Worker.getInputData()`](/reference/androidx/work/ListenableWorker#getInputData()). The
...
In the [States and observation](/topic/libraries/architecture/workmanager/how-to/states-and-observation) page, you'll learn more about work states
