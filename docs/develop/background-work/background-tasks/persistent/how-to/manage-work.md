---
title: Managing work  |  Background work  |  Android Developers
...
# Managing work Stay organized with collections Save and categorize content based on your preferences.
...
run once.
To achieve this goal, you can schedule the work as [unique work](#unique-work).
...
auto-generated by WorkManager. Unlike [tags](/topic/libraries/architecture/workmanager/how-to/define-work#tag), unique names are only
...
* [`WorkManager.enqueueUniqueWork()`](/reference/androidx/work/WorkManager#enqueueUniqueWork(kotlin.String,androidx.work.ExistingWorkPolicy,androidx.work.OneTimeWorkRequest)) for one time work
* [`WorkManager.enqueueUniquePeriodicWork()`](/reference/androidx/work/WorkManager#enqueueUniquePeriodicWork(kotlin.String,%20androidx.work.ExistingPeriodicWorkPolicy,%20androidx.work.PeriodicWorkRequest)) for periodic work
...
of a [`WorkInfo`](/reference/androidx/work/WorkInfo) object, which includes the
[`id`](/reference/androidx/work/WorkInfo#getId()) of the work, its tags, its
current [`State`](/reference/androidx/work/WorkInfo.State), and any output dataset using
[`Result.success(outputData)`](/reference/androidx/work/ListenableWorker.Result#success(androidx.work.Data)).

The [`LiveData`](/topic/libraries/architecture/livedata) and [`Flow`](/kotlin/flow)
...
[`WorkQuery`](/reference/androidx/work/WorkQuery) objects. WorkQuery supports
...
[`getWorkInfosLiveData()`](/reference/androidx/work/WorkManager#getWorkInfosLiveData(androidx.work.WorkQuery)),
and the Flow equivalent, [`getWorkInfosFlow()`](/reference/androidx/work/WorkManager#getWorkInfosFlow(androidx.work.WorkQuery)).
...
[`State`](/reference/androidx/work/WorkInfo.State) of the work. If the work is
already [finished](/reference/androidx/work/WorkInfo.State#isFinished()),
nothing happens. Otherwise, the work's state is changed to
[`CANCELLED`](/reference/androidx/work/WorkInfo.State#CANCELLED) and the work
will not run in the future. Any
[`WorkRequest`](/reference/androidx/work/WorkRequest) jobs that are [dependent
on this work](/topic/libraries/architecture/workmanager/how-to/chain-work) will
also be `CANCELLED`.

[`RUNNING`](/reference/androidx/work/WorkInfo.State#RUNNING) work
receives a call to
[`ListenableWorker.onStopped()`](/reference/androidx/work/ListenableWorker#onStopped()).
Override this method to handle any potential cleanup. See [stop a
running worker](#stop-worker) for more information.

**Note:**
[`cancelAllWorkByTag(String)`](/reference/androidx/work/WorkManager#cancelAllWorkByTag(java.lang.String))
...
* In the case of [unique work](#unique-work),
  you explicitly enqueued a new `WorkRequest` with an
  [`ExistingWorkPolicy`](/reference/androidx/work/ExistingWorkPolicy) of
  [`REPLACE`](/reference/androidx/work/ExistingWorkPolicy#REPLACE).
  The old `WorkRequest` is immediately considered cancelled.
* Your work's constraints are no longer met.
* The system instructed your app to stop your work for some reason. This can
  happen if you exceed the execution deadline of 10 minutes. The work is
  scheduled for retry at a later time.
...
You can call the
[`ListenableWorker.isStopped()`](/reference/androidx/work/ListenableWorker#isStopped()) method to check if your worker has already
...
[`Result`](/reference/androidx/work/ListenableWorker.Result) set by a Worker
