---
title: Schedule alarms  |  Background work  |  Android Developers
...
* You can use them in conjunction with broadcast receivers to schedule
  [jobs](/reference/android/app/job/JobScheduler) or
  [WorkRequests](/topic/libraries/architecture/workmanager) to perform other
  operations.
...
**Note:** For timing operations that are guaranteed to occur **during** the lifetime
of your application,
consider using the [`Handler`](/reference/android/os/Handler)
class in conjunction with [`Timer`](/reference/java/util/Timer) and
[`Thread`](/reference/java/lang/Thread)
...
[Doze](/training/monitoring-device-state/doze-standby).

**Note:** Most apps can schedule tasks and events using inexact alarms. If your
app's core functionality depends on a precisely-timed alarm—such as for an alarm
clock app or a calendar app—then it's OK to use an [exact alarm](#exact)
instead.
...
[`setInexactRepeating()`](/reference/android/app/AlarmManager#setInexactRepeating(int,%20long,%20long,%20android.app.PendingIntent)),
...
in effect such as [battery saver](/about/versions/pie/power#battery-saver) or
[Doze](/training/monitoring-device-state/doze-standby#understand_doze).

### Deliver an alarm during a time window

If your app calls [`setWindow()`](/reference/android/app/AlarmManager#setWindow(int,%20long,%20long,%20java.lang.String,%20android.app.AlarmManager.OnAlarmListener,%20android.os.Handler)), the alarm never goes off before the supplied
...
If your app calls [`setInexactRepeating()`](/reference/android/app/AlarmManager#setInexactRepeating(int,%20long,%20long,%20android.app.PendingIntent)),
the system invokes multiple alarms:

1. The first alarm goes off within the specified time window, starting from the
   given trigger time.
2. Subsequent alarms usually go off after the specified time window
   elapses. The time between two consecutive invocations of the alarm can vary.

## Set an exact alarm

The system invokes an *exact alarm* at a precise moment in the future.

Most apps can schedule tasks and events using [inexact alarms](#inexact) to
complete several [common use cases](#use-cases). If your app's core
...
**Note:** If your app targets Android 12 or higher, you must
[declare one of the "Alarms & reminders"](#exact-permission-declare)
permissions. Otherwise, a
...
:   The [`Handler`](/reference/android/os/Handler) class includes several good
    methods to handle timing operations, such as doing some work every
    *n* seconds, while your app is alive:
    [`postAtTime()`](/reference/android/os/Handler#postAtTime(java.lang.Runnable,%20long))
    and
    [`postDelayed()`](/reference/android/os/Handler#postDelayed(java.lang.Runnable,%20long)).
    Note that these APIs rely on [system uptime](/reference/android/os/SystemClock#uptimeMillis())
    and not [real time](/reference/android/os/SystemClock#elapsedRealtime()).
...
:   `WorkManager` provides a way to [schedule timing-sensitive periodic
    work](/topic/libraries/architecture/workmanager/how-to/define-work#flexible_run_intervals).
...
:   Use an inexact alarm. Specifically, call
    [`setAndAllowWhileIdle()`](/reference/android/app/AlarmManager#setAndAllowWhileIdle(int,%20long,%20android.app.PendingIntent)).

User-specified action that should happen after a specific time
:   Use an inexact alarm. Specifically, call
    [`set()`](/reference/android/app/AlarmManager#set(int,%20long,%20android.app.PendingIntent)).

User-specified action that can happen within a specified time window
:   Use an inexact alarm. Specifically, call
    [`setWindow()`](/reference/android/app/AlarmManager#setWindow(int,%20long,%20long,%20android.app.PendingIntent)).
...
It's highly recommended that you create an [inexact alarm](#inexact) whenever
possible. To perform longer work, schedule it using
[`WorkManager`](/topic/libraries/architecture/workmanager) or
[`JobScheduler`](/reference/android/app/job/JobScheduler) from your alarm's
`BroadcastReceiver`. To perform work while
the device is in Doze, create an inexact alarm using
[`setAndAllowWhileIdle()`](/reference/android/app/AlarmManager#setAndAllowWhileIdle(int,%20long,%20android.app.PendingIntent)),
and start a job from the alarm.

**Note:** Android considers exact alarms to be critical, time-sensitive
interruptions. For this reason, exact alarms aren't affected by
[foreground service launch
restrictions](/develop/background-work/services/fgs/restrictions-bg-start).
...
declare either the [`SCHEDULE_EXACT_ALARM`](/reference/android/Manifest.permission#SCHEDULE_EXACT_ALARM)
or the [`USE_EXACT_ALARM`](/reference/android/Manifest.permission#USE_EXACT_ALARM)
permission.
...
* [Limited use cases](#exact-acceptable-use-cases)
...
[`OnAlarmListener`](/reference/android/app/AlarmManager.OnAlarmListener)
object, such as with the
[`setExact`](/reference/android/app/AlarmManager#setExact(int,%20long,%20java.lang.String,%20android.app.AlarmManager.OnAlarmListener,%20android.os.Handler))
...
[`canScheduleExactAlarms()`](/reference/android/app/AlarmManager#canScheduleExactAlarms())
...
system sends it the
[`ACTION_SCHEDULE_EXACT_ALARM_PERMISSION_STATE_CHANGED`](/reference/android/app/AlarmManager#ACTION_SCHEDULE_EXACT_ALARM_PERMISSION_STATE_CHANGED)
broadcast. Your app should [implement a broadcast
receiver](/guide/components/broadcasts#receiving-broadcasts) that does the
following:

1. Confirms that your app still has the special app access. To do so, call
   [`canScheduleExactAlarms()`](/reference/android/app/AlarmManager#canScheduleExactAlarms()).
   This check protects your app from the case where the user grants your app the
   permission, then revokes it almost immediately afterward.
2. Reschedules any exact alarms that your app needs, based on its current state.
   This logic should be similar to what your app does when it receives the
   [`ACTION_BOOT_COMPLETED`](/reference/android/content/Intent#ACTION_BOOT_COMPLETED)
   broadcast.

#### Ask users to grant the `SCHEDULE_EXACT_ALARM` permission

![The option is called 'Allow setting alarms and reminders'](/static/images/training/scheduling/alarms-and-reminders.svg)


**Figure 1.** "Alarms & reminders" special app access
page in system settings, where users can allow your app to set exact
alarms.
...
1. In your app's UI, explain to the user why your app needs to schedule exact
   alarms.
2. Invoke an intent that includes the
   [`ACTION_REQUEST_SCHEDULE_EXACT_ALARM`](/reference/android/provider/Settings#ACTION_REQUEST_SCHEDULE_EXACT_ALARM)
   intent action.
...
repeating alarms are [inexact alarms](#inexact).

A repeating alarm has the following characteristics:

* An alarm type. For more discussion, see [Choose an alarm type](#type).
...
[`FLAG_NO_CREATE`](/reference/android/app/PendingIntent.getService()#FLAG_NO_CREATE)
to [`PendingIntent.getService()`](/reference/android/app/PendingIntent#getService(android.content.Context,%2520int,%2520android.content.Intent,%2520int))
to get an instance of the intent (if it exists), then pass that intent to
[`AlarmManager.cancel()`](/reference/android/app/AlarmManager#cancel(android.app.PendingIntent))
...
**Note:** If the `PendingIntent` was created with
[`FLAG_ONE_SHOT`](/reference/android/app/PendingIntent#FLAG_ONE_SHOT)
...
* [`ELAPSED_REALTIME`](/reference/android/app/AlarmManager#ELAPSED_REALTIME):
...
* [`RTC`](/reference/android/app/AlarmManager#RTC):
  Fires the pending intent at the specified time but does not wake up the device.
* [`RTC_WAKEUP`](/reference/android/app/AlarmManager#RTC_WAKEUP): Wakes
...
time requirements, you can set an [exact alarm](#exact) by calling
[`setRepeating()`](/reference/android/app/AlarmManager#setRepeating(int,%20long,%20long,%20android.app.PendingIntent)).

With [`setInexactRepeating()`](/reference/android/app/AlarmManager#setInexactRepeating(int,%20long,%20long,%20android.app.PendingIntent)),
you can't specify a custom interval the way you can with
[`setRepeating()`](/reference/android/app/AlarmManager#setRepeating(int,%20long,%20long,%20android.app.PendingIntent)).
You have to use one of the interval constants, such as
[`INTERVAL_FIFTEEN_MINUTES`](/reference/android/app/AlarmManager#INTERVAL_FIFTEEN_MINUTES),
[`INTERVAL_DAY`](/reference/android/app/AlarmManager#INTERVAL_DAY),
and so on. See [`AlarmManager`](/reference/android/app/AlarmManager)
for the complete list.

## Cancel an alarm

Depending on your app, you may want to include the ability to cancel the alarm.
To cancel an alarm, call [`cancel()`](/develop/background-work/services/reference/android/app/AlarmManager#cancel(android.app.AlarmManager.OnAlarmListener))
on the Alarm Manager, passing in the
[`PendingIntent`](/reference/android/app/PendingIntent) you no longer want
to fire. For example:
...
ensures that the [`AlarmManager`](/reference/android/app/AlarmManager) will
continue doing its task without the user needing to manually restart the alarm.

Here are the steps:

1. Set the [`RECEIVE_BOOT_COMPLETED`](/reference/android/Manifest.permission#RECEIVE_BOOT_COMPLETED)
...
[Doze mode](/training/monitoring-device-state/doze-standby)
...
* Use the WorkManager API, which is built to perform
  ...
  [Schedule tasks with WorkManager](/arch/work)
...
* Don't wake up the device unnecessarily (this behavior is determined by the
  alarm type, as described in [Choose an alarm type](#type)).
* Don't make your alarm's trigger time any more precise than it has to be.
  ...
  instead of
  [`setRepeating()`](/reference/android/app/AlarmManager#setRepeating(int,%20long,%20long,%20android.app.PendingIntent)).
  When you use [`setInexactRepeating()`](/reference/android/app/AlarmManager#setInexactRepeating(int,%20long,%20long,%20android.app.PendingIntent)),
  ...
  (API Level 19), all repeating alarms are [inexact alarm](#inexact). Note
  that while
  [`setInexactRepeating()`](/reference/android/app/AlarmManager#setInexactRepeating(int,%20long,%20long,%20android.app.PendingIntent))
...
* Avoid basing your alarm on clock time if possible.
  ...
  [`ELAPSED_REALTIME`](/reference/android/app/AlarmManager#ELAPSED_REALTIME) if
