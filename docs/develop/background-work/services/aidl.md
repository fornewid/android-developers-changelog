...
- `https://developer.android.com/reference/java/util/List`
  ...
  All elements in the `List` must be one of the supported data types in this list or one of the other AIDL-generated interfaces or parcelables you declare. A `List` can optionally be used as a parameterized type class, such as `List<String>`. The actual concrete class that the other side receives is always an `https://developer.android.com/reference/java/util/ArrayList`, although the method is generated to use the `List` interface.
- `https://developer.android.com/reference/java/util/Map`

  All elements in the `Map` must be one of the supported data types in this list or one of the other AIDL-generated interfaces or parcelables you declare. Parameterized type maps, such as those of the form `Map<String,Integer>`, aren't supported. The actual concrete class that the other side receives is always a `https://developer.android.com/reference/java/util/HashMap`, although the method is generated to use the `Map` interface. Consider using a `https://developer.android.com/reference/android/os/Bundle` as an alternative to `Map`.
...
To implement the interface generated from the `.aidl`, extend the generated `https://developer.android.com/reference/android/os/Binder` interface, such as `YourInterface.Stub`, and implement the methods inherited from the `.aidl` file.
...
Now the `binder` is an instance of the `Stub` class (a `https://developer.android.com/reference/android/os/Binder`), which defines the IPC interface for the service. In the next step, this instance is exposed to clients so they can interact with the service.
...
When the client receives the `https://developer.android.com/reference/android/os/IBinder` in the `onServiceConnected()` callback, it must call `YourServiceInterface.Stub.asInterface(service)` to cast the returned parameter to `YourServiceInterface` type:
...
And here is an example of how the `https://developer.android.com/reference/android/graphics/Rect` class implements the `Parcelable` protocol.
...
The marshalling in the `Rect` class is straightforward. Take a look at the other methods on `https://developer.android.com/reference/android/os/Parcel` to see the other kinds of values you can write to a `Parcel`.

**Warning:** Remember the security implications of receiving data from other processes. In this case, the `Rect` reads four numbers from the `https://developer.android.com/reference/android/os/Parcel`, but it is up to you to ensure that these are within the acceptable range of values for whatever the caller is trying to do. For more information about how to keep your application secure from malware, see [Security tips](https://developer.android.com/guide/topics/security/security).
