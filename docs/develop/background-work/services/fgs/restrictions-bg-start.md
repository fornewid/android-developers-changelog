---
title: Restrictions on starting a foreground service from the background  |  Background work  |  Android Developers
...
# Restrictions on starting a foreground service from the background Stay organized with collections Save and categorize content based on your preferences.
...
foreground service that another app owns, these restrictions apply only if
**both** apps target Android 12 or higher.
...
* Your app transitions from a user-visible state, such as an
  [activity](/guide/components/activities/intro-activities).
* Your app can [start an activity from the
  background](/guide/components/activities/background-starts), except for the case
...
* The user performs an action on a UI element related to your app. For example,
  they might interact with a [bubble](/guide/topics/ui/bubbles),
  [notification](/develop/ui/views/notifications),
  [widget](/guide/topics/appwidgets/overview), or activity.
* Your app invokes an [exact alarm](/training/scheduling/alarms#exact) to
  complete an action that the user requests.
* Your app is the device's current [input
  method](/guide/topics/text/creating-input-method).
* Your app receives an event that's related to
  [geofencing](/training/location/geofencing) or [activity
  recognition transition](/guide/topics/location/transitions).
* After the device reboots and receives the
  [`ACTION_BOOT_COMPLETED`](/reference/android/content/Intent#ACTION_BOOT_COMPLETED),
  [`ACTION_LOCKED_BOOT_COMPLETED`](/reference/android/content/Intent#ACTION_LOCKED_BOOT_COMPLETED),
  or [`ACTION_MY_PACKAGE_REPLACED`](/reference/android/content/Intent#ACTION_MY_PACKAGE_REPLACED)
  intent action in a [broadcast receiver](/guide/components/broadcasts).

  **Note:** If your app targets Android 14 or higher, there are
  restrictions on launching certain foreground service types from a
  `BOOT_COMPLETED` receiver. For more information, see [Restrictions on
...
* Your app receives the
  [`ACTION_TIMEZONE_CHANGED`](/reference/android/content/Intent#ACTION_TIMEZONE_CHANGED),
  [`ACTION_TIME_CHANGED`](/reference/android/content/Intent#ACTION_TIME_CHANGED),
  or
  [`ACTION_LOCALE_CHANGED`](/reference/android/content/Intent#ACTION_LOCALE_CHANGED)
  intent action in a broadcast receiver.
* Your app receives the
  [`ACTION_TRANSACTION_DETECTED`](/reference/android/nfc/NfcAdapter#ACTION_TRANSACTION_DETECTED)
...
* Your app uses the [Companion Device Manager](/guide/topics/connectivity/companion-device-pairing) and declares the
  ...
  permission or the [`REQUEST_COMPANION_RUN_IN_BACKGROUND`](/reference/android/Manifest.permission#REQUEST_COMPANION_RUN_IN_BACKGROUND)
...
[microphone](/develop/background-work/services/fgs/service-types#microphone), the operating
...
**Note:** On versions of Android lower than Android 14, if you
tried to create a
foreground service that needed while-in-use permissions while your app was in
the background, the system would let you create the service, but the service
wouldn't have access to the needed resources, and if it tried to use them, you'd
get an exception. On Android 14 or higher, your app gets the
exception as soon as it tries to create the foreground service.
...
[defined exemptions](#wiu-restrictions-exemptions).
...
[runs
in the background](/guide/background#definition), it can still access location,
...
type](/develop/background-work/services/fgs/declare) of `location` and is started by an app that
has the [`ACCESS_BACKGROUND_LOCATION`](/reference/android/Manifest.permission#ACCESS_BACKGROUND_LOCATION)
...
* The service starts by interacting with [app
  widgets](/guide/topics/appwidgets/overview).
* The service starts by interacting with a notification.
* The service starts as a
  [`PendingIntent`](/reference/android/app/PendingIntent) that is sent from a
  different, visible app.
* The service starts by an app that is a [device policy
  controller](/work/dpc/build-dpc) that runs in device owner mode.
* The service starts by an app which provides the `VoiceInteractionService`.
* The service starts by an app that has the
