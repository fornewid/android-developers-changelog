---
title: Services overview  |  Background work  |  Android Developers
...
a [separate thread](/training/multiple-threads) within the service to avoid Application
...
:   A foreground service performs some operation that is noticeable to the
    ...
    audio track. Foreground services must display a [Notification](/develop/ui/views/notifications).
    ...
    [foreground services](/develop/background-work/services/fgs) in your
    app.

    **Note:** The [WorkManager](/topic/libraries/architecture/workmanager) API offers a flexible way of scheduling tasks, and is
    able to [run
...
:   A background service performs an operation that isn't directly noticed by
    ...
    **Note:** If your app targets API level 26 or higher, the system imposes [restrictions on running background
    ...
    background](/training/location/background). Instead,
...
you might create a thread in `onCreate()`,
...
or [Kotlin coroutines](/kotlin/coroutines) instead of the traditional
`Thread` class. See the
[Threading on Android](/training/multiple-threads) document for more information about
...
focus, it's less likely to be killed; if the service is declared to [run in the foreground](/develop/background-work/services/fgs), it's rarely killed.
...
about when the system might destroy a service, see the [Processes and Threading](/guide/components/processes-and-threads)
document.
...
To declare your service, add a [`<service>`](/guide/topics/manifest/service-element) element
as a child of the [`<application>`](/guide/topics/manifest/application-element)
...
See the [`<service>`](/guide/topics/manifest/service-element) element
reference for more information about declaring your service in the manifest.

There are other attributes that you can include in the [`<service>`](/guide/topics/manifest/service-element) element to
define properties such as the permissions that are required to start the service and the process in
which the service should run. The [`android:name`](/guide/topics/manifest/service-element#nm)
...
including the [`android:exported`](/guide/topics/manifest/service-element#exported)
...
to add the
[`android:description`](/guide/topics/manifest/service-element#desc)
attribute to the
...
subclass of `Service` that uses a
...
You can use [JobIntentService](/reference/androidx/core/app/JobIntentService) as a
...
strongly consider using WorkManager instead for most use cases. Consult the [guide to background processing on Android](/guide/background)
to see if there is a solution that fits your needs.

### Extending the Service class

You can extend the `Service` class
...
The example code handles all incoming calls in `onStartCommand()`
...
For more information about the lifecycle of a service, see the section below about [Managing the Lifecycle of a Service](#Lifecycle).
...
separate document about [Bound Services](/guide/components/bound-services).
...
![](/static/images/service_lifecycle.png)
...
For more information about creating a service that provides binding, see the [Bound Services](/guide/components/bound-services) document,
which includes more information about the `onRebind()`
callback method in the section about [Managing the lifecycle of
a bound service](/guide/components/bound-services#Lifecycle).