---
title: Minimize the effect of regular updates  |  Connectivity  |  Android Developers
...
# Minimize the effect of regular updates Stay organized with collections Save and categorize content based on your preferences.
...
* **User-initiated.** Performing an update based on some user behavior, such as
  a pull-to-refresh gesture.
* **App-initiated.** Performing an update on a recurring basis.
* **Server-initiated.** Performing an update in response to a notification from
  a server.
...
[Avoid redundant
downloads](/training/efficient-downloads/redundant_redundant#LocalCache).
...
see [Access shared datasets](/preview/features/shared-datasets).
...
machine](/develop/connectivity/network-ops/network-access-optimization#radio-state).
...
You can use the
[`ConnectivityManager`](/reference/android/net/ConnectivityManager) to register
a listener for the default network, and the
[`TelephonyManager`](/reference/android/telephony/TelephonyManager) to register
a [`PhoneStateListener`](/reference/android/telephony/PhoneStateListener) to
...
[`Worker`](/reference/androidx/work/Worker) subclass called
...
[Guide to background processing](/guide/background).
