---
title: Bluetooth profiles  |  Connectivity  |  Android Developers
...
# Bluetooth profiles Stay organized with collections Save and categorize content based on your preferences.
...
* **Headset**. The Headset profile provides support for Bluetooth headsets to be
  ...
  [`BluetoothHeadset`](/reference/android/bluetooth/BluetoothHeadset) class,
  ...
  [Vendor-specific AT commands](#at-commands).
* **A2DP**. The Advanced Audio Distribution Profile (A2DP) profile defines how
  ...
  [`BluetoothA2dp`](/reference/android/bluetooth/BluetoothA2dp) class, which is
...
* **Health Device**. Android provides support for the Bluetooth Health Device
  ...
  Device Data
  Specializations](https://www.bluetooth.com/specifications/assigned-numbers/health-device-profile).
  ...
  information about HDP, see [Health Device Profile](#health-profile).

Here are the basic steps for working with a profile:

1. Get the default adapter, as described in
   [Bluetooth setup](/develop/connectivity/bluetooth/setup).
2. Set up a
   [`BluetoothProfile.ServiceListener`](/reference/android/bluetooth/BluetoothProfile.ServiceListener).
   This listener notifies
   [`BluetoothProfile`](/reference/android/bluetooth/BluetoothProfile) clients
   when they have been connected to or disconnected from the service.
3. Use
   [`getProfileProxy()`](/reference/android/bluetooth/BluetoothAdapter#getProfileProxy(android.content.Context,%20android.bluetooth.BluetoothProfile.ServiceListener,%20int))
   ...
   `BluetoothHeadset`.
4. In
   [`onServiceConnected()`](/reference/android/bluetooth/BluetoothProfile.ServiceListener#onServiceConnected(int,%20android.bluetooth.BluetoothProfile)),
   get a handle to the profile proxy object.
5. Once you have the profile proxy object, use it to monitor the state of the
...
**Caution:** The Health Device Profile (HDP) and MCAP protocols are no longer used.
New apps should use Bluetooth Low Energy based solutions such as
[`BluetoothGatt`](/reference/android/bluetooth/BluetoothGatt),
[`BluetoothAdapter.listenUsingL2capChannel()`](/reference/android/bluetooth/BluetoothAdapter#listenUsingL2capChannel()),
or
[`BluetoothDevice#createL2capChannel(int)`](/reference/android/bluetooth/BluetoothDevice#createL2capChannel(int)).
...
3. Establish a connection to a health device.
   ...
   **Note:** Some devices initiate the connection automatically. It is unnecessary
   to carry out this step for those devices.
