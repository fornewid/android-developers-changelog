---
title: Optimize network access  |  Connectivity  |  Android Developers
...
# Optimize network access Stay organized with collections Save and categorize content based on your preferences.
...
* **Full power**: Used when a connection is active, allowing the device to
  transfer data at its highest possible rate.
* **Low power**: An intermediate state that cuts battery power consumption by
  around 50%.
* **Standby**: The minimal power-consuming state during which no network
...
![](/static/images/develop/connectivity/mobile_radio_state_machine.png)   
...
![](/static/images/develop/connectivity/unbundled.png)   
...
![](/static/images/develop/connectivity/bundled.png)   
...
anticipating and [prefetching data](#prefetch-data). Other scenarios, such as
...
app-initiated
tasks](/develop/connectivity/minimize-effect-regular-updates#app-initiated) for
...
[WorkManager API](/topic/libraries/architecture/workmanager/how-to/define-work#constraints)
...
a connection using
[`ConnectivityManager`](/reference/android/net/ConnectivityManager), as shown in
[Monitor connectivity status and connection
metering](/training/monitoring-device-state/connectivity-status-type#DetermineConnection).
...
[`HttpURLConnection`](/reference/java/net/HttpURLConnection) and most HTTP
