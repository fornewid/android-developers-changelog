---
title: Read network state  |  Connectivity  |  Android Developers
...
# Read network state Stay organized with collections Save and categorize content based on your preferences.
...
* The [`Network`](/reference/android/net/Network) class represents one of the
  networks that the device is connected to. You can use the `Network`
  object as a key to gather information about the network with
  `ConnectivityManager` or to bind sockets on the network. When the network
  disconnects, the `Network` object stops being usable. Even if the device later
  reconnects to the same appliance, a new `Network` object represents the
  new network.
* The [`LinkProperties`](/reference/android/net/LinkProperties) object
  contains information about the link for a network, such as the list of DNS
  servers, local IP addresses, and network routes installed for the network.
* The [`NetworkCapabilities`](/reference/android/net/NetworkCapabilities)
  object contains information about properties of a network, such as the
  transports (Wi-Fi, mobile, Bluetooth) and what the network is capable of.
  For example, you can query the object to determine whether the network is
  capable of sending MMS, is behind a captive portal, or is metered.
...
**Note:** If your app needs to schedule tasks for when the device next has
connectivity or is on an unmetered network, such as to do background downloads,
see [WorkManager](/topic/libraries/architecture/workmanager)
instead. Although `ConnectivityManager` tells you about these states,
WorkManager lets your app run its jobs at more appropriate times with
finer control—and with a lot less work.
...
[Listen to network events](#listening-events).
...
[`NetworkCapabilities.hasTransport(int)`](/reference/android/net/NetworkCapabilities#hasTransport(int))
...
[`NetworkCapabilities.hasCapability(int)`](/reference/android/net/NetworkCapabilities#hasCapability(int))
method with one of the `NetworkCapabilities.NET_CAPABILITY_*` constants.

The most-useful `NET_CAPABILITY_*` constants include:

* [`NET_CAPABILITY_INTERNET`](/reference/android/net/NetworkCapabilities#NET_CAPABILITY_INTERNET):
...
* [`NET_CAPABILITY_NOT_METERED`](/reference/android/net/NetworkCapabilities#NET_CAPABILITY_NOT_METERED):
  ...
  issues.
* [`NET_CAPABILITY_NOT_VPN`](/reference/android/net/NetworkCapabilities#NET_CAPABILITY_NOT_VPN):
  indicates that the network isn't a virtual private network.
* [`NET_CAPABILITY_VALIDATED`](/reference/android/net/NetworkCapabilities#NET_CAPABILITY_VALIDATED):
...
* [`NET_CAPABILITY_CAPTIVE_PORTAL`](/reference/android/net/NetworkCapabilities#NET_CAPABILITY_CAPTIVE_PORTAL):
...
[`NetworkCallback`](/reference/android/net/ConnectivityManager.NetworkCallback)
class together with
[`ConnectivityManager.registerDefaultNetworkCallback(NetworkCallback)`](/reference/android/net/ConnectivityManager#registerDefaultNetworkCallback(android.net.ConnectivityManager.NetworkCallback))
and
[`ConnectivityManager.registerNetworkCallback(NetworkCallback)`](/reference/android/net/ConnectivityManager#registerNetworkCallback(android.net.NetworkRequest,%20android.net.ConnectivityManager.NetworkCallback)). These two methods serve different
purposes.
...
[`HttpsURLConnection`](/reference/javax/net/ssl/HttpsURLConnection), the
...
[`onAvailable(Network)`](/reference/android/net/ConnectivityManager.NetworkCallback#onAvailable(android.net.Network))
for the new network. Implement
[`onCapabilitiesChanged(Network,NetworkCapabilities)`](/reference/android/net/ConnectivityManager.NetworkCallback#onCapabilitiesChanged(android.net.Network,%20android.net.NetworkCapabilities)),
[`onLinkPropertiesChanged(Network,LinkProperties)`](/reference/android/net/ConnectivityManager.NetworkCallback#onLinkPropertiesChanged(android.net.Network,%20android.net.LinkProperties)),
or both to react appropriately to changes in connectivity.

**Caution:** Don't call the synchronous methods to find out about the properties of
a newly available network in the callback, as this suffers from race
conditions. Instead, wait for calls to `onCapabilitiesChanged()` and
`onLinkPropertiesChanged()` for that network, which are called
immediately after `onAvailable()` on devices running Android 8.0 (API level 26)
and higher.
...
Instead use
[`NetworkCapabilities.getLinkDownstreamBandwidthKbps()`](/reference/android/net/NetworkCapabilities#getLinkDownstreamBandwidthKbps())
to measure bandwidth, and
[`NetworkCapabilites.hasCapability(int)`](/reference/android/net/NetworkCapabilities#hasCapability(int))
with
[`NET_CAPABILITY_NOT_METERED`](/reference/android/net/NetworkCapabilities#NET_CAPABILITY_NOT_METERED)
arguments to determine meteredness. For more information, see the section about
[NetworkCapabilities and LinkProperties](#introducing-net-capabilities).
...
[`ConnectivityManager.registerDefaultNetworkCallback(NetworkCallback, Handler)`](/reference/android/net/ConnectivityManager#registerDefaultNetworkCallback(android.net.ConnectivityManager.NetworkCallback,%20android.os.Handler)).

Unregister your callback when you have no use for it anymore by calling
[`ConnectivityManager.unregisterNetworkCallback(NetworkCallback)`](/reference/android/net/ConnectivityManager#unregisterNetworkCallback(android.net.ConnectivityManager.NetworkCallback)).
Your main activity's [`onPause()`](/reference/android/app/Activity#onPause())
is a good place to do this, especially if you register the callback in
[`onResume()`](/reference/android/app/Activity#onResume()).

**Note:** There is a limit to the number of callbacks that can be registered
concurrently. Unregister callbacks once they are no longer needed so that
your app can register more.

## Additional networks (advanced use cases)

**Caution:** Most apps don't need to use additional networks, and instead
should use the default network. Only apps that must use other networks
should register additional network callbacks. For example, an app that must
use Wi-Fi, and only Wi-Fi, for a
particular operation is an app that must use additional networks.
...
build a [`NetworkRequest`](/reference/android/net/NetworkRequest) matching their
needs and call
[`ConnectivityManager.registerNetworkCallback(NetworkRequest, NetworkCallback)`](/reference/android/net/ConnectivityManager#registerNetworkCallback(android.net.NetworkRequest,%20android.net.ConnectivityManager.NetworkCallback)).
...
[`onLost(Network)`](/reference/android/net/ConnectivityManager.NetworkCallback#onLost(android.net.Network))
...
[`registerNetworkCallback(NetworkRequest, NetworkCallback, Handler)`](/reference/android/net/ConnectivityManager#registerNetworkCallback(android.net.NetworkRequest,%20android.net.ConnectivityManager.NetworkCallback,%20android.os.Handler))
...
For convenience, the
[`NetworkRequest`](/reference/android/net/NetworkRequest) object contains the
common capabilities most apps need, including the following:

* [`NET_CAPABILITY_NOT_RESTRICTED`](/reference/android/net/NetworkCapabilities#NET_CAPABILITY_NOT_RESTRICTED)
* [`NET_CAPABILITY_TRUSTED`](/reference/android/net/NetworkCapabilities#NET_CAPABILITY_TRUSTED)
* [`NET_CAPABILITY_NOT_VPN`](/reference/android/net/NetworkCapabilities#NET_CAPABILITY_NOT_VPN)
...
[`NET_CAPABILITY_MMS`](/reference/android/net/NetworkCapabilities#NET_CAPABILITY_MMS)
to the `NetworkRequest` to avoid being told about all the networks that can't
send MMS messages. Add
[`TRANSPORT_WIFI_AWARE`](/reference/android/net/NetworkCapabilities#TRANSPORT_WIFI_AWARE)
...
1. When the app calls `registerNetworkCallback()`, the callback immediately
   ...
   ![State diagram showing the register network callback event and the callbacks triggered by the event](/static/images/develop/network-ops/register-network-callbacks-01.png)   
...
2. Then, the app calls `registerDefaultNetworkCallback()`. The default network
   ...
   callbacks triggered by the event](/static/images/develop/network-ops/register-network-callbacks-02.png)   
...
3. Later, the device connects to an (unmetered) Wi-Fi network. The regular
   ...
   new network](/static/images/develop/network-ops/register-network-callbacks-03.png)   
...
4. At this point, it is possible the Wi-Fi network takes a while to validate. In
   ...
   connection validates](/static/images/develop/network-ops/register-network-callbacks-04.png)   
...
5. Later still, the device suddenly disconnects from Wi-Fi, because it went out
   ...
   ![State diagram showing the callbacks triggered when a Wi-Fi network
   connection is lost](/static/images/develop/network-ops/register-network-callbacks-05.png)   
...
[`ConnectivityManager.requestNetwork(NetworkRequest, NetworkCallback)`](/reference/android/net/ConnectivityManager#requestNetwork(android.net.NetworkRequest,%20android.net.ConnectivityManager.NetworkCallback))
