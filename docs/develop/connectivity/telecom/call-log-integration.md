---
title: Unified call history  |  Connectivity  |  Android Developers
...
# Unified call history Stay organized with collections Save and categorize content based on your preferences.
...
required changes to [VoIP calling apps](#changes-calling-app) and
[system dialer apps](#changes-dialer-app).

**Note:** The features described on this page require [Jetpack Telecom library
version `1.1.0-alpha01`](/jetpack/androidx/releases/core#core-telecom_version_11_2) or higher.
...
dialer. To exclude calls on a per-call basis, set the [`isLogExcluded`](/reference/androidx/core/telecom/CallAttributesCompat#isLogExcluded())
boolean to `true` within [`CallAttributesCompat`](/reference/androidx/core/telecom/CallAttributesCompat).
...
Calls added through [`CallsManager.addCall`](/reference/androidx/core/telecom/CallsManager#addCall(androidx.core.telecom.CallAttributesCompat,kotlin.coroutines.SuspendFunction1,kotlin.coroutines.SuspendFunction1,kotlin.coroutines.SuspendFunction0,kotlin.coroutines.SuspendFunction0,kotlin.Function1)) get a unique `UUID` through
[`CallControlScope.getCallId`](/reference/androidx/core/telecom/CallControlScope#getCallId()).

**Important:** The calling VoIP app must map the call details to this unique `UUID`
and store them. When a user selects a VoIP call log entry in the dialer, the
system dialer initiates a callback with the intent action
[`TelecomManager.ACTION_CALL_BACK`](/reference/android/telecom/TelecomManager#ACTION_CALL_BACK) and an intent extra
[`TelecomManager.EXTRA_UUID`](/reference/android/telecom/TelecomManager#EXTRA_UUID). The VoIP app uses the stored `UUID` to
retrieve the correct call details and handle the return call.
...
log, use [`CallLog.Calls.CONTENT_VOIP_URI`](/reference/android/provider/CallLog.Calls#CONTENT_VOIP_URI).
...
* On Android 17 (API level 37) and higher, use the following formalized
  ...
  + [`CallLog.Calls.CONTENT_URI_WITH_VOIP_CALLS`](/reference/android/provider/CallLog.Calls#CONTENT_URI_WITH_VOIP_CALLS)
  + [`CallLog.Calls.INCLUDE_VOIP_CALLS_PARAM_KEY`](/reference/android/provider/CallLog.Calls#INCLUDE_VOIP_CALLS_PARAM_KEY)

### Initiate callbacks from dialer app

To initiate a callback from a dialer, use [`TelecomManager.placeCall`](/reference/android/telecom/TelecomManager#placeCall(android.net.Uri,%20android.os.Bundle)). The
platform uses the unique `CallLog.Calls._ID` of the call log entry to launch
the correct VoIP app. This launch includes a
[`TelecomManager.ACTION_CALL_BACK`](/reference/android/telecom/TelecomManager#ACTION_CALL_BACK) intent, which is a system-defined action
for initiating a return call. This intent contains the call's `UUID` in
intent extra [`TelecomManager.EXTRA_UUID`](/reference/android/telecom/TelecomManager#EXTRA_UUID), allowing the VoIP app to identify
