...
following table describes the USB host APIs in the `https://developer.android.com/reference/android/hardware/usb/package-summary` package.

**Table 1.** USB Host APIs

| Class | Description |
|---|---|
| `https://developer.android.com/reference/android/hardware/usb/UsbManager` | Allows you to enumerate and communicate with connected USB devices. |
| `https://developer.android.com/reference/android/hardware/usb/UsbDevice` | Represents a connected USB device and contains methods to access its identifying information, interfaces, and endpoints. |
| `https://developer.android.com/reference/android/hardware/usb/UsbInterface` | Represents an interface of a USB device, which defines a set of functionality for the device. A device can have one or more interfaces on which to communicate. |
| `https://developer.android.com/reference/android/hardware/usb/UsbEndpoint` | Represents an interface endpoint, which is a communication channel for this interface. An interface can have one or more endpoints, and usually has input and output endpoints for two-way communication with the device. |
| `https://developer.android.com/reference/android/hardware/usb/UsbDeviceConnection` | Represents a connection to the device, which transfers data on endpoints. This class allows you to send data back and forth synchronously or asynchronously. |
| `https://developer.android.com/reference/android/hardware/usb/UsbRequest` | Represents an asynchronous request to communicate with a device through a `https://developer.android.com/reference/android/hardware/usb/UsbDeviceConnection`. |
| `https://developer.android.com/reference/android/hardware/usb/UsbConstants` | Defines USB constants that correspond to definitions in linux/usb/ch9.h of the Linux kernel. |

In most situations, you need to use all of these classes (`https://developer.android.com/reference/android/hardware/usb/UsbRequest` is only required if you are doing asynchronous communication)
when communicating with a USB device. In general, you obtain a `https://developer.android.com/reference/android/hardware/usb/UsbManager` to retrieve the desired `https://developer.android.com/reference/android/hardware/usb/UsbDevice`.
When you have the device, you need to find the appropriate `https://developer.android.com/reference/android/hardware/usb/UsbInterface` and the `https://developer.android.com/reference/android/hardware/usb/UsbEndpoint` of that
interface to communicate on. Once you obtain the correct endpoint, open a `https://developer.android.com/reference/android/hardware/usb/UsbDeviceConnection` to communicate with the USB device.
...
In your activity, you can obtain the `https://developer.android.com/reference/android/hardware/usb/UsbDevice` that represents
...
while your application is running, it can enumerate devices on the bus. Use the `https://developer.android.com/reference/android/hardware/usb/UsbManager#getDeviceList()` method to get a hash map of all
...
To display the dialog that asks users for permission to connect to the device, call the `https://developer.android.com/reference/android/hardware/usb/UsbManager#requestPermission(android.hardware.usb.UsbAccessory, android.app.PendingIntent)` method:
...
`https://developer.android.com/reference/android/hardware/usb/UsbManager#EXTRA_PERMISSION_GRANTED` extra, which is a boolean
...
`https://developer.android.com/reference/android/hardware/usb/UsbInterface` and `https://developer.android.com/reference/android/hardware/usb/UsbEndpoint` of the
device that you want to communicate on and send requests on this endpoint with a `https://developer.android.com/reference/android/hardware/usb/UsbDeviceConnection`. In general, your code should:

- Check a `https://developer.android.com/reference/android/hardware/usb/UsbDevice` object's attributes, such as product ID, vendor ID, or device class to figure out whether or not you want to communicate with the device.
- When you are certain that you want to communicate with the device, find the appropriate `https://developer.android.com/reference/android/hardware/usb/UsbInterface` that you want to use to communicate along with the appropriate `https://developer.android.com/reference/android/hardware/usb/UsbEndpoint` of that interface. Interfaces can have one or more endpoints, and commonly will have an input and output endpoint for two-way communication.
- When you find the correct endpoint, open a `https://developer.android.com/reference/android/hardware/usb/UsbDeviceConnection` on that endpoint.
- Supply the data that you want to transmit on the endpoint with the `https://developer.android.com/reference/android/hardware/usb/UsbDeviceConnection#bulkTransfer(android.hardware.usb.UsbEndpoint, byte[], int, int)` or `https://developer.android.com/reference/android/hardware/usb/UsbDeviceConnection#controlTransfer(int, int, int, int, byte[], int, int)` method. You should carry out this step in another thread to prevent blocking the main UI thread. For more information about using threads in Android, see [Processes and
...
To send data asynchronously, use the `https://developer.android.com/reference/android/hardware/usb/UsbRequest` class to `https://developer.android.com/reference/android/hardware/usb/UsbRequest#initialize(android.hardware.usb.UsbDeviceConnection, android.hardware.usb.UsbEndpoint)` and `https://developer.android.com/reference/android/hardware/usb/UsbRequest#queue(java.nio.ByteBuffer)` an asynchronous request, then wait for the result
with `https://developer.android.com/reference/android/hardware/usb/UsbDeviceConnection#requestWait()`.

### Terminating communication with a device

When you are done communicating with a device or if the device was detached, close the `https://developer.android.com/reference/android/hardware/usb/UsbInterface` and `https://developer.android.com/reference/android/hardware/usb/UsbDeviceConnection` by
calling `https://developer.android.com/reference/android/hardware/usb/UsbDeviceConnection#releaseInterface(android.hardware.usb.UsbInterface)` and
`https://developer.android.com/reference/android/hardware/usb/UsbDeviceConnection#close()`. To listen for detached events,
