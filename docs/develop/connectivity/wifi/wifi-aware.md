---
title: Wi-Fi Aware overview  |  Connectivity  |  Android Developers
...
# Wi-Fi Aware overview Stay organized with collections Save and categorize content based on your preferences.
...
distances than [Bluetooth](/develop/connectivity/bluetooth)
...
[`PublishConfig.Builder.setInstantCommunicationModeEnabled()`](/reference/android/net/wifi/aware/PublishConfig.Builder#setInstantCommunicationModeEnabled(boolean,%20int)) and
[`SubscribeConfig.Builder.setInstantCommunicationModeEnabled()`](/reference/android/net/wifi/aware/SubscribeConfig.Builder#setInstantCommunicationModeEnabled(boolean,%20int)) methods to
...
mode, use the [`isInstantCommunicationModeSupported()`](/reference/android/net/wifi/aware/Characteristics#isInstantCommunicationModeSupported()) method.

**Note:** Because instant communication mode uses additional power, this mode only
remains enabled for 30 seconds from the time a publisher or subscriber discovery
session is started.
...
* On devices running Android 12 (API level 31) or higher, you can use the
  [`onServiceLost()`](/reference/android/net/wifi/aware/DiscoverySessionCallback#onServiceLost(android.net.wifi.aware.PeerHandle,%20int))
...
session to [publish a service](#publish_a_service) or
[subscribe to a service](#subscribe_to_a_service).
...
[send a message](#send_a_message) back to the subscriber or
[create a connection](#create_a_connection) to it.
...
[`onServiceDiscovered()`](/reference/android/net/wifi/aware/DiscoverySessionCallback#onServiceDiscovered(android.net.wifi.aware.PeerHandle,%20byte%5B%5D,%20java.util.List%3Cbyte%5B%5D%3E))
callback method. You can use the `PeerHandle`
argument from this callback to [send a message](#send_a_message) or
[create a connection](#create_a_connection) to that publisher.
...
* A `DiscoverySession`. This object allows you
  ...
  [publishing a service](#publish_a_service) or [subscribing to a
  service](#subscribe_to_a_service).
...
[create a connection](#create_a_connection) instead.
...
the
[`setMatchFilter()`](/reference/android/net/wifi/aware/PublishConfig.Builder#setMatchFilter(java.util.List%3Cbyte%5B%5D%3E))
or
[`setServiceSpecificInfo()`](/reference/android/net/wifi/aware/PublishConfig.Builder#setServiceSpecificInfo(byte%5B%5D))
method of [`PublishConfig`](/reference/android/net/wifi/aware/PublishConfig) or
[`SubscribeConfig`](/reference/android/net/wifi/aware/SubscribeConfig). The
...
1. Use Wi-Fi Aware discovery to [publish a service](#publish_a_service) (on the
   server) and [subscribe to a service](#subscribe_to_a_service) (on the
   client).
2. Once the subscriber discovers the publisher,
   [send a message](#send_a_message) from the subscriber to the publisher.
3. Start a [`ServerSocket`](/reference/java/net/ServerSocket) on the publisher
   ...
   ```
4. Use the [`ConnectivityManager`](/reference/android/net/ConnectivityManager) to
   request a Wi-Fi Aware network on the publisher using a
   [`WifiAwareNetworkSpecifier`](/reference/android/net/wifi/aware/WifiAwareNetworkSpecifier),
   specifying the discovery session and the
   [`PeerHandle`](/reference/android/net/wifi/aware/PeerHandle) of the subscriber,
...
5. Once the publisher requests a network it should
   [send a message](#send_a_message) to the subscriber.
6. Once the subscriber receives the message from publisher, request a Wi-Fi
   ...
   [`NetworkSpecifier`](/reference/android/net/NetworkSpecifier). The
...
7. Once the `onAvailable()` method is called on the subscriber, a
   [`Network`](/reference/android/net/Network) object is available with
   which you can open a [`Socket`](/reference/java/net/Socket) to communicate
   ...
   [`NetworkCapabilities`](/reference/android/net/NetworkCapabilities) object
...
MAC address or its [PeerHandle](/reference/android/net/wifi/aware/PeerHandle).
...
* The publisher must enable ranging on the published service using
  [setRangingEnabled(true)](/reference/android/net/wifi/aware/PublishConfig.Builder#setrangingenabled).
...
* The subscriber must specify a geofence using some combination of
  [setMinDistanceMm](/reference/android/net/wifi/aware/SubscribeConfig.Builder#setmindistancemm)
  and
  [setMaxDistanceMm](/reference/android/net/wifi/aware/SubscribeConfig.Builder#setmaxdistancemm).
...
[onServiceDiscoveredWithinRange](/reference/android/net/wifi/aware/DiscoverySessionCallback#onServiceDiscoveredWithinRange(android.net.wifi.aware.PeerHandle,%20byte%5B%5D,%20java.util.List%3Cbyte%5B%5D%3E,%20int))
