...
- [Suggestion API](https://developer.android.com/develop/connectivity/wifi/wifi-suggest): targets apps that provision and provide internet-capable configurations. These configurations are not individually owned by the user. Users can disable specific configurations or disable the suggesting app, but can't delete them.
...
- [Network request API](https://developer.android.com/develop/connectivity/wifi/wifi-bootstrap): targets apps that need to connect to a peer device, such as when configuring an IoT device or transferring files to a camera. In such cases, the peer device starts up a SoftAP and the API allows the app to guide the user to connect to the device. The resulting network is not intended to provide internet access, can't be used by the system, and can't be used by any app except the configuring app.
...
- [`ACTION_WIFI_ADD_NETWORKS` API](https://developer.android.com/develop/connectivity/wifi/wifi-save-network-passpoint-config): allows apps (with user approval) to add networks or Passpoint configurations to the saved network or subscription list. These configurations are then treated as if the user added them directly. For example, the user can later delete them.
...
On Android 11 QPR1 and higher, the system mandates strict security configurations for TLS-based Wi-Fi Enterprise connections. For more information, see [Secure Wi-Fi Enterprise configuration](https://developer.android.com/develop/connectivity/wifi-enterprise).