---
title: VPN  |  Connectivity  |  Android Developers
...
# VPN Stay organized with collections Save and categorize content based on your preferences.
...
[always-on](#always-on) and [per-app](#per-app) VPN) and doesn’t cover the
built-in VPN client.
...
* Controls to manually start and stop a connection. [Always-on VPN](#always-on)
...
profile](/work/managed-profiles)) to a VPN gateway. Each user (or work profile) can run a
...
[`VpnService`](/reference/android/net/VpnService).
...
[`VpnService.Builder`](/reference/android/net/VpnService.Builder) methods to establish a new local interface.
...
[`VpnService`](/reference/android/net/VpnService). Declare the VPN [service](/guide/topics/manifest/service-element) in your app
manifest file with the following additions:

* Protect the service with the [`BIND_VPN_SERVICE`](/reference/android/Manifest.permission#BIND_VPN_SERVICE)
...
your service when running [always-on VPN](#always-on).

### Prepare a service

To prepare the app to become the user’s current VPN service, call
[`VpnService.prepare()`](/reference/android/net/VpnService#prepare(android.content.Context)). If the person using the device hasn’t
...
[`VpnService.prepare()`](/reference/android/net/VpnService#prepare(android.content.Context)) because a person might have set a different
app as the VPN service since your app last called the method. To learn more, see
the [Service lifecycle](#lifecycle) section.
...
1. Call [`VpnService.prepare()`](/reference/android/net/VpnService#prepare(android.content.Context)) to ask for permission (when
   needed).
2. Call [`VpnService.protect()`](/reference/android/net/VpnService#protect(int)) to keep your app's tunnel socket
   outside of the system VPN and avoid a circular connection.
3. Call [`DatagramSocket.connect()`](/reference/java/net/DatagramSocket#connect(java.net.SocketAddress)) to connect your app's tunnel
   socket to the VPN gateway.
4. Call [`VpnService.Builder`](/reference/android/net/VpnService.Builder) methods to configure a new local
   [TUN](https://en.wikipedia.org/wiki/TUN/TAP) interface on the
   device for VPN traffic.
5. Call [`VpnService.Builder.establish()`](/reference/android/net/VpnService.Builder#establish()) so that the system
...
handshaking. Your app calls [`VpnService.Builder`](/reference/android/net/VpnService.Builder) methods to configure a
...
The example in the [Per-app VPN](#per-app) section shows an IPv6 config including
more options. You need to add the following [`VpnService.Builder`](/reference/android/net/VpnService.Builder) values
before you can establish a new interface:

[`addAddress()`](/reference/android/net/VpnService.Builder#addAddress(java.net.InetAddress,%20int))
...
[`addRoute()`](/reference/android/net/VpnService.Builder#addRoute(java.net.InetAddress,%20int))
...
The [`establish()`](/reference/android/net/VpnService.Builder#establish()) method returns a
[`ParcelFileDescriptor`](/reference/android/os/ParcelFileDescriptor) instance that your app uses to read and write
packets to and from the interface’s buffer. The [`establish()`](/reference/android/net/VpnService.Builder#establish())
...
* The system starts the service because [always-on VPN](#always-on) is on.

Your app starts the VPN service by passing an intent to
[`startService()`](/reference/android/content/Context#startService(android.content.Intent)). To learn more, read [Starting a
service](/guide/components/services#StartingAService).

The system starts your service in the background by calling
[`onStartCommand()`](/reference/android/app/Service#onStartCommand(android.content.Intent,%20int,%20int)). However, Android places restrictions on
...
[`Service.startForeground()`](/reference/android/app/Service#startForeground(int,%20android.app.Notification)). To learn more, read [Running a
service in the foreground](/guide/components/services#Foreground).
...
The system calls your service’s [`onRevoke()`](/reference/android/net/VpnService#onRevoke()) method but this call
...
* Close the protected tunnel socket to the VPN gateway by calling
  [`DatagramSocket.close()`](/reference/java/net/DatagramSocket#close()).
* Close the parcel file descriptor (you don’t need to drain it) by calling
  [`ParcelFileDescriptor.close()`](/reference/android/os/ParcelFileDescriptor#close()).
...
You can also use [managed configurations](/work/managed-configurations) to configure a
...
1. Create an [`Intent`](/reference/android/content/Intent) instance to start the VPN service.
2. Flag the VPN service by [putting an extra](/reference/android/content/Intent#putExtra(java.lang.String,%20java.lang.String)) into the intent.
3. In the service’s [`onStartCommand()`](/reference/android/app/Service#onStartCommand(android.content.Intent,%20int,%20int)) method, look for the
...
list](#allowed-apps) or in a [disallowed list](#disallowed-apps) lose their
network connection. Consider warning people when making allowed or disallowed
lists. To learn more, see the following [Per-app VPN](#per-app) section.
...
[`SERVICE_META_DATA_SUPPORTS_ALWAYS_ON`](/reference/android/net/VpnService#SERVICE_META_DATA_SUPPORTS_ALWAYS_ON)
...
[`VpnService.Builder.addAllowedApplication()`](/reference/android/net/VpnService.Builder#addAllowedApplication(java.lang.String)). If
...
[`VpnService.Builder.addDisallowedApplication()`](/reference/android/net/VpnService.Builder#addDisallowedApplication(java.lang.String)).
...
bypass the VPN, call [`VpnService.Builder.allowBypass()`](/reference/android/net/VpnService.Builder#allowBypass()) when
...
[`ConnectivityManager.bindProcessToNetwork()`](/reference/android/net/ConnectivityManager#bindProcessToNetwork(android.net.Network)) or
[`Network.bindSocket()`](/reference/android/net/Network#bindSocket(java.net.Socket)) before connecting the socket.
