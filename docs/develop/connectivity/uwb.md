---
title: Ultra-wideband (UWB) communication  |  Connectivity  |  Android Developers
...
# Ultra-wideband (UWB) communication Stay organized with collections Save and categorize content based on your preferences.
...
([`UwbComplexChannel`](/reference/androidx/core/uwb/UwbComplexChannel)) that
...
[Bluetooth Low Energy](/develop/connectivity/bluetooth/ble-overview) (BLE).

The [ranging parameters](/reference/kotlin/androidx/core/uwb/RangingParameters)
...
supports it. To check your device capabilities, see [`RangingCapabilities`](/reference/kotlin/androidx/core/uwb/RangingCapabilities).
...
1. Use the `uwbConfigType` in [`RangingParameters`](/reference/kotlin/androidx/core/uwb/RangingParameters) that supports provisioned STS.
2. Provide the 16-byte key in the [`sessionKeyInfo`](/reference/kotlin/androidx/core/uwb/RangingParameters#sessionKeyInfo()) field.

For static STS:

1. Use the `uwbConfigType` in [`RangingParameters`](/reference/kotlin/androidx/core/uwb/RangingParameters) that supports static STS.
2. Provide the 8-byte key in the [`sessionKeyInfo`](/reference/kotlin/androidx/core/uwb/RangingParameters#sessionKeyInfo()) field.

## Steps

To use the UWB API, follow these steps:

1. Ensure Android devices are running on Android 12 or higher and that they
   support UWB using
   `PackageManager#hasSystemFeature("android.hardware.uwb")`.
2. If ranging against IoT devices, ensure that they are FiRa MAC 1.3
   compliant.
3. Discover UWB-capable peer devices using an OOB mechanism of your choice,
   such as
   [`BluetoothLeScanner`](/reference/android/bluetooth/le/BluetoothLeScanner).
4. Exchange ranging parameters using a secure OOB mechanism of your choice,
   such as [`BluetoothGatt`](/reference/android/bluetooth/BluetoothGatt).
...
1. The app initiating new UWB ranging sessions must be a foreground
   app or service unless background ranging is supported as illustrated
   earlier.
2. When the app moves to background (while the session is ongoing), the app
   may no longer receive ranging reports. The UWB session will however
   continue to be maintained in the lower layers. When the app moves back to the
   foreground, the ranging reports will resume.

## Code samples

### Sample app

For an end-to-end example on how to use the [UWB Jetpack library](/jetpack/androidx/releases/core-uwb), check our [sample application on Github](https://github.com/android/connectivity-samples/tree/main/UwbRanging). This sample app covers validating UWB compatibility on an Android device, enabling the discovery process using an OOB mechanism, and setting up UWB ranging between two UWB-capable devices. The sample also covers device control and media sharing use cases.
...
[current Jetpack library](/jetpack/androidx/releases/core-uwb#1.0.0-alpha04).
