---
title: Detect when users start or end an activity  |  Sensors and location  |  Android Developers
...
# Detect when users start or end an activity Stay organized with collections Save and categorize content based on your preferences.
...
1. To declare a dependency to the API, add a reference to the Google maven
   repository and add an implementation entry to
   `com.google.android.gms:play-services-location:12.0.0` to the dependencies
   section of your app `build.gradle` file. For more information, see [Set Up
...
* An
  ...
  object that specifies the type of activity and transition.
* A `PendingIntent` callback where your app receives
  notifications. For more information, see [Using a pending
  intent](/guide/components/intents-filters#PendingIntent).
...
[`ActivityTransitionRequest`](https://developers.google.com/android/reference/com/google/android/gms/location/ActivityTransitionRequest)
and your `PendingIntent` object to the
...
When the requested activity transition occurs, you app receives an `Intent` callback. An
...
You can implement your callback by creating a subclass of `BroadcastReceiver` and implementing the `onReceive()` method to get the list
of activity transition events. For more information, see
[Broadcasts](/guide/components/broadcasts). The following example shows how
...
and passing your `PendingIntent` object as a parameter, as
...
Create and monitor geofences](/develop/sensors-and-location/location/geofencing)

[Next

Test location workflows

arrow\_forward](/develop/sensors-and-location/location/testing)