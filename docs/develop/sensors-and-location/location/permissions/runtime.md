---
title: Request location access at runtime  |  Sensors and location  |  Android Developers
...
guide that explains how to [request app permissions](/training/permissions/requesting).
...
![After the user selects the Share Location button, the
    system's location permission dialog appears](/static/images/training/location/feature-requires-foreground.svg)


**Figure 1.** Location-sharing feature that requires
foreground location access. The feature is enabled if the user selects
...
permission and the [`ACCESS_COARSE_LOCATION`](/reference/android/Manifest.permission#ACCESS_COARSE_LOCATION) permission in a single runtime
...
in [Logcat](/studio/command-line/logcat):
...
access to only approximate location information. [Figure 2](#fig-approximate-only) shows the
...
* **Approximate**: Allows your app to get only approximate location
  information.

[Figure 3](#fig-approximate-full-prompt) shows that the dialog contains a visual cue for both options, to
...
you follow best practices for [requesting runtime permissions](/training/permissions/requesting).

![The dialog refers only to approximate location and
         contains 3 buttons, one above the other](/static/images/training/location/approximate-only.svg)


**Figure 2.** System permissions dialog that appears when
your app requests `ACCESS_COARSE_LOCATION` only.

![The dialog has 2 sets of options, one above the other](/static/images/training/location/approximate-full-prompt.svg)


**Figure 3.** System permissions dialog that appears when
your app requests both `ACCESS_FINE_LOCATION` and
`ACCESS_COARSE_LOCATION` in a single runtime request.
...
[manage the permission request code yourself](/training/permissions/requesting#manage-request-code-yourself).
...
devices over Bluetooth or Wi-Fi, consider using [companion device pairing](/guide/topics/connectivity/companion-device-pairing)
or [Bluetooth permissions](/guide/topics/connectivity/bluetooth/permissions), instead of requesting the `ACCESS_FINE_LOCATION`
permission.
...
2. Request the `ACCESS_FINE_LOCATION` and `ACCESS_COARSE_LOCATION` permissions
   together again. Because the user has already allowed the system to grant
   approximate location to your app, the system dialog is different this time,
   as shown in figure 4 and figure 5:

![The dialog contains the options 'Change to precise
         location', 'Only this time', and 'Deny'.](/static/images/training/location/upgrade-to-precise-while-using-the-app.svg)


**Figure 4.** The user previously
selected **Approximate** and **While using the app** (in the dialog
from figure 3).

![The dialog contains the options 'Only this time' and
         'Deny'.](/static/images/training/location/upgrade-to-precise-only-this-time.svg)


**Figure 5.** The user previously
selected **Approximate** and **Only this time** (in the dialog from
figure 3).
...
![The button that enables foreground location access is
    ...
    location](/static/images/training/location/incremental-permission-request.svg)


**Figure 6.** Both features require location access, but only
the "recommend nearby features" feature requires background location
access.
...
2. At a later time, when the user explores features that require
   ...
   location](/develop/sensors-and-location/location/permissions/background) access.
