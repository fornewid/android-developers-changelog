---
title: App orientation restricted on phones but not on large screens  |  Adaptive Apps  |  Android Developers
...
# App orientation restricted on phones but not on large screens Stay organized with collections Save and categorize content based on your preferences.





![Two star rating icon](/static/develop/adaptive-apps/cookbook/images/shared/two-star-rating.png)
...
The [Adaptive app quality guidelines](/docs/quality-guidelines/adaptive-app-quality) recommend that apps support all device
...
* [`LocalConfiguration`](/reference/kotlin/androidx/compose/ui/platform/package-summary#LocalConfiguration()): Composable local provider that exposes the current
  device `Configuration`, allowing you to read screen dimensions in DPs.
* [`LaunchedEffect`](/reference/kotlin/androidx/compose/runtime/package-summary#LaunchedEffect(kotlin.Any,kotlin.Function2)): Compose side-effect API that runs a block of code
  when a key changes (in this case, when the configuration changes).
...
apps run in multi-window mode. So, if you restrict your app to one orientation
or another, the app might not work properly in multi-window mode.
...
[`android:screenOrientation`](/guide/topics/manifest/activity-element#screen).
...
**Note:** If you did not specify `android:screenOrientation` in the app manifest,
you should replace `ActivityInfo.SCREEN_ORIENTATION_FULL_USER` with
`ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED` in your code to match the default
system behavior.
...
* [Support different screen sizes](/develop/ui/compose/layouts/adaptive/support-different-screen-sizes)
