---
title: What's new for enterprise in Android 15  |  Android Enterprise  |  Android Developers
...
# What's new for enterprise in Android 15 Stay organized with collections Save and categorize content based on your preferences.
...
* [Private space](/about/versions/15/features#private-space) is a new feature in Android 15 that lets users create a
  ...
  spaces.
* On managed devices:
  + Administrators are able to block users from having a private space and
    remove an existing private space on the personal profile of a corporate
    owned device. See [`UserManager.DISALLOW_ADD_PRIVATE_PROFILE`](/reference/android/os/UserManager#DISALLOW_ADD_PRIVATE_PROFILE).
  + Existing personal app allowlists and blocklists also apply to the
    private space on devices managed by the [`Android Management API`](https://developers.google.com/android/management/reference/rest/v1/enterprises.policies#PersonalApplicationPolicy).
    This feature will be available to [`custom DPCs`](/work/dpc/build-dpc) in the future.
...
* We introduce the concept of a "Managed SIM"/"Managed Subscription",
  ...
  + On company-owned devices, the IT admin can activate the eSIM silently
    ...
    eSIM. IT admins can set [`DISALLOW_CONFIG_MOBILE_NETWORKS`](/reference/android/os/UserManager#DISALLOW_CONFIG_MOBILE_NETWORKS) user
...
* Enforce the defaults for the personal profile on company-owned, personally
  ...
  + IT admins can enforce the OEM-default dialer, messages, and browser apps
    ...
    [`DISALLOW_CONFIG_DEFAULT_APPS`](/reference/android/os/UserManager#DISALLOW_CONFIG_DEFAULT_APPS).
