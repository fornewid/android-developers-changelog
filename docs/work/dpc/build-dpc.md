---
title: Build a device policy controller  |  Android Enterprise  |  Android Developers
...
# Build a device policy controller Stay organized with collections Save and categorize content based on your preferences.



**Caution:** Android Enterprise is no longer accepting new registrations for custom
device policy controllers (DPCs) to the Google Play EMM API. To get your DPC
approved by Android Enterprise for device management, visit [the DPC approval
...
**Note:** This guide does not cover the situation where the work
profile, under the enterprise's control, is the only profile on the device.
See [Deployment scenarios](https://developers.google.com/android/work/overview#deployment_scenarios)
...
* **Managed Google Play Accounts provisioning support**:
  ...
  Google Play Accounts](https://developers.google.com/android/work/overview#managed_google_play_accounts) from the DPC app requires that Google Play and Google
  ...
  See [managed Google Play Accounts
  provisioning support](#managed_google_play_account_support) for details.
* **Managed Configurations support**:
  ...
  to work apps](#apply_managed_configurations_to_work_apps) for more information about how to enable this capability in
  your DPC.
...
**Note:** Updating Google Play and Google Play services might
...
Specifically, your app must subclass `DeviceAdminReceiver` (a class from the `android.app.admin`
package) as described in
[Device Administration](/work/device-admin).
...
existence of the
[`FEATURE_MANAGED_USERS`](/reference/android/content/pm/PackageManager#FEATURE_MANAGED_USERS) system feature:
...
with an [ACTION\_PROVISION\_MANAGED\_PROFILE](/reference/android/app/admin/DevicePolicyManager#ACTION_PROVISION_MANAGED_PROFILE)
...
* Verifies that the device is encrypted. If it isn't, the system prompts
...
* Copies the DPC app into the work profile and sets the DPC itself as
  the profile owner.

Override `onActivityResult()` to
...
app's `DeviceAdminReceiver.onProfileProvisioningComplete()` method. Override
...
* Verifies that the device is complying with the EMM's device policies, as
  described in [Set up device policies](#set_up_policies).
* Enables the system applications that the admin has made
  available within the work profile using `DevicePolicyManager.enableSystemApp()`.
* If the device uses managed Google Play, adds the appropriate account to
  the work profile so that approved apps can be installed on the device.  
  + **Managed Google Play Accounts**: See [Ensure the working
    environment for managed Google Play Accounts](#ensure_the_working_environment) and [Add a managed Google Play Account](#add_a_managed_google_play_account)
    for details.
  + **Google Accounts**: Use `AccountManager.addAccount()`.

### Activate the work profile

Once you have completed these tasks, call the device policy manager's
`setProfileEnabled()` method to activate the work profile:
...
using the [Device
Administration](/work/device-admin) API.

For information on how to apply device policies, see
[Policies](/work/device-admin#policies).
...
Play (app developers, see [Set Up Managed Configurations](/work/managed-configurations) for details).
...
directly from the DPC](#apply_managed_configurations_from_dpc)). Using the Play EMM API
has several advantages, including easy implementation because you can use the
[DPC Support Library](#dpc_support_library) to simplify DPC tasks.
...
To use the Play
EMM API, download the [DPC Support Library](#dpc_support_library)
...
the ComponentName of the [DeviceAdminReceiver](/reference/android/app/admin/DeviceAdminReceiver).
...
[DevicePolicyManager.setApplicationRestrictions()](/reference/android/app/admin/DevicePolicyManager#setApplicationRestrictions(android.content.ComponentName, java.lang.String, android.os.Bundle)) method
and pass
parameters for the DPC app's
[DeviceAdminReceiver](/reference/android/app/admin/DeviceAdminReceiver), the package name of the target app,
and the [Bundle](/reference/android/os/Bundle) comprising
the app’s managed configuration as set by the admin. See
[How your
DPC and EMM console interact](https://developers.google.com/android/work/overview#how_your_dpc_and_emm_console_interact) and [Set up Managed Configurations](/work/managed-configurations) for details. However,
...
The [DPC Support Library](#dpc_support_library) includes support
for provisioning
managed Google Play Accounts. To use this support, you must first initialize the
library, and then you can [Ensure the
working environment](#ensure_the_working_environment) and [Add a
managed Google Play Account](#add_a_managed_google_play_account).
...
the `ComponentName` of the `DeviceAdminReceiver`.
...
After the DPC provisions a device in profile owner mode (`ACTION_PROVISION_MANAGED_PROFILE`) or device
owner mode (`ACTION_PROVISION_MANAGED_DEVICE`), make sure that the device can support managed Google Play Accounts by calling:
...
**Important:** The DPC should call `ensureWorkingEnvironment` as early as possible
during provisioning and before the user has set a passcode.
...
The Android framework's `AccountManager`
...
from the [DPC Support Library](#dpc_support_library). The function
...
* `token`—The user authentication token generated by the Google
  Play EMM API `Users.generateAuthenticationToken()` call.
* `accountAddedCallback`—Returns the managed Google Play Account
  that was successfully added to the device. This callback should include
  `onAccountReady()` and `onFailure()` methods.
...
* To learn more about the Device Administration API, see
  [Device
  Administration](/work/device-admin).
* To learn about Android Enterprise provisioning methods, see
  [Provision devices](https://developers.google.com/android/work/play/emm-api/prov-devices) in the
  Android Enterprise developer's guide.
* For a GitHub sample that demonstrates how to create a basic work profile,
  see [BasicManagedProfile](https://github.com/googlearchive/android-BasicManagedProfile).
* For a GitHub sample that demonstrates how to set configurations on other apps as a
  profile owner, see
  [AppRestrictionEnforcer](https://github.com/googlearchive/android-AppRestrictionEnforcer).