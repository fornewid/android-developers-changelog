---
title: Best practices for unique identifiers  |  Identity  |  Android Developers
...
# Best practices for unique identifiers Stay organized with collections Save and categorize content based on your preferences.
...
overview](/training/articles/user-data-overview). For specific best
practices for working with Android permissions, see [App permissions best
practices](/training/permissions/usage-notes).
...
1. ***Choose user-resettable identifiers whenever possible.*** Your app can
   achieve most of its use cases even when it uses identifiers other than
...
6. ***Use APIs that are appropriate for your use case to minimize privacy
   ...
   [Play Integrity APIs](/google/play/integrity/overview) for abuse protection.
...
* ***Segregating and monitoring access control lists for users or roles that have access to both the Advertising ID keyed data and PII***.
  ...
  1. Keep access control lists (ACLs) for Advertiser ID keyed data and PII
     disjoint to minimize the number of individuals or roles that are in both
     ACLs.
  2. Implement access logging and auditing to detect and manage any exceptions
     to this rule.
...
overview](/guide/topics/data/data-storage) page.
...
and
[`NetworkInterface.getHardwareAddress()`](/reference/java/net/NetworkInterface#getHardwareAddress())
...
[`ConnectivityManager`](/reference/android/net/ConnectivityManager) rather than
lower-level APIs like
[`NetworkInterface`](/reference/java/net/NetworkInterface),
[`getifaddrs()`](https://android.googlesource.com/platform/bionic/+/master/libc/include/ifaddrs.h#83),
or Netlink sockets. For example, an app that needs up-to-date information on the
current routes can get this information by listening for network changes using [`ConnectivityManager.registerNetworkCallback()`](/reference/android/net/ConnectivityManager#registerNetworkCallback(android.net.NetworkRequest,%20android.net.ConnectivityManager.NetworkCallback))
and calling the network's associated
[`LinkProperties.getRoutes()`](/reference/android/net/LinkProperties#getRoutes()).
...
* *Install-reset*: A new ID is used every time user uninstalls and reinstalls
  the app.
...
API](/reference/android/telephony/SubscriptionInfo#getSubscriptionId()) to
...
**Note:** Access to this ID requires the
...
ID](/reference/android/telephony/SubscriptionInfo#getIccId%28%29) for this
...
* **App set ID:** An App Set ID allows you to analyze a user's behavior across
  multiple apps that your organization owns, as long as you don't use user data
  for advertising purposes. If you're targeting devices powered by Google Play
  services, we recommend that you use App Set ID.
* **Firebase ID (FID):** An FID is scoped to the app that creates it, which
  prevents the identifier from being used to track users across apps. It is also
  easily resettable, as the user can clear app data or reinstall the app. The
  process of creating a FID is straightforward; see the Firebase installations
  guide.
...
[Google Play Integrity API](/google/play/integrity/overview).
