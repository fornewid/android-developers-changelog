---
title: Credential Manager - Verifier API  |  Identity  |  Android Developers
...
# Credential Manager - Verifier API Stay organized with collections Save and categorize content based on your preferences.
...
1. Add dependencies to your app's build script and initialize a
   `CredentialManager` class.
2. Construct a digital credential request and use it to initialize a
   `DigitalCredentialOption`, followed by building the `GetCredentialRequest`.
3. Launch the `getCredential` flow with the constructed request to receive a
   successful `GetCredentialResponse` or handle any exceptions that may occur.
   Upon successful retrieval, validate the response.
