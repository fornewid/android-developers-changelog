...
On devices configured with separate [system profiles](https://source.android.com/docs/devices/admin/multi-user) (such as a corporate device with a work profile and a personal profile), the restore key is only available to the profile that was set up first on the device.

## Limitations

Restore Credentials works on mobile devices and does not work across [form factors](https://developer.android.com/identity/form-factors).