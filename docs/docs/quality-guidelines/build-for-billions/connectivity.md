---
title: Connectivity for billions  |  Build for Billions  |  Android Developers
...
# Connectivity for billions Stay organized with collections Save and categorize content based on your preferences.
...
* Serve [WebP](https://developers.google.com/speed/webp/) files over the
  ...
  nearly identical image to the original. Android has included lossy [WebP support](/guide/topics/media/media-formats#image-formats) since
...
* Dynamic placeholders such as [pre-computed palette values](/reference/androidx/palette/graphics/Palette) or low-resolution thumbnails can improve
...
* Your app should not fetch any image more than once. Image
  loading libraries such as [Glide](https://github.com/bumptech/glide) and [Picasso](http://square.github.io/picasso/)
...
[WorkManager](/topic/libraries/architecture/workmanager) and [Room](/topic/libraries/architecture/room),
...
* Ensure that your app stores all data on disk using a database or similar
  ...
  library](/topic/libraries/architecture/room) to cache data in a local database, and use
  [WorkManager](/topic/libraries/architecture/workmanager) to update
...
* For transitory data, use a bounded disk cache such as a [`DiskLruCache`](https://github.com/JakeWharton/DiskLruCache).
...
* To achieve this, sequence your network requests such that text is
  ...
  Usage](/training/basics/network-ops/managing).
...
* On slower connections, consider downloading only lower-resolution media
  ...
  dynamic placeholder by using the [Palette library](/training/material/palette-colors) to generate placeholder colors that match the target
  image.
* On devices powered by Android 7.0 (API level 24) and higher,
  ...
  **Data Saver** setting, which helps minimize data use. Android 7.0
  ...
  [Data Saver.](/training/basics/network-ops/data-saver)
...
* On higher quality, unmetered networks, consider [prefetching data](/training/efficient-downloads/efficient-network-access#PrefetchData) to make it available ahead of time. From a user
  ...
  visit the Android training on [Monitoring the Connectivity Status](/training/monitoring-device-state/connectivity-monitoring).
* The broadcast [`CONNECTIVITY_CHANGE`](/reference/android/net/ConnectivityManager#CONNECTIVITY_ACTION) is sent when a change in network
  connectivity occurs. When your app is in the foreground, you can call [`registerReceiver`](/reference/android/content/Context#registerReceiver(android.content.BroadcastReceiver,%20android.content.IntentFilter)) to receive this broadcast. After receiving
  ...
  see [Android 7.0 Changes](/about/versions/nougat/android-7.0-changes).
...
* [Connect,
  No Matter the Speed](https://medium.com/google-design/connect-no-matter-the-speed-3b81cfd3355a)