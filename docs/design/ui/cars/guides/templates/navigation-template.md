---
title: Navigation template  |  Cars  |  Android Developers
...
# Navigation template Stay organized with collections Save and categorize content based on your preferences.
...
show a full-screen map that updates in real time. During active navigation,
apps can show optional cards with maneuvers and surface details, as well
as navigation alerts.

**Navigation template includes the following:**

* Full-screen base map drawn by app
* [Routing card](#routing-card) (optional) with upcoming
  maneuvers
* Travel estimate card (optional) with estimated time of arrival (ETA),
  time to destination, and remaining distance (or an alternate information
  display with custom text and icon options)
* [Action strip](/design/ui/cars/guides/components/action-strip)
  with up to 4 app actions, visible only as described in
  [Visibility of action strips](/design/ui/cars/guides/components/action-strip#visibility)
* Optional [map action strip](/design/ui/cars/guides/components/map-action-strip) with up to 4 buttons for map interactivity

![Wireframes of the Navigation template](/static/images/design/ui/cars/templates/navigation-template-1.png)
...
* **Dark theme.** Drawing a dark-themed version of the cluster map is strongly
  recommended, to reduce the potential for attracting the driver's attention away
  from the road.
...
**Note:** For AAOS, vehicle OEMs can opt to show their own turn-by-turn instructions
in the cluster, instead of the app's. When navigation ends, they can also decide
whether the cluster will keep showing the navigation app's map or revert to the
default map.

### Cluster and center screen examples

![Wireframe of a map cluster with a route overview map](/static/images/design/ui/cars/templates/nav-template-map-cluster.png)


Here, the app displays a close-up map in the cluster, while
displaying a route overview map in the center screen.


![Wireframe of a map cluster with settings](/static/images/design/ui/cars/templates/nav-template-map-cluster-2.png)


Here, the app's map continues to show in the cluster while the
user adjusts car settings on the center screen.

## Routing card details

![Mock-ups of routing card details in routing state](/static/images/design/ui/cars/templates/navigation-template-5.png)

When the routing card is in routing state (as opposed to
[message state](#message-state)), it shows the following
information:

1. **Current step**: includes icon (typically a direction
   arrow), distance, and cue text (which may include image spans such as route
   markers)
2. **Lane guidance** (optional): shown either as simple
   lane-assist images or as a larger junction image (flexible size with
   maximum height of 200dp)
3. **Next step** (optional): includes icon and cue, and can
   appear only at the bottom of a routing card that doesn't include a junction
   image
...
![Mockup of routing card message state](/static/images/design/ui/cars/templates/navigation-message-state.png)

### Navigation notifications: Turn-by-turn (TBT) and regular

![Routing card with TBT notification](/static/images/design/ui/cars/templates/navigation-tbt-notification.png)


![Routing card with regular notification](/static/images/design/ui/cars/templates/navigation-regular-notification.png)
...
* Icon (optional)
* Progress indicator – either a bar or (optionally) built into a timed
  button
* Up to 2
  [buttons](/design/ui/cars/guides/components/button),
  where a button can be designated as
  [primary](/design/ui/cars/guides/components/button#primary)
  or as a
  [timed button](/design/ui/cars/guides/components/button#timed)
  (with a progress indicator, as shown in the previous figure)
...
![A navigation alert showing an upcoming speed trap](/static/images/design/ui/cars/templates/navigation-template-9.png)





**Note:** Don't use navigation alerts to
show primary navigation information, such as upcoming turns or arrival time.
For those types of information, use the routing card or trip estimate
card.

## Navigation template examples

![Navigation template, no interaction](/static/images/design/ui/cars/templates/navigation-template-10.png)


Full-screen map when navigation and map interactivity are not
occurring (Android Auto example)

![Navigation template, no interaction](/static/images/design/ui/cars/templates/navigation-template-11.png)


Routing card with travel estimate, action strip (at top), and FAB
(at bottom right) during active navigation.
...
| --- | --- |
| Type | Link |
| API reference | `NavigationTemplate, NavigationTemplate.Builder` |
| Developer's Guide | [Access the navigation templates](/training/cars/apps/navigation#access-navigation-templates) |