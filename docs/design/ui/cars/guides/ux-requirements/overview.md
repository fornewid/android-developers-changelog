---
title: UX requirements  |  Cars  |  Android Developers
...
# UX requirements Stay organized with collections Save and categorize content based on your preferences.
...
need. UX requirements are expressed as instructions that you MUST, SHOULD,
or MAY follow. At a high level, you can understand these labels as follows:

|  |  |
| --- | --- |
| MUST | Required (enforced either in the API or in [Android app quality for cars)](/docs/quality-guidelines/car-app-quality) |
| SHOULD | Recommended |
| MAY | Optional |
...
* [General requirements (applies to all templated apps)](#general)
* [Navigation app requirements](#navigation)
* [Weather app requirements](#weather)
* [Template-specific requirements](#templates)
...
| **MUST** | Use the appropriate method (as stated in [Using the Android for Cars App Library](/training/cars/navigation#handle-user-input)) to direct Android Auto users to the phone for actions that are not allowed while driving, instructing them to look at their phone screens only when it's safe to do so (see [Grant permissions on phone](/design/ui/cars/guides/flows/grant-permissions-on-phone)). |
| **MUST NOT** | End 5-step task flows with a list-based template unless the [Adaptive task limits](/design/ui/cars/guides/ux-requirements/plan-task-flows#task-limits) feature is enabled (5th step must be one of these: Navigation, Message, or Pane). |
...
| **SHOULD** | Refresh content only for the purposes noted in [Limiting driver distraction](/design/ui/cars/guides/foundations/define-user-tasks#limit-distraction) or where explicitly permitted in guidance for specific templates or app types. |
...
| **MUST** | [Draw](/training/cars/apps/navigation#drawing-the-map) only map content and drive-related content on the surface of the template. |
...
| **SHOULD** | Use turn-by-turn (TBT) notifications to show directions when a user is completing a task outside of the Navigation template during active navigation (as shown in [Navigation notifications: TBT and regular](/design/ui/cars/guides/templates/navigation-template#notifications)). |
...
| **MAY** | Use [navigation alerts](/design/ui/cars/guides/templates/navigation-template#alerts) or heads-up notifications (HUNs) to alert users about general navigation-related updates (in addition to turn-by-turn directions), such as traffic ahead. |
...
Use the template-specific requirements to make sure each template in your task
flows meets UX requirements and recommendations.

Learn more about the requirements for each template:

* [Grid template](/design/ui/cars/guides/templates/grid-template#requirements)
* [List template](/design/ui/cars/guides/templates/list-template#requirements)
* [Long Message template](/design/ui/cars/guides/templates/long-message-template#requirements)
* [Message template](/design/ui/cars/guides/templates/message-template#requirements)
* [Map + Content template](/design/ui/cars/guides/templates/map-content-template#requirements)
* [Navigation template](/design/ui/cars/guides/templates/navigation-template#requirements)
* [Pane template](/design/ui/cars/guides/templates/pane-template#requirements)
* [Place List (map) template](/design/ui/cars/guides/templates/place-list-map-template#requirements)
* [Search template](/design/ui/cars/guides/templates/search-template#requirements)
* [Sign-in template](/design/ui/cars/guides/templates/sign-in-template#requirements)
* [Tab template](/design/ui/cars/guides/templates/tab-template#requirements)