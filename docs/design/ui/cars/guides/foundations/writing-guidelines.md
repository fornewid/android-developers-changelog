---
title: Writing principles  |  Cars  |  Android Developers
...
# Writing principles Stay organized with collections Save and categorize content based on your preferences.
...
* [Make content glanceable](#glanceable)
* [Use consistent language](#consistent)
* [Write with situational awareness](#situational)
* [Follow best practices for writing component text](#best-practices)

## Make content glanceable

Simple, glanceable content makes sure that the driver's attention returns
quickly to the road.

You should:

* [Use scannable content patterns](#scannable)
* [Use simpler, direct language](#simple-language)
* [Limit text to 120 characters](#limit-text)

### Use scannable content patterns

While both driving and parked, the goal is to reduce the driver's cognitive
load. People don't read what's on the screen; instead they scan the text in an
F-shaped pattern.

Front-load important information, clearly and concisely describing the action
the user needs to perform. Use clear hierarchy to convey information at a
glance. For example, use bullet points to organize content.
...
Avoid using jargon or complex words. They confuse the user and delay task
completion.
...
Limiting the amount of text the driver has to read before taking action reduces
cognitive load and increases comprehension. Based on guidance from
National Highway Traffic Safety Administration (NHTSA) and
Japan Automobile Manufacturers Association (JAMA):

* A single message or chunk of text in English shouldn't exceed 120 characters
  or 24 words
...
* [Be consistent in your writing style](#consistent-writing)
* [Unify terminology](#terminology)

### Be consistent in your writing style

Consistency creates a cohesive and recognizable voice for your brand. It helps
drivers navigate the text seamlessly and builds trust and loyalty over time.

Based on your brand's style choices, ensure consistency in choice of words,
sentence structure, capitalization, punctuation, and overall voice and tone.
...
Using different words to mean the same thing confuses users. Use consistent
terminology across your product to remove ambiguity during user journeys,
enhance user comprehension, and increase user trust.
...
Ensure that the language on the UI is relevant to the user's needs and
situation, including whether they are driving, parked, or experiencing an
emergency.

You should:

* [Balance brevity with clarity](#brevity-clarity)
* [Lead with benefit to the user](#lead-with-benefits)
* [Use reassuring tone of voice, except in emergency](#tone)

### Balance brevity with clarity

The text shown during driving should be brief and clear. However, it's OK to use
longer explanations for complex situations — such as privacy consents,
permissions, and data sharing — when the driver is parked. In those instances,
break up longer text using lists and hierarchy to keep text readable.
...
By starting with the value a feature offers, you can prioritize the importance
of the user's needs, reduce cognitive load, and help them make a decision to
move forward in a short span of time.
...
When the car is in motion, use a reassuring tone of voice to avoid distracting
the driver and causing needless anxiety.

However, exceptions should be made when the driver is in physical danger and
needs to take immediate action. In those cases, use direct instruction and
exclamation marks to convey urgency.
...
* [Make button text brief and actionable](#button-text)
* [Provide confirmation messages](#confirmation)
* [Provide recovery from errors](#error-recovery)
* [Front-load critical information in notifications](#critical-info)

### Make button text brief and actionable

Button labels, also known as CTAs (calls to action), should always start with a
verb and ideally contain no more than 3 words.
...
Provide a confirmation message to reassure users when tasks they initiated have
been completed.
...
Error messages should state the problem clearly and offer a way out of the
situation. If there's no recovery path, the message should be transparent about
it with the user.
...
Notifications should succinctly front-load critical information.
[Avoid causing alarm to the driver in any way](#tone).
