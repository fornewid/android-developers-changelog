...
![Hero illustration of the PiP symbol and the Play logo](https://developer.android.com/static/images/design/ui/mobile/pip-hero.png)
...
has an [active media session](https://developer.android.com/media/legacy/mediasession), controls for play, pause, next, and previous
also display. Read about how to [add these controls](https://developer.android.com/develop/ui/views/picture-in-picture#add_controls).
...
![User interface showing default picture-in-picture controls](https://developer.android.com/static/images/design/ui/mobile/pip-2-default-controls.png)
...
Use an interaction pattern that complements the viewing experience without
...
To make the user experience polished when shifting into and out of PiP mode,
customize the entry and exit transitions.
...
Android 12 introduced cosmetic improvements to transitions between fullscreen
and PiP windows. Once implemented, these updates scale automatically to tablets
and foldables.

Without these updates, transitions are functional but less polished. For
example, the PiP window might disappear temporarily during the transition.

Starting in Android 12, the
[`PictureInPictureParams.Builder.setAutoEnterEnabled(true)`](https://developer.android.com/reference/android/app/PictureInPictureParams.Builder#setAutoEnterEnabled(boolean)) flag provides
smoother gesture transitions (like swiping up to home). We recommend this for
`ENTERTAINMENT`, `COMMUNICATION`, or `VIDEO_PLAYER` apps.

Without this flag, gesture transitions shrink the window into the app icon
before it reappears as the PiP window (as shown in Video 1).
...
When we introduced PiP in Android 8.0, [`sourceRectHint`](https://developer.android.com/reference/android/app/PictureInPictureParams.Builder#setSourceRectHint(android.graphics.Rect)) indicated the area
...
Alas, your browser doesn't support HTML5 video. That's OK! You can still [download the video](https://developer.android.com/static/images/design/ui/mobile/PiP-video-3-before-sourceRectHint.mp4) and watch it with a video player. **Video 3:** Less polished transition experience when PiP doesn't have `sourceRectHint` properly implemented

To see an example of how PiP animation looks when `sourceRectHint` is properly
implemented, see video 2 in the preceding section.
