---
title: Jetifier  |  Android Studio  |  Android Developers
...
# Jetifier Stay organized with collections Save and categorize content based on your preferences.
...
| `-l`, `--log` <level> | no | Logging level. Allowed values are:  * error * warning * info * verbose  If not specified, defaults to "warning". |
...
| `-t`, `-timestamp` <arg> | no | Timestamps policy to use for the archived entries as their modified time. Values: keepPrevious (default) epoch or now. |
...
1. Extract the file `default.generated.config` from the utility's
