...
all system services for your connected device, run `adb shell dumpsys`.
...
**Table 1.** List of available options for
dumpsys
...
**Event Hub State** of the input diagnostics:
...
The `InputReader` is responsible for decoding input events from the kernel. Its
...
Specifying the `gfxinfo` service provides output with performance information
relating to frames of animation that occur during the recording phase.
The following command uses `gfxinfo` to gather UI performance data for a
...
You can also include the `framestats` option to provide even more detailed frame
...
Specifying the `netstats` service provides network usage statistics collected
...
package your-package-name`. Then look for the line labeled
`userId`.
...
To see a list of additional options available to `batterystats`, include the
`-h` option. The following example outputs battery usage statistics for a
...
(`wifi`, `blue`, `cell`) for each component.
...
`minPSS-avgPSS-maxPSS/minUSS-avgUSS-maxUSS/minRSS-avgRSS-maxRSS` over
...
The `-h` flag prints all supported flags.
...
**Note:** The information you see might vary slightly from
...
In general, be concerned with only the `Pss Total` and `Private Dirty` columns.
In some cases, the `Private Clean` and `Heap Alloc` columns also
...
:   The RAM used by Dalvik allocations in your app. The `Pss Total`
    ...
    **Note:** On newer platform versions that have the
    `Dalvik Other` section, the `Pss Total` and `Private
    ...
    `Pss Total` and `Private Dirty` because your process was
...
:   Any RAM pages that the system couldn't classify into one of the other more
    ...
    Randomization (ASLR). Like the Dalvik heap, the `Pss Total` for
    `Unknown` takes into account sharing with Zygote, and `Private Dirty`
...
:   The total Proportional Set Size (PSS) RAM used by your process. This is the
    ...
    back to the system. `Private Clean` can also be paged out and released
