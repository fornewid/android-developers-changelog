---
title: R8 retrace  |  Android Studio  |  Android Developers
...
# R8 retrace Stay organized with collections Save and categorize content based on your preferences.



R8 retrace is a tool for obtaining the original stack trace from an obfuscated
...
**Note:** R8 retrace is a standalone tool in version
4.0 of the command-line tools
package, released with Android Studio 4.2.
...
| `--regex <regular_exp>` | no | Overwrites the default regular expression for parsing stack trace lines. For example, the following is a regex that can parse basic stack traces: `(?:.*? at %c\.%m\(%s(?::%l)?\))|(?:(?:.*?[:"] +)?%c(?::.*)?)`. |
