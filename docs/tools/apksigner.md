---
title: apksigner  |  Android Studio  |  Android Developers
...
# apksigner Stay organized with collections Save and categorize content based on your preferences.
...
`apksigner` tool is used for signing your APKs, see [Sign your app](/studio/publish/app-signing).

**Caution:** If you sign your APK using `apksigner`
and make further changes to the APK, the APK's signature is invalidated.
If you use
[`zipalign`](/studio/command-line/zipalign) to align
...
* Specify the private key file and certificate file separately using the
  `--key` and `--cert` options, respectively. The
...
to separate the set of [general options](#options-sign-general) to
...
:   Determines whether `apksigner` signs the given APK package
    using the [APK
    Signature Scheme v2](/about/versions/nougat/android-7.0#apk_signature_v2). By default, the tool uses the values of
...
:   Determines whether `apksigner` signs the given APK package
    using the [APK
    Signature Scheme v4](/about/versions/11/features#signature-scheme-v4). This scheme
    produces a signature in a separate file (`apk-name.apk.idsig`).
...
:   The password for the KeyStore that contains the signer's private key and
    ...
    * `env:<name>` – Password is stored in the given
      environment variable.
    * `file:<filename>` – Password is stored as a
      single line in the given file.
    * `stdin` – Password is provided as a single line in
...
:   Includes the specified character encodings, such as
    ...
    You may need to specify `--pass-encoding` when a
    non-ASCII password is used. You may also need to specify this option with KeyStores that
    keytool created on a different OS or in a different locale.
...
:   The password for the signer's private key, which is needed if the
    ...
    * `pass:<password>` – Password is provided inline
      with the rest of the `apksigner sign` command.
    * `env:<name>` – Password is stored in the given
      environment variable.
    * `file:<filename>` – Password is stored as a
