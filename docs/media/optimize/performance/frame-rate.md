---
title: Frame rate  |  Android media  |  Android Developers
...
# Frame rate Stay organized with collections Save and categorize content based on your preferences.
...
**Important:** The scheduler takes a number of factors into consideration when
deciding on the display refresh rate, so there is no guarantee that your app
will get the frame rate you request with `setFrameRate()`.
...
* [`Surface.setFrameRate()`](/reference/android/view/Surface#setFrameRate(float,%20int))
* [`SurfaceControl.Transaction.setFrameRate()`](/reference/android/view/SurfaceControl.Transaction#setFrameRate(android.view.SurfaceControl,%20float,%20int))
* [`ANativeWindow_setFrameRate()`](/ndk/reference/group/a-native-window#anativewindow_setframerate)
* [`ASurfaceTransaction_setFrameRate()`](/ndk/reference/group/native-activity#asurfacetransaction_setframerate)
...
[`DisplayManager.registerDisplayListener()`](/reference/android/hardware/display/DisplayManager#registerDisplayListener(android.hardware.display.DisplayManager.DisplayListener,%20android.os.Handler))
or [`AChoreographer_registerRefreshRateCallback()`](/ndk/reference/group/choreographer#achoreographer_registerrefreshratecallback).
...
when the [`Surface.setFrameRate()`](/reference/android/view/Surface#setFrameRate(float,%20int,%20int))
...
* **Users**: To opt in, users can enable the [Match content frame rate](https://cs.android.com/android/platform/superproject/+/master:frameworks/base/core/java/android/provider/Settings.java;l=8393;drc=eb151ee8d0ffaca0acbc68e3ac2548efaa3acc32) user
  setting.
* **Apps**: To opt in, apps can pass
  [`CHANGE_FRAME_RATE_ALWAYS`](/reference/android/view/Surface#CHANGE_FRAME_RATE_ALWAYS)
  into [`setFrameRate()`](/reference/android/view/Surface#setFrameRate(float,%20int,%20int)).

We recommend you always use [`CHANGE_FRAME_RATE_ALWAYS`](/reference/android/view/Surface#CHANGE_FRAME_RATE_ALWAYS)
...
* [`ASurfaceTransaction_setDesiredPresentTime()`](/ndk/reference/group/native-activity#group___native_activity_1gae7b547aa6e72b83d0a151cac62880d47)
...
[frame pacing guide](/games/sdk/frame-pacing#short_game_frames_lead_to_stuttering)
for more info on avoiding judder, and consider using the
[Android Frame Pacing library](/games/sdk).
...
[`WindowManager.LayoutParams.preferredDisplayModeId`](/reference/android/view/WindowManager.LayoutParams#preferredDisplayModeId)
...
* If the app wants to change the resolution or other display mode settings,
  use `preferredDisplayModeId`.
* The platform will only switch display modes in response to a call to
  `setFrameRate()` if the mode switch is lightweight and unlikely to be
  ...
  device), use `preferredDisplayModeId`.
* Apps that can't handle the display running at a multiple of the app's frame
  rate, which requires setting presentation timestamps on each frame, should
  use `preferredDisplayModeId`.

### setFrameRate() vs preferredRefreshRate

[`WindowManager.LayoutParams#preferredRefreshRate`](/reference/android/view/WindowManager.LayoutParams#preferredRefreshRate)
...
### Usage of FRAME\_RATE\_COMPATIBILITY\_FIXED\_SOURCE
...
* We strongly recommend that apps, when displaying long-running videos such as
  movies, call `setFrameRate(`fps`, FRAME_RATE_COMPATIBILITY_FIXED_SOURCE, CHANGE_FRAME_RATE_ALWAYS)`
  where fps is the frame rate of the video.
* We strongly recommend against apps calling `setFrameRate()` with `CHANGE_FRAME_RATE_ALWAYS`
  when you expect video playback to last several minutes or less.
...
1. Decide the `changeFrameRateStrategy`:
   1. If playing a long running video such as a movie use [`MATCH_CONTENT_FRAMERATE_ALWAYS`](/reference/android/hardware/display/DisplayManager#MATCH_CONTENT_FRAMERATE_ALWAYS)
   2. If playing a short video such as a move trailer use [`CHANGE_FRAME_RATE_ONLY_IF_SEAMLESS`](/reference/android/view/Surface#CHANGE_FRAME_RATE_ONLY_IF_SEAMLESS)
2. If the `changeFrameRateStrategy` is [`CHANGE_FRAME_RATE_ONLY_IF_SEAMLESS`](/reference/android/view/Surface#CHANGE_FRAME_RATE_ONLY_IF_SEAMLESS)
   , go to step 4.
3. Detect if a non-seamless refresh rate switch is about to happen by checking
   that both of these facts are true:
   1. Seamless mode switch isn't possible from the current refresh rate (let’s
      call it C) to the frame rate of the video (let’s call it V). This will
      be the case if C and V are different and
      [`Display.getMode().getAlternativeRefreshRates`](/reference/android/view/Display.Mode#getAlternativeRefreshRates())
      does not contain a multiple of V.
   2. The user has opted in to non-seamless refresh rate changes. You can detect
      this by checking if [`DisplayManager.getMatchContentFrameRateUserPreference`](/reference/android/hardware/display/DisplayManager#getMatchContentFrameRateUserPreference())
      returns [`MATCH_CONTENT_FRAMERATE_ALWAYS`](/reference/android/hardware/display/DisplayManager#MATCH_CONTENT_FRAMERATE_ALWAYS)
4. If the switch is going to be seamless, do the following:
   1. Call [`setFrameRate`](/reference/android/view/Surface#setFrameRate(float,%20int,%20int))
      and pass it `fps`, [`FRAME_RATE_COMPATIBILITY_FIXED_SOURCE`](/reference/android/view/Surface#FRAME_RATE_COMPATIBILITY_FIXED_SOURCE),
      and `changeFrameRateStrategy`, where `fps` is the frame rate of the video.
   2. Begin video playback
5. If a non-seamless mode change is about to happen, do the following:
   1. Show UX to notify the user. Note that we recommend you implement a way for
      ...
      exhibit faster switching times.
   2. Call [`setFrameRate`](/reference/android/view/Surface#setFrameRate(float,%20int,%20int))
      and pass it `fps`, [`FRAME_RATE_COMPATIBILITY_FIXED_SOURCE`](/reference/android/view/Surface#FRAME_RATE_COMPATIBILITY_FIXED_SOURCE),
      and [`CHANGE_FRAME_RATE_ALWAYS`](/reference/android/view/Surface#CHANGE_FRAME_RATE_ALWAYS),
      where `fps` is the frame rate of the video.
   3. Wait for the [`onDisplayChanged`](/reference/android/hardware/display/DisplayManager.DisplayListener#onDisplayChanged(int))
      callback.
