---
title: Configuration options  |  Android media  |  Android Developers
...
# Configuration options Stay organized with collections Save and categorize content based on your preferences.
...
images](/training/camerax/analyze), and [Image
capture](/training/camerax/take-photo).
...
configurations, [`CameraXConfig`](/reference/androidx/camera/core/CameraXConfig)
is the interface for that purpose.

With `CameraXConfig`, an application can do the following:

* Optimize startup latency with
  [`setAvailableCameraLimiter()`](/reference/androidx/camera/core/CameraXConfig.Builder#setAvailableCamerasLimiter(androidx.camera.core.CameraSelector)).
* Provide the application's executor to CameraX with
  [`setCameraExecutor()`](/reference/androidx/camera/core/CameraXConfig.Builder#setCameraExecutor(java.util.concurrent.Executor)).
* Replace the default scheduler handler with
  [`setSchedulerHandler()`](/reference/androidx/camera/core/CameraXConfig.Builder#setSchedulerHandler(android.os.Handler)).
* Change the logging level with
  [`setMinimumLoggingLevel()`](/reference/androidx/camera/core/CameraXConfig.Builder#setMinimumLoggingLevel(int)).
...
2. Implement the
   [`CameraXConfig.Provider`](/reference/androidx/camera/core/CameraXConfig.Provider)
   interface in your [`Application`](/reference/android/app/Application), and
   return your `CameraXConfig` object in
   [`getCameraXConfig()`](/reference/androidx/camera/core/CameraXConfig.Provider#getCameraXConfig()).
3. Add your `Application` class to your `AndroidManifest.xml` file, as
   described [here](/reference/android/app/Application).
...
[`ProcessCameraProvider.getInstance()`](/reference/androidx/camera/lifecycle/ProcessCameraProvider#getInstance(android.content.Context)),
...
If the [`CameraSelector`](/reference/androidx/camera/core/CameraSelector) passed
to
[`CameraXConfig.Builder.setAvailableCamerasLimiter()`](/reference/androidx/camera/core/CameraXConfig.Builder#setAvailableCamerasLimiter(androidx.camera.core.CameraSelector))
...
[`CameraXConfig.Builder.setCameraExecutor()`](/reference/androidx/camera/core/CameraXConfig.Builder#setCameraExecutor(java.util.concurrent.Executor))
and
[`CameraXConfig.Builder.setSchedulerHandler()`](/reference/androidx/camera/core/CameraXConfig.Builder#setSchedulerHandler(android.os.Handler)).

**Note:** When providing a custom executor or scheduler handler, use
one that doesn't execute code on the main thread.
...
[`Executor`](/reference/java/util/concurrent/Executor) to perform these tasks.
...
[`Handler`](/reference/android/os/Handler) for callbacks. In these cases, the
callbacks are still only dispatched directly to the camera executor. CameraX
allocates and manages an internal
[`HandlerThread`](/reference/android/os/HandlerThread) to perform these tasks,
...
Refer to the [Android Log documentation](/reference/android/util/Log#DEBUG)
for detailed descriptions of these log levels. Use
[`CameraXConfig.Builder.setMinimumLoggingLevel(int)`](/reference/androidx/camera/core/CameraXConfig.Builder#setMinimumLoggingLevel(int))
...
* The device has compatibility issues, such as legacy devices that require
  certain resolutions to operate correctly.
* On some devices, certain formats are only available at certain aspect
  ratios.
* The device has a preference for a "nearest mod16" for JPEG or video
  encoding. For more information, see
  [`SCALER_STREAM_CONFIGURATION_MAP`](/reference/android/hardware/camera2/CameraCharacteristics#SCALER_STREAM_CONFIGURATION_MAP).
...
[`ImageAnalysis.setTargetRotation()`](/reference/androidx/camera/core/ImageAnalysis#setTargetRotation(int))),
...
output from
[`Preview.PreviewOutput()`](/reference/androidx/camera/core/Preview.PreviewOutput)
...
* **Preview**: Metadata output is provided so that the rotation of the target
  ...
  [`Preview.getTargetRotation()`](/reference/androidx/camera/core/Preview#getTargetRotation()).
* **ImageAnalysis**: Metadata output is provided so that image buffer
  coordinates are known relative to display coordinates.
* **ImageCapture**: The image Exif metadata, buffer, or both the buffer and
  metadata are altered to note the rotation setting. The value altered
  depends upon the HAL implementation.

## Crop rect

By default, the crop rect is the full buffer rect. You can customize it with
[`ViewPort`](/reference/androidx/camera/core/ViewPort) and
[`UseCaseGroup`](/reference/androidx/camera/core/UseCaseGroup). By grouping use
...
to use [`PreviewView`](/training/camerax/preview#implementation).
...
output](/training/camerax/transform-output).
...
* Request the default front facing camera with
  [`CameraSelector.DEFAULT_FRONT_CAMERA`](/reference/androidx/camera/core/CameraSelector#DEFAULT_FRONT_CAMERA).
* Request the default rear facing camera with
  [`CameraSelector.DEFAULT_BACK_CAMERA`](/reference/androidx/camera/core/CameraSelector#DEFAULT_BACK_CAMERA).
* Filter the list of available devices by their
  ...
  with
  [`CameraSelector.Builder.addCameraFilter()`](/reference/androidx/camera/core/CameraSelector.Builder#addCameraFilter(androidx.camera.core.CameraFilter)).
...
level](/reference/android/hardware/camera2/CameraCharacteristics#INFO_SUPPORTED_HARDWARE_LEVEL),
...
available](/reference/android/hardware/camera2/CameraDevice#createCaptureSession(android.hardware.camera2.params.SessionConfiguration))).
...
* Specify a custom resolution, which CameraX attempts to find the closest
  match to
...
| **Max resolution:** The camera device's maximum output resolution of YUV\_420\_888 format which is retrieved from [`StreamConfigurationMap.getOutputSizes()`](/reference/android/hardware/camera2/params/StreamConfigurationMap#getOutputSizes(int)). The target resolution is set as 640x480 by default, so if you want a resolution larger than 640x480, you must use [`setTargetResolution()`](/reference/kotlin/androidx/camera/core/ImageAnalysis.Builder#settargetresolution) and [`setTargetAspectRatio()`](/reference/kotlin/androidx/camera/core/ImageAnalysis.Builder#settargetaspectratio) to get the closest one from the supported resolutions. |
...
| **Max resolution:** The camera device's maximum output resolution in a JPEG format. Use [`StreamConfigurationMap.getOutputSizes()`](/reference/android/hardware/camera2/params/StreamConfigurationMap#getOutputSizes(int)) to retrieve this. |
...
Express the resolution [`Size`](/reference/android/util/Size) in the coordinate
...
**Note:** If you use `setTargetResolution()`, then you can receive a buffer whose
aspect ratio doesn't match other use cases. If the aspect ratios must match,
check the dimensions of the buffers returned by both use cases and crop or scale
one of them to match the other.

If your app requires an exact resolution, see the table within
[`createCaptureSession()`](/reference/android/hardware/camera2/CameraDevice#regular-capture)
...
[`StreamConfigurationMap.getOutputSizes(int)`](/reference/android/hardware/camera2/params/StreamConfigurationMap#getOutputSizes(int)).

If your app is running on Android 10 or higher, you can use
[`isSessionConfigurationSupported()`](/reference/android/hardware/camera2/CameraDevice#isSessionConfigurationSupported(android.hardware.camera2.params.SessionConfiguration))
...
* [`CameraControl`](/reference/androidx/camera/core/CameraControl) lets you
  configure common camera features.
* [`CameraInfo`](/reference/androidx/camera/core/CameraInfo) lets you query
  the states of those common camera features.
...
[`Camera`](/reference/androidx/camera/core/Camera) object returned by
[`ProcessCameraProvider.bindToLifecycle()`](/reference/androidx/camera/lifecycle/ProcessCameraProvider).
...
**Note:** All changes to the state of zoom, torch, focus and metering, and exposure
compensation controls revert to their default values after the `Camera` is
closed, which happens when the
[`LifecycleOwner`](/reference/androidx/lifecycle/LifecycleOwner) is stopped or
destroyed.
...
* [`setZoomRatio()`](/reference/androidx/camera/core/CameraControl#setZoomRatio(float))
...
* [`setLinearZoom()`](/reference/androidx/camera/core/CameraControl#setLinearZoom(float))
  ...
  [`Slider`](/reference/com/google/android/material/slider/Slider) view.

[`CameraInfo.getZoomState()`](/reference/androidx/camera/core/CameraInfo#getZoomState())
...
[`ZoomState.getZoomRatio()`](/reference/androidx/camera/core/ZoomState#getZoomRatio())
and
[`ZoomState.getLinearZoom()`](/reference/androidx/camera/core/ZoomState#getLinearZoom()).
...
[`CameraControl.enableTorch(boolean)`](/reference/androidx/camera/core/CameraControl#enableTorch(boolean))
enables or disables the torch (also known as the flashlight).

[`CameraInfo.getTorchState()`](/reference/androidx/camera/core/CameraInfo#getTorchState())
can be used to query the current torch state. You can check the value returned
by
[`CameraInfo.hasFlashUnit()`](/reference/androidx/camera/core/CameraInfo#hasFlashUnit())
...
[`flashMode`](/reference/android/hardware/Camera.Parameters#getFlashMode()) in
`ImageCapture` works only when the torch is disabled.

### Focus and Metering

[`CameraControl.startFocusAndMetering()`](/reference/androidx/camera/core/CameraControl#setExposureCompensationIndex(int))
...
To begin, create a
[`MeteringPoint`](/reference/androidx/camera/core/MeteringPoint) using
[`MeteringPointFactory.createPoint(float x, float y, float
size)`](/reference/androidx/camera/core/MeteringPointFactory#createPoint(float,%20float,%20float)).
A `MeteringPoint` represents a single point on the camera
[`Surface`](/reference/android/view/Surface). It’s stored in a normalized form
...
To invoke
[`startFocusAndMetering()`](/reference/androidx/camera/core/CameraControl#startFocusAndMetering(androidx.camera.core.FocusMeteringAction)),
applications must build a
[`FocusMeteringAction`](/reference/androidx/camera/core/FocusMeteringAction),
which consists of one or more `MeteringPoints` with optional metering mode
combinations from
[`FLAG_AF`](/reference/androidx/camera/core/FocusMeteringAction#FLAG_AF),
[`FLAG_AE`](/reference/androidx/camera/core/FocusMeteringAction#FLAG_AE),
[`FLAG_AWB`](/reference/androidx/camera/core/FocusMeteringAction#FLAG_AWB). The
...
[`MeteringRectangles`](/reference/android/hardware/camera2/params/MeteringRectangle)
and sets the corresponding
[`CONTROL_AF_REGIONS`](/reference/android/hardware/camera2/CaptureRequest#CONTROL_AF_REGIONS)
/
[`CONTROL_AE_REGIONS`](/reference/android/hardware/camera2/CaptureRequest#CONTROL_AE_REGIONS)
/
[`CONTROL_AWB_REGIONS`](/reference/android/hardware/camera2/CaptureRequest#CONTROL_AWB_REGIONS)
...
[`Camera.CameraControl.setExposureCompensationIndex()`](/reference/androidx/camera/core/CameraControl#setExposureCompensationIndex(int))
...
[`CameraInfo.ExposureState.exposureCompensationRange()`](/reference/androidx/camera/core/ExposureState#getExposureCompensationRange())
...
* [`Camera.CameraInfo.getExposureState()`](/reference/androidx/camera/core/CameraInfo#getExposureState())
  retrieves the current
  [`ExposureState`](/reference/androidx/camera/core/CameraInfo#getExposureState())
  including:
  ...
  + The exposure compensation step used in exposure compensation value
    calculation.

For example, the following code initializes the settings for an exposure
[`SeekBar`](/reference/android/widget/SeekBar) with current `ExposureState`
values:
