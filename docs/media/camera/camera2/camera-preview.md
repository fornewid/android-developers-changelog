---
title: Camera preview  |  Android media  |  Android Developers
...
# Camera preview Stay organized with collections Save and categorize content based on your preferences.
...
devices](/develop/ui/compose/layouts/adaptive/foldables/learn-about-foldables),
and display modes such as
[multi‑window](/develop/ui/compose/layouts/adaptive/support-multi-window-mode)
and
[multi‑display](/develop/ui/compose/layouts/adaptive/support-multi-window-mode#multi-display_support).
...
![Phone and camera sensor both in portrait orientation.](/static/images/training/camera/camera2/camera-preview/camera_preview_portrait_front_facing.png)


**Figure 1.** Typical relationship of phone and camera sensor
orientation.
...
[camera2](/reference/android/hardware/camera2/package-summary) API includes a
[`SENSOR_ORIENTATION`](/reference/android/hardware/camera2/CameraCharacteristics#SENSOR_ORIENTATION)
...
**Note:** Natural (or native) orientation is the typical orientation of a device
or sensor—portrait for phones, landscape for tablets and laptops, landscape
...
![Camera sensor rotated to landscape orientation with image
            sideways, top left.](/static/images/training/camera/camera2/camera-preview/camera_sensor_portrait_rotated_90.png)

The image must be rotated 270 degrees counterclockwise so that the preview's
orientation matches the device orientation:

![Camera sensor in portrait orientation with image upright.](/static/images/training/camera/camera2/camera-preview/camera_sensor_portrait_without_coordinates.png)
...
* [`Display#getRotation()`](/reference/android/view/Display#getRotation())
  provides the counterclockwise rotation of the device (from the user's point
  of view). This value plugs into the formula above as is.
* [`OrientationEventListener#onOrientationChanged()`](/reference/android/view/OrientationEventListener#onOrientationChanged(int))
  returns the clockwise rotation of the device (from the user's point of view).
  Negate the value for use in the formula above.

### Front-facing cameras

![Camera preview and sensor both in landscape orientation, sensor
            is right side up.](/static/images/training/camera/camera2/camera-preview/camera_preview_landscape_counterclockwise_front_facing.png)


**Figure 2.** Camera preview and sensor with phone turned 90 degrees to
landscape orientation.

Here's the image buffer produced by the camera sensor in figure 2:

![Camera sensor in landscape orientation with image upright.](/static/images/training/camera/camera2/camera-preview/camera_sensor_landscape.png)

The buffer has to be rotated 270 degrees counterclockwise to adjust for sensor
orientation (see [Camera orientation](#camera-orientation) above):

![Camera sensor rotated to portait orientation with image sideways,
            top right.](/static/images/training/camera/camera2/camera-preview/camera_sensor_landscape_rotated_270.png)
...
![Camera sensor rotated to landscape orientation with image
            upright.](/static/images/training/camera/camera2/camera-preview/camera_sensor_landscape_without_coordinates.png)
...
![Camera preview and sensor both in landscape orientation, but
            sensor is upside down.](/static/images/training/camera/camera2/camera-preview/camera_preview_landscape_clockwise_front_facing.png)


**Figure 3.** Camera preview and sensor with phone turned 270 degrees
(or -90 degrees) to landscape orientation.

Here's the image buffer:

![Camera sensor rotated to landscape orientation with image upside
            down.](/static/images/training/camera/camera2/camera-preview/camera_sensor_landscape_clockwise.png)
...
![Camera sensor rated to portrait orientation with image sideways,
            top left.](/static/images/training/camera/camera2/camera-preview/camera_sensor_landscape_clockwise_rotated_270.png)
...
![Camera preview and sensor both in landscape orientation, but
            sensor is upside down.](/static/images/training/camera/camera2/camera-preview/camera_preview_landscape_clockwise_back_facing.png)


**Figure 4.** Phone with back-facing camera in landscape orientation
(turned 270 or -90 degrees).
...
![Unfolded foldable device with portrait camera preview turned
            sideways.](/static/images/training/camera/camera2/camera-preview/foldable_with_rotation_problem.png)


**Figure 5.** Foldable device transitions from portrait to landscape
aspect ratio but camera sensor remains in portrait orientation.
...
![Unfolded foldable device with camera preview upright but squashed
            because of incorrect scaling.](/static/images/training/camera/camera2/camera-preview/foldable_with_scaling_problem.png)


**Figure 6.** Foldable device transitions from portrait to landscape
aspect ratio but camera sensor remains in portrait orientation.
...
![Camera preview on laptop is upright but app UI is sideways.](/static/images/training/camera/camera2/camera-preview/laptop_camera_preview_fixed-orientation_app.png)
...
([`resizeableActivity="false"`](/guide/topics/manifest/application-element#resizeableActivity))
and restrict their orientation
([`screenOrientation="portrait"`](/guide/topics/manifest/activity-element#screen)
or [`screenOrientation="landscape"`](/guide/topics/manifest/activity-element#screen))
...
![Camera preview and app UI in proper portrait orientation on laptop.
            ...
            orientation.](/static/images/training/camera/camera2/camera-preview/laptop_inset_portrait_mode.png)


**Figure 8.** Fixed-orientation portrait app in inset portrait mode on
laptop.
...
![Camera preview on laptop is upright but app UI is sideways.](/static/images/training/camera/camera2/camera-preview/laptop_inset_portrait_mode_fixed-orientation_app.png)
...
![App rotated to portrait orientation and letterboxed. Image is
            sideways, top to the right.](/static/images/training/camera/camera2/camera-preview/laptop_inset_portrait_mode_rotate_ui.png)

The camera image is rotated 90 degrees to adjust for the reorientation of the
app:

![Sensor image rotated 90 degrees to make it upright.](/static/images/training/camera/camera2/camera-preview/laptop_inset_portrait_mode_rotate_sensor_image.png)

The image is cropped to the aspect ratio of the camera preview, then scaled to
fill the preview (field of view is reduced):

![Cropped camera image scaled to fill camera preview.](/static/images/training/camera/camera2/camera-preview/laptop_inset_portrait_mode_crop_and_scale.png)
...
![Camera preview and app UI turned sideways of wide, unfolded display.](/static/images/training/camera/camera2/camera-preview/unfolded_phone_camera_preview_fixed-orientation_app.png)


**Figure 10.** Unfolded device with portrait-only camera app and
different aspect ratios of camera sensor and display.
...
![Letterboxed app in portait orientation with camera preview
            upright on foldable device.](/static/images/training/camera/camera2/camera-preview/unfolded_phone_inset_portrait_mode.png)

### API

As of Android 12 (API level 31) apps can also explicitly control inset portrait
mode by means of the
[`SCALER_ROTATE_AND_CROP`](/reference/android/hardware/camera2/CaptureRequest#SCALER_ROTATE_AND_CROP)
property of the [`CaptureRequest`](/reference/android/hardware/camera2/CaptureRequest)
class.

The default value is
[`SCALER_ROTATE_AND_CROP_AUTO`](/reference/android/hardware/camera2/CameraMetadata#SCALER_ROTATE_AND_CROP_AUTO),
which enables the system to invoke inset portrait mode.
[`SCALER_ROTATE_AND_CROP_90`](/reference/android/hardware/camera2/CameraMetadata#SCALER_ROTATE_AND_CROP_90)
...
[`CameraCharacteristics#SCALER_AVAILABLE_ROTATE_AND_CROP_MODES`](/reference/android/hardware/camera2/CameraCharacteristics#SCALER_AVAILABLE_ROTATE_AND_CROP_MODES).

**Note:** Only devices with a camera hardware abstraction layer
([HAL](https://source.android.com/devices/camera/camera3)) that supports the
`SCALER_ROTATE_AND_CROP` API can enable inset portrait mode.

## CameraX

**Note:** Jetpack CameraX provides backward compatibility to Android 5.0 (API
level 21).
...
The [`PreviewView`](/reference/androidx/camera/view/PreviewView) layout element
...
the [CameraViewfinder](/reference/androidx/camera/viewfinder/package-summary)
...
[`CameraViewfinder`](/reference/androidx/camera/viewfinder/CameraViewfinder)
...
create a [`ViewfinderSurfaceRequest`](/reference/androidx/camera/viewfinder/ViewfinderSurfaceRequest).
...
Calling [`requestSurfaceAsync()`](/reference/androidx/camera/viewfinder/CameraViewfinder#requestSurfaceAsync(androidx.camera.viewfinder.ViewfinderSurfaceRequest))
...
Calling [`markSurfaceSafeToRelease()`](/reference/kotlin/androidx/camera/viewfinder/ViewfinderSurfaceRequest#markSurfaceSafeToRelease())
...
[`SurfaceView`](/reference/android/view/SurfaceView) is a
...
[`onMeasure()`](/reference/android/view/SurfaceView#onMeasure(int,%20int))
method:

(The `computeRelativeRotation()` source code is in
[Relative rotation](#relative-rotation) below.)
...
[`TextureView`](/reference/android/view/TextureView) is less performant than
...
[Matrix](/reference/android/graphics/Matrix) transformation. To learn how to
correctly scale and rotate a `TextureView`, see
[Support resizable surfaces in your camera app](/codelabs/android-camera2-preview)
...
and
[`Surface`](/reference/android/view/Surface) classes enable calculation of the
...
[`WindowMetricsCalculator#computeCurrentWindowMetrics()`](/reference/androidx/window/layout/WindowMetricsCalculator#computeCurrentWindowMetrics(android.app.Activity))
and
[`WindowInfoTracker#currentWindowMetrics()`](/reference/kotlin/androidx/window/layout/WindowInfoTracker)
provide similar support with backward compatibility to API level 14.

**Note:** [`Display`](/reference/android/view/Display) methods
[`getRealSize()`](/reference/android/view/Display#getRealSize(android.graphics.Point))
and
[`getRealMetrics()`](/reference/android/view/Display#getRealMetrics(android.util.DisplayMetrics))
are deprecated as of API level 31. The methods do not support multi-window
mode or free-form mode. To get the display density, use
[`Configuration#densityDpi`](/reference/android/content/res/Configuration#densityDpi)
instead of `getRealMetrics()`.
...
[`DisplayListener`](/reference/android/hardware/display/DisplayManager.DisplayListener)
and check the device rotation with a call to
[`Display#getRotation()`](/reference/android/view/Display#getRotation())
in the
[`onDisplayChanged()`](/reference/android/hardware/display/DisplayManager.DisplayListener#onDisplayChanged(int))
callback.
...
[`onDisconnected()`](/reference/android/hardware/camera2/CameraDevice.StateCallback#onDisconnected(android.hardware.camera2.CameraDevice))
...
doesn't guarantee access to exclusive resources because another activity or an
application running on a secondary display might have higher priority. Also,
as of Android 12 (API level 31), `resizeableActivity="false"` does not prevent
an activity from entering multi-window mode in which other activities might
take precedence for exclusive resources.
...
* For a Camera2 sample, see the [Camera2Basic app](https://github.com/android/camera-samples/tree/master/Camera2Basic)
  on GitHub.
* To learn about the CameraX preview use case, see CameraX
  [Implement a preview](/training/camerax/preview).
* For a CameraX camera preview sample implementation, see the
  [CameraXBasic](https://github.com/android/camera-samples/tree/master/CameraXBasic)
  repository on GitHub.
* For information about camera preview on ChromeOS, see
  [Camera orientations](https://chromeos.dev/en/android/camera-orientation).
* For information about developing for foldable devices, see
