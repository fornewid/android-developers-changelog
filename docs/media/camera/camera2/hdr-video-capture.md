...
repeating
[`CaptureRequest`](/training/camera2/capture-sessions-requests#repeating-capture-requests)
...
use [`MediaCodec`](/reference/android/media/MediaCodec#createPersistentInputSurface()).

First, get the [`OutputSurface`](/reference/kotlin/androidx/camera/core/impl/OutputSurface),
which maps to a buffer that stores raw video data.
For `MediaCodec`,
use [`createInputSurface()`](/reference/android/media/MediaCodec#createInputSurface()).

To initialize `MediaCodec`, an app must create a
[`MediaFormat`](/reference/android/media/MediaFormat) with a specified
...
using the [HDR capture architecture](#hdr_capture_architecture).
