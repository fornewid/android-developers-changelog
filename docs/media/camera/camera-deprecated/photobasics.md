---
title: Take photos  |  Android media  |  Android Developers
...
# Take photos Stay organized with collections Save and categorize content based on your preferences.
...
[Controlling the camera](/training/camera-deprecated/cameradirect).)
...
camera, put a
[`<uses-feature>`](/guide/topics/manifest/uses-feature-element) tag in
your manifest file:
...
`Intent` delivered to
`onActivityResult()`
as a small `Bitmap` in the extras,
...
[`maxSdkVersion`](/guide/topics/manifest/uses-permission-element#maxSdk)
attribute:
...
eligible paths to be configured in a dedicated resource file, res/xml/file\_paths.xml. Here
