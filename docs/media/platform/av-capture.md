...
To handle the loss of a token, register a callback on the `MediaProjection` instance using the [`registerCallback`](https://developer.android.com/reference/android/media/projection/MediaProjection#registerCallback(android.media.projection.MediaProjection.Callback,%20android.os.Handler)) method, and stop recording when the [`onStop`](https://developer.android.com/reference/android/media/projection/MediaProjection.Callback#onStop()) method is called.

For more information, see [Media projection](https://developer.android.com/media/grow/media-projection).

## Capture video

See the [ScreenCapture sample app](https://github.com/android/media-samples/tree/main/ScreenCapture) to learn how to learn how to use Media Projection API to capture a device's screen in real time and show it on a SurfaceView.

You can use the [`DevicePolicyManager`](https://developer.android.com/reference/android/app/admin/DevicePolicyManager) to prevent screen recording. For enterprise accounts (Android for Work), the administrator can disable the collection of assistant data for the work profile by using the [setScreenCaptureDisabled](https://developer.android.com/reference/android/app/admin/DevicePolicyManager#setScreenCaptureDisabled(android.content.ComponentName,%20boolean)) method.

The codelab [Managing Android Devices Without an App](https://developer.android.com/codelabs/android-management-api#8) shows how to forbid screenshots.

## Capture audio playback

The [AudioPlaybackCapture API](https://developer.android.com/guide/topics/media/playback-capture) was introduced in Android 10. This API gives apps the ability to copy the audio being played by other apps. This feature is the analog of screen capture, but for audio. The primary use case is for streaming apps that want to capture the audio being played by games.
...
To capture audio from another app, your app must build an [`AudioRecord`](https://developer.android.com/reference/android/media/AudioRecord) object and add an [`AudioPlaybackCaptureConfiguration`](https://developer.android.com/reference/android/media/AudioPlaybackCaptureConfiguration) to it. Follow these steps:
...
The player producing the audio must [set its usage](https://developer.android.com/reference/android/media/AudioAttributes.Builder#setUsage(int)) to [`USAGE_MEDIA`](https://developer.android.com/reference/android/media/AudioAttributes#USAGE_MEDIA), [`USAGE_GAME`](https://developer.android.com/reference/android/media/AudioAttributes#USAGE_GAME), or [`USAGE_UNKNOWN`](https://developer.android.com/reference/android/media/AudioAttributes#USAGE_UNKNOWN).

#### Capture policy

The player's capture policy must be [`AudioAttributes.ALLOW_CAPTURE_BY_ALL`](https://developer.android.com/reference/android/media/AudioAttributes#ALLOW_CAPTURE_BY_ALL), which allows other apps to capture playback. This can be done in a number of ways:
