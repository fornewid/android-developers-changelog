---
title: Get started with Media Player  |  Android media  |  Android Developers
...
* [`MediaPlayer`](/reference/android/media/MediaPlayer): This class is the primary API for playing sound and
  video.
* [`AudioManager`](/reference/android/media/AudioManager): This class manages audio sources and audio output on a
  device.
...
**Note:** If you're passing a URL to stream an online media file, the file must be
capable of progressive download.**Caution:** You must either catch or pass [`IllegalArgumentException`](/reference/java/lang/IllegalArgumentException) and
[`IOException`](/reference/java/io/IOException) when using [`setDataSource()`](/reference/android/media/MediaPlayer#setDataSource(android.content.Context,%20android.net.Uri)) because the file you are
referencing might not exist.
