---
title: The Player Interface  |  Android media  |  Android Developers
...
# The Player Interface Stay organized with collections Save and categorize content based on your preferences.
...
* Querying properties of the currently playing media, such as the playback
  position
* Managing a playlist/queue of media items
* Configuring playback properties, such as shuffling, repeating, speed, and
  volume
...
**Tip:** If you're looking to implement a media player app using ExoPlayer, check
out the
[Create a basic media player app using Media3 ExoPlayer](/media/implement/playback-app)
guide.
...
| [`RemoteCastPlayer`](/reference/kotlin/androidx/media3/cast/RemoteCastPlayer) | A `Player` implementation for controlling playback on a remote Cast receiver app. |
| [`CastPlayer`](/reference/kotlin/androidx/media3/cast/CastPlayer) | A `Player` implementation for controlling both local and remote Cast playback. |
...
![A diagram showing how Media3 playback components fit into a media app architecture.](/static/guide/topics/media/images/backgroundplayback.png)


**Figure 1**: The `Player` interface plays a key
role in the architecture of Media3.
...
1. Playback state
   * Retrieve with [`getPlaybackState()`](/reference/kotlin/androidx/media3/common/Player#getPlaybackState()).
   * The state value defined by the interface are
     [`STATE_IDLE`](/reference/kotlin/androidx/media3/common/Player#STATE_IDLE()),
     [`STATE_BUFFERING`](/reference/kotlin/androidx/media3/common/Player#STATE_BUFFERING()),
     [`STATE_READY`](/reference/kotlin/androidx/media3/common/Player#STATE_READY()),
     and [`STATE_ENDED`](/reference/kotlin/androidx/media3/common/Player#STATE_ENDED()).
2. Playlist of media items
   * A sequence of `MediaItem` instances for playback.
   * Retrieve with [`getCurrentTimeline()`](/reference/kotlin/androidx/media3/common/Player#getCurrentTimeline())
   * `Player` instances can provide playlist operation methods like
     [adding](/reference/kotlin/androidx/media3/common/Player#addMediaItem(androidx.media3.common.MediaItem)) or [removing](/reference/kotlin/androidx/media3/common/Player#removeMediaItem(int))
     a `MediaItem` and convenience methods like
     [`getCurrentMediaItem()`](/reference/kotlin/androidx/media3/common/Player#getCurrentMediaItem()).
3. Play/pause properties, such as:
   * [`playWhenReady`](/reference/kotlin/androidx/media3/common/Player#getPlayWhenReady()):
     An indication of whether the user wants media to play when possible or remain
     paused
   * [Playback suppression reason](/reference/kotlin/androidx/media3/common/Player#getPlaybackSuppressionReason()):
     An indication of why playback is suppressed, if applicable, even if
     `playWhenReady` is `true`
   * [`isPlaying`](/reference/kotlin/androidx/media3/common/Player#isPlaying()):
     An indication of whether the player is currently playing, which will only be
     `true` if the playback state is `STATE_READY`, `playWhenReady` is `true`, and
     playback is not suppressed
4. Playback position, including:
   * [Current media item index](/reference/kotlin/androidx/media3/common/Player#getCurrentMediaItemIndex()):
     The index of the current `MediaItem` in the playlist.
   * [`isPlayingAd`](/reference/kotlin/androidx/media3/common/Player#isPlayingAd()):
     An indication of whether an inserted ad is playing.
   * [Current playback position](/reference/kotlin/androidx/media3/common/Player#getCurrentPosition()):
     The current playback position within the current `MediaItem` or inserted ad.

In addition, the `Player` interface allows access to the
[available tracks](/reference/kotlin/androidx/media3/common/Player#getCurrentTracks()),
[media metadata](/reference/kotlin/androidx/media3/common/Player#getMediaMetadata()),
[playback speed](/reference/kotlin/androidx/media3/common/Player#getPlaybackParameters()),
[volume](/reference/kotlin/androidx/media3/common/Player#getVolume()) and other
auxiliary properties of the playback.

**Note:** `Player` instances don't have to support all features of the interface.
You can check the
[available commands](/reference/kotlin/androidx/media3/common/Player#isCommandAvailable(int))
to see which feature is supported and which method is allowed to be called.

### Listen for changes

Use a [`Player.Listener`](/reference/kotlin/androidx/media3/common/Player.Listener)
to listen for changes in a `Player`. See the ExoPlayer documentation on
[Player events](/guide/topics/media/exoplayer/listening-to-player-events) for
...
* [Basic playback controls](/guide/topics/media/exoplayer/hello-world#populate-playlist)
  like `play()`, `pause()`, `prepare()` and `stop()`.
* [Playlist operations](/guide/topics/media/exoplayer/playlists) like
  `addMediaItem()` or `removeMediaItem()`.
* [Seeking](/reference/androidx/media3/common/Player#seekTo(long)) to change the
  current item or position.
* Set [repeat modes](/guide/topics/media/exoplayer/playlists#repeat-modes) and
  the [shuffle mode](/guide/topics/media/exoplayer/playlists#shuffle-mode).
* Update
  [track selection preferences](/guide/topics/media/exoplayer/track-selection#modifying-track).
* Set the
  [playback speed](/reference/androidx/media3/common/Player#setPlaybackSpeed(float)).
...
[`SimpleBasePlayer`](/reference/androidx/media3/common/SimpleBasePlayer)
...
* Playback properties, such as whether the player should start playing when the
  playback state is `STATE_READY`, the index of the currently playing media item,
  and the playback position within the current item
...
call [`invalidateState()`](/reference/androidx/media3/common/SimpleBasePlayer#invalidateState()).
...
override the [`handleSeek()`](/reference/androidx/media3/common/SimpleBasePlayer#handleSeek(int,long,int))
method to support operations like [`COMMAND_SEEK_IN_CURRENT_MEDIA_ITEM`](/reference/androidx/media3/common/Player#COMMAND_SEEK_IN_CURRENT_MEDIA_ITEM())
and [`COMMAND_SEEK_TO_NEXT_MEDIA_ITEM`](/reference/androidx/media3/common/Player#COMMAND_SEEK_TO_NEXT_MEDIA_ITEM()).
...
`Player`. See the guide on the [Customization page](/media/media3/exoplayer/customization#player-operations) for
more details.