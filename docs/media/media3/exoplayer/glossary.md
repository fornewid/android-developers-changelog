---
title: Glossary  |  Android media  |  Android Developers
...
:   Adaptive Bitrate. An ABR algorithm is an algorithm that selects between a
    number of [tracks](#track) during playback, where each track presents the same
    media but at different bitrates.

Adaptive streaming
:   In adaptive streaming, multiple [tracks](#track) are available that present
    the same media at different bitrates. The selected track is chosen
    dynamically during playback using an [ABR](#abr) algorithm.

Access unit
:   A data item within a media [container](#container). Generally refers to a
...
:   A media container format such as MP4 and Matroska. Such formats are called
    container formats because they contain one or more [tracks](#track) of
    media, where each track uses a particular [codec](#codec) (for example, AAC
    audio and H.264 video in an MP4 file). Note that some media formats are both
    a container format and a codec (for example, MP3).

DASH
:   Dynamic [Adaptive Streaming](#adaptive-streaming) over HTTP. An industry
...
:   HTTP Live Streaming. Apple's
    [adaptive streaming](#adaptive-streaming) protocol.
...
:   A single audio, video, text, or metadata stream within a piece of media. A
    ...
    languages. In [adaptive streaming](#adaptive-streaming), there are also
...
:   An Android API for playing audio.
    ...
    For more information, see the
    [Javadoc](/reference/android/media/AudioTrack).

CDM
:   Content Decryption Module. A component in the Android platform responsible for
    decrypting [DRM](#drm)-protected content. CDMs are accessed using Android's
    [`MediaDrm`](#mediadrm) API.

    For more information, see the
    [Javadoc](/reference/android/media/MediaDrm).
...
:   An Android API for accessing media [codecs](#codec) (i.e. encoder and decoder
    ...
    For more information, see the
    [Javadoc](/reference/android/media/MediaCodec).
...
Surface
:   See the [Javadoc](/reference/android/view/Surface)
    and the
...
![ExoPlayer architecture overview](/static/guide/topics/media/exoplayer/images/glossary-exoplayer-architecture.png)

![ExoPlayer rendering overview](/static/guide/topics/media/exoplayer/images/glossary-rendering-architecture.png)

BandwidthMeter
:   Component that estimates the network bandwidth, for example by listening to
    data transfers. In [adaptive streaming](#adaptive-streaming), bandwidth
    estimates can be used to select between different bitrate [tracks](#track)
    during playback.

    For more information, see the component
    [Javadoc](/reference/androidx/media3/exoplayer/upstream/BandwidthMeter).
...
:   Component for requesting data (which may be over HTTP, from a local file,
    ...
    For more information, see the component
    [Javadoc](/reference/androidx/media3/datasource/DataSource).
...
:   Component that parses a media [container](#container) format, outputting
    ...
    For more information, see the component
    [Javadoc](/reference/androidx/media3/extractor/Extractor).
...
:   Component that decides when to start and stop loading, and when to start
    ...
    For more information, see the component
    [Javadoc](/reference/androidx/media3/exoplayer/LoadControl).
...
:   Provides high-level information about the structure of media (as a
    ...
    For more information, see the component
    [Javadoc](/reference/androidx/media3/exoplayer/MediaSource).
...
:   Loads a single piece of media (such as an audio file, an ad, content
    ...
    (typically by [`Renderers`](#renderer)). The decisions about which
    [tracks](#track) within the media are loaded and when loading starts and stops
    are made by the [`TrackSelector`](#trackselector) and the
    [`LoadControl`](#loadcontrol) respectively.

    For more information, see the component
    [Javadoc](/reference/androidx/media3/exoplayer/MediaPeriod).
...
:   Component that reads, decodes, and renders media samples. [`Surface`](#surface)
    ...
    For more information, see the component
    [Javadoc](/reference/androidx/media3/exoplayer/Renderer).
...
:   Represents the structure of media, from simple cases like a single media file
    ...
    For more information, see the component
    [Javadoc](/reference/androidx/media3/common/Timeline).
...
:   Group containing one or more representations of the same video, audio, or text
    ...
    For more information, see the component
    [Javadoc](/reference/androidx/media3/common/TrackGroup).
...
:   A selection consisting of a static subset of [tracks](#track) from a
    ...
    For more information, see the component
    [Javadoc](/reference/androidx/media3/common/TrackSelection).
...
:   Selects [tracks](#track) for playback. Given track information for the
    ...
    For more information, see the component
    [Javadoc](/reference/androidx/media3/exoplayer/trackselection/TrackSelector).