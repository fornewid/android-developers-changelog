...
- [Tell the preload manager the index of the currently playing item](https://developer.android.com/media/media3/exoplayer/preloading-media/preloadmanager/manage-play#set-index).
  This information is used by the preload manager to determine how close
  each media item is to the playing item, and dynamically prioritize
  preloading accordingly. When the user navigates to a new item, update that
  index with the preload manager.
...
- Each time its priorities are updated, it queries your app by calling a [`TargetPreloadStatusControl`](https://developer.android.com/reference/androidx/media3/exoplayer/source/preload/TargetPreloadStatusControl) implemented by your app. It calls this to find out *how much* of each media item to load.
...
5. When the user starts playing content, [the app calls the preload manager](https://developer.android.com/media/media3/exoplayer/preloading-media/preloadmanager/manage-play#fetch-play) to request a `MediaSource` for that media item. The app also calls `setCurrentPlayingIndex()` to tell the preload manager which media item is being played, so that the priorities of the media items will be updated.
6. Whenever the preload manager's priorities are updated, it calls the target preload control once again to find out how much of each item to load.
7. When the app closes the carousel, it [releases the preload manager](https://developer.android.com/media/media3/exoplayer/preloading-media/preloadmanager/manage-play#release) to free its resources.