...
* [Fixing "SSLHandshakeException", "CertPathValidatorException" and "ERR\_CERT\_AUTHORITY\_INVALID" errors](#fixing-sslhandshakeexception-certpathvalidatorexception-and-err_cert_authority_invalid-errors)
...
* [Why does ExoPlayer support my content but the ExoPlayer Cast library doesn't?](#why-does-exoplayer-support-my-content-but-the-exoplayer-cast-library-doesnt)
* [Why does content fail to play, but no error is surfaced?](#why-does-content-fail-to-play-but-no-error-is-surfaced)
* [How can I get a decoding library to load and be used for playback?](#how-can-i-get-a-decoding-library-to-load-and-be-used-for-playback)
...
* [Unstable API lint errors](#unstable-api-lint-errors)
...
[network security documentation](/training/articles/security-config)
...
#### Fixing "SSLHandshakeException", "CertPathValidatorException" and "ERR\_CERT\_AUTHORITY\_INVALID" errors
...
[Android's SSL documentation](/training/articles/security-ssl#CommonProblems)
for more details.
...
1. For exact seeking, a container format will ideally provide a precise
   ...
   specifying this mapping in MP3 (such as XING headers) are, unfortunately, often
   imprecise.
2. For container formats that don't provide a precise time-to-byte mapping (or
...
player. This strategy can be enabled by using [`FLAG_ENABLE_INDEX_SEEKING`](/reference/androidx/media3/extractor/mp3/Mp3Extractor#FLAG_ENABLE_INDEX_SEEKING),
which can be [set on a `DefaultExtractorsFactory`](/guide/topics/media/exoplayer/customization#customizing-extractor-flags) using
[`setMp3ExtractorFlags`](/reference/androidx/media3/extractor/DefaultExtractorsFactory#setMp3ExtractorFlags(int)). Note that it doesn't scale well to large MP3 files,
...
[`FLAG_ENABLE_INDEX_SEEKING`](/reference/androidx/media3/extractor/mp3/Mp3Extractor#FLAG_ENABLE_INDEX_SEEKING) is therefore disabled by default.
...
2. Flush the video decoder and start decoding from the I-frame (keyframe) before
   the new playback position, due to the [intra-frame coding](https://en.wikipedia.org/wiki/Intra-frame_coding) used by most video
   compression formats. In order to ensure the seek is *accurate* (that is,
...
of data buffered in memory by the player, or [pre-caching the data to disk](/guide/topics/media/exoplayer/downloading-media).

The latency introduced by (2) can be mitigated by either reducing the accuracy
of the seek using [`ExoPlayer.setSeekParameters`](/reference/androidx/media3/ExoPlayer#setSeekParameters(androidx.media3.exoplayer.SeekParameters)), or re-encoding the video
...
you can do so using [`FLAG_DETECT_ACCESS_UNITS`](/reference/androidx/media3/extractor/ts/DefaultTsPayloadReaderFactory#FLAG_DETECT_ACCESS_UNITS) and
[`FLAG_ALLOW_NON_IDR_KEYFRAMES`](/reference/androidx/media3/extractor/ts/DefaultTsPayloadReaderFactory#FLAG_ALLOW_NON_IDR_KEYFRAMES) respectively. These flags can be [set on a
`DefaultExtractorsFactory`](/guide/topics/media/exoplayer/customization#customizing-extractor-flags) using [`setTsExtractorFlags`](/reference/androidx/media3/extractor/DefaultExtractorsFactory#setTsExtractorFlags(int)) or on a
`DefaultHlsExtractorFactory` using the
[constructor](/reference/androidx/media3/exoplayer/hls/DefaultHlsExtractorFactory#DefaultHlsExtractorFactory-int-boolean-).
...
try setting [`Mp4Extractor.FLAG_WORKAROUND_IGNORE_EDIT_LISTS`](/reference/androidx/media3/extractor/mp4/Mp4Extractor#FLAG_WORKAROUND_IGNORE_EDIT_LISTS) or
[`FragmentedMp4Extractor.FLAG_WORKAROUND_IGNORE_EDIT_LISTS`](/reference/androidx/media3/extractor/mp4/FragmentedMp4Extractor#FLAG_WORKAROUND_IGNORE_EDIT_LISTS), which will cause
the extractor to ignore edit lists entirely. These can be [set on a
`DefaultExtractorsFactory`](/guide/topics/media/exoplayer/customization#customizing-extractor-flags) using [`setMp4ExtractorFlags`](/reference/androidx/media3/extractor/DefaultExtractorsFactory#setMp4ExtractorFlags(int)) or
[`setFragmentedMp4ExtractorFlags`](/reference/androidx/media3/extractor/DefaultExtractorsFactory#setFragmentedMp4ExtractorFlags(int)).
...
when instantiating [`DefaultHttpDataSource.Factory`](/reference/androidx/media3/datasource/DefaultHttpDataSource.Factory) instances used in your
application. Learn about selecting and configuring the network stack
[here](/guide/topics/media/exoplayer/customization#configuring-the-network-stack).
...
streams, you must depend on the respective [ExoPlayer module](/guide/topics/media/exoplayer/hello-world#add-exoplayer-modules). In cases where
...
using the [`setPlaybackParameters`](/reference/androidx/media3/common/Player#setPlaybackParameters(androidx.media3.common.PlaybackParameters)) API. This is because an optimization
...
See [A note on threading](/guide/topics/media/exoplayer/hello-world#a-note-on-threading) on the getting started page.
...
You can query the player's [`isCurrentWindowLive`](/reference/androidx/media3/common/Player#isCurrentWindowLive()) method. In addition, you
can check [`isCurrentWindowDynamic`](/reference/androidx/media3/common/Player#isCurrentWindowDynamic()) to find out whether the window is dynamic
...
1. You need to have a running [foreground service](/guide/components/services#Foreground). This prevents the system
   from killing your process to free up resources.
2. You need to hold a [`WifiLock`](/reference/android/net/wifi/WifiManager.WifiLock) and a [`WakeLock`](/reference/android/os/PowerManager.WakeLock). These ensure that the
   system keeps the WiFi radio and CPU awake. This can be easily done if using
   [`ExoPlayer`](/reference/androidx/media3/exoplayer/ExoPlayer) by calling [`setWakeMode`](/reference/androidx/media3/exoplayer/ExoPlayer#setWakeMode(int)), which will automatically
...
an [`EventLogger`](/guide/topics/media/exoplayer/debug-logging) as a listener to your player, and looking for a line
...
documentation](/guide/topics/media/media-formats#core) for information about supported sample formats. [How can I get
a decoding library to load and be used for playback?](#how-can-i-get-a-decoding-library-to-load-and-be-used-for-playback) may also be useful.

#### How can I get a decoding library to load and be used for playback?

* Most decoder libraries have manual steps to check out and build the dependencies, so
  make sure you've followed the steps in the README for the relevant library.
  For example, for the ExoPlayer FFmpeg library it's necessary to follow the
  instructions in [libraries/decoder\_ffmpeg/README.md](https://github.com/androidx/media/tree/release/libraries/decoder_ffmpeg/README.md), including passing
  configuration flags to [enable decoders](/guide/topics/media/exoplayer/supported-formats#ffmpeg-library) for any formats you want to play.
* For libraries that have native code, make sure you're using the correct
  ...
  files appear in the `libs` subdirectory of the library's path for each
  supported architecture after following the steps in the README.
* To try out playback using the library in the [demo application](/guide/topics/media/exoplayer/demo-application), see
  [enabling bundled decoders](/guide/topics/media/exoplayer/demo-application#enabling-bundled-decoders). See the README for the library for
  instructions on using the library from your own app.
* If you're using [`DefaultRenderersFactory`](/reference/androidx/media3/exoplayer/DefaultRenderersFactory), you should see an info-level
  log line like "Loaded FfmpegAudioRenderer" in Logcat when the decoder loads.
  If that's missing, make sure the application has a dependency on the
  decoding library.
* If you see warning-level logs from [`LibraryLoader`](/reference/androidx/media3/common/util/LibraryLoader) in Logcat, this
  indicates that loading the native component of the library failed. If this
  happens, check you've followed the steps in the library's README correctly
...
[enabling asynchronous buffer queueing](/guide/topics/media/exoplayer/customization#enabling-asynchronous-buffer-queueing).
...
[`@UnstableApi`](/reference/androidx/media3/common/util/UnstableApi). In order to make this distinction clear, usages of unstable
...
* Continue using the unstable API and annotate the usage with `@OptIn`, as
  shown later.
...
![Screenshot: How to add the Optin annotation](/static/media/media3/exoplayer/images/unstable-api.png)


**Figure 2**: Adding an @androidx.annotations.OptIn annotation with Android Studio.
...
[`lint.xml` file](/studio/write/lint#pref):
