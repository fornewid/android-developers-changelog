---
title: Common Media Client Data (CMCD)  |  Android media  |  Android Developers
...
# Common Media Client Data (CMCD) Stay organized with collections Save and categorize content based on your preferences.
...
* **CMCD-Session**: keys whose values are expected to be invariant over the
  life of the session.
...
**Note:** When using the default configuration, data is transmitted as custom HTTP
request headers.
...
**Note:** When utilising a custom configuration, you have the option to transmit
data as custom request headers (`MODE_REQUEST_HEADER`) or as query parameters
