...
* `MediaSource` instances that define media to be played, load the media, and
  from which the loaded media can be read. A `MediaSource` instance is created
  from a `MediaItem` by a `MediaSource.Factory` inside the player. They can also
  be passed directly to the player using the [media source based playlist API](/guide/topics/media/exoplayer/media-sources#media-source-based-playlist-api).
* A `MediaSource.Factory` instances that converts a `MediaItem` to a `MediaSource`. The
  `MediaSource.Factory` is injected when the player is created.
* `Renderer` instances that render individual components of the media. These are
  injected when the player is created.
* A `TrackSelector` that selects tracks provided by the `MediaSource` to be
  consumed by each available `Renderer`. A `TrackSelector` is injected
...
We have a page about [customizing the network stack used by ExoPlayer](/guide/topics/media/exoplayer/network-stacks).

### Caching data loaded from the network

See the guides for
[temporary on-the-fly caching](/media/media3/exoplayer/network-stacks#caching)
and [downloading media](/media/media3/exoplayer/downloading-media).
...
Implementing a custom [`LoadErrorHandlingPolicy`](/reference/androidx/media3/exoplayer/upstream/LoadErrorHandlingPolicy) allows apps to customize the
...
pipeline, which operates `MediaCodec` instances in [asynchronous mode](/reference/android/media/MediaCodec#asynchronous-processing-using-buffers) and
...
* `MediaSource.Factory` – Implementing a custom `MediaSource.Factory`
  allows an application to customize the way in which a `MediaSource` is created
  from a `MediaItem`.
