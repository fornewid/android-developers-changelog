---
title: Extend your media app to Wear OS  |  Android media  |  Android Developers
...
# Extend your media app to Wear OS Stay organized with collections Save and categorize content based on your preferences.
...
[notifications](/develop/ui/views/notifications) might be familiar to you.
...
[principles](/training/wearables/principles) before you start building your
Wear OS app.
...
review the [feature table](/training/wearables/wear-v-mobile).
...
[Working with downloaded content](/training/wearables/principles#downloaded-content).
...
![A list of audio libraries includes ](/static/wear/images/design/media_apps_5.png)
![A list of audio libraries, including a playlist and an audio book](/static/wear/images/design/media_apps_3.png)

## Build with Compose for Wear OS

[Compose for Wear OS](/training/wearables/compose) is part of Android Jetpack,
...
Wear OS apps](/training/wearables/user-interfaces#apps).
...
see [Why Compose](/jetpack/compose/why-adopt). If you are unfamiliar with using
the Jetpack Compose toolkit, see the
[Compose pathway](/courses/jetpack-compose/course).
To learn more about Compose for Wear OS, see the
[Compose for Wear OS Pathway](/courses/pathways/wear-compose) and the
[Wear OS samples
...
implemented with [Compose for Wear OS](/training/wearables/compose), to
playback capabilities implemented using [Media3](/guide/topics/media/media3).
...
[audio offload](/guide/topics/media/exoplayer/battery-consumption#audio-playback),
and avoiding the need to ping the network unnecessarily.

[Design media apps](/design/ui/wear/guides/foundations/media-apps) provides the
...
* Built-in speakers are not designed for music playback. Therefore, use
  Bluetooth speakers or headphones.
* Network connectivity is limited or varied, so you should optimize your app
  for varied network conditions and minimize network use.
* Smaller batteries have limited power. The battery is consumed more quickly
  when the device performs audio processing on the main CPU and when the device
  has a poor LTE signal. Add support for audio offload to help conserve power.
* The device offers several UI surfaces for letting users re-engage with your
  app. Showcase your app's capabilities on these surfaces.
...
that decorates the [ExoPlayer](/guide/topics/media/exoplayer) instance and
...
* Optimize the content by choosing a low bitrate for streaming, such as
  48 kbps and codecs such as
  [AAC and MP3](/guide/topics/media/platform/supported-formats#audio-formats).
* Optimize the [prefetch strategy](/develop/connectivity/network-ops/network-access-optimization#prefetch-data)
  for images and tracks to
  allow for continuing playback when you lose a connection temporarily.
* Test your app in all network configurations: Wi-Fi, LTE, and watch connected
  to the phone using Bluetooth. Also test what happens when the watch switches
  between networks.
...
use the [`AudioManager.isOffloadedPlaybackSupported()`](/reference/android/media/AudioManager#isOffloadedPlaybackSupported(android.media.AudioFormat,%20android.media.AudioAttributes)) method.
...
[AndroidX WorkManager](/topic/libraries/architecture/workmanager) to schedule
downloads.
...
within the Wear OS user interface. When the [ongoing activity](/training/wearables/notifications/ongoing-activity#ongoing-not)
notification is tapped, the app opens the player screen.

With Media3, Wear OS automatically takes care of creating
[ongoing activities](/training/wearables/notifications/ongoing-activity)
