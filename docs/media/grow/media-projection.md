---
title: Media projection  |  Android media  |  Android Developers
...
# Media projection Stay organized with collections Save and categorize content based on your preferences.




The [`android.media.projection`](/reference/kotlin/android/media/projection/package-summary) APIs introduced in Android 5 (API level 21)
...
a [`Surface`](/reference/kotlin/android/view/Surface).

![Real device display projected onto virtual display. Contents of
              virtual display written to application-provided `Surface`.](/static/media/images/grow/media-projection.png)


**Figure 1.** Real device screen or app window projected onto
virtual display. Virtual display written to application-provided
`Surface`.

The application provides the `Surface` by means of a [`MediaRecorder`](/reference/kotlin/android/media/MediaRecorder),
[`SurfaceTexture`](/reference/kotlin/android/graphics/SurfaceTexture), or [`ImageReader`](/reference/kotlin/android/media/ImageReader), which consumes the contents of the
...
is represented by an instance of the [`MediaProjection`](/reference/kotlin/android/media/projection/MediaProjection) class.

Use the [`getMediaProjection()`](/reference/kotlin/android/media/projection/MediaProjectionManager#getmediaprojection) method of the [`MediaProjectionManager`](/reference/kotlin/android/media/projection/MediaProjectionManager)
...
[`createScreenCaptureIntent()`](/reference/kotlin/android/media/projection/MediaProjectionManager#createscreencaptureintent) method to specify a screen capture operation:
...
**Note:** Apps that use the [`android.media.projection`](/reference/kotlin/android/media/projection/package-summary) APIs are capable of app
screen sharing automatically through the call to `createScreenCaptureIntent()`.

### Virtual display

The centerpiece of a media projection is the virtual display, which you create
by calling [`createVirtualDisplay()`](/reference/kotlin/android/media/projection/MediaProjection#createvirtualdisplay) on a `MediaProjection` instance:
...
display. To obtain values for width and height, use the [`WindowMetrics`](/reference/kotlin/android/view/WindowMetrics) APIs
introduced in Android 11 (API level 30). (For details, see the [Media projection
size](#media_projection_size) section.)

**Note:** The screen density parameter of the `createVirtualDisplay()` method
enables adjustment of the resolution of the captured display for sharing or
casting to a lower resolution external display. Use
[`Configuration#densityDpi`](/reference/kotlin/android/content/res/Configuration#densitydpi) rather than [`Display#getRealMetrics()`](/reference/kotlin/android/view/Display#getrealmetrics) (which
...
permission declaration for the [`mediaProjection`](/about/versions/14/changes/fgs-types-required#media-projection)
...
Start the media projection service with a call to [`startForeground()`](/reference/kotlin/androidx/core/app/ServiceCompat#startForeground(android.app.Service,int,android.app.Notification,int)).
...
[`MissingForegroundServiceTypeException`](/reference/kotlin/android/app/MissingForegroundServiceTypeException).
...
[`SecurityException`](/reference/java/lang/SecurityException) if your app does either of the following:

* Passes an `Intent` instance returned from `createScreenCaptureIntent()` to
  `getMediaProjection()` more than once
* Calls `createVirtualDisplay()` more than once on the same `MediaProjection`
  instance
...
Use the platform `WindowManager` [`getMaximumWindowMetrics()`](/reference/kotlin/android/view/WindowManager#getmaximumwindowmetrics) method to
return a [`WindowMetrics`](/reference/kotlin/android/view/WindowMetrics) object for the device screen even if the media
...
[`computeMaximumWindowMetrics()`](/reference/kotlin/androidx/window/layout/WindowMetricsCalculator#computeMaximumWindowMetrics(android.app.Activity)) method from the Jetpack `WindowManager`
library.

Call the `WindowMetrics` [`getBounds()`](/reference/kotlin/android/view/WindowMetrics#getbounds) method to get the width and height of
the device display.
...
information, see the [Customization](#customization) section, which follows).

## Customization

Your app can customize the media projection user experience with the following
[`MediaProjection.Callback`](/reference/kotlin/android/media/projection/MediaProjection.Callback) APIs:

* [`onCapturedContentVisibilityChanged()`](/reference/kotlin/android/media/projection/MediaProjection.Callback#oncapturedcontentvisibilitychanged): Enables the host app (the app
...
* [`onCapturedContentResize()`](/reference/kotlin/android/media/projection/MediaProjection.Callback#oncapturedcontentresize): Enables the host app to change the size of
...
Your app should register the `MediaProjection` [`onStop()`](/reference/kotlin/android/media/projection/MediaProjection.Callback#onstop) callback to be
...
* the user stops the session using the app's UI or the system's [media
  ...
  chip](/media/grow/media-projection#status_bar_chip_auto_stop)
...
throws [`IllegalStateException`](/reference/java/lang/IllegalStateException).
...
[`createScreenCaptureIntent(MediaProjectionConfig)`](/reference/kotlin/android/media/projection/MediaProjectionManager#createscreencaptureintent_1) method with a
[`MediaProjectionConfig`](/reference/kotlin/android/media/projection/MediaProjectionConfig) argument returned from a call to
[`createConfigForDefaultDisplay()`](/reference/kotlin/android/media/projection/MediaProjectionConfig#createconfigfordefaultdisplay).

A call to `createScreenCaptureIntent(MediaProjectionConfig)` with a
`MediaProjectionConfig` argument returned from a call to
[`createConfigForUserChoice()`](/reference/kotlin/android/media/projection/MediaProjectionConfig#createconfigforuserchoice) is the same as the default behavior, that is,
a call to [`createScreenCaptureIntent()`](/reference/kotlin/android/media/projection/MediaProjectionManager#createscreencaptureintent).

**Warning:** Device manufacturers can override your app's opt out of app screen
sharing if the app misuses the opt out to show private user data from an app
window that shouldn't be shared. See [Device compatibility mode](/guide/practices/device-compatibility-mode#override_disable_media_projection_single_app_option).

## Resizable apps

Always make your media projection apps resizable
([`resizeableActivity="true"`](/guide/topics/manifest/application-element#resizeableActivity)). Resizable apps support device configuration
changes and multi‑window mode (see [Multi-window support](/guide/topics/ui/multi-window)).
...
**Caution:** [`Display#getRealSize()`](/reference/kotlin/android/view/Display#getrealsize) and [`Display#getRealMetrics()`](/reference/kotlin/android/view/Display#getrealmetrics) are
deprecated as of API level 31. For non-resizable apps
(`resizeableActivity="false"`), the methods return the bounds of the app window
rather than the bounds of the device display.
...
* Create an instance of [`MediaProjection.Callback`](/reference/kotlin/android/media/projection/MediaProjection.Callback).
* Implement the callback [`onStop()`](/reference/kotlin/android/media/projection/MediaProjection.Callback#onstop) method. The method is called when
...
playback](/guide/topics/media/av-capture).