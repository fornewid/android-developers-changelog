---
title: Get started with the NDK  |  Android NDK  |  Android Developers
...
# Get started with the NDK Stay organized with collections Save and categorize content based on your preferences.
...
2. [Create
   a CMake build script](/studio/projects/configure-cmake#create_script) to tell CMake how to build your native sources
   ...
   * You can skip this step if your existing native library already has a
     ...
     build script.
3. [Link Gradle to
   your native library](/studio/projects/gradle-external-native-builds) by providing a path to your CMake or ndk-build
   ...
   **Note:** If your existing project uses the deprecated
   ...
   ```
4. [Build and run your app](/studio/run) by clicking
   **Run** ![run then run app from the main menu](/static/studio/images/buttons/toolbar-run.png). Gradle adds
