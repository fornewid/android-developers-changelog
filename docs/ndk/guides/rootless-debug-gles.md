---
title: GLES layers  |  Android NDK  |  Android Developers
...
# GLES layers Stay organized with collections Save and categorize content based on your preferences.
...
[Vulkan validation layer](/ndk/guides/graphics/validation-layer) usage.

## Requirements

GLES layers are supported only on GLES versions 2.0+.

**Warning:** When layering is enabled, GLES 1.x exclusive functions continue to route to
GLES 1.x drivers, but functions shared with GLES 2.0+, such as `glGetString()`,
are routed to 2.0+ drivers.
...
[Vulkan loader](/ndk/guides/graphics/validation-layer#ilp) does.
...
* The target app's manifest file includes the following
  [meta-data element](/guide/topics/manifest/meta-data-element) (only applies
...
[EGL Loader initialization](#egl-loader):
