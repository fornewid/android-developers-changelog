---
title: Android SDK version properties  |  Android NDK  |  Android Developers
...
# Android SDK version properties Stay organized with collections Save and categorize content based on your preferences.
...
**Warning:** Your app *might* work on older devices even if your native libraries
are built with a newer `minSdkVersion`. **Do not rely on this behavior.** It is
not guaranteed to work, and may not on other NDK versions, OS versions, or
individual devices.
...
`<NDK>/meta/platforms.json`. This is the lowest API level supported by the NDK.
Do not set your app's `minSdkVersion` lower than this. Play may allow your app
to be installed on older devices, but your NDK code may not work.
