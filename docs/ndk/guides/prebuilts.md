---
title: Use prebuilt libraries  |  Android NDK  |  Android Developers
...
# Use prebuilt libraries Stay organized with collections Save and categorize content based on your preferences.
...
1. Give the module a name. This name does not need to be the same as that of the prebuilt
   library, itself.
2. In the module's [Android.mk](/ndk/guides/android_mk) file, assign to `LOCAL_SRC_FILES` the path to
   ...
   **Note:** You must make sure to select the version of your prebuilt library
   appropriate to your target ABI. For more information on ensuring library
   support for ABIs, see [Select ABIs for prebuilt libraries](#sa).
