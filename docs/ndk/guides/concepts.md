---
title: Concepts  |  Android NDK  |  Android Developers
...
* Manifest: If you are writing an app with no Java component to it, you must
  ...
  native\_activity.h interface](#na) for more detail on how to do this.
...
1. Design your app, deciding which parts to implement in Java, and which parts
   ...
   **Note:** While it is possible to completely avoid Java, you are likely to find
   the Android Java framework useful for tasks including controlling the display
   and UI.
...
* The [native\_activity.h](/ndk/reference/native__activity_8h) header
  defines the native version of the [NativeActivity](/reference/android/app/NativeActivity) class. It contains the
...
* The `android_native_app_glue.h` file defines a static helper library built on
  top of the [native\_activity.h](/ndk/reference/native__activity_8h) interface. It spawns another thread, which
...
### Use the native\_activity.h interface

To implement a native activity with the [native\_activity.h](/ndk/reference/native__activity_8h) interface:
...
2. Declare your native activity in the `AndroidManifest.xml` file.
   ...
   **Note:** You can subclass [NativeActivity](/reference/android/app/NativeActivity). If you do, use the name of the
   subclass instead of [NativeActivity](/reference/android/app/NativeActivity).
...
3. Create a file for your native activity, and implement the function named in
   the [ANativeActivity\_onCreate](/ndk/reference/group___native_activity#ga02791d0d490839055169f39fdc905c5e) variable. The app calls this function when the
