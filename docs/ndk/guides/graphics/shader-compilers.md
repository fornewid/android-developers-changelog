   ...
   libs/
     <stl_version>/
       {all of the abis}
          libshaderc.a
   ```
3. Add the generated includes and libs using [`target_include_directories`](https://cmake.org/cmake/help/v3.12/command/target_include_directories.html?highlight=target_include_directories) and [`target_link_libraries`](https://cmake.org/cmake/help/v3.12/command/target_link_libraries.html?highlight=target_link_lib#target-link-libraries), as you normally do for similar [external
   libraries](https://github.com/android/ndk-samples/blob/master/hello-libs/app/src/main/cpp/CMakeLists.txt). Your app's STL type must match one of the `stl` types specified in `stl_version`. The NDK recommends using `c++_shared` or `c++_static`, although `gnustl_static` and `gnustl_shared` are also supported.
