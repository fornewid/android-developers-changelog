---
title: Vulkan validation layers on Android  |  Android NDK  |  Android Developers
...
# Vulkan validation layers on Android Stay organized with collections Save and categorize content based on your preferences.
...
**Note:** The unified `VK_LAYER_KHRONOS_validation` layer is
also supported on non-mobile devices so you can share the same validation
...
**Note:** Android 9 (API level 28) and higher allows Vulkan to load layers that are
located separately from your app's APK. See [Use external validation
layers](#vl-external) for more information.
...
* Apps targeting Android 11 (API level 30) or higher only: Your target Android
  ...
  [`meta-data`](/guide/topics/manifest/meta-data-element) element:
...
validation layer from your app's local storage](#layers-local-storage). Starting
in Android 10 (API level 29), Vulkan can also load the validation layer [from a
separate APK](#apk-containing-layers). You can choose whichever method you like
...
Bridge (adb)](/studio/command-line/adb), as follows:

1. Use the [`adb push`](/studio/command-line/adb#copyfiles) command to load the
   layer binary into your app’s data storage on the device:

   ```
   $ adb push libVkLayer_khronos_validation.so /data/local/tmp
   ```
2. Use the [`adb shell`](/studio/command-line/adb#shellcommands) and `run-as`
   ...
   ```
3. [Enable the layer](#enable-layers-outside-app).

#### Load a validation layer binary from another APK

You can use `adb` to [install an APK](/studio/command-line/adb#move) that
contains the layer and then [enable the layer](#enable-layers-outside-app).
...
**Note:** You can also enable the `enable_gpu_debug_layers` setting through the
on-device developer options. After you enable developer options, open the
**Settings** app on your test device, navigate to **Developer options >
Debugging** and make sure the option to **Enable GPU debug layers** is
turned on.
