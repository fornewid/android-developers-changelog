...
- **Macro-level health with Android vitals:** For broad, population-level visibility without additional overhead, Google Play Console's Android vitals provides essential metrics like [Memory Usage (Anonymous RSS + swap)](https://developer.android.com/google/play/vitals/memory-usage)and [Bitmap Memory Usage](https://developer.android.com/google/play/vitals/bitmap-memory-usage). This gives you a clear snapshot of memory distribution across different process states (foreground, background, user-perceived services, and cached) and RAM class ranges, helping you spot memory outliers.
...
- **Android Vitals** : [Memory Usage (RSS + swap metric)](https://developer.android.com/google/play/vitals/memory-usage) and [Bitmap Memory Usage](https://developer.android.com/google/play/vitals/bitmap-memory-usage)
