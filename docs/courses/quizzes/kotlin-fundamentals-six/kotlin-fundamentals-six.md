---
title: Kotlin Fundamentals: Architecture components quiz  |  Android Developers
...
8. ## Assume your app includes this string resource Hello %s. Which of the following is the correct syntax for formatting the string, using the data-binding expression?
   ...
   android:text= "@{@string/generic\_name(user.name)}"

   android:text= "@{string/generic\_name(user.name)}"

   android:text= "@{@generic\_name(user.name)}"
...
10. ## The Transformations.map() method provides an easy way to perform data manipulations on the LiveData and returns \_\_\_.
