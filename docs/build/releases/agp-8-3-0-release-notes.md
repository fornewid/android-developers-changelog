---
title: Android Gradle Plugin 8.3.0 (Feb 2024)  |  Android Developers
...
# Android Gradle Plugin 8.3.0 (Feb 2024) Stay organized with collections Save and categorize content based on your preferences.
...
| --- | --- | --- | --- |
| Gradle | 8.4 | 8.4 | To learn more, see [updating Gradle](/build/releases/gradle-plugin?buildsystem=ndk-build#updating-gradle). |
...
[set up and configure Gradle Version Catalogs](/build/dependencies)
or how to
[migrate your build to version catalogs](/studio/build/migrate-to-catalogs).

**Note:** Starting with Android Studio Giraffe Canary 10, new projects use Gradle
Version Catalogs by default. If an existing project uses a catalog in
`gradle/libs.versions.toml`, new modules that are added to that project also
use the catalog.
...
(`Command+Space` on macOS). In addition, you can quickly navigate from
a dependency reference in your app's `build.gradle` file to where it's declared
in the version catalog by pressing `Ctrl+b`
(`Command+b` on macOS).

![Code completion when adding a dependency](/static/studio/images/gradle-version-catalogs-code-completion.png)
...
![Variables from a version catalog in the Project Structure dialog](/static/studio/images/gradle-version-catalogs-variables.png)
...
![Dependencies from a version catalog in the Project Structure dialog](/static/studio/images/gradle-version-catalogs-dependencies.png)
...
* Navigation for KTS build files not supported: navigating to a dependency
  definition in a version catalog by using `Control`+click
  (`Command`+click on macOS) isn't yet supported for build files
...
* "Find usages" functionality not supported: finding usages of a version catalog
  ...
  in KTS or Groovy. That is, using `Control`+click
  (`Command`+click on macOS) on a variable definition in a version
...
in the [Google Play SDK Index](/distribute/sdk-index). You should update any
...
the [outdated version warnings](/build/dependencies#sdk-insights)
...
[upgrading Android Studio might also require you upgrade AGP](/studio/releases#android_gradle_plugin_and_android_studio_compatibility).
...
| **Shrinker (R8)** | |  | | --- | | [Issue #293820078](https://issuetracker.google.com/issues/293820078)  R8 doesn't work after upgrade from AGP 8.0.2 to 8.1.0 | | [Issue #311837855](https://issuetracker.google.com/issues/311837855)  isShrinkResources overoptimizes in 8.3.0-alpha11 to alpha14 | | [Issue #315877832](https://issuetracker.google.com/issues/315877832)  R8 Flurry SDK crash with AGP 8.2.0 | | [Issue #314984596](https://issuetracker.google.com/issues/314984596)  Android - R8 causes subclass of LinearLayoutManager to crash | | [Issue #315186101](https://issuetracker.google.com/issues/315186101)  R8 v8.2.33, "java.lang.VerifyError: Bad type on operand stack" runtime crash after upgrade | | [Issue #316100042](https://issuetracker.google.com/issues/316100042)  [R8 8.3.21] R8 8.3.21 is 1.57MB larger than R8 8.1.56 | | [Issue #316100042](https://issuetracker.google.com/issues/316100042)  [R8 8.3.21] R8 8.3.21 is 1.57MB larger than R8 8.1.56 | | [Issue #318787479](https://issuetracker.google.com/issues/318787479)  class.getInterfaces() return empty | | [Issue #323512667](https://issuetracker.google.com/issues/323512667)  Test SimpleKotlinEnumUnboxingTest fails on kotlin\_dev bot | |