...
visibility in Android 11](/about/versions/11/privacy/package-visibility).
...
[add a
`<queries>` element](/training/basics/intents/package-visibility#package-name) in your app or library’s
Android manifest.
...
[upgrade](/studio/releases/gradle-plugin?buildsystem=ndk-build#updating-plugin)
to one of the following versions:

|  | Minimum version | Default version | Notes |
| --- | --- | --- | --- |
| Gradle | 6.1.1 | 6.1.1 | To learn more, see [updating Gradle](/build/releases/gradle-plugin?buildsystem=ndk-build#updating-gradle). |
| SDK Build Tools | 29.0.2 | 29.0.2 | [Install](/studio/intro/update#sdk-manager) or [configure](/tools/releases/build-tools) SDK Build Tools. |
...
3. In the **Build** window, open the **Build Analyzer** window in one of the
   following ways:
   * After Android Studio finishes building your project, click the **Build
     Analyzer** tab.
   * After Android Studio finishes building your project, click the link on
     the right side of the **Build Output** window.

![](/static/studio/images/build/build-analyzer/plugins-breakdown.png)
...
To learn more, read [identify build speed regressions](/studio/build/build-analyzer).
...
* Optionals (`java.util.Optional`, `java.util.OptionalInt` and
  `java.util.OptionalDouble`) and some other new classes useful with the above
  APIs
...
dependencies {
coreLibraryDesugaring 'com.android.tools:desugar\_jdk\_libs:1.0.4'
}
...
![Feature on feature dependencies](/static/images/app-bundle/feature-on-feature.png)
...
[create
feature modules](/studio/projects/dynamic-delivery/on-demand-delivery)
...
this file using the [`import&endashmodule`](/ndk/guides/android_mk#import-module) command, which searches the paths you
specify using the `import&endashadd&endashpath` property in your ndk-build project. For
...
[CMAKE\_FIND\_ROOT\_PATH](https://cmake.org/cmake/help/latest/variable/CMAKE_FIND_ROOT_PATH.html){: .external} variable. This value will be set automatically by Gradle when
...
instant apps using [Android App Bundles](/guide/app-bundle).
...
Bundles](/topic/google-play-instant/feature-module-migration).
...
processors](/studio/build/optimize-your-build#annotation_processors) to improve
build performance.
...
processors](/studio/build/dependencies#annotation_processor).
...
when running with `&endash&endashno&endashdaemon` and versions of Gradle 6.3 or lower, causing
