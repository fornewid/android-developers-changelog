---
title: Android Gradle Plugin 8.0.0 (April 2023)  |  Android Developers
...
# Android Gradle Plugin 8.0.0 (April 2023) Stay organized with collections Save and categorize content based on your preferences.
...
[Android Studio 2022.2.1 closed issues](/studio/releases/fixed-bugs/studio/2022.2.1#android-studio-flamingo-patch-2-2022.2.1.20).
...
* Previous versions of AGP infer the test namespace from the main namespace, or
  ...
  the same. If the upgrade is blocked, you need to manually change `testNamespace`
  and modify your source code accordingly.
* After you change the test namespace, it's possible that your code compiles
...
| `android.bundletool.includeRepositoriesInDependencyReport` | `true` | When adding SDK dependency information in AABs and APKs is enabled, AGP 8.0 also adds a list of project repositories to this information. To learn more, see [Dependency information for Play Console](/build/dependencies#dependency-info-play). |
| `android.enableArtProfiles` | `true` | Baseline profiles are now always generated. See [Baseline Profiles](/topic/performance/baselineprofiles/overview) for details. |
...
[`SettingsExtension`](/reference/tools/gradle-api/8.0/com/android/build/api/dsl/SettingsExtension).
To learn more, see the [settings plugin documentation](#settings-plugin).

To preview experimental flags, see the
[preview release notes](/studio/preview/features#experimental-build-flags).
...
![Build Analyzer task categories.](/static/studio/images/ba-task-categories.png)
...
If you need to manually [set the JDK version](/studio/intro/studio-config#jdk)
