...
**Note:** The `extractNativeLibs` manifest attribute has been replaced by the `useLegacyPackaging` DSL option. For more information, see the release note [Use the DSL to package compressed native libraries](https://developer.android.com/build/releases/agp-3-6-0-release-notes#compress-native-libs-dsl).
