...
`true` as shown in the following example, and to run lint using
...
missing rules, writing them to a path such as the following:
...
To enable this feature, set `compileOptions` to your chosen
Java version and set `compileSdkVersion` to 30 or higher:
...
## Addition of JNI libraries in a Java resources
folder is not supported

Previously, you could add a JNI library in a Java resources folder, and
register the folder using `android.sourceSets.main.resources.srcDirs
`so that the JNI library would be extracted and added to the final
APK. Starting with AGP 7.0, this is not supported and JNI libraries in a
Java resources folder are ignored. Instead, use the DSL method intended for
JNI libraries, `android.sourceSets.main.jniLibs.srcDirs`. For
