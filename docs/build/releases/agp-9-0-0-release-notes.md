---
title: Android Gradle plugin 9.0.1 (January 2026)  |  Android Studio  |  Android Developers
...
# Android Gradle plugin 9.0.1 (January 2026) Stay organized with collections Save and categorize content based on your preferences.
...
use the [Android Gradle plugin Upgrade Assistant](/build/agp-upgrade-assistant).
...
| --- | --- | --- | --- |
| Gradle | 9.1.0 | 9.1.0 | To learn more, see [updating Gradle](/build/releases/gradle-plugin?buildsystem=ndk-build#updating-gradle). |
...
[new interfaces](/reference/tools/gradle-api) for our DSL and API in order to
...
* **Ensure your project is compatible with [built-in
  Kotlin](#android-gradle-plugin-built-in-kotlin):** The
  `org.jetbrains.kotlin.android` plugin is not compatible with the new DSL.
* **Switch KMP projects to the
  [Android Gradle Library Plugin for KMP](/kotlin/multiplatform/plugin):**
  ...
  **Note:** The new KMP integration does not support using KMP and the Android
  Application plugin in the same Gradle subproject. To migrate, extract
  your Android app to a separate subproject.
* **Update your build files:**
  While the change of interfaces is meant to keep
  the DSL as similar as possible, there might be
  [some small changes](#android-gradle-plugin-changed-dsl).
* **Update your custom build logic to reference the new DSL and API:**
  ...
  [`androidComponents` API](/build/extend-agp#variant-api-artifacts-tasks).
...
| --- | --- | --- |
| `applicationVariants`,  `libraryVariants`,  `testVariants`, and  `unitTestVariants` | Extension points for plugins to add new functionality to AGP. | Replace this with the [`androidComponents.onVariants`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/AndroidComponentsExtension#onVariants(com.android.build.api.variant.VariantSelector,kotlin.Function1)) API, for example:    ``` androidComponents {     onVariants() { variant ->         variant.signingConfig             .enableV1Signing.set(false)     } } ```  There might not be a direct replacement for all previous APIs. [File an issue](/studio/report-bugs) if there is a use case that is not covered by the new variant APIs. |
| `variantFilter` | Allows selected variants to be disabled. | Replace this with the [`androidComponents.beforeVariants`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/AndroidComponentsExtension#beforeVariants(com.android.build.api.variant.VariantSelector,kotlin.Function1))) API, for example:    ``` androidComponents {     beforeVariants(         selector()             .withBuildType("debug")             .withFlavor("color", "blue")     ) { variantBuilder ->         variantBuilder.enable = false     }   } ``` |
| `deviceProvider` and  `testServer` | Registration of custom test environments for running tests against Android devices and emulators. | Switch to [Gradle-managed devices](/studio/test/gradle-managed-devices). |
| `sdkDirectory`,  `ndkDirectory`,  `bootClasspath`,  `adbExecutable`, and  `adbExe` | Using various components of the Android SDK for custom tasks. | Switch to [`androidComponents.sdkComponents`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/SdkComponents). |
...
| `dexOptions` | Obsolete settings related to the `dx` tool, which has been replaced by [`d8`](/tools/d8). None of the settings have had any effect since Android Gradle plugin 7.0. | There is no direct replacement. |
...
| `aidlPackagedList` | AIDL files to package in the AAR to expose it as API for libraries and apps that depend on this library. | This is still exposed on [`LibraryExtension`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/LibraryExtension) but not on the other extension types. |
...
of new and old. The [AGP API reference docs](/reference/tools/gradle-api) show the API
...
[migrate to built-in Kotlin](/build/migrate-to-built-in-kotlin) or [opt out](/build/migrate-to-built-in-kotlin#opt-out-of-built-in-kotlin).

You can also [selectively disable built-in Kotlin support](/build/migrate-to-built-in-kotlin#selectively-disable) for Gradle
subprojects that don't have Kotlin sources.

## Runtime dependency on Kotlin Gradle plugin

To provide [built-in Kotlin](/build/migrate-to-built-in-kotlin) support, Android Gradle plugin 9.0 now has a
...
[opted out of built-in Kotlin](/build/migrate-to-built-in-kotlin#opt-out-of-built-in-kotlin).
...
**Caution:** When you downgrade the KGP version, your project might not be
compatible with future minor releases of AGP 9, and it might not work with test
fixtures.

## IDE support for test fixtures

AGP 9.0 brings full Android Studio IDE support for
[test fixtures](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/TestFixtures).
...
[Publish multiple Android libraries as one with Fused Library](/build/publish-library/fused-library).
...
| Android Gradle plugin 9.0 by default requires consumers of a library to use the same or higher compile SDK version. | Use the same or higher compile SDK when consuming a library. If this is not possible, or you want to give consumers of a library you publish more time to switch, set [`AarMetadata.minCompileSdk`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/AarMetadata#minCompileSdk()) explicitly. |
...
| `android.builtInKotlin` | Enables [built-in Kotlin](#android-gradle-plugin-built-in-kotlin) | `false` → `true` | [Migrate to built-in Kotlin](/build/migrate-to-built-in-kotlin) if you can or [opt out](/build/migrate-to-built-in-kotlin#opt-out-of-built-in-kotlin). |
...
| `android.useAndroidx` | Use [`androidx`](/jetpack/androidx) dependencies by default. | `false` → `true` | Adopt [`androidx`](/jetpack/androidx/migrate) dependencies. |
| `android.default.androidx.test.runner` | Run on-device tests with the [`androidx.test.runner.AndroidJUnitRunner`](/training/testing/instrumented-tests/androidx-test-libraries/runner) class by default, replacing the default of the deprecated [`InstrumentationTestRunner`](/reference/android/test/InstrumentationTestRunner) for    ``` android {   defaultConfig {     testInstrumentationRunner = "..."   } } ``` | `false` → `true` | Adopt [`AndroidJUnitRunner`](/training/testing/instrumented-tests/androidx-test-libraries/runner), or specify your custom `testInstrumentationRunner` explicitly. |
...
| `android.sourceset.disallowProvider` | Disallow passing providers for generated sources using the [`AndroidSourceSet`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/AndroidSourceSet) DSL. | `false` → `true` | Use the [`Sources`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/Sources) API on `androidComponents` to register generated sources. |
...
* **Embedded Wear OS app support**  
  AGP 9.0 removes support for embedding Wear OS apps, which is no longer
  supported in Play. This includes removing the `wearApp` configurations and
  the
  [`AndroidSourceSet.wearAppConfigurationName`](/reference/tools/gradle-api/8.13/com/android/build/api/dsl/AndroidSourceSet#wearAppConfigurationName()) DSL. See
  [Distribute to Wear OS](/distribute/best-practices/launch/distribute-wear) for how to publish your app to Wear OS.
* **`androidDependencies` and `sourceSets` report task**
* **Density split APK support**  
  AGP 9.0 removes support for creating split APKs based on screen density. The
  functionality and the related APIs have been removed. To split APKs based on
  screen density using AGP 9.0 or higher, use [app bundles](/guide/app-bundle).

## Changed DSL

Android Gradle plugin 9.0 has the following breaking DSL changes:

* The parameterization of [`CommonExtension`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/CommonExtension) has been removed.
  ...
  the block methods need to move from [`CommonExtension`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/CommonExtension) to
  [`ApplicationExtension`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/ApplicationExtension), [`LibraryExtension`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/LibraryExtension),
  [`DynamicFeatureExtension`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/DynamicFeatureExtension) and [`TestExtension`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/TestExtension).
...
* [`AndroidSourceSet.jni`](/reference/tools/gradle-api/8.13/com/android/build/api/dsl/AndroidSourceSet#jni(kotlin.Function1)), because it was not functional.
* [`AndroidSourceSet.wearAppConfigurationName`](/reference/tools/gradle-api/8.13/com/android/build/api/dsl/AndroidSourceSet#wearAppConfigurationName()), as it relates to the
  removed embedded Wear OS app support.
* [`BuildType.isRenderscriptDebuggable`](/reference/tools/gradle-api/8.13/com/android/build/api/dsl/BuildType#isRenderscriptDebuggable()), because it was not functional.
* [`DependencyVariantSelection`](/reference/tools/gradle-api/8.13/com/android/build/api/dsl/DependencyVariantSelection). It is replaced By
  [`DependencySelection`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/DependencySelection), which is exposed as
  [`kotlin.android.localDependencySelection`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/KotlinMultiplatformAndroidLibraryExtension#localDependencySelection(kotlin.Function1))
* [`Installation.installOptions(String)`](/reference/tools/gradle-api/8.13/com/android/build/api/dsl/Installation#installOptions(kotlin.String)). It is replaced by the
  mutable property of [`Installation.installOptions`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/Installation#installOptions()).
* The experimental, but never stabilized [`PostProcessing`](/reference/tools/gradle-api/8.13/com/android/build/api/dsl/PostProcessing) block.
* [`ProductFlavor.setDimension`](/reference/tools/gradle-api/8.13/com/android/build/api/dsl/ProductFlavor#setDimension(kotlin.String)), which is replaced by the
  [`dimension`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/ProductFlavor#dimension()) property
* `LanguageSplitOptions`, which was only useful for
  [Google Play Instant](/topic/google-play-instant), which is deprecated.
* [`DensitySplit`](/reference/tools/gradle-api/8.13/com/android/build/api/dsl/DensitySplit), because the feature is no longer supported.
  Replacement is to use [App Bundles](/guide/app-bundle).

## Removed APIs

Android Gradle plugin 9.0 removes:

* [`AndroidComponentsExtension.finalizeDSl`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/AndroidComponentsExtension#finalizeDSl(org.gradle.api.Action)). It is replaced by
  [`finalizeDsl`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/DslLifecycle#finalizeDsl(kotlin.Function1))
* [`Component.transformClassesWith`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/Component#transformClassesWith(java.lang.Class,com.android.build.api.instrumentation.InstrumentationScope,kotlin.Function1)). It is replaced by
  [`Instrumentation.transformClassesWith`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/Instrumentation#transformClassesWith(java.lang.Class,com.android.build.api.instrumentation.InstrumentationScope,kotlin.Function1))
* [`Component.setAsmFramesComputationMode`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/Component#setAsmFramesComputationMode(com.android.build.api.instrumentation.FramesComputationMode)). It is replaced by
  [`Instrumentation.setAsmFramesComputationMode`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/Instrumentation#setAsmFramesComputationMode(com.android.build.api.instrumentation.FramesComputationMode))
* [`ComponentBuilder.enabled`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/ComponentBuilder#enabled()). It is replaced by
  [`ComponentBuilder.enable`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/ComponentBuilder#enable()).
* [`DependenciesInfoBuilder.includedInApk`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/DependenciesInfoBuilder#includedInApk()). Is is replaced by
  [`includeInApk`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/DependenciesInfoBuilder#includeInApk())
* [`DependenciesInfoBuilder.includedInBundle`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/DependenciesInfoBuilder#includedInBundle()). Is is replaced by
  [`includeInBundle`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/DependenciesInfoBuilder#includeInBundle())
* [`GeneratesApk.targetSdkVersion`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/GeneratesApk#targetSdkVersion()). Is is replaced by [`targetSdk`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/GeneratesApk#targetSdk())
* [`Variant.minSdkVersion`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/Variant#minSdkVersion()). Is is replaced by [`minSdk`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/Variant#minSdk())
* [`Variant.maxSdkVersion`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/Variant#maxSdkVersion()). Is is replaced by [`maxSdk`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/Variant#maxSdk())
* [`Variant.targetSdkVersion`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/Variant#targetSdkVersion()). Is is replaced by [`targetSdk`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/Variant#targetSdk())
* [`Variant.unitTest`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/Variant#unitTest()), as it was not applicable to the
  [`com.android.test`](/reference/tools/gradle-api/9.0/com/android/build/gradle/TestPlugin) plugin.
  `unitTest` is available on [`VariantBuilder`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/VariantBuilder) subtypes extending
  [`HasUnitTest`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/HasUnitTest).
* [`VariantBuilder.targetSdk`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/VariantBuilder#targetSdk()) and [`targetSdkPreview`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/VariantBuilder#targetSdkPreview()),
  as they were not meaningful in libraries. Use
  [`GeneratesApkBuilder.targetSdk`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/GeneratesApkBuilder#targetSdk()) or
  [`GeneratesApkBuilder.targetSdkPreview`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/GeneratesApkBuilder#targetSdkPreview())
  instead.
* [`VariantBuilder.enableUnitTest`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/VariantBuilder#enableUnitTest()), as it was not applicable to the
  [`com.android.test`](/reference/tools/gradle-api/9.0/com/android/build/gradle/TestPlugin) plugin.
  `enableUnitTest` is available on [`VariantBuilder`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/VariantBuilder) subtypes extending
  [`HasUnitTestBuilder`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/HasUnitTestBuilder).
* [`VariantBuilder.unitTestEnabled`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/VariantBuilder#unitTestEnabled()) is removed in favor of the more
  consistently
  named [`enableUnitTest`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/HasUnitTestBuilder#enableUnitTest()) on the [`VariantBuilder`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/VariantBuilder) subtypes
  extending [`HasUnitTestBuilder`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/HasUnitTestBuilder).
* [`VariantOutput.enable`](/reference/tools/gradle-api/8.13/com/android/build/api/variant/VariantOutput#enable()). Is is replaced by [`enabled`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/VariantOutput#enabled())
...
The [Android Gradle plugin Upgrade Assistant](/build/agp-upgrade-assistant) won't upgrade projects to
...
* `remove_message` rewrites each check method call to a call to
  `getClass()` on the first argument of the call (effectively keeping the null
  check, but without any message).
...
1. Compile a library with `default`/`static`/`private` interface methods to
   DEX with `minSdk` < 24 and rules that keep the interface methods.
...
1. Desugar the library with `default`/`static`/`private` interface methods to
   class files with `minSdk` < 24.
2. Compile the desugared artifact using R8 and rules that keep the interface
   methods on the companion classes.
3. Compile the app with the library on classpath.
4. Merge the app and
   the desugared artifact.
...
[automated retracing of stack traces in Logcat](/studio/preview/features#logcat-retrace).
