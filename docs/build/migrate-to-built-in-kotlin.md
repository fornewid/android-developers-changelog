---
title: Migrate to built-in Kotlin  |  Android Studio  |  Android Developers
...
**Note:** Built-in Kotlin replaces the `kotlin-android` plugin only.
If you are writing a Kotlin Multiplatform (KMP) library module, you still need
to apply the `org.jetbrains.kotlin.multiplatform` plugin and
the [`com.android.kotlin.multiplatform.library`](/kotlin/multiplatform/plugin) plugin.
Also, using the `org.jetbrains.kotlin.multiplatform` plugin together with the
`com.android.library` or `com.android.application` plugin is no longer allowed
when built-in Kotlin is enabled.
...
[opted out of built-in Kotlin](#opt-out-of-built-in-kotlin) by setting `android.builtInKotlin=false`
in the `gradle.properties` file, you need to remove that setting or set it to
`true`.

**Note:** You can also enable built-in Kotlin for [one module at a time](#module-by-module-migration).
...
you manually [enable built-in Kotlin](#enable-built-in-kotlin), you might see the following error
message:
...
**Note:** If you're not yet ready to migrate your project, you can also
[opt out of built-in Kotlin](#opt-out-of-built-in-kotlin).

1. [Remove the `kotlin-android` plugin](#migration-steps-remove-kotlin-android-plugin)
2. [Migrate the `kotlin-kapt` plugin if necessary](#migration-steps-migrate-kotlin-kapt-plugin)
3. [Migrate the `android.kotlinOptions{}` DSL if necessary](#migration-steps-migrate-kotlin-options)
4. [Migrate the `kotlin.sourceSets{}` DSL if necessary](#migration-steps-migrate-kotlin-source-sets)
...
[migrate your project to KSP](/build/migrate-to-ksp).
...
**Note:** If you declare the `kotlin-kapt` plugin in the `plugins{}` block
as `kotlin("kapt") version "<KOTLIN_VERSION>"`, then remove that line instead.
...
**Note:** With built-in Kotlin, you don't need to set
...
source directories using either the [`android.sourceSets{}`](/reference/tools/gradle-api/9.0/com/android/build/api/dsl/AndroidSourceSet) DSL or the
...
**Note:** You also need to set `android.newDsl=false` to opt out of the
[new DSL](/r/tools/new-dsl) because the `kotlin-android` plugin is not compatible with it.

Once you're ready to migrate your project, [enable built-in Kotlin](#enable-built-in-kotlin)
and follow the [migration steps](#migration-steps).
...
3. Follow the previous [migration steps](#migration-steps) to migrate this module to
   built-in Kotlin.
