---
title: Use the Android Gradle plugin Upgrade Assistant  |  Android Studio  |  Android Developers
...
* **Configure the build using the Gradle build files**: The Upgrade Assistant
  ...
  Upgrade Assistant, configure your build using these build files. Use
  [Gradle Version Catalogs](https://docs.gradle.org/current/userguide/platforms.html)
  to refer to dependencies and plugins defined in a `libs.versions.toml` file in
  the `gradle` subdirectory of your project.

**Note:** The Upgrade Assistant does not support projects that refer to constants
or variables defined in the `buildSrc` directory, projects that use
Gradle Version Catalogs defined in settings files, or projects that refer to
Gradle Version Catalogs for things other than dependencies and plugins. In
general, we don't recommended using `buildSrc` to structure your project,
because it can be inefficient. Any change to `buildSrc` constants and variables,
no matter how minor, triggers a full rebuild of the project.

* **Use the declarative build domain-specific language**: Gradle build files are
  expressed in Groovy or Kotlin. However, the more declarative the expression of
  the project configuration, the more likely that the Upgrade Assistant
  successfully finds all the places that need adjustment for an upgrade.
...
1. To launch the Upgrade Assistant, navigate to
   **Tools > AGP Upgrade Assistant** or click on the notification prompt, as
   ...
   ![The AGP Upgrade Assistant tool window when launched, and the notification prompt.](/static/studio/images/build/build-analyzer/agp-upgrade-assistant.png)
