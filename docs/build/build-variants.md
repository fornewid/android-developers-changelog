---
title: Configure build variants  |  Android Studio  |  Android Developers
...
true`](/guide/topics/manifest/application-element#debug). This lets you debug the app on secure Android devices and
...
or change certain settings. The following sample specifies an [`applicationIdSuffix`](/reference/tools/gradle-api/9.1/com/android/build/api/dsl/ApplicationVariantDimension#applicationIdSuffix()) for the debug build type and configures
...
**Sync Project** ![](/static/studio/images/buttons/toolbar-sync-gradle.png) from the toolbar. If Android
...
read the [`BuildType`](/reference/tools/gradle-api/current/com/android/build/api/dsl/BuildType) reference.
...
[`ProductFlavor`](/reference/tools/gradle-api/current/com/android/build/api/dsl/ProductFlavor) class. This means you can provide the base
...
matching](/studio/build/build-variants#variant_aware) relies on how you name a flavor dimension, name your flavor dimensions
...
[`applicationIdSuffix`](/studio/build/build-variants#change-app-id) and
`versionNameSuffix`:
...
support](/google/play/publishing/multiple-apks) in Google Play, assign the same `applicationId` value
to all variants and give each variant a different
[`versionCode`](/studio/publish/versioning#appversioning). To distribute
...
sets](#sourcesets), as described on this page.
...
[build variants](/studio/build/build-variants) that each have a different
application ID.

In this case, define each build variant as a separate
[product flavor](/studio/build/build-variants#product-flavors). For each flavor
...
your [build type](/studio/build/build-variants#build-types), as shown here:
...
[Multiple APK support](/google/play/publishing/multiple-apks). Publishing
...
variables into the manifest](/studio/build/manage-manifests#inject_build_variables_into_the_manifest).
...
[`flavorDimensions`](/reference/tools/gradle-api/current/com/android/build/api/dsl/ProductFlavor#dimension) property to create a "mode" flavor
...
or **Build Variants** ![](/static/studio/images/buttons/window-monitor.png) in
the tool window bar.

## Create source sets

By default, Android Studio creates the `main/`
[source set](/studio/build#sourcesets) and directories for
...
dimensions](#flavor-dimensions)), and build variants.
...
[testing source sets](/studio/test/advanced-test-setup#create-instrumented-test-for-build-variant).
...
[creating source sets](#sourcesets), you can use the
[`sourceSets`](/reference/tools/gradle-api/current/com/android/build/api/dsl/AndroidSourceSet) block to change where Gradle looks to gather
...
see the [Android Gradle plugin API reference](/reference/tools/gradle-api).
...
flavors](#flavor-dimensions), priority between the product flavors is determined by the flavor
dimension they belong to. When listing flavor dimensions with the
[`android.flavorDimensions`](/reference/tools/gradle-api/current/com/android/build/api/dsl/ProductFlavor#dimension) property, product flavors that
...
* Manifests are merged together into a single manifest. Priority is given
  ...
  more, read about [manifest merging](/studio/build/manage-manifests#merge-manifests).
...
[testing source set](/studio/test/advanced-test-setup#create-instrumented-test-for-build-variant),
...
see [Add build dependencies](/studio/build/dependencies).
...
[provide matching fallbacks](#resolve_matching_errors) as described in
...
For more information, see [`matchingFallbacks`](/reference/tools/gradle-api/7.0/com/android/build/api/dsl/ProductFlavor#matchingfallbacks)
and [`missingDimensionStrategy`](/reference/tools/gradle-api/7.0/com/android/build/api/dsl/BaseFlavor#missingdimensionstrategy_1)
...
[generate an upload key and keystore](/studio/publish/app-signing#generate-key)
...
3. Add the signing configuration to the module-level `build.gradle.kts`
   file:
...
developer. To learn more, read [Remove signing information from your build files](/studio/publish/app-signing#secure-shared-keystore).
...
[Package and distribute Wear apps](/training/wearables/packaging).