---
title: Migrate from kapt to KSP  |  Android Studio  |  Android Developers
...
**Note:** If a module has any kapt processors remaining, stubs are still generated
in that module. This means that the majority of performance improvement will
occur only when all usages of kapt are removed from the module.
...
**Note:** While not a traditionally-included library dependency, [Data Binding](/topic/libraries/data-binding)
also uses an annotation processor to provide its functionality, and [KSP support
for Data Binding is not planned](https://issuetracker.google.com/issues/173030256#comment10). You can mitigate the impact of
kapt on your build by isolating the usages of Data Binding to separate modules.
...
**Note:** Some libraries (such as [Glide](https://bumptech.github.io/glide/doc/download-setup.html#kotlin---ksp)) might require you to
change the dependency to a different artifact as well. Make sure to consult
their documentation.
...
**Note:** [Data Binding](/topic/libraries/data-binding) also requires kapt to be enabled in the module. In
modules where Data Binding is used, kapt can't be removed.
