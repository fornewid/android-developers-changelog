---
title: Optimize your build speed  |  Android Studio  |  Android Developers
...
1. [Optimize your build configuration](#optimize) by taking a few
   steps that immediately benefit most Android Studio projects.
2. [Profile your build](/studio/build/profile-your-build) to identify and diagnose some
   of the trickier bottlenecks that may be specific to your project or workstation.
...
such as the [Android
Runtime (ART)](https://source.android.com/devices/tech/dalvik/) and native support for [multiple DEX files](/studio/build/multidex).
...
processes annotations (such as [Room](/training/data-storage/room))
that supports KSP, you'll want to [migrate to KSP](/build/migrate-to-ksp).
...
Look for code in your app that you can convert into an [Android library module](/studio/projects/android-library).
...
After you [create a build profile](/studio/build/profile-your-build), if the build
...
[convert your images
to WebP](/studio/write/convert-webp#convert_images_to_webp).

### Disable PNG crunching

If you don't [convert your PNG
images to WebP](/studio/write/convert-webp#convert_images_to_webp), you can still speed up your build by disabling automatic
image compression every time you build your app.

If you're using [Android Gradle plugin 3.0.0](/studio/releases/gradle-plugin#3-0-0)
...
[testing your Gradle builds](/studio/build/profile-your-build) with the parallel
...
[Profile your build](/studio/build/profile-your-build#profiling_different_memorycpu_settings).
**Note:** Be aware of Gradle
[issue #19750](https://github.com/gradle/gradle/issues/19750):
setting the `org.gradle.jvmargs` property can lead to
"Daemon disappeared" failures. Until the bug is fixed you must explicitly set
JVM argument defaults. The key JVM arguments that you must explicitly set are
the `-XX:MaxMetaspaceSize=256m` and `-XX:+HeapDumpOnOutOfMemoryError`
arguments; for the full list of arguments, see the
[`java` command documentation](https://docs.oracle.com/javase/8/docs/technotes/tools/windows/java.html).
...
build time in your
[Build Analyzer](/studio/build/build-analyzer)
...
**Note:** If you change the default memory limit,
you also have to set the `-XX:MaxMetaspaceSize=1g` argument due to
[Gradle issue #19750](https://github.com/gradle/gradle/issues/19750).

If you also use the
[JVM parallel garbage collector](/studio/build/profile-your-build#experiment-gc),
...
Use non-transitive [`R` classes](/reference/android/R) to have faster builds
...
Use non-constant [`R` class](/reference/android/R)
...
[Jetifier](/studio/command-line/jetifier) flag for better build performance. To remove
...
[Troubleshoot build performance](/studio/build/build-analyzer).
...
Use the
[Build Analyzer](/studio/build/build-analyzer#warnings-types) to check whether your
...
a list of plugins that are supported.- Add the following code to the `gradle.properties` file:
...
Input | Issue Tracker | Fixed in || $GRADLE\_USER\_HOME/android/FakeDependency.jar | [Issue #289232054](https://issuetracker.google.com/289232054) | AGP 8.2 |
