---
title: Build your app from the command line  |  Android Studio  |  Android Developers
...
tools, see [Build and run your app](/studio/run).
...
see [Configure Build Variants](/studio/build/build-variants).

## Build and deploy an APK

Although [building an app bundle](#build_bundle) is the best way to
...
allows debugging through [`adb`](/studio/command-line/adb).
...
[`zipalign`](/tools/help/zipalign), so you can
...
camel-case version of the [build variant](/studio/build/build-variants) name, so it can be replaced with whichever build type or
...
2. Now you can install your app using either one of the Gradle install tasks
   mentioned in the section about how to [build a debug APK](#DebugMode)
   or the
   `adb` tool.
...
in the section about how to [build a debug APK](#DebugMode) or the
`adb` tool:
...
[Android App Bundles](/guide/app-bundle) include all your app’s
...
else, you should instead [build an APK](#build_apk).

The easiest way to build an app bundle is by
[using Android Studio](/studio/run#reference). However, if you need to build an
...
[Configure Gradle to sign your app](#gradle_signing).
...
[test app bundles with `bundletool`](/studio/command-line/bundletool) and locally recreate
...
[Google Maven repository](/studio/build/dependencies#google-maven).

**Caution:** Do not use the version of AAPT2 that is included in the Android build
tools package—that version of AAPT2 does not support `bundletool`.

To download AAPT2 from Google's Maven repository, proceed as follows:

1. Navigate to **com.android.tools.build > aapt2** in the
   [repository index](https://maven.google.com/).
...
**Note:** Although you can pass resource directories to AAPT2 using the `--dir` flag,
doing so recompiles all files in the directory, regardless of how many files
actually changed.
...
**Note:** Alternatively, when specifying compiled resources with the `-R` flag, you
can specify a text file that includes the absolute path for each of
your compiled resources—with each path separated by a single space. You can
then pass that text file to AAPT2 as follows:
`aapt2 link ... -R @compiled_resources.txt`.
...
similar to that of [the Android App Bundle format](/guide/app-bundle#aab_format).
...
**Note:** If you plan to publish the app bundle, you need to sign it using
[`jarsigner`](https://docs.oracle.com/javase/8/docs/technotes/tools/windows/jarsigner.html).
You can not use `apksigner` to sign your app bundle.
...
**Note:** To learn more about how the JSON maps to the protobuf format, read
[JSON Mapping](https://developers.google.com/protocol-buffers/docs/proto3#json).
However, that information is more advanced than what is required for this page.
...
[use `bundletool`](/studio/command-line/bundletool) to generate APKs and deploy them to
a device.
...
3. Sign your APK with your private key using [`apksigner`](/studio/command-line/apksigner):
   ...
   signing an APK using multiple signers. For more details, see the [`apksigner`](/studio/command-line/apksigner)
   reference.

   **Note:** To use the `apksigner` tool, you must
   have revision 24.0.3 or higher of the Android SDK Build Tools installed.
   You can update this package using the [SDK Manager](/studio/intro/update#sdk-manager).
...
**Note:** In this case, the keystore and key password are visible
directly in the `build.gradle` file. For improved security,
you should [remove
the signing information from your build file](/studio/publish/app-signing#secure-shared-keystore).
...
your app](/studio/publish) and review the [Google Play launch checklist](/distribute/tools/launch-checklist).