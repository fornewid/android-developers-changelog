---
title: Migrate your build configuration from Groovy to Kotlin  |  Android Studio  |  Android Developers
...
**Note:** Kotlin is the default language for build configuration starting with
Android Studio Giraffe. If you're using AGP 8.1 and the Kotlin DSL for build
configuration, you should use Gradle 8.1 for the best experience.
...
**Kotlin DSL:** Refers primarily to the [Android Gradle plugin Kotlin DSL](/reference/tools/gradle-api)
...
* Gradle build files written in Kotlin use the `.gradle.kts` file name
  extension.
...
**Tip:** As a first step, even before changing the file extensions, add parentheses
to your Groovy code. This makes the conversion to Kotlin easier.
...
**Tip:** Before changing file extensions, add `=` to your Groovy code.
This makes the conversion to Kotlin easier.
...
* **Double quotes for strings:**
  While Groovy allows strings to be defined using single quotes, Kotlin requires
  double quotes.
...
`debug`, `release`, and
[`benchmark`](/topic/performance/benchmarking/macrobenchmark-overview) build
types in Groovy.
...
works well with
[version catalogs](/studio/build/migrate-to-catalogs).
...
1. If you still have repositories for plugins declared in the `buildscript {}`
   block, move them to the [`settings.gradle`](/studio/build#settings-file)
   file instead.
...
* If you have a ProGuard configuration, refer to
  [Enable shrinking, obfuscation, and optimization](/studio/build/shrink-code#enable).
* If you have a `signingConfig {}` block, refer to [Remove signing information from
  your build files](/studio/publish/app-signing#secure-shared-keystore).
* If you use project-wide properties, refer to [Configure project-wide
  properties](/studio/build#project_wide_properties).

**Caution:** Although Gradle lets you define project-wide properties at the module
level, avoid doing so, because it causes the modules that share those properties
to be coupled. Module coupling makes it more difficult to later export a module
as a standalone project and prevents Gradle from using parallel project
execution to speed up multi-module builds.
...
[Details for build tools and Gradle bugs](/studio/report-bugs#build-bugs). Then,
