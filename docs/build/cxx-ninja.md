---
title: Integrate custom C/C++ build systems using Ninja (experimental)  |  Android Studio  |  Android Developers
...
* `ninja.arguments` is a list of arguments that will be passed to the script defined by ninja.configure. Elements in this list can reference a set of macros whose values depend on the current configuration context in AGP:
  ...
  + `${ndk.abi}` is the name of the current AGP ABI that is being built. For example, `x86` or `arm64-v8a`.**Note:** You must pass `${ndk.abi}` to the configure script as one of the arguments, because the same `build.ninja` file cannot be shared by multiple ABIs.
