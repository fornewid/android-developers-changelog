---
title: Keep all classes that implement the specific interface and their constructors  |  Android Developers
url: https://developer.android.com/agents/skills/performance/r8-analyzer/references/REFLECTION-GUIDE
source: html-scrape
---

# Keep all classes that implement the specific interface and their constructors Stay organized with collections Save and categorize content based on your preferences.
...
* **Look for:** `::class.java` (Kotlin) or `.class` (Java) passed as an
  argument.
* **Example Code:**
  `kotlin
...
* **Look for:** `try-catch` blocks around `Class.forName()` used to toggle
  features.
* **Example Code:**
  ```kotlin
  private const val VIDEO\_TRACKER\_CLASS = "com.example.analytics.video.VideoEventTracker"

try {
Class.forName(VIDEO\_TRACKER\_CLASS).getDeclaredConstructor().newInstance()
...
* **Look for:** `getDeclaredField("...")` or `getDeclaredMethod("...")`
  followed by `isAccessible = true`.
* **Example Code:**
  `kotlin
