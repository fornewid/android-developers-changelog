---
title: Inline Install API eligibility criteria for SDKs  |  App quality  |  Android Developers
...
# Inline Install API eligibility criteria for SDKs Stay organized with collections Save and categorize content based on your preferences.
...
we will notify you that you can [integrate the API](/distribute/marketing-tools/inline-installs-sdk).

**Note:** Even if your SDK has successfully passed the review process and been
allowlisted for the Inline Install API, the Inline Install experience may not be
available because of [app eligibility criteria](/quality/core-value/app-eligibility). In that case, the
call redirects to the standard Play Store App deep link page. More information
is provided in the technical onboarding.
...
**Warning:** Ad SDKs must only use the Inline Install API according to the API Terms
of Service. Google Play wants to provide users with contextual and seamless
journeys. If we see that the Inline Install API is inappropriately used, we may
revoke access. It's your responsibility as the SDK provider to ensure that the
implementation of your SDK is aligned with the criteria in the table.
