---
title: Webviews – Unsafe URI Loading  |  Security  |  Android Developers
...
* Checking the host but not the scheme, allowing an attacker to use schemes like
  `http://`, `content://` or `javascript://` with an authenticated host.
* Failing to parse the URI correctly, especially in cases where the URI is
