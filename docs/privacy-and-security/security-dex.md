...
- Set the `android::useEmbeddedDex` attribute to `true` in the `https://developer.android.com/guide/topics/manifest/application-element` element of your app's manifest file.

- Set `useLegacyPackaging` to `false` in the module-level `build.gradle.kts` file (`build.gradle` file if you're using Groovy).
...
If you're using the Bazel build system, to enable this feature set the `android:useEmbeddedDex` attribute to `true` in the `<application>` element of your app's manifest file and leave DEX files uncompressed:
