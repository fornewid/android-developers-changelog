...
# Improve your app's security Stay organized with collections Save and categorize content based on your preferences.
...
* [`android:protectionLevel`](/guide/topics/manifest/permission-element#plevel)
...
[`android:exported`](/guide/topics/manifest/provider-element#exported)
attribute of the
...
authentication](/training/sign-in/biometric-auth).
...
security configuration](/training/articles/security-config)
...
* You can't use a [network security
  configuration](#network-security-config).

To learn more about how to complete these steps, see the discussion about
handling an [unknown
certificate authority](/training/articles/security-ssl#UnknownCa).

**Related info:**

* [Security
  with network protocols](/training/articles/security-ssl)
* `CertificateFactory`
* `HttpsURLConnection`
* `TrustManager`
...
support](/guide/webapps/webview#UsingJavaScript) unless you completely control and trust the content in your app's
`WebView` objects.
...
* [Common Intents](/guide/components/intents-common)
* `Intent`
...
* Provide clients one-time access to data by using the
  ...
  `FLAG_GRANT_WRITE_URI_PERMISSION` flags.
...
external storage](/training/data-storage/app-specific#external).
...
* **Media files:** To store and access images, audio files, and videos that are
  shared between apps, [use the Media Store
  API](/training/data-storage/shared/media).
* **Other files:** To store and access other types of shared files, including
  downloaded files, [use the Storage Access
...
available](/training/data-storage/app-specific#external-verify-availability).
...
When using
`getSharedPreferences()` to
create or access your app's `SharedPreferences` objects,
use `MODE_PRIVATE`. That way, only your app can
...
data securely across apps](#permissions-share-data).
...
provider to protect against SSL exploits](/training/articles/security-gms-provider).
...
**Related info:** 
[Add
build dependencies](/studio/build/dependencies#google_and_android_support_repositories)
...
* [Core app quality
  security checklist](/distribute/essentials/quality/core#sc)
* [App security improvement program](/google/play/asi)
* [Android Developers
