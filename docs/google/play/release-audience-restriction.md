---
title: Use a release audience restriction to limit your app's distribution  |  Other Play guides  |  Android Developers
...
# Use a release audience restriction to limit your app's distribution Stay organized with collections Save and categorize content based on your preferences.
...
**Note:** Because the restriction is built into your app bundle or APK, you can
only change it by compiling a new build of your app with a new versionCode.
...
**Important:** The `<meta-data>` element must be a direct child of the
`<application>` element in order for it to be detected by Google Play. It won't
be detected if it is part of an `<activity>` or `<service>` element. If
there are multiple restrictions in your application manifest, then the most
restrictive will be applied.**Tip:** If you have an Android library that adds some optional features for
testing or internal debugging, you can add the restriction to the library's
manifest. Any build which includes the library will then automatically include
the release audience restriction.