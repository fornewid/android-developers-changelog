---
title: Adding Client-Side License Verification to Your App  |  Other Play guides  |  Android Developers
...
# Adding Client-Side License Verification to Your App Stay organized with collections Save and categorize content based on your preferences.
...
[perform server-side
license verification instead](/google/play/licensing/server-side-verification).

After you've set up a publisher account and development environment (see [Setting Up for Licensing](/google/play/licensing/setting-up)), you are ready to add license verification to
your app with the License Verification Library (LVL).

Adding license verification with the LVL involves these tasks:

1. [Adding the licensing permission](#manifest-permission) your application's manifest.
2. [Implementing a Policy](#impl-Policy) — you can choose one of the full implementations provided in the LVL or create your own.
3. [Implementing an Obfuscator](#impl-Obfuscator), if your `Policy` will cache any
   license response data.
4. [Adding code to check the license](#impl-lc) in your application's main
   Activity.
5. [Implementing a DeviceLimiter](#impl-DeviceLimiter) (optional and not recommended for
   most applications).
...
Environment](/google/play/licensing/setting-up#test-env).

For an overview of the full set of source files included in the LVL, see [Summary of LVL Classes
and Interfaces](/google/play/licensing/licensing-reference#lvl-summary).
...
To request the licensing permission in your application, declare a [`<uses-permission>`](/guide/topics/manifest/uses-permission-element)
...
* `processServerResponse()` lets you preprocess the raw response
  ...
  obfuscated (see [Implementing an Obfuscator](#impl-Obfuscator),
  below).
...
* [ServerManagedPolicy](#ServerManagedPolicy), a flexible `Policy`
  that uses server-provided settings and cached responses to manage access across
  varied network conditions, and
* [StrictPolicy](#StrictPolicy), which doesn't cache any response
...
1. Caches (and properly obfuscates) the most recent successful license response
   in local persistent storage.
2. Returns the cached response for all license checks, for as long as the
   ...
   extra is highly recommended. See [Server Response Extras](/google/play/licensing/licensing-reference#extras)
   for more information.
3. Uses an exponential backoff period, if retrying any requests the result in
...
4. Provides for a "grace period" that allows the user to access your
...
Extras](/google/play/licensing/licensing-reference#extras).
...
includes an example of such a `Policy` — see [StrictPolicy](#StrictPolicy) for more information.
...
LicenseCheckerCallback](#lc-lcc) for more information.
...
[Instantiate LicenseChecker and LicenseCheckerCallback](#lc-lcc)
for more information.
...
3. A device identifier string, derived from as many device-specific sources
...
1. [Add imports](#imports)
2. [Implement LicenseCheckerCallback](#lc-impl) as a private inner class
3. [Create a Handler](#thread-handler) for posting from LicenseCheckerCallback to the UI thread
4. [Instantiate LicenseChecker](#lc-lcc) and LicenseCheckerCallback
5. [Call checkAccess()](#check-access) to initiate the license check
6. [Embed your public key](#account-key) for licensing
7. [Call your LicenseChecker's onDestroy() method](#handler-cleanup) to close IPC connections.
...
* A callback that returns the result of the license check. In the LVL, this is
  ...
  these methods don't determine *whether* to allow access — that
...
![](/static/images/licensing_flow.png)
...
1. Code in the application's main Activity instantiates `LicenseCheckerCallback`
...
2. The code then calls the `checkAccess()` method on the
...
3. When a response is received, `LicenseChecker` creates a LicenseValidator that
...
4. In case of a recoverable local or server error, such as when the network is
   ...
   unavailable.
5. In case of a application error, such as when the application attempts to
...
to provide a [Policy implementation](#impl-Policy) and, if the `Policy`
stores response data (such as ServerManagedPolicy), an [Obfuscator](#impl-Obfuscator) implementation.
...
* Display a "Purchase this application" dialog, including a button that
  ...
  links, see [Linking to Your Products](/distribute/tools/promote/linking).
* Display a Toast notification that indicates that the features of the
  application are limited because it isn't licensed.
...
Response Codes](/google/play/licensing/licensing-reference#server-response-codes) in the [Licensing Reference](/google/play/licensing/licensing-reference). You can implement
...
1. Your `LicenseCheckerCallback` methods will be invoked, in many cases, from a
   background thread.
2. Those methods won't be able to update state or invoke any processing in the
...
* A reference to the `Policy` implementation to use for the license check. In
  most cases, you would use the default `Policy` implementation provided by the LVL,
  ServerManagedPolicy.
* The String variable holding your publisher account's public key for
  licensing.
...
1. Go to the Google [Play Console](http://play.google.com/apps/publish) and sign in.
   Make sure that you sign in to the account from which the application you are
   licensing is published (or will be published).
2. In the application details page, locate the **Services & APIs**
   link and click it.
3. In the **Services & APIs** page, locate the
...
publish the application on Google Play. Follow the normal steps to [prepare](/tools/publishing/preparing), [sign](/tools/publishing/app-signing), and then [publish the application](/distribute/tools/launch-checklist).
...
| [Market Licensing Support FAQ](http://market.android.com/support/bin/answer.py?answer=186113) |
...
For general information about how to post to the groups listed above, see [Community Resources](/resources/community-groups) section
