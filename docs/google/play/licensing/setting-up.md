---
title: Setting Up for Licensing  |  Other Play guides  |  Android Developers
...
# Setting Up for Licensing Stay organized with collections Save and categorize content based on your preferences.
...
For more information, see [Get Started with Publishing](/distribute/googleplay/start).

If you already have a publisher account on Google Play, use your
Play Console to set up licensing.
...
administrative controls for Google Play licensing in the Play Console. The controls
let you:
...
* Set up multiple "test accounts," identified by email address. The licensing
  ...
  Account Details page of the Play Console.
* Configure static test responses that the server sends, when it receives a
  ...
  in the Account Details page of the Play Console.
...
![](/static/images/licensing_public_key.png)

**Figure 1.** An app's license key is available from the
Services & APIs page in the Play Console.

For more information about how to work with test accounts and static test
responses, see [Setting Up a Testing Environment](#test-env), below.

## Setting Up the Development Environment

Setting up your environment for licensing involves these tasks:

1. [Setting up the runtime environment](#runtime-setup) for development
2. [Downloading the LVL](#download-lvl) into your SDK
3. [Setting up the Licensing Verification Library](#lvl-setup)
4. [Including the LVL library project in your application](#add-library)

The sections below describe these tasks. When you are done with setup,
you can begin [Adding
Licensing to Your App](/google/play/licensing/adding-licensing).
...
* An Android emulator running the Google APIs Add-on, API level 8 (release 2)
  or higher
...
Android applications, see [Using Hardware Devices](/tools/device).
...
* A set of underlying account services that let you add a Google Account on
  ...
  your application without having publish it. For more information see [Signing in to an authorized account](#acct-signin), below.
...
1. Launch the Android SDK Manager, available under the Android Studio Tools menu
   (**Tools > Android > SDK Manager**) or by executing
   `<sdk>/tools/android sdk`.
2. Select and download **Google APIs** for the Android version you'd like to target
   (must be Android 2.2 or higher).
3. When the download is complete, open the AVD Manager, available under the Android Studio
   Tools menu (**Tools > Android > AVD Manager**) or by executing
   `<sdk>/tools/android avd`.
4. In the *Android Virtual Device Manager* window, select
   **+ Create Virtual Device** to set the configuration details for the new AVD.
5. In the *Virtual Device Configuration* window, select device hardware, then
   select **Next**.
6. Select a **Google API** as the system image to run on the new AVD, then
   select **Next**.
7. Assign a descriptive name to the AVD and then set the other configuration details as
   needed.
8. Select **Finish** to create the new AVD configuration, which will appear in the
   list of available Android Virtual Devices.

If you are not familiar with AVDs or how to use them, see [Managing Virtual Devices](/tools/devices).
...
should have no impact whatsoever on the application's build configuration.

### Setting Up the Licensing Verification Library

You will need to set up LVL in your
...
across all of your projects and maintain it centrally. If you aren't familiar
with library projects or how to use them, see
[Managing Projects](/tools/projects#LibraryProjects).

If you are working in Android Studio, you need to add the LVL to your
project as a new module.

1. Use the "Import Module from Source" window to import a library module by selecting
   **File > New > Import Module**.
2. In the *Import Module from Source* window, in *Source directory*, enter the LVL's
   `library` directory (the directory containing the library's AndroidManifest.xml file)
   as the project root (`<sdk>/extras/google/play_licensing/library/AndroidManifest.xml`),
   then select **Next**.
3. Select **Finish** to import the library module.

For more information about how to work with library modules in Android Studio, see
...
If you are developing in Android Studio, you should already have added the
...
see [Setting up a Library Project](/tools/projects/projects-cmdline#SettingUpLibraryProject).

## Setting Up the Testing Environment

The Google Play Console provides configuration tools that let you
and others test licensing on your application before it is published. As you are
implementing licensing, you can make use of the Play Console tools to test
...
1. [Setting static test responses](#test-response) that are returned by the licensing server.
2. [Setting up test accounts](#test-acct-setup) as needed.
3. [Signing in](#acct-signin) properly to an emulator or device, before initiating a license check test.
...
Response Codes](/google/play/licensing/licensing-reference#server-response-codes) in the [Licensing Reference](/google/play/licensing/licensing-reference).

![](/static/images/licensing_test_response.png)

**Figure 2.** The License Testing
panel of your Account details page lets you set up test accounts and
manage test responses.
...
See [Server
Response Codes](/google/play/licensing/licensing-reference#server-response-codes) for a list of
...
sign-in credentials. To meet that need, the Google Play Console lets
...
account. As shown in Figure 2, you
...
Play Console do not have access to the app's public key, and without
...
For more information, see [Setting up the runtime environment](#runtime-setup), above.
...
4. Enter the username and password of either the publisher account or a test
   account that is registered in the publisher account.
5. Select **Sign in**. The system signs you in to the new
   account.
...
Once you've completed the setup procedures, continue to [Adding Licensing to Your App](/google/play/licensing/adding-licensing).