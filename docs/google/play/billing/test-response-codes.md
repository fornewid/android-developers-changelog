---
title: Test BillingResult response codes  |  Play Billing  |  Android Developers
...
# Test BillingResult response codes Stay organized with collections Save and categorize content based on your preferences.
...
**Note:** Before testing, ensure that your app is using
[Play Billing Library](/google/play/billing/integrate#dependency) version 7.1.1 or later.
...
**Note:** Even though adding only the `enableBillingOverridesTesting` tag and
setting it to `true` is sufficient to enable billing overrides testing
for your app, it's recommended that you also add the `NONPRODUCTION` tag.
The `NONPRODUCTION` tag prevents accidental deployment of your app to
the production environment with billing overrides testing enabled.
...
1. In the Play Billing Lab app, sign in with a [license tester](/google/play/billing/test#license-testers) account for your app.
   ...
   ![Dashboard with Response Simulator card](/static/images/google/play/billing/play-billing-lab-dashboard-response-simulator.png)
...
4. Enable the **Simulate Play Billing Library responses** switch, if it isn't
   ...
   ![Simulate Play Billing Library responses switch](/static/images/google/play/billing/play-billing-lab-response-simulator.png)
...
**Note:** When testing using the Response Simulator, ensure that you first open
Play Billing Lab, and then open your app. Opening your app before opening
Play Billing Lab may result in issues connecting to the Response Simulator.
...
1. Open your app.
   ...
   * If you have allowed notifications from Play Billing Lab, you will see the Play Billing Lab notification icon in your device's status bar which indicates a successful connection to the Response Simulator.
   ![Play Billing Lab notification icon in status bar](/static/images/google/play/billing/play-billing-lab-response-simulator-notification.png)
