---
title: Setup  |  Play Integrity  |  Android Developers
...
**Note:** Projects that are enabled in the Google Cloud Console but not linked in
the Play Console or Play SDK Console are not eligible for additional
configuration options or quota increases.
...
4. Choose the Google Cloud project you plan to use with the Play Integrity API;
   if the Play Integrity API is not already enabled for the project, it will be
   enabled automatically upon linking.
...
3. Under **Play Integrity API** click **Link a Cloud project**.
4. Choose the Google Cloud project you plan to use with the Play Integrity API;
   if the Play Integrity API is not already enabled for the project, it will
   be enabled automatically upon linking.
...
[classic requests](/google/play/integrity/classic) to infrequent, high-value actions to preserve
user battery and data usage.

**Caution:** Very high volume changes should be rolled out gradually. Sudden
traffic spikes may cause throttling.
...
2. Verify you have correctly implemented API logic including proper retry
   strategies.
...
1. Download the latest [`.unitypackage`](//github.com/google/play-integrity-unity/releases/latest)
   release from GitHub.
...
Follow the [native setup guide](/google/play/integrity/native-setup). For more details, see Play Integrity's
[native API reference documentation](/reference/native/play/core/group/integrity).
...
If you distribute to [Google Play Games for PC](/games/playgames/overview), you will automatically be
...
**Caution:** Changes to integrity responses take effect immediately,
including for apps and SDKs in production. Make sure your server is prepared
to accept new responses before saving changes.
...
**Note:** [Device recall](/google/play/integrity/device-recall) is available in beta to apps and subject to change.
Pricing for high-scale usage may apply after general release.
SDKs cannot use device recall.
...
| Response field | Label | Description |  |
| --- | --- | --- | --- |
| Device integrity | `MEETS_BASIC_INTEGRITY` | The app is running on a device that passes basic system integrity checks. The device bootloader can be locked or unlocked, and the boot state can be verified or unverified. The device may not be certified, in which case Google cannot provide any security, privacy, or app compatibility assurances. On Android 13 and higher, the `MEETS_BASIC_INTEGRITY` verdict requires that the attestation [root of trust](/privacy-and-security/security-key-attestation#root_certificate) is provided by Google. | |
...
| Device attributes | `sdkVersion: 19, 20, ..., 37` | The SDK version of the Android OS running on the device. The number returned maps to [`Build.VERSION_CODES`](/reference/android/os/Build.VERSION_CODES). | |
...
**Note:** The app access risk verdict automatically exempts verified accessibility
services known to Google Play. These services will not trigger a capturing,
controlling, or overlay response, meaning legitimate accessibility tools are
not flagged as risks.

## Configure classic request settings (optional)

Skip this section if you only plan to make [standard API requests](/google/play/integrity/standard).
...
**Caution:** Never decrypt tokens or expose keys within your client app.**Tip:** Even when self-managing keys, your app can still fall back to Google Play's
...
1. Select **Manage and download my response encryption keys** and upload your
   public key.
...
**Caution:** Switching to Google-managed encryption immediately invalidates
previously downloaded keys, even in production. Before switching, make sure your
server is configured to handle the change to avoid disruption.
...
arrow\_forward](/google/play/integrity/standard)