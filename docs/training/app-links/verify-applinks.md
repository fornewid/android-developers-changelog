...
2. For each unique hostname found in the intent filters, Android queries the corresponding websites for the Digital Asset Links file at https://<var translate="no">hostname</var>/.well-known/assetlinks.json.
...
`none`
:
    Nothing has been recorded for this domain. Wait a few more minutes for the verification agent to finish the requests related to domain verification, and then [invoke the domain verification process](https://developer.android.com/training/app-links/verify-applinks#invoke-domain-verification) again.
...
:   Custom error code that's specific to the device's verifier.
    ...
    Double-check that you have [established a network connection](https://developer.android.com/training/app-links/verify-applinks#establish-internet-connection), and [invoke the domain verification process](https://developer.android.com/training/app-links/verify-applinks#invoke-domain-verification) again.
...
> [!NOTE]
> **Note:** The system can only associate one app at a time with a domain, even when you use shell commands. Some special cases, such as [installing two app variants simultaneously](https://developer.android.com/training/app-links/add-applinks#check-multiple), require special handling to open a given web link in the intended app.
...
After the user understands what your app is asking them to do, make the request. To do so, invoke an intent that includes the [`ACTION_APP_OPEN_BY_DEFAULT_SETTINGS`](https://developer.android.com/reference/android/provider/Settings#ACTION_APP_OPEN_BY_DEFAULT_SETTINGS) intent action, and a data string matching `package:com.example.pkg` for the target app, as shown in the following code snippet:
...
## Digital Asset Links update latency

When you update the `assetlinks.json` file on your web server, the time it takes for these changes to be reflected on end-user devices depends on the OS version:

- **Android 15 (API level 35) and higher:** The system periodically re-verifies domains in the background. Changes can take up to seven days to propagate to all end-user devices due to the system's caching and scheduled re-verification.
- **Android 14 (API level 34) and lower:** The system does not perform periodic background re-verification. Updates to the file are typically only picked up when the app is installed or updated.

**Testing updates**

To force a re-verification on a specific device during testing, you can uninstall and reinstall the app. However, be aware that server-side caches might still delay the delivery of the updated `assetlinks.json` file to the device, even after reinstalling. Because the cache is time-based, the delay typically resolves automatically if you try again after a few hours.