---
title: Build TV games  |  Android TV  |  Android Developers
...
# Build TV games Stay organized with collections Save and categorize content based on your preferences.
...
controllers](/training/tv/start/controllers) for more information about using TV controllers and
[Handle controllers for games](/training/tv/start/controllers#games) for specific
...
[`android:isGame`](/guide/topics/manifest/application-element#isGame) attribute to `"true"` in your app manifest's
[`<application>`](/guide/topics/manifest/application-element) tag. For example:
...
**Note:** You can use the `WebView` class for logins to
