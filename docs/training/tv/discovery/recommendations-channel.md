---
title: Channels on the home screen  |  Android TV  |  Android Developers
...
# Channels on the home screen Stay organized with collections Save and categorize content based on your preferences.
...
[home screen codelab](/codelabs/tv-recommendations-kotlin#0)
...
must use the
[recommendations row](/training/tv/discovery/recommendations-row)
instead.
...
the Watch Next channel](/training/tv/discovery/watch-next-add-programs).
...
1. Create a channel builder and set its attributes. Note that the
   ...
   [attributes](#attributes) as required.
...
- Make your default channel appear before your app is opened. You can
  ...
  your.package.name/.YourReceiverName is your app's
  `BroadcastReceiver`:
...
available for each type of program, refer to the [tables](#attributes) below.)
...
Next channel](/training/tv/discovery/watch-next-add-programs).
...
* The home screen sends the `Uri` stored in the APP\_LINK\_INTENT\_URI attribute of a channel to the app when the user selects the channel's logo. The app should just launch its main UI or a view related to the selected channel.
* The home screen sends the `Uri` stored in the INTENT\_URI attribute of a program to the app when the user selects a program. The app should play the selected content.
* The user can indicate that they are no longer interested in a program and want it removed from the home screen's UI. The system removes the program from the UI and sends the app that owns the program an intent (android.media.tv.ACTION\_PREVIEW\_PROGRAM\_BROWSABLE\_DISABLED or android.media.tv.ACTION\_WATCH\_NEXT\_PROGRAM\_BROWSABLE\_DISABLED) with the program's ID. The app should remove the program from the provider and should NOT reinsert it.
...
* When your app is not in the foreground and you need to update a channel or a
  ...
  and [JobService](https://developer.android.com/reference/android/app/job/JobService.html)).
...
| DISPLAY\_NAME | set to the name of the channel. |
| APP\_LINK\_INTENT\_URI | When the user selects the channel's logo the system sends an intent to start an activity that presents content relevant to the channel. Set this attribute to the Uri used in the intent filter for that activity. |
...
* INTERNAL\_PROVIDER\_ID
* INTERNAL\_PROVIDER\_DATA
* INTERNAL\_PROVIDER\_FLAG1
* INTERNAL\_PROVIDER\_FLAG2
* INTERNAL\_PROVIDER\_FLAG3
* INTERNAL\_PROVIDER\_FLAG4
...
* [Watch Next Program Attributes](/training/tv/discovery/watch-next-programs)

## Sample Code

To learn more about building apps that interact with the home screen and add channels and programs to the Android TV home screen, see our home screen [codelab](/codelabs/tv-recommendations-kotlin#0).

[Previous

arrow\_back

About recommending TV content](/training/tv/discovery/recommendations)

[Next

Video program attributes

arrow\_forward](/training/tv/discovery/video-programs)