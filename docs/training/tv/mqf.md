...
TV-watching experience. Profiles and settings can be managed per package name
(app) or per input ID. Additionally, profiles can be created by system apps or
apps on the allowlist, and they can be updated or removed only by their owner.

Specifically, the framework manages Picture and Sound Profiles. Examples of
picture parameters include brightness, contrast, and sharpness, while examples
of sound parameters include bass and treble.

These profiles can be applied in various contexts, including setting global
default profiles, streaming profiles using `MediaCodec`, setting app default
profiles for specific media apps, and setting TV input picture profiles for
specific inputs like HDMI. The framework also supports status changes (allowing
different parameter sets for states like HDR), manages active picture profiles
for ongoing processing, and handles ambient backlight features.
...
The Media Quality Framework is composed of several key components that work
together across the application, framework, and hardware layers.
![PQ AQ Standardization Diagram](https://developer.android.com/training/tv/images/MQ_Architecture.png) **Figure 1.** Interactions between Media Quality Framework components.
...
The Media Quality Framework supports a wide range of features to provide a
flexible and powerful system for managing media quality.
...
The following diagrams illustrate the call sequences for common media quality
operations.

### Create and Apply a Picture Profile

![Creating a Picture Profile](https://developer.android.com/training/tv/images/create-picture-profile.png) **Figure 2.** Sequence diagram for creating a picture profile.
...
4. The `MediaQualityService` processes the request, saves the profile to the database, and notifies the app of success or failure using the registered callback (`onPictureProfileAdded()` or `onError()`).

### Set a Per-Stream Picture Profile

This workflow demonstrates how an application can apply a specific profile to
its video content during playback.
![Set Stream Picture Profile](https://developer.android.com/training/tv/images/set-stream-profile.png) **Figure 3.** Sequence diagram for setting a per-stream picture profile.

1. The media app queries the `MediaQualityManager` to get a list of available picture profiles.
2. The app then uses `MediaCodec.setParameters` to select and set the selected profile for the stream.
...
Only system apps can set the global default profile, which serves as a fallback
for any content that does not have a specific profile assigned.
...
The Media Quality SDK API provides the necessary tools for apps to interact with
the framework. The main entry point is the `android.media.quality` package.
...
| `MediaQualityContract` | A class containing constants for all predefined parameter keys. |
...
The `PictureProfile` object encapsulates all the information for a given picture
mode.
...
The Media Quality HAL is an **AIDL-based** interface defined in
`hardware/interfaces/tv/mediaquality/aidl/`. SoC vendors must implement this
interface to enable the Media Quality Framework on their devices. The HAL serves
as the communication bridge between the `MediaQualityService` in the Android
framework and the underlying hardware, responsible for applying picture and
sound adjustments.

The HAL manages picture and sound profiles using AIDL parcelables. This involves
receiving profile changes from the framework and notifying the framework of any
adjustments made at the hardware level.
...
| `IPictureProfileAdjustmentListener.aidl` | A listener interface that the HAL uses to notify the framework of adjustments made at the hardware level, for example, when a user changes a setting directly using a hardware remote. |
