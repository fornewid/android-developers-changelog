---
title: Audio capabilities  |  Android TV  |  Android Developers
...
# Audio capabilities Stay organized with collections Save and categorize content based on your preferences.
...
[`AudioTrack`](/reference/android/media/AudioTrack) with the best
[`AudioFormat`](/reference/android/media/AudioFormat) supported by the output
audio device.

## Create a track with the right format

Apps should create an `AudioTrack`, start playing it, and call
[`getRoutedDevice()`](/reference/android/media/AudioRouting#getRoutedDevice())
...
Use
[`getAudioProfiles()`](/reference/android/media/AudioDeviceInfo#getAudioProfiles())
(API level 31 and higher) or
[`getEncodings()`](/reference/android/media/AudioDeviceInfo#getEncodings())
(API level 23 and higher) to determine the audio formats available on the
default audio device.

### Check supported audio profiles and formats

Use [`AudioProfile`](/reference/android/media/AudioProfile)
(API level 31 and higher) or
[`isDirectPlaybackSupported()`](/reference/android/media/AudioTrack#isDirectPlaybackSupported(android.media.AudioFormat,%20android.media.AudioAttributes))
...
[`FLAG_HW_AV_SYNC`](/reference/android/media/AudioAttributes#FLAG_HW_AV_SYNC).
...
[`getDirectPlaybackSupport()`](/reference/android/media/AudioManager#getDirectPlaybackSupport(android.media.AudioFormat,%20android.media.AudioAttributes))
...
[`AudioFormat`](/reference/android/media/AudioFormat) instances. It is ordered
...
[`AudioProfile`](/reference/android/media/AudioProfile) objects for the
currently routed audio device with the supplied
[`AudioAttributes`](/reference/android/media/AudioAttributes). The list of
...
Finally, an appropriate [`AudioFormat`](/reference/android/media/AudioFormat)
instance is created.
...
* For API levels equal to or greater than 24, add an
  [`OnRoutingChangedListener`](/reference/android/media/AudioRouting.OnRoutingChangedListener)
  to monitor audio device changes (HDMI, Bluetooth, and so on).
* For API level 23, register an
  [`AudioDeviceCallback`](/reference/android/media/AudioDeviceCallback)
  to receive changes in the available audio device list.
* For API levels 21 and 22, monitor for
  [HDMI plug events](/reference/android/media/AudioManager#ACTION_HDMI_AUDIO_PLUG)
  and use the extra data from the broadcasts.
* Also register a `BroadcastReceiver` to monitor
  [`BluetoothDevice`](/reference/android/bluetooth/BluetoothDevice) state changes
  for devices lower than API 23, since
  [`AudioDeviceCallback`](/reference/android/media/AudioDeviceCallback) is not
  yet supported.
...
Background playback in a Now Playing card](/training/tv/playback/now-playing)

[Next

Adjust display settings

arrow\_forward](/training/tv/playback/adjust-display-settings)