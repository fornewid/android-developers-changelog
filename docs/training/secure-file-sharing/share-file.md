---
title: Sharing a file  |  App data and files  |  Android Developers
...
# Sharing a file Stay organized with collections Save and categorize content based on your preferences.
...
becoming a [dangerous permission](/guide/topics/permissions/requesting#normal-dangerous), which the receiving app might lack.
...
* Requires that your app have
  ...
  permission on devices running Android 4.4 (API level 19) or lower.
* Requires that receiving apps have the
  `READ_EXTERNAL_STORAGE` permission, which
  will fail on important share targets, like Gmail, that don't have that permission.

Instead of using `Uri.fromFile()`,
you can use [URI permissions](#GrantPermissions) to grant other apps
...
[Specify sharable directories](/training/secure-file-sharing/setup-sharing#DefineMetaData).
...
described in the section [Specify sharable directories](/training/secure-file-sharing/setup-sharing#DefineMetaData). If you call
`getUriForFile()` for a
...
as explained in [File sharing](/training/secure-file-sharing).
...
* [Designing Content URIs](/guide/topics/providers/content-provider-creating#ContentURI)
* [Implementing Content Provider Permissions](/guide/topics/providers/content-provider-creating#Permissions)
* [Permissions](/guide/topics/security/permissions)
...
arrow\_forward](/training/secure-file-sharing/request-file)