---
title: Personalize with photos  |  Wear OS  |  Android Developers
...
# Personalize with photos Stay organized with collections Save and categorize content based on your preferences.
...
watchface_photos_multi.xml
```

The preceding example shows the two options that are introduced when using
`MULTIPLE`:

1. The `change` attribute allows the developer to specify what events should
   cause the photo to cycle through the available images.
...
1. The `changeAfterEvery` attribute applies only to the `ON_VISIBLE` change
   event, specifing how many times the photo should have become visible
   before the photo is changed. For example, you may feel that changing the photo
   every time the user wakes their watch is too frequent. The default value here
   is 3.
