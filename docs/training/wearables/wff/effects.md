---
title: Apply masks, blend effects, and tints  |  Wear OS  |  Android Developers
...
# Apply masks, blend effects, and tints Stay organized with collections Save and categorize content based on your preferences.
...
2. All child elements with `SOURCE` set (which is the default) are drawn
   1. All child elements that are part of the mask (`MASK, ALL`) are applied to
      the canvas to clip the resulting image.
...
* The **first and third** elements are `MASK` elements, so they're combined
  together to form a masking layer
...
[Android Graphics blend mode effects](/reference/android/graphics/BlendMode), and applies effects in the order that
...
As a demonstration of using blend modes, choosing [`SRC_ATOP`](/reference/android/graphics/BlendMode#SRC_ATOP), discards
...
More complex effects can be applied, such as using [`COLOR_DODGE`](/reference/android/graphics/BlendMode#COLOR_DODGE) instead of
`SRC_ATOP`, which *makes destination brighter to reflect source*.

An example of combining multiple `Part*` elements using [`HUE`](/reference/android/graphics/BlendMode#HUE) and
[`COLOR_BURN`](/reference/android/graphics/BlendMode#COLOR_BURN) blend modes:
