---
title: Display ongoing activities  |  Wear OS  |  Android Developers
...
# Display ongoing activities Stay organized with collections Save and categorize content based on your preferences.
...
The solution is to pair an ongoing notification with an [`OngoingActivity`](/reference/androidx/wear/ongoing/OngoingActivity).
...
information, see [Keep your app visible on Wear](/training/wearables/always-on#background).

For example, in this workout app, the information can appear on the user's watch
face as a tappable running icon:

![running-icon](/static/wear/images/ongoing_activity_5.png)
...
![launcher](/static/wear/images/ongoing_activity_4.png)
...
![timer](/static/wear/images/ongoing_activity_2.png)

**Figure 3.** **Timer:** Actively counts down time and ends when the timer is
paused or stopped.

![map](/static/wear/images/ongoing_activity_3.png)
...
![music](/static/wear/images/ongoing_activity_1.png)
...
See the [Ongoing Activity codelab](/codelabs/ongoing-activity) for an in-depth example of creating
...
1. Create a standard [`NotificationCompat.Builder`](/reference/androidx/core/app/NotificationCompat.Builder) and configure it as
   ongoing.
2. Create and configure an [`OngoingActivity`](/reference/androidx/wear/ongoing/OngoingActivity) object, passing the
   notification builder to it.
3. Apply the ongoing activity to the notification builder and post the
   resulting notification.

### Create and configure the notification

Start by creating a **`NotificationCompat.Builder`**. The key step is to call
[`setOngoing(true)`](/reference/android/app/Notification.Builder#setOngoing(boolean)) to mark it as an ongoing notification. You can also set
...
* **Animated and static icons**: Provide icons that are displayed on the watch
  face in active and ambient modes.
* **Touch intent**: A `PendingIntent` that brings the user back to your app
  when they tap the ongoing activity icon. You can reuse the `pendingIndent`
...
post it. The [`ongoingActivity.apply()`](/reference/androidx/wear/ongoing/OngoingActivity#apply(android.content.Context)) method modifies the original
...
[`Status`](/reference/androidx/wear/ongoing/Status) object and attach it to your `OngoingActivity`. If you don't
provide a custom `Status`, the system defaults to using the notification's
content text (set using [`setContentText()`](/reference/android/app/Notification.Builder#setContentText(java.lang.CharSequence))). To display dynamic text, use a
...
* The **category** set determines the priority of the ongoing activity.
  + **`CATEGORY_CALL`:** an incoming voice or video call or a similar
...
* **[`LocusId`](/reference/android/content/LocusId):** ID that assigns the launcher shortcut that the ongoing
...
* **Ongoing Activity ID:** ID used to disambiguate calls to
  [`fromExistingOngoingActivity()`](/reference/androidx/wear/ongoing/OngoingActivity?#fromExistingOngoingActivity(android.content.Context)) when an application has more than one
  ongoing activity.
...
* Set a static icon for your Ongoing Activity, either [explicitly](/reference/androidx/wear/ongoing/OngoingActivity.Builder#setStaticIcon(android.graphics.drawable.Icon)) or as a
  fallback using the [notification](/reference/androidx/core/app/NotificationCompat.Builder#setSmallIcon(int)). If you don't, you get an
  `IllegalArgumentException`.
* Use black and white vector icons with transparent backgrounds.
* Set a touch intent for your ongoing activity, either [explicitly](/reference/androidx/wear/ongoing/OngoingActivity.Builder#setTouchIntent(android.app.PendingIntent)) or as
  a fallback using the [notification](/reference/androidx/core/app/NotificationCompat.Builder#setContentIntent(android.app.PendingIntent)). If you don't, you get an
  `IllegalArgumentException`.
* If your app has more than one `MAIN LAUNCHER` activity declared in the
  manifest, publish a [dynamic shortcut](/guide/topics/ui/shortcuts/creating-shortcuts#dynamic) and associate it with your
...
notification](/reference/android/app/Notification.MediaStyle). This lets the system create the corresponding ongoing
activity.

**Note:** If media content is playing on a handheld device and not on a Wear OS
device, don't post a media notification on the Wear OS device.
...
[`MediaStyleNotificationHelper.MediaStyle`](/reference/androidx/media3/session/MediaStyleNotificationHelper.MediaStyle), and the corresponding
`MediaSession` should have its [session activity](/reference/androidx/media3/session/MediaSession.Builder#setSessionActivity(android.app.PendingIntent)) populated.
