---
title: Run and debug a tile  |  Wear OS  |  Android Developers
...
# Run and debug a tile Stay organized with collections Save and categorize content based on your preferences.
...
available for use in both [Android Studio](/studio) and [`adb`](/tools/adb).

## Deploy tiles

To deploy your tile, you can install your app on a physical device or emulator
using [Android Studio](/studio), as described in [Build and run your app](/studio/run). Then,
...
Studio's ["Wear OS Tile" Run/Debug Configuration](/studio/run/rundebugconfig) feature. This method
...
exist. See [Wear OS run/debug configurations](/studio/run/rundebugconfig#wear-os-configs) for more information.

![Screenshot with red arrow pointing to "▷" in the gutter](/static/training/wearables/images/as-gutter-run.png)


Click "▷" in the gutter


![Screenshot showing red arrow pointing to run configuration dropdown, and the "TestTileService" configuration option outlined in red](/static/training/wearables/images/as-run-command.png)


A "run configuration" is automatically created
...
1. Select the "⋮" ("hamburger") option from the title bar.
   ...
   ![Screenshot with red arrow pointing to the "⋮" ("hamburger") option from the title bar.](/static/training/wearables/images/as-hamburger.png)
2. Select "Edit…".

   !["Edit…" is the first option under the Configuration section of the drop-down list.](/static/training/wearables/images/as-edit.png)
3. Select "+" to add a configuration on the dialog that appears.

   ![Screenshot with red arrow pointing to "+".](/static/training/wearables/images/as-plus.png)
4. Select "Wear OS Tile" as the type of the new configuration.

   ![Screenshot with red arrow pointing to the "Wear OS Tile" configuration option.](/static/training/wearables/images/as-wear-os-tile.png)
5. Enter a name for your configuration and select the module and tile as
   appropriate.

   ![Screenshot with three red arrows pointing to areas of the dialog window.](/static/training/wearables/images/as-edit-configuration.png)
...
![Group names in the preview panel match the group name given in the
    preview annotation](/static/studio/images/design/wear-tiles-preview-panel.png)
...
[composable previews](/develop/ui/compose/tooling/previews).
...
**Note:** Although the `@Preview` annotation is specific to Wear OS tiles, other
options such as `device`, `name`, `group`, `fontScale`, and `locale` work the
same for previews of Wear OS tiles as they do for previews of components that
use [Compose on Wear OS](/training/wearables/compose/screen-size#previews).
...
[`adb`](/tools/adb) can be used to programmatically install, remove and activate tiles
during development.
...
* **result=0**: No broadcast receiver responded to this command, most likely
  because the Wear OS version is too old, or the debug process is not running.
* **result=1**: Success.
* **result>1**: Error.