---
title: Jetpack Compose performance on Wear OS  |  Android Developers
...
# Jetpack Compose performance on Wear OS Stay organized with collections Save and categorize content based on your preferences.
...
Use the [Jetpack Compose Performance](/jetpack/compose/performance) guide to configure and develop your app
...
Update your [Compose](/jetpack/androidx/releases/compose) dependency to version 1.8 or higher, which introduced
several significant new features and improved the overall stability of the
library. See the instructions in [Declaring dependencies](/jetpack/androidx/releases/compose#declaring_dependencies) to learn how to
...
To improve your app's performance, use [baseline profiles](/jetpack/compose/performance#use-baseline). Group together
...
* Use specific ADB commands to validate your app's profile configuration state.
  Steps for both of these techniques are explained in the
  [performance measurement and validation](#performance-measurement) guide.

#### Startup Profiles

As a subset of baseline profiles, [Startup Profiles](/topic/performance/baselineprofiles/dex-layout-optimizations) further optimize the
...
To get started, read [Create a Startup Profile](/topic/performance/baselineprofiles/dex-layout-optimizations#create-startup).

### R8

Use the [R8 compiler](/studio/build/shrink-code) to shrink and optimize apps. R8 removes unused code and
resources, rewrites code to optimize runtime performance, and more.

In the [improve performance Overview](/topic/performance/improving-overview) guides, read the considerations for R8, including key steps for [removing unused resources](/studio/build/shrink-code#shrink-resources).

## Performance Measurement and Validation

To learn about general performance measurement strategies on Android,
see [Overview of measuring app performance](/topic/performance/measuring-performance). This section describes some of
...
[release mode](/studio/run#changing-variant).
...
key differences from release debugging. See the [Macrobenchmark setup guide](/topic/performance/benchmarking/macrobenchmark-overview#set-up-app)
for details.
...
profile](/topic/performance/baselineprofiles#measuring-baseline).

### UI Automator API

The [UI Automator API](/training/testing/other-components/ui-automator) automates interactions programmatically. Use this API
...
complex UI manipulations. To get started, see the [implementation guide](/topic/performance/benchmarking/macrobenchmark-overview).
...
Use the [JankStats library](/topic/performance/jankstats) to track and analyze performance problems in
applications.
...
[System Trace](/studio/profile/cpu-profiler) feature in Android Studio to inspect and diagnose latency in
...
For example, using tools like [Live Edit](/develop/ui/compose/tooling/iterative-development) and [Composable Previews](/develop/ui/compose/tooling/previews),
...
If you are migrating from views, check out out [migration guide](/develop/ui/compose/migrate/strategy) and our
[best practices for Jetpack Compose performance](/develop/ui/compose/performance/bestpractices) to verify that your app's
UIs are performant when using Jetpack Compose.

## Other resources

To stay up to date with the latest in android performance, check out the
[Latest news and videos](/topic/performance/overview#latest-news-and-videos) in the App performance guide.