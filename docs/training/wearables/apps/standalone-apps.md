---
title: Standalone versus non-standalone Wear OS apps  |  Android Developers
...
# Standalone versus non-standalone Wear OS apps Stay organized with collections Save and categorize content based on your preferences.
...
the Google Play Store, see [Package and distribute Wear OS Apps](/training/wearables/packaging) and the guide to getting started with
[Android App Bundles](/guide/app-bundle#get_started).

For new apps, the target API level must be 34 or higher. For more information, see
[Meet Google Play's target API
level requirement](/google/play/requirements/target-sdk). Set the
...
see [Network access and sync on Wear OS](/training/wearables/data-layer/network-access).

### Define an app as a Wear OS app

You must define the `<uses-feature>` tag in your app's Android manifest file.
...
* **Standalone**: a completely independent app that doesn't require a phone app for core
  ...
  on a tethered phone.
* **Non-standalone**: a dependent app that requires an app on a phone or another device for
...
In your Wear OS app, set the value of `meta-data` element `com.google.android.wearable.standalone`
...
[detects](#detecting-your-app) that a companion phone
...
would on a phone. You can use the
[DataStore](/topic/libraries/architecture/datastore) to store
key-value pairs and typed objects, or you can use the
...
[Advertise capabilities](/training/wearables/data-layer/messages#advertise-capabilities).
...
[Wear OS API reference](/reference/packages-wearable-support).
That reference also includes information about the
[`PhoneTypeHelper`](/reference/kotlin/androidx/wear/phone/interactions/PhoneTypeHelper) class, which contains a
[`getPhoneDeviceType()`](/reference/kotlin/androidx/wear/phone/interactions/PhoneTypeHelper#getPhoneDeviceType(android.content.Context)) method that lets your
...
For more information, see [Advertise capabilities](/training/wearables/data-layer/messages#advertise-capabilities).
...
1. Use the
   `CapabilityClient` to check whether your phone app is installed
   on the paired phone. For more information, see the
   [Datalayer helpers sample](https://github.com/google/horologist/tree/v0.6.10/datalayer/sample) on GitHub.
2. If your phone app isn't installed on the phone, use the
   [`PhoneDeviceType.getPhoneDeviceType()`](/reference/kotlin/androidx/wear/phone/interactions/PhoneTypeHelper#getPhoneDeviceType(android.content.Context)) method to
   check the type of the phone. See the following section for details.
3. If
   [`PhoneDeviceType.DEVICE_TYPE_ANDROID`](/reference/kotlin/androidx/wear/phone/interactions/PhoneTypeHelper#DEVICE_TYPE_ANDROID())
   is returned, the phone is an Android phone. Call
   [`RemoteActivityHelper.startRemoteActivity()`](/reference/kotlin/androidx/wear/remote/interactions/RemoteActivityHelper#startRemoteActivity(android.content.Intent,kotlin.String)) on the Wear OS device to
...
4. If
   [`PhoneDeviceType.DEVICE_TYPE_IOS`](/reference/kotlin/androidx/wear/phone/interactions/PhoneTypeHelper#DEVICE_TYPE_IOS())
...
| --- | --- |
| [`DEVICE_TYPE_ANDROID`](/reference/kotlin/androidx/wear/phone/interactions/PhoneTypeHelper#DEVICE_TYPE_ANDROID()) | The companion phone is an Android device. |
| [`DEVICE_TYPE_IOS`](/reference/kotlin/androidx/wear/phone/interactions/PhoneTypeHelper#DEVICE_TYPE_IOS()) | The companion phone is an iOS device. |
| [`DEVICE_TYPE_UNKNOWN`](/reference/kotlin/androidx/wear/phone/interactions/PhoneTypeHelper#DEVICE_TYPE_UNKNOWN()) | The companion phone is an unknown device. |
| [`DEVICE_TYPE_ERROR`](/reference/kotlin/androidx/wear/phone/interactions/PhoneTypeHelper#DEVICE_TYPE_ERROR()) | An error occurred in determining the type of the paired phone; another check should be made later. |
...
1. Using the
   [`NodeClient`](https://developers.google.com/android/reference/com/google/android/gms/wearable/NodeClient), find all watches connected to the user's
   phone. For more information, see the
   [Datalayer helpers sample](https://github.com/google/horologist/tree/v0.6.10/datalayer/sample) on GitHub.
2. Using the
   `CapabilityClient`, check which of the user's watches has
   your app installed.
3. If your app isn't installed on all of the user's watches, let the user
   open the Play Store on the remaining Wear OS devices from the phone using
   the `RemoteActivityHelper.startRemoteActivity()` method.
...
[Detect location on Wear OS](/training/articles/wear-location-detection).
...
[High-bandwidth network access](/training/wearables/data-layer/network-access#high-bandwidth-network-access).
