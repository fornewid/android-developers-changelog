---
title: Build View-based UIs on Wear OS  |  Android Developers
...
# Build View-based UIs on Wear OS Stay organized with collections Save and categorize content based on your preferences.
...
* `CurvedTextView`: a component for easily writing text that follows
  the curvature of the largest circle that can be inscribed in the view.
* `DismissibleFrameLayout`: a layout that lets the user
  dismiss any view by pressing the back button or swiping on the screen from left to right.
  Wear OS users expect left-to-right swiping for the back action.
* `WearableRecyclerView`: a view that provides basic offsetting logic for updating
  child layouts using a
  `WearableLinearLayoutManager`.
* `AmbientModeSupport`: a class used with the
  `AmbientModeSupport.AmbientCallbackProvider` interface
  to provide support for ambient mode.
...
`DismissibleFrameLayout` class from the Wear OS UI Library, you could specify
