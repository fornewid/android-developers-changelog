---
title: Dynamic expressions in Wear OS  |  Android Developers
...
# Dynamic expressions in Wear OS Stay organized with collections Save and categorize content based on your preferences.
...
* `DynamicInt32` supports conversion to a `DynamicString` using
  [`format()`](/reference/androidx/wear/protolayout/expression/DynamicBuilders.DynamicInt32#format()).
* `DynamicDuration` lets you extract specific parts, such as the seconds part
  of a duration, as `DynamicInt32` objects.
...
4. The `constant(2)` expression, which is implied by the value `2` in the
   `plus()` dynamic expression.
...
* [Use dynamic values in complications](/training/wearables/complications/exposing-data#providing-dynamic-values)
* [Dynamic expressions in Tiles](/training/wearables/tiles/dynamic)
