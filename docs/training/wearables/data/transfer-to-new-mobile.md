---
title: Transfer Wear OS data to a new mobile device  |  Android Developers
...
# Transfer Wear OS data to a new mobile device Stay organized with collections Save and categorize content based on your preferences.
...
**Note:** This process of transferring data to a new mobile device is different from
[cloud backup and restore](/training/wearables/data/cloud-backup-restore), where the user restores data to a new Wear OS
device.
...
1. Your app is installed on both mobile devices that are involved in the
   transfer.
2. The mobile apps, installed on each mobile device, have package
...
3. Create a service class which extends `WearableListenerService` and overrides
   ...
   **Caution:** To preserve system resources such as battery life, and to prevent
   ANRs, don't execute any power-demanding or time-consuming tasks in the
   `onNodeMigrated()` handler.
