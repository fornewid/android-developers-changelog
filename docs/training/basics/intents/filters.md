---
title: Let other apps start your activity  |  App architecture  |  Android Developers
...
# Let other apps start your activity Stay organized with collections Save and categorize content based on your preferences.
...
support the `ACTION_SEND` intent.
...
To let other apps start your activity in this way, you need to add an [`<intent-filter>`](/guide/topics/manifest/intent-filter-element)
element in your manifest file for the corresponding [`<activity>`](/guide/topics/manifest/activity-element) element.
...
When an app calls `startActivity()` or `startActivityForResult()`
...
The system might send a given `Intent` to an activity if that activity has
...
:   A string naming the action to perform. Usually one of the platform-defined values, such
    as `ACTION_SEND` or `ACTION_VIEW`.

    Specify this in your intent filter with the [`<action>`](/guide/topics/manifest/action-element) element.
...
:   A description of the data associated with the intent.
    ...
    Specify this in your intent filter with the [`<data>`](/guide/topics/manifest/data-element) element. Using one
    ...
    **Note:** If you don't need to declare specifics about the data
    `Uri`, such as when your activity handles
...
:   Provides an additional way to characterize the activity handling the intent, usually related
    ...
    Specify this in your intent filter with the [`<category>`](/guide/topics/manifest/category-element)
    element.

In your intent filter, you can declare which criteria your activity accepts
by declaring each of them with corresponding XML elements nested in the [`<intent-filter>`](/guide/topics/manifest/intent-filter-element)
element.
...
from your activity's default icon, add `android:icon` in the [`<intent-filter>`](/guide/topics/manifest/intent-filter-element)
element.

Each incoming intent specifies only one action and one data type, but it's OK to declare multiple
instances of the [`<action>`](/guide/topics/manifest/action-element), [`<category>`](/guide/topics/manifest/category-element), and [`<data>`](/guide/topics/manifest/data-element) elements in each
...
For example, suppose your activity handles both text and images for both the `ACTION_SEND` and `ACTION_SENDTO` intents. In this case, you must define two separate
intent filters for the two actions, because an `ACTION_SENDTO` intent must use the data `Uri` to specify
...
`CATEGORY_DEFAULT` category in the intent filter. The methods `startActivity()` and `startActivityForResult()` treat all intents as if they
...
intents that perform social sharing behaviors, see [Receiving simple data from other apps](/training/sharing/receive).
You can also find useful information about sharing data in
[Sharing simple data](/training/sharing) and
[Sharing files](/training/secure-file-sharing).

## Handle the intent in your activity

To decide what action to take in your activity, read the
`Intent` that is used to start it.

As your activity starts, call `getIntent()` to retrieve the
...
If you want to return a result to the activity that invoked yours, call `setResult()` to specify the result code and result `Intent`. When your operation is done and the user returns to the original
activity, call `finish()` to
...
You must always specify a result code with the result. Generally, it's either `RESULT_OK` or `RESULT_CANCELED`. You can then
provide additional data with an `Intent`, as necessary.
...
have no need to include the `Intent`,
...
with `startActivity()` or `startActivityForResult()`. Simply call `setResult()` if the intent that started your activity
...
arrow\_forward](/training/basics/intents/limit-play-loading)