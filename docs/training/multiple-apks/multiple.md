---
title: Creating multiple APKs with several dimensions  |  Other Play guides  |  Android Developers
...
# Creating multiple APKs with several dimensions Stay organized with collections Save and categorize content based on your preferences.
...
When trying to create an application that works across the huge range of available Android
...
case. The [Using
Single APK Instead](/google/play/publishing/multiple-apks#ApiLevelOptions) section of the multiple APK guide includes some useful information on how to
accomplish all of this with a single APK, including use of our [support library](http://android-developers.blogspot.com/2011/03/fragments-for-all.html),
and links to resources throughout the Android Developer guide.
...
in some "blobs", each color representing one APK.
...
chart out and hand it to every person working on your codebase. Life just got a lot easier.
...
By way of example, let’s take the set of multiple APKs described earlier, and assume that each
APK has been set to support all screen sizes larger than its "target" screen size. Let’s look at
the sample chart from earlier:
...
Since it’s okay for coverage to overlap, we can describe the area covered by each APK like
so:
...
Note that there’s a *lot* of overlap in those rules. For instance, an
...
other two don’t. The [Filters on Google Play](/google/play/filters) page
...
* If the APKs overlap in platform version, the one with the higher minSdkVersion must have a
  higher version code.
...
Create multiple APKs for different GL textures ⍈](/develop/ui/views/graphics/multiple-apks/texture)