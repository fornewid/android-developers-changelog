---
title: Build a templated media app  |  Android for Cars  |  Android Developers
...
# Build a templated media app Stay organized with collections Save and categorize content based on your preferences.
...
**Note:** Media apps built with the Car App Library templates are currently only
available on Android Auto.
**Design guidelines:**
Refer to [Media Apps](https://developers.google.com/cars/design/create-apps/app-types/media) for UX guidance specific to media apps.

This guide assumes that you already have a media app that plays audio on a phone
and that your media app conforms to the [Android media app architecture](/media/legacy). The
Car App Library gives you the ability to replace the in-app experience with
templates instead of those built using the [Build media apps for cars](/training/cars/media)
...
**Note:** Both a `MediaBrowserService` and Car App Library implementation can be
supported by the same app. In the case that a user has an older version of
Android Auto installed or cannot use the Car App Library version,
the host will fall back to the `MediaBrowserService` implementation.

## Configure your app's manifest

In addition to the steps described in
[Using the Android for Cars App Library](/training/cars/apps#configure-manifest-files), the following are
...
To get access to the [`MediaPlaybackTemplate`](/reference/kotlin/androidx/car/app/media/model/MediaPlaybackTemplate), your app also
...
be sure your minimum [`Car App API level`](/training/cars/apps#api-level) is set to 8.
...
See [support voice actions for media](/training/cars/media#support_voice) for more detailed implementation
...
is required to meet the [`VC-1`](/docs/quality-guidelines/car-app-quality#car-optimized-tier) quality guideline.

## Create your Playback Template

The [`MediaPlaybackTemplate`](/reference/kotlin/androidx/car/app/media/model/MediaPlaybackTemplate) displays media playback
...
When you use [`MediaPlaybackTemplate`](/reference/kotlin/androidx/car/app/media/model/MediaPlaybackTemplate), register a
`MediaSession` token using the [`MediaPlaybackManager`](/reference/androidx/car/app/media/MediaPlaybackManager) in your
...
[`SectionedItemTemplate`](/reference/androidx/car/app/model/SectionedItemTemplate),
which lets you use the [`GridSection`](/reference/androidx/car/app/model/GridSection) and
[`RowSection`](/reference/androidx/car/app/model/RowSection) together to create layouts that mix lists of images
and text items.

**Caution:** The screen stack can have a maximum depth of five screens. See
[Template restrictions](/training/cars/apps#template-restrictions).
...
[`SectionedItemTemplate`](/reference/androidx/car/app/model/SectionedItemTemplate) inside a
[`TabTemplate`](/reference/androidx/car/app/model/TabTemplate).
...
[`MFT-1`](/docs/quality-guidelines/car-app-quality#car-optimized-tier) quality requirement, your app must have a way to access
...
floating action button that [navigates you](/training/cars/apps#implement-screen-navigation) to the media playback screen. For
