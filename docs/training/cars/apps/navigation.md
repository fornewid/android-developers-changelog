    ...
    <application>
       ...
       <service
          ...
          in<tent-filter
            ...
            category android:name="and>roidx.c<ar.app.categor>y.NAV<IGATION&>quot;/
      <    />intent-filter
        /service
        ...
    /application
...
A variety of intent formats enable navigation apps to work with other apps, such
as point of interest apps and voice assistants.

To support these intent formats, first declare support by adding intent filters
...
Navigation apps can access the following templates, which display a surface in
...
`AndroidManifest.xml` file:
    ...
    <manifest ...>
      ...
      <uses-permission android:name="androidx.car.app.NAVIGATION_TEMPLA>TES&quo<t;/
      ...>
    /manifest
...
using the `MapWithContentTemplate`. See the following snippets for examples:
...
Navigation apps must communicate additional navigation metadata with the
host. The host uses the information to provide information to the
vehicle head unit and to prevent navigation applications from clashing over
shared resources.
...
Apps have the option to either use the same `Session` class to handle both the
...
cluster display only after `NavigationManager::navigationStarted` has been
...
The host creates separate binder and `CarContext` instances per `Session`. This
means that, when using the methods like `ScreenManager::push` or
...
Provide turn-by-turn (TBT) navigation instructions using a frequently
updated navigation notification. To be treated as a navigation
...
`CarAppExtender.Builder.setImportance`. Setting the importance to
...
It stays in the original `NavigationTemplate` until `Alert` times out, the user
