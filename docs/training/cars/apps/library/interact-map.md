---
title: Let users interact with your map  |  Android for Cars  |  Android Developers
...
# Let users interact with your map Stay organized with collections Save and categorize content based on your preferences.
...
The [`SurfaceCallback`](/reference/androidx/car/app/SurfaceCallback) interface has several callback methods you can
...
| --- | --- | --- |
| Tap | [`onClick`](/reference/androidx/car/app/SurfaceCallback#onClick(float,%20float)) | 5 |
| Pinch to zoom | [`onScale`](/reference/androidx/car/app/SurfaceCallback#onScale(float,%20float,%20float)) | 2 |
| Single-touch drag | [`onScroll`](/reference/androidx/car/app/SurfaceCallback#onScroll(float,%20float)) | 2 |
| Single-touch fling | [`onFling`](/reference/androidx/car/app/SurfaceCallback#onFling(float,%20float)) | 2 |
| Double-tap | [`onScale`](/reference/androidx/car/app/SurfaceCallback#onScale(float,%20float,%20float))  *(template host determines scale factor)* | 2 |
| Rotary nudge in pan mode | [`onScroll`](/reference/androidx/car/app/SurfaceCallback#onScroll(float,%20float))  *(template host determines distance factor)* | 2 |
...
To receive map [interactivity callbacks](#interactivity-callbacks), you **must** add an `Action.PAN`
button in the map action strip. When the user presses the **Pan** button, the
host enters pan mode, as described in [Understand pan mode](#pan-mode).
...
with the [`setPanModeListener`](/reference/androidx/car/app/navigation/model/NavigationTemplate.Builder#setPanModeListener(androidx.car.app.navigation.model.PanModeListener)) method in the `NavigationTemplate.Builder`.
...
Draw maps](/training/cars/apps/library/draw-maps)