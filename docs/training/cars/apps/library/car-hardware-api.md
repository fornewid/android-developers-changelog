---
title: Car Hardware APIs  |  Android for Cars  |  Android Developers
...
# Car Hardware APIs Stay organized with collections Save and categorize content based on your preferences.
...
relevant permissions](/training/permissions/declaring) needed to request the car data you want to use. These
permissions must also be [granted](/training/permissions/requesting) to you by the user. You can use
the [same code](/training/cars/apps/car-apps-library/request-permissions) on Android Auto and Android Automotive OS, rather than
creating platform-dependent flows. However, the permissions needed differ.

## CarInfo

This table describes the properties provided in the [`CarInfo`](/reference/androidx/car/app/hardware/info/CarInfo) APIs and the
permissions you must request to use them.

| Methods | Properties | Permissions: Android Auto | Permissions: AAOS | Car App API |
| --- | --- | --- | --- | --- |
| [`fetchModel`](/reference/androidx/car/app/hardware/info/CarInfo#fetchModel(java.util.concurrent.Executor,%20androidx.car.app.hardware.common.OnCarDataAvailableListener<androidx.car.app.hardware.info.Model>)) | Make, model, year |  | `android.car.permission.CAR_INFO` | 3 |
| [`fetchEnergyProfile`](/reference/androidx/car/app/hardware/info/CarInfo#fetchEnergyProfile(java.util.concurrent.Executor,%20androidx.car.app.hardware.common.OnCarDataAvailableListener<androidx.car.app.hardware.info.EnergyProfile>)) | EV connector types, fuel types | `com.google.android.gms.permission.CAR_FUEL` | `android.car.permission.CAR_INFO` | 3 |
...
error, check the[status](/reference/androidx/car/app/hardware/common/CarValue#getStatus()) of the value you requested to better understand why
the data you requested couldn't be retrieved. To learn more about the `CarInfo`
class definition, see the [reference documentation](/reference/androidx/car/app/hardware/info/CarInfo).

## CarSensors

The [`CarSensors`](/reference/androidx/car/app/hardware/info/CarSensors) class gives you access to the vehicle's accelerometer,
...
and compass is the same as you would get from the [`SensorManager` API](/reference/android/hardware/SensorManager).
...
**Caution:** On Android Automotive OS, `CarSensors` data returns
[`STATUS_UNIMPLEMENTED`](/reference/androidx/car/app/hardware/common/CarValue#STATUS_UNIMPLEMENTED()). Instead, use standard Android APIs to access the
data, such as `SensorManager` or [`LocationManager`](/reference/android/location/LocationManager).

## Test

To simulate sensor data when testing on Android Auto, see the
[Sensors](/training/cars/testing/dhu#sensors) and [Sensor configuration](/training/cars/testing/dhu#sensor-configuration) sections of the Desktop Head Unit
guide. To simulate sensor data when testing on Android Automotive OS, refer to
[Emulate hardware state](/training/cars/testing/emulator#emulate-state) in the Android Automotive OS emulator guide.

[Previous

arrow\_back

Constraints API](/training/cars/apps/library/constraints-api)

[Next

Record from the car microphone

arrow\_forward](/training/cars/apps/library/car-microphone)