...
With each [Android release](https://developer.android.com/about/versions), API and behavior changes impact how apps work on
...
- For apps built using the Android for Cars App Library, the locale of the `Configuration` provided by the `CarContext` reflects the [Per-application
  language preferences](https://developer.android.com/guide/topics/resources/app-languages) for the host app, not that of your app.

## Android 12

- On devices running Android 12 or higher, Android Auto doesn't change the [UI mode](https://developer.android.com/reference/android/content/res/Configuration#uiMode) of the device when running. See [How can I detect if Android Auto
  is running?](https://developer.android.com/training/cars/platforms/android-auto#detect-connection).
