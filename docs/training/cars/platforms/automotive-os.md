---
title: Android Automotive OS overview  |  Android for Cars  |  Android Developers
...
# Android Automotive OS overview Stay organized with collections Save and categorize content based on your preferences.
...
**Figure 1**: Android Automotive OS
...
**Note:** Android Automotive OS and [Android Auto](/training/cars/platforms/android-auto) are not exclusive. That is, a
car can run Android Automotive OS and also support Android Auto.
...
factors. See [Work with window insets and display cutouts](/training/cars/parked/automotive-os#insets-and-cutouts) for detailed
guidance.

### Audio

Android Automotive OS devices are generally fixed-volume devices. To learn more
about how this may affect your app, see [Working with fixed-volume devices](/media/platform/output#fixed-volume).
...
**Tip:** The [Android Automotive OS compatibility mode](/training/cars/platforms/automotive-os/compatibility-mode) present on some devices
addresses many of the implications of these differences.
...
affordances to enable in-app navigation to meet the [`AN-1`](/docs/quality-guidelines/car-app-quality#AN-1) quality
guideline.

#### System bar layout

As with other form factors, Android Automotive OS includes [system bars](/design/ui/mobile/guides/foundations/system-bars)
...
See [Work with window insets and display cutouts](/training/cars/parked/automotive-os#insets-and-cutouts) for detailed
guidance.

#### Immersive mode

Android Automotive OS allows OEMs to control whether or not apps can [show or
hide the system bars to enter and exit immersive mode](/develop/ui/views/layout/immersive). By preventing apps
...
![The activity blocking activity being displayed over an app not marked as
    distraction optimized.](/static/training/cars/images/activity-blocking-activity.png)


**Figure 2**: An app being blocked by UX restrictions
...
element for the `CarAppActivity`](/training/cars/apps/automotive-os#car-app-activity) of an app built using the Car App Library.
...
form factors. [TalkBack](/guide/topics/ui/accessibility/testing#talkback), [Switch Access](/guide/topics/ui/accessibility/testing#switch-access), and [Voice Access](/guide/topics/ui/accessibility/testing#voice-access) are
not available on Android Automotive OS devices.

Captioning preferences are supported on Android Automotive OS devices. See
[Adopt system caption settings](/training/tv/accessibility/system-caption-settings) for integration details.
...
Most apps use only the [default network](/develop/connectivity/network-ops/reading-network-state#listening-events) assigned to them and only stand to
...
benefit from preferences set by the OEM. See [Read network state](/develop/connectivity/network-ops/reading-network-state#additional-networks) for more
guidance on using networks other than the default.

## System features

You can detect whether a given feature is available using
[`PackageManager::hasSystemFeature`](/reference/android/content/pm/PackageManager#hasSystemFeature(java.lang.String)) and adjust your app's behavior
accordingly.

**Tip:** You can use the [Device catalog](https://play.google.com/console/about/devicecatalog/) in the Google Play Console to
determine which vehicles support any given feature by using the *Form factor*
filter with value *Car* combined with the *System feature* filter.
...
[`android.hardware.location.network`](/guide/topics/manifest/uses-feature-element#location-hw-features) system feature. Although network
location may not be available, accessing coarse location is still supported –
see [Coarse location on Android Automotive OS](/training/cars/platforms/automotive-os/coarse-location).
...
* [App Widgets](/develop/ui/views/appwidgets/overview)
  ([`FEATURE_APP_WIDGETS`](/reference/android/content/pm/PackageManager#FEATURE_APP_WIDGETS))
* [Picture-in-picture](/develop/ui/views/picture-in-picture)
  ([`FEATURE_PICTURE_IN_PICTURE`](/reference/android/content/pm/PackageManager#FEATURE_PICTURE_IN_PICTURE))

### Meet Google Play feature requirements

To be distributed to cars through Google Play, apps built for Android Automotive
OS must include a [`<uses-feature>`](/guide/topics/manifest/uses-feature-element) element in the
[`AndroidManifest.xml`](/guide/topics/manifest/manifest-intro) file for the `android.hardware.type.automotive`
feature:
...
Additionally, if your app has any [explicit feature declaration](/guide/topics/manifest/uses-feature-element#declared) with
`android:required="true"` or [implicit feature requirement](/guide/topics/manifest/uses-feature-element#implicit) for any of the
...
* [`android.hardware.wifi`](/guide/topics/manifest/uses-feature-element#wi-fi-hw-features)
* [`android.hardware.screen.portrait`](/guide/topics/manifest/uses-feature-element#screen-hw-features)
* [`android.hardware.screen.landscape`](/guide/topics/manifest/uses-feature-element#screen-hw-features)
* [`android.hardware.camera`](/guide/topics/manifest/uses-feature-element#camera-hw-features)

**Tip:** Use the [Merged Manifest view](/build/manage-manifests#inspect_the_merged_manifest_and_find_conflicts) when validating your app's feature
requirements as dependencies can introduce requirements you might not be aware
of.

For features explicitly declared with `android:required="true"`, you can do
one of the following:

* Delete the `<uses-feature>` element if the feature isn't otherwise
  implicitly required.
...
* Explicitly declare the feature with `android:required="false"`.
* Remove or update the manifest values that introduce the implicit requirement
  on the feature.
