---
title: Build templated messaging experiences for Android Auto  |  Android for Cars  |  Android Developers
...
# Build templated messaging experiences for Android Auto Stay organized with collections Save and categorize content based on your preferences.




Templated messaging experiences are in beta

At this time, anyone can publish communication apps with templated messaging experiences to internal testing and closed testing tracks on the Play Store. Publishing to open testing and production tracks will be permitted at a later date.
...
built using the [Android for Cars App Library](/training/cars/apps).

## Support notification-powered messaging experiences

All apps that support templated messaging experiences must also [extend
messaging notifications for Android Auto](/training/cars/communication/notification-messaging). This
...
category](/training/cars/apps#supported-app-categories) in the intent filter of its
...
**Note:** If your app supports both messaging and [calling](/training/cars/communication/calling), include both
...
Because the [`ConversationItem`](/reference/androidx/car/app/messaging/model/ConversationItem) API is only supported on Car
API 7 or higher, you should also set the `minCarApiLevel` metadata to that
value. See [Car App API Level](/training/cars/apps#api-level) for more information.
...
support](/training/cars/apps/auto#declare-android-auto-support), verify that both the `notification` and
...
If your app can be set as the [default SMS handler](/reference/android/provider/Telephony#creating-an-sms-app),
...
[`ConversationItem`](/reference/androidx/car/app/messaging/model/ConversationItem) objects in a
[`ListTemplate`](/reference/androidx/car/app/model/ListTemplate) or
...
[`ConversationCallbackDelegate`](/reference/androidx/car/app/messaging/model/ConversationCallbackDelegate) you supply when
building the `ConversationItem`.

**Tip:** You can re-use the logic for marking messages as read and replying that
enables your app's notification-powered experience.

If your app provides [conversation shortcuts](/develop/ui/views/notifications/conversations#api-shortcuts), verify
...
include the new messages by calling [`invalidate()`](/reference/androidx/car/app/Screen#invalidate()). See [Refresh
the contents of a template](/training/cars/apps#refresh-template).
...
`Screen` that displays it. See [The lifecycle of a screen](/training/cars/apps#screen-lifecycle).

To prevent a notification from appearing as a HUN, set the priority to
[`IMPORTANCE_DEFAULT`](/reference/androidx/core/app/NotificationManagerCompat#IMPORTANCE_DEFAULT) or lower.
