---
title: Request runtime permissions  |  Privacy  |  Android Developers
...
access. These steps are part of the [workflow for using permissions](/training/basics/permissions#workflow).

If you declare any [dangerous permissions](/guide/topics/permissions/overview#dangerous_permissions), and if your app is installed on
...
**Note:** Some permissions guard access to system resources that are particularly
sensitive or are not directly related to user privacy. There's a [different
process for requesting these special permissions](/training/permissions/requesting-special).
...
* Don't assume any system behavior. For example, don't assume that
  permissions appear in the same *permission group*. A permission group
  merely helps the system minimize the number of system dialogs that are
  presented to the user when an app requests closely related permissions.

## Workflow for requesting permissions

Before you declare and request runtime permissions in your app, [evaluate
whether your app needs to do so](/training/permissions/evaluating). You can fulfill many use cases in your
...
1. [**Declare** the permissions](/training/permissions/declaring) your app might need to request in your
   app's manifest file.
2. **Design** your app's user experience (UX) so that specific actions in your
   app are associated with specific runtime permissions. Let users know which
   actions might require them to grant permission for your app to access
   private user data.
3. [**Wait** for the user](#principles) to invoke the task or action in your
   app that requires access to specific private user data. At that time, your
   app can request the runtime permission that is required for accessing that
   data.
...
5. [**Check** whether your app should show a rationale](#explain) to the user,
...
6. [**Request** the runtime permission](#request-permission) your app requires
   to access the private user data. The system displays a runtime permission
   prompt, such as the one shown on the [permissions overview page](/guide/topics/permissions/overview#fig-runtime).
...
8. If the user granted the permission to your app, you can **access** the
   ...
   degrade your app experience](#handle-denial) so that it provides
...
![A flowchart illustrating the decision process for requesting runtime permissions, starting with an app action, checking if permission is granted, checking if rationale is needed, requesting permission, and handling the user's response.](/static/images/training/permissions/workflow-runtime.svg)
...
that permission into the [`ContextCompat.checkSelfPermission()`](/reference/androidx/core/content/ContextCompat#checkSelfPermission(android.content.Context,%20java.lang.String)) method.
This method returns either [`PERMISSION_GRANTED`](/reference/android/content/pm/PackageManager#PERMISSION_GRANTED) or
[`PERMISSION_DENIED`](/reference/android/content/pm/PackageManager#PERMISSION_DENIED), depending on whether your app has the permission.

## Explain why your app needs the permission

The permissions dialog shown by the system when you call
[`requestPermissions()`](/reference/androidx/core/app/ActivityCompat#requestPermissions(android.app.Activity,%20java.lang.String%5B%5D,%20int)) says what permission your app wants, but doesn't
...
**Note:** Starting in Android 12 (API level 31), [privacy indicators](/training/permissions/explaining-access#indicators) notify
the user whenever applications access the microphone or camera.
...
call [`shouldShowRequestPermissionRationale()`](/reference/androidx/core/app/ActivityCompat#shouldShowRequestPermissionRationale(android.app.Activity,%20java.lang.String)). If this method returns
...
or camera, consider [explaining why your app needs access](/training/permissions/explaining-access) to this
information.
...
To do this, use the [`RequestPermission`](/reference/androidx/activity/result/contract/ActivityResultContracts.RequestPermission) contract, included in an AndroidX
library, where you [allow the system to manage the permission request code](#allow-system-manage-request-code)
...
[manage a request code yourself](#manage-request-code-yourself) as part of the permission request and
...
* To request multiple permissions at the same time, use
  [`RequestMultiplePermissions`](/reference/androidx/activity/result/contract/ActivityResultContracts.RequestMultiplePermissions).
...
1. In your activity or fragment's initialization logic, pass in an
   implementation of [`ActivityResultCallback`](/reference/androidx/activity/result/ActivityResultCallback) into a call to
   [`registerForActivityResult()`](/reference/androidx/activity/result/ActivityResultCaller#registerForActivityResult(androidx.activity.result.contract.ActivityResultContract%3CI,%20O%3E,%20androidx.activity.result.ActivityResultCallback%3CO%3E)). The `ActivityResultCallback` defines
   ...
   is of type [`ActivityResultLauncher`](/reference/androidx/activity/result/ActivityResultLauncher).
2. To display the system permissions dialog when necessary, call the
   [`launch()`](/reference/androidx/activity/result/ActivityResultLauncher#launch(I)) method on the instance of `ActivityResultLauncher` that
   ...
   **Note:** Your app *cannot* customize the dialog that appears
   ...
   [permission
   group](/guide/topics/permissions/overview#group) associated with the permission that you requested. This
...
code](#allow-system-manage-request-code), you can manage the permission request code yourself. To do so,
include the request code in a call to [`requestPermissions()`](/reference/androidx/core/app/ActivityCompat#requestPermissions(android.app.Activity,%20java.lang.String%5B%5D,%20int)).

**Note:** Your app cannot customize the dialog that appears when you call
`requestPermissions()`. The text in the system permission dialog references a
[permission group](/guide/topics/permissions/overview#perm-groups), but this permission grouping is designed for system
ease-of-use. Your app must not rely on permissions being within or outside of a
...
invokes your app's implementation of [`onRequestPermissionsResult()`](/reference/androidx/core/app/ActivityCompat.OnRequestPermissionsResultCallback#onRequestPermissionsResult(int,%20java.lang.String%5B%5D,%20int%5B%5D)). The
...
for any other [runtime permission](/training/permissions/requesting). One important difference when it comes
...
* Your app is running a foreground service. When a foreground service is
  ...
  service type](/guide/topics/manifest/service-element#foregroundservicetype) of `location`, as shown in the following code snippet. On
...
[`ACCESS_COARSE_LOCATION`](/reference/android/Manifest.permission#ACCESS_COARSE_LOCATION) permission or the
[`ACCESS_FINE_LOCATION`](/reference/android/Manifest.permission#ACCESS_FINE_LOCATION) permission, as shown in the following snippet:
...
* Within an IoT app, a feature lets users configure their home devices such
  that they turn off when the user leaves their home and turn back on when
...
the [foreground location](#foreground-location) section. The background
location accuracy is the same as the [foreground location accuracy](/training/location/permissions#accuracy), which
depends on the location permissions that your app declares.

On Android 10 (API level 29) and higher, you must declare the
[`ACCESS_BACKGROUND_LOCATION`](/reference/android/Manifest.permission#ACCESS_BACKGROUND_LOCATION) permission in your app's manifest to request
...
core functionality and meet related policy requirements.
...
To provide the best user experience when asking for app permissions, also see [App permissions best practices](/training/permissions/usage-notes).
...
reference page](/reference/android/Manifest.permission#constants_1).

## One-time permissions

![The option called 'Only this time' is the second of three buttons in
    the dialog.](/static/images/training/permissions/one-time-prompt.svg)
...
* If the user sends your app to the background, your app can continue to
  access the data for a short period of time.
* If you launch a foreground service while the activity is visible, and the
  user then moves your app to the background, your app can continue to access
  the data until the foreground service stops.
...
**Note:** If your app already follows [best practices](/training/permissions/usage-notes) when it requests runtime
permissions, you don't need to add or change any logic in your app to support
one-time permissions.
...
* An API where you can proactively [remove your app's access](#remove-app-access) to an
  unused runtime permission.
* A system mechanism that automatically [resets the permissions of unused
  apps](#auto-reset-permissions-unused-apps).
...
[`revokeSelfPermissionOnKill()`](/reference/android/content/Context#revokeSelfPermissionOnKill(java.lang.String)). To remove access to a group of runtime
permissions at the same time, pass a collection of permission names into
[`revokeSelfPermissionsOnKill()`](/reference/android/content/Context#revokeSelfPermissionsOnKill(java.util.Collection%3Cjava.lang.String%3E)). The permission removal process happens
asynchronously and kills all processes associated with your app's UID.

**Note:** For system settings to show that your app doesn't access data in a
particular [permission group](/reference/android/Manifest.permission_group), you must remove access to **all**
permissions in that permission group. In this case, it can be helpful to call
`revokeSelfPermissionsOnKill()` and pass in multiple permissions within the
permission group.
...
the guide about [app hibernation](/topic/performance/app-hibernation).
...
practices](/codelabs/android-privacy-codelab).

[Previous

arrow\_back

Declare app permissions](/training/permissions/declaring)

[Next

Request special permissions

arrow\_forward](/training/permissions/requesting-special)