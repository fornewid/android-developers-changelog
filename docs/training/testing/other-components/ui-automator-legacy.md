---
title: Use the UI Automator legacy API  |  Test your app on Android  |  Android Developers
...
# Use the UI Automator legacy API Stay organized with collections Save and categorize content based on your preferences.



**Note:** A [new version of UI Automator](/training/testing/other-components/ui-automator) provides a modern API.
We strongly recommended you use the new API for any new UI Automator
development.
...
with visible elements on a device, regardless of which [`Activity`](/reference/android/app/Activity) is in
...
with the [`AndroidJUnitRunner`](/reference/androidx/test/runner/AndroidJUnitRunner) test runner. It's well-suited for writing
...
* An API to retrieve state information and perform operations on the target
  device. For more information, see [Accessing device state](#accessing-device-state).
* APIs that support cross-app UI testing. For more information, see [UI
  Automator APIs](#ui-automator).

**Note:** UI Automator and Espresso have some feature overlap but Espresso has more
synchronization mechanisms so it's preferred for common UI tests.

## Accessing device state

The UI Automator testing framework provides a [`UiDevice`](/reference/androidx/test/uiautomator/UiDevice) class to access
...
* [`UiObject2`](/reference/androidx/test/uiautomator/UiObject2): Represents a UI element that is visible on the device.
* [`BySelector`](/reference/androidx/test/uiautomator/BySelector): Specifies criteria for matching UI elements.
* [`By`](/reference/androidx/test/uiautomator/By): Constructs [`BySelector`](/reference/androidx/test/uiautomator/BySelector) in a concise manner.
* [`Configurator`](/reference/androidx/test/uiautomator/Configurator): Lets you set key parameters for running UI Automator tests.

**Note:** Some existing APIs, including [`UiCollection`](/reference/androidx/test/uiautomator/UiCollection), [`UiObject`](/reference/androidx/test/uiautomator/UiObject),
[`UiScrollable`](/reference/androidx/test/uiautomator/UiScrollable), and [`UiSelector`](/reference/androidx/test/uiautomator/UiSelector) are being deprecated in a future
release. For continued support, you should instead use only the APIs mentioned
in this document.
...
for AndroidX Test](/training/testing/instrumented-tests/androidx-test-libraries/test-setup).
...
**Note:** **See [release notes](/jetpack/androidx/releases/test-uiautomator) for the latest updates.**
...
[`android:contentDescription`](/reference/android/view/View#attr_android:contentDescription) values, or both.
...
2. Hover over the snapshot in the left-hand panel to see the UI components
   identified by the `uiautomatorviewer` tool. The properties are listed in the
   lower
   right-hand panel and the layout hierarchy in the upper right-hand panel.
3. Optionally, click on the **Toggle NAF Nodes** button to see UI components
   that are non-accessible to UI Automator. Only limited information might be
   available for these components.

To learn about the common types of UI components provided by Android, see [User
Interface](/guide/topics/ui).

### Ensure your activity is accessible

The UI Automator test framework performs better on apps that have implemented
Android accessibility features. When you use UI elements of type [`View`](/reference/android/view/View), or
...
1. Create a concrete class that extends [ExploreByTouchHelper](/reference/androidx/customview/widget/ExploreByTouchHelper).
2. Associate an instance of your new class with a specific custom UI element by
   calling [setAccessibilityDelegate()](/reference/androidx/core/view/ViewCompat#setAccessibilityDelegate(android.view.View,%20android.support.v4.view.AccessibilityDelegateCompat)).

For additional guidance on adding accessibility features to custom view
elements, see [Building Accessible Custom Views](/guide/topics/ui/accessibility/custom-views). To learn more about
general best practices for accessibility on Android, see [Making Apps More
Accessible](/reference/android/app/Instrumentation).
...
assertions and annotations, see [Create an Instrumented Unit Test Class](/training/testing/unit-testing/instrumented-unit-tests#build).

Add the @RunWith(AndroidJUnit4.class) annotation at the beginning of your test
class definition. You also need to specify the [AndroidJUnitRunner](/reference/androidx/test/runner/AndroidJUnitRunner) class,
provided in AndroidX Test, as your default test runner. This step is described
in more detail in [Run UI Automator tests on a device or emulator](/training/testing/ui-testing/uiautomator-testing#run).

Implement the following programming model in your UI Automator test class:

1. Get a `UiDevice` object to access the device you want to test, by calling
   the [getInstance()](/reference/androidx/test/uiautomator/UiDevice#getInstance(android.app.Instrumentation)) method and passing it an [Instrumentation](/reference/android/app/Instrumentation) object as
   the argument.
2. Get a [`UiObject2`](/reference/androidx/test/uiautomator/UiObject2) object to access a UI component that is displayed on the
   device (for example, the current view in the foreground), by calling the
   [findObject()](/reference/androidx/test/uiautomator/UiDevice#findObject(androidx.test.uiautomator.UiSelector)) method.
3. Simulate a specific user interaction to perform on that UI component, by
   calling a `UiObject2` method; for example, call
   [scrollUntil()](/reference/androidx/test/uiautomator/UiObject2#scrollUntil(androidx.test.uiautomator.Direction,androidx.test.uiautomator.Condition%3C?%20super%20androidx.test.uiautomator.UiObject2,U%3E)) to scroll, and [setText()](/reference/androidx/test/uiautomator/UiObject2#setText(java.lang.String)) to edit a text field.
   You can call on the APIs in steps 2 and 3
   repeatedly as necessary to test more complex user interactions that involve
   multiple UI components or sequences of user actions.
4. Check that the UI reflects the expected state or behavior, after these user
...
The [`UiDevice`](/reference/androidx/test/uiautomator/UiDevice) object is the primary way you access and manipulate the
...
[`By`](/reference/androidx/test/uiautomator/By) class to construct a [`BySelector`](/reference/androidx/test/uiautomator/BySelector) instance. `BySelector`
...
You can use the [`hasChild()`](/reference/androidx/test/uiautomator/By#hasChild(androidx.test.uiautomator.BySelector)) or [`hasDescendant()`](/reference/androidx/test/uiautomator/By#hasDescendant(androidx.test.uiautomator.BySelector)) method to nest
multiple `BySelector` instances. For example, the following code example shows
how your test might specify a search to find the first [`ListView`](/reference/android/widget/ListView) that
...
clear them, call the [`checked()`](/reference/androidx/test/uiautomator/By#checked(boolean)) method with the argument set to true.
...
* [`click()`](/reference/androidx/test/uiautomator/UiObject2#click()) : Clicks the center of the visible bounds of the UI element.
* [`drag()`](/reference/androidx/test/uiautomator/UiObject2#drag(android.graphics.Point,int)) : Drags this object to arbitrary coordinates.
* [`setText()`](/reference/androidx/test/uiautomator/UiObject2#setText(java.lang.String)) : Sets the text in an editable field, after clearing the
  field's content. Conversely, the [`clear()`](/reference/androidx/test/uiautomator/UiObject2#clear()) method clears the existing text
  in an editable field.
* [`swipe()`](/reference/androidx/test/uiautomator/UiObject2#swipe(androidx.test.uiautomator.Direction,float)) : Performs the swipe action toward specified direction.
* [`scrollUntil()`](/reference/androidx/test/uiautomator/UiObject2#scrollUntil(androidx.test.uiautomator.Direction,androidx.test.uiautomator.Condition%3C?%20super%20androidx.test.uiautomator.UiObject2,U%3E)): Performs the scroll action toward specified direction
  until [`Condition`](/reference/androidx/test/uiautomator/Condition) or [`EventCondition`](/reference/androidx/test/uiautomator/EventCondition) is satisfied.

The UI Automator testing framework lets you send an [Intent](/reference/android/content/Intent) or launch
an [Activity](/reference/android/app/Activity) without using shell commands, by getting a [Context](/reference/android/content/Context)
object through [`getContext()`](/reference/android/app/Instrumentation#getContext()).

The following snippet shows how your test can use an [Intent](/reference/android/content/Intent) to launch the
...
The [InstrumentationTestCase](/reference/android/test/InstrumentationTestCase) extends [TestCase](/reference/junit/framework/TestCase), so you can use
...
You can run UI Automator tests from [Android Studio](/studio) or from the
...
![Turn off disturb](/static/images/testing/uiautomator_turn_off_disturb.gif)


**Figure 1.** UI Automator turns off Do Not Disturb mode on a
test device.
...
* [UI Automator API Reference](/reference/androidx/test/uiautomator/package-summary)
