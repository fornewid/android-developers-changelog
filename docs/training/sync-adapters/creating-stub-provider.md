---
title: Create a stub content provider  |  Connectivity  |  Android Developers
...
# Create a stub content provider Stay organized with collections Save and categorize content based on your preferences.
...
`ContentProvider` and stub out its required methods. The following
...
:   Sets a flag that indicates that the provider is syncable. If you set this flag to
    `true`, you don't have to call `setIsSyncable()` in your code. The flag allows the sync adapter framework to make data
