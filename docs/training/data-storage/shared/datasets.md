---
title: Access shared datasets  |  App data and files  |  Android Developers
...
# Access shared datasets Stay organized with collections Save and categorize content based on your preferences.
...
* To grant access to a specific set of apps on a device, pass the package names
  of these apps into
  [`allowPackageAccess()`](/reference/android/app/blob/BlobStoreManager.Session#allowPackageAccess(java.lang.String,%20byte%5B%5D)).
* To allow only apps whose certificates are signed using the same key as the one
  used for your app—such as an app suite that you manage—call
  [`allowSameSignatureAccess()`](/reference/android/app/blob/BlobStoreManager.Session#allowSameSignatureAccess()).
* To grant access to all apps on a device, call
  [`allowPublicAccess()`](/reference/android/app/blob/BlobStoreManager.Session#allowPublicAccess()).
