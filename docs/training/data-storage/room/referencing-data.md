...
Sometimes, you need your app to store a custom data type in a single database column. You support custom types by providing *type converters* . These are functions that tell Room how to convert custom types to and from known types that Room can persist. You identify type converters by using the [`@ColumnTypeConverter`](https://developer.android.com/reference/kotlin/androidx/room3/ColumnTypeConverter) annotation.

> [!NOTE]
> **Note:** Room includes a default type converter for persisting enums. Existing type converters take precedence over the default. However, if you haven't defined a type converter for enums, you don't need to define one.

Suppose you need to persist instances of [`Date`](https://developer.android.com/reference/java/util/Date) in your Room database. Room can't natively persist `Date` objects, so you need to define type converters:
...
This example defines two type converter functions: one that converts a `Date` object to a `Long` object, and one that converts a `Long` object back to a `Date` object. Because Room can persist `Long` objects, it can use these converters to persist `Date` objects.

Next, you add the [`@ColumnTypeConverters`](https://developer.android.com/reference/kotlin/androidx/room3/ColumnTypeConverters) annotation to the `AppDatabase` class so that Room can use the converter class that you've defined:
...
Because you annotated `AppDatabase` with `@ColumnTypeConverters` in this example, Room can use the defined type converter everywhere. To scope type converters to specific entities or DAOs instead, annotate your `@Entity` or `@Dao` classes with `@ColumnTypeConverters`.

### Control type converter initialization

Ordinarily, Room instantiates type converters for you. However, if you need to pass additional dependencies to your type converter classes, your app must directly control their initialization. In that case, annotate your converter class with [`@ProvidedColumnTypeConverter`](https://developer.android.com/reference/kotlin/androidx/room3/ProvidedColumnTypeConverter):
...
In addition to declaring your converter class in `@ColumnTypeConverters`, use the [`RoomDatabase.Builder.addColumnTypeConverter`](https://developer.android.com/reference/kotlin/androidx/room3/RoomDatabase.Builder#addColumnTypeConverter(kotlin.Any)) function to pass an instance of your converter class to the `RoomDatabase` builder:
...
For example, consider a UI that loads a list of `Book` objects, with each book having an `Author` object. You might initially design your queries to use lazy loading to have instances of `Book` retrieve the author. The first retrieval of the `author` property queries the database. Some time later, you realize that you need to display the author name in your app's UI, as well. You can access this name, as shown in the following code snippet:
...
If you query author information ahead of time but don't need it, it's difficult to change how data is loaded. For example, if your app's UI no longer needs to display `Author` information, your app effectively loads data that it doesn't display, wasting valuable memory space. Your app's efficiency degrades even further if the `Author` class references another table, such as `Books`.
