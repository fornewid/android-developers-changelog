---
title: Emulator download archives  |  Android Studio  |  Android Developers
...
# Emulator download archives Stay organized with collections Save and categorize content based on your preferences.
...
1. Locate your SDK installation directory. The default location of this directory varies by
   platform:
   * On Windows, it's the `%LocalAppData%\Android\Sdk` directory. This normally
     expands to `C:\Users\<username>\AppData\Local\Android\Sdk`, although it might
...
2. Rename the existing `emulator` directory in the SDK installation directory, because
   you'll unzip the newly downloaded `emulator` directory here in the next step. For
   example, call it `emulator_original`.
3. Unzip the emulator zip file you downloaded. Move the contents into the SDK installation
   directory.
...
5. Paste the `package.xml` file from the `emulator_original` directory into
   the new `emulator` directory.
