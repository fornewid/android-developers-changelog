---
title: Test platform code with atest  |  Android Studio for Platform  |  Android Developers
...
# Test platform code with atest Stay organized with collections Save and categorize content based on your preferences.
...
* Initialize your build environment using `source build/envsetup.sh` and
  `lunch`.
...
* **Gutter run icons:** Click the **Run** icon play\_circle
...
2. Find the `atest` configuration you want to modify or create a new one by
   clicking the **+** button and selecting **atest**.
3. Specify the test module, class, method, and add any necessary `atest`
   command-line options.

## Tips for testing

* **Targeted testing:** Run only the specific tests you need to save time. Use
  the format `Module:Class#Method` or other `atest` filters.
