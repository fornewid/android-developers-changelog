---
title: Create or import a project  |  Android Studio for Platform  |  Android Developers
...
# Create or import a project Stay organized with collections Save and categorize content based on your preferences.
...
3. Fill in the project configuration details in the wizard: ![ASfP New Project
   ...
   * **Module paths:** Specify the absolute path to the root of your AOSP
     source code checkout (for example, `/path/to/aosp`).
   * **Lunch target:** Enter the lunch target you use for building (for
     example, `aosp_arm64-eng`).
   ...
   * **Directories / modules:** List the initial directories or modules you
     ...
     relative paths from the repository root (for example, `frameworks/base,
...
2. In the **Project Name** field, navigate to and select the directory
   containing the existing `.asfp-project` file you want to open. The fields in
   the wizard will populate based on the selected `.asfp-project` file.
