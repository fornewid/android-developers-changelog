...
Emulator](/studio/run/emulator) and [Start the emulator from the command line](/studio/run/emulator-commandline).
...
1. Open a terminal window and enter the following command:
   ...
   Pixel8\_API\_34:5554. Also, the `adb devices` command prints a list of running
...
2. After the console displays **OK**, enter the `auth auth_token` command.
   ...
   Before you can enter [console commands](/studio/run/emulator-console#querycontrol), the emulator
...
3. After you're connected to the console, enter
   [console commands](/studio/run/emulator-console#querycontrol).
...
#### EV\_ABS — Absolute Motion Events
...
#### EV\_REL & EV\_SW — Motion & Hardware Switches
...
* **event text message**: Sends a string of characters that simulate keypresses.
  The message must be a UTF-8 string. Unicode posts are reverse-mapped according
  to the current device keyboard, and unsupported characters are discarded
  silently.
...
* **`network capture {start| stop} file`**
  ...
  **Note:** Starting with Android Emulator 36.5 and later, Wi-Fi connection
  ...
  versions, see [Capture network traffic](/studio/run/emulator-networking-advanced#capture-network) in the Advanced
...
* **`gsm {call| accept| cancel| busy} phonenumber`**
  ...
  + **accept**: Accepts an inbound call from phonenumber and changes the call
    state to active. You can change a call state to active only when its current
    state is waiting or held.
  + **cancel**: Terminates an inbound phone call from or outbound phone call
    to phonenumber.
  + **busy**: Closes an outbound call to phonenumber and changes the call
    state to busy. You can change a call state to busy only when its current
    state is waiting.
...
* **cdma prl\_version version**: Dumps the current preferred roaming list (PRL)
...
about [fingerprint simulation and validation](/studio/run/emulator-console#finger-print).
...
* **`screenrecord {start| stop} file`**
  Starts or stops the display recording process.
  + **start file**: Starts video recording. The output file must end with
    the mandatory `.webm` extension. The captured video is saved directly under
    your AVD's console output directory:
    `~/.android/avd/avd_name.avd/console_out/file`.
...
* **`automation {record| stop-record| play| stop-play}`**
  Manages automated macro recordings.
...
* **`physics {record-gt| stop}`**
  Manages physical pose model recordings.
...
* **`proxy {set| clear}`**
  ...
  + **clear**: Resets proxy configurations back to default.
    **Note:** Proxy set operations fail in headless execution environments.
...
2. Set up your app to accept [fingerprint authentication](/about/versions/marshmallow/android-6.0#fingerprint-authentication).
