...
[Run apps on the Android Emulator](https://developer.android.com/studio/run/emulator) and
...
4. After the console displays `OK`, enter the `auth
   auth_token` command.
5. Before you can enter [console commands](https://developer.android.com/studio/run/emulator-console#querycontrol), the emulator console requires authentication. `auth_token` must match the contents of the `.emulator_console_auth_token` file in your home directory.
6. If that file doesn't exist, the `telnet localhost console-port` command creates the file, which contains a randomly generated authentication token. To disable authentication, delete the token from the `.emulator_console_auth_token` file or create an empty file if it doesn't exist.
...
8. Enter `help`, `help command`, or `help-verbose` to see a list of console commands and learn about specific commands.
...
| `avd snapshot {list|save name|load name|delete name}` | Saves and restores the device state in snapshots, as follows: - `list`: Lists all saved snapshots. - `save name`: Saves the snapshot as <var translate="no">name</var>. - `load name`: Loads the named snapshot. - `delete name`: Deletes the named snapshot. The following example saves a snapshot with the name `firstactivitysnapshot`: ``` avd snapshot save firstactivitysnapshot ``` |
...
2. Set up your app to accept [fingerprint
   authentication](https://developer.android.com/about/versions/marshmallow/android-6.0#fingerprint-authentication). After you perform this setup, your device displays the fingerprint authentication screen.
