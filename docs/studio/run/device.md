...
2. Set up your system to detect your device.
   ...
   - **Windows** : Install a USB driver for ADB (if applicable). For an installation guide and links to OEM drivers, see [Install OEM USB drivers](https://developer.android.com/studio/run/oem-usb).
...
When you're set up and plugged in over USB, click **Run** ![](https://developer.android.com/static/studio/images/buttons/toolbar-run.png) in Android Studio to [build and run your app](https://developer.android.com/studio/run) on the device.
...
Android 17, alongside [adb 37.0.0](https://developer.android.com/tools/releases/platform-tools) introduces adb Wi-Fi 2.0 which solves many of the usability issues with the previous version. Notably, the device will automatically connect to the workstation when the device connects to a wireless debugging trusted network.
...
2. If the Connection Assistant can detect the secondary device, assume that the problem is with the primary device, and [check whether the device is set up for development](https://developer.android.com/studio/run/device#check-device-setup).
...
Check that the workstation and device meet the [prerequisites](https://developer.android.com/studio/run/device#wireless).

#### Check adb troubleshooting guide

Follow the adb troubleshooting [guide](https://developer.android.com/tools/adb#wireless-android11-troubleshoot).
