---
title: Interconnecting Emulator instances  |  Android Studio  |  Android Developers
...
[Network Service Discovery](/develop/connectivity/wifi/use-nsd)
(NSD) and
[Wi-Fi Direct](/develop/connectivity/wifi/wifip2p).
...
[Start and stop a console session](/studio/run/emulator-console#console-session)
...
commands](/studio/run/emulator-console).
...
port forwarding](/studio/command-line/adb#forwardports) in the `adb`
documentation.
...
2. On the B console, set up a redirection from A: `localhost:<localPort>` to
