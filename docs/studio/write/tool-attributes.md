---
title: Tools attributes reference  |  Android Studio  |  Android Developers
...
[`@TargetApi`](/reference/android/annotation/TargetApi) annotation in Java
...
[`GridLayout`](/reference/android/widget/GridLayout) is only available on
API level 14 and higher, but you know this layout is not used in your code
...
(However, note that we recommend that you use [`GridLayout`](/reference/androidx/gridlayout/widget/GridLayout)
...
qualifier](/guide/topics/resources/providing-resources#LocaleQualifier).
...
![The tools:text attribute sets Google Voice as the value for the layout
      preview](/static/studio/images/write/tools-attribute-text_2x.png)


**Figure 1.** The `tools:text`
attribute sets "Google Voice" as the value for the layout preview.
...
When using the [Layout Editor](/studio/write/layout-editor) in
...
icon](/static/studio/images/buttons/layout-tools-attr.png) next to the
...
Editor](/studio/write/layout-editor#sample-data)
...
![Quickfix
    for the onClick attribute works only if you've set tools:context](/static/studio/images/write/tools-attribute-context_2x.png)


**Figure 2.** Quickfix for the
`onClick`
attribute works only if you've set `tools:context`.
...
[Layout Editor toolbar](/studio/write/layout-editor#change-appearance).
...
For a given `RecyclerView`, this attribute specifies the number of items the
Layout Editor should render in the
**Preview** window.
...
[`<include>`](/training/improving-layouts/reusing-layouts), so you can
...
[app bar](/training/appbar). The value is one or more menu IDs,
...
[resource shrinking](/studio/build/shrink-code#shrink-resources).
...
* **Safe mode:** Keep all resources that are explicitly cited and
  that *might* be referenced dynamically with a call to
  [`Resources.getIdentifier()`](/reference/android/content/res/Resources#getIdentifier(java.lang.String,%20java.lang.String,%20java.lang.String)).
* **Strict mode:** Keep only the resources
...
When you enable strict mode, you might need to use [`tools:keep`](#toolskeep)
to keep resources that were removed but that you actually want, and use
[`tools:discard`](#toolsdiscard) to explicitly remove even more resources.

For more information, see
[Shrink your resources](/studio/build/shrink-code#shrink-resources).
