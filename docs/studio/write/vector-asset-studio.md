---
title: Add multi-density vector graphics  |  Android Studio  |  Android Developers
...
[Support Library Backward Compatibility](#sloption).
The `VectorDrawableCompat`
...
[tint](/training/material/drawables#DrawableTint) to the vector drawable
...
| --- | --- | --- | --- | --- | --- |
| **PNG generation** | Vector and raster | [Subset supported](#apilevel) | SVG: [Android plugin for Gradle](/studio/releases/gradle-plugin) 1.5.0 or higher  PSD: Android Studio 2.2 or higher | Default | Variety of coding techniques supported |
...
2. In the *Project* window, select the
   [Android view](/studio/projects#ProjectFiles).
3. Right-click the **res** folder and select **New** >
   **Vector Asset**.
...
![](/static/images/tools/vas-materialicon_2-2_2x.png)
...
3. In the **Android Plugin Version** field, change the Android Plugin for Gradle
   version to **1.5.0** or higher, and click **OK**.

Gradle syncs the project.

4. In the [Android view](/studio/projects#ProjectFiles)
...
5. Continue with [Importing a Vector Graphic](#importing).
...
3. The **Select Icon** dialog appears. You
   ...
   ![](/static/studio/images/write/vector-asset-studio-icon-filter_2-3_2x.png)
...
4. Optionally change the resource name, size, opacity, and Right-To-Left (RTL) mirroring setting:
   ...
   * **Enable auto mirroring for RTL layout** - Select this option if you want a
     ...
     supported on Android 5.0 (API level 21) and higher, and with the Support Library.- Click **Next**.
...
![](/static/images/tools/vas-svgerror_2-2_2x.png)
...
3. Optionally change the resource name, size, opacity, and Right-To-Left (RTL) mirroring
   setting:
...
![](/static/images/tools/vas-layout_2-2_2x.png)
...
This example uses a Phone/Tablet project generated with the New Project Wizard.- In the [Android view](/studio/projects#ProjectFiles) of
  the *Project* window, double-click a layout XML file, such as
  `content_main.xml`.
- Click the **Design** tab to display the
  [Layout Editor](/studio/write/layout-editor).
- Drag the `ImageButton`
  widget from the *Palette* window onto the Layout Editor.
- In the *Resources* dialog, select **Drawable** in the left pane, and
...
- To change the color of the image to the accent color defined in the theme,
  ...
  **tint** property and click **…** .
- In the *Resources* dialog, select
  **Color** in the left pane, and then select **colorAccent**.
  Click **OK**.
...
* Occasionally, you might need to typecast the drawable resource to its exact class, such as
  when you need to use specific features of the
  `VectorDrawable`
...
[not supported](#apilevel) on Android 4.4 (API level 20) and lower, your vector and PNG
...
![](/static/images/tools/vas-codepreview_2-2_2x.png)

**Figure 5**. A vector XML file displayed in the Code Editor and
the Preview window.

2. Edit the XML code based on what’s supported by the minimum API level:

* Android 5.0 (API level 21) and higher - Vector Asset Studio supports all of the
  `Drawable` and
  `VectorDrawable`
  elements. You can add XML elements and change values.
* Android 4.4 (API level 20) and lower - Vector Asset Studio supports all of the
  `Drawable`
  elements and a subset of the
  `VectorDrawable`
  elements. See
  [Vector Drawable Backward-Compatibility Solutions](#apilevel) for a list. You can change values in
  the generated code and add XML elements that are supported.

3. Build the project and check that the vector drawable and corresponding raster images
   look the same.

Remember that the generated PNGs could display differently in the *Preview* window than
...
[Project Files view](/studio/projects#ProjectFiles) of the
*Project*
window; the left margin of the Code Editor also displays the PNG image when your code refers to
the drawable, as shown in figure 6.

![](/static/images/tools/vas-imageincode_2-2_2x.png)

**Figure 6**. A PNG image displayed in the left margin of the
Code Editor.
...
1. In the *Project* window, delete the generated vector XML file by selecting the file and
   pressing the **Delete** key (or select **Edit** >
   **Delete**).

The *Safe Delete* dialog appears.

2. Optionally select options to find where the file is used in the project, and click
   **OK**.

Android Studio deletes the file from the project and the drive. However, if you chose to search
...
* Create one APK that includes both the vector drawables and the corresponding
  raster representations. This solution is the simplest to implement.
* Create separate APKs for different API levels. When you don’t include the
  ...
  [Multiple APK Support](/google/play/publishing/multiple-apks).
...
* A PSD color mode of bitmap, grayscale, indexed, RGB, Lab, or CMYK.* A color depth of 8, 16, or 32 bits.
...
* Clipping masks, if the clipping base is another shape.* Shape operations, including merge/add, intersect, subtract, and
    exclude.
...
* If a stroke is dashed, clipped using a clipping base, or uses an alignment
  different from center, Vector Asset Studio converts it into a fill shape in the
  vector drawable.
...
* Vector Asset Studio multiplies the fill opacity with the layer opacity to
  compute the fill alpha.* The tool multiplies the opacity of the clipping base (if there is a clipping
    base) with the fill alpha to compute the final fill alpha.* The tool multiplies the stroke opacity with the layer opacity to compute the
      stroke alpha.* The tool multiplies the opacity of the clipping base (if there is a clipping
        base) with the stroke alpha to compute the final stroke alpha.
...
* Vector Asset Studio preserves the name of the layers in the vector drawable
  file.

Not supported:

* Layer effects.* Adjustment and text layers.* Blending modes (ignored).
