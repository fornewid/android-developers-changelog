...
:
    ...
    To learn more, read about [Play Feature Delivery](https://developer.android.com/studio/projects/dynamic-delivery).
...
**manifests**
:
    Contains the `https://developer.android.com/guide/topics/manifest/manifest-intro` file.
...
**res**
:
    Contains all non-code resources, such as UI strings and bitmap images, divided into corresponding subdirectories. For more information about possible resource types, see [App resources overview](https://developer.android.com/guide/topics/resources/providing-resources).
...
:
    ...
    :
        ...
        `androidTest/`
        :
            Contains code for instrumentation tests that run on an Android device. For more information, see [Test in Android Studio](https://developer.android.com/studio/test/test-in-android-studio).


        `cpp/`
        :
            Contains native C or C++ code using the Java Native Interface (JNI). For more information, see the [Android NDK documentation](https://developer.android.com/ndk).
        ...
        :
            ...
            `AndroidManifest.xml`
            :
                Describes the nature of the application and each of its components. For more information, see the [app manifest](https://developer.android.com/guide/topics/manifest/manifest-intro) overview.
            ...
            `res/`
            :
                Contains application resources, such as drawable files and UI string files. For more information, see the [app resources](https://developer.android.com/guide/topics/resources) overview.


            `assets/`
            :
                Contains files to be compiled into an APK file as-is. For example, this is a good location for textures and game data. You can navigate this directory in the same way as a typical file system, using URIs and read files as a stream of bytes using the `https://developer.android.com/reference/android/content/res/AssetManager`.
...
- **Project:** Sets the version for [Gradle and the Android Gradle plugin](https://developer.android.com/studio/build#build-files) and the repository location name.
...
- **Modules:** Lets you edit module-specific build configurations, including the target and minimum SDK, the app signature, and library dependencies. Each module's settings page is divided into the following tabs:
  ...
  - **Signing:** Specifies the certificate to use to [sign your app](https://developer.android.com/tools/publishing/app-signing#sign-auto).
- **Dependencies:** Lists the library, file, and module dependencies for this module. You can add, modify, and delete dependencies from this pane. For more information about module dependencies, see [Configure build variants](https://developer.android.com/tools/building/configuring-gradle#declareDeps).
...
- **Build Variants:** Lets you configure different flavors and build types for your project.
  ...
  - **Flavors:** Lets you create multiple build *flavors* , where each flavor specifies a set of configuration settings, such as the module's minimum and target SDK version and the [version code and version name](https://developer.android.com/studio/publish/versioning#versioningsettings.html).
