...
[Android Studio release notes](/studio/releases).
...
[Android Gradle plugin](/build/releases/gradle-plugin). This build
...
**Note:** We might reference either the `build.gradle.kts` or `build.gradle`
file alone in the documentation, but they're conceptually interchangeable. For
example if you see `build.gradle.kts` but you use the Groovy DSL to configure
your build, you can think of it as the `build.gradle` file (and the other way
around).
...
[Build multiple APKs](/build/configure-apk-splits).
...
[Google Sign-In](/training/sign-in), then resource shrinking can
...
[Shrink, obfuscate, and optimize your app](/build/shrink-code).
...
![](/static/studio/images/studio-debug-settings-icon.png) and select **Show Variable Values in Editor**.
...
[Capture a heap dump](/studio/profile/capture-heap-dump).
...
[View memory allocations](/studio/profile/memory-profiler#record-allocations).

### Data file access

The Android SDK tools, such as
[Systrace](/topic/performance/tracing/command-line)
and [Logcat](/studio/debug/am-logcat),
...
[lint](/studio/write/lint) checks and other
...
[Improve code inspection with annotations](/studio/write/annotations).

### Log messages

When you build and run your app with Android Studio, you can view
[`adb`](/studio/command-line/adb) output and device log messages
in the [**Logcat** window](/studio/debug/am-logcat).
...
Vitals in [App Quality Insights](/studio/debug/app-quality-insights) and
[Gemini in Android Studio](/studio/preview/gemini). By signing in, you give
