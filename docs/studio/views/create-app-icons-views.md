---
title: Create app icons (Views)  |  Android Developers
...
# Create app icons (Views) Stay organized with collections Save and categorize content based on your preferences.





[Jetpack Compose implementationarrow\_forward](/studio/write/create-app-icons)
...
app icons, see [Create app icons](/studio/write/create-app-icons), which covers our
recommended UI framework.

## Create an action bar or tab icon

Use [Image Asset Studio](/studio/write/create-app-icons#about) to create action bar and tab
...
Actions](/training/appbar/actions), [App Bar - Material Design](https://material.google.com/layout/structure.html#structure-app-bar)
{:.external}, and [Action Bar Design](/design/patterns/actionbar) for more information.
...
[Creating Swipe Views with Tabs](/training/implementing-navigation/lateral) and [Tabs - Material
...
[support libraries](/topic/libraries/support-library) to deliver your material design UI to
older platform versions.

As an alternative to Image Asset Studio, you can use [Vector Asset
Studio](/studio/write/vector-asset-studio) to create action bar and tab icons. Vector
drawables are appropriate for simple icons and can reduce the size of your app.

After you [open Image Asset Studio](/studio/write/create-app-icons#access), you can add an
...
3. Optionally change the name and display options:
   ...
   * **Theme** - Select **[HOLO\_LIGHT](/guide/topics/ui/themes#SelectATheme)** or **HOLO\_DARK**. Or, to
...
5. Optionally change the resource directory:
   ...
   * **Res Directory** - Select the resource source set where you want to
   ...
   they will appear in [Project Files view](/studio/projects#ProjectFiles) of the
   **Project** window.
6. Click **Finish**.
...
[Compose launcher icon guidance](/studio/write/create-app-icons#material-icons-compose), which applies to Views too.
...
* In most cases, you can refer to image resources as `@drawable` in XML code
  or [`Drawable`](/reference/android/graphics/drawable/Drawable) in Java code.
  ...
  The [`getResources()`](/reference/android/content/Context#getResources()) method resides in the
  [`Context`](/reference/android/content/Context) class, which applies to UI objects, such as
...
The following Java code sets an [`ImageView`](/reference/android/widget/ImageView) to use the
...
See [Access your app resources](/guide/topics/resources/accessing-resources#ResourcesFromCode) for more information.