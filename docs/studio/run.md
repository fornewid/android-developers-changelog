---
title: Build and run your app  |  Android Studio  |  Android Developers
...
users](/studio/run/build-for-release). For more detailed information about
...
2. In the target device menu, select the device that you want to run your app
   ...
   Android Virtual Device](/studio/run/managing-avds#createavd) to use the
   [Android Emulator](/studio/run/emulator) or [connect a physical
   device](/studio/run/device#connect).
...
**Note:** Android Studio includes deployment optimizations for incremental changes
...
![](/static/studio/images/buttons/toolbar-build.png) in the tool window bar. The **Build** tool window
displays the
tasks that Gradle executes to build your app, as shown in figure 1.

![](/static/studio/images/run/window-build.png)


**Figure 1.** The **Build** tool window in Android Studio.

1. **Sync tab:** Displays tasks that Gradle executes to sync with your
   project files. Similar to the **Build Output** tab, if you encounter a sync
   error, select elements in the tree to get more information about the error.
   Also displays a summary of
   [download impact](/build/build-analyzer#download-impact) to
   determine whether dependency downloads are negatively affecting your build.
2. **Build Output tab:** Displays the tasks that Gradle executes as a tree,
   ...
   figure 2.
   ![](/static/studio/images/run/build-output-window-error.png)


   **Figure 2.** Inspect the **Build Output** tab for error messages.
3. **Build Analyzer tab:** Provides build performance analysis
   information about your build. See
   [Troubleshoot build performance with
   Build Analyzer](/studio/build/build-analyzer) for more information.
4. **Restart:** Performs the last build action again. If you last ran
   **Build > Make Selected Module**, it'll build the current module. If you
   last ran **Build > Make Project**, it'll generate intermediate build
   files for all modules in your project.
5. **Filters:** Filters out warnings, tasks, or both that
...
![](/static/studio/images/buttons/toolbar-gradle.png) in the tool window bar.
...
1. Open the **Settings** or **Preferences**
   ...
   * On macOS, select **Android Studio** >
...
When you use [Agent Mode](/studio/gemini/agent-mode), the agent can perform
tasks such as:
...
* **Interact with your app** using `adb shell` commands to navigate or enter
  data.
...
![Device interaction tools in Android Studio.](/static/studio/preview/features/images/device-interaction-tools.gif)


**Figure 3:** The AI agent can help you test and verify changes on a device.
...
* To deploy your app in debug mode, click **Debug**
  ...
  learn more, see [Debug your app](/studio/debug).
* If you have a larger, more complex app, use Apply Changes instead of
  clicking **Run** ![](/static/studio/images/buttons/toolbar-run.png). This saves time, because you avoid restarting
...
* If you're using Jetpack Compose, Live Edit is an experimental feature that
  ...
  **Run** ![](/static/studio/images/buttons/toolbar-run.png).
  ...
  information, see the [Live Edit (experimental)](#live-edit) section.
* If you have an app with multiple build variants or versions, you can choose
  ...
  the build variant](/studio/run#changing-variant) section.
* To fine-tune app installation, launch, and test options, you can change the
  ...
  configurations](/studio/run#run-configuration) section.
* We recommend that you use Android Studio for your development needs, but you
  ...
  line](/studio/build/building-cmdline).
...
**Apply Changes and Restart Activity** ![Apply Changes and Restart Activity icon](/static/studio/images/buttons/toolbar-apply-changes.png): Attempts
...
**Apply Code Changes** ![Apply Code Changes icon](/static/studio/images/buttons/toolbar-apply-code-changes.png) : Attempts to apply only
...
You can also perform this action by pressing `Control`+`F10`
(`Control`+`Command`+`R` on macOS).

**Run** ![](/static/studio/images/buttons/toolbar-run.png): Deploys all changes and restarts the app. Use this
...
restart, see the [Limitations of Apply Changes](#apply-changes-limitations)
section.
...
Apply Changes to fail, Android Studio prompts you to **Run** ![Run icon](/static/studio/images/buttons/toolbar-run.png) your app again instead.
...
**Note:** Some types of changes don't cause Apply Changes to fail but still require
...
must restart your app to see those changes.
...
* If a library or plugin makes changes to your app's resource files, you can't
  use **Apply Code Changes** ![Apply Code Changes icon](/static/studio/images/buttons/toolbar-apply-code-changes.png). You must use **Apply
  Changes and Restart Activity** ![Apply Changes and Restart Activity icon](/static/studio/images/buttons/toolbar-apply-changes.png)
...
using **Apply Code Changes** ![Apply Code Changes icon](/static/studio/images/buttons/toolbar-apply-code-changes.png)
...
Changes** ![Apply Code Changes icon](/static/studio/images/buttons/toolbar-apply-code-changes.png). This behavior occurs
...
and Restart Activity** ![Apply Changes and Restart Activity icon](/static/studio/images/buttons/toolbar-apply-changes.png)
or **Run** ![Run icon](/static/studio/images/buttons/toolbar-run.png) instead.
...
Edit](/jetpack/compose/tooling/iterative-development#live-edit)

[

](/static/studio/videos/LiveEditShortVideo.mp4)
...
modules, the **Active ABI** value determines the [ABI](/ndk/guides/abis) that
...
public release](/studio/publish/preparing). To define other variations of your
...
an APK or an [Android App Bundle](/guide/app-bundle) as well as the module to
...
configurations](/studio/run/rundebugconfig).