---
title: Troubleshoot Android Studio  |  Android Developers
...
This page contains guidance for troubleshooting common issues and configuration
problems in Android Studio.

## High-density displays

As of version 1.5, Android Studio provides support for high-density displays
(like HiDPI and Retina monitors) on all platforms.
...
property in the
`idea.properties` file as described in [Customize your IDE
properties](/studio/intro/studio-config#customize_ide). Note that this property has no effect on Mac machines.
...
* On a multi-monitor setup running Windows 8.1 or later, when you move a
  ...
  [186007](https://code.google.com/p/android/issues/detail?id=186007)).
  There is no known workaround at this time.
* Older versions of the JRE 1.8 had an issue for blurry fonts
  (JRE 1.8.0\_25-b18 amd64 in particular, see bug [192316](https://code.google.com/p/android/issues/detail?id=192316).)
  As of version 2.2, Android studio includes a bundled version of the latest
...
see [Scaling factor settings](#scaling-factor). If some elements of
...
* If you are using a custom editor scheme, the editor font may appear too
  ...
  **Editor > Colors and Fonts > Font** and change the size of the editor font. Note
  ...
  automatically (see bug [186920](https://code.google.com/p/android/issues/detail?id=186920)).
* If some UI elements of Android Studio are the right size, but others are
  too small or too big, you may be experiencing issue [186923](https://code.google.com/p/android/issues/detail?id=186923).
...
The following directories are used by Android Studio to store settings,
caches, plugins, and logs.
...
* `<informal directory name>`
  ([`<IDE property>`](/studio/intro/studio-config#customize_ide)):
  `<default path>`.
