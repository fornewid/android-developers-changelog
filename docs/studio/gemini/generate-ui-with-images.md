---
title: Generate UI with image attachments  |  Android Studio  |  Android Developers
...
# Generate UI with image attachments Stay organized with collections Save and categorize content based on your preferences.
...
1. Create a mockup of the app UI that you want. You can export a PNG from a
   ...
   ![Hand-drawn wireframe of an app user interface](/static/studio/gemini/images/app-wireframe.png)


   Figure 1: A wireframe of an app user interface.
2. Attach the image to your query by clicking the **Attach image file** button
   ![Attach Image File icon](/static/studio/images/attach-image-file.png). You can also click
   **Generate Code From Screenshot** directly from the **Preview** panel
   ...
   Figure 2: Generate code from a screenshot in an empty Preview panel.
3. In the chat field, as the AI agent to generate the UI code, for example "Generate
   ...
   ask it to
   [generate the Compose previews](/studio/gemini/generate-compose-previews).

   ![Gemini chat interface showing generated Jetpack Compose code based on an image.](/static/studio/gemini/images/generate-compose-code.png)
...
4. Once you import the code and can see the Compose preview in the preview
   ...
   [asking Gemini to transform it](/studio/gemini/transform-ui). If you have an image of what you want,
   you can also iterate on the UI by right-clicking the preview and selecting
   **AI Actions** > **Match UI to Target Image**.

   ![Android Studio showing a Compose preview with a selected UI element and Gemini chat.](/static/studio/gemini/images/transform-ui.png)
...
Figure 7: Fix UI issues with AI


![](/static/studio/preview/features/images/ui-check-fixed.png)


Figure 8: Example UI after applying fixes.