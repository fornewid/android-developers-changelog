...
Use the App Crawler tool, part of [Jetpack](https://developer.android.com/jetpack), to automatically test
...
This comes with [Android Studio](https://developer.android.com/studio).
...
| Option | Description |
|---|---|
| `--android-sdk path` | Specifies the path to your Android SDK. This is a **required** flag. |
| `--apk-file file` | Specifies the path to your app APK, which App Crawler then installs and crawls. This is a **required** flag if `--app-package-name` isn't specified. |
| `--app-package-name name` | Specifies the package name of your app. Use this option when your app is already installed on the device and no re-installation is required. This is a **required** flag if `--apk-file` isn't specified. |
...
| `--key-store-password password` | Specifies the password for the keystore you provided for `--key-store` option. This is a **required** flag if `--key-store` is specified. |
| `--timeout-sec timeout` | Specifies the timeout for your crawl in seconds. If not specified, the crawl stops after 60 seconds. |
