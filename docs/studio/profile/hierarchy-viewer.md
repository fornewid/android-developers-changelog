---
title: Profile your layout with Hierarchy Viewer  |  Android Studio  |  Android Developers
...
# Profile your layout with Hierarchy Viewer Stay organized with collections Save and categorize content based on your preferences.
...
2. Set the environment variable `ANDROID_HVPROTO=ddm`
...
![](/static/images/tools/performance/hierarchy-viewer/gettingstarted_image005.png)

**Figure 1.** Android Device Monitor

1. Connect your device to your computer. If prompted by a dialog on the device that asks,
   **Allow USB debugging?**, tap **OK**.
2. Open your project in Android Studio, [build and run it](/studio/run)
   on your device.
3. Start [Android Device Monitor](/studio/profile/monitor).
   Android Studio might show a **Disable adb integration** dialog because only one process can
   connect to the device via adb at once, and Android Device Monitor is requesting a connection.
   So click **Yes**.
...
5. Double-click your app's package name in the **Windows** tab on the left.
   This populates the panes with the view hierarchy of your app.

## Get familiar with the tools

![](/static/images/tools/performance/hierarchy-viewer/gettingstarted_image008.png)

**Figure 2.**  Hierarchy Viewer panes.

The Hierarchy Viewer provides the following panes (as shown in figure 2):

* **Tree View** (center): Shows a tree view of your view hierarchy.
  You can drag and zoom the tree using your mouse and the zoom control at the bottom.
  Each node indicates it's `View` class name and ID name.
* **Tree Overview** (top right): Gives you a bird's-eye view of your
  app's complete view hierarchy.
  Move the grey rectangle to change the viewport that's visible in the **Tree View**.
* **Layout View** (bottom right): Shows a wireframe view of your layout.
  The outline of the currently selected view is red, and its parent view is light red.
...
![](/static/studio/images/buttons/device-monitor-load-hierarchy.png).

To invalidate a view (request that the system call `onDraw()`
during the next layout update), select a view in the hierarchy and
then click **Invalidate the layout** ![](/static/studio/images/buttons/device-monitor-invalidate.png)
...
![](/static/studio/images/buttons/device-monitor-lay-out.png).
...
![](/static/images/developing/hv_treeview_screenshot.png)
...
![](/static/images/tools/performance/hierarchy-viewer/gettingstarted_image010.png)
...
**Capture the window layers** ![](/static/studio/images/buttons/device-monitor-save-layers.png) in the toolbar. Each view is saved as its own layer so you
can easily create a new mock by hiding and adjusting each view.

## Profile your layout

![](/static/images/tools/performance/hierarchy-viewer/gettingstarted_image014.png)

Now that you know your way around the tool, you can use it to profile your view hiearchy
and interpret the results.

1. In the **Tree View** or the **Layout View**, click on the view node whose
   children you want to profile.
2. To start profiling, click **Obtain layout times**
   ![](/static/images/tools/performance/hierarchy-viewer/gettingstarted_image013.png)
...
![](/static/images/tools/performance/hierarchy-viewer/gettingstarted_image015.png)
...
* Look for red dots in leaf nodes or view groups with only a few children. This
  ...
  [Systrace](/topic/performance/tracing/command-line) or
  [Traceview](/studio/profile/traceview-walkthru)
...
* If the root view of a hierarchy has a red measure phase, red layout phase, and
  ...
  parent of all the other views and its layout isn't finished until the children finish.
