---
title: Record native allocations  |  Android Studio  |  Android Developers
...
* **Allocations**: A count of objects allocated using `malloc()` or the `new`
  operator during the selected time period.
* **Deallocations**: A count of objects deallocated using `free()` or the
