---
title: Engage SDK for video recommendations  |  Other Play guides  |  Android Developers
...
# Engage SDK for video recommendations Stay organized with collections Save and categorize content based on your preferences.
...
The [`RecommendationClusterType`](/reference/com/google/android/engage/common/datamodel/RecommendationClusterType) determines how the
...
* Existing `RecommendationsCluster` data from the developer partner is
  removed.
* Data from the request is parsed and stored in the updated
  `RecommendationsCluster`. In case of an error, the entire request is
  rejected and the existing state is maintained.

**Note:** Publish APIs are upsert operations, replacing existing content; update
entities by republishing the entire cluster. Only publish recommendations for
adult accounts. Avoid using delete APIs followed by publish, as the latter
inherently replaces content.
...
1. [`MediaActionFeedEntity`](/reference/com/google/android/engage/video/datamodel/MediaActionFeedEntity)
2. [`MovieEntity`](/reference/com/google/android/engage/video/datamodel/MovieEntity)
3. [`TvShowEntity`](/reference/com/google/android/engage/video/datamodel/TvShowEntity)
4. [`LiveTvChannelEntity`](/reference/com/google/android/engage/video/datamodel/LiveTvChannelEntity)
5. [`LiveTvProgramEntity`](/reference/com/google/android/engage/video/datamodel/LiveTvProgramEntity)
...
create a [`MediaActionFeedEntity`](/reference/com/google/android/engage/video/datamodel/MediaActionFeedEntity) which includes the
...
**Note:** If the additional fields are provided directly within the entity, these
values will be prioritized. The locale, if provided in the Account Profile, will
be used to fetch the remaining entity metadata from Google's database in the
language consistent with the request.