---
title: Support in-app updates (Native)  |  Other Play guides  |  Android Developers
...
# Support in-app updates (Native) Stay organized with collections Save and categorize content based on your preferences.
...
cases where your implementation uses [Unity](/guide/playcore/in-app-updates/unity) or [Unreal Engine](/guide/playcore/in-app-updates/unreal-engine).

**Note:** The Native API for in-app updates requires your app to [use Play Core
library](/guide/playcore#native) version 1.10.0 or higher.

## Native SDK overview

The Play Core Native SDK is part of the [Play Core SDK](/reference/com/google/android/play/core/release-notes) family. The Native
SDK includes a C header file, `app_update.h`, that wraps [`AppUpdateManager`](/reference/com/google/android/play/core/appupdate/AppUpdateManager)
...
[`AppUpdateManager_init()`](/reference/native/play/core/group/appupdate#appupdatemanager_init) function, as shown in the following example built
with [`android_native_app_glue.h`](/ndk/guides/concepts#naa):
...
app. [`AppUpdateManager_requestInfo()`](/reference/native/play/core/group/appupdate#appupdatemanager_requestinfo) starts an asynchronous request that
...
[`AppUpdateManager_getInfo()`](/reference/native/play/core/group/appupdate#appupdatemanager_getinfo). In addition to the error code, this function
returns an [`AppUpdateInfo`](/reference/native/play/core/group/appupdate#appupdateinfo) opaque struct, which you can use to retrieve
...
Use [`AppUpdateInfo_getClientVersionStalenessDays()`](/reference/native/play/core/group/appupdate#appupdateinfo_getclientversionstalenessdays) to check the number of
...
* Minor UI improvements: **Low-priority** update; request neither a flexible
  update nor an immediate update. Update only when the user isn't interacting
  with your app.
...
[`AppUpdateInfo_getPriority()`](/reference/native/play/core/group/appupdate#appupdateinfo_getpriority):
...
[`AppUpdateManager_requestStartUpdate()`](/reference/native/play/core/group/appupdate#appupdatemanager_requeststartupdate). Before you request an update, get
an up-to-date `AppUpdateInfo` object and create an [`AppUpdateOptions`](/reference/native/play/core/group/appupdate#appupdateoptions)
...
the [`AppUpdateOptions_setAssetPackDeletionAllowed()`](/reference/native/play/core/group/appupdate#appupdateoptions_setassetpackdeletionallowed) method to set it to
`true` instead:
...
**Caution:** Use the `AllowAssetPackDeletion` API option with care to avoid
accidentally deleting assets. Before setting this option to `true`, you should
check whether the update is allowed with this option set to `false`.
...
[`AppUpdateInfo_destroy()`](/reference/native/play/core/group/appupdate#appupdateinfo_destroy) and [`AppUpdateOptions_destroy()`](/reference/native/play/core/group/appupdate#appupdateoptions_destroy),
respectively.
...
[`AppUpdateManager_requestCompleteUpdate()`](/reference/native/play/core/group/appupdate#appupdatemanager_requestcompleteupdate), as shown in the following
example:
...
Free up resources by calling the [`AppUpdateManager_destroy()`](/reference/native/play/core/group/appupdate#appupdatemanager_destroy) function
after your app has finished using the API.

## Error handling

This section describes solutions for common errors indicated by specific
[`AppUpdateErrorCode`](/reference/native/play/core/group/appupdate#appupdateerrorcode) values:

* An error code of `-110, APP_UPDATE_INITIALIZATION_NEEDED` indicates that the
  API has not been initialized successfully. Call `AppUpdateManager_init()` to
  initialize the API.
* An error code of `-4, APP_UPDATE_INVALID_REQUEST` indicates that some
  parameters of the update flow request are malformed. Check to make sure that
  the `AppUpdateInfo` and `AppUpdateOptions` objects are not null and are
  correctly formatted.
* An error code of `-5, APP_UPDATE_UNAVAILABLE` indicates that there is no
  applicable update available. Make sure that the target version has the same
  [package name](/studio/build/configure-app-module#change_the_package_name), [application ID](/studio/build/configure-app-module#set_the_application_id), and [signing key](/studio/publish/app-signing). If there is
  an update available, clear the app's cache and call
  `AppUpdateManager_requestAppUpdateInfo()` again to refresh `AppUpdateInfo`.
* An error code of `-6, APP_UPDATE_NOT_ALLOWED` indicates that the update type
  indicated by the `AppUpdateOption` object is not allowed. Check whether the
  `AppUpdateInfo` object indicates that the update type is allowed before
