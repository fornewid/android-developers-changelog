---
title: Design your navigation graph  |  App architecture  |  Android Developers
...
# Design your navigation graph Stay organized with collections Save and categorize content based on your preferences.
...
**Note:** The navigation graph is distinct from the [back stack](/guide/navigation/backstack), which is a
stack within the `NavController` that holds destinations the user has recently
visited.
...
* [Dialog destinations](/guide/navigation/design/dialog-destinations)
* [Activity destinations](/guide/navigation/design/activity-destinations)
...
* **Compose:** Use the `NavHost` composable. Add a `NavGraph` to it using the
  [Kotlin DSL](/guide/navigation/navigation-kotlin-dsl). You can create the graph in two ways:
  + **As part of the NavHost:** Construct the navigation graph directly as
    part of adding the `NavHost`.
  + **Programmatically:** Use the [`NavController.createGraph()`](/reference/androidx/navigation/NavController#(androidx.navigation.NavController).createGraph(kotlin.String,kotlin.String,kotlin.Function1)) method
    to create a `NavGraph` and pass it to the `NavHost` directly.
* **Fragments:** When using fragments with the views UI framework, use a
  `NavHostFragment` as the host. There are several ways to create a navigation
  graph:
  + **Programmatically:** Use the Kotlin DSL to create a `NavGraph` and
    directly apply it on the `NavHostFragment`.
    - The `createGraph()` function used with the Kotlin DSL for both
      fragments and Compose is the same.
  + **XML:** Write your navigation host and graph directly in XML.
  + **Android Studio editor:** Use the GUI editor in Android Studio to
    create and adjust your graph as an XML resource file.

**Note:** How you interact with the graph through the `NavController` is similar
between frameworks. See the [Navigate to a destination](/guide/navigation/use-graph/navigate) guide for more
details.
...
instructions to add this plugin](/guide/navigation#set-up).
...
1. A serializable object represents each of the two routes, `Profile` and
   `FriendsList`.
2. The call to the `NavHost` composable passes a `NavController` and a route
   for the start destination.
3. The lambda passed to the `NavHost` ultimately calls
   [`NavController.createGraph()`](/reference/androidx/navigation/NavController#(androidx.navigation.NavController).createGraph(kotlin.String,kotlin.String,kotlin.Function1)) and returns a `NavGraph`.
4. Each route is supplied as a type argument to
   [`NavGraphBuilder.composable<T>()`](/reference/kotlin/androidx/navigation/NavGraphBuilder#(androidx.navigation.NavGraphBuilder).composable(kotlin.collections.Map,kotlin.collections.List,kotlin.Function1,kotlin.Function1,kotlin.Function1,kotlin.Function1,kotlin.Function1,kotlin.Function2)) which adds the destination to the
   resulting `NavGraph`.
5. The lambda passed to `composable` is what the `NavHost` displays for that
   destination.
...
`NavGraph` separately using [`NavController.createGraph()`](/reference/androidx/navigation/NavController#(androidx.navigation.NavController).createGraph(kotlin.String,kotlin.String,kotlin.Function1)) and pass it to
...
**Important:** A [`NavController`](/reference/androidx/navigation/NavController) is associated with a single `NavHost`
composable. The `NavHost` provides the `NavController` access to its navigation
graph. When you use the `NavController` to navigate to a destination, you cause
...
* The `Profile` route specifies the starting destination in the navigation
  graph, with `"John Smith"` as the argument for `name`.
* The destination itself is the `composable<Profile>{}` block.
* The `ProfileScreen` composable takes the value of `profile.name` for its own
  `name` argument.
...
**Note:** By using the parameters of the route class you can pass data to the given
destination with full type safety. For example, in the previous code
`Profile.name` ensures that `name` is always a `String`.
...
**Note:** The Navigation component is designed for apps that have one main activity
with multiple fragment destinations. The main activity is associated with a
navigation graph and contains a `NavHostFragment` that is responsible for
swapping destinations as needed. In an app with multiple activity destinations,
...
[`NavController.findNavController`](/reference/androidx/navigation/Navigation#findNavController(android.view.View))(). This associates the NavController with
the `NavHostFragment`.

Subsequently, the call to [`NavController.createGraph()`](/reference/androidx/navigation/NavController#(androidx.navigation.NavController).createGraph(kotlin.String,kotlin.String,kotlin.Function1)) links the graph to
...
preceding section on [Compose](#compose). For example, both there and here, the
...
[`NavGraphBuilder.fragment()`](/reference/kotlin/androidx/navigation/NavGraphBuilder#(androidx.navigation.NavGraphBuilder).fragment(kotlin.Int)) adds a fragment destination.

For more information on how to use the Kotlin DSL, see [Build a graph with the
NavGraphBuilder DSL](/guide/navigation/navigation-kotlin-dsl).
...
fragments](/guide/navigation/design/actions).

**Note:** The DSL example does not define actions because they don't apply in that
context. When using the DSL, [use `NavController.navigate()` directly](/guide/navigation/design/kotlin-dsl#host).
...
For more information, see [Navigation editor](/guide/navigation/design/editor).

## Nested graphs

You can also use nested graphs. This involves using a graph as a navigation
destination. For more information, see [Nested graphs](/guide/navigation/design/nested-graphs).

## Further Reading

For more core navigation concepts, see the following guides:

* **[Overview](/guide/navigation):** Make sure to read the general overview of the Navigation
  component.
* **[Activity destinations](/guide/navigation/design/activity-destinations):** Examples of how to implement destinations
  that take the user to activities.
* **[Dialog destinations](/guide/navigation/design/dialog-destinations):** Examples of how to create destinations that
  take the user to a dialog.
* **[Navigate to a destination](/guide/navigation/use-graph/navigate):** A detailed guide that covers how to
  navigate from one destination to another.
* **[Nested graphs](/guide/navigation/design/nested-graphs):** An in-depth guide on how to nest one navigation
  graph within another.