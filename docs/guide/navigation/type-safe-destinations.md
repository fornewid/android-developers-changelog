---
title: Guide: Migrating to Type-Safe Navigation in Compose and Navigation 2  |  App architecture  |  Android Developers
url: https://developer.android.com/guide/navigation/type-safe-destinations
source: html-scrape
---
...
# Guide: Migrating to Type-Safe Navigation in Compose and Navigation 2 Stay organized with collections Save and categorize content based on your preferences.
...
* Add the dependencies to your top-level `build.gradle.kts` and module-level
  `build.gradle.kts`.
...
1. **Create the Custom Type**:
   ```kotlin
   val SearchFilterType = object : NavType(isNullableAllowed = false) {
...
* **Sealed Hierarchies**: For large apps, group your routes using a sealed
  interface or class to keep the navigation structure organized
* **Object Instances**: For routes without parameters, always use `object`
  instead of `class` to avoid unnecessary allocations
...
* **Testing**: Use `navController.currentBackStackEntry?.hasRoute<T>()` to
  check the current destination in a type-safe manner during UI tests