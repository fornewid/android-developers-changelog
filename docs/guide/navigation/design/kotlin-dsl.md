---
title: Fragments and the Kotlin DSL  |  App architecture  |  Android Developers
...
# Fragments and the Kotlin DSL Stay organized with collections Save and categorize content based on your preferences.
...
![The Sunflower app has two destinations along with an action that
            connects them.](/static/images/guide/navigation/navigation-kotlin-dsl-1.png)


**Figure 1.** The Sunflower app has two destinations,
...
isn't defined as a [resource](/guide/topics/resources/providing-resources) in
...
[XML-based navigation graphs](/guide/navigation/design#xml) are parsed as part
...
type](/jetpack/compose/navigation#nav-with-args). This lets you have type safety
...
[`fragment()`](/reference/kotlin/androidx/navigation/fragment/package-summary#(androidx.navigation.NavGraphBuilder).fragment(kotlin.collections.Map,kotlin.Function1))
DSL builder function. This function requires two [type
arguments](https://kotlinlang.org/docs/generics.html#generic-functions)
.

First, a [`Fragment`](/reference/kotlin/androidx/fragment/app/Fragment) class
...
[`NavController.navigate()`](/reference/androidx/navigation/NavController#navigate(kotlin.Any,kotlin.Function1))
calls:
...
the current
[`NavBackStackEntry`](/reference/kotlin/androidx/navigation/NavBackStackEntry)
and calling
[`toRoute`](/reference/kotlin/androidx/navigation/NavBackStackEntry#(androidx.navigation.NavBackStackEntry).toRoute())
...
with your Kotlin DSL graph](#navigate) section.
...
The
[`activity()`](/reference/kotlin/androidx/navigation/package-summary#(androidx.navigation.NavGraphBuilder).activity(kotlin.collections.Map,kotlin.Function1))
...
intent](/guide/components/intents-filters#ExampleSend), where an explicit
...
The
[`navigation()`](/reference/kotlin/androidx/navigation/package-summary#(androidx.navigation.NavGraphBuilder).navigation(kotlin.Any,kotlin.collections.Map,kotlin.Function1))
DSL function can be used to build a [nested navigation
graph](/guide/navigation/navigation-nested-graphs). This function takes a type
...
destination](/reference/kotlin/androidx/navigation/NavGraphBuilder#label()).
This label can be useful for binding the navigation graph to UI components using
[`NavigationUI`](/guide/navigation/navigation-ui).
...
If you're using a [new destination type](/guide/navigation/navigation-add-new)
that does not directly support the Kotlin DSL, you can add these destinations to
your Kotlin DSL using
[`addDestination()`](/reference/androidx/navigation/NavGraph#addDestination(androidx.navigation.NavDestination)):
...
changes](/guide/topics/resources/runtime-changes) and [system-initiated process
death](/topic/libraries/architecture/saving-states#options). For this reason,
...
for a destination](/guide/navigation/navigation-deep-link) apply to the process
of creating a deep link using the Kotlin DSL.

When creating an [implicit deep link](/guide/navigation/navigation-deep-link)
...
filters](/training/app-links/deep-linking) to your activity manually. The intent
...
[`deepLink()`](/reference/kotlin/androidx/navigation/NavDestinationBuilder#deepLink(kotlin.String))
a new deep link is appended to a list that is maintained for that destination.

## Limitations

The [Safe Args](/guide/navigation/navigation-pass-data#Safe-args) plugin is
