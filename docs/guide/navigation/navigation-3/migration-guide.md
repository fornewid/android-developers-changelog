---
title: Migrate from Navigation 2 to Navigation 3  |  App architecture  |  Android Developers
...
# Migrate from Navigation 2 to Navigation 3 Stay organized with collections Save and categorize content based on your preferences.
...
* You should be familiar with [navigation terminology](/guide/navigation).
* Destinations are composable functions. Navigation 3 is designed exclusively
  ...
  Compose](/develop/ui/compose/migrate/interoperability-apis/views-in-compose).
...
* You have one or several top-level routes (usually displayed in a bottom
  navigation bar), and each has its own back stack.
...
* [Custom destination types](/guide/navigation/design/kotlin-dsl#custom)
* Deep links
...
Use the [Get started](/guide/navigation/navigation-3/get-started) page to add the Navigation 3 dependencies to your
...
Update every navigation [route](/guide/navigation#types) so that it implements the `NavKey`
interface. This lets you use `rememberNavBackStack` to assist with [saving your
navigation state](/guide/navigation/navigation-3/save-state).
...
plugin. You can add this by following [these project setup steps](/guide/navigation/navigation-3/get-started#project-setup).
...
**Architecture principles:** These classes follow the principles of [Unidirectional Data Flow](/topic/architecture):
...
functions here as described in [Encapsulate your navigation code](/guide/navigation/design/encapsulate).

In Navigation 3, you define your destinations using an `entryProvider`. This
`entryProvider` resolves a route to a [`NavEntry`](/guide/navigation/navigation-3/basics#resolve-keys). Importantly, the
...
Create an `entryProvider` [using the DSL](/guide/navigation/navigation-3/basics#entry-provider-DSL) at the same scope as the
`NavigationState`.
...
You can refactor [`NavGraphBuilder` extension functions](/guide/navigation/design/encapsulate) to
