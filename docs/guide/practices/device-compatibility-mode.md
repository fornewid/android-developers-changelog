---
title: Device compatibility mode  |  Compatibility  |  Android Developers
...
[Per-app overrides](#per-app_overrides) enable device manufacturers, virtual
device owners[1](#fn1), and users to change app behavior to improve app layout or
...
* [FORCE\_RESIZE\_APP](#force_resize_app)
* [FORCE\_NON\_RESIZE\_APP](#force_non_resize_app)
* [OVERRIDE\_MIN\_ASPECT\_RATIO](#override_min_aspect_ratio)
* [OVERRIDE\_MIN\_ASPECT\_RATIO\_PORTRAIT\_ONLY](#override_min_aspect_ratio_portrait_only)
* [OVERRIDE\_MIN\_ASPECT\_RATIO\_MEDIUM](#override_min_aspect_ratio_medium)
* [OVERRIDE\_MIN\_ASPECT\_RATIO\_LARGE](#override_min_aspect_ratio_large)
* [OVERRIDE\_MIN\_ASPECT\_RATIO\_TO\_ALIGN\_WITH\_SPLIT\_SCREEN](#override_min_aspect_ratio_to_align_with_split_screen)
* [OVERRIDE\_MIN\_ASPECT\_RATIO\_EXCLUDE\_PORTRAIT\_FULLSCREEN](#override_min_aspect_ratio_exclude_portrait_fullscreen)
* [OVERRIDE\_ANY\_ORIENTATION](#override_any_orientation)
* [OVERRIDE\_ANY\_ORIENTATION\_TO\_USER](#override_any_orientation_to_user)
* [OVERRIDE\_UNDEFINED\_ORIENTATION\_TO\_PORTRAIT](#override_undefined_orientation_to_portrait)
* [OVERRIDE\_UNDEFINED\_ORIENTATION\_TO\_NOSENSOR](#override_undefined_orientation_to_nosensor)
* [OVERRIDE\_LANDSCAPE\_ORIENTATION\_TO\_REVERSE\_LANDSCAPE](#override_landscape_orientation_to_reverse_landscape)
* [OVERRIDE\_ORIENTATION\_ONLY\_FOR\_CAMERA](#override_orientation_only_for_camera)
* [OVERRIDE\_USE\_DISPLAY\_LANDSCAPE\_NATURAL\_ORIENTATION](#override_use_display_landscape_natural_orientation)
* [OVERRIDE\_ENABLE\_COMPAT\_IGNORE\_REQUESTED\_ORIENTATION](#override_enable_compat_ignore_requested_orientation)
* [OVERRIDE\_ENABLE\_COMPAT\_IGNORE\_ORIENTATION\_REQUEST\_WHEN\_LOOP\_DETECTED](#override_enable_compat_ignore_orientation_request_when_loop_detected)
* [OVERRIDE\_RESPECT\_REQUESTED\_ORIENTATION](#override_respect_requested_orientation)
* [OVERRIDE\_EXCLUDE\_CAPTION\_INSETS\_FROM\_APP\_BOUNDS](#override_exclude_caption_insets_from_app_bounds)

### Opt out

Your app can target API level 36 but opt out of the Android 16 behavior, in
which case [OVERRIDE\_ANY\_ORIENTATION\_TO\_USER](#override_any_orientation_to_user)
is not applicable.

**Warning:**

* All overrides other than
  `OVERRIDE_ANY_ORIENTATION_TO_USER` are applicable even
  though your app opts out of the Android 16 behavior; and so, your
  app's screen orientation, aspect ratio, and resizability
  restrictions can still be overridden if device manufacturers have
  implemented the overrides.
* Users can override app aspect ratio in **Settings** even
  if your app opts out. See [User
  per-app overrides](#user_per-app_overrides).
* Desktop windowing:
...
doesn't lock the display orientation or prevent screen rotation](/about/versions/16/behavior-changes-16#ignore-orientation) on
large displays.
...
* **Tablets:** The natural orientation of some tablets, such as Pixel Tablet,
  ...
  [`Display#getRotation()`](/reference/kotlin/android/view/Display#getRotation()) returns
  [`Surface.ROTATION_0`](/reference/kotlin/android/view/Surface#rotation_0). If apps assume `ROTATION_0` is portrait,
...
* **Landscape foldables:** Some foldable devices, such as Pixel Fold, are in
  ...
  loops](#flickering_loops) or layout issues are likely.
...
* **Car displays:** Many, but not all, car displays are landscape. Developing
  [parked apps](/training/cars/parked) for car displays is similar to developing for tablets.
...
![](/static/images/guide/practices/compatibility-mode/letterboxing.png)
...
* [`screenOrientation`](/reference/android/R.attr#screenOrientation): Specifies a fixed orientation for
  an app. Apps can also set orientation at runtime by using
  [`Activity#setRequestedOrientation()`](/reference/kotlin/android/app/Activity#setrequestedorientation).
* [`resizeableActivity`](/guide/topics/manifest/activity-element#resizeableActivity): Indicates whether the system can
  ...
  [window size class](/develop/ui/compose/layouts/adaptive/use-window-size-classes)). On Android 12 and higher, apps
  support multi‑window mode on large screens (medium or expanded window
  size class) regardless of this setting.
* [`maxAspectRatio`](/reference/android/R.attr#maxAspectRatio): Specifies the maximum aspect ratio
  supported by the app. Only apps with `resizeableActivity` set to `false` can
  set `maxAspectRatio`.
* [`minAspectRatio`](/reference/android/R.attr#minAspectRatio): Specifies the minimum aspect ratio
  supported by the app. Only apps with `resizeableActivity` set to `false` can
  set `minAspectRatio`.

![](/static/images/guide/practices/compatibility-mode/letterboxing_issue.png)
...
App should support all device and [multi-window mode](/develop/ui/compose/layouts/adaptive/support-multi-window-mode) display
...
![](/static/images/guide/practices/compatibility-mode/letterboxing_optimization.png)
...
![](/static/images/guide/practices/compatibility-mode/enhanced_letterboxing.png)
...
* **Redesigned restart button:** Device manufacturers can give the restart
  button for [size compatibility mode](#size_compatibility_mode) a new look
...
![](/static/images/guide/practices/compatibility-mode/enhanced_letterboxing_user_dialog.png)
...
* Is resizable with [`resizeableActivity="true"`](/guide/topics/manifest/activity-element#resizeableActivity)
* Supports [picture-in-picture (PIP)](/develop/ui/views/picture-in-picture#declaring) mode
* Is [embedded](/develop/ui/views/layout/activity-embedding)
* Has the [`FORCE_RESIZE_APP`](#force_resize_app) per-app override applied by
...
![](/static/images/guide/practices/compatibility-mode/size_compatibility_mode_issue.gif)
...
`android:resizeableActivity` attribute of the [`<activity>`](/guide/topics/manifest/activity-element#resizeableActivity)
or [`<application>`](/guide/topics/manifest/application-element#resizeableActivity) element to `true` in the
app manifest. Design responsive/adaptive layouts for your app. For more
information, see
[Support different display sizes](/develop/ui/compose/layouts/adaptive/support-different-display-sizes) and [Support
multi-window mode](/develop/ui/compose/layouts/adaptive/support-multi-window-mode#resizeableActivity).

![](/static/images/guide/practices/compatibility-mode/size_compatibility_mode_optimization.gif)
...
display. See also [Processes and threads overview](/guide/components/processes-and-threads).

![](/static/images/guide/practices/compatibility-mode/size_compatibility_mode_workaround.gif)
...
[color mode](/guide/topics/manifest/activity-element#colormode),
[touchscreen availability](/guide/topics/manifest/activity-element#config), or
[screen density](/guide/topics/manifest/activity-element#config) change.

Display compatibility mode mode is enabled by default for games (based on the
[`android:appCategory`](/guide/topics/manifest/application-element#appCategory) flag) to improve stability and continuity.
...
APIs such as the [`onConfigurationChanged()`](/reference/kotlin/android/app/Activity#onconfigurationchanged) callback but is spared from a
...
activity's [`android:screenOrientation="portrait"`](/guide/topics/manifest/activity-element#screen)
...
[`Activity#setRequestedOrientation()`](/reference/kotlin/android/app/Activity#setrequestedorientation). The call
triggers an app restart if the app is not handling configuration changes (see
[Handle configuration changes](/guide/topics/resources/runtime-changes)). After the
...
[natural orientation](/reference/kotlin/androidx/browser/trusted/ScreenOrientation#NATURAL()) (the *usual* orientation as
determined by Android) of a device screen is landscape (that is, calling
[`Display#getRotation()`](/reference/kotlin/android/view/Display#getRotation()) returns
[`Surface.ROTATION_0`](/reference/kotlin/android/view/Surface#rotation_0) while the device has a landscape aspect
...
`setRequestedOrientation(`[`ActivityInfo.SCREEN_ORIENTATION_PORTRAIT`](/reference/kotlin/android/content/pm/ActivityInfo#screen_orientation_portrait)
...
![](/static/images/guide/practices/compatibility-mode/flickering_loops_issue.gif)
...
* Set orientation based on signals not related to the current window size such
  ...
  [`FoldingFeature`](/reference/kotlin/androidx/window/layout/FoldingFeature), or [deprecated APIs](#misused_apis).

![](/static/images/guide/practices/compatibility-mode/flickering_loops_optimization.gif)
...
* The activity has already relaunched from a previous call to the method or
  ...
  preview](#camera_preview) below).

  Device manufacturers can apply this behavior to an app with
  [`OVERRIDE_ENABLE_COMPAT_IGNORE_REQUESTED_ORIENTATION`](#override_enable_compat_ignore_requested_orientation).
* The activity made more than two orientation requests in one second, which
  ...
  Device manufacturers can apply this behavior to an app with
  [`OVERRIDE_ENABLE_COMPAT_IGNORE_ORIENTATION_REQUEST_WHEN_LOOP_DETECTED`](#override_enable_compat_ignore_orientation_request_when_loop_detected).
* Virtual device owners have overridden the method call on select devices.

**Note:** Apps can opt out of both device manufacturer overrides, but can opt in
only for `OVERRIDE_ENABLE_COMPAT_IGNORE_REQUESTED_ORIENTATION`.
...
See also [Camera preview in desktop windowing](#camera_preview_desktop_windowing).
...
![](/static/images/guide/practices/compatibility-mode/camera_preview_issue.png)
...
guidance, see [Camera preview](/training/camera2/camera-preview) and [Introducing Camera
Viewfinder](https://android-developers.googleblog.com/2022/11/introducing-camera-viewfinder.html).

![](/static/images/guide/practices/compatibility-mode/camera_preview_optimization.png)
...
A device is in natural orientation when [`Display#getRotation()`](/reference/kotlin/android/view/Display#getRotation())
returns [`Surface.ROTATION_0`](/reference/kotlin/android/view/Surface#rotation_0). The system calculates
[`CameraCharacteristics.SENSOR_ORIENTATION`](/reference/kotlin/android/hardware/camera2/CameraCharacteristics#sensor_orientation)
...
* **Force rotate camera previews for portrait-restricted apps:** Apps
  ...
  On some tablets (see [reference devices](#reference_devices)), the app
  ...
  ![](/static/images/guide/practices/compatibility-mode/camera_preview_workaround_tablet.gif)
  ...
  devices](#reference_devices)), portrait-only activities are rotated to
  landscape to align with the unfolded natural orientation. The app is
  letterboxed after force rotation.

  ![](/static/images/guide/practices/compatibility-mode/camera_preview_workaround_foldable.gif)

  warning

  ### Foldable — Force rotation of portrait-restricted app. The app is also letterboxed.

  **Note:** Force rotation does not happen if your app is responsive and handles
...
* **Force refresh camera previews:** The system cycles through activity
  ...
  [OVERRIDE\_CAMERA\_COMPAT\_ENABLE\_REFRESH\_VIA\_PAUSE](#override_camera_compat_enable_refresh_via_pause)
...
correctly. See [Per-app overrides](#per-app_overrides).
...
* [`getMetrics()`](/reference/kotlin/android/view/Display#getmetrics)
* [`getSize()`](/reference/kotlin/android/view/Display#getSize(android.graphics.Point))
* [`getRealMetrics()`](/reference/kotlin/android/view/Display#getRealMetrics(android.util.DisplayMetrics))
* [`getRealSize()`](/reference/kotlin/android/view/Display#getRealSize(android.graphics.Point))
* [`getRectSize()`](/reference/kotlin/android/view/Display#getRectSize(android.graphics.Rect))
* [`getWidth()`](/reference/kotlin/android/view/Display#getWidth())
* [`getHeight()`](/reference/kotlin/android/view/Display#getHeight())

For more information, see
[Support multi-window mode](/develop/ui/compose/layouts/adaptive/support-multi-window-mode#window_metrics).

**Misused view APIs:**

* [`getWindowVisibleDisplayFrame()`](/reference/android/view/View#getWindowVisibleDisplayFrame(android.graphics.Rect))
* [`getLocationOnScreen()`](/reference/android/view/View#getLocationOnScreen(int%5B%5D))

![](/static/images/guide/practices/compatibility-mode/misused_APIs_issue.png)
...
app to APIs based on [`WindowMetrics`](/reference/kotlin/androidx/window/layout/WindowMetrics), including the following
`WindowManager` APIs:

* Platform:

  + [`getCurrentWindowMetrics()`](/reference/kotlin/android/view/WindowManager#getcurrentwindowmetrics)
  + [`getMaximumWindowMetrics()`](/reference/kotlin/android/view/WindowManager#getmaximumwindowmetrics)
* Jetpack:

  + [`WindowMetricsCalculator#computeCurrentWindowMetrics()`](/reference/kotlin/androidx/window/layout/WindowMetricsCalculator#computeCurrentWindowMetrics(android.app.Activity))
  + [`WindowMetricsCalculator#computeMaximumWindowMetrics()`](/reference/kotlin/androidx/window/layout/WindowMetricsCalculator#computeMaximumWindowMetrics(android.app.Activity))

![](/static/images/guide/practices/compatibility-mode/misused_APIs_optimization.png)
...
[`ALWAYS_SANDBOX_DISPLAY_APIS`](#always_sandbox_display_apis) for `Display`
APIs; [`OVERRIDE_SANDBOX_VIEW_BOUNDS_APIS`](#override_sandbox_view_bounds_apis)
...
![](/static/images/guide/practices/compatibility-mode/transparent_activities_issue.png)
...
layout typically uses [LayoutParams.WRAP\_CONTENT](/reference/android/view/ViewGroup.LayoutParams#WRAP_CONTENT) rather than
[LayoutParams.MATCH\_PARENT](/reference/android/view/ViewGroup.LayoutParams#MATCH_PARENT).

![](/static/images/guide/practices/compatibility-mode/transparent_activities_optimization.png)
...
multi-window mode](/develop/ui/compose/layouts/adaptive/support-multi-window-mode#lifecycle) correctly, causing
...
![](/static/images/guide/practices/compatibility-mode/unity_games_issue.gif)
...
![](/static/images/guide/practices/compatibility-mode/unity_games_optimization.gif)
...
[`OVERRIDE_ENABLE_COMPAT_FAKE_FOCUS`](#override_enable_compat_fake_focus)
...
to [portrait orientation](/guide/topics/manifest/activity-element#screen), users can still resize the app to landscape
orientation.

![Animation of a portrait app being resized to landscape.](/static/images/guide/practices/compatibility-mode/windowing_free_resizing.gif)
...
false](/guide/topics/manifest/activity-element#resizeableActivity)), the activity UI scales while maintaining the
same aspect ratio.

![Animation of app being resized. UI scales to fill the desktop window.](/static/images/guide/practices/compatibility-mode/windowing_non_resizable.gif)
...
window configuration changes. For detailed guidance, see [Camera preview](/training/camera2/camera-preview).
...
**Note:** Simulating requested orientation does not happen if your app is responsive
and handles portrait and landscape orientations.
...
![](/static/images/guide/practices/compatibility-mode/windowing_camera.gif)
...
* **Device streaming:** To test your app on production devices (including
  [reference devices](#reference_devices)) hosted in Google data centers, see
  [Android Device Streaming, powered by Firebase](/studio/preview/android-device-streaming)
* **Emulators in Android Studio:** For information on creating
  emulators for reference devices, see
  [Create and manage virtual devices](/studio/run/managing-avds)
* **Android Studio resizable emulator:** For information on accessing virtual
  devices, see [Run apps on the Android Emulator](/studio/run/emulator)
...
example, the [`FORCE_RESIZE_APP`](#force_resize_app) override instructs the
system to bypass size compatibility mode and resize the app to fit display
dimensions even if [`resizeableActivity="false"`](/guide/topics/manifest/activity-element#resizeableActivity) is
...
**Note:** Per-app overrides are intended to fix broken app behavior or improve the
user experience.
...
devices](#reference_devices) implement the menu.
...
**Note:** If users override the app aspect ratio to full screen, the app becomes
resizable and able to enter multi‑window mode, as if the app had been
configured with the manifest attribute
[`resizeableActivity="true"`](/guide/topics/manifest/activity-element#resizeableActivity).

Apps can opt out of the compatibility override by setting the following [`PackageManager.Property`](/reference/android/content/pm/PackageManager.Property) tags:
...
* **PROPERTY\_COMPAT\_ALLOW\_USER\_ASPECT\_RATIO\_FULLSCREEN\_OVERRIDE**
  ...
  **Caution:** If
  [`PROPERTY_COMPAT_ALLOW_USER_ASPECT_RATIO_OVERRIDE`](#aspect_ratio_override)
  is `false`, this property has no effect.

  Setting this property to `true` has no effect.

**Note:** To implement the property tags, your app must include the Jetpack
[WindowManager](/jetpack/androidx/releases/window) library dependency.

**Optimize your app for all screens:** Don't set aspect ratio restrictions in
your app. Use [window size classes](/develop/ui/compose/layouts/adaptive/use-window-size-classes) to support different
...
devices](#reference_devices) may apply some of the overrides to a variety of
apps by default.

Apps can opt out of most overrides (see the [Per-app
overrides](#device_manufacturer_overrides_table) table below).

You can test your app with overrides enabled or disabled using the compatibility
framework (see [Compatibility framework tools](/guide/app-compatibility/test-debug)).
When enabled, overrides apply to the entire app.

You can also use the [Android Debug Bridge (adb)](/tools/adb) to enable or disable
...
For the [reference devices](#reference_devices), check which overrides apply to
your app:
...
**Note:** If `dumpsys platform_compat` returns `true` for
`OVERRIDE_CAMERA_COMPAT_DISABLE_FORCE_ROTATION`, your app is *not* force
rotated, as mentioned in [Camera preview compatibility
workaround](#camera_preview_workaround).

If `dumpsys platform_compat` returns `false` for `OVERRIDE_CAMERA_LANDSCAPE_TO_PORTRAIT`, your app is *not* inner front camera cropped (see [Camera preview compatibility workaround](#camera_preview_workaround)).
...
| Input Compat | [OVERRIDE\_MOUSE\_TO\_TOUCH](#override_mouse_to_touch) | 413207127 | Converts `MotionEvent` instances from a mouse device into touch events by rewriting the `MotionEvent` source and tool type when they're delivered to the application. |
| Resizability | [FORCE\_RESIZE\_APP](#force_resize_app) | 174042936 | Bypasses size compatibility mode for app on configuration changes. |
|  | [FORCE\_NON\_RESIZE\_APP](#force_non_resize_app) | 181136395 | Forces app into size compatibility mode on configuration changes. |
| Aspect ratio | [OVERRIDE\_MIN\_ASPECT\_RATIO](#override_min_aspect_ratio) | 174042980 | Gatekeeper override that must be enabled to apply any other aspect ratio overrides. |
|  | [OVERRIDE\_MIN\_ASPECT\_RATIO\_PORTRAIT\_ONLY](#override_min_aspect_ratio_portrait_only) | 203647190 | If enabled (the default), limits override scope to portrait-only activities. |
|  | [OVERRIDE\_MIN\_ASPECT\_RATIO\_SMALL](#override_min_aspect_ratio_small) | 349045028 | Changes the minimum aspect ratio to 4:3. |
|  | [OVERRIDE\_MIN\_ASPECT\_RATIO\_MEDIUM](#override_min_aspect_ratio_medium) | 180326845 | Changes the minimum aspect ratio to 3:2. |
|  | [OVERRIDE\_MIN\_ASPECT\_RATIO\_LARGE](#override_min_aspect_ratio_large) | 180326787 | Changes the minimum aspect ratio to 16:9. |
|  | [OVERRIDE\_MIN\_ASPECT\_RATIO\_TO\_ALIGN\_WITH\_SPLIT\_SCREEN](#override_min_aspect_ratio_to_align_with_split_screen) | 208648326 | Changes the minimum aspect ratio to fit 50% of the display size (or split-screen aspect ratio). |
|  | [OVERRIDE\_MIN\_ASPECT\_RATIO\_EXCLUDE\_PORTRAIT\_FULLSCREEN](#override_min_aspect_ratio_exclude_portrait_fullscreen) | 218959984 | Disables the minimum aspect ratio override so that apps are full screen when device is portrait. |
| Orientation | [OVERRIDE\_ANY\_ORIENTATION](#override_any_orientation) | 265464455 | Enables overriding any orientation. |
|  | [OVERRIDE\_ANY\_ORIENTATION\_TO\_USER](#override_any_orientation_to_user) | 310816437 | Overrides orientation, resizability, and aspect ratio restrictions. |
|  | [OVERRIDE\_UNDEFINED\_ORIENTATION\_TO\_PORTRAIT](#override_undefined_orientation_to_portrait) | 265452344 | Overrides the orientation to be portrait when an activity has an undefined orientation. |
|  | [OVERRIDE\_UNDEFINED\_ORIENTATION\_TO\_NOSENSOR](#override_undefined_orientation_to_nosensor) | 265451093 | Overrides the orientation to be `nosensor` (use the natural orientation of device) when an activity has an undefined orientation. |
|  | [OVERRIDE\_LANDSCAPE\_ORIENTATION\_TO\_REVERSE\_LANDSCAPE](#override_landscape_orientation_to_reverse_landscape) | 266124927 | Rotates landscape-only apps 180 degrees. |
|  | [OVERRIDE\_ORIENTATION\_ONLY\_FOR\_CAMERA](#override_orientation_only_for_camera) | 265456536 | Limits orientation override scope to when app is connected to the camera. |
|  | [OVERRIDE\_USE\_DISPLAY\_LANDSCAPE\_NATURAL\_ORIENTATION](#override_use_display_landscape_natural_orientation) | 255940284 | Sets the display to fixed landscape natural orientation when a task is full screen (including when letterboxed). |
|  | [OVERRIDE\_ENABLE\_COMPAT\_IGNORE\_REQUESTED\_ORIENTATION](#override_enable_compat_ignore_requested_orientation) | 254631730 | Ignores orientation requests from app to avoid rotation infinite loops. |
...
|  | [OVERRIDE\_RESPECT\_REQUESTED\_ORIENTATION](#override_respect_requested_orientation) | 236283604 | Prevents letterboxing by disabling the device manufacturer ignore orientation request setting. |
| Sandbox APIs | [NEVER\_SANDBOX\_DISPLAY\_APIS](#never_sandbox_display_apis) | 184838306 | Prevents changing the behavior of any display APIs. |
|  | [ALWAYS\_SANDBOX\_DISPLAY\_APIS](#always_sandbox_display_apis) | 185004937 | Forces the `Display` APIs in the app to return app bounds. `Display` APIs return logical display area bounds, but sometimes the app assumes `Display` APIs return app bounds, which leads to UI issues. |
|  | [OVERRIDE\_SANDBOX\_VIEW\_BOUNDS\_APIS](#override_sandbox_view_bounds_apis) | 237531167 | Forces the `View` APIs used in the app to return app bounds. `View` APIs return logical display area bounds, but sometimes the app assumes `View` APIs return app bounds, which leads to UI issues. |
| Camera compat | [OVERRIDE\_CAMERA\_COMPAT\_DISABLE\_FORCE\_ROTATION](#override_camera_compat_disable_force_rotation) | 263959004 | Turns off force rotation. By default, all fixed-orientation camera apps are force rotated when the camera preview is open. |
|  | [OVERRIDE\_CAMERA\_COMPAT\_DISABLE\_SIMULATE\_REQUESTED\_ORIENTATION](#override_camera_compat_disable_simulate_requested_orientation) | 398195815 | Turns off simulating requested orientation treatment for camera compatibility. By default, fixed-orientation camera apps are adjusted based on their requested orientation when the camera preview is open. |
|  | [OVERRIDE\_CAMERA\_COMPAT\_DISABLE\_REFRESH](#override_camera_compat_disable_refresh) | 264304459 | Removes the default hard refresh applied when a camera preview is force rotated. |
|  | [OVERRIDE\_CAMERA\_COMPAT\_ENABLE\_REFRESH\_VIA\_PAUSE](#override_camera_compat_enable_refresh_via_pause) | 264301586 | Switches the hard refresh to a soft refresh when a camera preview is force rotated, which helps preserve state during the force rotation. By default, Android applies a hard refresh when the camera preview is force rotated. The hard refresh can cause issues with apps losing state or blacking out depending on how the apps cached their previous state. |
|  | [OVERRIDE\_CAMERA\_LANDSCAPE\_TO\_PORTRAIT](#override_camera_landscape_to_portrait) | 250678880 | Crops the image buffer of the inner front camera. If the override is disabled, the inner front camera cropping is removed and the field of view of the camera preview is increased. By default on some foldables (see [reference devices](#reference_devices)), the system crops the camera preview of all camera apps when using the inner front camera. |
| Miscellaneous | [OVERRIDE\_ENABLE\_COMPAT\_FAKE\_FOCUS](#override_enable_compat_fake_focus) | 263259275 | Prevents the app from being blacked out when the app loses focus in split-screen mode. App waits for focus before drawing the app content, which can cause the app to freeze or be blacked out. The override enables Android to send a fake focus event to the app, which signals to the app to begin drawing content again. |

### OVERRIDE\_MOUSE\_TO\_TOUCH

Enables the compatibility treatment that converts [`MotionEvent`](/reference/kotlin/android/view/MotionEvent) instances from a mouse or touchpad to touch events by rewriting the [`MotionEvent` source](/reference/kotlin/android/view/MotionEvent#getsource) to [`SOURCE_TOUCHSCREEN`](/reference/kotlin/android/view/InputDevice#source_touchscreen) and [`MotionEvent` tool type](/reference/kotlin/android/view/MotionEvent#gettooltype) to [`TOOL_TYPE_FINGER`](/reference/kotlin/android/view/MotionEvent#tool_type_finger) when the motion events are delivered to the application.
...
Your app should handle input events from mouse and touchpad, including touchpad gesture and mouse wheel scrolling. See [Keyboard, mouse, and trackpad](/guide/topics/large-screens/keyboard-mouse-and-trackpad-tier-2).

#### How to disable or opt out of override

Declare [`FEATURE_PC`](/reference/android/content/pm/PackageManager#FEATURE_PC) in a [`<uses-feature>`](/guide/topics/manifest/uses-feature-element) element in the application's manifest.
...
In the app manifest, set the [`android:resizeableActivity`](/guide/topics/manifest/activity-element#resizeableActivity)
attribute to `true`.
...
**Tip:** To support resizing while disabling multi‑window mode with
`android:resizeableActivity=false`, add the following `<meta-data>`
tag to the app manifest:
...
[android:resizeableActivity](/guide/topics/manifest/activity-element#resizeableActivity).
...
Set [`android:minAspectRatio`](/reference/android/R.attr#minAspectRatio) at the activity or app level.

#### How to optimize apps

Don't set aspect ratio restrictions in your app. Make sure your app [supports
different display sizes](/develop/ui/compose/layouts/adaptive/support-different-display-sizes). Use window size
classes to support different layouts based on the amount of space your app has
on the screen. See the
[Compose `WindowSizeClass` API](/reference/kotlin/androidx/compose/material3/windowsizeclass/WindowSizeClass) and [View
`WindowSizeClass` API](/reference/kotlin/androidx/window/core/layout/WindowSizeClass).
...
See [OVERRIDE\_MIN\_ASPECT\_RATIO](#override_min_aspect_ratio).
...
Set the [`activity:screenOrientation`](/guide/topics/manifest/activity-element#screen) manifest attribute,
or use the [`Activity#setRequestedOrientation()`](/reference/kotlin/android/app/Activity#setrequestedorientation) API.
...
orientations, see [Handle configuration changes](/guide/topics/resources/runtime-changes),
[ViewModel overview](/topic/libraries/architecture/viewmodel), and [App orientation restricted on
phones but not on large screen devices](/guide/topics/large-screens/large-screen-cookbook#restricted_app_orientation).

#### How to disable or opt out of override

Set the property flag
[`PROPERTY_COMPAT_ALLOW_ORIENTATION_OVERRIDE`](/reference/android/view/WindowManager#PROPERTY_COMPAT_ALLOW_ORIENTATION_OVERRIDE)
to `false`.
...
[`Activity#setRequestedOrientation()`](/reference/kotlin/android/app/Activity#setrequestedorientation) or
[`Activity#getRequestedOrientation()`](/reference/kotlin/android/app/Activity#getrequestedorientation).

#### How apps can achieve same result as override

* Do *not* set the [`android:screenOrientation`](/guide/topics/manifest/activity-element#screen) manifest
  attribute, or set the attribute to `"user"`.
* Set the [`android:resizeableActivity`](/guide/topics/manifest/activity-element#resizeableActivity) manifest
  attribute to `true`.

  **Note:** Settings for [`android:minAspectRatio`](/reference/android/R.attr#minAspectRatio) and
  [`android:maxAspectRatio`](/reference/android/R.attr#maxAspectRatio) are ignored if
...
* On small screens, to support app resizing while disabling multi‑window
  ...
  **Note:** On Android 12 (API level 31), all apps run in multi-window mode on
  large screens (medium or expanded [window size class](/develop/ui/compose/layouts/adaptive/use-window-size-classes))
  regardless of app configuration. See
  [Support multi-window mode](/develop/ui/compose/layouts/adaptive/support-multi-window-mode#resizeableActivity).
...
See [`OVERRIDE_ANY_ORIENTATION`](#override_any_orientation).
...
[OVERRIDE\_ANY\_ORIENTATION](#override_any_orientation) is enabled, the override
...
Enables [nosensor](/guide/topics/manifest/activity-element#screen) orientation for all activities in the
package. Unless [OVERRIDE\_ANY\_ORIENTATION](#override_any_orientation) is
...
Enables [reverseLandscape](/guide/topics/manifest/activity-element#screen) orientation for all activities in
the package. Unless [OVERRIDE\_ANY\_ORIENTATION](#override_any_orientation) is
enabled, the override is used only when no other fixed orientation has been
specified by the activity.
...
Set the property flag
[`PROPERTY_COMPAT_ALLOW_DISPLAY_ORIENTATION_OVERRIDE`](/reference/android/view/WindowManager#PROPERTY_COMPAT_ALLOW_DISPLAY_ORIENTATION_OVERRIDE)
to `false`.
...
calling [`Activity#setRequestedOrientation()`](/reference/kotlin/android/app/Activity#setrequestedorientation) when app
is relaunching or has an active camera compat treatment.

#### How apps can achieve same result as override

Set property flag
[`PROPERTY_COMPAT_IGNORE_REQUESTED_ORIENTATION`](/reference/android/view/WindowManager#PROPERTY_COMPAT_IGNORE_REQUESTED_ORIENTATION)
to `true`.
...
[`Activity#setRequestedOrientation()`](/reference/kotlin/android/app/Activity#setrequestedorientation) more than twice
...
Forces packages to never have [`Display`](/reference/kotlin/android/view/Display) API sandboxing applied
...
[`android:resizeableActivity`](/guide/topics/manifest/activity-element#resizeableActivity) manifest attribute to `true`
...
[`WindowSizeClass`](/reference/kotlin/androidx/compose/material3/windowsizeclass/WindowSizeClass) API to draw the UI based on how much screen
area the app has on the current display. See
[Use window size classes](/develop/ui/compose/layouts/adaptive/use-window-size-classes).
...
Forces packages to always have [`Display`](/reference/kotlin/android/view/Display) API sandboxing applied
...
[`android:resizeableActivity`](/guide/topics/manifest/activity-element#resizeableActivity) attribute to `false` or the
...
[`WindowMetricsCalculator`](/reference/kotlin/androidx/window/layout/WindowMetricsCalculator).
...
Forces packages to sandbox the following [`View`](/reference/kotlin/android/view/View) APIs to activity bounds:
...
Set property flag
[`PROPERTY_COMPAT_ALLOW_SANDBOXING_VIEW_BOUNDS_APIS`](/reference/android/view/WindowManager#PROPERTY_COMPAT_ALLOW_SANDBOXING_VIEW_BOUNDS_APIS)
to `false`.
...
Set property flag
[`PROPERTY_CAMERA_COMPAT_ALLOW_FORCE_ROTATION`](/reference/android/view/WindowManager#PROPERTY_CAMERA_COMPAT_ALLOW_FORCE_ROTATION)
to `false`.
...
app](/codelabs/android-camera2-preview#0).
...
Set property flag
[`PROPERTY_CAMERA_COMPAT_ALLOW_SIMULATE_REQUESTED_ORIENTATION`](/reference/android/view/WindowManager#PROPERTY_CAMERA_COMPAT_ALLOW_SIMULATE_REQUESTED_ORIENTATION)
to `false`.
...
Set property flag
[`PROPERTY_CAMERA_COMPAT_ALLOW_REFRESH`](/reference/android/view/WindowManager#PROPERTY_CAMERA_COMPAT_ALLOW_REFRESH)
to `false`.

#### How to optimize apps

See
[OVERRIDE\_CAMERA\_COMPAT\_DISABLE\_FORCE\_ROTATION](#override_camera_compat_disable_force_rotation).
...
Set property flag
[`PROPERTY_CAMERA_COMPAT_ENABLE_REFRESH_VIA_PAUSE`](/reference/android/view/WindowManager#PROPERTY_CAMERA_COMPAT_ENABLE_REFRESH_VIA_PAUSE)
to `true`.
...
Set property flag
[`PROPERTY_COMPAT_OVERRIDE_LANDSCAPE_TO_PORTRAIT`](/reference/android/content/pm/PackageManager#PROPERTY_COMPAT_OVERRIDE_LANDSCAPE_TO_PORTRAIT)
to `true`.
...
[Media projection](/media/grow/media-projection#partial_screen_sharing_opt_out)). Implemented when apps misuse the
[`createConfigForDefaultDisplay()`](/reference/kotlin/android/media/projection/MediaProjectionConfig#createconfigfordefaultdisplay) API to force
...
level 34, with [`createScreenCaptureIntent()`](/reference/kotlin/android/media/projection/MediaProjectionManager#createscreencaptureintent)),
...
[`createScreenCaptureIntent(MediaProjectionConfig)`](/reference/kotlin/android/media/projection/MediaProjectionManager#createscreencaptureintent_1)
with a [`MediaProjectionConfig`](/reference/kotlin/android/media/projection/MediaProjectionConfig) argument returned from a
call to [`createConfigForUserChoice()`](/reference/kotlin/android/media/projection/MediaProjectionConfig#createconfigforuserchoice).
...
Make your app resizable ([`resizeableActivity="true"`](/guide/topics/manifest/activity-element#resizeableActivity)) to
...
Set property flag
[`PROPERTY_COMPAT_ENABLE_FAKE_FOCUS`](/reference/android/view/WindowManager#PROPERTY_COMPAT_ENABLE_FAKE_FOCUS) to
`true`.
...
and app windowing modes by following the [Adaptive app quality guidelines](/docs/quality-guidelines/adaptive-app-quality).
...
* Compose: [About window insets](/develop/ui/compose/system/insets)
* Views: [Display content edge-to-edge in views](/develop/ui/views/layout/edge-to-edge)
* Configuration: [Behavior changes: Apps targeting Android 15 or higher](/about/versions/15/behavior-changes-15#stable-configuration)
...
Set property flag
[`PROPERTY_COMPAT_ALLOW_EXCLUDE_CAPTION_INSETS`](/reference/android/view/WindowManager#PROPERTY_COMPAT_ALLOW_EXCLUDE_CAPTION_INSETS) to
`false`.
...
* [Adaptive app quality guidelines](/docs/quality-guidelines/adaptive-app-quality)
* [Core app quality guidelines](/docs/quality-guidelines/core-app-quality)

---

1. A virtual device owner is a trusted or privileged app that manages a virtual device. Virtual device owners create virtual devices to render apps and then project the apps to remote devices, such as personal computers, virtual reality devices, or car infotainment systems. The virtual device owner is on a local device, such as a phone. [↩](#fnref1)