...
Next, check the RELRO segment alignment by summing its virtual offset address (VirtAddr) with
the segment memory size (MemSiz) and dividing by 16 KB (0x4000). If the
remainder (modulo) is zero, then the RELRO segment is 16 KB aligned.
...
Since 0x1000 is not zero, the libbad.so isn't 16 KB compliant. The RELRO
