...
:   Changes that affect all apps are enabled by
    ...
    you could install your app on a device running
    Android 16 and test your app using your typical testing
    workflows. If your app encounters issues, you could disable the change that
    is causing the issue and continue testing for other issues.

    These changes can affect all apps, regardless of `targetSDKVersion`,
    so test and update your app for these changes before changes
    that are gated by `targetSDKVersion`. This helps ensure that users'
    experience won't degrade when they update to a new
    platform version.
...
app manifest and opting in to every change at once.

This extra control can help
...
![List of changes that can be toggled on or off](https://developer.android.com/static/images/guide/app-compatibility/appcompat-dev-options-toggle.png) **Figure 2.** Toggling changes on or off.
