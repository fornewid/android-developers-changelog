---
title: Compatibility framework tools  |  Platform  |  Android Developers
...
# Compatibility framework tools Stay organized with collections Save and categorize content based on your preferences.
...
options or ADB](#toggle). Use this flexibility as you prepare to [target the latest
stable API version](/distribute/best-practices/develop/target-sdk) and as you [test your app with the preview release of the
next Android version](/about/versions/13).
...
[`targetSDKVersion`](/guide/topics/manifest/uses-sdk-element#target) or recompile your app to perform basic testing. Because
...
![](/static/images/guide/app-compatibility/appcompat-dev-options.png)

**Figure 1.** App Compatibility Changes
screen in developer options.

You can see which changes are enabled and [toggle those changes on or off](#toggle) in
a device's developer options. To access these options, follow these steps:

1. If developer options are not already enabled, [enable them](/studio/debug/dev-options#enable).
2. Open your device's Settings app and navigate to
   **System > Advanced > Developer options > App Compatibility Changes**.
3. Select your app from the list.

   **Note:** You must use a [debuggable build variant](/studio/debug#enable-debug) of your app to toggle
   changes on and off.
...
* Changes that only affect apps that are targeting certain Android versions.
  ...
  **Enabled for targetSdkVersion >=33**. On some lower versions of Android,
  this section is titled "Enabled After SDK API\_LEVEL"
  instead.
...
framework list for that Android version](#learn-more).
...
* [Android 16 (API level 36)](/about/versions/16/reference/compat-framework-changes)
* [Android 15 (API level 35)](/about/versions/15/reference/compat-framework-changes)
* [Android 14 (API level 34)](/about/versions/14/reference/compat-framework-changes)
* [Android 13 (API level 33)](/about/versions/13/reference/compat-framework-changes)
* [Android 12 (API levels 31 and 32)](/about/versions/12/reference/compat-framework-changes)
* [Android 11 (API level 30)](/about/versions/11/reference/compat-framework-changes)
...
:   Changes that affect all apps are enabled by
    ...
    these changes off [when using a public release build of Android](#restrictions).
...
[restrictions on when you can toggle changes](#restrictions).
...
4. From the list of changes, find the change that you want to toggle on or off
   ...
   ![List of changes that can be toggled on or off](/static/images/guide/app-compatibility/appcompat-dev-options-toggle.png)
...
a change depends on the type of change, whether your app is [debuggable](/guide/topics/manifest/application-element#debug),
