---
title: Restrictions on non-SDK interfaces  |  Platform  |  Android Developers
...
# Restrictions on non-SDK interfaces Stay organized with collections Save and categorize content based on your preferences.
...
Android framework [Package Index](/reference/packages). Handling of non-SDK interfaces is an
...
[give us feedback](#feature-request), and time to plan and adjust to the new policies.
...
| SDK | * Both `public-api` and `sdk` * Deprecated: Both `public-api` and `whitelist` | Interfaces that can be freely used and are now supported as part of the officially documented Android framework [Package Index](/reference/packages). |
| Test APIs | * `test-api` | Interfaces that are used for internal system testing, such as APIs that facilitate testing through the Compatibility Test Suite (CTS). Test APIs are **not** part of the SDK. [Starting in Android 11 (API level 30)](/about/versions/11/non-sdk-11#test-api-restrictions), test APIs are included in the blocklist, so apps aren't allowed to use them regardless of their target API level. All test APIs are unsupported and subject to change without notice, regardless of the platform API level. |
...
[request a new public API](#feature-request).
...
Android 16](/about/versions/16/changes/non-sdk-16).
...
see [Updates to non-SDK interface restrictions in Android 15](/about/versions/15/changes/non-sdk-15).
...
see [Updates to non-SDK interface restrictions in Android 14](/about/versions/14/changes/non-sdk-14).
...
restrictions in Android 13](/about/versions/13/changes/non-sdk-13).
...
Android 12](/about/versions/12/non-sdk-12#list-changes).
...
Android 11, see [List changes for Android 11](/about/versions/11/non-sdk-11#r-list-changes).
...
suggested public API alternatives for APIs that are conditionally blocked in
Android 10, see [List changes for Android 10](/about/versions/10/non-sdk-q#q-list-changes).
...
[debuggable app](/studio/debug#enable-debug) on a device or emulator running Android 9 (API level 28) or
higher. Make sure that the device or emulator that you are using matches the
[target API level](/distribute/best-practices/develop/target-sdk) of your app.
...
* The declaring class, name, and type (in the format that is used by the
  Android runtime).
...
[`detectNonSdkApiUsage`](/reference/android/os/StrictMode.VmPolicy.Builder#detectNonSdkApiUsage()) method to enable this. After enabling the
`StrictMode` API, you can receive a callback for each usage of a non-SDK
interface by using a [`penaltyListener`](/reference/android/os/StrictMode.VmPolicy.Builder#penaltyListener(java.util.concurrent.Executor,%20android.os.StrictMode.OnVmViolationListener)), where you can implement custom
...
Whenever you build your app in Android Studio, the [lint tool](/studio/write/lint) inspects your
code for potential issues. If your app uses non-SDK interfaces, you might see
build errors or warnings, depending on [which list](#list-names) those interfaces belong
to.

You can also [run the lint tool from the command line](/studio/write/lint#commandline) or [run inspections
manually](/studio/write/lint#manuallyRunInspections) on a specific project, folder, or file.
...
the pre-launch report, depending on [which list](#list-names) those interfaces belong to.
...
* Which unsupported API you are using, including the full descriptor seen in
  the `Accessing hidden ...` logcat message.
...
* additional static analysis that was designed to conservatively include more
  false positives
...
* 0: Disable all detection of non-SDK interfaces. Using this setting disables
  ...
  app [using the `StrictMode` API](#test-strictmode-api). This setting is not recommended.
* 1: Enable access to all non-SDK interfaces, but print log messages with
  warnings for any non-SDK interface usage. Using this setting also lets you
  test your app [using the `StrictMode` API](#test-strictmode-api).
* 2: Disallow usage of non-SDK interfaces that belong to the blocklist or are
  conditionally blocked for your [target API level](/distribute/best-practices/develop/target-sdk).
...
SDK interfaces or other alternatives, and [request a new public API](#feature-request) whenever
