---
title: Content provider basics  |  App data and files  |  Android Developers
...
# Content provider basics Stay organized with collections Save and categorize content based on your preferences.
...
![Relationship between content provider and other components.](/static/guide/topics/providers/images/content-provider-tech-stack.png)
...
`Activity` or `Fragment` in your UI calls a
...
![Interaction between ContentProvider, other classes, and storage.](/static/guide/topics/providers/images/content-provider-interaction.png)
...
[Content provider permissions](#Permissions) section.
...
| --- | --- | --- |
| `Uri` | `FROM table_name` | `Uri` maps to the table in the provider named table\_name. |
...
the User Dictionary Provider's `Words` table. The `ContentResolver`
...
* The `content://` string is the *scheme*, which is always present
...
a URI. The previous snippet uses `withAppendedId()` to append an ID to the User Dictionary Provider content URI.
...
use the `CursorLoader` class, which is described
in more detail in the [Loaders](/guide/components/loaders) guide. Also, the lines of code are snippets only. They don't show a complete
application.
...
return `null`, or it can throw an `Exception`.
...
other providers. To do this, you iterate over the rows in the `Cursor`, as shown in the following example:
...
this, read the [Protect against malicious input](#Injection) section.
...
this, read the [Protecte against malicious input](#Injection) section.
...
described in the [Contract classes](#ContractClasses) section.
...
For example, the `ContactsContract.Data`
...
The [MIME type reference](#MIMETypeReference) section describes the
...
* [Batch access](#Batch): you can create a batch of access calls with methods in
  the `ContentProviderOperation` class and then apply them with
  `ContentResolver.applyBatch()`.
* Asynchronous queries: do queries in a separate thread. You can
  use a `CursorLoader` object. The examples in the
  [Loaders](/guide/components/loaders) guide demonstrate
  how to do this.
* [Data access using intents](#Intents): although you can't send an intent
  directly to a provider, you can send an intent to the provider's application, which is
  usually best-equipped to modify the provider's data.
...
create an array of `ContentProviderOperation` objects and then
dispatch them to a content provider with
`ContentResolver.applyBatch()`. You pass the
content provider's [authority](/guide/topics/manifest/provider-element#auth) to this method, rather than a particular content URI.
...
* Read from, or write to, the data that the content URI represents,
  depending on the flag included in the intent.
* Gain [package
  visibility](/training/basics/intents/package-visibility) into the app containing the content provider that matches the
  URI authority. The app that sends the intent and the app that
  contains the content provider might be two different apps.
...
[Permissions on Android](/guide/topics/permissions/overview#uri) guide.
...
requesting `READ_CONTACTS`, which gives you access to all
...
1. In your application, send an intent containing the action
   `ACTION_PICK` and the "contacts" MIME type
   `CONTENT_ITEM_TYPE`, using the
   method `startActivityForResult()`.
2. Because this intent matches the intent filter for the
   People app's "selection" activity, the activity comes to the foreground.
3. In the selection activity, the user selects a
...
4. Your activity returns to the foreground, and the system calls your activity's
   `onActivityResult()`
   method. This method receives the result intent created by the selection activity in
   the People app.
5. With the content URI from the result intent, you can read the contact's data
   ...
   or email address and then send the e-greeting.
...
`ACTION_INSERT` intent that lets you activate the
...
`ACTION_VIEW` intent that displays a particular date or event.
...
complex type and subtype values. For multiple rows, the type value is always the following:
...
[Content URIs](#ContentURIs) section.

[Next

Creating a content provider

arrow\_forward](/guide/topics/providers/content-provider-creating)