---
title: Per-app language preferences  |  App architecture  |  Android Developers
...
# Per-app language preferences Stay organized with collections Save and categorize content based on your preferences.
...
* **System settings**: A centralized location where users can select a
  ...
  [enabling automatic per-app language support](#auto-localeconfig).
* **Additional APIs**: These public APIs, such as the
  ...
  Apps](/guide/topics/data/autobackup) to store app-level user language
  settings.
  ...
  [Appcompat 1.6.0](/jetpack/androidx/releases/appcompat#version_160_3)
  or higher.
...
support manually](#use-localeconfig).

### Enable automatic per-app language support

**Note:** This feature won't make changes to your project in the same way as the
[manual setup process](/guide/topics/resources/app-languages#use-localeconfig).
All changes from the automatic process are done in generated files. For
example, you won't see an `android:localeConfig` entry in your app's manifest
or a file at `res/xml/locales_config.xml`. That is expected since all of the
changes are generated automatically when the app is built.

Starting with Android Studio Giraffe and AGP 8.1, you can configure your app to
support [per-app language
preferences](/guide/topics/resources/app-languages)
automatically. Based on your project resources, AGP generates the `LocaleConfig`
file and adds a reference to it in the final manifest file, so you no longer
have to do it manually. AGP uses the resources in the `res` folders of your app
modules and any library module dependencies to determine the locales to include
in the `LocaleConfig` file. This means that if you add resources for a new
language to your app, you don't have to worry about updating the `LocaleConfig`
file.
...
[use the APIs and in-app language pickers](/guide/topics/resources/app-languages#api-implementation).
...
2. Specify a default locale:
   ...
   2. In the `resources.properties` file, set the default
      ...
      locale names, see [How to form locale names](#locale-names).

AGP adds this default locale and any
[alternative locales](/guide/topics/resources/localization#creating-alternatives)
...
[enabling automatic per-app language support](#auto-localeconfig) instead of
configuring it manually if possible.

You can manually set up your app to to ensure its languages are configurable in
system settings on devices running Android 13 or higher. To do this, create a
`locales_config` XML file and add it your app's manifest using the
...
1. Create a file called `res/xml/locales_config.xml` and specify your app's
   languages, including your app's
   [ultimate fallback locale](/guide/topics/resources/multilingual-support),
   which is the locale specified in `res/values/strings.xml`.

   See [How to form locale names](#locale-names) for the format requirements.
   See also the [sample `locale_config.xml`file](#sample-config) for a list of
   the most commonly used locales.

   For example, format the `locales_config.xml` file like this for an app that
...
[`LocaleManager.getApplicationLocales`](/reference/android/app/LocaleManager#getApplicationLocales(java.lang.String))
...
[`resourceConfigurations`](/reference/tools/gradle-api/7.4/com/android/build/api/dsl/BaseFlavor#resourceConfigurations())
...
When the `resourceConfigurations` property is present, the build system only includes
...
sure to follow the guidance in [localize your
app](/guide/topics/resources/localization) for those languages. Adding per-app
...
match whichever language the user selects through your in-app experience.

For backward compatibility with previous Android versions, we strongly recommend
using the AndroidX support library when implementing an in-app language picker.
However, you can also [implement the framework APIs directly](#framework-impl)
if you need to.

### Implement using the AndroidX support library

Use the [`setApplicationLocales()`](/reference/androidx/appcompat/app/AppCompatDelegate#setApplicationLocales(androidx.core.os.LocaleListCompat)) and [`getApplicationLocales()`](/reference/androidx/appcompat/app/AppCompatDelegate#getApplicationLocales())
methods in [Appcompat 1.6.0](/jetpack/androidx/releases/appcompat#version_160_3)
or higher. Note the backward compatible APIs work with the AppCompatActivity context,
not the application context, for Android 12 (API level 32) and earlier.

**Caution:** If you're using Compose with `setApplicationLocales()`, you must extend
your activity from `AppCompatActivity`. Otherwise, setting the app locale
won't work. Refer to our
[sample app](https://github.com/android/user-interface-samples/tree/main/PerAppLanguages/compose_app)
to get started.

For example, to set a user's preferred language, you would ask the user to
select a locale in the language picker, then set that value in the system:
...
your app [handles locale configuration
changes](/guide/topics/resources/runtime-changes) by itself.

Use [`AppCompatDelegate.getApplicationLocales()`](/reference/androidx/appcompat/app/AppCompatDelegate#getApplicationLocales()) to retrieve the user's preferred locale. The user might have selected their app locale from [system settings](#app-language-settings) or from your in-app language picker.
...
[`StrictMode`](/reference/kotlin/android/os/StrictMode) `diskRead` and
`diskWrite` violation if you are logging thread violations. See
[`AppCompatDelegate.setApplicationLocales()`](/reference/androidx/appcompat/app/AppCompatDelegate#setApplicationLocales(androidx.core.os.LocaleListCompat))
...
[`setApplicationLocales()`](/reference/android/app/LocaleManager#setApplicationLocales(android.os.LocaleList))
and [`getApplicationLocales()`](/reference/android/app/LocaleManager#getApplicationLocales())
...
To get a user's current preferred language to display in the language picker,
your app can get the value back from the system:
...
[`EXTRA_LANGUAGE`](/reference/android/speech/RecognizerIntent#EXTRA_LANGUAGE)
feature from the Speech Recognizer API.

### Consider the Accept-Language header for Chrome Custom tab

Consider adding the [Accept-Language Header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Accept-Language)
through the [`Browser.EXTRA_HEADERS`](/reference/android/provider/Browser#EXTRA_HEADERS)
...
[`LocaleListCompat.getEmptyLocaleList()`](/reference/androidx/core/os/LocaleListCompat#getEmptyLocaleList())
or
[`LocaleList.getEmptyLocaleList()`](/reference/android/os/LocaleList#getEmptyLocaleList())
...
Language and locale resolution](/guide/topics/resources/multilingual-support)