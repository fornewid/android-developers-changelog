...
- Moving the app between displays with different screen densities (such as connected external monitors or the displays of a foldable device)
...
- **Configurations:** Device configurations define how the UI displays to the user, such as app display size, screen density, locale, or system theme. In Compose, you can access configuration values using `LocalConfiguration`.
