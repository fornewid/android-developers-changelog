---
title: App resources overview  |  App architecture  |  Android Developers
...
# App resources overview Stay organized with collections Save and categorize content based on your preferences.
...
[Put app icons in mipmap directories](/training/multiscreen/screendensities#mipmap).
...
| `xml/` | Arbitrary XML files that can be read at runtime by calling `Resources.getXML()`. Various XML configuration files must be saved here, such as a [Search configuration](/guide/topics/search/searchable-config). |
| `font/` | Font files with extensions such as TTF, OTF, or TTC, or XML files that include a `<font-family>` element. For more information about fonts as resources, see [Add a font as an XML resource](/guide/topics/ui/look-and-feel/fonts-in-xml). |

**Caution:** Never save resource files directly inside the
`res/` directory. It causes a compiler error.

For more information about the individual types of resources, see the [Resource types overview](/guide/topics/resources/available-resources).
...
* You can specify multiple qualifiers for a single set of resources, separated by dashes. For
  ...
  orientation.
* The qualifiers must be in the order listed in [table 2](#table2).
...
* Only one value for each qualifier type is supported. For example, if you want to use
  ...
  can create an *alias* to a resource, as described in the [Create
  alias resources](#AliasResources) section.
...
best-matching resource](#BestMatch).
...
[Other simple values](/guide/topics/resources/more-resources) work the
...
* The *resource type*: each resource is grouped into a "type," such as `string`,
  `drawable`, and `layout`. For more information about the different types, see [Resource types overview](/guide/topics/resources/available-resources).
...
* **In code:** using a static integer from a subclass of your `R`
  ...
  information, see the [Access resources in code](#ResourcesFromCode) section.
* **In XML:** using a special XML syntax that corresponds to
  ...
  information, see the [Access resources from XML](#ResourcesFromXml) section.
...
For
more information about each resource type and how to reference them, see [Resource types overview](/guide/topics/resources/available-resources).

#### Use cases

There are many methods that accept a resource ID parameter, and you can retrieve resources using
methods in `Resources`. You
can get an instance of `Resources` using `Context.getResources()`.
...
For example, if you add a `Button` to your layout, use
a [string resource](/guide/topics/resources/string-resource) for the button text:
...
example, if you have the following resource file that includes a [color resource](/guide/topics/resources/more-resources#Color) and a [string resource](/guide/topics/resources/string-resource):
...
resources](#AlternativeResources). For a complete guide to localizing your application for other languages,
see [Localize your app](/guide/topics/resources/localization).
...
information, see the section about [creating alias resources](#AliasResources).
...
For example, if your app supports several languages, always include a `values/` directory (in which your strings are saved) *without* a [language and region qualifier](#LocaleQualifier). If you instead put all your string files
...
For example, if your [`minSdkVersion`](/guide/topics/manifest/uses-sdk-element#min) is set to 4, and you qualify all of your drawable resources using [night mode](#NightQualifier) (`night` or `notnight`, which were added in API
...
resources with the [screen density](#DensityQualifier) qualifier. Even without default
...
1. Eliminate resource files that contradict the device configuration.
   ...
   **Exception:** Screen pixel density is the one qualifier that is not
   ...
   considered to be a match at this point. For information, see [Screen compatibility overview](/guide/practices/screens_support).
2. Find the next-highest-precedence qualifier in the list ([table 2](#table2)).
   (Start with MCC.)
...
4. Eliminate resource directories that don't include this qualifier. In this example, the system
   ...
   **Exception:** If the qualifier in question is screen pixel density,
   ...
   compatibility overview](/guide/practices/screens_support).
5. Repeat steps two, three, and four until only one directory remains. In this example, screen
...
**Note:** The *precedence* of the qualifier (in [table 2](#table2)) is more important
