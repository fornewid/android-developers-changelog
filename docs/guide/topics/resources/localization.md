...
You need to have a working knowledge of Kotlin and be familiar with [Android resource loading](https://developer.android.com/guide/topics/resources/providing-resources), development considerations such as the [activity lifecycle](https://developer.android.com/guide/components/activities/activity-lifecycle), and general principles of internationalization and localization.
...
This guide covers the core Android localization system used across all Android apps. To learn how to load these localized resources in your Jetpack Compose UI, see [Resources in Compose](https://developer.android.com/develop/ui/compose/resources).
...
An app's Kotlin-based code refers to just two strings, `text_a` and `text_b`. The app includes a localized resource file (`res/values-en/strings.xml`) that defines `text_a` and `text_b` in English. The app also includes a default resource file (`res/values/strings.xml`) that includes a definition for `text_a`, but not for `text_b`.
...
The default resource set also includes any default drawables and can include other types of resources such as icons or strings. These resources go in the following directories:
...
A large part of localizing an app is providing alternative text for different languages. In some cases, you also provide alternative graphics, sounds, and other locale-specific resources.
...
If your Kotlin-based code refers to `R.string.title`, here is what happens at runtime:
...
In your app's Kotlin-based code, you refer to resources using the syntax `R.resource_type.resource_name` or `android.R.resource_type.resource_name`*.* For more information, see [Access your app resources](https://developer.android.com/guide/topics/resources/accessing-resources).
...
Make sure that you read and follow the Material Design recommendations for [writing style and word choice](https://m3.material.io/foundations/content-design/style-guide/ux-writing-best-practices). Doing so makes your apps appear more polished to the user and helps users understand your UI more quickly.
...
You probably don't need to create a locale-specific alternative for every resource in your app. For example, an app logo defined in the `res/drawable/` directory might work in any locale, in which case there is no need to create any alternative graphic files.
...
You can look up the locale using the `LocalConfiguration` that Android makes available, as shown in the following example:
...
You can use Gemini in Android Studio to translate your app's string resources directly in your project. For more information, see [Translate and localize your app](https://support.google.com/googleplay/android-developer/answer/9844778#apps_translation_using_gemini).
...
Before testing on a device, you can use [composable previews](https://developer.android.com/develop/ui/compose/tooling/previews) in Android Studio to test localized UIs without deploying to an emulator. To preview your UI in different languages, use the [`@Preview`](https://developer.android.com/reference/kotlin/androidx/compose/ui/tooling/preview/Preview) annotation (like `@Preview(locale = "fr")`). You can also test right-to-left (RTL) layouts by specifying an RTL locale like `@Preview(locale = "ar")`.
...
This causes the emulator to restart. Once the Home screen appears again, relaunch your app, and the app launches with the new locale.
...
If the test is successful, repeat it for other types of configurations. For example, if the app has a string file called `res/values-land/strings.xml` but doesn't contain a file called `res/values-port/strings.xml`, then set the emulator or device to portrait orientation and see whether the app runs.
