...
:   Adds an action to an intent filter.
    An `https://developer.android.com/guide/topics/manifest/intent-filter-element`
    element must contain one or more `<action>` elements. If there
    are no `<action>` elements in an intent filter, the filter
    doesn't accept any `https://developer.android.com/reference/android/content/Intent` objects.
...
:
    ...
    :   The name of the action. Some standard actions are defined in the
        `https://developer.android.com/reference/android/content/Intent#ACTION_CHOOSER` class as
