...
description:
:   Declares an activity (an `https://developer.android.com/reference/android/app/Activity` subclass) that
...
:
    ...
    :   Indicates that the activity can be launched as the embedded child of another
    ...
    :   Whether the activity can move from the task that started it to
        ...
        front. It's `"true"` if it can move, and `"false"` if it
        remains with the task where it started.


        If this attribute isn't set, the value set by the corresponding
        `https://developer.android.com/guide/topics/manifest/application-element#reparent`
        attribute of the `https://developer.android.com/guide/topics/manifest/application-element` element
        ...
        `https://developer.android.com/guide/topics/manifest/activity-element#aff` attribute. The affinity
        ...
        modes. (See also the `https://developer.android.com/guide/topics/manifest/activity-element#lmode`
        attribute.)

    `android:alwaysRetainTaskState`
    :   Whether the state of the task that the activity is in is always
        maintained by the system. `"true"` if it is, and
        `"false"` if the system can reset the task to its initial
    ...
    :   Whether tasks launched by the activity with this attribute remain in the
        [Recents screen](https://developer.android.com/guide/components/recents) until the last activity in the
        task completes. If `true`, the task is
        automatically removed from the Recents screen. This overrides the caller's use of
        `https://developer.android.com/reference/android/content/Intent#FLAG_ACTIVITY_RETAIN_IN_RECENTS`. It must be a boolean value, either
        `"true"` or `"false"`.
    ...
    :   A [drawable resource](https://developer.android.com/guide/topics/resources/drawable-resource)
        ...
        `<activity>` tag to supply a default banner for a specific activity or with the
        ...
        the image, such as `"@drawable/banner"`. There is no default banner.
    ...
    :   Indicates whether the activity can be displayed on a remote device which
    ...
    :   Whether all activities are removed from the task, except for
        ...
        When the value is `"true"`, every time users start the task, they
        ...
        leave it. When the value is `"false"`, the task can be cleared of activities in
        some situations, but not always. For more information, see the
        `https://developer.android.com/guide/topics/manifest/activity-element#always` attribute.
        ...
        of the activities on top of it---in this case, Q---are removed when the user launches
        activity P from the home screen. So, the user sees only P when returning to the task.


        If this attribute and `https://developer.android.com/guide/topics/manifest/activity-element#reparent`
    ...
    :   Specifies the activity's color mode. If specified, can be either `hdr` or
        `wideColorGamut`.

        If `hdr`, requests that the activity be displayed in a high dynamic range if the
        device supports it.
        ...
        If `wideColorGamut`, requests that the activity be displayed in wide color gamut mode
        on compatible devices. In wide color gamut mode, a window can render outside of the
        `https://developer.android.com/reference/android/graphics/ColorSpace.Named#SRGB` gamut to
    ...
    :   Lists configuration changes that the activity handles itself. When a configuration
        ...
        activity remains running and its `https://developer.android.com/reference/android/app/Activity#onConfigurationChanged(android.content.res.Configuration)` method is called.

        **Note:** Use this attribute only in special cases to improve application
        performance and responsiveness. For more information, see [Handle configuration changes](https://developer.android.com/guide/topics/resources/runtime-changes).


        The following strings are valid values for this attribute. Multiple values are
        separated by `|`, such as `"locale|navigation|orientation"`.
        ...
        | `"uiMode"` | A change to the user interface mode, such as when the user places the device into a desk or car dock, or the night mode changes. For more information about the different UI modes, see `https://developer.android.com/reference/android/app/UiModeManager`. *Added in API level 8*. |
    ...
    :   Whether the activity is *Direct-Boot aware*---that is,
        whether it can run before the user unlocks the device.

        **Note:** During
        [Direct Boot](https://developer.android.com/training/articles/direct-boot), an activity in
    ...
    :   Specifies how a new instance of an activity is added to a task each time it is launched.
        This attribute permits the user to have multiple documents from the same application
        appear in the [Recents screen](https://developer.android.com/guide/components/recents).

        This attribute has four values, which produce the following effects when the user opens a document
        with the application:


        | Value | Description |
        |---|---|
        | `"intoExisting"` | The system searches for a task whose base intent's `ComponentName` and data URI match those of the launching intent. If the system finds such a task, the system clears the task and restarts, with the root activity receiving a call to `https://developer.android.com/reference/android/app/Activity#onNewIntent(android.content.Intent)`. If the system doesn't find such a task, the system creates a new task. |
        | `"always"` | The activity creates a new task for the document, even if the document is already opened. This is the same as setting both the `https://developer.android.com/reference/android/content/Intent#FLAG_ACTIVITY_NEW_DOCUMENT` and `https://developer.android.com/reference/android/content/Intent#FLAG_ACTIVITY_MULTIPLE_TASK` flags. |
        | `"none"` | The activity doesn't create a new task for the activity. This is the default value, which creates a new task only when `https://developer.android.com/reference/android/content/Intent#FLAG_ACTIVITY_NEW_TASK` is set. The Recents screen treats the activity as it does by default: it displays a single task for the app, which resumes from whatever activity the user last invoked. |
        | `"never"` | The activity isn't launched into a new document even if the intent contains `https://developer.android.com/reference/android/content/Intent#FLAG_ACTIVITY_NEW_DOCUMENT`. Setting this overrides the behavior of the `FLAG_ACTIVITY_NEW_DOCUMENT` and `https://developer.android.com/reference/android/content/Intent#FLAG_ACTIVITY_MULTIPLE_TASK` flags, if either of these are set in the activity, and the Recents screen displays a single task for the app, which resumes from whatever activity the user last invoked. |


        **Note:** For values other than `"none"` and `"never"`, the
        activity is defined with `launchMode="standard"`. If this attribute isn't specified,
    ...
    :   Whether the activity can be instantiated by the system. It's
        ...
        The `https://developer.android.com/guide/topics/manifest/application-element`
        element has its own `https://developer.android.com/guide/topics/manifest/application-element#enabled`
        ...
        is `"false"`, it can't be instantiated.
    ...
    :   This flag lets you opt out of predictive system animations at the activity level.
        ...
        Set `android:enableOnBackInvokedCallback=false` to turn off predictive back animations at the activity
        level and instruct the system to ignore calls to the `OnBackInvokedCallback` platform API.

    `android:excludeFromRecents`

    :   Whether the task initiated by this activity is excluded from the [Recents screen](https://developer.android.com/guide/components/recents). That is, when this activity is the root activity of a new
        task, this attribute determines whether the task appears in the list of recent apps. It's
        `"true"` if the task is *excluded* from the list; `"false"` if it
        is *included* . The default value is `"false"`.
    ...
    :   Whether the activity can be launched by components of other
        ...
        `"true"` to let other apps start it.
        For example, if the activity is the main activity
        of the app and includes the `https://developer.android.com/guide/topics/manifest/category-element`
    ...
    :   Whether an existing instance of the activity is shut down,
        ...
        home screen. It's `"true"` if it is shut down, and `"false"`
    ...
    :   Whether hardware-accelerated rendering is enabled for this
        activity. `"true"` if it is enabled, and `"false"` if
        not. The default value is `"false"`.


        On Android 3.0 and higher, a hardware-accelerated OpenGL renderer is
        ...
        enable hardware acceleration, your app consumes more RAM.
        ...
        use the renderer without errors.
    ...
    :   An icon representing the activity. The icon is displayed to users when
        ...
        The icon is often accompanied by a label; for information about the label, see the [`android:label`](https://developer.android.com/guide/topics/manifest/activity-element#label) attribute.
        ...
        `https://developer.android.com/guide/topics/manifest/application-element`
        element's `https://developer.android.com/guide/topics/manifest/application-element#icon` attribute.
    ...
    :   A user-readable label for the activity. The label displays on-screen
        ...
        used instead. see the `https://developer.android.com/guide/topics/manifest/application-element` element's
        `https://developer.android.com/guide/topics/manifest/application-element#label` attribute.
        ...
        `https://developer.android.com/guide/topics/manifest/intent-filter-element` element's
        `https://developer.android.com/guide/topics/manifest/intent-filter-element#label` attribute.
    ...
    :   An instruction for how the activity launches. There are five modes,
        which work in conjunction with activity flags (`FLAG_ACTIVITY_*` constants)
        in `https://developer.android.com/reference/android/content/Intent` objects to determine what happens when
        ...
        `"singleInstancePerTask"` activities on the other. An activity with the
        `"standard"` or `"singleTop"` launch mode
        can be instantiated multiple times.

        The instances can belong to any task
        and can be located anywhere in the activity task. Typically, they're
        launched into the task that called
        `https://developer.android.com/reference/android/content/Context#startActivity(android.content.Intent)`, unless the `Intent` object contains a
        `https://developer.android.com/reference/android/content/Intent#FLAG_ACTIVITY_NEW_TASK`
        instruction, in which case a different task is chosen. For more information, see the
        [`taskAffinity`](https://developer.android.com/guide/topics/manifest/activity-element#aff) attribute.


        In contrast, `"singleTask"`, `"singleInstance"`, and
        `"singleInstancePerTask"` activities have different behaviors.
        `"singleInstancePerTask"` is always at the root of the activity task.
        Also, the device can hold only one instance of the
        `"singleInstance"` activity at a time, while the
        `"singleInstancePerTask` activity can be instantiated multiple times
        ...
        `"singleInstancePerTask"`: the activity can be instantiated multiple
        times and can be located anywhere in a task of the same `taskAffinity`.
        But the device can only hold one task for locating the
        `"singleTask"` activity at the root of the activity task.


        The `"standard"` and `"singleTop"` modes differ from each other
        in one respect: every time there's a new intent for a `"standard"`
        activity, a new instance of the class is created to respond to that intent.
        Each instance handles a single intent.
        Similarly, a new instance of a `"singleTop"` activity can also be
        created to handle a new intent.

        However, if the target task already has an
        existing instance of the activity at the top of its stack, that instance
        receives the new intent, in an
        `https://developer.android.com/reference/android/app/Activity#onNewIntent(android.content.Intent)` call.
        A new instance isn't created.
        Otherwise---if an existing instance of the
        `"singleTop"` activity is in the target task but not at the top of
        the stack, or if it's at the top of a stack, but not in the target task---a new instance is
        created and pushed on the stack.

        Similarly, if the user
        [navigates
        up](https://developer.android.com/training/implementing-navigation/ancestral) to an activity on the current stack, the behavior is determined by the
        parent activity's launch mode. If the parent activity has launch mode `singleTop` (or the `up` intent contains `https://developer.android.com/reference/android/content/Intent#FLAG_ACTIVITY_CLEAR_TOP`), the parent is brought to the
        top of the stack, and its state is preserved.

        The navigation intent is received
        by the parent activity's `https://developer.android.com/reference/android/app/Activity#onNewIntent(android.content.Intent)`
        method. If the parent activity has launch mode `standard`, and the
        `up` intent doesn't contain `FLAG_ACTIVITY_CLEAR_TOP`, the current activity and its
        parent both pop off the stack, and a new instance of the parent activity is created to receive the navigation intent.


        The `"singleInstance"` mode also differs from `"singleTask"`
        and `"singleInstancePerTask"` in only one respect: an activity with the
        `"singleTask"` or `"singleInstancePerTask"` launch mode
        lets other activities, necessarily `"standard"` and
        `"singleTop"` activities, be part of its task.

        A
        `"singleInstance"` activity, on the other hand, permits no other
        ...
        if `FLAG_ACTIVITY_NEW_TASK` were in the intent.
        ...
        As shown in the preceding table, `"standard"` is the default mode and is
        appropriate for most types of activities. `"singleTop"` is also a
        common and useful launch mode for many types of activities. The other modes,
        `"singleTask"` , `"singleInstance"`, and `"singleInstancePerTask"`, are
        *not appropriate* for most applications.
        ...
        other activities and tasks using the Back button.

        For more information about launch modes and their interaction with `Intent`
        flags, see
        [Tasks and the back stack](https://developer.android.com/guide/components/tasks-and-back-stack).
    ...
    :   Determines how the system presents this activity when the device is running in
        ...
        Android can run tasks in an immersive, kiosk-like fashion called lock task mode. When the system
        runs in lock task mode, device users typically can't see notifications, access non-allowlisted apps,
        or return to the home screen, unless the Home app is allowlisted.

        Only apps that are
        allowlisted by a device policy controller (DPC) can run when the system is in lock task mode. System
        and [privileged apps](https://source.android.com/devices/tech/config/perms-allowlist),
        however, can run in lock task mode without being allowlisted.


        The value can be any one of the following
        [`R.attr.lockTaskMode`](https://developer.android.com/reference/android/R.attr#lockTaskMode) string values:
    ...
    `android:maxRecents`
    :   The maximum number of tasks rooted at this activity in the [Recents screen](https://developer.android.com/guide/components/recents). When this number of entries is reached, the system removes the least-recently
    ...
    :   The maximum aspect ratio the activity supports.
        ...
        If the app runs on a device with a wider aspect ratio, the system automatically letterboxes the app,
        leaving portions of the screen unused so the app can run at its specified maximum aspect ratio.

        Maximum aspect ratio is expressed as the decimal form of the quotient of the device's longer
        dimension divided by its shorter dimension. For example, if the maximum aspect ratio is 7:3,
        set the value of this attribute to 2.33.
        ...
        [R.attr.maxAspectRatio](https://developer.android.com/reference/android/R.attr#maxAspectRatio).
    ...
    :   The minimum aspect ratio the activity supports.
        ...
        If the app runs on a device with a narrower aspect ratio, the system automatically letterboxes the app, leaving
        portions of the screen unused so the app can run at its specified minimum aspect ratio.

        Minimum aspect ratio is expressed as the decimal form of the quotient of the device's longer dimension divided by
        ...
        For more information about this attribute, see
        [R.attr.minAspectRatio](https://developer.android.com/reference/android/R.attr#minAspectRatio).

    `android:multiprocess`
    :   Whether an instance of the activity can be launched into the process of the component
        that started it. It's `"true"` if it can be, and `"false"` if not.
        The default value is `"false"`.
        ...
        wherever they are used, provided permissions let it---something that is almost
        never necessary or desirable.

    `android:name`
    :   The name of the class that implements the activity, a subclass of
        `https://developer.android.com/reference/android/app/Activity`. The attribute value is
        ...
        [namespace](https://developer.android.com/studio/build/configure-app-module#set-namespace) specified in the
        `build.gradle` file.

        Once you publish your application, [don't
        change this name](http://android-developers.blogspot.com/2011/06/things-that-cannot-change.html), unless you set `https://developer.android.com/guide/topics/manifest/activity-element#exported="false"`. There is no default. The name must be specified.

    `android:noHistory`
    :   Whether the activity is removed from the activity stack and
        finished, by calling its `https://developer.android.com/reference/android/app/Activity#finish()`
        ...
        `https://developer.android.com/reference/android/app/Activity#onActivityResult(int, int, android.content.Intent)` is never called if you
    ...
    :   The class name of the logical parent of the activity. The name here must match the class
        name given to the corresponding `<activity>` element's
        [`android:name`](https://developer.android.com/guide/topics/manifest/activity-element#nm) attribute.

        The system reads this attribute to determine which activity to start when
        the user taps the Up button in the action bar. The system can also use this information to
        synthesize a back stack of activities with `https://developer.android.com/reference/android/app/TaskStackBuilder`.
        ...
        read [Providing Up
        Navigation](https://developer.android.com/training/implementing-navigation/ancestral).
    ...
    :   Defines how an instance of an activity is preserved within a containing
        ...
        `persistAcrossReboots` are preserved.
    ...
    :   The name of a permission that clients must have to launch the activity
        ...
        `https://developer.android.com/guide/topics/manifest/application-element`
        element's
        `https://developer.android.com/guide/topics/manifest/application-element#prmsn`
    ...
    :   Whether the activity relinquishes its task identifiers to an activity above it in the
        task stack. A task whose root activity has this attribute set to `"true"` replaces the base
        `Intent` with that of the next activity in the task.

        If the next activity also has this attribute set
        to `"true"` then it yields the base `Intent` to any activity that it launches in the same
        ...
        `https://developer.android.com/reference/android/app/ActivityManager.TaskDescription` to change labels, colors,
        and icons in the [Recents screen](https://developer.android.com/guide/components/recents).

    `android:requireContentUriPermissionFromCaller`

    :   Specifies permissions necessary to launch this activity when passing
        content URIs. The default value is `none`, meaning no specific
        ...
        the required permissions, the activity start will be denied via a `
        https://developer.android.com/reference/java/lang/SecurityException
        `.

        Note that the enforcement works for content URIs inside
        [`Intent.getData()`](https://developer.android.com/reference/android/content/Intent#getData()),
        [`Intent.EXTRA_STREAM`](https://developer.android.com/reference/android/content/Intent#EXTRA_STREAM),
        and
        [`Intent.getClipData()`](https://developer.android.com/reference/android/content/Intent#getClipData()).
    ...
    :
        ...
        You can set this attribute in either the `<activity>` or
        [`<application>`](https://developer.android.com/guide/topics/manifest/application-element)
        element.
        ...
        Setting this attribute to `"false"` doesn't guarantee that there are no
    ...
    :   The requested orientation of the activity.
        ...
        > - `userLandscape`
        >
        > Your app can opt out of the Android 16 behavior, but the opt out will be eliminated in a future release. See
        > [Device compatibility mode](https://developer.android.com/guide/practices/device-compatibility-mode#android_16).
        ...
        When an activity fills the entire screen, the requested orientation acts as a suggestion to
        ...
        On [Android 12 (API level
        31)](https://developer.android.com/about/versions/12/12L/summary#dev-device-orientation-request) and higher, device manufacturers can configure individual device
        ...
        physically rotate their device.

        In multi-window mode, the requested orientation does not act as a suggestion for the overall
        orientation. If the activity is
        [letterboxed](https://developer.android.com/guide/practices/device-compatibility-mode#letterboxing), the requested
        orientation impacts the letterboxing applied to the activity.
        ...
        **Note:** When you declare one of the landscape or portrait values,
        ...
        `"sensorLandscape"`, then your application is available only to devices that support
        landscape orientation.

        Also explicitly declare that
        your application requires either portrait or landscape orientation with the [`<uses-feature>`](https://developer.android.com/guide/topics/manifest/uses-feature-element)
        Element, such as `<uses-feature
        android:name="android.hardware.screen.portrait"/>`. This is a filtering behavior
        provided by Google Play and other services that support it, and the platform itself doesn't
        control whether your app can install when a device supports only certain orientations.
    ...
    :   Whether the activity is shown when the device's current user is
        ...
        to a literal value, like `"true"` or `"false"`, or
    ...
    :   Whether the activity can be terminated and successfully restarted
        without having saved its state. It's `"true"` if it can be restarted
        ...
        `https://developer.android.com/reference/android/os/Bundle` object, which is then passed to
        `https://developer.android.com/reference/android/app/Activity#onCreate(android.os.Bundle)` when the activity
        is restarted. If this attribute is set to `"true"`,
        `onSaveInstanceState()` might not be called, and `onCreate()` is passed
        `null` instead of the `Bundle`, as it is when the
        activity starts for the first time.


        A `"true"` setting means that the activity can be restarted without
    ...
    :   The task that the activity has an affinity for. Activities with
        ...
        to (see the `https://developer.android.com/guide/topics/manifest/activity-element#reparent`
        attribute) and the task that houses the activity when it launches
        with the `https://developer.android.com/reference/android/content/Intent#FLAG_ACTIVITY_NEW_TASK`
        flag.
        ...
        `https://developer.android.com/guide/topics/manifest/application-element`
        element's
        `https://developer.android.com/guide/topics/manifest/application-element#aff`
        attribute. The name of the default affinity for an application is
        the [namespace](https://developer.android.com/studio/build/configure-app-module#set-namespace) set in the
        `build.gradle` file.
    ...
    :   A reference to a style resource defining an overall theme for the activity.
        ...
        `https://developer.android.com/reference/android/content/Context#setTheme(int)` and might also
        ...
        information, see [Styles and themes](https://developer.android.com/guide/topics/ui/themes).
    ...
    :   How the main window of the activity interacts with the window containing
        ...
        `"state..."` values, has undefined results.
        Individual values are separated by a vertical bar (`|`), as shown in the following example:
...
:   API level 1 for all attributes except
    `https://developer.android.com/guide/topics/manifest/activity-element#nohist` and
    `https://developer.android.com/guide/topics/manifest/activity-element#wsoft`, which were added in API
    level 3.
