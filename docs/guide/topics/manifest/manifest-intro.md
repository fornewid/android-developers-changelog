---
title: App manifest overview  |  App architecture  |  Android Developers
...
# App manifest overview Stay organized with collections Save and categorize content based on your preferences.
...
* The components of the app, including all activities, services,
  ...
  [Read more about app components](#components) in a following section.
* The permissions that the app needs in order to
  ...
  [Read more about permissions](#perms) in a following section.
* The hardware and software features the app requires, which affects which
  devices can install the app from Google Play. [Read more about device compatibility](#compatibility) in a following section.

If you're using [Android Studio](/studio) to build your app, the manifest file
is created for you and most of the essential manifest elements are added as
you build your app, especially when using [code templates](/studio/projects/templates).
...
For each [app
component](/guide/components/fundamentals#Components) that you create in your app,
...
For more information about setting the package name or namespace, see [Set the namespace](/studio/build/configure-app-module#set-namespace).
...
For more information, see the [Intents and Intent Filters](/guide/components/intents-filters) document.
...
chooser dialog. For more information, see [Allow other apps to start your activity](/training/basics/intents/filters).
...
on Android](/guide/topics/permissions/overview).
...
The [`<uses-feature>`](/guide/topics/manifest/uses-feature-element) element lets you declare hardware and
...
Chromebooks](/topic/arc/manifest).
...
compatible, your manifest must include the [`<uses-sdk>`](/guide/topics/manifest/uses-sdk-element) tag
and its [`minSdkVersion`](/guide/topics/manifest/uses-sdk-element#min)
attribute.

However, be aware that attributes in the `<uses-sdk>` element
are overridden by corresponding properties
in the [`build.gradle`](/studio/build#build-files) file.
...
For more information about the `build.gradle` file, read about [how to configure your build](/studio/build).

To learn more about how to declare your app's support for different devices,
see the [Device compatibility
overview](/guide/practices/compatibility).
...
:   Technically, all attributes are optional. However, many attributes
    ...
    For truly optional attributes, the [reference documentation](#reference)
...
:   Some attributes have values that are displayed to users, such as
    ...
    resources](/guide/topics/resources/providing-resources) that you provide for different device configurations.
    ...
    merged into yours](/studio/write/add-resources#resource_merging)). The only other valid package name is
    `android`, when you want to use a resource from the Android
    framework.

    The type is a type of resource, such as [`string`](/guide/topics/resources/string-resource) or
    ...
    To instead apply a value that's defined in a [theme](/guide/topics/ui/look-and-feel/themes), the first character
    must be `?` instead of `@`:
...
| `<queries>` | Declares the set of other apps that your app intends to access. Learn more in the guide about [package visibility filtering](/training/package-visibility). |
