---
title: <application>  |  App architecture  |  Android Developers
...
# <application> Stay organized with collections Save and categorize content based on your preferences.
...
:   `android:allowTaskReparenting`
    ...
    :   Whether to let the application participate in the backup
        ...
        **Note:** For apps targeting Android 12 (API level 31) or higher, this behavior
        ...
        [backup and restore](/about/versions/12/behavior-changes-12#backup-restore) section
    ...
    :   Whether to let the application reset user data. This data includes
        ...
        **Note:** Only apps that are part of the system image can
        ...
        For more information, see [Data backup overview](/guide/topics/data/backup).
    ...
    :   The name of the class that implements the application's backup agent,
        a subclass of `BackupAgent`. The attribute value is
    ...
    :   Indicates that
        [Auto Backup](/guide/topics/data/autobackup) operations
    ...
    :   A [drawable resource](/guide/topics/resources/drawable-resource)
        ...
        For more information, see [Provide a home screen banner](/training/tv/start/start#banner).
    ...
    :   Applications can set this attribute to an XML resource where they specify the
        ...
        For information about the format of the XML file, see [Backup and restore](/about/versions/12/behavior-changes-12#backup-restore).
    ...
    Starting with AGP 4.2.0, the DSL option
    [`useLegacyPackaging`](/reference/tools/gradle-api/7.1/com/android/build/api/dsl/JniLibsPackagingOptions#uselegacypackaging)
    replaces the `extractNativeLibs` manifest attribute.
    Use `useLegacyPackaging` in your app's `build.gradle` file
    instead of `extractNativeLibs` in the manifest file to configure native library
    compression behavior. For more information, see the release note
    [Use the DSL to package compressed native libraries](/build/releases/past-releases/agp-4-2-0-release-notes#compress-native-libs-dsl).
    ...
    :   This attribute points to an XML file that contains full backup rules for
        [Auto Backup](/guide/topics/data/autobackup).
        These rules determine what files get backed up. For more information, see the
        [XML config syntax](/guide/topics/data/autobackup#XMLSyntax)
        for Auto Backup.
        ...
        [Files that are backed up](/guide/topics/data/autobackup#Files).

    `android:fullBackupOnly`
    :   This attribute indicates whether to use
        [Auto Backup](/guide/topics/data/autobackup) on devices
        ...
        [key/value backups](/guide/topics/data/keyvaluebackup).

        The default value is `"false"`.

    `android:gwpAsanMode`
    :   This attribute indicates whether to use
        [GWP-ASan](/ndk/guides/gwp-asan), a native memory allocator feature that helps
    ...
    :   Whether the application contains any DEX code—that is, code using the
        ...
        **This property must account for code included in the application by
        ...
        [Play feature delivery](/platform/technology/app-bundle) and include feature
    ...
    :   Whether hardware-accelerated rendering is enabled for all
        ...
        either [`minSdkVersion`](/guide/topics/manifest/uses-sdk-element#min)
        or [`targetSdkVersion`](/guide/topics/manifest/uses-sdk-element#target)
        ...
        [Hardware acceleration](/guide/topics/graphics/hardware-accel)
        guide.
    ...
    **Warning:** This attribute has been deprecated as of Android 8.0 (API level 26). Use the
    [appCategory](#appCategory) attribute set to `game` instead.
    ...
    :   Specifies the name of the XML file that contains your application's
        [Network security
        configuration](/training/articles/security-config). The value is a reference to the XML resource file
    ...
    :   Indicates that the application is prepared to attempt a restore of any
        ...
        this attribute to `"true"` lets the Backup Manager- `$1`
        attempt a restore even when a version mismatch suggests that the data is
    ...
    :   Whether the application wants to opt out of
        [scoped storage](/training/data-storage/files/external-scoped).
    ...
    :   Specifies the account type required by the application to function.
        ...
        [`android:restrictedAccountType`](#restrictedAccountType) with
        the same value.

        **Caution:**
        If the account data might reveal personally identifiable information, it's important
        that you declare this attribute and leave [`android:restrictedAccountType`](#restrictedAccountType) null, so that restricted profiles cannot use
    ...
    :   Specifies whether the app supports [multi-window mode](/guide/topics/ui/multi-window).
        ...
        **Warning:** To improve the layout of apps on form factors with smallest width >=
        ...
        You can set this attribute in either the [`<activity>`](/guide/topics/manifest/activity-element)
        ...
        * **Large screens (sw >= 600dp):** all apps support multi-window mode. The attribute indicates whether
          an app can be resized, not whether the app supports multi-window mode. If `resizeableActivity="false"`,
          the app is put into compatibility mode when necessary to conform to display dimensions.
        * **Small screens (sw < 600dp):** if `resizeableActivity="true"` and the minimum width
          and minimum height of the main activity are within the multi-window requirements, the app supports multi-window
          mode. If `resizeableActivity="false"`, the app doesn't support multi-window mode regardless of the
    ...
    :   Specifies the account type required by this application and indicates that restricted profiles
        ...
        `Account` and restricted profiles *can*
        ...
        **Caution:**
        ...
        use this attribute. Instead, declare the [`android:requiredAccountType`](#requiredAccountType) attribute
    ...
    :   Indicates whether this application is only for testing purposes. For example,
        ...
        through `adb`. You can't publish it to Google Play.
    ...
    :   A reference to a style resource defining a default theme for all
        ...
        attributes. For more information, see [Styles and themes](/guide/topics/ui/themes).
    ...
    :   Indicates whether the app intends to use cleartext network traffic, such as cleartext HTTP.
        ...
        no expectation that the `Socket` API honors
        ...
        **Note:**
        `WebView` honors this attribute for
