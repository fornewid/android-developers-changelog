---
title: Advanced RenderScript  |  Views  |  Android Developers
...
# Advanced RenderScript Stay organized with collections Save and categorize content based on your preferences.
...
* Every `.rs` file that you create is generated into a class named
  ...
  + Non-static, global RenderScript variables. Accessor
    ...
    generated. Look [here](/guide/topics/renderscript/compute#script-globals)
    for more details.
...
[`rsSendToClient()`](/reference/renderscript/rs__core_8rsh)
function.
...
specified number of `struct`s. For example, if you define the following struct:
...
by the RenderScript runtime and to interact with `struct`s
...
* A static nested class, `Item`, allows you to create an instance of the
  ...
  the array. The RenderScript runtime automatically has access to the newly written memory.* Accessor methods to get and set the values of each field in a struct. Each of these
...
allocated memory is automatically created for your RenderScript code at compile time. See [Figure 1](#figure1)
...
an array of `Element`s. The `Allocation` class does the actual
...
[Allocating and binding memory to the RenderScript](#allocating-mem) section.
The following table describes the three memory management classes in more detail:

| Android Object Type | Description |
| --- | --- |
| `Element` | An element describes one cell of a memory allocation and can have two forms: basic or complex.  A basic element contains a single component of data of any valid RenderScript data type. Examples of basic element data types include a single `float` value, a `float4` vector, or a single RGB-565 color.  Complex elements contain a list of basic elements and are created from `struct`s that you declare in your RenderScript code. For instance an allocation can contain multiple `struct`s arranged in order in memory. Each struct is considered as its own element, rather than each data type within that struct. |
...
not generated. Look [here](/guide/topics/renderscript/compute#script-globals) for more details.
...
[`rsSendToClient()`](/reference/renderscript/rs__core_8rsh) function
...
(look [here](/guide/topics/renderscript/compute#script-globals)
for more details).
...
propagated back to the RenderScript runtime level [asynchronously](/guide/topics/renderscript/compute#asynchronous-model):
