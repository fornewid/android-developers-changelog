---
title: RenderScript Overview  |  Views  |  Android Developers
...
# RenderScript Overview Stay organized with collections Save and categorize content based on your preferences.




**Caution:** Starting with Android 12, the RenderScript APIs are deprecated. They will continue
to function, but we expect that device and component manufacturers will stop providing
hardware acceleration support over time. To take full advantage of GPU acceleration, we
recommend [migrating away from RenderScript](/guide/topics/renderscript/migrate).
  
  
Following the deprecation of RenderScript in the Android platform, we are also removing
support for RenderScript in the Android Gradle plugin. Starting with Android Gradle plugin
7.2, the RenderScript APIs are deprecated. They will continue to function, but will invoke
warnings, and will be completely removed in future versions of AGP. For more information, see
[Migrate from Renderscript](/guide/topics/renderscript/migrate).
...
* The *language* itself is a C99-derived language for writing high-performance compute
  code. [Writing a RenderScript Kernel](#writing-an-rs-kernel) describes
...
* The *control API* is used for managing the lifetime of RenderScript resources and
  ...
  [Using RenderScript from Java Code](#using-rs-from-java) and
  [Single-Source RenderScript](#single-source-rs) describe the first and the third
...
* Zero or more ***script globals***. A script global is similar to a global variable in C. You can
  ...
  kernels. Script globals are explained in more detail [here](#script-globals).
* Zero or more ***compute kernels***. A compute kernel is a function
  ...
  + Here is an example of a simple **mapping kernel**:
    ...
    function. The [`RS_KERNEL`](#RS_KERNEL) property applied to the
    ...
    discussed [below](#special-arguments). The value returned from the kernel is
  ...
  its [accumulator function](#accumulator-function) executes once for every
  ...
  + Here is an example of a simple **reduction
    ...
    Reduction kernels are explained in more detail [here](#reduction-in-depth).
  ...
  `context` of type [rs\_kernel\_context](/guide/topics/renderscript/reference/rs_for_each#android_rs:rs_kernel_context).
  It is needed by a family of runtime APIs that are used to query
  certain properties of the current execution -- for example, [rsGetDimX](/guide/topics/renderscript/reference/rs_for_each#android_rs:rsGetDimX).
...
* **`android.support.v8.renderscript`** - The APIs in this package are
  ...
  Library](/tools/support-library/features#v8), which allows you to use them on devices running Android 2.3 (API level 9) and
  higher.
...
[Android SDK Manager](/tools/help/sdk-manager).
...
2. Update the settings for the Android build process to include the RenderScript settings:
   ...
   * Add the following RenderScript settings to the file:
     ...
     + `renderscriptTargetApi` - Specifies the bytecode version to be
       generated. We recommend you set this value to the lowest API level able to provide
       all the functionality you are using and set `renderscriptSupportModeEnabled`
       to `true`. Valid values for this setting are any integer value
       from 11 to the most recently released API level. If your minimum SDK version
       specified in your application manifest is set to a different value, that value
       is ignored and the target value in the build file is used to set the minimum
       SDK version.
     + `renderscriptSupportModeEnabled` - Specifies that the generated
       bytecode should fall back to a compatible version if the device it is running
       on does not support the target version.
3. In your application classes that use RenderScript, add an import for the Support Library
   classes:
...
1. **Initialize a RenderScript context.** The `RenderScript` context, created with `create(Context)`, ensures that RenderScript can be used and provides an
...
2. **Create at least one `Allocation` to be passed to a
...
3. **Create whatever scripts are necessary.** There are two types of scripts available
   to you when using RenderScript:
   * **ScriptC**: These are the user-defined scripts as described in [*Writing a RenderScript Kernel*](#writing-an-rs-kernel) above. Every script has a Java class
     ...
     * **ScriptIntrinsic**: These are built-in RenderScript kernels for common operations,
       ...
       `ScriptIntrinsic`.
4. **Populate Allocations with data.** Except for Allocations created with `createFromBitmap()`, an Allocation is populated with empty data when it is
   first created. To populate an Allocation, use one of the "copy" methods in `Allocation`. The "copy" methods are [synchronous](#asynchronous-model).
5. **Set any necessary [script globals](#script-globals).** You may set globals using methods in the
   ...
   are [asynchronous](#asynchronous-model).
6. **Launch the appropriate kernels and invokable functions.**
   ...
   These launches are [asynchronous](#asynchronous-model).
...
7. **Retrieve data from `Allocation` objects
   and *[javaFutureType](#javaFutureType)* objects.**
   In order to
   ...
   The "copy" and `get()` methods are [synchronous](#asynchronous-model).
8. **Tear down the RenderScript context.** You can destroy the RenderScript context
...
The reflected *[javaFutureType](#javaFutureType)* classes provide
...
In Single-Source RenderScript, you write kernels as described in [Writing a RenderScript Kernel](#writing-an-rs-kernel). You then write an invokable function that calls
[`rsForEach()`](/guide/topics/renderscript/reference/rs_for_each#android_rs:rsForEach) to launch them. That API takes a kernel function as the first
parameter, followed by input and output allocations. A similar API
[`rsForEachWithOptions()`](/guide/topics/renderscript/reference/rs_for_each#android_rs:rsForEachWithOptions) takes an extra argument of type
[`rs_script_call_t`](/guide/topics/renderscript/reference/rs_for_each#android_rs:rs_script_call_t), which specifies a subset of the elements from the input and
...
Follow the steps in [Using RenderScript from Java Code](#using-rs-from-java).
In the step [launch the appropriate kernels](#launching_kernels), call
...
[rsCreateAllocation()](/guide/topics/renderscript/reference/rs_allocation_create#android_rs:rsCreateAllocation). One easy-to-use form of that API is `rsCreateAllocation_<T><W>(…)`, where *T* is the data type for an
...
[`rsClearObject(rs_allocation* alloc)`](/guide/topics/renderscript/reference/rs_object_info#android_rs:rsClearObject) to indicate you no longer need the handle
`alloc` to the underlying allocation,
so that the system can free up resources as early as possible.

The [Writing a RenderScript Kernel](#writing-an-rs-kernel) section contains an example
...
* The `set_var` method (if it exists) writes the
  ...
  [asynchronously](#asynchronous-model).
...
An example above shows a simple [addint](#example-addint) reduction kernel.
...
**Example:** In the [addint](#example-addint)
...
**Example:** In
the [findMinAndMax](#example-findMinAndMax) kernel, the accumulator data items
...
**Example:** In
the [findMinAndMax](#example-findMinAndMax) kernel, the accumulator function
...
data items](#accumulator-data-items) together into a single accumulator data item. You may write a *combiner
function* to do this. If the accumulator function has a single input and
no [special arguments](#special-arguments), then you do not need to write a combiner
...
**Example:** In
the [findMinAndMax](#example-findMinAndMax) kernel, the combiner function
...
**Example:** In the [addint](#example-addint) kernel,
...
**Example:** In
the [findMinAndMax](#example-findMinAndMax) kernel, the outconverter function
...
* `initializer(initializerName)` (optional): Specifies the name of the
  ...
  this function once for each [accumulator data item](#accumulator-data-items). The
...
* `accumulator(accumulatorName)`
  ...
  per input. The accumulator function may optionally take any of the [special arguments](#special-arguments).

  An example kernel with multiple inputs is [`dotProduct`](#dot-product).
* `combiner(combinerName)`
  ...
  **NOTE:** It is possible
  ...
  the [findMinAndMax](#example-findMinAndMax) kernel, the combiner
  function `fMMCombiner` explicitly checks for `idx < 0` because that
  indicates such an accumulator data item, whose value is [INITVAL](#INITVAL).
  ...
  or more [special arguments](#special-arguments).
* `outconverter(outconverterName)`
  ...
  computed by the [combiner function](#combiner-function).
...
the [findMinAndMax](#example-findMinAndMax) kernel, the input
...
One consequence of this is that the [findMinAndMax](#example-findMinAndMax)
...
different ways](#assume), you must follow certain rules to ensure that your kernel behaves the
...
a mathematical reduction such as [addint](#example-addint), it usually makes sense
for "the same" to mean mathematical equality. For a "pick any" search such
as [findMinAndMax](#example-findMinAndMax) ("find the location of minimum and
...
different than those for [findMinAndMax](#example-findMinAndMax).
...
* `combinerName(&A, &I)` must
  leave `A` [the same](#the-same)
* `combinerName(&I, &A)` must
  leave `I` [the same](#the-same) as `A`
...
**Example:** In the [findMinAndMax](#example-findMinAndMax)
...
set `A` to [the same value](#the-same)
...
**Example:** In the [findMinAndMax](#example-findMinAndMax) kernel,
...
**Example:** In the [addint](#example-addint) kernel, the
...
**Example:** In the [addint](#example-addint) kernel, for an input value *V*:
...
**Example:** In the [findMinAndMax](#example-findMinAndMax) kernel, for an input
...
Here are some examples of calling the [addint](#example-addint) kernel:
...
function](#accumulator-function). The RenderScript runtime checks to ensure that all of the input Allocations
...
of type *javaResultType*. `get()` is [synchronous](#asynchronous-model).
...
[outconverter function](#outconverter-function). Unless *resultType* is an
...
function](#outconverter-function).
...
the [accumulator function](#accumulator-function). Unless *inXType* is an
...
Note that for [Method 3](#reduce-method-3), input types are handled differently
than result types:
