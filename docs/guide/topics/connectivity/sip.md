---
title: Session initiation protocol overview  |  Connectivity  |  Android Developers
...
# Session initiation protocol overview Stay organized with collections Save and categorize content based on your preferences.
...
* SIP runs over a wireless data connection, so your device must have a data
  ...
  [Testing SIP applications](#testing).
...
[API Levels](/guide/topics/manifest/uses-sdk-element#ApiLevels)
...
an intent filter to receive calls](#intent_filter)). This is the registration step:
...
* A `SipProfile` that is making the call (the
  ...
  "peer profile").* A `SipManager` object.
...
code from the [SipDemo sample](/resources/samples/SipDemo).
...
2. Set up your mobile device for testing, as described in [Developing on a Device](/tools/device).
3. Run your application on your mobile device, as described in [Developing on a Device](/tools/device).
4. If you are using Android Studio, you can view the application log output by
   opening the Event Log console (**View > Tool Windows > Event Log**).- Ensure your application is configured to launch Logcat automatically when it runs:
     1. Select **Run > Edit Configurations**.- Select the **Miscellaneous** tab in the **Run/Debug Configurations** window.- Under **Logcat**, select **Show logcat automatically** then
            select **OK**.