---
title: Define a custom app permission  |  Privacy  |  Android Developers
...
# Define a custom app permission Stay organized with collections Save and categorize content based on your preferences.
...
with other apps. For more information about permissions, see the [permissions overview](/guide/topics/permissions/requesting).
...
All APKs must be [signed with a certificate](/studio/publish/app-signing#certificates-keystores)
...
permissions](/guide/topics/manifest/permission-element#plevel) and grant or deny an app's [request to be given
the same Linux identity](/guide/topics/manifest/manifest-element#uid) as another app.
...
in your app's [`protectionLevel`](/reference/android/R.attr#protectionLevel)
attribute
...
**Warning:** The `sharedUserId` attribute in the `AndroidManifest.xml`
file's [`<manifest>`](/guide/topics/manifest/manifest-element)
...
remove this value, because migrating off a shared user ID is not supported.

## Define and enforce permissions

To enforce your own permissions, you must first declare them in your
`AndroidManifest.xml` using one or more [`<permission>`](/guide/topics/manifest/permission-element) elements.
...
The `protectionLevel` attribute is required and tells the system how to
...
The [`android:permissionGroup`](/guide/topics/manifest/permission-group-element)
...
([`android:label`](/guide/topics/manifest/permission-element#label))
or details on a single permission
([`android:description`](/guide/topics/manifest/permission-element#desc)).
...
[`android:permissionGroup`](/guide/topics/manifest/permission-group-element) attribute to help the system describe
...
from other apps by defining [`<uses-permission>`](/guide/topics/manifest/uses-permission-element) elements.
...
[signature permissions](/guide/topics/permissions/overview#signature).
...
Restrict interactions with other apps](/training/permissions/restrict-interactions)