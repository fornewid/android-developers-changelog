---
title: Permissions used only in default handlers  |  Privacy  |  Android Developers
...
# Permissions used only in default handlers Stay organized with collections Save and categorize content based on your preferences.
...
page regardless of where you publish your Android app.
...
![Screen capture of default apps settings](/static/images/guide/topics/permissions/list-default-handlers.svg)


**Figure 1.** System settings screen showing list of default handlers
on a device.

![Screen capture of default SMS app settings](/static/images/guide/topics/permissions/edit-default-handlers.svg)


**Figure 2.** System settings screen showing how to change the default
SMS handler.
...
* Your app must be able to perform the functionality for which it's a default
  handler. For example, a default SMS handler must be able to send text
  messages.
...
* Your app must declare permissions that are appropriate for its use case. For
  more details about which permissions you can declare as a given handler, see the
  [guidance on using SMS or call log permission
  groups](https://support.google.com/googleplay/android-developer/answer/9047303#intended)
...
* Your app must ask to become a default handler **before** it requests the
  ...
  to become the default SMS handler before it requests the `READ_SMS` permission.
...
![Screen capture showing a user-facing dialog](/static/images/guide/topics/permissions/change-default-handler-prompt.svg)


**Figure 3.** Prompt asking the user whether they want to change their
device's default SMS handler.

**Note:** Your app must ask to become a default handler **before** it requests the
permissions associated with being that handler. For example, an app must request
to become the default SMS handler before it requests the `READ_SMS` permission.
...
arrow\_forward](/training/permissions/restrict-interactions)