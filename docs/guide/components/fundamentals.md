...
* An app can request permission to access device data such as the device's
  ...
  to explicitly grant these permissions. For more information about permissions, see
  [Permissions on Android](/training/permissions).
...
* The manifest file in which you declare the components and the required device
  features for your
  app.
* Resources that are separate from the app code and that let your app
...
:   An *activity* is the entry point for interacting with the user. It represents a single
    ...
    * Keeping track of what the user currently cares about—what is on-screen—so that the
      system keeps running the process that is hosting the activity.
    * Knowing which previously used processes contain stopped activities the user might return to
      and prioritizing those processes more highly to keep them available.
    ...
    * Providing a way for apps to implement user flows between each other, and for the system to
      coordinate these flows. The primary example of this is sharing.
...
:   A *service* is a general-purpose entry point for keeping an app running in the background
    ...
    * Music playback is something the user is directly aware of, and the app communicates this to
      the system by indicating that it wants to be in the foreground, with a notification to tell
      the user that it is running. In this case, the system prioritizes keeping that service's
      process running, because the user has a bad experience if it goes away.
    ...
    **Bound services** run because some other app (or the system) has said that it wants to make use of the
    ...
    about the `Service` class, see the [Services overview](/guide/components/services).
    ...
    **Note:** If your app targets Android 5.0 (API level 21) or higher,
    ...
    and by working with the [Doze](/training/monitoring-device-state/doze-standby) API.
...
:   A *broadcast receiver* is a component that lets the system deliver events to the
    ...
    Although broadcast
    receivers don't display a user interface, they can [create a status bar notification](/develop/ui/views/notifications)
    ...
    For instance, a broadcast receiver might schedule a `JobService` to perform some work based
    on an event using `JobScheduler`.
...
:   A *content provider* manages a shared set of app data that you can store in
    ...
    * Assigning a URI doesn't require that the app remain running, so URIs can persist after their
      owning apps exit. The system only needs to make sure that an owning app is
      still running when it retrieves the app's data from the corresponding URI.
    * These URIs also provide an important fine-grained security model. For example, an
      ...
      to access that URI on the clipboard, the system can let that app
      access the data using a temporary *URI permission grant*
      so that it accesses the data only behind that URI, and nothing else in the second app.
    ...
    transactions. For more information, see the [Content providers](/guide/topics/providers/content-providers) developer
    guide.
...
* On Android 5.0 (API level 21) and higher, you can use
...
Intent Filters](/guide/components/intents-filters) document.
...
* Declares API libraries the app needs to be linked against (other than the Android framework
  APIs), such as the [Google Maps library](http://code.google.com/android/add-ons/google-apis/maps-overview.html).
...
As discussed in the [Activate components](#ActivatingComponents) section, you can use an
...
`Service`, make sure that your app is secure by using an
[explicit](/guide/components/intents-filters#Types)
...
adding an [`<intent-filter>`](/guide/topics/manifest/intent-filter-element) element as a child of the component's declaration element.
...
[Specify API level requirements](/studio/publish/versioning#minsdk).
...
*require* it. To do so, you set the [`required`](/guide/topics/manifest/uses-feature-element#required)
...
is provided in the [Device compatibility overview](/guide/practices/compatibility).
...
create alternative resources for different device configurations, read the [App resources overview](/guide/topics/resources/providing-resources). To
learn more about best practices and designing robust, production-quality apps,
see the [Guide to app architecture](/topic/libraries/architecture/guide).
...
:   Learn how to use the `Intent` APIs to
...
[Introduction to activities](/guide/components/activities/intro-activities)
:   Learn how to create an instance of the `Activity` class,
