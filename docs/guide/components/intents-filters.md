---
title: Intents and intent filters  |  App architecture  |  Android Developers
...
* **Starting a service**
  ...
  from another component by passing an `Intent` to `bindService()`. For more information, see the [Services](/guide/components/services) guide.
...
:   Flags are defined in the `Intent` class that function as metadata for the
    ...
    [task](/guide/components/tasks-and-back-stack)
...
[Services](/guide/components/services) guide.
...
another app](/training/basics/intents/sending).
...
intent launch, and a [StrictMode](/reference/android/os/StrictMode) violation
occurs:

1. Your app unparcels a nested intent from the extras of a delivered intent.
2. Your app immediately starts an [app
   component](/guide/components/fundamentals#Components) using that nested intent,
...
[`detectUnsafeIntentLaunch()`](/reference/android/os/StrictMode.VmPolicy.Builder#detectUnsafeIntentLaunch())
...
[`detectAll()`](/reference/android/os/StrictMode.VmPolicy.Builder#detectAll())
...
app calls
[`putExtras(Intent)`](/reference/android/content/Intent#putExtras(android.content.Intent))
or
[`putExtras(Bundle)`](/reference/android/content/Intent#putExtras(android.os.Bundle)).
...
[exported](/guide/topics/manifest/activity-element#exported), sanitize and
...
**Use a [`PendingIntent`](/reference/android/app/PendingIntent) instead of a
...
[`<action>`](/guide/topics/manifest/action-element),
[`<data>`](/guide/topics/manifest/data-element), or
[`<category>`](/guide/topics/manifest/category-element).
...
about [Intent Resolution](#Resolution).
...
[`exported`](/guide/topics/manifest/activity-element#exported) attribute
...
* The `CATEGORY_LAUNCHER` category indicates that this activity's
  icon should be placed in the system's app launcher. If the [`<activity>`](/guide/topics/manifest/activity-element) element
  does not specify an icon with `icon`, then the system uses the icon from the [`<application>`](/guide/topics/manifest/application-element)
  element.
...
Learn more about [intent matching](#imatch).
...
* Declaring an intent to be executed when the user performs an action with your [Notification](/develop/ui/views/notifications)
  (the Android system's `NotificationManager`
  executes the `Intent`).* Declaring an intent to be executed when the user performs an action with your
    [App Widget](/guide/topics/appwidgets)
    (the Home screen app executes the `Intent`).* Declaring an intent to be executed at a specified future time (the Android
...
of the respective use cases, such as in the [Notifications](/develop/ui/views/notifications)
and [App Widgets](/guide/topics/appwidgets) API guides.
...
[`PendingIntent.FLAG_MUTABLE`](/reference/android/app/PendingIntent#FLAG_MUTABLE)
or
[`PendingIntent.FLAG_IMMUTABLE`](/reference/android/app/PendingIntent#FLAG_IMMUTABLE)
flag, respectively.

If your app attempts to create a `PendingIntent` object
without setting either mutability flag, the system throws an
[`IllegalArgumentException`](/reference/java/lang/IllegalArgumentException), and
the following message appears in [Logcat](/studio/command-line/logcat):
...
* Supporting [direct reply actions in
  notifications](/training/notify-user/build-notification#reply-action). The
  ...
  [`fillIn()`](/reference/android/content/Intent#fillIn(android.content.Intent,%20int))
  method.
...
* Placing conversations in [bubbles](/guide/topics/ui/bubbles) using instances
...
* Requesting device location information by calling
  [`requestLocationUpdates()`](/reference/android/location/LocationManager#requestLocationUpdates(java.lang.String,%20long,%20float,%20android.app.PendingIntent))
...
* Scheduling alarms using [`AlarmManager`](/reference/android/app/AlarmManager).
  The mutable `PendingIntent` object allows the system to add the
  [`EXTRA_ALARM_COUNT`](/reference/android/content/Intent#EXTRA_ALARM_COUNT)
...
that you use an [explicit intent](#ExampleExplicit) and fill in the
[`ComponentName`](/reference/android/content/ComponentName). That way, whenever
...
wrap a pending intent around an [explicit intent](#ExampleExplicit).
...
1. Check that the action, package, and component fields of the base intent
   are set.
2. Use [`FLAG_IMMUTABLE`](/reference/android/app/PendingIntent#FLAG_IMMUTABLE),
...
[`<action>`](/guide/topics/manifest/action-element) elements, as shown in the following example:
...
[`<category>`](/guide/topics/manifest/category-element) elements, as shown in the following example:
...
[`<data>`](/guide/topics/manifest/data-element) elements, as shown in the following example:
...
Each of these attributes is optional in a [`<data>`](/guide/topics/manifest/data-element) element,
...
The following example shows a typical case in which a [`<data>`](/guide/topics/manifest/data-element) element
...
example, a [`<data>`](/guide/topics/manifest/data-element)
