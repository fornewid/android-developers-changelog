...
`startActivity(android.content.Intent)` in starting a new Activity,
the app can pass in
...
`writeToParcel(android.os.Parcel, int)` method.
It must also provide a non-null field called `CREATOR` that
implements the `Parcelable.Creator` interface, whose
`createFromParcel()`
method is used for converting the `Parcel` back to the current object.
For more information,
see the reference documentation for the `Parcelable` object.
