---
title: Parcelables and bundles  |  App architecture  |  Android Developers
...
# Parcelables and bundles Stay organized with collections Save and categorize content based on your preferences.
...
`Parcelable` and `Bundle` objects.

**Note:** `Parcel` is not a general-purpose
serialization mechanism, and you should never
store any `Parcel` data on disk or send it over the network.

## Sending data between activities

When an app creates an `Intent` object to use in
`startActivity()` in starting a new Activity,
the app can pass in
parameters using the `putExtra()`
method.
...
The OS parcels the underlying `Bundle` of the intent. Then, the OS creates
the new activity,
un-parcels the data, and passes the intent to the new activity.

We recommend that you use the `Bundle` class to set primitives known to the OS on
`Intent` objects. The `Bundle` class is highly
optimized for marshalling and unmarshalling using parcels.

In some cases, you may need to pass complex objects across activities or preserve them in UI state.
In such cases, the custom class should implement Parcelable. For modern Kotlin and Jetpack Compose
apps, the recommended approach is to use the `@Parcelize`
annotation. This automatically generates the serialization logic needed to safely handle your data
when using Bundles. This is the same approach used to handle your data when using
`rememberSaveable`.
For more information about using `@Parcelize`,
see [Parcelable implementation generator](/kotlin/parcelize).
...
`TransactionTooLargeException` exception.
...
`Parcelable` object from one app to another, you need to be certain that the
...
the app such as `startActivity`, `rememberSaveable` (which uses
`onSaveInstanceState` under the hood), and any interaction with the system. When the size
limit is exceeded, a `TransactionTooLargeException` is thrown.

For the specific case of saving state with `rememberSaveable`, the amount of data should be kept small
...
We recommend that you keep saved state to less than 50KB of data.
