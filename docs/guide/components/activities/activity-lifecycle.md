...
* Consuming valuable system resources when the user is not actively
  using it.
* Losing the user's progress if they leave your app and return to
  it at a later time.
...
best practices, see [Handling Lifecycles with Lifecycle-Aware Components](/topic/libraries/architecture/lifecycle) and [Save UI states](/topic/libraries/architecture/saving-states).
...
[Guide to app architecture](/topic/libraries/architecture/guide).
...
[restricted from
starting activities when running in the background](/guide/components/activities/background-starts).
...
vulnerability to ejection, see the section about [activity state and ejection from memory](#asem).
...
your dependent components lifecycle-aware, see [Handling Lifecycles with Lifecycle-Aware Components](/topic/libraries/architecture/lifecycle).
...
[`ON_CREATE`](/reference/androidx/lifecycle/Lifecycle.Event#ON_CREATE)
...
[user interface](/guide/topics/ui) documentation.
...
[`ON_START`](/reference/androidx/lifecycle/Lifecycle.Event#ON_START) event.
...
to the activity's lifecycle receives the
[`ON_RESUME`](/reference/androidx/lifecycle/Lifecycle.Event#ON_RESUME)
...
mode, see [Multi-window support](/guide/topics/ui/multi-window).
...
* An event that interrupts app execution, as described in the section about
  the [onResume()](#onresume) callback, pauses the current activity. This is the most common
  case.
* In multi-window mode, only one app has focus
  at any time, and the system pauses all the other apps.
* The opening of a new, semi-transparent activity, such as a dialog,
  pauses the activity it covers. As long as
  the activity is partially visible but not in focus, it
  remains paused.

When an activity moves to the Paused state, any lifecycle-aware component tied
to the activity's lifecycle receives the
[`ON_PAUSE`](/reference/androidx/lifecycle/Lifecycle.Event#ON_PAUSE) event. This is where
...
Use the [`onPause()`](/reference/android/app/Activity#onPause()) method to pause or
...
The following example of a [`LifecycleObserver`](/reference/androidx/lifecycle/LifecycleObserver)
...
data, see the section about [saving and restoring state](#saras).
...
to the activity's lifecycle receives the
[`ON_STOP`](/reference/androidx/lifecycle/Lifecycle.Event#ON_STOP) event. This is where
the lifecycle components can stop any functionality that does not need to run
...
see the
[Room Persistence Library](/topic/libraries/architecture/room)
guide.
...
section about [saving and restoring state](#saras).
...
[`onDestroy()`](/reference/android/app/Activity#onDestroy()) is called before the
...
1. The activity is finishing, due to the user completely dismissing the
   activity or due to
   [`finish()`](/reference/android/app/Activity#finish()) being
...
to the activity's lifecycle receives the
[`ON_DESTROY`](/reference/androidx/lifecycle/Lifecycle.Event#ON_DESTROY) event. This is where
...
use a [`ViewModel`](/reference/androidx/lifecycle/ViewModel) object to contain the
...
[`onCleared()`](/reference/androidx/lifecycle/ViewModel#onCleared()) method called, where
it can clean up any data it needs to before being destroyed. You can distinguish between these two scenarios with the
[`isFinishing()`](/reference/android/app/Activity#isFinishing()) method.
...
callbacks, such as [`onStop()`](/reference/android/app/Activity#onStop()).
...
occurs, see the section about [saving and restoring state](#saras).
...
overview](/guide/components/processes-and-threads).
...
outlined in [Save UI states](/topic/libraries/architecture/saving-states).
...
[Handle configuration changes](/guide/topics/resources/runtime-changes) page.
...
that [`Activity`](/reference/android/app/Activity) instance is gone forever. In these
...
key-value pairs stored in a [`Bundle`](/reference/android/os/Bundle) object. By default, the
system uses the `Bundle` instance state to save information
about each [`View`](/reference/android/view/View) object in your activity layout, such as
the text value entered into an
[`EditText`](/reference/android/widget/EditText) widget.
...
`ViewModel` class, as outlined in [Save UI states](/topic/libraries/architecture/saving-states).
...
text in an [`EditText`](/reference/android/widget/EditText) widget or the scroll position of a
[`ListView`](/reference/android/widget/ListView) widget.
...
[`onStop()`](/reference/android/app/Activity#onStop()) method.
...
can recover your saved instance state from the [`Bundle`](/reference/android/os/Bundle) that the
system passes to your activity. Both the
[`onCreate()`](/reference/android/app/Activity#onCreate(android.os.Bundle)) and
...
[`onStart()`](/reference/android/app/Activity#onStart()) method. The system calls
...
2. Activity B's `onCreate()`,
   `onStart()`, and
   `onResume()` methods execute in sequence.
