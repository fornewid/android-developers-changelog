---
title: Processes and threads overview  |  App quality  |  Android Developers
...
The [`<application>`](/guide/topics/manifest/application-element)
...
[Processes and app lifecycle](/guide/topics/processes/process-lifecycle).
...
from the Android UI toolkit's `android.widget` and
`android.view` packages.
...
responding](https://developer.android.com/guide/practices/responsiveness.html)" (ANR) dialog. The user might then decide to quit your application or even uninstall
it.
...
1. Don't block the UI thread.- Don't access the Android UI toolkit from outside the UI thread.
...
while the `ImageView` is always manipulated from the UI thread.
...
using a `Handler` in your worker thread
...
[Background Work Overview](/training/multiple-threads).
...
This is primarily true for methods that can be called remotely, such as methods in a [bound service](/guide/components/bound-services). When a call on a
method implemented in an `IBinder` originates in the same process in which the
...
`onBind()` method is called from the UI thread of the
...
The `ContentResolver` and `ContentProvider`
...
To perform IPC, your application must bind to a service using `bindService()`. For more information, see the [Services overview](/guide/components/services).