---
title: https://developer.android.com/guide/playpoints/deliver-items-one-time
url: https://developer.android.com/guide/playpoints/deliver-items-one-time
source: md.txt
---

# Detect and deliver one-time items

This document describes how to detect and deliver one-time products in your game after players purchase them with Play Points in the Google Play app.

Once users redeem their Play Points for a one-time product in the Google Play app, the items should be delivered immediately in your game. The following displays how a user purchases items with Play Points.
...
| ![A screenshot](https://developer.android.com/static/images/guide/playpoints/deliver-step1.png) | ![A screenshot](https://developer.android.com/static/images/guide/playpoints/deliver-step2.png) | ![A screenshot](https://developer.android.com/static/images/guide/playpoints/deliver-step3.png) | ![A screenshot](https://developer.android.com/static/images/guide/playpoints/deliver-step4.png) |
...
Before you detect and deliver a one-time product, you must[create the product and Play Points promotions](https://developer.android.com/guide/playpoints/create-products).

## Delivery requirements

When you deliver one-time products in your game using a Play Points promotion, you must follow the requirements in this section.

### Delivery timing

When a player exchanges Play Points for a one-time product, you must deliver the item immediately without requiring the player to restart the game. In cases where game content must be completed beforehand, such as completing a tutorial, the item should be delivered immediately after.

### Delivery message

Once a user returns to your game after purchasing a one-time product outside of the game, you must display a confirmation that the product was successfully granted within the game. The message should come in the form of a dialog or an in-game message. Users shouldn't have to take any additional steps to receive the item.
...
If your game uses the[Google Play Billing Library](https://developer.android.com/google/play/billing/billing_overview), make the following changes in order to detect one-time products that are received outside of the game.

1. In your game's`[onResume()][onResume]`callback, call the`[queryPurchases()][queryPurchases]`method to retrieve a list of items, so you can determine which ones are not acknowledged.

2. If your game has a server, we strongly recommend that you[verify the purchase from your server](https://developer.android.com/google/play/billing/billing_library_overview#Verify-purchase)by using the[Subscriptions and In-App Purchases API](https://developers.google.com/android-publisher#subscriptions).

3. If there are owned but unacknowledged items, acknowledge the purchase either with`[consumeAsync()][consumeAsync]`for consumable items or with`[acknowledgePurchase()][acknowledgePurchase]`for non-consumable items.

4. Grant entitlement to the purchased item inside the game.

## Detect items received in split-screen mode

If your game supports[multi-window mode](https://developer.android.com/guide/topics/ui/multi-window), it is possible for users to redeem Play Points and receive the item, while running the Play Store app and your game at the same time. This screenshot displays an example:

![A screenshot where the Redeem points button is displayed while a game is running.](https://developer.android.com/static/images/guide/playpoints/split-screen.png)

To support this scenario with the Google Play Billing Library, do the following:

1. Google Play calls the`[onPurchasesUpdated()][onPurchasesUpdated]`method to notify your game that there is a new pending item.

2. If your game has a server, we strongly recommend that you[verify the purchase from your server](https://developer.android.com/google/play/billing/billing_library_overview#Verify-purchase)by using the[Subscriptions and In-App Purchases API](https://developers.google.com/android-publisher#subscriptions).

3. Acknowledge the purchase either with[consumeAsync()](https://developer.android.com/reference/com/android/billingclient/api/BillingClient#consumeasync)for consumable items or with[acknowledgePurchase()](https://developer.android.com/reference/com/android/billingclient/api/BillingClient#acknowledgePurchase(com.android.billingclient.api.AcknowledgePurchaseParams,%20com.android.billingclient.api.AcknowledgePurchaseResponseListener))for non-consumable items.
...
- If you call[Purchases.products: get](https://developers.google.com/android-publisher/api-ref/rest/v3/purchases.products/get)on your server, verify whether you need to handle Play rewards exchange items and other one-time products separately based on the`productId`values.

- If you use[Onetimeproducts: list](https://developers.google.com/android-publisher/api-ref/rest/v3/monetization.onetimeproducts/list)on your server, verify whether you need to separate Play rewards exchange items from other one-time products by`productId`values.
