---
title: Handle device orientation with Vulkan pre-rotation  |  Android game development  |  Android Developers
...
# Handle device orientation with Vulkan pre-rotation Stay organized with collections Save and categorize content based on your preferences.
...
1. The Android OS can use the device's Display Processing Unit (DPU),
   which can efficiently handle surface rotation in hardware. Available on
   supported devices only.
2. The Android OS can handle surface rotation by adding a compositor pass. This
   will have a performance cost depending on how the compositor has to deal with
   rotating the output image.
3. The application itself can handle the surface rotation by rendering a
   rotated image onto a render surface that matches the current orientation of
   the display.
...
the Android [`Activity`](/reference/android/app/Activity) and calling the
[`onDestroy()`](/reference/android/app/Activity#onDestroy()) function on the
...
to the activity’s
[`configChanges`](/guide/topics/manifest/activity-element#config) section:
...
If your application fixes its screen orientation using the [`screenOrientation`](/guide/topics/manifest/activity-element#screen)
...
[`onNativeWindowResized()`](/ndk/reference/struct/a-native-activity-callbacks#struct_a_native_activity_callbacks_1a21b6cb2746c27f1874a3b6b5d6a1d6fb) callback to call a function that sets the
...
`surfaceCapabilities.currentTransform` field and the
`swapchainCreateInfo.preTransform` field **match** because this lets
Android know that we are handling the orientation change ourselves, thus
avoiding the Android Compositor.
...
* Ensure that during swapchain creation or recreation, the pretransform flag is
  set to match the flag returned by the Android operating system. This will avoid
  the compositor overhead.
* Keep the swapchain size fixed to the identity resolution of the app's window
  surface in the display’s natural orientation.
* Rotate the MVP matrix in clip space to account for the devices orientation,
  because the swapchain resolution/extent no longer updates with the orientation
  of the display.
