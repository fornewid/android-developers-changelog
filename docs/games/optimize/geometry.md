---
title: Geometry  |  Android game development  |  Android Developers
...
# Geometry Stay organized with collections Save and categorize content based on your preferences.
...
![](/static/images/games/vertices-edges-triangles.jpg)
...
* **Triangle:** Three vertices connected to each other by three edges creates a
  ...
  **Note:** For the purpose of this guide, quads might be referred to as triangles.
  This is a shorthand for a pair of triangles that make up a quad in a 3D
  authoring tool.

Use the following sections to learn more about geometry:

* [Triangle and polygon usage](#triangle-and-polygon-usage)
...
* [Reduce the number of triangles](#reduce-the-number-of-triangles)
* [Use triangles in important areas](#use-triangles-in-important-areas)
* [Remove micro triangles](#remove-micro-triangles)
* [Avoid long, thin triangles](#avoid-long-thin-triangles)
...
![The object on the left has 584 triangles, while the object on the right has 704.](/static/images/games/triangle-comparison.png)
...
![Details in foreground objects are created with triangles. Background details are baked into flat surfaces.](/static/images/games/foreground-and-background.png)
...
![](/static/images/games/triangle-model-comparison.png)
...
![](/static/images/games/total-triangles.png)
...
![](/static/images/games/soldier-triangle-count.png)
...
![](/static/images/games/focus-on-shapes.png)
...
![](/static/images/games/normal-maps.png)
...
![](/static/images/games/micro-triangle-distance.png)
...
![](/static/images/games/micro-triangle-comparison.png)
...
![](/static/images/games/long-thin-triangles.png)
...
camera moves. [LOD](#level-of-detail) can help remove long, thin triangles when
...
of [micro triangles](#remove-micro-triangle). It also looks better for objects
...
![On the left, there's the robot with the most detail and largest number of triangles. To the right, the robot is further away, has less detail, and uses fewer triangles.](/static/images/games/vertices-at-distance.png)
...
![](/static/images/games/distance-comparison.png)
...
![](/static/images/games/static-camera-no-lod.png)
...
![](/static/images/games/lod-comparison.png)

**Figure 15.** A comparison between models as the LOD decreases.

**Note:** Don’t include dense triangles on low LOD. The density is costly and might
not be visible at great distances.

Verify the LOD levels change at the proper distances from the camera. Base your
analysis on how they're used in the game.

![](/static/images/games/lod-closeup.png)

**Figure 16.** Comparison of a LOD 3 model's appearance close up and at its
intended distance.

**Important:** Too little triangle reduction leads to less performance improvement,
as the device still renders a similar number of triangles. Too much polygon
reduction, though, and the change in model from one LOD version to the next is
more noticeable.
...
* You can manually create LOD meshes with any 3D software.
  + To do this, remove edge loops or reduce the number of vertices on the 3D
    object.
  + This gives the artist the most control over the final product but might take
    longer.
* LOD meshes can be created automatically.
  + You can use a modifier inside a 3D package such as ProOptimizer within 3ds
    Max or Generate LOD Meshes within Maya.
  + You can use LOD generation software such as Simplygon or InstaLOD.
  + Some game engines have an automatic LOD generation feature that allows you
    to create and apply LOD meshes.
...
![An example of a smoothing group. On the left, the robot has a smoothing group applied.](/static/images/games/smoothing-group.png)
...
![](/static/images/games/mesh-topology.png)
...
![](/static/images/games/shape-exaggeration.png)
