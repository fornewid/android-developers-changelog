---
title: https://developer.android.com/games/optimize/optimization-tips
...
Inl_RenderCameraStack are observed. It is advisable to ascertain the necessity
of enabled cameras and optimize them accordingly.
[![Unity Profiler timeline showing Main Thread waiting on Gfx.WaitForPresentOnGfxThread](https://developer.android.com/static/images/games/performance-tuner/opt2.png)](https://developer.android.com/static/images/games/performance-tuner/opt2.png) GPU bound example for Unity Profiler (click to enlarge).
...
[![Perfetto trace showing execution times for GameThread, RenderThread, and RHIThread](https://developer.android.com/static/images/games/performance-tuner/opt3.png)](https://developer.android.com/static/images/games/performance-tuner/opt3.png) Perfetto traces with CPU execution details (click to enlarge).

#### GPU overhead case

The trace indicates that the GPU completion itself exceeds 25ms, which signifies
a GPU-bound scenario.
[![Perfetto trace showing GPU completion block waiting for GPU completion](https://developer.android.com/static/images/games/performance-tuner/opt6.png)](https://developer.android.com/static/images/games/performance-tuner/opt6.png) Perfetto traces with GPU overhead details (click to enlarge).
...
[![Simpleperf output displaying functions with the highest CPU usage](https://developer.android.com/static/images/games/performance-tuner/opt8.png)](https://developer.android.com/static/images/games/performance-tuner/opt8.png) Simpleperf CPU profiling: Analyzing function call hierarchy and resource utilization (click to enlarge).
...
[![RenderDoc event browser displaying Vulkan render passes and draw calls](https://developer.android.com/static/images/games/performance-tuner/opt15.png)](https://developer.android.com/static/images/games/performance-tuner/opt15.png) RenderPass and GPU command sequence in RenderDoc (click to enlarge).
...
[![RenderDoc event browser and resource inspector analyzing image layout and render passes](https://developer.android.com/static/images/games/performance-tuner/opt17.png)](https://developer.android.com/static/images/games/performance-tuner/opt17.png) RenderDoc rendering pipeline analysis (click to enlarge).
...
[![Table comparing CPU and GPU performance metrics when Early-Z is enabled versus disabled](https://developer.android.com/static/images/games/performance-tuner/opt18.png)](https://developer.android.com/static/images/games/performance-tuner/opt18.png) Performance impact of Early-Z acceleration (click to enlarge).
...
[![Fragments per pixel comparison with and without the discard shader keyword](https://developer.android.com/static/images/games/performance-tuner/opt19.png)](https://developer.android.com/static/images/games/performance-tuner/opt19.png) RenderDoc GPU debugger for analysis (click to enlarge).
...
1. **Use `D24S8` instead of `D32S8` for depth-stencil buffers:** Using `D24S8` for depth-stencil buffers reduces memory consumption by 20% compared to `D32S8`, with little to no noticeable difference in visual quality across most applications.
2. **Use `ASTC` compression for color textures:** `ASTC` compression significantly reduces texture memory usage---by a factor of up to 8x compared to uncompressed formats---while preserving high visual quality.
3. **Use half-float formats instead of full-float:** Use `R16F` or `RG16F` to reduce memory bandwidth and storage consumption. These formats are well-suited for post-processing buffers.
...
1. **Check actual usage:** Are there any draw calls or shaders that are writing to or reading from the attachment?
2. **Analyze frame output:** Use `RenderDoc` or comparable utilities to determine whether the attachment contributes to the final image.
3. **Consider transient or dummy attachments:** Transient attachments or a 'Don't Care' store operation should be used for temporary data that does not require persistent storage.
...
[![Table comparing CPU and GPU performance metrics when using mediump versus highp shader precision](https://developer.android.com/static/images/games/performance-tuner/opt21.png)](https://developer.android.com/static/images/games/performance-tuner/opt21.png) Performance impact of shader precision (click to enlarge).
...
[![Mali Varying Usage profiler displaying 16-bit interpolation alongside shader code using mediump](https://developer.android.com/static/images/games/performance-tuner/opt22.png)](https://developer.android.com/static/images/games/performance-tuner/opt22.png) Example of profiling tools and GPU debuggers (click to enlarge).
...
workload.
[![Vulkan command log displaying vkCmdSetCullMode set to VK_CULL_MODE_NONE](https://developer.android.com/static/images/games/performance-tuner/opt23.png)](https://developer.android.com/static/images/games/performance-tuner/opt23.png) Debug logs with back-face culling (click to enlarge).
...
1. Verify the absence of superfluous overdraw. In user interface contexts, where the entire screen might render, confirm that preceding rendering passes are not unnecessarily overdrawn.
2. Enable depth testing and culling to optimize performance.
3. Consider rendering order from front to back.

[![RenderDoc event browser and texture viewer identifying an unnecessary overdraw render pass](https://developer.android.com/static/images/games/performance-tuner/opt24.png)](https://developer.android.com/static/images/games/performance-tuner/opt24.png) Example to eliminate superfluous draw calls and render passes (click to enlarge).