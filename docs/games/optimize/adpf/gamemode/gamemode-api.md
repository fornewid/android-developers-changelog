---
title: Game Mode API  |  Android game development  |  Android Developers
...
# Game Mode API Stay organized with collections Save and categorize content based on your preferences.
...
**Warning:** OEMs may choose to implement Game Mode interventions without developer
feedback.
...
**Important:** Battery Saver mode is game-specific and does not impact system
level Android Battery Saver behavior.

1. Add code to query the game mode state in the
   [`onResume`](/reference/android/app/Activity#onResume()) function:

   ![Example with user setting Battery mode](/static/games/optimize/adpf/gamemode/images/set-battery-flow.png)

**Important:** Your app must always query the `getGameMode()` method when resuming a
paused process. See the diagram above for details.
...
**Important:** If your game declares support for a Game Mode, the game must
implement its own optimization. As a result, the platform resets any previously
applied Game Mode interventions by OEMs.
...
![Game Dashboard Activity!](/static/images/games/game-mode/gamedashboardactivity.png "Game Dashboard Activity")
