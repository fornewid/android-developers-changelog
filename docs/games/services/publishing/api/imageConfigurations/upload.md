---
title: ImageConfigurations: upload  |  API reference  |  Android Developers
...
# ImageConfigurations: upload Stay organized with collections Save and categorize content based on your preferences.
...
| --- | --- | --- |
| **Path parameters** | | |
| `imageType` | `string` | Selects which image in a resource for this method.   Acceptable values are:  * "`ACHIEVEMENT_ICON`": The icon image for an achievement resource. * "`LEADERBOARD_ICON`": The icon image for a leaderboard resource. |
...
| `imageType` | `string` | The image type for the image.   Acceptable values are:  * "`ACHIEVEMENT_ICON`" * "`LEADERBOARD_ICON`" |  |