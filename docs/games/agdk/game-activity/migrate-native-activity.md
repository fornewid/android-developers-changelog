---
title: Migrate from NativeActivity  |  Android game development  |  Android Developers
...
# Migrate from NativeActivity   Part of [Android Game Development Kit](/games/agdk/overview).

This page describes how to migrate from
[`NativeActivity`](/reference/android/app/NativeActivity) to
[`GameActivity`](/games/agdk/game-activity) in your Android game project.
...
* Handles touch and key events in the `GameActivity` Java class rather than
  the `NativeActivity` `onInputEvent` interface.

Before migrating, we recommend reading the
[get started guide](/games/agdk/game-activity/get-started), which describes how
to set up and integrate `GameActivity` in your project.

## Java build script updates

`GameActivity` is [distributed](/games/agdk/game-activity#releases) as a
Jetpack library. Make sure to apply the Gradle script updating steps described
in the [get started guide](/games/agdk/game-activity/get-started):
...
4. Add the `GameActivity` dependency to your application:
   ...
   2. If your current minimum supported API level is less than 16, update it
      to at least 16.
   3. Update the compiled SDK version to the one that the `games-activity`
      library requires. Jetpack typically requires the latest SDK version at
      the release build time.
...
3. Apply the theme to the application in the `AndroidManifest.xml` file:
   ...
   immersive guide](/training/system-ui/immersive) and example implementation in
...
[ndk-build documentation page](/ndk/guides/ndk-build).
...
* [TextInput](/games/agdk/add-support-for-text-input).
* [Game Controller](/games/sdk/game-controller).
