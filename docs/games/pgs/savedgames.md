---
title: Cloud save  |  Android game development  |  Android Developers
...
# Cloud save Stay organized with collections Save and categorize content based on your preferences.
...
To meet the Google Play Games Level Up [user experience guidelines](/games/guidelines),
...
1. **Cloud Save**
   ...
   **Note:** If your game supports guest mode, player progress is not saved or
   restored. This limitation occurs because guest mode is tied to a
   single device.
2. **Conflict resolution for multiple game states**
   ...
   * **Multiple accounts per user:** Handle instances where a single user
...
* **Synchronize game data across multiple devices.** For example, a player can
  start a game on an Android phone and seamlessly continue playing on a tablet
  without losing progress.
* **Ensure data persistence.** Players can resume their game even if their
  device is lost, destroyed, or traded in for a newer model.

**Note:** Before using the Saved Games service, you must first
[enable it in Google Play Console](/games/pgs/console/enable-features#enabling-saved-games).
...
* Structured metadata - additional [properties](#saved-game-metadata)
  ...
  [Google Play Games app](//play.google.com/store/apps/details?id=com.google.android.play.games) (for example, last updated timestamp).
...
| **Name** | A developer-supplied short name for the saved game, for example "Save slot 1" or "PlayerName\_Save1". This is not shown to players. |
