---
title: Integrate the achievements  |  Android game development  |  Android Developers
...
# Integrate the achievements Stay organized with collections Save and categorize content based on your preferences.
...
* Review the [design high quality achievements](/games/pgs/achievements#design-achievements) guidelines.
* Review the terminology in
  [achievements basics](/games/pgs/achievements#achievements-basics).
...
* [Create an achievement](#create-achievement): Add definitions and metadata for each
  achievement one at a time.
* [Import achievements](#import-achievements): Import definitions and other metadata
  for multiple achievements together in a single step.

**Note:** Use APIs to [unlock achievements](/games/pgs/android/achievements#unlock_achievements) for the signed in
player. This will reveal any hidden achievements to the player.
...
1. In the [Google Play Console](https://play.google.com/console),
   select a game.
2. In the **Play Games Services - Achievements** page
   (**Grow users > Play Games Services > Setup and management > Achievements**),
   select **Create achievement**.
3. In the **Add achievements** page, complete the form.
4. Click **Save as draft**.
5. After you have created an achievement, you need to
   [publish your achievements](/games/pgs/integrate-achievements#publish-achievements).
...
2. In the **Play Games Services - Achievements** page
   (**Grow users > Play Games Services > Setup and management > Achievements**),
   select **Import achievements**.
3. In the **Import achievements** page, click **Upload**.
4. Select the zip file to upload.
5. Click **Save as draft**.
6. After you have imported achievements, you need to
   [publish your game](/games/pgs/integrate-achievements#publish-achievements).
...
| --- | --- | --- |
| [`AchievementsMetadata.csv`](/games/pgs/integrate-achievements#achievementsmetadatacsv-format) | Required | Metadata for each achievement. See [Attributes](/games/pgs/integrate-achievements#attributes). |
| [`AchievementsLocalizations.csv`](/games/pgs/integrate-achievements#achievementslocalizationcsv-format) | Optional | Provides translations for achievement names and descriptions. |
| [`AchievementsIconsMappings.csv`](/games/pgs/integrate-achievements#achievementsiconsmappingscsv-format) | Optional | Maps achievements to their icon files. |
| [Icon files](/games/pgs/integrate-achievements#icon-files) | Optional | Icons in PNG, JPEG, or JPG format. |
...
* Each row must have seven values. If you want to omit a value,
  leave it blank.
* The `Name` and `Description` fields are used as the
  default locale.
* The `Name` field should be unique across all achievements.
* The following fields shouldn't contain commas: `Name` and
  `Description`.
...
| Locale | Required | The locale code such as `en-US`. | [Add translations](/games/pgs/console/enable-features#add_translations) for your game before specifying a locale. Note that you cannot specify the default locale. Supported locale codes can be found in the list of [supported languages](https://support.google.com/googleplay/android-developer/table/4419860). |

`AchievementsLocalizations.csv` file requirements:

* Achievement names within a game must be unique per locale.

  For example, if a game supports both en-US and fr-FR locales, an achievement
  named "Achievement1" for en-US can also be named "Achievement1" for fr-FR.
* Each row should have four values. If you want to omit the Localized
  description value, leave it blank.
...
![A sample achievement icon.](/static/images/games/pgs/achievementIconExample.png)
...
4. Click **Save as draft**.
5. In the **Play Games Services - Achievements** page, the edited
   achievement is in the "Available to testers" status.
...
**Note:** Once an achievement is published, its initial state (hidden or
revealed) and its type (incremental or standard) is fixed and can't be changed.
...
1. In the **Play Games Services - Achievements** page, select an
   achievement.
...
3. To reset achievement data programmatically, call the
   [Management API `Achievements` methods](/games/services/management/api/achievements).
...
[Adding translations for your game](/games/pgs/console/enable-features#add_translations).

There are two ways in which you can add translations for your game:

* You can use the [import achievements](#import-achievements) option to upload
...
* You can add translations for each achievement in your game.
  ...
  1. In the Google Play Console open the **Achievements** tab for
     your game, then select an existing achievement.
  2. In the achievements details page, select the tab for a language that you
     previously added in the **Game details** tab.
  3. In the achievement details page for that language, edit the form with your
     translations for that achievement.
...
* [Android](/games/pgs/android/achievements)
* [Unity](/games/pgs/unity/achievements)
* [Web](/games/services/web/api/rest#achievements)

## Testing achievements

**Note:** During testing, it is normal that the "achievement unlock" popup shows 0
XP gained. This is to prevent interfering with live scoring systems.
...
* [Add test accounts](/games/pgs/console/publish) to your game project for testers.
* Create Play Games Services profile for one of the test account using Play
  Games App.
* Open the game using the same test account for which you created the
  Play Games Services profile.
* Verify that Play Games Services "Welcome toast" is shown on the screen as a
  confirmation of successful automatic authentication on game launch.
...
* On your **Achievements** page, you can find statistics to understand whether
  your players are unlocking the achievements. For those whose achievements
  are not hidden, you can see what percentage are unlocking them. For early
  stage achievements, the unlock percentage is typically closer to 100%.
...
3. In the **Play Games Services - Publishing** page
   (**Grow users > Play Games Services > Setup and management > Publishing**),
   review the actions and fix the issues.
4. Click **Publish**.
...
* In the Play Console, go to
  **Grow users > Play Games Services > Setup and management > Achievements**.
  ...
  + Before publishing, verify that achievements configured in the
    Play Console are not in "Draft" state.
  + Don't add achievement ID values from the Play Console to
    the game code. Use the [client implementations](#client-implementations) instead.
