---
title: Manage Play Games Services project settings in Google Cloud Platform  |  Android game development  |  Android Developers
...
# Manage Play Games Services project settings in Google Cloud Platform Stay organized with collections Save and categorize content based on your preferences.
...
* Editing authorized URIs or JavaScript origins in Client IDs that you've
  created in the past.
...
1. Select a credential for your game on the **Configuration** page
...
1. Open the [Consent Screen page](https://console.developers.google.com/apis/credentials/consent)
   in Play Console. If prompted, select a project or create a
   new one.
...
**Important:** If you change the launch URL of
your web app, you must follow the instructions below to make the corresponding
change in Play Console to avoid getting `origin_mismatch` errors. If you
change the package name of your Android app, you must create a new linked app
entry and remove the existing linked app entry that has the old package name.
...
2. Select a credential for your game on the **Configuration** page
   (**Grow users > Play Games
   Services > Setup and
   management > Configuration**).
...
5. In the sidebar on the left, select **APIs & auth**. Make sure that the
