---
title: Debugger  |  Android game development  |  Android Developers
...
# Debugger Stay organized with collections Save and categorize content based on your preferences.
...
game on Android. See the [Run the sample](/games/agde/quickstart#run-sample) section for details.
...
4. Select the game process from the list of available processes and
   click **Attach**.

![Attach to process](/static/images/agde/debug-in-visual-studio-img-attach-to-process.png)
...
**Caution:** Some shell commands may cause Visual Studio to fall out of sync with
LLDB and behave unexpectedly.
...
expressions. See [Natvis](#Natvis) for more information.
...
| character | c | show the bytes as ASCII characters  e.g. (int \*) c.sp.x = P\xf8\xbf\_\xff\x7f\0\0 |
...
| enumeration | E,en | show this as an enumeration, printing the value's name if available or the integer value otherwise e.g. (enum enumType) val\_type = eValue2 |
