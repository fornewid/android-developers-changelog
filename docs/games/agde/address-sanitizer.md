---
title: Debugging Memory Corruption using Address Sanitizer  |  Android game development  |  Android Developers
...
# Debugging Memory Corruption using Address Sanitizer Stay organized with collections Save and categorize content based on your preferences.
...
[HWAddress Sanitizer](/ndk/guides/hwasan) (HWASan) and [Address
Sanitizer](/ndk/guides/asan) (ASan) are memory corruption debugging tools that
...
**Note:** Use HWASan rather than ASan, as HWASan is faster and has lower memory
overhead. However, HWASan has some additional requirements.
...
**Caution:** You must use the shared version of the Standard Library in your project
when using these features.

To change this setting, see the [Linking the C++ Standard Library](#linking-c++)
section of this document.

**Note:** If you've enabled ASan and haven't enabled the shared C++ library, you'll
...
generation](#enabling-frame) section of this document.

**Note:** If you've enabled ASan and haven't enabled Frame Pointer generation,
you'll encounter a build error.
...
**Caution:** To avoid unnecessary overhead, remember to set the property value back
to
**Disabled** when you no longer need to use these tools.
...
**Caution:** Be sure to reset this property value back to your original setting when
not using HWASan or ASan.
...
**Caution:** Don't use **GNU STL** libraries here, as they are unsupported by
versions 25 or higher of the NDK. For more information, see the NDK
documentation on [C++ Support](/ndk/guides/cpp-support).**Note:** Remember to restore these settings back to their original values once
you're done using HWASan or ASan.
...
[Teapot sample](/games/agde/samples#teapot). You might do this if, for example,
...
**Note:** See the [PoolAllocator sample](/games/agde/samples#poolallocator) that
shows how to integrate HWASan into a custom memory allocator.
...
in your pool. See [PoolAllocator sample](/games/agde/samples#poolallocator) for
