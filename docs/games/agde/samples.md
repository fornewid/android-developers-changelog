---
title: Samples  |  Android game development  |  Android Developers
...
[downloads page](/games/agde#span-iddownloadsandroid-game-development-extension-downloadsspan):
...
* Install the Android Game Development Extension and the samples. See the
  [quickstart](/games/agde/quickstart) for details. The topic also describes how to build
  and run a sample and uses the Android version of the [Teapot](#teapot) sample
  as an example.
* The [project configuration](/games/agde/adapt-existing-project) guide describes how to
...
[modifying your Gradle build scripts](/games/agde/adapt-existing-project#change-build-scripts).
...
2. Add an **Android arm64-v8a** platform. For more
   information, see
   [Adding an Android Platform](/games/agde/adapt-existing-project#add-android-platform).
3. [Add an Android APK item](/games/agde/adapt-existing-project#add_an_android_apk_item)
   to the new platform.
4. Compile the project.
5. Add the following Android platforms and then add an Android APK item
   to each of them: **Android-armeabi-v7a**,
   **Android-x86**, and **Android-x86\_64**.
6. [Build and run](/games/agde/quickstart#run-sample) the sample.
...
* glm: a snapshot of the [OpenGL Math repo](https://github.com/g-truc/glm)
  that is built as a static library.
* native\_app\_glue: an NDK wrapper that communicates with the
...
* Custom Android configurations that demonstrate how to use
  [Address Sanitizer](https://developer.android.com/ndk/guides/asan) (ASan) and
  [Hardware Address Sanitizer](/ndk/guides/hwasan) (HWASan).
...
To set up the sample and run it on Android, see the [quickstart](/games/agde/quickstart). To
...
1. Install GLEW:
   ...
   2. Copy the binary files from
      `$your-glew-directory\bin\Release\x64` to
      `%SystemRoot%\system32`.
2. Install freeglut:
...
3. Add the freeglut project dependencies:
   ...
   4. Add `$your-freeglut-dir\include` to the path.  
      ![Screenshot of the Additional Include Directories dialog.](/static/images/agde/agde-samples-teapot-include.png)
   5. Click **Ok**.
   6. Choose **Linker > General > Additional Library Directories**.
   7. Add `$your-freeglut-dir\lib\x64` to the path.
      ![Screenshot of the Additional Library Directories dialog.](/static/images/agde/agde-samples-teapot-library.png)
   8. Click **Ok**.
...
5. Run the sample on Windows:
   **Note:** To improve build times, Android source files that aren't
   shared with Windows should be excluded when you build the sample for
   Windows. To exclude files, use the
   **Exclude from Build** command in the
   **Properties Pages** dialog for each file you want to
   exclude in **Solution Explorer**.
   1. On the Visual Studio toolbar, click the **Local Windows Debugger** run button.
   2. The sample should look as follows:  
      ![Screenshot of the Teapot sample running on Windows.](/static/images/agde/agde-samples-teapot-windows.png)
...
1. In Visual Studio, verify that custom build tools are configured for
   ...
   6. Ensure that
      **Configuration Properties > Custom Build Tools > Command Line**:
      ...
      uses the NDK toolchain to build both x86 and x86\_64 Android
      projects. If you want to use yasm for the x86\_64 Android platform,
   ...
   ![Screenshot of the Property Page for Custom Build Tools.](/static/images/agde/agde-samples-assembly-properties.png)
2. Build the project. This builds the
   `libAssmeblyCodeLinkObjects.so` file:
...
3. To confirm the functions are exported correctly to the Android
   ...
   2. Go to the Android library location that was generated by your
      ...
      and `$sample_dir\Debug\Android-arm64-v8a\arm64-v8a` for
