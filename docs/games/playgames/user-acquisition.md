---
title: User Acquisition for Google Play Games on PC  |  Android game development  |  Android Developers
...
# User Acquisition for Google Play Games on PC Stay organized with collections Save and categorize content based on your preferences.
...
1. The developer generates a Google Play URL, which includes marketing
   ...
   **Note:** The referrer string must be URL encoded, 512 characters or less, and
   included in the referrer query parameter of the Google Play URL.
2. When a PC user clicks on an ad with this link, they are redirected to the
   ...
   *Install on Windows*.
   ...
   PC](/static/images/games/playgames/web-play-store-ui.png)
   ...
   client if needed](/static/images/games/playgames/gpg-install-client-prompt.png)
   ...
   **Note:** Depending on the browser, the user may need to accept a permission to
   allow the browser to open links within the Google Play Games on PC client
...
3. Upon successful install and launch, the developer can retrieve referrer
   ...
   referrer string](/static/images/games/playgames/gpg-client-referrer-string.png)

**Note:** After a user initiates an install through the Google Play web UI, the GPG
on PC client will retain the referrer until the game is successfully installed
(up to 28 days after the install was originally initiated). After a successful
game install, the referrer details will be maintained on the client for 90 days,
as is [standard for the Play Referrer
API](/google/play/installreferrer/igetinstallreferrerservice). As such, the
referrer may be retained for at most 118 days.
