---
title: PC compatibility and optimization for Google Play Games on PC  |  Android game development  |  Android Developers
...
# PC compatibility and optimization for Google Play Games on PC Stay organized with collections Save and categorize content based on your preferences.



[Google Play Games on PC](/games/playgames/overview) requires that you make PC
...
[Get started guide](/games/playgames/start#requirements-checklist).

When making the PC compatibility and optimization changes (other than x86-64
support), you can set up your game to
[detect Google Play Games on PC](#detect-hpe) and then disable or enable
platform-specific features. This allows you to use the same APK or App bundle
for your Android mobile and PC releases.

**Note:** For information about the minimum requirements for players to run
Google Play Games on PC on a PC, see
[minimum PC requirements](https://support.google.com/googleplay?p=eligibility_requirements)
in Google Help Center.

Here's a summary of the requirements and recommendations on this page:

* [Include x86-64 ABI](#x86-requirement) (required)
* [Detect Google Play Games at runtime](#detect-hpe) (recommended)
* [Handle the onPause event](#lifecycle-events) (recommended)
* [Update UI elements](#ui-compatibility) (required)
* [Disable Android app permissions dialogs](#permissions-dialogs) (required)
* [Disable unsupported Android features and permissions](#unsupported-android-features) (required)
* [Replace WebViews with browser intents](#browser-intent) (recommended)
* [Disable unsupported Google Play Service APIs](#unsupported-google-apis) (required)
* [Enable scoped storage](#scoped-storage) (required)
* [Migrate to Google Analytics 4](#analytics) (recommended)
...
*Required by the [release process](/games/playgames/checklist)*
...
[full certification](#target_architecture_in_unity) with an
...
[`onPause`](/guide/components/activities/activity-lifecycle#onpause)
([c++](/ndk/reference/struct/a-native-activity-callbacks#struct_a_native_activity_callbacks_1ad7bd685219371874af86549772d7fbaa))
...
* Replace UI actions that need two or more fingers (multi-touch gestures). For
  example, you should replace pinch-to-zoom and other multi-touch gestures
  with the corresponding mouse and keyboard input. For details about device
  input changes, see [Input support](/games/playgames/input).

Recommended:

* All user-visible text should say "click" instead of "tap".
* Scrollable lists should have scrollbars.
* Areas that users can pan should either have scrollbars or some other way to
  traverse large distances quickly.
...
* Dialogs should have a visible close button. Don't detect a click outside of
  the dialog bounds.
...
[displayed permission dialogs previously](/training/permissions/usage-notes#be_transparent),
...
* **Don't** attempt to use missing features at runtime. If you are using the
  ...
  runtime](/games/playgames/pc-compatibility#detect-hpe) and avoid the relevant
  code paths.
...
[Chromebook app manifest compatibility](/topic/arc/manifest) guide.

### Functional testing requirements

Remove these hardware features before submitting the first
[test build](/games/playgames/checklist#m1-requirements) to
Google Play Console:
...
[Monitor connectivity status and connection metering](/training/monitoring-device-state/connectivity-status-type). For a
complete list of unsupported features, see the [app manifest compatibility guide
for Chromebooks](/topic/arc/manifest#unsupported-hardware-features).

### Quality testing requirements

These commonly-used hardware features *aren't compatible* with PCs, so you must
remove them before the
[final submission](/games/playgames/checklist#m2-requirements) to
Google Play Console:
...
* [How to enable scoped storage](/about/versions/11/privacy/storage)
* [Best practices for handling non-media files](/training/data-storage/use-cases#handle-non-media-files)
