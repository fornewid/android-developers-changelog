---
title: Kotlin style guide  |  Android Developers
...
* Line break after the closing brace, *only if* that brace terminates a
  statement or terminates the body of a function, constructor, or *named* class.
...
* A method or constructor name stays attached to the open parenthesis (`(`) that
  follows it.
...
* *Between* consecutive members of a class: properties, constructors,
  ...
  + **Exception:** A blank line between two
  ...
  + **Exception:** Blank lines between enum constants are covered
    below.
* Between statements, *as needed* to organize the code
  into logical subsections.
* *Optionally* before the first statement in a function,
...
* Separating any reserved word, such as `if`,
  `for`, or `catch`
  from an open parenthesis (`(`) that follows it on that line.
  ...
  ```
* Separating any reserved word, such as `else` or
  `catch`, from a
...
* On both sides of any binary operator.
  ...
  + the range operator (`..`).
    ...
    ```
* Before a colon (`:`) only if used in a class declaration for specifying
  a base class or interfaces, or when used in a `where` clause
  for
  [generic constraints](https://kotlinlang.org/docs/reference/generics.html#generic-constraints).
...
Constant names use UPPER\_SNAKE\_CASE: all uppercase letters,
