---
title: Set up Room Database for KMP  |  Kotlin  |  Android Developers
...
# Set up Room Database for KMP Stay organized with collections Save and categorize content based on your preferences.
...
information on using Room, see [Save data in a local database using Room](/training/data-storage/room) or
...
**Note:** To help with migrating from `SupportSQLiteDatabase` to `SQLiteDriver`, use
the new artifact `androidx.room:room-sqlite-wrapper` available in Room
2.8.0 and higher. Read more in the
[Use Room SQLite Wrapper](#migrate-room-sqlite-wrapper) section.
...
[platform-specific implementation of SQLite](#selecting-sqlitedriver), but we
recommend bundling it with your app to prevent any inconsistencies between the
platform implementations of SQLite.
...
location using Room Gradle Plugin](/training/data-storage/room/migrating-db-versions#set_schema_location_using_room_gradle_plugin).
...
`kotlin.native.disableCompilerDaemon = true` to the `gradle.properties`
configuration file for Room's KSP processor to function properly. This property
is not needed when using Kotlin version 2.0 or higher. For more information, see
[KT-65761](https://youtrack.jetbrains.com/issue/KT-65761).
...
Next, either define a new [DAO interface](/training/data-storage/room/accessing-data) or move an existing one to
`commonMain`:
...
Define or move your [entities](/training/data-storage/room/defining-data) to `commonMain`:
...
**Note:** You can optionally use [expect / actual declarations](https://kotlinlang.org/docs/multiplatform-expect-actual.html)
to create platform-specific Room implementations. For example, you can add a
platform-specific DAO that is defined in common code using `expect` and then
specify the `actual` definitions with additional queries in platform-specific
source sets.
...
[`Context.getDatabasePath()`](/reference/android/content/Context#getDatabasePath(java.lang.String)) API. To create the database instance, specify a
[`Context`](/reference/android/content/Context) along with the database path.
...
**Note:** `System.getProperty("java.io.tmpdir")` points to the temporary folder on
the system, which might be cleaned upon reboot. For example, on macOS, you can
instead use the `~/Library/Application Support/[your-app]` folder.
...
target platform. The previous code snippets use the [`BundledSQLiteDriver`](/reference/kotlin/androidx/sqlite/driver/bundled/BundledSQLiteDriver).
...
[Driver implementations](/kotlin/multiplatform/sqlite#sqlite-driver-implementations) for descriptions of available driver
implementations. You can use either of the following:

* [`AndroidSQLiteDriver`](/reference/androidx/sqlite/driver/AndroidSQLiteDriver) in `androidMain`
* [`NativeSQLiteDriver`](/reference/kotlin/androidx/sqlite/driver/NativeSQLiteDriver) in `iosMain`
...
[`androidx.sqlite.db`](/reference/kotlin/androidx/sqlite/db/package-summary) need to be refactored with SQLite Driver APIs,
because the APIs in [`androidx.sqlite.db`](/reference/kotlin/androidx/sqlite/db/package-summary) are Android-only (note the
different package than the KMP package).

**Note:** If you're using any low-level SQLite APIs in your codebase, refer to
[Migrate SQLite to Kotlin Multiplatform](/kotlin/multiplatform/sqlite#migrate).
...
Transaction APIs](#transactions) section.
...
**Note:** Migrating existing DAO blocking functions to suspend functions can be
complicated if the existing codebase does not already incorporate coroutines.
Refer to [Coroutines in Android](/kotlin/coroutines) to get started on using coroutines
and [Flows in Android](/kotlin/flow) to get started on using flows in your codebase.
...
**Note:** The write transactions always block parallel write transactions. If the
journal mode is rollback (non-WAL), then sticking to immediate is still the best
...
* [Save data in local database with Room](/training/data-storage/room)