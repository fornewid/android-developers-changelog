---
title: Set up the Android Gradle Library Plugin for KMP  |  Kotlin  |  Android Developers
...
# Set up the Android Gradle Library Plugin for KMP Stay organized with collections Save and categorize content based on your preferences.
...
Gradle Plugin APIs that are deprecated and [require opt-in](/r/tools/new-dsl) in Android
Gradle plugin 9.0 and higher (Q4 2025). These APIs
[are expected to be removed](/build/releases/gradle-plugin-roadmap) in Android Gradle plugin 10.0
(second half of 2026).

**Note:** There isn't a direct replacement for configuring a Kotlin Multiplatform
module using `com.android.application` plugin. To migrate, extract your
Android **application** to a separate Gradle module.

To apply this plugin, refer to the [Apply the Android-KMP plugin](#apply)
section. If you need to migrate from the legacy APIs, check the [Migration
Guide](#migrate).

**Caution:** Before you migrate to the new plugin, be aware of the
[known issues](#known-issues) and
[unsupported features](#unsupported-features-and-workarounds).
...
* **No top-Level Android extension:** Configuration is handled with an
  `android` block within the Gradle KMP DSL, maintaining a consistent
...
* **Opt-in Java compilation:** Java compilation is disabled by default. Use
  `withJava()` in the `android` block to enable it. This improves build
...
* **Build variants**
  ...
  For more details, see [Create an Android library](/studio/projects/android-library) and
  [Configure build variants](/build/build-variants).
...
4. **Configure Android KMP target.** Configure the Kotlin Multiplatform block
   ...
   the Android target using `android`:
...
**Note:** For Android Gradle Plugin versions below 8.12.0, the configuration
block for the Android target is `androidLibrary{}`. Starting with AGP 8.12.0,
a new `android{}` block was introduced to replace `androidLibrary{}`.
`androidLibrary{}` is deprecated since AGP 9.1.0-alpha09 and will be removed in
a future release. If you are using AGP 8.12.0 or higher, use `android{}`,
otherwise use `androidLibrary{}`.
...
device tests](#configure-tests) for more information).
...
is now configured directly inside the `kotlin { android {} }` block
...
`kotlin.android.localDependencySelection` block. This makes it much
...
Refer to [Resolve Matching Errors](/build/build-variants#resolve_matching_errors) for more
...
[Compose Preview](/develop/ui/compose/tooling/previews)) but is not part of the compile
classpath or the published API of your library.

**Preview:** This workaround is only available in Android Gradle plugin version
9.0.0-beta01 and higher.
...
[Prepare your library for release](/build/publish-library/prep-lib-release).
...
* [`KotlinMultiplatformAndroidLibraryExtension`](/reference/tools/gradle-api/8.11/com/android/build/api/dsl/KotlinMultiplatformAndroidLibraryExtension)
* [`KotlinMultiplatformAndroidLibraryTarget`](/reference/tools/gradle-api/8.11/com/android/build/api/dsl/KotlinMultiplatformAndroidLibraryTarget)
* [`KotlinMultiplatformAndroidDeviceTest`](/reference/tools/gradle-api/8.11/com/android/build/api/dsl/KotlinMultiplatformAndroidDeviceTest)
* [`KotlinMultiplatformAndroidHostTest`](/reference/tools/gradle-api/8.11/com/android/build/api/dsl/KotlinMultiplatformAndroidHostTest)
* [`KotlinMultiplatformAndroidVariant`](/reference/tools/gradle-api/8.11/com/android/build/api/variant/KotlinMultiplatformAndroidVariant)
...
* [Compose preview fails when using new android-KMP plugin](https://issuetracker.google.com/issues/422373442)
  ...
  + Fixed in `Android Studio Otter 2 Feature Drop | 2025.2.2 Canary 3`
    and `Android Gradle Plugin 9.0.0-alpha13`
