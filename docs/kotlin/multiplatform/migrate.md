...
![Adding a new run script build phase](https://developer.android.com/static/kotlin/images/kmp_xcode_runscript.png) **Figure 5.** Run script build phase

> [!NOTE]
> **Note:** `cd "$SRCROOT/.."` navigates the script to the root folder of the KMP Gradle project, which in this case is a parent folder. If in your case the iOS project is on a different path, you need to tweak that here.
