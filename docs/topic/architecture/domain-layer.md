...
# Domain layer Stay organized with collections Save and categorize content based on your preferences.
...
![When it is included, the optional domain layer provides dependencies to
    the UI layer and depends on the data layer.](/static/topic/libraries/architecture/images/mad-arch-domain-overview.png)
...
guidance with other definitions you may have read elsewhere. There may be subtle
...
![GetLatestNewsWithAuthorsUseCase depends on repository classes from the
    ...
    that is also in the domain layer.](/static/topic/libraries/architecture/images/mad-arch-domain-usecase-deps.png)


**Figure 2.** Example dependency graph for a use case that depends on other
use cases.
...
**Note:** In some cases, logic that can exist in use cases could instead be part of
static methods in `Util` classes. However, the latter is discouraged because
`Util` classes are often hard to find and their functionality is hard to
discover. Furthermore, use cases can share common functionality such as
threading and error handling in base classes that can benefit larger teams at
scale.
...
![GetLatestNewsWithAuthorsUseCase depends on two different repository
    classes from the data layer: NewsRepository and AuthorsRepository.](/static/topic/libraries/architecture/images/mad-arch-domain-multiple-repos.png)
...
**Note:** The [Room library](/training/data-storage/room) lets you query
[relationships](/training/data-storage/room/relationships) between different
entities in a database. If the database is the [source of
truth](/jetpack/guide/data-layer#source-of-truth), you can create a query that
does all that work for you. In that case, it's better to create a repository
class like `NewsWithAuthorsRepository` instead of a use case.
...
![UI layer cannot access data layer directly, it must go through the Domain layer](/static/topic/libraries/architecture/images/mad-arch-domain-data-access-restriction.png)


**Figure 4.** Dependency graph showing UI layer being denied access to
the data layer.
...
[General testing guidance](/training/testing) applies when testing the domain
