...
![](/static/topic/libraries/architecture/images/mad-arch-ui-basic-case-study.png)
...
[Jetpack Compose](/jetpack/compose)). Because the role of the [data
...
Refer to the [case study](#case-study) outlined earlier. In short, the UI shows
...
![](/static/topic/libraries/architecture/images/mad-arch-ui-basic-case-study-item.png)
...
* **UI behavior logic** or **UI logic** is *how* to display state changes on
  ...
  [snackbar](/training/snackbar).

The UI logic, particularly when it involves UI types like
[`Context`](/reference/android/content/Context), should live in the UI, not in
...
codelab](/codelabs/basic-android-kotlin-training-livedata). For a similar
introduction to Kotlin flows, see [Kotlin flows on Android](/kotlin/flow).
...
such as `StateFlow` or `LiveData` that you see in this guide can be easily
consumed in Compose using the appropriate
[extensions](/jetpack/compose/libraries#streams).
...
[Show errors on the screen](#show-errors) section to learn more about error
handling.

**Note:** The pattern shown in the example above where state is mutated through
...
events](/jetpack/guide/ui-layer/events) page to learn more.
...
like the ones covered in the [Consume UI state](#consume-ui-state) section
above, except that the consumption implementation should defer to the
[Navigation component](/guide/navigation).
...
stream. See the [Android Paging](/codelabs/android-paging) codelab for a
