---
title: Create an instant-enabled app bundle  |  Other Play guides  |  Android Developers
...
# Create an instant-enabled app bundle Stay organized with collections Save and categorize content based on your preferences.
...
an [Android App Bundle](/guide/app-bundle). Such a bundle is known as an
...
[migrate your instant app to support Android App Bundles](/topic/google-play-instant/feature-module-migration).
...
* [Install Android Studio 3.6](/studio) or higher. After opening Android Studio,
  download the Google Play Instant Development SDK from the **SDK Tools**
  tab in the [SDK Manager](/studio/intro/update#sdk-manager).
...
either a physical or [virtual](/studio/run/managing-avds) device.
...
* `BILLING` – *Deprecated as of [Play
  Billing Library 1.0](/google/play/billing/billing_library_releases_notes#release-1_0).*
* [`CAMERA`](/reference/android/Manifest.permission#CAMERA)
* [`INSTANT_APP_FOREGROUND_SERVICE`](/reference/android/Manifest.permission#INSTANT_APP_FOREGROUND_SERVICE) –
  *Only in Android 8.0 (API level 26) and higher.*
* [`INTERNET`](/reference/android/Manifest.permission#INTERNET)
...
* [`VIBRATE`](/reference/android/Manifest.permission#VIBRATE)
* [`WAKE_LOCK`](/reference/android/Manifest.permission#WAKE_LOCK)
...
* `BILLING`: This is a deprecated permission. Use the
  [Google Play Billing Library](/google/play/billing/billing_library_overview),
...
* Use [background services](/training/run-background-service/create-service).
* [Send notifications](/training/notify-user/build-notification) when running in
  the background.
...
* An installed app contains an intent filter that includes
  [`CATEGORY_BROWSABLE`](/reference/android/content/Intent#CATEGORY_BROWSABLE).
* The instant experience is sending an intent using either the
  [`ACTION_SEND`](/reference/android/content/Intent#ACTION_SEND),
  [`ACTION_SENDTO`](/reference/android/content/Intent#ACTION_SENDTO), or
...
* If you have an existing app bundle that contains only a [base
  module](/studio/projects/dynamic-delivery/configure-base), you can instant-enable
...
* If you have an existing app bundle that contains multiple modules, you can
  ...
  module](/studio/projects/dynamic-delivery/instant-delivery). This process
  also instant-enables your app's base module, giving you the option to [support
  multiple instant entry
  points](/topic/google-play-instant/guides/multiple-entry-points) within your
  app.
...
an ["Instant play" game](/topic/google-play-instant/instant-play-games),
...
* Don't share the value of [`myUid()`](/reference/android/os/Process#myUid()),
...
* If your app targets Android 8.1 (API level 27) or lower, create a
  [Network Security Config](/training/articles/security-config) file, and set
...
of `2`](#target-sandbox-version), then the user's data is transferred
...
about building your project, see [Build your project](/studio/run/build-for-release).

### Command-line interface

You can also [build the app bundle from the command
line](/studio/build/building-cmdline) using Gradle.
...
* Install onto a local device using [Android Studio](#test-from-studio).
* Install onto a local device using the [command-line
  interface](#test-from-cli).
* Publish to the [internal test track](#test-publish-internal-test-track) on the
  Google Play Console.
...
2. In Android Studio, from the installation options that appear on the
   **General** tab of the [Run/Debug Configurations
   dialog](/studio/run/rundebugconfig#opening), enable the **Deploy as instant
   app** checkbox.
...
1. Upload your app bundle by following the steps in the [Upload your app bundle
   to the Play Console](/studio/publish/upload-bundle) guide.
...
**Note:** The [Designed for Families program](/google-play/guides/families) is not
...
1. If you haven't already, [sign your app bundle with a release
   key](/studio/publish/app-signing#sign-apk) and [upload the app bundle to the
   Play Console](/studio/publish/upload-bundle).
...
[The Android App Bundle Format](/guide/app-bundle/build#aab_format)
