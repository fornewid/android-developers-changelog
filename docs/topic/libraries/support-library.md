---
title: Support Library  |  Android Developers
...
# Support Library Stay organized with collections Save and categorize content based on your preferences.
...
* **Backward Compatibility for newer APIs** - A large amount of the
  ...
  classes and methods. For example, the `Fragment`
...
* **Convenience and Helper Classes** - The support libraries provides a
  ...
  example the `RecyclerView` class provides a user
...
* **Debugging and Utilities** - There are a number of features
  ...
  the [`support-annotations`](/studio/write/annotations)
  library for improved code lint checks on method inputs and [Multidex support](/studio/build/multidex) for
...
* **Compatibility for Related Library Features** - More sophisticated
  ...
  dependencies. For example, the `ViewPager`
  support class should be used with `FragmentPagerAdapter` or the `FragmentStatePagerAdapter` support classes.
* **General Device Compatibility** - If you do not have a specific
  ...
  For example, you may want to use `ActivityCompat` in place of the framework `Activity` class, so you can take advantage of newer features
