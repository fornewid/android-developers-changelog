---
title: App Architecture: Data Layer - DataStore - Android Developers  |  App architecture
...
# DataStore   Part of [Android Jetpack](/jetpack).
...
If you're using [`SharedPreferences`](/reference/kotlin/android/content/SharedPreferences) to store data, consider migrating to
DataStore instead.

**Note:** If you need to support large or complex datasets, partial updates, or
referential integrity, consider using [Room](/training/data-storage/room) instead of DataStore. DataStore
is ideal for small datasets and does not support partial updates or referential
integrity.

## DataStore API

The [`DataStore`](/reference/kotlin/androidx/datastore/core/DataStore) interface provides the following API:
...
not provide type safety. It has a [`SharedPreferences`](/reference/kotlin/android/content/SharedPreferences)-like API but doesn't
...
3. **Do not mix usages of `SingleProcessDataStore` and `MultiProcessDataStore`**
   ...
   process, you must use [`MultiProcessDataStore`](#multiprocess).
...
**Note:** The class for your stored objects is generated at compile time from the
message defined in the proto file. Make sure you rebuild your project.
...
The Preferences DataStore implementation uses the [`DataStore`](/reference/kotlin/androidx/datastore/core/DataStore) and
[`Preferences`](/reference/kotlin/androidx/datastore/preferences/core/Preferences) classes to persist key-value pairs to disk. Use the
property delegate created by [preferencesDataStore](/reference/kotlin/androidx/datastore/preferences/package-summary#dataStore) to create an instance
...
Alternatively, use [RxPreferenceDataStoreBuilder](/reference/kotlin/androidx/datastore/rxjava2/RxDataStoreBuilder) if you're using RxJava.
...
a key for an int value, use [`intPreferencesKey()`](/reference/kotlin/androidx/datastore/preferences/core/package-summary#intPreferencesKey(kotlin.String)). Then, use the
[DataStore.data](/reference/kotlin/androidx/datastore/core/DataStore#data) property to expose the appropriate stored value using a
Flow.
...
DataStore provides an [updateData()](/reference/kotlin/androidx/datastore/core/DataStore#updatedata) function that transactionally updates a
...
**Note:** You can also use the `[edit][11]` suspend function. This function
provides a `MutablePreferences` object that you can modify.
...
**Caution:** Avoid blocking threads on DataStore data reads whenever possible.
Blocking the UI thread can cause [ANRs](/topic/performance/vitals/anr) or unresponsive UI, and blocking
other threads can result in [deadlock](https://en.wikipedia.org/wiki/Deadlock).
...
**Important:** To run the service in a different process, use the android:process
attribute. Note that the process ID is prefixed with a colon (':'). This makes
the service run in a new process, private to the application.
...
You can use [Hilt](/training/dependency-injection/hilt-android) dependency injection so that your DataStore
