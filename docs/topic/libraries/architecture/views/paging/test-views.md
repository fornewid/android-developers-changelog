---
title: Test your Paging implementation (Views)  |  Android Developers
...
# Test your Paging implementation (Views) Stay organized with collections Save and categorize content based on your preferences.





[Concepts and Jetpack Compose implementationarrow\_forward](/topic/libraries/architecture/paging/test)
...
[`PagingSource`](/reference/kotlin/androidx/paging/PagingSource) and [`RemoteMediator`](/reference/kotlin/androidx/paging/RemoteMediator) to ensure that they work as
...
This guide explains how to test the Paging library in the [data layer](/topic/architecture#data-layer) of
...
the data from your data sources appropriately. Provide [fake](/training/testing/fundamentals/test-doubles) versions of
...
function returns the correct [`MediatorResult`](/reference/kotlin/androidx/paging/RemoteMediator.MediatorResult). Tests for side effects, such
...
the [PagingSource tests](/topic/libraries/architecture/paging/test#pagingsource-tests) section of [Test your Paging implementation](/topic/libraries/architecture/paging/test).
Providing a mock version of the Room database is very involved, so it can be
easier to provide an [in-memory implementation](/training/data-storage/room/testing-db#android) of the database instead of a
full mock version. Because creating a Room database requires a [`Context`](/reference/android/content/Context)
...
* The first case is when `mockApi` returns valid data. The `load()` function
  should return `MediatorResult.Success`, and the `endOfPaginationReached`
  property should be `false`.
* The second case is when `mockApi` returns a successful response, but the
  returned data is empty. The `load` function should return
  `MediatorResult.Success`, and the `endOfPaginationReached` property should
  be `true`.
...
implementation](/training/dependency-injection#di-alternatives) to provide and change dependencies as needed. In larger
apps, using a dependency injection library like [Hilt](/training/dependency-injection/hilt-android) can help manage
