...
it in a [lazy list](https://developer.android.com/develop/ui/compose/lists).
...
- If the `PagingSource` is no longer valid and should be replaced by a new instance (for example, due to an underlying data change), return a `LoadResult.Invalid` object.
...
in the UI, see [Manage and present loading states](https://developer.android.com/topic/libraries/architecture/paging/load-state).
...
> [!NOTE]
> **Note:** To support placeholders, make sure to set `PagingConfig.enablePlaceholders` to true. Custom paging sources must also implement `nextKey` and `prevKey` in the returned `LoadResult.Page`.
...
> [!CAUTION]
> **Caution:** When collecting the paging flow with `collectAsLazyPagingItems`, the collection runs in the composition scope. Don't call suspending collection from the wrong scope. The recommended approach is to use `collectAsLazyPagingItems` in a composable.
...
- [Load and display paged data (Views)](https://developer.android.com/topic/libraries/architecture/views/paging/v3-paged-data-views)
