...
**Objective:** This guide explains the basics of ViewModels, how they fit into
[Modern Android Development](/modern-android-development), and how you can implement them in your app.

For more information on state holders, see the [state holders](/topic/architecture/ui-layer/stateholders) guidance.
Similarly, for more information on the UI layer generally, see the [UI layer](/topic/architecture/ui-layer)
guidance.
...
**Note:** ViewModel fully supports integration with [Jetpack Compose](/compose)
and other key Jetpack libraries such as [Hilt](/training/dependency-injection/hilt-android) and [Navigation](/guide/navigation).
...
the [`rememberViewModelStoreOwner`](/reference/kotlin/androidx/lifecycle/viewmodel/compose/rememberViewModelStoreOwner.composable) API.
Your ViewModel is then scoped to the [Lifecycle](/reference/androidx/lifecycle/Lifecycle) of the
...
[`ComponentActivity`](/reference/androidx/activity/ComponentActivity) and [`NavBackStackEntry`](/reference/androidx/navigation/NavBackStackEntry).
For a full list of indirect subclasses, see the
[`ViewModelStoreOwner` reference](/reference/kotlin/androidx/lifecycle/ViewModelStoreOwner). To scope ViewModels to individual items
...
For more information, see the [ViewModel lifecycle](#lifecycle) section that follows,
[ViewModel Scoping APIs](/topic/libraries/architecture/viewmodel/viewmodel-apis#vm-api-composable),
and the guide on [state hoisting](/jetpack/compose/state-hoisting#viewmodels-as-state-owner) for Jetpack Compose.

#### SavedStateHandle

[SavedStateHandle](/topic/libraries/architecture/viewmodel/viewmodel-savedstate) lets you persist data not just through configuration
...
see [Save UI state in Compose](/develop/ui/compose/state-saving).
...
of users sits with the ViewModel, not an Activity directly.
...
The lifecycle of a [`ViewModel`](/reference/androidx/lifecycle/ViewModel) is tied directly to its scope. A `ViewModel`
remains in memory until the [`ViewModelStoreOwner`](/reference/kotlin/androidx/lifecycle/ViewModelStoreOwner) to which it is scoped
...
* In the case of a composable, when it exits the Composition.
  You can use `rememberViewModelStoreOwner` to scope a ViewModel directly to an
  arbitrary part of your UI (like a `Pager` or `LazyList`).
...
[`ViewModel`](/reference/androidx/lifecycle/ViewModel) next to the associated activity lifecycle. This particular
diagram illustrates the states of an activity.

![Illustrates the lifecycle of a ViewModel as an activity changes state.](/static/images/topic/libraries/architecture/viewmodel-lifecycle.png)


**Figure 1.** Lifecycle states of an activity and a ViewModel.

You usually request a [`ViewModel`](/reference/androidx/lifecycle/ViewModel) the first time the system calls an
activity object's [`onCreate()`](/reference/android/app/Activity#onCreate(android.os.Bundle)) method. The system may call
[`onCreate()`](/reference/android/app/Activity#onCreate(android.os.Bundle)) several times throughout the existence of an activity, such
as when a device screen is rotated. The [`ViewModel`](/reference/androidx/lifecycle/ViewModel) exists from when you
first request a [`ViewModel`](/reference/androidx/lifecycle/ViewModel) until the activity is finished and destroyed.

### Clearing ViewModel dependencies

The ViewModel calls the [`onCleared`](/reference/androidx/lifecycle/ViewModel#onCleared()) method when the `ViewModelStoreOwner`
...
The following example shows an alternative to [`viewModelScope`](/topic/libraries/architecture/coroutines#viewmodelscope).
...
of `viewModelScope` for [easier testing](/kotlin/coroutines/test), the ViewModel can receive a
...
From lifecycle [version 2.5](/jetpack/androidx/releases/lifecycle#version_25_2) and above,
...
* Because of [their scoping](/topic/libraries/architecture/viewmodel#lifecycle), use ViewModels as implementation details of a
  ...
  ViewModelStoreOwner unless you use an explicit view model key per chip.
* ViewModels shouldn't know about the UI implementation details. Keep the names
  ...
  UI: a mobile phone, foldable, tablet, or even a Chromebook!
* As they can potentially live longer than the `ViewModelStoreOwner`, ViewModels
  shouldn't hold any references of lifecycle-related APIs such as the `Context`
  or `Resources` to prevent memory leaks.
* Don't pass ViewModels to other classes, functions or other UI components.
  Because the platform manages them, you should keep them as close to it as you
  can—close to your Activity, screen level composable function, or
...
to load the data. The purpose of [`ViewModel`](/reference/androidx/lifecycle/ViewModel) is to encapsulate the data for
...
[Saved UI States](/topic/libraries/architecture/saving-states).

The [Guide to Android App Architecture](/topic/libraries/architecture/guide#fetching_data) suggests building a repository class
