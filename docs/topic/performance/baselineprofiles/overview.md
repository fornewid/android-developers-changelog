---
title: Baseline Profiles overview  |  App quality  |  Android Developers
...
launch by avoiding interpretation and [just-in-time (JIT)](/about/versions/nougat/android-7.0#jit_aot) compilation steps
...
performance impacts business metrics in stories from [Josh](/stories/apps/josh), [Lyft](/stories/apps/lyft),
[TikTok](/stories/apps/tiktok), and [Zomato](/stories/apps/zomato).
...
on [Cloud Profiles](#cloud-profiles) alone.
...
Profiles, see [Compare Baseline Profiles and Startup Profiles](#compare-baseline-startup).
...
**Note:** We recommend using both Baseline Profiles and startup profiles to fully
optimize app startup.

## Get started

To start optimizing performance in your existing app, see [Create Baseline
Profiles](/about/versions/nougat/android-7.0#jit_aot).
...
Plugin](/topic/performance/baselineprofiles/create-baselineprofile)
...
optimization](/topic/performance/baselineprofiles/dex-layout-optimizations)
...
| 8.3 | * **Partial source set directory support (library modules):**   Declare variant-aware Baseline Profile files, such as   `src/free/generated/baselineProfiles/baseline-prof.txt`,   for library modules. * Baseline Profiles include   [desugared classes](/studio/write/java8-support#library-desugaring). |
| 8.2 | * **R8 rewriting of rules:** D8 and R8 can transform the   human-readable Baseline and Startup Profile rules to fully capture   all the rules you need to optimize app performance. This lets you   generate profiles from an unminified build and apply them to a   minified release build. Increases Baseline Profile coverage of   methods by ~30% and increases app performance by ~15%. * [**Startup   Profiles:**](/topic/performance/baselineprofiles/dex-layout-optimizations) generate this type of Baseline Profile to   inform layout of code within DEX. Increases startup performance by   an additional ~15%, or significantly more for large apps. |
...
journeys, see [Write automated tests with UI Automator](/about/versions/nougat/android-7.0#jit_aot).
...
**Note:** To learn how to use `testTag()` for accessing UI elements in Jetpack
Compose, see [Interoperability with UiAutomator](/jetpack/compose/testing#uiautomator-interop).
...
Jetpack Compose performance](/jetpack/compose/performance#use-baseline).
...
![](/static/topic/performance/images/benchmark_images/baselineprofile_workflow.png)


**Figure 1.** This diagram demonstrates the Baseline Profile
...
arrow\_forward](/topic/performance/baselineprofiles/create-baselineprofile)