---
title: Debug Baseline Profiles  |  App quality  |  Android Developers
...
![Add a custom run configuration to create Baseline Profiles in Now in Android](/static/topic/performance/images/studio/run-config-nowinandroid-baselineprofiles.png)
...
3. Confirm that the `baseline.prof` file exists:
   ...
   * If you're inspecting an APK, the profile is at
     ...
     ![Check for a Baseline Profile using APK Analyzer in Android Studio](/static/topic/performance/images/studio/baseline-profile-in-apk.png) 

     **Figure 2.** Check for a Baseline Profile
...
using other tools, the Jetpack [`ProfileInstaller`](/jetpack/androidx/releases/profileinstaller) library is responsible
...
you might need to [force compilation of Baseline Profiles](#force-compilation).
[`ProfileVerifier`](/reference/androidx/profileinstaller/ProfileVerifier) lets you query the status of the profile installation and
...
more information, see [Minimum recommended stable versions](/topic/performance/baselineprofiles/overview#recommended-versions).
...
:   No profile was installed for this app when installing the app through app
    ...
    installer did not run due to [`ProfileInstallerInitializer`](/reference/androidx/profileinstaller/ProfileInstallerInitializer) being disabled.
...
:   A profile is found in the APK or AAB and is enqueued for compilation. When a
    ...
    Baseline Profiles](#force-compilation). This error won't occur when app is installed from Play
...
`RESULT_CODE_ERROR_PACKAGE_NAME_DOES_NOT_EXIST`
:   A [`PackageManager.NameNotFoundException`](/reference/android/content/pm/PackageManager.NameNotFoundException) is thrown when querying the
    [`PackageManager`](/reference/android/content/pm/PackageManager) for the app package. This should rarely happen. Try
...
compilation using [`adb`](/tools/adb):
...
2. Search the file for the `dexFiles` array, which lists the generated DEX
   files.
...
3. Inspect the contents of this DEX file. You should be able to verify that the
...
well-defined. The two key metrics are [time to initial display (TTID)](/topic/performance/vitals/launch-time#time-initial) and
...
state. You can do this by calling [`reportFullyDrawn()`](/reference/androidx/activity/ComponentActivity#reportFullyDrawn()), or
[`ReportDrawn`](/reference/kotlin/androidx/activity/compose/ReportDrawn.composable#ReportDrawn()) if you're using Jetpack Compose. If you have multiple
background tasks that all need to complete before the app is considered fully
drawn, then you can use [`FullyDrawnReporter`](/reference/androidx/activity/FullyDrawnReporter), as described in [Improve
...
* **Local benchmarking**: Locally benchmark your app with and without the
  ...
  Profiles](/topic/performance/baselineprofiles/overview#cloud-profiles) from ART that are present in production devices.