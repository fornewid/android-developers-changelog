...
![Add a custom run configuration to create Baseline Profiles in Now in Android](https://developer.android.com/static/topic/performance/images/studio/run-config-nowinandroid-baselineprofiles.png) **Figure 1.** Add a custom run configuration to create Baseline Profiles in Now in Android.

> [!NOTE]
> **Note:** Creating a custom run configuration isn't necessary when you add Baseline Profiles using the Baseline Profile module wizard in Android Studio, which is recommended. The wizard creates a run configuration for you.
...
[`ReportDrawn`](https://developer.android.com/reference/kotlin/androidx/activity/compose/ReportDrawn.composable#ReportDrawn()) if you're using Jetpack Compose. If you have multiple
