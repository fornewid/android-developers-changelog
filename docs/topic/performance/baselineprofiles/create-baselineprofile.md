---
title: Create Baseline Profiles  |  App quality  |  Android Developers
...
[benchmark](/topic/performance/baselineprofiles/measure-baselineprofile)
...
2. Select the **Baseline Profile Generator** template in the
   **Templates** panel and configure it:
   ![](/static/topic/performance/images/studio/baseline-profile-generator-module-template.png)
   ...
   * **Module name**: the name you want for the Baseline Profile module
     being created.
   * **Package name**: the package name you want for the Baseline Profile
     module.
   * **Language**: whether you want the generated code to be Kotlin or
     Java.
   * **Build configuration language**: whether you want to use Kotlin
     Script (KTS) or Groovy for your build configuration scripts.
   * **Use Gradle-managed device**: if you're using
     [Gradle-managed devices](/studio/test/gradle-managed-devices) to
     test your app.
3. Click **Finish** and the new module is created. If you are using source
...
[Startup Profile](/topic/performance/baselineprofiles/dex-layout-optimizations).
...
`:app:generateVariantBaselineProfile` Gradle tasks.

![The Generate Baseline Profile run configuration.](/static/topic/performance/images/studio/generate-baseline-profile.png)


**Figure 2.** Running this configuration generates the Baseline
Profile.
...
[Baseline Profile module template](#create-new-profile), use the
...
1. [Set
   up a Macrobenchmark module](/topic/performance/benchmarking/macrobenchmark-overview#project-setup) in your Gradle project.
...
4. To generate the Baseline Profile, run the
   ...
   [Gradle Managed Device](/studio/test/gradle-managed-devices).
...
[Baseline Profile module template](#create-new-profile)
(preferred) or [Macrobenchmark template](#create-new-profile-8-1), but you can
...
[Baseline Profile module template](/topic/performance/create-baselineprofile#create-new-profile).
...
2. Configure the `build.gradle.kts` file for
   ...
   2. Ensure the `targetProjectPath` points to the
      `:app` module.
   3. Optionally, add a
      [Gradle-managed device (GMD)](/studio/test/gradle-managed-devices).
      In the following example, it's `pixel6Api31`. If not specified,
      the plugin uses a connected device, either emulated or physical.
   4. Apply the configuration you want, as shown in the following
      example.
...
as the [steps for AGP 8.1](#create-new-profile-8-1), with the following
exceptions:
...
* You must [manually apply the generated Baseline Profile rules to your code](#apply-rules).
...
2. Copy the generated profile to `src/main/` and rename the file to
   ...
   **Note:** If you're using a version of the Android Gradle plugin earlier than
   ...
   Android Studio.
3. Add a dependency to the [ProfileInstaller library](/jetpack/androidx/releases/profileinstaller)
   in your app's `build.gradle.kts` file to enable local Baseline Profile
   compilation where [Cloud Profiles](/topic/performance/baselineprofiles/overview#cloud-profiles) aren't available. This is
...
class](/topic/performance/benchmarking/macrobenchmark-overview#create-macrobenchmark)
and [Automate measurement with the Macrobenchmark
library](/topic/performance/baselineprofiles/measure-baselineprofile).

![](/static/topic/performance/images/studio/gutter-action.png)


**Figure 3.** Run Android Tests from the gutter
action.
...
[`reportFullyDrawn()`](/reference/androidx/activity/ComponentActivity#reportFullyDrawn())
method of the
...
list such as a [`RecyclerView`](/develop/ui/views/layout/recyclerview) or [lazy
list](/jetpack/compose/lists#lazy), the list might be populated by a background
...
[`getFullyDrawnReporter()`](/reference/androidx/activity/ComponentActivity#getFullyDrawnReporter())
...
If your app uses [Jetpack Compose](/jetpack/compose), use the following APIs to
indicate fully drawn state:

* [`ReportDrawn`](/reference/kotlin/androidx/activity/compose/ReportDrawn.composable#ReportDrawn())
  indicates that your composable is immediately ready for interaction.
* [`ReportDrawnWhen`](/reference/kotlin/androidx/activity/compose/ReportDrawnWhen.composable#ReportDrawnWhen(kotlin.Function0))
  takes a predicate, such as `list.count > 0`, to indicate when your
  composable is ready for interaction.
* [`ReportDrawnAfter`](/reference/kotlin/androidx/activity/compose/ReportDrawnAfter.composable#ReportDrawnAfter(kotlin.coroutines.SuspendFunction0))
...
Overview](/topic/performance/baselineprofiles/overview)

[Next

Create Baseline Profiles for a library

arrow\_forward](/topic/performance/baselineprofiles/create-baselineprofile-library)