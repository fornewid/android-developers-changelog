---
title: Managing Bitmap Memory  |  App quality  |  Android Developers
...
# Managing Bitmap Memory Stay organized with collections Save and categorize content based on your preferences.
...
In addition to the steps described in [Caching Bitmaps](/topic/performance/graphics/cache-bitmap),
...
* On Android 2.2 (API level 8) and lower, when garbage
...
* On Android 2.3.3 (API level 10) and lower, the backing pixel data for a
  ...
  to briefly exceed its memory limits and crash. From Android 3.0 (API level 11)
  through Android 7.1 (API level 25), the pixel data is stored on the
  Dalvik heap along with the associated bitmap. In Android 8.0 (API level 26),
  and higher, the bitmap pixel data is stored in the native heap.
