---
title: Practical performance debugging example: ANR  |  App quality  |  Android Developers
...
![Perfetto UI navigation to app main thread.](/static/topic/performance/images/tracing/debugging-anr-example-main-thread.png)


**Figure 1.** Navigation to app's main thread.
...
![Perfetto UI showing the end of a trace, highlighting the ANR trigger
 location.](/static/topic/performance/images/tracing/debugging-anr-example-anr-trigger.png)


**Figure 2.** ANR trigger location.
...
![Perfetto UI showing the CPU time consumed by the handleNetworkResponse
 execution at the time of the ANR.](/static/topic/performance/images/tracing/debugging-anr-example-execution-at-trigger.png)


**Figure 3.** Execution at the time of ANR.
...
**Tip:** We recommend you to [add custom trace slices](/topic/performance/tracing/custom-events) to identify long running
operations and to identify important user journeys.
...
![Perfetto UI showing last frame rendered by UI thread prior the ANR.](/static/topic/performance/images/tracing/debugging-anr-example-last-frame-pre-anr.png)


**Figure 4.** Last app frame generated before the ANR.
...
![Perfetto UI showing thread states during an operation, highlighting 75%
 sleeping and 24% running time.](/static/topic/performance/images/tracing/debugging-anr-example-blockingop-thread-states.png)


**Figure 5.** Thread states during `MyApp:SubmitButton` operation.
...
![Perfetto UI showing the first sleeping interval within the
 MyAppSubmitButton trace slice.](/static/topic/performance/images/tracing/debugging-anr-example-first-sleep.png)


**Figure 6.** First sleeping time within `MyAppSubmitButton`.


![Perfetto UI showing the second sleeping interval within the
 MyAppSubmitButton trace slice.](/static/topic/performance/images/tracing/debugging-anr-example-second-sleep.png)


**Figure 7.** Second sleeping time within `MyAppSubmitButton`.


![Perfetto UI showing the third sleeping interval within the
 MyAppSubmitButton trace slice.](/static/topic/performance/images/tracing/debugging-anr-example-third-sleep.png)


**Figure 8.** Third sleeping time within `MyAppSubmitButton`.


![Perfetto UI showing the fourth sleeping interval within the
 MyAppSubmitButton trace slice.](/static/topic/performance/images/tracing/debugging-anr-example-fourth-sleep.png)


**Figure 9.** Fourth sleeping time within `MyAppSubmitButton`.
...
![Perfetto UI showing a summary of delays during the MyApp:SubmitButton
 trace slice, indicating multiple 2-second sleep intervals.](/static/topic/performance/images/tracing/debugging-anr-example-delay-summary.png)


**Figure 10.** Summary of delays during `MyApp:SubmitButton` slice.
...
**Note:** When trace analysis leads to a hypothesis (for example, "this is a network
timeout") but the trace data is insufficient to prove it, use an iterative
approach. Add new trace slices around the suspected logic and collect new field
