---
title: Background optimization  |  App quality  |  Android Developers
...
* Apps cannot send or receive `ACTION_NEW_PICTURE` or `ACTION_NEW_VIDEO` broadcasts. This optimization
...
the new [WorkManager](/arch/work) provide robust mechanisms to schedule network
...
* **Optimized (default):** Optimize an app's ability to perform background work,
  based on how the user interacts with the app.
* **Restricted:** Fully prevents an app from running in the background. Apps
  may not work as expected.

**Note:** When a user places your app in the "restricted" state but later launches
...
vitals](/topic/performance/vitals), the system might prompt the user to restrict
...
* Excessive background services: If app targets API levels lower than 26 and has
...
specified `JobService.class`. To see more examples of `JobScheduler` implementation, see the [JobScheduler sample app](https://github.com/googlearchive/android-JobScheduler/tree/master/kotlinApp#readme).
...
more, see [WorkManager](/arch/work).
...
:   Passes a `TriggerContentUri` object to `JobInfo`. A `ContentObserver`
...
:   Add the `TriggerContentUri.FLAG_NOTIFY_FOR_DESCENDANTS` flag to
    ...
    corresponds to the `notifyForDescendants` parameter passed to `registerContentObserver()`.
...
The following [Android Debug Bridge (ADB)](/tools/help/adb)
commands can help you test app behavior with background processes disabled:

* To simulate conditions where implicit broadcasts and background services
  are unavailable, enter the following command:
* ```
  $ adb shell cmd appops set <package_name> RUN_IN_BACKGROUND ignore
  ```
* To re-enable implicit broadcasts and background services, enter the
  following command:
* ```
  $ adb shell cmd appops set <package_name> RUN_IN_BACKGROUND allow
  ```
* You can simulate the user placing your app in the "restricted" state for
