...
need for these implicit broadcasts. For example, `https://developer.android.com/reference/android/app/job/JobScheduler` and
...
network, are met. You can now also use `https://developer.android.com/reference/android/app/job/JobScheduler`
to react to changes to content providers. `https://developer.android.com/reference/android/app/job/JobInfo`
objects encapsulate the parameters that `https://developer.android.com/reference/android/app/job/JobScheduler`
uses to schedule your job. When the conditions of the job are met, the system
executes this job on your app's `https://developer.android.com/reference/android/app/job/JobService`.


On this page, we will learn how to use alternative methods, such as
`https://developer.android.com/reference/android/app/job/JobScheduler`, to adapt your app to these new
restrictions.
...
Apps targeting Android 7.0 (API level 24) do not receive `https://developer.android.com/reference/android/net/ConnectivityManager#CONNECTIVITY_ACTION` broadcasts if they
...
**Note:** A `https://developer.android.com/reference/android/content/BroadcastReceiver` registered with
`https://developer.android.com/reference/android/content/Context#registerReceiver(android.content.BroadcastReceiver, android.content.IntentFilter)`
continues to receive these broadcasts while the app is running.

### Schedule network jobs on unmetered connections


When using the `https://developer.android.com/reference/android/app/job/JobInfo.Builder` class
to build your `https://developer.android.com/reference/android/app/job/JobInfo` object, apply the `https://developer.android.com/reference/android/app/job/JobInfo.Builder#setRequiredNetworkType(int)` method and pass `https://developer.android.com/reference/android/app/job/JobInfo` as a job parameter. The following code sample
...
the `https://developer.android.com/reference/android/app/job/JobService#onStartJob(android.app.job.JobParameters)` method in the
specified `JobService.class`. To see more examples of `https://developer.android.com/reference/android/app/job/JobScheduler` implementation, see the [JobScheduler sample app](https://github.com/googlearchive/android-JobScheduler/tree/master/kotlinApp#readme).
...
registered `https://developer.android.com/reference/android/content/BroadcastReceiver`. However, the `https://developer.android.com/reference/android/net/ConnectivityManager` API provides a more robust method to request
a callback only when specified network conditions are met.


`https://developer.android.com/reference/android/net/NetworkRequest` objects define the parameters of the
network callback in terms of `https://developer.android.com/reference/android/net/NetworkCapabilities`. You
...
then passes the `https://developer.android.com/reference/android/net/NetworkRequest` object to the system. When
the network conditions are met, the app receives a callback to execute the
`https://developer.android.com/reference/android/net/ConnectivityManager.NetworkCallback#onAvailable(android.net.Network)` method defined in its `https://developer.android.com/reference/android/net/ConnectivityManager.NetworkCallback` class.


The app continues to receive callbacks until either the app exits or it calls
`https://developer.android.com/reference/android/net/ConnectivityManager#unregisterNetworkCallback(android.app.PendingIntent)`.

## Restrictions on receiving image and video broadcasts


In Android 7.0 (API level 24), apps are not able to send or receive `https://developer.android.com/reference/android/hardware/Camera#ACTION_NEW_PICTURE` or `https://developer.android.com/reference/android/hardware/Camera#ACTION_NEW_VIDEO` broadcasts. This restriction helps
...
extends `https://developer.android.com/reference/android/app/job/JobInfo` and `https://developer.android.com/reference/android/app/job/JobParameters` to provide an alternative solution.

### Trigger jobs on content URI changes


To trigger jobs on content URI changes, Android 7.0 (API level 24) extends
the `https://developer.android.com/reference/android/app/job/JobInfo` API with the following methods:
...
:
    Passes a `TriggerContentUri` object to `https://developer.android.com/reference/android/app/job/JobInfo`. A `https://developer.android.com/reference/android/database/ContentObserver`
...
:
    ...
    corresponds to the `notifyForDescendants` parameter passed to `https://developer.android.com/reference/android/content/ContentResolver#registerContentObserver(android.net.Uri, boolean, android.database.ContentObserver)`.


**Note:** `TriggerContentUri()` cannot be used in
combination with `https://developer.android.com/reference/android/app/job/JobInfo.Builder#setPeriodic(long)` or `https://developer.android.com/reference/android/app/job/JobInfo.Builder#setPersisted(boolean)`. To continually monitor for content changes, schedule a new
`https://developer.android.com/reference/android/app/job/JobInfo` before the app's `https://developer.android.com/reference/android/app/job/JobService` finishes handling the most recent callback.
...
receives a callback and a `https://developer.android.com/reference/android/app/job/JobParameters` object is
passed to the `https://developer.android.com/reference/android/app/job/JobService#onStartJob(android.app.job.JobParameters)`
method in `MediaContentJob.class`.

### Determine which content authorities triggered a job


Android 7.0 (API level 24) also extends `https://developer.android.com/reference/android/app/job/JobParameters` to
...
The following sample code overrides the `https://developer.android.com/reference/android/app/job/JobService#onStartJob(android.app.job.JobParameters)` method and
