---
title: Adopt optimizations incrementally  |  App quality  |  Android Developers
...
* Obfuscation (or identifier minification): shortens the names of classes and
  methods
...
**Note:** R8 runs in compat mode by default for Android Gradle plugin versions 7.0
and lower.
...
**Note:** Never write a keep rule that keeps all of your app, for example -`keep **`
or -`keep com.myapp.**`. These rules prevent optimization across such a wide
portion of the app that you'll pay the build cost of R8 with effectively none of
the benefit. Instead, always start with library- or package- based keep rules,
and try to remove them gradually over time.
