---
title: Better performance through threading  |  App quality  |  Android Developers
...
# Better performance through threading Stay organized with collections Save and categorize content based on your preferences.
...
process](/guide/components/fundamentals) along with an execution thread. This **main thread,**
...
the [*Application
Not Responding*](/training/articles/perf-anr) (ANR) dialog, allowing the user to close the app directly.
...
`View`; but before the work completes, the `View` is
removed from the view hierarchy. When these two actions happen simultaneously,
the reference keeps the `View` object in memory and sets properties on it.
However, the user never sees
this object, and the app deletes the object once the reference to it is gone.

In another example, `View` objects contain references to the activity
that owns them. If
...
work completes. As a result, there may be two `Activity` objects in
...
not recommended for use in new code. See [Threading on Android](/training/multiple-threads)
...
with [`LiveData`](/reference/androidx/lifecycle/LiveData) allows you to
load data and be notified when it changes
without having to worry about the lifecycle.
[`ViewModel`](/reference/androidx/lifecycle/ViewModel) objects are
...
[ViewModel guide](/topic/libraries/architecture/viewmodel), and to learn more about
LiveData see the [LiveData guide](/topic/libraries/architecture/livedata). If you
...
and the Application Lifecycle](/guide/topics/processes/process-lifecycle), the priority that your app’s threads receive
...
`Thread` and `Process` classes.

## Helper classes for threading

For developers using Kotlin as their primary language, we recommend using [coroutines](/kotlin/coroutines).
...
threading, such as the `Thread`, `Runnable`
, and `Executors` classes,
as well as additional ones such as `HandlerThread`.
For further information, please refer to [Threading on Android](/training/multiple-threads).
...
In this example, when your app delegates the `Camera.open()` command to a
...
When your app creates a thread using `HandlerThread`, don’t
...
factors, `ThreadPoolExecutor` decides on how many
...
[Systrace](/topic/performance/tracing/command-line) is as
