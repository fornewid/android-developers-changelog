...
When the user launches your app, Android creates a new [Linux
process](https://developer.android.com/guide/components/fundamentals) along with an execution thread. This **main thread,**
...
Nearly [any
block of code your app executes](https://www.youtube.com/watch?v=qk5F6Bxqhr4&index=1&list=PLWz5rJ2EKKc9CBxr3BVjPTPoDPLdPIFCE) is tied to an event callback, such as input,
...
order to render smoothly at [60
frames per second](https://www.youtube.com/watch?v=CaMTIgxCSqU&index=62&list=PLWz5rJ2EKKc9CBxr3BVjPTPoDPLdPIFCE). For the system to reach this goal, the UI/View hierarchy
must update on the main thread. However, when the main thread's messaging queue
contains tasks that are either too numerous or too long for the main thread to
complete the update fast enough, the app should move this work to a worker
...
the [*Application
Not Responding*](https://developer.android.com/training/articles/perf-anr) (ANR) dialog, allowing the user to close the app directly.
...
By design, [Android
View objects are not thread-safe](https://www.youtube.com/watch?v=tBHPmQQNiS8&index=3&list=PLWz5rJ2EKKc9CBxr3BVjPTPoDPLdPIFCE). An app is expected to create, use, and
...
`https://developer.android.com/reference/android/view/View`; but before the work completes, the `https://developer.android.com/reference/android/view/View` is
removed from the view hierarchy. When these two actions happen simultaneously,
the reference keeps the `https://developer.android.com/reference/android/view/View` object in memory and sets properties on it.
However, the user never sees
this object, and the app deletes the object once the reference to it is gone.


In another example, `https://developer.android.com/reference/android/view/View` objects contain references to the activity
that owns them. If
...
work completes. As a result, there may be two `https://developer.android.com/reference/android/app/Activity` objects in
...
in Kotlin). This declaration creates an implicit reference to the enclosing `https://developer.android.com/reference/android/app/Activity`
...
instances either as static classes, or in their own files, thus removing
the implicit reference.


Another solution would be to always cancel and clean up background tasks in the appropriate
`Activity` lifecycle callback, such as `onDestroy`. This approach can be
tedious and error prone, however. As a general rule, you should not put complex, non-UI logic
directly in activities. In addition, `AsyncTask` is now deprecated and it is
not recommended for use in new code. See [Threading on Android](https://developer.android.com/training/multiple-threads)
...
activities, although they will be terminated together with the application process once there are no
more active application components. In some cases, this persistence is desirable.
...
`https://developer.android.com/reference/android/view/View` object. Although the object no longer exists, the acts of downloading and
...
with [`LiveData`](https://developer.android.com/reference/androidx/lifecycle/LiveData) allows you to
load data and be notified when it changes
without having to worry about the lifecycle.
[`ViewModel`](https://developer.android.com/reference/androidx/lifecycle/ViewModel) objects are
...
[ViewModel guide](https://developer.android.com/topic/libraries/architecture/viewmodel), and to learn more about
LiveData see the [LiveData guide](https://developer.android.com/topic/libraries/architecture/livedata). If you
...
As described in [Processes
and the Application Lifecycle](https://developer.android.com/guide/topics/processes/process-lifecycle), the priority that your app's threads receive
...
`https://developer.android.com/reference/android/os/Process#setThreadPriority(int, int)`.
The system's thread
...
in the [foreground
group get about 95%](https://www.youtube.com/watch?v=NwFXVsM15Co&list=PLWz5rJ2EKKc9CBxr3BVjPTPoDPLdPIFCE&index=9) of the total execution time from the device, while the
background group gets roughly 5%.


The system also assigns each thread its own priority value, using the
`https://developer.android.com/reference/android/os/Process` class.
...
The `https://developer.android.com/reference/android/os/Process`
...
`https://developer.android.com/reference/android/os/Process#THREAD_PRIORITY_DEFAULT`
...
Your app can use the `https://developer.android.com/reference/android/os/Process#THREAD_PRIORITY_LESS_FAVORABLE`
and `https://developer.android.com/reference/android/os/Process#THREAD_PRIORITY_MORE_FAVORABLE`
constants as incrementers to set relative priorities. For a list of
thread priorities, see the
`https://developer.android.com/reference/android/os/Process#THREAD_PRIORITY_AUDIO` constants in
the `https://developer.android.com/reference/android/os/Process` class.

For more information on
managing threads, see the reference documentation about the
`https://developer.android.com/reference/java/lang/Thread` and `https://developer.android.com/reference/android/os/Process` classes.

## Helper classes for threading


For developers using Kotlin as their primary language, we recommend using [coroutines](https://developer.android.com/kotlin/coroutines).
Coroutines provide a number of benefits, including writing async code without callbacks as well as
structured concurrency for scoping, cancellation and error handling.


The framework also provides the same Java classes and primitives to facilitate
threading, such as the `https://developer.android.com/reference/java/lang/Thread`, `https://developer.android.com/reference/java/lang/Runnable`
, and `https://developer.android.com/reference/java/util/concurrent/Executors` classes,
as well as additional ones such as `https://developer.android.com/reference/android/os/HandlerThread`.
For further information, please refer to [Threading on Android](https://developer.android.com/training/multiple-threads).

### The HandlerThread class


A handler thread is effectively a long-running thread that grabs work from a queue and operates
on it.


Consider a common challenge with getting preview frames from your
`https://developer.android.com/reference/android/hardware/Camera` object.
When you register for Camera preview frames, you receive them in the
`https://developer.android.com/reference/android/hardware/Camera.PreviewCallback#onPreviewFrame(byte[], android.hardware.Camera)`
...
In this example, when your app delegates the `https://developer.android.com/reference/android/hardware/Camera#open()` command to a
...
When your app creates a thread using `https://developer.android.com/reference/android/os/HandlerThread`, don't
forget to set the thread's
[priority based on the type of work it's doing](https://www.youtube.com/watch?v=NwFXVsM15Co&index=9&list=PLWz5rJ2EKKc9CBxr3BVjPTPoDPLdPIFCE). Remember, CPUs can only
...
creates, `HandlerThread` isn't the appropriate class to use.


`https://developer.android.com/reference/java/util/concurrent/ThreadPoolExecutor` is a helper class to make
...
constructs a `https://developer.android.com/reference/java/util/concurrent/ThreadPoolExecutor`
object, the app sets a minimum and maximum
number of threads. As the workload given to the
`https://developer.android.com/reference/java/util/concurrent/ThreadPoolExecutor` increases,
...
factors, `https://developer.android.com/reference/java/util/concurrent/ThreadPoolExecutor` decides on how many
...
threads, doing so can create performance issues. Your app shares limited CPU
resources with background services, the renderer, audio engine,
networking, and more. CPUs really only have the
ability to handle a small number of threads in parallel; everything above that runs
[into priority and scheduling issue](https://www.youtube.com/watch?v=NwFXVsM15Co&list=PLWz5rJ2EKKc9CBxr3BVjPTPoDPLdPIFCE&index=9).
As such, it's important to only create
as many threads as your workload needs.


Practically speaking, there's a number of variables responsible for this, but
picking a value (like 4, for starters), and testing it with
[Systrace](https://developer.android.com/topic/performance/tracing/command-line) is as
