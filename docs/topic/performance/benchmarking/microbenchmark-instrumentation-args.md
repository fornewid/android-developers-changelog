---
title: Microbenchmark instrumentation arguments  |  App quality  |  Android Developers
...
1. Edit the run configuration by clicking **Edit** and selecting the
   configuration you want to edit.
   ![](/static/topic/performance/images/benchmark_images/bench_instr_arg_1.png)


   **Figure 1.** Edit the run configuration.
2. Edit instrumentation arguments by clicking
   more\_horiz next to the **Instrumentation
   arguments** field.
   ![](/static/topic/performance/images/benchmark_images/bench_instr_arg_2.png)


   **Figure 2.** Edit the instrumentation argument.
3. Click add and add the required
   instrumentation argument.
   ![](/static/topic/performance/images/benchmark_images/bench_instr_arg_3.png)
...
Counts the CPU events specified in `androidx.benchmark.cpuEventCounter.events`.
...
* [Configuration errors](/topic/performance/benchmarking/microbenchmark-write#configuration-errors) aren't enforced (for example, to make it
  easier to run with regular correctness tests on emulators)
...
Configures how many measurements occur. This does not directly define the number
of loops executed because each measurement typically runs many loops, defined
dynamically based on run time during warmup.
...
By default, Microbenchmark kills any existing Perfetto (System Trace) recordings when
...
* Available options:
  ...
  + `StackSampling`
  + `None`
* **Defaults to:** Safe version of `MethodTracing` which only captures a
  method trace if the device can do so without affecting measurements.
...
Profile a benchmark](/topic/performance/benchmarking/microbenchmark-profile)

[Next

Microbenchmark and Hilt

arrow\_forward](/topic/performance/benchmarking/microbenchmark-and-hilt)