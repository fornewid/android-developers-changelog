...
To learn how to use the Microbenchmark library by adding changes to your application code, see the [Quickstart](https://developer.android.com/topic/performance/benchmarking/microbenchmark-write#quickstart) section. To learn how to complete a full setup with more complicated changes to your codebase, see the [Full project setup](https://developer.android.com/topic/performance/benchmarking/microbenchmark-write#full-setup) section.
...
1. Add the library to your module's `build.gradle` or `build.gradle.kts` file:
   ...
   Use an `implementation` dependency instead of an `androidTestImplementation` dependency. If you use `androidTestImplementation`, the benchmarks fail to run because the library [manifest](https://developer.android.com/guide/topics/manifest/manifest-intro) isn't merged into the app manifest.
...
3. Change the `testInstrumentationRunner` to be [`AndroidBenchmarkRunner`](https://developer.android.com/reference/kotlin/androidx/benchmark/junit4/AndroidBenchmarkRunner):
   ...
   ```
4. Add an instance of [`BenchmarkRule`](https://developer.android.com/reference/kotlin/androidx/benchmark/junit4/BenchmarkRule) in a test file in the [`androidTest`](https://developer.android.com/studio/test/test-in-android-studio#test_types_and_locations) directory to add your benchmark. For more information about writing benchmarks, see [Create a Microbenchmark class](https://developer.android.com/topic/performance/benchmarking/microbenchmark-write#create-microbenchmark).
...
To set up regular benchmarking rather than one-off benchmarking, isolate benchmarks into their own module. This helps ensure that their configuration, such as setting [`debuggable`](https://developer.android.com/guide/topics/manifest/application-element#debug) to `false`, is separate from regular tests.
...
After the module is created, change its `build.gradle` or `build.gradle.kts` file and add [`androidTestImplementation`](https://developer.android.com/training/testing/instrumented-tests#set-testing) to the module containing code to benchmark:
...
Alternatively, from the command line, run the [`connectedCheck`](https://developer.android.com/studio/test/command-line) to run all of the tests from specified Gradle module:
...
To suppress specific error types as warnings and prevent them from halting the benchmark, pass the error type in a comma-separated list to the instrumentation argument [`androidx.benchmark.suppressErrors`](https://developer.android.com/studio/profile/microbenchmark-instrumentation-args#suppresserrors).
