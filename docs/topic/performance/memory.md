...
of memory management](/topic/performance/memory-overview).
...
object references in static member variables—and release any [`Reference`](/reference/java/lang/ref/Reference)
...
You can [use R8 to reduce your app's memory footprint](/topic/performance/app-optimization/enable-app-optimization). While R8 is
traditionally known for [shrinking APK size](/topic/performance/reduce-apk-size), it has a direct, positive
...
that your app relies on. For more information, see the [Keep rules overview](/topic/performance/app-optimization/keep-rules-overview).
...
* **Global rules to avoid:**
  + `-dontoptimize`: Completely disables optimization for the entire app,
    resulting in larger, slower executables.
  + `-dontshrink`: Prevents the removal of unused code and resources.
  + `-dontobfuscate`: Prevents name minification, missing out on valuable
    memory savings (especially in large apps).
...
For more information about writing keep rules, see the [Keep rules overview](/topic/performance/app-optimization/keep-rules-overview).
For specific patterns to use and avoid, see [Keep rules best practices](/topic/performance/app-optimization/keep-rules-best-practices).
...
rules that block optimization, see [R8 Configuration Analyzer](/topic/performance/app-optimization/r8-configuration-analyzer).
...
While [optimizing your app using R8](/topic/performance/app-optimization/enable-app-optimization) can remove unused code from
...
libraries wisely](/topic/performance/app-optimization/choose-libraries-wisely).
...
using [Hilt](/training/dependency-injection/hilt-android) or [Dagger](https://dagger.dev/). Hilt is a dependency injection library for
...
images](/develop/ui/compose/graphics/images/optimization).

## Monitor available memory and memory usage

You must find your app's memory usage problems before you can fix them. The
Android Studio [memory profiler](/studio/profile/memory-profiler) helps you find and diagnose memory issues
in the following ways:

* [See how your app allocates memory over time.](/studio/profile/inspect-app-live) The memory profiler shows
  a realtime graph of how much memory your app is using, the number of
  allocated Java objects, and when garbage collection occurs.
* Initiate garbage collection events and
  [take a snapshot of the Java heap](/studio/profile/capture-heap-dump) while your app runs.
* [Record your app's memory allocations](/studio/profile/record-java-kotlin-allocations), inspect all allocated objects,
  view the stack trace for each allocation, and jump to the corresponding code
  in the Android Studio editor.
...
workflow. For more information, see the [Android Studio release notes](/studio/preview/features#leakcanary).

There are other tools you can use to diagnose memory issues based on data from
users running your production app:

* [Use Android Vitals to track low memory kill (LMK) events.](#lmk)
* [Use the Profiling Manager](#profiling-triggers) to track out of memory errors, as well as
  anomalous app behavior that might be caused by memory leaks.
...
management](/topic/performance/memory-overview). To further help balance the system memory and avoid the system's
need to stop your app process, you can implement the [`ComponentCallbacks2`](/reference/android/content/ComponentCallbacks2)
interface in your [`Activity`](/reference/android/app/Activity) classes. The provided [`onTrimMemory()`](/reference/android/content/ComponentCallbacks2#onTrimMemory(int))
...
figure by calling [`getMemoryInfo()`](/reference/android/app/ActivityManager#getMemoryInfo(android.app.ActivityManager.MemoryInfo)). This returns an
[`ActivityManager.MemoryInfo`](/reference/android/app/ActivityManager.MemoryInfo) object that provides information about the
...
[`lowMemory`](/reference/android/app/ActivityManager.MemoryInfo#lowMemory), which is a simple boolean that tells you whether the device
...
`ProfilingManager`](/topic/performance/tracing/profiling-manager/overview), an
...
17](/about/versions/17/features#profilingmanager-new-triggers) are especially
useful for spotting memory problems:

* [`TRIGGER_TYPE_OOM`](/reference/android/os/ProfilingTrigger#TRIGGER_TYPE_OOM)
  indicates that the app has thrown an `OutOfMemoryError`. It triggers the
  next time the app starts *after* the crash, when the app registers for
  profiling triggers.
* [`TRIGGER_TYPE_ANOMALY`](/reference/android/os/ProfilingTrigger#TRIGGER_TYPE_ANOMALY)
  triggers when the system detects anomalous behavior from the app. Among
  other things, this can be triggered by excessive memory usage. It triggers
  after the app has exhibited excessive memory usage, and *before* the system
  takes any action to stop the offending process. For example, if the app
  exceeds the [memory limits introduced in Android
  17](/about/versions/17/behavior-changes-all#app-memory-limits),
  `TRIGGER_TYPE_ANOMALY` triggers before the system kills the app.

For more information on using `ProfilingManager` to programmatically register
and retrieve triggers, see the [trigger-based
profiling](/topic/performance/tracing/profiling-manager/trigger-based-capture)
documentation.

You can also [use app-driven
profiling](/topic/performance/tracing/profiling-manager/how-to-capture) to
...
[service](/guide/components/services) to work in the background, don't
...
implementation, such as [`WorkManager`](/reference/androidx/work/WorkManager).
For more information about how to use `WorkManager` to schedule background
processes, see [Persistent work](/guide/background/persistent).
...
[`HashMap`](/reference/java/util/HashMap) implementation can be memory
...
[`SparseArray`](/reference/android/util/SparseArray),
[`SparseBooleanArray`](/reference/android/util/SparseBooleanArray), and
[`LongSparseArray`](/reference/androidx/collection/LongSparseArray). For
...
resource footprint](#reduce-footprint), a larger compiled codebase directly
...
footprint](#reduce-footprint)) and contributes to APK size increase.
...
Or, you might create new [`Paint`](/reference/android/graphics/Paint) or
[`Bitmap`](/reference/android/graphics/Bitmap) objects inside the
...
Use the [Memory Profiler](/studio/profile/memory-profiler) to find the
