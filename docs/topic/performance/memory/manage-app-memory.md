---
title: https://developer.android.com/topic/performance/memory/manage-app-memory
url: https://developer.android.com/topic/performance/memory/manage-app-memory
source: md.txt
---
...
documentation. Firebase Crashlytics [also supports](https://firebase.google.com/support/release-notes/android#crashlytics_v20-1-0) both the
`TRIGGER_TYPE_OOM` and `TRIGGER_TYPE_ANOMALY` triggers, correlating them with
...
### Track process state and memory

In addition to memory callbacks and events, you can explicitly query the state
and priority of your app's processes. This is particularly useful for
understanding how the operating system classifies your app's current
...
#### For debugging with `adb`

When you are debugging memory behavior and process lifecycles, you can also use
`adb` commands to help you investigate your app's [OOM scores](https://developer.android.com/topic/performance/memory-management#low-memory_killer) and memory
footprint in real-time. To monitor your app's processes and memory states, you
can use the following commands:

- **Find active process IDs (PIDs):** First, find all the processes your app is currently running by using the `ps` command filtered by your package name.  
  `shell
  adb shell ps -A | grep your.package.name`
- **Check the current OOM Score:** Use a process's PID to read its OOM score from the process file system. A score of 0-199 typically indicates a foregrounded app, 200-249 represents a perceptible service, 250-899 is for a background service, and scores of 900+ mean the process is cached and likely to be killed.  
  `shell
  adb shell cat /proc/pid/oom_score_adj`
- **Continuously monitor OOM Scores:** To continuously monitor changes to an app's OOM Score (which will be the lowest OOM Score among all its processes), you can use the `watch-uids` command.  
  `shell
  adb shell am watch-uids`
- **Measure memory footprints:** To get a fast reading of a process's core memory footprint, such as [Anonymous RSS and Swap memory usage](https://developer.android.com/topic/performance/vitals/memory-usage), read its status directly from the linux process file system.  
  `shell
  adb shell cat /proc/pid/status | grep -E "VmRSS|RssAnon|VmSwap"`  
  For a more detailed and comprehensive breakdown of the memory allocations, use the `dumpsys meminfo` command:  
  `shell
  adb shell dumpsys meminfo pid`
- **Investigate active components:** If your process is maintaining an unexpectedly high priority (such as staying at an OOM score of `200` instead of dropping to cached), you can inspect exactly what the process is running. The `dumpsys activity processes` command reveals the specific Activities, Services, Providers, and Receivers keeping the process alive.  
  `shell
...
resource footprint](https://developer.android.com/topic/performance/memory/manage-app-memory#reduce-footprint), a larger compiled codebase directly
