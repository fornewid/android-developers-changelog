...
[App Links](https://developer.android.com/training/app-links) are deep links based on your website URL that are verified to belong to your website. App Link verifications can fail for the following reasons:
...
On rooted devices, consider using a [`lockClocks` script](https://developer.android.com/studio/profile/benchmark#lock-clocks) for Microbenchmarks. Among other things, these scripts do the following:
...
When possible, consider using a testing framework like [Macrobenchmark](https://developer.android.com/studio/profile/macrobenchmark), which can reduce noise in your measurements and prevent measurement inaccuracy.
...
![alt_text](https://developer.android.com/static/topic/performance/images/benchmark_images/trace_startup.png "Trace showing trampoline activity.") **Figure 1.** A trace showing trampoline activity.
...
![alt_text](https://developer.android.com/static/topic/performance/images/benchmark_images/trace_gc.png "Trace showing space between GC events.") **Figure 2.** A trace showing space between GC events.
...
![alt_text](https://developer.android.com/static/topic/performance/images/benchmark_images/measuring-performance-figure-3.png "Trace showing frequent fast frames.") **Figure 3.** A trace showing frequent fast frames.
...
![alt_text](https://developer.android.com/static/topic/performance/images/benchmark_images/measuring-performance-figure-4.png "Trace showing frequent fast frames with periodic bursts of work.") **Figure 4.** A trace showing frequent fast frames with periodic bursts of work.

When you see a disruption to this regular cadence, it is a janky frame, as shown in figures 5 and 6:

![alt_text](https://developer.android.com/static/topic/performance/images/benchmark_images/measuring-performance-figure-5.png "Trace showing janky frames.") **Figure 5.** A trace showing a janky frame.

![alt_text](https://developer.android.com/static/topic/performance/images/benchmark_images/measuring-performance-figure-6.png "Trace showing more janky frames.") **Figure 6.** A trace showing more janky frames.

In some cases, you need to zoom into a tracepoint for more information about which UI components are being [updated](https://developer.android.com/develop/ui/compose/performance/phases) by Compose or, as in Figure 6, what a `LazyColumn` is doing. When diagnosing these UI bottlenecks, standard system tracing may not show which composables are the root cause. In these instances, use [Jetpack Compose composition tracing](https://developer.android.com/develop/ui/compose/tooling/tracing), which surfaces exact composable functions directly within the trace, making it easier to pinpoint unexpected recompositions. Figures 5 and 6 show the results of composition tracing.

For more information about optimizing Compose performance, see [Jetpack Compose Performance](https://developer.android.com/develop/ui/compose/performance). For more information about identifying janky frames and debugging their causes, see [Slow rendering](https://developer.android.com/topic/performance/vitals/render).

### Common lazy layout mistakes

Invalidating the entire backing state of a [lazy layout](https://developer.android.com/develop/ui/compose/lists) unnecessarily can lead to excessive recompositions, long frame rendering times, and jank. To minimize the number of list items that need to update, use [item keys](https://developer.android.com/develop/ui/compose/lists#item-keys) for your items and only mutate the specific state elements that change.

See [Use lazy layout keys](https://developer.android.com/develop/ui/compose/performance/bestpractices#use-lazylist-keys) for ways to avoid costly full-list re-allocations, which cause content to update rather than replacing it entirely.
...
See [App startup time](https://developer.android.com/topic/performance/vitals/launch-time) for an overview of the app startup process, and see the following video for an overview of system tracing and using the Android Studio profiler.
...
We recommend capturing Systraces with the [System Tracing app on the device](https://developer.android.com/topic/performance/tracing/on-device). For Android 10 and higher, use [Perfetto](http://perfetto.dev/docs). For Android 9 and lower, use [Systrace](https://developer.android.com/topic/performance/tracing). We also recommend viewing trace files with the [web-based Perfetto trace viewer](http://ui.perfetto.dev). For more information, see [Overview of system tracing](https://developer.android.com/topic/performance/tracing).
...
We recommend you call [`reportFullyDrawn`](https://developer.android.com/reference/kotlin/android/app/Activity#reportfullydrawn) when startup is completed from the app's perspective for improved app startup metric reporting. See the [Time to full display](https://developer.android.com/topic/performance/vitals/launch-time#time-full) section for more information about using `reportFullyDrawn`. You can extract RFD-defined start times through the Perfetto trace processor, and a user-visible trace event is emitted.

### Use System Tracing on the device

You can use the system-level app called System Tracing to [capture a system trace on a device](https://developer.android.com/topic/performance/tracing/on-device). This app lets you record traces from the device without having to plug it in or connect it to `adb`.

### Use Android Studio Memory Profiler

You can use the [Android Studio Memory Profiler](https://developer.android.com/studio/profile/record-java-kotlin-allocations) to inspect memory pressure that might be caused by memory leaks or bad usage patterns. It provides a live view of object allocations.
...
1. Detect memory problems.
   ...
   ![alt_text](https://developer.android.com/static/topic/performance/images/benchmark_images/gc-increase-object-count.png "Increasing object count.") **Figure 7.** Increasing object count.

   ![alt_text](https://developer.android.com/static/topic/performance/images/benchmark_images/gc-after-increase-object-count.png "Garbage collections.") **Figure 8.** Garbage collections.
...
2. Diagnose memory pressure hot spots.
   ...
   ![alt_text](https://developer.android.com/static/topic/performance/images/benchmark_images/measuring-performance-figure-9.png "Values for Allocations and Shallow Size.") **Figure 9.** Values for **Allocations** and **Shallow Size**.
   ...
   - **Retained Size** : shows the total memory due to the object itself plus any references that are solely referenced by the object. It's useful for tracking memory pressure due to complex objects. To get this value, take a full memory dump, as shown in figure 10. **Retained Size** is added as a column, as shown in figure 11.
     ...
     ![alt_text](https://developer.android.com/static/topic/performance/images/benchmark_images/take-memory-dump.png "Full memory dump.") **Figure 10.** Full memory dump.
