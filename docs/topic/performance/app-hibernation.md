...
2. Reschedule any jobs, alerts, and notifications that were scheduled before your app went into hibernation.
   ...
   To support this workflow more easily, use [WorkManager](https://developer.android.com/topic/libraries/architecture/workmanager). You can also add rescheduling logic in the [`ACTION_BOOT_COMPLETED`](https://developer.android.com/reference/android/content/Intent#ACTION_BOOT_COMPLETED) broadcast receiver, which is invoked when your app leaves hibernation and after the device boots up.
...
When an [activity](https://developer.android.com/guide/components/activities/intro-activities) in your app is resumed, the system considers this event to be a user interaction. Therefore, the system extends the amount of time before your app enters hibernation.
...
- The user interacts with a notification, except for dismissing the [notification](https://developer.android.com/guide/topics/ui/notifiers/notifications).
...
Android grants system-level exemptions from app hibernation in certain use cases. If your app falls into one of the following categories, it is exempt from the [app usage standards](https://developer.android.com/topic/performance/app-hibernation#app-usage) and will not hibernate.
...
> [!NOTE]
> **Note:** If your app is a [device policy controller](https://developer.android.com/work/dpc/build-dpc) that implements [`DeviceAdminService`](https://developer.android.com/reference/android/app/admin/DeviceAdminService), runs in the [device owner or profile owner mode of operation](https://developers.google.com/android/work/play/emm-api/prov-devices#modes_of_operation), or is a [carrier-privileged app](https://source.android.com/devices/tech/config/carrier), the system never places your app into a hibernation state.
...
To check whether the user has already disabled hibernation for your app, use the [`getUnusedAppRestrictionsStatus()`](https://developer.android.com/reference/androidx/core/content/PackageManagerCompat#getUnusedAppRestrictionsStatus(android.content.Context)) API.

For additional details on how to use this API in your app, see the [API code example](https://developer.android.com/topic/performance/app-hibernation#api-code-example) on this page.
...
2. Invoke the [`createManageUnusedAppRestrictionsIntent()`](https://developer.android.com/reference/androidx/core/content/IntentCompat#createManageUnusedAppRestrictionsIntent(android.content.Context,%20java.lang.String)) API, as shown in the [API code example](https://developer.android.com/topic/performance/app-hibernation#api-code-example). This API creates an intent that loads the **App info** screen in Settings. From here, the user can turn off hibernation for your app.
