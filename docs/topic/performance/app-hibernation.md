---
title: App hibernation  |  Android Developers
...
[interact with your app](#app-usage) for a few months, the system places your
...
2. Reschedule any jobs, alerts, and notifications that were scheduled before
   ...
   [WorkManager](/topic/libraries/architecture/workmanager). You can also add
...
When an [activity](/guide/components/activities/intro-activities) in your app is
...
* The user interacts with a notification, except for dismissing the
  [notification](/guide/topics/ui/notifiers/notifications).

  **Note:** If the user dismisses the notification and doesn't otherwise interact
  with the notification, this doesn't count as app usage.
...
* Apps that have a service or content provider bound by another app on the device
  or the OS. For example, Input Method Editors (IMEs) or password managers.
* Broadcast receivers in the package receiving an explicit broadcast from an
  external package.

  **Note:** This excludes usage that would not occur if the app is force stopped. For
  example, running a job does not count as usage even though the job service
...
from the [app usage standards](#app-usage) and will not hibernate.
...
* Provide family safety by periodically reporting the location of family
  members.
...
**Note:** If your app is a [device policy controller](/work/dpc/build-dpc) that
implements
[`DeviceAdminService`](/reference/android/app/admin/DeviceAdminService), runs
in the [device owner or profile owner mode of
operation](https://developers.google.com/android/work/play/emm-api/prov-devices#modes_of_operation),
or is a [carrier-privileged
app](https://source.android.com/devices/tech/config/carrier), the system never
places your app into a hibernation state.
...
use the [`getUnusedAppRestrictionsStatus()`](/reference/androidx/core/content/PackageManagerCompat#getUnusedAppRestrictionsStatus(android.content.Context))
API.

For additional details on how to use this API in your app, see the [API
code example](#api-code-example) on this page.
...
1. Display a UI that explains to the user why they need to disable
   hibernation for your app.
2. Invoke the [`createManageUnusedAppRestrictionsIntent()`](/reference/androidx/core/content/IntentCompat#createManageUnusedAppRestrictionsIntent(android.content.Context,%20java.lang.String))
   API, as shown in the [API code example](#api-code-example).
...
**Note:** This API can be used on a device running any version of Android, but it
provides information for your app only if your app targets
...
**Note:** If you test your app on a device that runs Android 11, you
can skip several steps in this section. These steps are labeled "Android 12 and
higher only".
...
6. *(Android 12 and higher only)* Confirm that the app is hibernated, using one
   ...
   * Observe that the test device now shows a notification, indicating that
     unused apps are hibernated.
