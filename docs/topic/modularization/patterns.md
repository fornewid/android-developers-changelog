...
There is no single [modularization](https://developer.android.com/topic/modularization) strategy that fits all projects. Due to
...
[Video](https://www.youtube.com/watch?v=16SwTvzDO0A)
...
- **Low coupling** means that modules should be as independent as possible from one another, so that changes to one module have zero or minimal impact on other modules. **Modules shouldn't have knowledge of the inner workings of
  other modules**.
...
our [recommended app architecture](https://developer.android.com/topic/architecture).
...
> Delivery](https://developer.android.com/guide/playcore/feature-delivery) describing a module that can be delivered conditionally or downloaded on-demand. However, in the context of this guide a feature module is a module that encapsulates a distinct part of your application's functionality.
...
and state](https://developer.android.com/topic/architecture/ui-layer/stateholders). A single feature doesn't have to be limited to a single view or
...
to a number of different binaries thanks to [build variants](https://developer.android.com/studio/build/build-variants).
...
[Test modules](https://developer.android.com/training/dependency-injection) are Android modules that are used for testing purposes only.
...
checkout feature using the [**Navigation**](https://developer.android.com/guide/navigation/navigation-pass-data) component.

    navController.navigate("checkout/$bookId")

The checkout destination receives a book id as an argument which it uses to
fetch information about the book. You can use the [saved state handle](https://developer.android.com/topic/libraries/architecture/viewmodel-savedstate) to
...
implementation module. The answer is [Dependency Injection](https://developer.android.com/training/dependency-injection). The feature
...
externally, usually in the [app module](https://developer.android.com/topic/modularization/patterns#app-modules).
...
4. **Provide implementation module** : Finally, you need to provide the actual implementation for your dependencies. The specific implementation depends on your project setup, but the [app module](https://developer.android.com/topic/modularization/patterns#app-modules) is usually a good place to do this. To provide the implementation specify it as a [dependency for your selected
   build variant or a testing source set](https://developer.android.com/build/build-variants#dependencies). ![App module provides actual implementation.](https://developer.android.com/static/topic/modularization/images/2_api_impl_app.png) **Figure 12**. App module provides actual implementation.
...
dependencies](https://developer.android.com/studio/build/dependencies#dependency_configurations) in your module, prefer `implementation` over `api`. The latter
